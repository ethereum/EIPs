...
description: Multibyte opcodes for little-endian 64-bit arithmetic, comparison, bitwise, flow and memory operations in EVM.
...
This EIP introduces multibyte opcodes prefixed by `C0` for 64-bit arithmetic (`C001`-`C00B`), comparison (`C010`-`C015`), bitwise (`C016`-`C019`), flow (`C056` and `C057` in "legacy", or `C0E1` and `C0E2` in EOFv1), and memory (`C051` and `C052`) operations.
