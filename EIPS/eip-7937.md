...
This EIP assumes that the majority of EVM implementations target native little-endian 64-bit architectures (like `x86_64`, `arm64` and `riscv64gc`. A 256-bit stack item is stored efficiently internally as 4 items of 64-bit unsigned integer `[u64; 4]`. This EIP works best, in terms of optimizations, for those implementations. The opcodes simply operates on the last `u64`, instead of the whole `[u64; 4]`. There are basically no other changes.

#### `BYTE`

This EIP does not currently contain a 64-bit mode for `BYTE`. The reason for this is mainly endianness. We expect the interpreter to store optimized 64-bit stack items in its native endian (which likely will be litte endian). Therefore, any opcodes that deal with endianness will have the interpreter exit 64-bit mode anyway. Because `BYTE` depends on big endian, it was under this reason that it was not added.

However, as @jochem-brouwer pointed out, `BYTE` can be defined in an endian-independent way `(x >> (248 - i * 8)) & 0xFF` so it may be possible to define a 64-bit mode for `BYTE`.

#### `MLOAD`, `MSTORE` and `MSTORE8`

This EIP does not define a 64-bit mode for memory operations `MLOAD`, `MSTORE` and `MSTORE8`. For certain complex algorithms, those may be needed to stay fully in 64-bit mode. The main reason at this point is that those opcodes have endianness. In addition, the cycle costs for memory operation is small (so that the extra 192-bit loading/unloading matters little in terms of performance). With the design of prefix (mode) opcodes, it can, however, be envisioned that 64-bit memory operations can be easily added in the future.
...
* `PUSH0`-`PUSH8` is "automatically 64-bit" because the value it pushes only set the least significant 64 bits.
