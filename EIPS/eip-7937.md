...
* This EIP does not define a 64-bit mode for memory operations `MLOAD`, `MSTORE` and `MSTORE8`. For certain complex algorithms, those may be needed to stay fully in 64-bit mode. The main reason at this point is that those opcodes have endianness. In addition, the cycle costs for memory operation is small (so that the extra 192-bit loading/unloading matters little in terms of performance).
