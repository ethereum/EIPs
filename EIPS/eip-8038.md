...
|:---:|:---:|:---:|:---:|:---:|:---:|
| `COLD_ACCOUNT_ACCESS` | Cold touch of an account | 2,600 | TBD | TBD | `*CALL` opcodes, `BALANCE`, `SELFDESTRUCT` and `EXT*` opcodes |
| `COLD_ACCOUNT_WRITE` | Surcharge for when writing to an account changes one account leaf value for the first time | 6,700<sup>1</sup> | TBD | TBD | `*CALL` opcodes, `BALANCE`, `SELFDESTRUCT` and `EXT*` opcodes |
| `COLD_STORAGE_ACCESS` | Cold touch of a storage slot | 2,100 | TBD | TBD | `SSTORE` and `SLOAD` |
| `COLD_STORAGE_WRITE` | Cold write to a storage slot | 5,000 | TBD | TBD | `SSTORE` |
...
Case 1: Cold (i.e., first access)

- Value is updated (e.g., 1 → 2): `COLD_STORAGE_WRITE`
- Value is not updated (e.g., 1 → 1): `COLD_STORAGE_ACCESS` + `WARM_ACCESS`

Case 2: Warm (there were already previous accesses)

- Value is updated (e.g., 1 → 2): `COLD_STORAGE_WRITE` - `COLD_STORAGE_ACCESS`
- Value is not updated or was updated before: `WARM_ACCESS`
...
|:---:|:---:|:---:|:---:|
| `test_account_access` | CacheStrategy = NO_CACHE | runtime = intercept + slope × operation_count + update × value_sent x operation_count | `slope` → `COLD_ACCOUNT_ACCESS`; `update` → `COLD_ACCOUNT_WRITE` |
| `test_sload_bloated` | CacheStrategy = NO_CACHE | runtime = intercept + slope × operation_count | `slope` → `COLD_STORAGE_ACCESS` |
| `test_sstore_bloated` | CacheStrategy = NO_CACHE; write_new_value = False  | runtime = intercept + slope × operation_count | `slope` → `COLD_STORAGE_ACCESS` |
| `test_sstore_bloated` | CacheStrategy = NO_CACHE; write_new_value = True  | runtime = intercept + slope × operation_count | `slope` → `COLD_STORAGE_WRITE` |
