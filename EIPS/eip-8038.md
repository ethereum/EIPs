...
`CREATE` and`CREATE2` operation don't have explicit warm/cold pricing. Instead, they are charged `CREATE_ACCESS` in regular-gas and `STATE_BYTES_PER_NEW_ACCOUNT x CPSB` in state-gas.

`CREATE_ACCESS` is defined as `ACCOUNT_WRITE` + `COLD_STORAGE_ACCESS`.
