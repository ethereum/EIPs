...
| 0 | x | y | Warm | Modified slot, zero at transaction start | `WARM_ACCESS` charged | no state-gas adjustments |
| x | x | 0 | Cold | Cleared slot, non-zero at transaction start | `COLD_STORAGE_ACCESS` charged, `STORAGE_CLEAR_REFUND` refunded | no state-gas adjustments |
...
| x | y | 0 | Warm | Cleared slot, modified earlier in transaction | `WARM_ACCESS` charged, `STORAGE_CLEAR_REFUND` refunded | no state-gas adjustments |
| x | y | x | Warm | Existing slot, value reset | `WARM_ACCESS` charged, `STORAGE_WRITE` refunded | no state-gas adjustments |
| 0 | 0 | 0 | Cold | No-op write of zero on empty slot, cold first access | `COLD_STORAGE_ACCESS` charged | no state-gas adjustments |
| x | x | x | Cold | No-op write, cold first access, unchanged non-zero slot | `COLD_STORAGE_ACCESS` charged | no state-gas adjustments |
| - | 0 | 0 | Warm | No-op write of zero (current value is zero) | `WARM_ACCESS` charged | no state-gas adjustments |
| - | x | x | Warm | No-op write, current value equals new non-zero value | `WARM_ACCESS` charged | no state-gas adjustments |

In the no-op rows above, `-` denotes any value (zero or non-zero).
