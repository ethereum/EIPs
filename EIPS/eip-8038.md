...
Refunds eligible to accumulate in this counter exclude `STORAGE_CLEAR_REFUND`; such refunds continue to be credited to the transaction refund counter and remain subject to the existing 20% cap.
...
Additionally, any refund associated with state access or state update operations MUST be credited to `state_access_gas_reservoir`.
