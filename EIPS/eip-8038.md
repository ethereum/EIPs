...
|:---:|:---:|:---:|:---:|:---:|:---:|
| `COLD_ACCOUNT_ACCESS` | Cold touch of an account | 2,600 | TBD | TBD | `*CALL` opcodes, `BALANCE`, `SELFDESTRUCT`, `EXT*` opcodes and ETH transfers |
| `ACCOUNT_WRITE` | Surcharge for when writing to an account changes one account leaf value for the first time | 6,700<sup>1</sup> | TBD | TBD | `*CALL` opcodes, `SELFDESTRUCT` and ETH transfers |
...
For `CALL` and `CALLCODE` operations, `CALL_VALUE` is set to `ACCOUNT_WRITE + CALL_STIPEND`.
