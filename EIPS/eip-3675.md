...
* Block validity
	* Begginning with `TRANSITION_BLOCK`, block is invalidated if any of the following is true:
		* `ommersHash != Keccak256(RLP([]))`
		* `difficulty != 0`
		* `nonce != 0x0000000000000000`
		* `len(extraData) > MAX_EXTRA_DATA_BYTES`
  * Begginning with `TRANSITION_BLOCK`, block rewards aren't added to `beneficiary` account
