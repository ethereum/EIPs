...
1. Find the `contentcontract` text record on ENS resolver on chain **chainid**.  Return error if the chain does not have ENS or the record is an invalid ETH address.
2. If the `contentcontract` text record does not exist, the protocol will use the ETH address of **name**.

Note that `contentcontract` text record may return an Ethereum address or an [ERC-3770](./eip-3770.md) chain-specific address.  If the address is an ERC-3770 chain-specific address, then the **chainid** to call the message will be overridden by the **chainid** specified by the ERC-3770 address.
