...
For example, an airline company issues a series of ERC721/1155 tokens to Crypto Punk holders to give them privileges, in order to attract them to join their club. However, since these tokens are not bound to the original NFT, if the original NFT is transferred, these privileges remain in the hands of the original holders, and the new holders cannot enjoy the privileges automatically.
...
///  Note: the ERC-165 identifier for this interface is 0x076e1bbb
...
If the owner of this standard NFT transfer ownership to another user, there is no impact on "privileges". But errors may be occurred when the owner tries to withdraw the original EIP-721 token from EIP-5496 via calling function `unwrap()` in our implementation contract if any available privilege is still ongoing. We protect the rights of holders of the privileges by checking the last expire date of the privilege.
