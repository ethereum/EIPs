...
A diamond stores within it a mapping of function selectors to facet addresses.

When an external function is called on a diamond it finds which facet has that function and executes it using `delegatecall`. 

`delegatecall` enables a diamond to retrieve a facet's external function and execute it as its own external function. The `msg.sender` and `msg.value` values do not change. Only the diamond's contract storage is read and written to.

A diamond can use a `diamondCut` function to add/replace/remove any number of functions from any number of facets in a single transaction. `diamondCut` updates the mapping of functions selectors to facet addresses. Other such functions can be used.

An event is emitted any time external functions are added, replaced or removed to report what changed.

A diamond has four functions that can be called to show what facet addresses and function selectors it currently uses.

## Upgrades and Immutability

A diamond can be made immutable by not adding an external function to it that can add/replace/remove functions or by removing such functions. 
...
This standard is designed to make diamonds work well with user-interface software. Function selectors with the ABI of a contract provide enough information about functions to be useful for user-interface software. Instead of function selectors and a contract ABI, function signatures could be used but they do not provide information about their return values or if they are read-only or not. Function selectors are used because they are more gas efficient to store and contract ABIs need to be used anyway. 
...
### Immutable Diamond
It is possible to make an upgradeable diamond immutable. This is done by calling the `diamondCut` function to remove the `diamondCut` function. With this gone it is no longer possible to add, replace or remove functions.
...
1. Copy an internal function code in one facet to the other facet.
