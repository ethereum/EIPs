...
A diamond provides:
1. A way to create a set of contracts (facets) under a single address that are independent from each other but can share functionality and contract storage.
2. A way to add, replace and remove multiple external functions atomically (in the same transaction).
3. An event that shows what functions are added, replaced and removed.
4. A way to look at a diamond to see its facets and functions.
5. Solves the 24KB maximum contract size limitation. Diamonds can be any size.
6. Enables zero, partial or full diamond immutability as desired, and when desired.
7. Designed for tooling and user-interface software.
