...
`delegatecall` enables a diamond to retrieve a facet's external function and execute it as its own external function. The `msg.sender` and `msg.value` values do not change. Only the diamond's contract storage is read or written to.
