...
1. A `Diamond` is a contract that uses functions from its `Facet` to execute function calls. A `Diamond` NAY have any number of `Facet`s.
2. A `Facet` is a contract containing the implementation code of one or more functions of a `Diamond`.
3. The **loupe functions** are functions of a `Diamond` that MAY be used to inspect the implementation code of that diamond.
4. An **immutable function** is a function that is defined in a `Diamond`'s contract, or whose corresponding `Facet` cannot be changed.
