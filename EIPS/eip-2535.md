...
A diamond is a contract with the functionality of multiple contracts that can share internal functions, libraries and state variables.
...
contract OurFacet is DiamondStorageContract {
  ...
  function registerInterface(bytes4 _interfaceId) external {
    ...
    require(ds.owner == msg.sender, "OurFacet: Must be owner.");
...
The diagram below shows two different diamonds using the same two facets.
