...
2. An event that shows what functions are added, replaced, removed and one or more messages describing changes.
...
Immutable, trustless contracts cannot be improved, resulting in increasingly inferior contracts over time. Contracts rot.
...
1. If you will exceed the max size of a contract and you have related functionality that needs to access and use the same storage variables then make a diamond.
...
```
<img align="right" src="../assets/eip-2535/magnifying-glass.svg" width="100" height="100">
...
Consensus functionality could be implemented such as an approval function that multiple different people call to approve changes before they are executed with the `cut` function.  These are just examples.
