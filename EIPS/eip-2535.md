...
### Diamond Interface
The `diamondCut` function is marked `external` in the interface below but it can be a public function when implemented.
...
contract DiamondExample is Storage {
  ...
  constructor() public {
    // $ is used here to distinguish internal storage variables from local variables
