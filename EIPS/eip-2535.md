...
When an external function is called on a diamond its fallback function is exectued. The fallback function finds in the `selectorToFacet` mapping which facet has the function that has been called and then executes the function from the facet using `delegatecall`. 
...
A diamond has four external functions that can be called to show what facets and functions it currently has.
