...
EOF container is a binary format with capability of providing EOF version number and a list of EOF sections.
...
The EOF header is followed by at least one section header. Each section header contains `section_kind` and `section_size`.
| description  | length  | value         | |
|--------------|---------|---------------|-|
...
The list of section headers is terminated with the _terminator byte_ `0x00`.

#### Validation rules

1. `version` must not be `0`.
2. There MUST be at least one section (and therefore section header).
3. `section_kind` MUST NOT be `0`. The value `0` is reserved for _terminator byte_.
4. `section_size` MUST NOT be `0`. If a section is empty its section header must be omitted.
...
|--------------|------------|
| 0            | _reserved for terminator byte_ |
| 1            | code       |
| 2            | data       |


#### EOF version 1 validation
