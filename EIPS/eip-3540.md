...
EOF-formatted contracts are created using new instructions `EOFCREATE`, `TXCREATE` and `RETURNCONTRACT`. They are introduced in a separate [EIP-7620](./eip-7620.md).
...
Code validation is performed during `TXCREATE` instruction, and is elaborated on in [EIP-3670](./eip-3670.md) and the contract creation [EIP-7620](./eip-7620.md).
...
Moved to the section [Lack of `EXTDATACOPY` in EIP-7480](./eip-7480.md#lack-of-extdatacopy).
