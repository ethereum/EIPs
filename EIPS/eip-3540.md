...
- `CODESIZE`, `CODECOPY`, `EXTCODECOPY`, `EXTCODEHASH`, `GAS` are rejected by validation in EOF contracts, with no replacements
...
For either an EOF or legacy contract:
...
### Banning `EXTCODESIZE` in EOF contracts

In line with the notion of removing code observability, `EXTCODESIZE` has been initially planned to be banned in EOF bytecode. However, there exists a use case, where a smart contract calling other smart contract checks for a call target being an EOA or not, which has been deemed important enough to keep the opcode.

Observing size of code does not allow the caller to observe the code itself. Also for EOF targets, not even the size of code is observed, just its existence. Other alternatives considered were: swap `EXTCODESIZE` for a new `IS_CONTRACT` opcode, introduce rich return values from call instructions, introduce a new `CALL_STATUS` opcode, introduce behavior-cusomizing input flags for call instructions.
