...
1. If code starts with `0xEF 0x00 0x01 0x01 codelen1 codelen2 0x02 datalen1 datalen2 0x00`, then calculate `total_size = (10 + (codelen1 << 8 | codelen2) + (datalen1 << 8 | datalen2))`. If `total_size == container_size` then it is a valid EOF1 code with a code and data section.
2. If code starts with `0xEF 0x00 0x01 0x01 codelen1 codelen2 0x00`, then calculate `total_size = 7 + (codelen1 << 8 | codelen2)`. If `total_size == container_size` then it is a valid EOF1 code with a code section only.
