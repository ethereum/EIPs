...
Every contract implementing this EIP MUST implement the `EIP-4824` interface below:
...
contract EIP-4824Registration is IEIP-4824, AccessControl {
    ...
    function initialize(
        ...
        address _EIP-4824Index
    ) external {
        initialize(_daoAddress, daoURI_, _EIP-4824Index);
    ...
    function initialize(
        ...
        EIP-4824Index(_EIP-4824Index).logRegistration(address(this));
    }
    ...
    function supportsInterface(
        ...
        return
            interfaceId == type(IEIP-4824).interfaceId ||
...
### Indexing
If a DAO inherits the EIP-4824 interface from a 4824-compliant DAO factory, then the DAO factory SHOULD incorporate a call to an indexer contract as part of the DAO's initialization to enable efficient network indexing. If the DAO is [EIP-165](./eip-165)-compliant, the factory can do this without additional permissions. If the DAO is _not_ compliant with EIP-165, the factory SHOULD first obtain access control rights to the indexer contract and then call logRegistration directly with the address of the new DAO and the daoURI of the new DAO. Note that any user, including the DAO itself, MAY call logRegistration and submit a registration for a DAO which inherits the EIP-4824 interface and which is also EIP-165-compliant.

```solidity
pragma solidity ^0.8.1;

error EIP-4824InterfaceNotSupported();

contract EIP-4824Index is AccessControl {
    ...
    function logRegistration(address daoAddress) external {
        if (!daoAddress.supportsInterface(type(IEIP-4824).interfaceId))
            revert EIP-4824InterfaceNotSupported();
        emit DAOURIRegistered(daoAddress);
    }
}
```
...
contract EIP-4824RegistrationSummoner {
    ...
    address public EIP-4824Index;
    address public template; /*Template contract to clone*/

    constructor(address _template, address _EIP-4824Index) {
        template = _template;
        EIP-4824Index = _EIP-4824Index;
    }
    ...
    function summonRegistration(
        ...
        if (manager == address(0)) {
            EIP-4824Registration(registration).initialize(
                msg.sender,
                daoURI_,
                EIP-4824Index
            );
...
Further, every contractsURI SHOULD include at least the contract inheriting the EIP-4824 interface.
...
```
{
    ...
    "contracts": [
        ...
        }
        {
            ...
            "description": "<description, e.g. EIP-4824 registration contract>"
        }
    ]
}
```
