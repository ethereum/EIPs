...
* `BLOCK_GAS_TARGET` is set a `PARENT_GAS_TARGET * (1 + 1//1024)` or `PARENT_GAS_TARGET * (1 - 1//1024)`, depending on miner decision.
* At `block.number == FORK_BLOCK_NUMBER` we set `BASE_FEE = INITIAL_BASE_FEE`
* `BASE_FEE` is set, from `FORK_BLOCK_NUMBER + 1`, as follows
  * Let `DELTA = PARENT_GAS_USED - PARENT_GAS_TARGET` (possibly negative).
  * Set `BASE_FEE = PARENT_BASE_FEE * (1 + DELTA // PARENT_GAS_TARGET // BASE_FEE_MAX_CHANGE_DENOMINATOR)`
