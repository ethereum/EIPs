...
To formally propose the inclusion of an EIP in a network upgrade, someone **MUST** open a pull request against the Upgrade Meta EIP to add the EIP in the `Proposed for Inclusion` section. Reasonable pull requests **SHOULD** be merged in a timely fashion by the Upgrade Meta EIP author.  
...
`Scheduled for Inclusion` signals that implementation and testing work are underway, and that, assuming no issues arise as part of this process, the EIP **SHOULD** be included in the network upgrade. This implies that the latest Upgrade Devnet should contain all `Scheduled for Inclusion` Core EIPs. 
...
Once a network upgrade has been activated, all Core EIPs that were part of the upgrade and all non-Core EIPS that were `Scheduled for Inclusion` and activated prior to the upgrade **MUST** be moved to `Included` in the Upgrade Meta EIP, and the `Proposed for Inclusion`, `Considered for Inclusion`, `Declined for Inclusion` and `Scheduled for Inclusion` lists **MUST** be removed from the Meta EIP. 
