...
When the Upgrade Meta EIP is moved to `Review`, the `Declined for Inclusion` list **SHOULD** be removed. When it is moved to `Last Call`, the `Proposed for Inclusion` and `Considered for Inclusion` lists **SHOULD** be removed, leaving only the `Scheduled for Inclusion` list.
