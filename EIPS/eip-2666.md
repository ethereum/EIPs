...
If `block_number >= X` set the following prices to precompiles and Keccak256 opcode:
  ...
  - Keccak256 built-in function (opcode `SHA3 (0x20)`) should be priced as `13 + (len(input)/136 + 1)*15`

This EIP *ideally* requires that `MODEXP` repricing is [implemented](https://eips.ethereum.org/EIPS/eip-2565) to also accurately reflect that there is implicit compensation for an old `STATICCALL (0xfa)` cost (pre-2046).
