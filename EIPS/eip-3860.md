...
1. If length of transaction data (`initcode`) in a create transaction (recipient T~t~ being empty) exceeds `MAX_INITCODE_SIZE`, execution results in an exceptional abort. Such a transaction is valid and may be included in a block.
