...
Test vectors are not yet included in this draft. On progressing toward implementation, test cases would be added to the `epoch_processing` and `block_processing` test suites in the consensus specifications repository, following the existing formats for `process_rewards_and_penalties` and `process_sync_aggregate`, covering: active balance at the `get_total_balance` floor (no active validators, where the burn must round to zero rather than fault), active balance strictly between that floor and `SATURATION_BALANCE` with the resulting burn fraction checked against $(D/D_\text{sat})^{3/2}$, active balance at and above `SATURATION_BALANCE`, the boundary behaviour of `get_issuance_burn` at `sqrt_active == sqrt_sat`, the suspension of the attestation pass alone while `is_in_inactivity_leak(state)` holds (and its resumption on the first finalising epoch thereafter), `get_participating_increments` at full and at partial participation, and with one and with all three flag sets empty (each empty set must contribute zero, not the `get_total_balance` floor of one increment — a case distinguishable from the correct result only at a small total active balance, so the vector must pin one), the attestation basis matching a perfect attester's reward from `get_flag_index_deltas` to the Gwei at participation levels chosen so the three flags round differently (a basis that pre-sums the weighted participation before flooring exceeds the reward by 1 to 2 Gwei in most epochs, and must fail), the equivalence of the attestation and proposer bases with their unscaled forms at full participation, the net balance change of a perfectly performing validator at `sqrt_active == sqrt_sat` being exactly zero for each of the three duties at both full and partial participation, the proposer basis covering both the attestation-driven and the sync-driven components of proposer issuance (a vector with `SYNC_REWARD_WEIGHT` altered must change the basis, catching any reliance on the current weights summing as they do), the proposer basis error at equal and at divergent attestation and sync participation, the proposer basis reading current-epoch participation while the attestation basis reads the previous epoch's (a vector in which the two differ must move the two burns independently, and a participation step across the epoch boundary must charge each epoch's proposers on its own conditions rather than the preceding epoch's), the sign and magnitude of the aggregate residual under partial participation, the sync committee deduction falling on participating and non-participating members alike, the epoch alignment of the attestation pass across the activation and exit boundaries (a validator whose `activation_epoch` equals the current epoch is charged nothing, a validator whose `exit_epoch` equals the current epoch is charged in full, and the charged set matches the set paid by `process_rewards_and_penalties` in the same `process_epoch` call), and the value of `get_base_reward_factor` at `TRANSITION_START_EPOCH` (exactly `TRANSITION_BASE_REWARD_FACTOR`), one epoch later (still `TRANSITION_BASE_REWARD_FACTOR`, which truncation would have reduced immediately), at the midpoint, one epoch before `TRANSITION_START_EPOCH + TRANSITION_DURATION_EPOCHS`, at that epoch itself and beyond it (`BASE_REWARD_FACTOR` in each case), and across a step boundary in both directions, with the returned value never exceeding `TRANSITION_BASE_REWARD_FACTOR` nor falling below `BASE_REWARD_FACTOR` at any epoch in the range.
