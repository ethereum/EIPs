...
The main reason for not adding these options was because implementing
...
Execution requests can be created via smart contracts, allowing for decentralized
...
credential of the validator must be the same as the `msg.sender` when processing
...
The problem for existing validators that have their withdrawal credentials
set to an EOA account is that they will never be able to use smart contracts
for creating execution requests, because the current design does not allow for these
credentials to ever be changed.

Allowing validators to update their withdrawal credentials means they can opt-in
...
At the end of processing any execution block where `block.timestamp >= FORK_TIMESTAMP` (i.e. after processing all transactions and after performing the block body requests validations) client software **MUST** call the contract as `SYSTEM_ADDRESS` with empty input data to trigger the system subroutine execution. The response should be treated as a new request type (0x03) according to [EIP-7685](./eip-7685.md).
...
validating the execution block (including any requests of the newly defined type 0x03).
...
class WithdrawalCredentialUpdateRequest(Container):
    ...
    old_address: ExecutionAddress  # request contract will set this to msg.sender
