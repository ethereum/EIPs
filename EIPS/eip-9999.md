...
The renewal-age tier determines the gas cost for mutations of state that already exists in the trie. Read operations are unaffected, and state-creating operations are not tiered (see below).

If an item is in the Inactive tier at the start of a mutating operation, the operation is charged its normal gas plus the corresponding inactive surcharge. Active mutations are charged their normal gas only, exactly as today.
...
#### Tier evaluated before the write

The tier MUST be determined from `last_written_block` as it stands at the start of the operation, before [EIP-8188](./eip-8188.md) sets the item's `last_written_block` to the current block number. Writing to Inactive state therefore pays the surcharge even though the same write moves the item into the Active tier for subsequent operations. When `period_age == 0`, the item is already Active and pays no surcharge.
...
#### Gas availability before loading Inactive state

Before executing a state mutation that may target an Inactive item, the current call frame MUST have enough gas to pay the normal cost of the operation plus the maximum applicable inactive surcharge. If it does not, the operation MUST fail with out-of-gas before the client is required to load the target from an inactive tier.

This is a change to state-access ordering. Like the cold-access pre-charge of [EIP-2929](./eip-2929.md), the gas check happens before the load rather than after it. It guarantees that pulling cold state back onto the hot path is always paid for by the transaction that triggers it, so an attacker cannot force nodes to perform uncompensated loads from the inactive tier with writes that would run out of gas anyway. A client MAY use a cheap index over the inactive tier to make the check without first performing the load.

#### State creation is not tiered

The Active and Inactive tiers apply only to mutations of existing state. State *creation* is never classified as Inactive and never pays the Inactive surcharge.
