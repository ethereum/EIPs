...
- If the node does not have the BAL for a requested block hash, it **must** return an empty entry (RLP-encoded empty list `0xc0`) at that position.
...
Response to `GetBlockAccessLists`. Each element corresponds positionally to a block hash from the request. Empty BALs (`0xc0`) are returned for blocks where the BAL is unavailable.
