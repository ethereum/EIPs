...
2. Pick a pivot block P near the head.
3. Bulk download state at P via `GetAccountRange`, `GetStorageRanges`, `GetByteCodes`.
4. While downloading, the chain advances to P+K. Fetch BALs for blocks P+1 through P+K via `GetBlockAccessLists`. Apply state diffs sequentially, verifying each BAL against its header hash.
5. Recompute and verify the state root against the latest header.

If the chain advances during step 4, repeat for the newly produced blocks.

Notably, if block P and block P+K are on different sides of a chain fork, the state differences between them cannot be healed by applying the BALs. In this case, the existing state must be fully purged and state synchronization must restart from block P+K. It is recommended to select a pivot block with at least 64 confirmations, as such blocks are very unlikely to be reorganized.
