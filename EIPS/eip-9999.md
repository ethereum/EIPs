...
author: Danny Ryan (@djrtwo), Mikhail Kalinin (@mkalinin), Ansgar Dietrichs (@adietrichs)
discussions-to: https://test.com
status: Draft
...
As currently specified, only the active key can initiate a validator exit. This means that in any non-standard custody relationships (i.e. active key is separate entity from withdrawal credentials), that the ultimate owner of the funds -- the possessor of the withdrawal credentials -- cannot independently choose to exit and begin the withdrawal process. This leads to either trust issues (e.g. ETH can be "held hostage" by the active key owner) or insufficient work-arounds such as pre-signed exits.
...
| Name | Value | Comment |
| - | - | - |
| `EXIT_PRECOMPILE_ADDRESS` | *TBD* | Where to call and store relevant details about exit mechanism |
...
`CALL`s to `EXIT_PRECOMPILE_ADDRESS` perform the following:
...
* Insert an exit into the queue for the source address and validator index (`insert_exit_to_queue()`)
...
def trigger_exit(uint64 validator_index):
    check_exit_fee(msg.value)
    increment_exit_count()
    insert_exit_to_queue(msg.sender, validator_index)
...
def insert_exit_to_queue(address source_address, unit64 validator_index):
    queue_tail_index = sload(EXIT_PRECOMPILE_ADDRESS, EXIT_MESSAGE_QUEUE_TAIL_STORAGE_SLOT)
    queue_storage_slot = EXIT_MESSAGE_QUEUE_STORAGE_OFFSET + queue_tail_index * 2
    sstore(EXIT_PRECOMPILE_ADDRESS, queue_storage_slot, source_address)
    sstore(EXIT_PRECOMPILE_ADDRESS, queue_storage_slot + 1, validator_index)
    sstore(EXIT_PRECOMPILE_ADDRESS, EXIT_MESSAGE_QUEUE_TAIL_STORAGE_SLOT, queue_tail_index + 1)

def increment_exit_count():
    exit_count = sload(EXIT_PRECOMPILE_ADDRESS, EXIT_COUNT_STORAGE_SLOT)
    sstore(EXIT_PRECOMPILE_ADDRESS, EXIT_COUNT_STORAGE_SLOT, exit_count + 1)
...
def get_exit_fee() -> int:
    excess_exits = sload(EXIT_PRECOMPILE_ADDRESS, EXCESS_EXITS_STORAGE_SLOT)
...
At the end of processing any execution block where `block.timestamp >= FORK_TIMESTAMP` (i.e. after processing all transactions):
...
def update_exit_queue():
    queue_head_index = sload(EXIT_PRECOMPILE_ADDRESS, EXIT_MESSAGE_QUEUE_HEAD_STORAGE_SLOT)
    queue_tail_index = sload(EXIT_PRECOMPILE_ADDRESS, EXIT_MESSAGE_QUEUE_TAIL_STORAGE_SLOT)
    ...
    if new_queue_head_index == queue_tail_index:
        ...
        sstore(EXIT_PRECOMPILE_ADDRESS, EXIT_MESSAGE_QUEUE_HEAD_STORAGE_SLOT, 0)
        sstore(EXIT_PRECOMPILE_ADDRESS, EXIT_MESSAGE_QUEUE_TAIL_STORAGE_SLOT, 0)
    else:
        sstore(EXIT_PRECOMPILE_ADDRESS, EXIT_MESSAGE_QUEUE_HEAD_STORAGE_SLOT, new_queue_head_index)

def update_excess_exits():
    previous_excess_exits = sload(EXIT_PRECOMPILE_ADDRESS, EXCESS_EXITS_STORAGE_SLOT)
    exit_count = sload(EXIT_PRECOMPILE_ADDRESS, EXIT_COUNT_STORAGE_SLOT)
    ...
    sstore(EXIT_PRECOMPILE_ADDRESS, EXCESS_EXITS_STORAGE_SLOT, new_excess_exits)
    
def reset_exit_count():
    sstore(EXIT_PRECOMPILE_ADDRESS, EXIT_COUNT_STORAGE_SLOT, 0)
```
...
2. The list of exit operations contained in the block body **MUST** be equivalent to list of exits at the head of the exit precompile's exit message queue up to the maximum of `MAX_EXITS_PER_BLOCK`, respecting the order in the queue. To illustrate:
...
for i in range(num_exits_to_dequeue):
    ...
    source_address = SLOAD(EXIT_PRECOMPILE_ADDRESS, queue_storage_slot)
    validator_index = SLOAD(EXIT_PRECOMPILE_ADDRESS, queue_storage_slot + 1)
    exit = Exit(
...
Alternative design (2) has two main drawbacks. The first is that with the message queue contains an unbounded amount of state (as opposed to simple the `base_fee` in the similar 1559 design). Additionally, even if the state was constrained to a single variable or two, this design pattern reinforces that the Ethereum state transition function signature be more than `f(pre_state, block) -> post_state` by putting another dependency on the `pre_block_header`. These additional dependencies hinder the elegance of future stateless designs. Providing these dependencies within the EVM state as specified, allows for them to show up naturally in block witnesses.

### `validator_index` field

Multiple validators can utilize the same execution layer withdrawal credential, thus the `validator_index` field is required to disambiguate which validator is being exited.
...
Method (a) (not used in this EIP) would require EIP-4788 (the `BEACON_ROOT` opcode) against which to prove withdrawal credentials in relation to validator indices as well as a bitfield tracking mechanism per-unit-time (e.g. 4 months) to ensure that a validator cannot grief the mechanism by submitting many exits. The downsides of this method are that it requires another cross-layer EIP and that it is of higher cross-layer complexity (e.g. care that might need to be taken in future upgrades if, for example, the shape of the merkle tree of `BEACON_ROOT` changes, then the exit precompile and proof structure might need to be updated).

Method (b) has been utilized in this EIP to eliminate additional EIP requirements and to reduce cross-layer complexity to allow for correctness of this EIP (now and in the future) to be easier to analyze. The 1559-style mechanism with a dynamically adjusting fee mechanism allows for users to pay `MIN_EXIT_FEE` for exits in the normal case (fewer than 2 per block on average), but scales the fee up exponentially in response to high usage (i.e. potential abuse).
