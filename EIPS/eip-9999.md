...
Each constant is the write cost for that operation in that tier. It replaces the flat per-write cost for mutations of existing state rather than being charged on top of it. `ACTIVE_*` SHOULD be set close to the current flat write cost so that mutating recently written state is not materially repriced, while `INACTIVE_*` carries the surcharge.
...
The tier MUST be determined from `last_written_block` as it stands at the start of the operation, before [EIP-8188](./eip-8188.md) sets the item's `last_written_block` to the current block number. Writing to Inactive state therefore pays the Inactive write cost even though the same write moves the item into the Active tier for subsequent operations. When `period_age == 0`, the item is already Active and pays the Active cost.
...
Wallets and RPC providers MUST update `eth_estimateGas` to reflect tier-based write costs. Estimators must determine each written item's renewal-age tier from its `last_written_block` and apply the corresponding write gas constant.
