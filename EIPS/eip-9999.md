...
discussions-to: https://ethereum-magicians.org/t/eip-0000-static-relative-jumps-and-calls/0000
status: Draft
...
A version with an 8-bit immediate would only allow moving `PC` backward by 126 or forward by 127 bytes. While that seems to be a good enough distance for many for-loops, it is likely not good enough for cross-function jumps, and since the 16-bit immediate is the same size as what a dynamic jump would take in such cases (3 bytes: `JUMP PUSH1 n`), we think having less instructions is better.
