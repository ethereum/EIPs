...
category: Core
created: 2025-12-01
requires: 4844, 7732, 7928
---

## Abstract

This EIP introduces Payload Chunking, a protocol upgrade that restructures Ethereum blocks into self-contained execution chunks with separated Chunk Access Lists (CALs). Proposer propagates beacon and execution block headers, followed by CALs and Chunks. CALs propagate separately and contain state diffs that enable streaming chunk execution. Chunks are executed and verified separately (as long as CALs `0..N` are available for chunk `N`). After all chunks execute, the CL validates and finalizes the block. This architecture transforms block processing from a monolithic operation into a streaming pipeline while preserving block atomicity.
...
MAX_CHUNKS_PER_BLOCK = 256  # Maximum chunks in a block (allowing 4G gas blocks)
MAX_TRANSACTIONS_PER_CHUNK = 2**10 # Maximum number of transactions in a chunk
```
...
class ExecutionChunkHeader(Container):
    slot: Slot                       # The beacon chain slot
    index: ChunkIndex                # Position in block (0 to MAX_CHUNKS_PER_BLOCK-1)
    is_last: bool                    # Whether chunk is the last chunk in block
...
CALs are incremental. This means that changes and storage reads from earlier chunks are not included in CALs of later chunks.
...
def on_chunk_access_list_received(store, cal_sidecar):
    ...
    execution_engine.new_chunk_access_list(block.slot, chunk_index, cal_sidecar.chunk_access_list)
...
def on_chunk_received(store, chunk_sidecar):
    ...
    # Get or wait priot CALs
    prior_cals = []
    for chunk_index in range(chunk_header.index):
        prior_cals.append(wait_cal(root, chunk_index))

    # Get or wait chunk's CAL
    cal = wait_cal(root, chunk_header.index)

    # Validate chunk
    execution_status = execution_engine.validate_chunk(block, chunk, cal, prior_cals)
...
def maybe_finalize_block(store, root):
    ...
    if last_chunk_index not in chunk_headers or 
        # The chunk with last_chunk_index was not validated
        return
    if not chunk_headers[last_chunk_index].is_last:
        # The chunk with last_chunk_index is not last chunk in a block
        return

    # Verify that all chunks are valid
    for chunk_index in range(chunk_count):
        _chunk_header, status = chunks[chunk_index]
...
In the context of EIP-7732 (ePBS), the PTC (Payload Timeliness Committee) validator should attest whether all chunks and CALs are available in the timely manner.
...
```python
# CL sends CAL for EL to store for the future
...
1. **Parallel Execution**: Multiple chunks can execute simultaneously once their CALs are available
    - This could be achieved without cascade approach, but CALs would need to contain relevant state diffs from previous chunks. This would lead to very big chunks in the worse case scenario.
