...
EIP-8141 enforces per-sender serialization through a linear account nonce. For transactions intended to be short-lived (privacy spends, atomic intents, time-boxed sponsorships) this is unnecessarily restrictive: such transactions either succeed within seconds or become irrelevant, so the only replay risk worth defending against is rebroadcast inside that window. A linear nonce forces the sender to commit to an ordering, prevents multiple pending transactions, and ties inclusion order to nonce order.
...
`EXPIRING_NONCE_GAS` at 13,000 covers the access and mutation work only: two cold `SLOADs` (replay check on slot_seen(h) and ring-head read on slot_ring_ptr), one warm `SLOAD` (eviction-safety check on slot_seen(oldHash)), and three `SSTORE_RESET` (clearing the evicted entry, installing the new ring entry, and advancing the ring pointer). It omits any state-creation surcharge, because ring rotation overwrites existing slots rather than allocating new ones: total `NONCE_RING` state is fixed at 2 × `RING_CAPACITY` slots regardless of usage.
...
Nodes MAY admit multiple pending expiring-nonce transactions per sender. EIP-8141's one-pending-frame-transaction-per-sender guidance does not apply to expiring-nonce mode.
...
The replay window of an expiring-nonce transaction is its deadline `d`. Rebroadcasts after `d` are rejected by the `expiry_verify` frame itself; rebroadcasts before `d` are rejected by the `seen[h].deadline <= now` check. The ring guarantees that an entry is not evicted while still live as long as `MAX_EXPIRY_SECS × peak_tps ≤ RING_CAPACITY`; `BufferFull` enforces this invariant at execution time.
