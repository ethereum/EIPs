...
discussions-to: [https://ethereum-magicians.org/t/TBD](https://ethereum-magicians.org/t/TBD)
status: Draft
...
With Electra/MaxEB accounting, the weak subjectivity period (in epochs) for a 10% safety decay target is:
...
256
;+;
\frac{0.1 \cdot \text{total_stake}}{2 \cdot \big(\text{activation_exit_churn} + \text{consolidation_churn}\big)}
]

and in days: (\text{WS}_{\text{epochs}} / 225).

Under this EIP, for sufficiently large total stake (so rounding and minimums don’t dominate):

* (\text{activation_exit_churn} \approx \text{total_stake}/2^{15})
* (\text{consolidation_churn} \approx \text{total_stake}/2^{16})

Hence the sum** is (\text{total_stake}\cdot(1/2^{15} + 1/2^{16}) = \text{total_stake}/21845.333...), and the total cancels out in the fraction. This yields:
...
[
\Rightarrow\ \textbf{WS} \approx \mathbf{5.99\ \text{days}}
]
...
= 256 + 1638.4
= 1894.4
\Rightarrow\ \textbf{WS} \approx \mathbf{8.42\ \text{days}}
]
