...
category: Core
created: 2026-08-03
requires: 684, 1014, 3529, 8037, 8038
---
...
- At the end of the transaction, all code, nonce, and storage created during transaction execution are removed from the target account. The account balance is preserved. An account that becomes empty is removed from the state trie at the end of the transaction in accordance with EIP-161.
...
This behavior is consistent with the assumptions of EIP-7610 that an empty account contains no meaningful persistent storage.

All other opcodes retain their existing behavior.

### Gas Accounting

Each execution of `TCREATE` is charged `BASE_OPCODE_COST` together with the existing `CREATE2` execution costs defined by EIP-8037.
...
Regular-gas accounting for account updates (following the principles introduced in EIP-8038) is defined as follows.
...
This property is also beneficial for account abstraction. For example, an EOA authorized under EIP-7702 may deploy transient contracts while producing only transaction-local state changes, simplifying mempool management and transaction simulation.
