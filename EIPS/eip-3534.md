...
The use of "blacklist" in conjunction with a "whitelist" is logically inconsistent. It is not allowed.

A `MAX_ELEMENTS` limits of `3` is chosen to balance the interests of limiting the potential size of transactions, and to provide a sufficient level of articulation for the user. At the time of writing, the top 3 miners of Ethereum (by block, measured by known public addresses) account for 52% of all blocks produced.

One may note that in order for a miner entity to avoid ineligibility by a blacklist, they would only need to use an alternative adhoc address as the block beneficiary.
...
The chance of a colliding `ancestorId` is `1/(16^8=4_294_967_296)` times whatever we take the chance of the existance of an equivalently-numbered block (on an alternative chain) to be. Assuming a generous ballpark chance of 10% (`1/10`) for any given block having a public uncle, this yields `(1/(16^8=4_294_967_296) * 1/10`. Note that this ballpark assumes "normal" chain and network behavior. In the case of an enduring competing chain segment, this value rises to 100% (`1`).

__TL;DR__: The chance of an ineffectual `ancestorId` is 1 in about 4 billion, times about 10 (or times 1 for intentional duplication scenarios, eg. malicious reorgs).
...
Miner lists and expiry depend on easily cacheable, contextually available, conditions. Their infrastructural overhead costs are expected to be nominal.

Validation of `ancestorId` demands the assertion of a positive database hit by block number (cross-referencing a stored block's hash). 
...
The proposed fields are not associated with any proposed gas costs, establishing no protocol-defined economic mitigation for potential spam.
