...
We assume the trait of collision resistance is uniformly applicable to all possible subsets of the block hash value; that is: the whole hash, the first 4 bytes, the first 14, the middle 8, or the last 30; any subset is as unique as any other subset. 

- `e78b1ec31bcb535548ce4b6ef384deccad1e7dc599817b65ab5124eeaaee3e58`. Whole hash, unique.
- `e78b1ec3`. First 4 bytes, unique.
- `e78b1ec31bcb535548ce4b6ef384`. First 14 bytes, unique.
- `384deccad1e7dc59`. Middle 8 bytes, unique.
- `1ec31bcb535548ce4b6ef384deccad1e7dc599817b65ab5124eeaaee3e58`. Last 30 bytes, unique.
...
The author of a transaction using the `eligibleMinerList` must assume that the "general availability" of the blockchain state database for such a transaction will be lower than an nonrestrictive transaction (since only a subset of miners will be able to process the transaction). 
