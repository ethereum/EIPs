...
Subclasses are defined with with a high level of conceptual independence, and can be modified and/or extended independently from this EIP.
...
Practically, the "designated chain segment" can be understood as the segment of blocks from `0..ancestorId` inclusive.
...
When a transaction's `ancestorId` references a block, we want to be pretty sure that that reference won't get confused with a different block than the author of the transaction had in mind.
...
For the sake of legibility and accessibility, the following arguments will reference the hex representation of bytes, which is 8 characters in length, eg. `e78b1ec3`. 

The chance of a colliding `ancestorId` is:

- the chance of "rolling" any one of all possible 4-byte prefixes: `1/(16^8=4_294_967_296)`
- times itself (like rolling two simultaneous 6's in Yahtzee :registered:), because the hash collision needs to happen with the same block number (simultanaity): `1/(16^8=4_294_967_296) * 1/(16^8=4_294_967_296)`
- times a very generous ballpark chance of 10% (`1/10`) for any given block having an uncle: `(1/(16^8=4_294_967_296) * 1/(16^8=4_294_967_296)) * 1/10`

Yields: `5.42101e-21`.

In layman's terms: one in 4 billion squared times 10.

This seems sufficiently safe to me.
...
We can define the probability of a colliding ID composed of block number number an first-4 bytes as follows.

The chance of a colliding random 4 bytes for any two blocks with the same number can be defined as `1/(16^8=4_294_967_296)` (remember the hex representation).
About a 1-in-4 billion chance. 
This could be expected to happen about once on Ethereum every 2000 years, _if_ every for every canonical block there were a competitor block.
The uncle rate on Ethereum is generally around 4% (being a little generous). This is for _any_ uncles. Its most common for uncles to occur alone, but the protocol allows for up to 7.
Let's fudge this rate to an exhorbitant 10%. We're ballparking.

So we multiply `1/4_294_967_296` by `0.1`, giving `1/40_294_967_296`. 1-in-40 billion. That's every 20,000 years now.
