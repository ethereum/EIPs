...
- Subclass `ANNOTATION_PREFIX`s are defined as octal-derived positive integers, limited to the set `2^0,2^1,2^2,2^3,2^4,2^5,2^6,2^7`. 
...
`ANNOTATION_PREFIX` only goes up to `0x7f`. This leaves room for 8 constraints (`2^0`..`2^7`), which seems to be plenty for now, and leaves 4 options open.
`ANNOTATION_PREFIX` values should be OCTAL values, eg. `1, 2, 4, 8...`. This follows a conventional pattern of representing combinations uniquely, eg. Unix-style file permissioning.
