...
Most uses of signatures, such as block proposals and attestations, can move to any secure post-quantum scheme. RANDAO, however, additionally depends on signature *uniqueness*, and most post-quantum schemes do not provide it, allowing one to grind for a favorable RANDAO contribution. The hash-based signature scheme currently planned for the consensus layer (a generalized eXtended Merkle Signature Scheme (XMSS) ([RFC 8391](https://www.rfc-editor.org/info/rfc8391/)), commonly referred to as the lean signature scheme) is grindable in exactly this way, since it includes a *salt* component.
...
Concretely, proposer duties for epoch `E` are drawn from the mix fixed at the end of epoch `E - MIN_SEED_LOOKAHEAD - 1` (`E - 2` with `MIN_SEED_LOOKAHEAD = 1`). For a registration included in epoch `N`, the seeds for epochs up to `N + 1` are already final, and the seed for `N + 2` (the epoch-`N` mix) is nearly complete for a registrant landing in the last slot of `N`, so a registration activating in any epoch up to `N + 2` is grindable. The seed for `N + 3` fixes only at the end of epoch `N + 1`, entirely after inclusion, making `N + MIN_SEED_LOOKAHEAD + 2 = N + 3` the earliest safe activation epoch; `COMMITMENT_REGISTRATION_DELAY = 3` meets the bound exactly.
