...
A set of test vectors for verifying implementations is located in a separate file (in `assets/KAT`) and tests of verifications are provided in Solidity.

## Reference Implementation

An implementation is provided in `assets` of this EIP. NIST submission KAT vectors are verified, and a test is also provided for the EVM-friendly version (following the same format as for NIST submission).
