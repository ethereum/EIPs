...
description: Introduce `EOFCREATE`, `RETURNCONTRACT` instructions
...
EVM Object Format (EOF) removes the possibility to create contracts using `CREATE` or `CREATE2` instructions. We introduce two new instructions: `EOFCREATE` and `RETURNCONTRACT` to provide a way to create contracts using EOF containers.
