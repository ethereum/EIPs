...
- Hooks and Callbacks - With more protocol allowing hooks to interact with their protocol and different token standard which trigger callbacks on a transfer (i.e. [ERC-1155](https://eips.ethereum.org/EIPS/eip-1155)) it is important to make sure that these interactions are well curated to minimate the security risks as much as possible.
- ….
...
With this the user (or token owner) can trigger the registration flow (2). There are two steps (3 and 6) that will be performed: verify that the provided audit is valid and has been signed by a trusted auditor (4) and check that the token contract implements ERC-1155 (7).
...
- Audit
    ...
    - `ercs` - A list of ERCs that are implemented by the target contract. The ERCs listed MUST be fully implemented. This lust MAY be empty.
...
EIP712Domain auditDomain = EIP712Domain("ERC-XXX: Onchain Audit Representation", "1.0");
```
