...
EIPs targeting a network upgrade must be proposed for inclusion in that upgrade's Meta EIP. The inclusion stages—from "Proposed for Inclusion" through "Scheduled for Inclusion" to "Included"—are defined in [EIP-7723](./eip-7723.md). Once the scope for each upgrade is finalized (generally within a few weeks of the previous upgrade going live on mainnet), only security-critical new proposals will be considered. Proposals do not carry over between upgrades—authors must re-propose their EIP for each upgrade cycle.
