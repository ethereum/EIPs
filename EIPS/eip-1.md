...
For Ethereum implementers, EIPs are a convenient reference during implementation. Ideally each Ethereum implementation would list the EIPs that they have implemented. This will give end users a convenient way to know the current status of a given implementation or library.
...
- A **Standard Track EIP** describes any change that affects most or all Ethereum implementations, such as a change to the network protocol, a change in block or transaction validity rules, proposed application standards/conventions, or any change or addition that affects the interoperability of applications using Ethereum. Furthermore Standard EIPs can be broken down into the following categories. Standards Track EIPs consist of three parts, a design document, implementation, and finally if warranted an update to the [formal specification].
  - **Core** - improvements requiring a consensus fork (e.g. [EIP5], [EIP101]), as well as changes that are not necessarily consensus critical but may be relevant to Ethereum implementers (for example, [EIP90], and the miner/node strategy changes 2, 3, and 4 of [EIP86]).
...
These calls generally result in a "rough consensus" around what EIPs should be implemented. This "rough consensus" rests on the assumptions that EIPs are technically sound and their implementation is consistent with the best knowledge available of the political will of the Ethereum community.

:warning: The EIPs process is not intended to fully capture the political will of the community. As a technical standardization process, the best case scenario for EIPs that may be politically contentious is to standardize the technical details of a proposal as a reference point for political debate to occur. No political decisions occur as a result of the EIP process, as political speech cannot itself be subject to standardization.

:warning: The client implementers will do their best to be as aware of the political climate surrounding a proposal as they can find the time and energy to do so, but their failure to understand all political discussion around contentious issues should not be considered a shortcoming of the process, nor a value judgement of their effectiveness at this role. As the shepherd of an EIP, you can make the process of building community consensus easier by making sure that [the Ethereum Magicians forum](https://ethereum-magicians.org/) thread for your EIP includes or links to as much of the community discussion as possible and that various stakeholders are well-represented. This collective sentiment of the community should be presented, in full, with detailed descriptions of the various facts and opinions the community holds surrounding the proposal, to the client implementors on the AllCoreDevs call, so they have the most relevant information available to make decisions of what to implement in their clients on behalf of their users. The community ultimately uses the software that client developers implement to signal the configuration of the main Ethereum network during critical upgrade periods, and the decisions the client developers make should reflect the simplest configuration possible for what they expect the communities' decisions to be.
