...
When referring to an EIP by number, it should be written in the hyphenated form `EIP-X` where `X` is the EIP's assigned number. If the referenced EIP has a `category` of `ERC`, then it must be written `ERC-X` instead, where `X` is the EIP's assigned number.
