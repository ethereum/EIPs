...
Clients may prune history that is older than a year.
...
Pruning history also allows clients to remove code that processes historical blocks. This means that execution clients don't need to maintain code paths that deal with each upgrade's compound changes.

Finally, the p2p network also benefits from history pruning since there will be less networking strain as clients adopt more lightweight sync strategies.

Although there are strong benefits to pruning client history, securely bootstrapping nodes without it is hard. The move to PoS presents a great opportunity to rely on the weak subjectivity assumption in PoS and simplify client syncing logic.
...
Clients MAY prune block/state/receipt history that is older than `HISTORY_PRUNE_EPOCHS` epochs.
