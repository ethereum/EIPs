...
This subsection offers a comparison of the censorship resistance achieved under FOCIL and FOCILR. The premise is a builder seeking to censor a transaction under different fee market designs. We begin by rudimentarily quantifying the cost $C_i$ of censoring a transaction in block $i$ ($i=0$ for the first censored block) as 
...
Under FOCILR, slow transactions do not contribute meaningfully to censorship due to the $g_df_r +  m(i, f_r)$ terms, and there is furthermore no rationale for paying a priority fee upon listing, further strengthening the public mempool.
...
Block-building becomes slightly more complex under the FOCILR inclusion check. Builders may develop more refined strategies for MEV under adherence to the stricter inclusion rules. Here is a simple heuristic that builders can use to ensure that they always end up with a valid block without having to re-execute. They first include the $n$ highest ranked transactions with an aggregate gas limit within the block's gas limit, ordering them freely for MEV. After executing these transactions to determine their used gas and the remaining space in the block, they add the additional $m$ remaining highest ranked transactions ordered freely but after the first $n$ transactions, continue execution, filling up remaining space, etc.

For example, the block may have a gas limit of 120M gas and the builder may decide to first include $n=187$ transactions that would pass the inclusion check by having a ranking fee higher any potentially than left out transactions from the ILs. These are ordered freely to extract MEV. Upon execution, it turns out that 20M gas remains. They now select $m=31$ remaining transactions that have a ranking fee higher than any potentially left out transactions from the ILs. These are ordered freely to extract MEV, but positioned after the $n=187$ first transactions. Upon executing the $m$ transactions, it turns out that 1M gas remains, and the process continues until all remaining gas is used up.
