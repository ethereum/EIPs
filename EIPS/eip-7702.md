...
Owned Accounts (EOAs) to set the code value in their account. This is achieved
by adding a a list of `[chain_id, address, nonce, y_parity, r, s]` authorization
tuples to the transaction. For each tuple, a delegation indicator `(0xef0100 ||
address)` is written to the authorizing account's code. All code executing
operations must load and execute the code pointed to by the delegation.
...
8. Set the code of `authority` to be `0xef0100 || address`. This is a delegation
    ...
    * As a special case, if `address` is
    ...
    indicator. Clear the account's code and reset the account's code hash to the
    empty hash
...
reverting), setting delegation indicator is *not* rolled back.

##### Delegation Indicator

The delegation indicator uses the banned opcode `0xef` from
[EIP-3541](./eip-3541.md) to indicate the code has a special purpose. The
delegation requires all code executing operations to follow the address pointer
to get the account's executable code, and requires all other code reading
operations to act only on the delegation indicator (`0xef0100 || address`). The
...
`destination` targeting the code with delegation indicator.
...
In case a delegation indicator points to a precompile address, retrieved code
...
indicator can be revoked at any time by signing and sending an EIP-7702
...
is design, and for other EIPs such as inclusion lists. However, because the
accounts are listed statically in the outer transaction, it is possible to
modify transaction propagation rules so that conflicting transactions are not
forwarded.
