...
authorization_list = [[address, nonce, y_parity, r, s], ...]
```
...
The `authorization_list` is a list of tuples that store the address to code which the signer desires to set in their EOA temporarily. Nonce is considered optional.
...
1. Compute `authority`
    * If nonce is null, `authority = ecrecover(keccak(NULL_NONCE_MAGIC || rlp([address])), y_parity, r, s]`
    * Otherwise, `authority = ecrecover(keccak(NONCE_MAGIC || rlp([nonce, address])), y_parity, r, s]`
2. Verify that the code of `authority` is empty.
3. If nonce is not null, verify the nonce of `authority` is equal to `nonce`.
4. Set the code of `authority` to code associated with `address`.
5. Add the `authority` account to `accessed_addresses` (as defined in [EIP-2929](./eip-2929.md).)
...
The EIP proposes two signature format:
* `keccak(NULL_NONCE_MAGIC || rlp([address]))`. 
* `keccak(NONCE_MAGIC || rlp([nonce, address]))`. 

Each signature has it's own magic prefix to force separation. This allows for maximally flexible design which allows for both full delegation to `address` and more protected delegations to `address`.
