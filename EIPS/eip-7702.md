...
For the duration of the transaction, instructions that modify the storage of any of the above recovered `signer` accounts MUST cause execution to become invalid (in the same way as a stack underflow or invalid jump.) Examples of such instructions include `SSTORE` (`0x55`) in the context of a `signer` or in a `delegatecall`/`callcode` target from a `signer`.
