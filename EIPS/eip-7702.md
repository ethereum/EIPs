...
1. Compute `authority`
    * If nonce is null, `authority = ecrecover(keccak(NULL_NONCE_MAGIC || rlp([chain_id, address])), y_parity, r, s]`
    * Otherwise, `authority = ecrecover(keccak(NONCE_MAGIC || rlp([chain_id, nonce, address])), y_parity, r, s]`
2. Verify the chain id is either 0 or the chain's current ID.
3. Verify that the code of `authority` is empty.
4. If nonce is not null, verify the nonce of `authority` is equal to `nonce`.
...
* `keccak(NULL_NONCE_MAGIC || rlp([chain_id, address]))`. 
* `keccak(NONCE_MAGIC || rlp([chain_id, nonce, address]))`. 
