...
Once code exists in the EOA, it's possible for self-sponsored EIP-7702 transactions to have `msg.sender == tx.origin` anytime the code in the EOA dispatches a call. Without EIP-7702, this situation typically occurs only in the topmost execution layer of a transaction. However, proxy contracts can also create scenarios where msg.sender == tx.origin, meaning this EIP further expands such cases. Therefore, this EIP breaks that invariant and so it affects smart contracts containing `require(msg.sender == tx.origin)` checks. This check is used for at least three purposes:
