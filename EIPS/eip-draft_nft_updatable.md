...
The **metadata updates extension** is OPTIONAL for [ERC-721](./eip-721.md) and [ERC-1155](./eip-1155.md) contracts.

```solidity
/// @title ERC-721/ERC-1155 Updatable Metadata Extension
interface IERC????UpdatableMetadata {
...
The original metadata SHOULD conform to the "ERC-???? Updatable Metadata JSON Schema" which is a compatible extension of the "ERC-721 Metadata JSON Schema" defined in ERC-721.

"ERC-???? Updatable Metadata JSON Schema" :
...
The original metadata for a given NFT is always defined as the JSON pointed by the result of `tokenURI` for ERC-721 and function `uri` for ERC-1155.
...
Smart contracts should be careful and conscious of using this extension and still allow the metadata URI to be updated in some contexts (by not having the same URI returned by `tokenURI` or `uri` for a given tokenId). They need to take into account if previous changes could have been already broadcasted for that NFT, if these changes are compatible with the new "original metadata" and what semantic they decide to associate by combining these two kinds of "updates".

## Backwards Compatibility

The proposal is fully compatible with both ERC-721 and ERC-1155. Third party applications that don't support Updatable Metadata Extension will still be able to use the original metadata for each NFT.
