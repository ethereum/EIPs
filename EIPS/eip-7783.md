...
If we set `blockNumStart` to the current block number, `initialGasTarget` to the current gas target (`15_000_000`), `r` to 3, and `gasTargetCap` to `30_000_000`, the gas target will increase by 3 gas per block for two years, reaching 30 million gas at the end of a 2 years period. This means that the Gas Limit will grow proportionally to the Gas Target, and will end up being twice the value of the gas target
