...
- Otherwise the created account has adopted code through `SETCODEFROM`, and the initcode's return data is not code. It is neither validated nor charged code-deposit gas, and the account's `codeHash` is left as adopted.

The gas and state costs of the creation itself are unchanged, as is failure handling: if the creation frame reverts or halts exceptionally, the code hash update reverts with every other state change in that frame.
