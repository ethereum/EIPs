...
In one example, victims that browse a malicious phishing website. It requests the victim to sign a message that will put their NFT token for sale on OpenSea platform, virtually for free.
...
The proposed solution solves the readability issues by allowing the wallet to query the verifyingContract. The incentives for keeping the EIP-712 message descritption as accurate as possible are alligned, as the responsibility for the description is now owned by the contract, that:
...
- We chose to return an array of strings and not a single string as there are potential cases where the message is composed of multiple parts. for example in the case of a multiple assets transfers in the same typedDataBuffer, the contract is advised to describe each transfer in a seperate string to allow the wallet to display each transfer separately.
