...
- `extensions`: A list of EIP numbers that specify additional fields in the domain. Their inclusion in this list means they are part of the domain, and the value of `fields` does not change that. The method to obtain the values for these additional fields is expected to be specified in their respective EIPs.
