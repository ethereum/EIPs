...
The signature values in `r` and `s` and the public key `(x, y)` provides direct computations in signing and verification part, so these formats are chosen in the input data format which are 32 bytes. (Nalin: ??? what does this mean ???)
