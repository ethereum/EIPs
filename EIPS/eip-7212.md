...
Following requirements **must** be checked by the precompile to verify signature components are valid:
...
- Verify that the point formed by `(x, y)` is on the curve and both `x` and `y` are in `(0, p)` (exclusive) where `p` is the prime field modulus.
...
This is different from the `ecrecover` precompiled address spec. The advantage is that it 1. follows the NIST spec, 2. matches the rest of the (large) P256 ecosystem, and most importantly 3. allows execution clients to use existing well-vetted verifier implementations.
...
Implementation of the `P256VERIFY` precompiled contract is applied to go-ethereum client to create a [reference](TODOLINK). Also, an “secp256r1” package [has already been included in the Besu Native library](TODOLINK) which is used by Besu client. Other client implementations are in the future roadmap.
