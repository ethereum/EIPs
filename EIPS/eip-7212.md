...
- Verify that the point formed by `(x, y)` is on the curve and both `x` and `y` are in `(0, p)` (exclusive) where `p` is the prime field modulus. Note that in particular, often implementations use `(0, 0)` as the reference point at infinity, which is not on the curve and should therefore be rejected.
...
This is different from the `ecrecover` precompiled address spec. The advantage is that it 1. follows the NIST spec[^1], 2. matches the rest of the (large) P256 ecosystem, and most importantly 3. allows execution clients to use existing well-vetted verifier implementations.
