...
- **Server-Sent Events (SSE).** A WHATWG HTML standard for one-way, long-lived HTTP event streams from server to client. In this EIP, SSE is used in two places: by the beacon node to push `Block` events to the prover, and by the proof node to push proof-completion events to the prover.
...
- **Verification** (`verify_execution_proof`) is called on every incoming
  ...
  verified. The value of `k` (and whether it is fixed or specified
  per-`proof_type`) will be pinned before this EIP transitions to
  `Review`; it is intentionally left open while the network gathers
  real-world data.
