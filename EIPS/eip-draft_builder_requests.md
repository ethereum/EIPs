...
- A `BuilderDepositRequest` (type `0x03`) for a `pubkey` **not** yet in the builder set is a first deposit: the consensus layer verifies the proof-of-possession `signature` over the `DepositMessage` `(pubkey, withdrawal_credentials, amount)` under `DOMAIN_DEPOSIT` — the same check validator deposits use (gloas `is_valid_deposit_signature`) — and, if valid, registers the builder with the record's `withdrawal_credentials` and credits its `amount`; an invalid signature is ignored.
