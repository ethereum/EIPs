...
## Abstract
The following are the transfer and callback functions standardized in this EIP:
...
Note that `transferAndCall`, `transferFromAndCall` and `approveAndCall` MUST revert if called on an EOA as it can't return the [EIP-1363](./eip-1363.md) expected value.
