...
We implement Lean Staking using LeanVM, a VM coupled with a post-quantum proving system based on a combination of WHIR[^1] and [SuperSpartan](../assets/eip-8222/customizable-constraint-systems.pdf). LeanVM exposes a Poseidon1 precompile (`poseidon16`, where `16` denotes the permutation width — two 8-element Koala-Bear field arrays — not a hash version). Since the commitment preimage exceeds 16 field elements (31 elements total: 8 for `nullifier_preimage`, 13 for `validator_key`, 9 for `withdrawal_cred`, 1 for `amount`), the commitment hash requires chained `poseidon16` calls.
