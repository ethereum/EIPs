...
`secret_call` is a required parameter, is used for approving a specific call without revealing it. `peer_address` can sign a pre-signed message with this value, or use `merkle_proof` and `hash_to_peer` and call it directly. 
...
The setup takes 3 parameters: `public_hash`, `setup_delay`, and the `address_list_merkle_root`.
