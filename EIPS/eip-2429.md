...
`user_secret_data` is any format and contains user data in plain format. Is never exposed, its never saved. Should be based on biometrics because users are very unlikely to loose their biometrics. Biometrics are usually unsafe for generating a seed phrase because they are not really secret, any high resolution camera can in fact read most of biometrics, and this information is also usually known by governaments. 
...
`user_secret_data`is never exposed, right after input its hashed once with keccak256 to become the `private_hash`. This was done to standerize the `private_hash` format. 
