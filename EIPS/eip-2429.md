...
This URL is constructed according to the following:
    ...
    helprecover       = erc831_part recovery_contract [ "@" chain_id ] "/" call_dest "/" peer_address [ "?" parameters ]
    ...
    parameters        = parameter *( "&" parameter )
    parameter         = key "=" value
    key               = STRING | "hash_to_peer" | "call_data" | "weight"
    value             = STRING | hash_to_peer | call_data | weight
    hash_to_peer      = "0x" 64*HEXDIG
    approve_hash      = "0x" 64*HEXDIG
    weight            = UINT

`call_dest` required the account contract being recovered. Any account contract can be used, as the recovery contract can execute to any interface or address (like a regular multisig). 
...
`call_data` is a required parameter, is used for approving a specific call 

`peer_address` the peer address that needs to approve. Can be an ENS_NAME.

`weight` is always required, part of the leaf hashing.
...
`approve_hash` is `keccak256(hash_to_execute, call_destination, call_data)`;
...
It's important that wallets follows correctly the `public_hash` generation, specifically don't ignore the use of `recovery_contract` and `recovery_nonce` in the calculation of `hash_to_execute`. This was designed so that the `secret_set` can be reused. 
