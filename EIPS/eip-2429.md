...
The addresses are stored in a standard merkle tree.

`merkle_root` is a hashed as a standard merkle tree composed of leafs of `address_list` on this format `keccak256(boolean(is_ens), bytes32(ethereum_address))`.
...
```mermaid
	graph TB
        ...
        hash_to_execute --> partial_reveal; 
...
`partial_reveal` is `keccak256(hash_to_execute)`. Is exposed at recovery authorization request, is used to proof user knows `hash_to_execute` and `keccak256(merkle_root)`, which are the seed of `public_hash`.

`public_hash` is `keccak256(partial_reveal, keccak256(merkle_root))`. Is public since configuration. Can only be used once, and this is also used to prevent replay of execution. After execution successful, must be reconfigured with `setup(bytes32,uint256)`.
...
`weight` defaults to `Math.ceil(THRESHOLD / address_list.length * 3)`, is custom by user setting. 
...
```abnf
    helprecover       = erc831_part recovery_contract [ "@" chain_id ] "/" partial_reveal "+" reveal_proof ":" peer_address "/" call_dest "/" function_name  [ "?" function_params ]
    erc831_part       = "ethereum:helprecover-" 
    recovery_contract = ADDRESS
    chain_id          = 1*DIGIT
    partial_reveal    = "0x" 64*HEXDIG
    reveal_proof      = "0x" 64*HEXDIG
...
`partial_reveal` is a required. Together with `reveal_proof` is used to authenticate with guardians with `call_dest` identity, and also used to calculate `approve_hash`.

`reveal_proof` is required, is the hash of the `merkle_root`. Together with `partial_reveal` is used to authenticate with guardians with `call_dest` identity.  

`peer_address` defines where the approval must came from. Used for calculating `leaf_hash`, which is used to clculate `approve_hash`.

`call_dest` the wallet contract being recovered. Used to calculate `approve_hash`.
...
There is no enforced ABI for the recovered contract, the `call_destination` and `call_data` are pre-approved by the `address_list`, however this parameters are secret and tied to `partial_reveal`, with values revealed at execution. The peers should know what they are signing. All peers approve the same hash.

`approve_hash` is `keccak256(merkle_leaf, partial_reveal, call_destination, call_data)`;
...
The called function is `approve(approveHash, ensNode)`, the parameters as follow:
...
`signed_message` Is the [Pre-Signed Message](#pre-signed-messages) that approves the request, will be the case guardian used the pre-signed path.
...
The called function is `approvePreSigned(approveHash, ensNode, signer, signature)`, the parameters as follow:
- approveHash: the `approve_hash`.
...
- signature: the signature provided by the [Help Recover Response URL](#help-recover-response-url).

#### Executing Recovery

After all approvals submitted to the recovery contract, the execution can be called. 

The called function is `execute(executeHash, merkleRoot, calldest, calldata, leafHashes, proofs, indexes)`, the parameters as follow:
...
This step reveals the `hash_to_execute`, `merkle_root` and the used merkle tree leafs.
...
Consider the following case of a merkle tree of 4 guardians were we intend to reveal 2 guardians:
...
graph LR
    ...
    subgraph Unrevealed Guardian
        unknown1["Address/ENS"]
        unknown2["Weight"]
    end
    subgraph Unrevealed Guardian
        unknown3["Address/ENS"]
        unknown4["Weight"]
    end
    proof1["Proof 1 - Index 0"];
    proof2["Proof 2 - Index 1"];
    hash1["Hash 1 - Index 2"];
    hash2["Hash 2 - Index 3"];
    hash3["Hash 3 - Index 4"];
    hash4["Hash 4 - Index 5"];
    root["Merkle Root"]   
...
For the example above, the indexes must be given as follow: `[2,0,3,1,4,5]`. Different ordering for the index could be given as long they match the same result.
...
interface ERC2429 {
    ...
    /**
     * @notice Approves a recovery. This method is important for when the address is an contract and don't implements EIP1271.
...
In order to keep wallet owner privacy, any recovery should use anonymized funds (e.g. public mixer).
If funds directly from an exchange are used to execute an recovery, is likely that the wallet owner real name could get deducted/leaked by the other party.
