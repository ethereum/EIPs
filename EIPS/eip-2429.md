...
`secret_hash` is `keccak256(user_secret_data)`. Is never exposed, could be exported with secret-set.  

`seedpub_hash` is `keccak256(secret_hash, recovery_address, recovery_address.nonce())`. Is exposed only at execution. 
