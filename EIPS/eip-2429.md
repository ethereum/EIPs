...
`hash_to_peer` is `keccak256(hash_to_execute, chain_id)`. Is exposed at recovery authorization request, is used to proof user knows `hash_to_execute` by revealing the seed of `public_hash`. This is also known as a "partial reveal hash". `chain_id` must match ID of the executing chain ([EIP-1344]).

`public_hash` is `keccak256(hash_to_peer)`. Is public since configuration. Can only be used once, and this is also used to prevent replay of execution. After execution successfull, must be reconfigured with `setup(bytes32,uint256,bytes32,bytes32)`.
...
`secret_call` is `keccak256(recovery_contract, hash_to_execute, call_destination, call_data)`;
