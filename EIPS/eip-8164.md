...
Ed25519. Tuples with any other element count are skipped.
...
##### Ed25519-Signed Authorization (Key Rotation)
...
|-------|------|-------------|
| `chain_id` | `uint256` | Target chain ID, or `0` for any chain |
| `new_pubkey` | `bytes32` | New Ed25519 public key to install |
| `nonce` | `uint64` | Current nonce of the authority account |
| `authority` | `address` | Address of the native-key account authorizing rotation |
| `signature` | `bytes64` | Ed25519 signature by the currently installed key |
