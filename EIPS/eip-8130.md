...
AA_TX_TYPE || rlp([
  ...
  extensions          // Reserved for future EIPs (e.g., token payments, multisig)
])
```
...
- **Token payments**: `[0x01, token_address, max_amount]`
- **Native multisig**: `[0x02, required_signatures, additional_signatures]`
...
2. Resolve payer from `payer_auth`:
   ...
   - Empty: `from` pays ETH
3. Verify `extensions` is empty (reject if non-empty until future EIPs define valid extensions)
...
1. **Token payments**: A future EIP can define `[0x01, token_address, max_amount]` for gas abstraction with ERC-20 tokens
2. **Native multisig**: If needed at protocol level, `[0x02, threshold, signatures...]` could be added
3. **Other features**: Session keys, batched nonces, etc.
