...
- **Ensuring Extensibility**: Supports future quantum-safe algorithms via new auth types and extensible payer modes
...
| `ENTRY_POINT_ADDRESS` | TBD <!-- TODO --> | Protocol caller address for calldata delivery |
...
**Default behavior**: An empty configuration means only the account's EOA key can authorize transactions. The EOA key always retains authorization regardless of configured keys, ensuring recovery capability (for EOA-based accounts). Note that this feature will likely be removed to enable accounts to revoke their EOA access in a quantum migration EIP.

**Account policy**: Accounts may set a policy bitfield at `base_slot + 0` via `setAccountPolicy()`. The `requireSponsor` bit (bit 0) prohibits self-pay — the protocol rejects transactions with empty `payer_auth` for accounts with this bit set. Only `msg.sender` can modify their own policy. The policy slot is only read when `payer_auth` is empty; sponsored transactions satisfy the constraint without an additional SLOAD.
...
Account policy:
  base_slot + 0: account_policy (uint256, packed bitfield)
    bit 0: requireSponsor (1 = self-pay prohibited)
    bits 1-255: reserved
...
AA_TX_TYPE || rlp([
  ...
  calldata,
  payer_data,         // Reserved for future payer extensions | empty
  sender_auth,
  payer_auth          // K1 sig (65 bytes) | payer_address (20 bytes) | empty
])
```
...
| `payer_data` | **Empty**: No payer data. **Non-empty**: Reserved for future payer extensions. Signed over by both sender and payer when sponsored. Included in `tx_payload_cost`. |
| `sender_auth` | See [Signature Format](#signature-format) |
| `payer_auth` | Payer authorization, disambiguated by length. **Empty**: self-pay. **65 bytes**: K1 sponsor (ecrecover). **20 bytes**: delegate payer. See [Payer Modes](#payer-modes) |
...
|------------|-----|-----------|
| Self-pay (0 bytes) | 2,100 | 1 cold SLOAD (account policy check) |
| K1 sponsor (65 bytes) | 3,000 | ecrecover |
| Delegate payer (20 bytes) | 2,100 | 1 cold SLOAD (delegate key verification) |
...
keccak256(AA_TX_TYPE || rlp([
  ...
  authorization_list, account_initialization, key_changes, calldata, payer_data,
  0x80
]))
...
Gas payment mode is disambiguated by `payer_auth` length:

| Mode | Sender signs | `payer_auth` (wire) | Gas Payer | Description |
|------|-------------|---------------------|-----------|-------------|
| **Self-pay** | `0x80` (empty) | MUST be empty (0 bytes) | `from` | Sender pays gas. Rejected if account has `requireSponsor` set. |
| **K1 sponsor** | `<payer_address>` or `0x00` | K1 signature (65 bytes) | Payer (ecrecover) | Any K1 key can sponsor. Committed or open. |
| **Delegate payer** | `<payer_address>` (20 bytes) | payer_address (20 bytes exactly) | `payer_address` | Payer has DELEGATE key for sender. No payer signature. |


**Delegate payer**: The protocol reads the payer's key struct for `keyId == from` and verifies `auth_type == 0xFF` (DELEGATE). This confirms the payer has granted the sender standing authorization to use their ETH for gas. The sender's signature serves as the sole transaction authorization — the on-chain DELEGATE key replaces a per-transaction payer signature. The sender always commits to a specific `payer_address` (no open delegate mode).

Combined with `requireSponsor` (see [Account Policy](#account-configuration)), this enables a "gas piggy bank" pattern: an account that cannot self-pay draws gas from a dedicated funding account. One gas account can fund multiple accounts by registering a DELEGATE key for each, fully isolating gas spend from the accounts' own ETH balances.

Future EIPs may extend `payer_auth` with account config signatures or other mechanisms.
...
The `calldata` is delivered to `from` via a protocol-initiated call from `ENTRY_POINT_ADDRESS`:
...
This proposal constrains **validation** to enshrined auth types but does not constrain **execution**. Once a transaction passes validation, the account's code can implement any logic: multisig, timelocks, spending limits, session keys, or arbitrary business rules. The `calldata` is always delivered to `from` via `ENTRY_POINT_ADDRESS`, ensuring wallet code controls interpretation.
...
7. Resolve payer from `payer_auth` length:
   - If `payer_auth` is empty (0 bytes): read `account_policy` from Account Config (`base_slot + 0` for sender). If `requireSponsor` bit is set, reject. Verify sender signed `0x80`. Payer is `from`.
   - If `payer_auth` is 65 bytes: K1 sponsor. Recover payer address via ecrecover using `AA_PAYER_TYPE` signature hash. Verify sender signed `<payer_address>` or `0x00`; reject if neither validates. Payer is recovered address.
   - If `payer_auth` is exactly 20 bytes: Delegate payer. Parse `payer_address`. Read payer's key struct for `keyId == from` — verify `auth_type == 0xFF` (DELEGATE). Verify sender signed `<payer_address>` as committed marker. Payer is `payer_address`.
8. Verify nonce (read from Nonce Manager), payer ETH balance, expiry
9. Mempool threshold: payer's pending sponsored transaction count must be below node-configured limits
...
8. Deliver calldata to `from` via `ENTRY_POINT_ADDRESS` call
...
interface IAccountConfig {
    ...
    event PolicyChanged(address indexed account, uint256 policy);
    ...
    // Account policy (requires msg.sender)
    function setAccountPolicy(uint256 policy) external;
    function getAccountPolicy(address account) external view returns (uint256);
...
**Payer Security**: Separate type bytes (`AA_TX_TYPE` vs `AA_PAYER_TYPE`) provide domain separation, preventing signature reuse between sender and payer roles. The sender's signed marker commits to a payer mode — see [Payer Modes](#payer-modes). The delegate payer mode relies on the payer's standing DELEGATE key authorization rather than a per-transaction signature; revoking the DELEGATE key immediately invalidates all pending transactions using that payer. Delegate payer always requires committed mode (sender signs `payer_address`), preventing unauthorized gas deduction.
