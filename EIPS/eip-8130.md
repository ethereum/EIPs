...
| `MAX_KEYS` | 127 | Maximum authorized keys per account |
| `MAX_KEY_CHANGES` | 20 | Maximum key change entries per transaction |
...
Each account can authorize a set of keys through the Account Configuration Contract at `ACCOUNT_CONFIG_ADDRESS`. Keys are identified by their `keyId`, a 20-byte content-derived identifier (address-sized) from the key type and public key material:

```
keyId = bytes20(keccak256(key_type || public_key))
```

Only `msg.sender` can modify their own key configuration via EVM calls. Keys can also be modified via the `key_changes` transaction field (see [Cross-Chain Key Changes](#cross-chain-key-changes)), which enables cross-chain key portability. This enables existing ERC-4337 smart accounts to register keys and migrate to native AA without redeployment.
...
Key count:
  base_slot + 0: key_count (uint8, max MAX_KEYS)

Key change sequence:
  keccak256(base_slot || "key_sequence"): latest applied sequence (uint64)
```

The protocol validates signatures by reading these slots directly - no abitrary validation executed within the EVM.

Key enumeration is performed off-chain via `KeyAuthorized` / `KeyRevoked` event logs rather than on-chain storage, eliminating the gas overhead of maintaining an enumeration array. The `key_count` is retained solely to enforce the `MAX_KEYS` limit.

**Key limit**: Accounts may authorize up to `MAX_KEYS` (127) keys, enforced via `key_count`.
...
The protocol reads and increments nonce slots directly during AA transaction processing—no EVM execution required. Only the protocol may increment nonces; there is no public `increment` function.
...
| `key_changes_cost` | Per applied entry: 5,000 (signature verification) + `num_operations` × (20,000 per SSTORE for key struct slots + 5,000 key_count update). Per skipped entry (already applied): 2,100 (SLOAD to check sequence). 0 if `key_changes` empty |
...
Consider bounding `nonce_key` to a smaller range (e.g., `uint16` for 65,536 channels) to limit per-account state growth in the Nonce Manager's storage trie.
...
**K1 compact path** (`0x00`): The protocol ecrecovers the public key from `(r, s, v)`, computes `keyId = bytes20(keccak256(0x01 || recovered_pubkey))`, and checks the key struct at slot +0 (`key_type > 0`). Only 1 SLOAD — the most gas-efficient path. K1 configured keys MAY also use the keyId path if preferred.

**keyId path** (`0x01`): The protocol reads 20 bytes as keyId, looks up the key struct. Reads `key_type` from slot +0 (authorization check + type identification), then reads the public key from slots +1/+2 (warm, 100 gas each). Verifies the signature against the stored public key. Used for R1, WebAuthn, BLS, and DELEGATE key types.
...
Key changes are processed during transaction execution, after account initialization (if any) and before `sender_auth` validation:
...
2. For each entry in `key_changes` (must be ordered by sequence):
   ...
   - Apply each operation:
     - `authorizeKey`: compute `keyId = bytes20(keccak256(key_type || public_key))`, write key_type and public_key to the key struct slots, increment `key_count`
     - `revokeKey`: compute `keyId`, clear the key struct slots to zero, decrement `key_count`
   - Increment `key_change_sequence`
3. Validate `sender_auth` against the **resulting** key state (after all key_changes applied)

#### EVM Key Changes vs key_changes

Keys can be modified through two paths:

| | `key_changes` (transaction field) | EVM calls (`authorizeKey()` / `revokeKey()`) |
|--|---|---|
| Authorization | Signed operation (any auth type) | `msg.sender` during execution |
| Portability | Cross-chain replayable (no chain_id) | Chain-local only |
| Sequence | Tracked by `key_change_sequence` | Does NOT affect `key_change_sequence` |
| When processed | Before `sender_auth` validation | During calldata execution |

EVM-based key changes do **not** increment the `key_change_sequence`. This means:

- `key_changes` are the portable, cross-chain canonical log of key state
- EVM changes are chain-local customizations (e.g., session keys, chain-specific authorizations)
- When `key_changes` are replayed on a new chain, they write to specific keyIds, potentially overwriting chain-local EVM changes at those same keyIds
- Wallet teams should use `key_changes` for any key modification that should be portable, and EVM calls for chain-local additions
...
Processing: deploy account → register initial_keys (Owner A) → apply key_changes (Owner A → Owner B) → validate sender_auth against Owner B ✓ → execute calldata.
...
```
sorted_keys = sort(initial_keys, by: bytes20(keccak256(key_type || public_key)))
...
- **On non-8130 chains**: The contracts operate as standard smart contracts. `createAccount()` works as an unstaked factory compatible with ERC-4337. Key management functions (`authorizeKey()`, `revokeKey()`, etc.) and nonce queries (`getNonce()`) operate normally through EVM execution.
...
2. Sort `initial_keys` by keyId: `sorted_keys = sort(initial_keys, by: bytes20(keccak256(key_type || public_key)))`
...
2. Authorize `initial_keys` in Account Config storage for `from`: for each key, compute `keyId`, write key_type and public_key to the key struct slots, increment `key_count`
3. Apply `key_changes` (if non-empty) — see [Cross-Chain Key Changes](#cross-chain-key-changes)
...
6. Validate `sender_auth` against the **resulting** key state:
   - EOA signature: verify via ecrecover, compute keyId, check authorization
...
4. Process account_initialization (if non-empty): deploy `init_code`, authorize `initial_keys`
5. Apply key_changes to Account Config storage (if non-empty)
6. Deliver calldata to `from` via self-call
...
3. **Onchain interface**: Smart contracts call the system contract during execution to:
   - Modify keys (`authorizeKey()`, `revokeKey()`)
...
| Growth | Bounded (max 127 keys) | Unbounded (nonce channels) |
...
Keys are identified by `keyId = bytes20(keccak256(key_type || public_key))` rather than by array index. This content-addressed approach provides several advantages:
...
3. **Efficient validation**: Signatures carry the 20-byte keyId, and the protocol reads the key struct from storage. For K1, the protocol reads only `key_type` (1 SLOAD) and uses ecrecover to verify the keyId matches—no pubkey storage read needed. For non-K1 types, the protocol reads the full key struct. The uniform `0xFF || keyId || signature_data` format is simple for wallet SDKs to implement.
...
`key_changes` solves this by creating **signed, chain-agnostic key operations** that can be attached to any transaction on any chain:
...
The `key_change_sequence` counter in the Account Config Contract tracks which portable operations have been applied. EVM-based key changes (via `authorizeKey()`/`revokeKey()` calls) do not increment this counter, keeping the portable log and chain-local modifications cleanly separated.
...
interface IAccountConfig {
    ...
    // Key management (max MAX_KEYS keys per account)
    ...
    // Key change sequence (for cross-chain key portability)
...
**Key Management**: Only `msg.sender` can modify account configuration via EVM calls. Keys can also be modified via the `key_changes` transaction field with proper authorization. EOA key always retains authorization for recovery (if created with an EOA via 7702). Maximum 127 keys per account enforced by `MAX_KEYS` constant.
...
**Key Change Sequence Integrity**: The `key_change_sequence` counter only increments for portable `key_changes` operations, not for EVM-based key modifications. This means chain-local EVM changes cannot disrupt the portable key change log. When `key_changes` are replayed on a new chain, they may overwrite chain-local modifications at the same keyIds—this is correct behavior, as the portable log represents the canonical cross-chain key state.
