...
| `POLICY_MANAGER_ADDRESS` | `0x813000000000000000000000000000000000aa03` | Enshrined policy manager (`policyType == 0x01` default owner) |
...
Each owner occupies a single `owner_config` slot containing the verifier address, scope byte, policy type, and an optional expiry. When `policyType == 0x01`, the owner is restricted to a single policy manager whose address is held in a second slot, `owner_manager(account, ownerId) → address` (see [Owner Policies](#owner-policies)). Non-EOA owners are revoked by deleting the `owner_config` slot. The implicit EOA owner (`ownerId == bytes32(bytes20(account))`) is revoked by overwriting the slot with `verifier = REVOKED_VERIFIER` (`type(uint160).max`), making it distinguishable from an empty (implicitly authorized) slot.

| Field | Bytes | Description |
|-------|-------|-------------|
| `verifier` | 0–19 | Verifier contract address |
| `scope` | 20 | Permission bitmask (`0x00` = unrestricted; see [Owner Scope](#owner-scope)) |
| `policyType` | 21 | Policy selector (`0x00` = none, `0x01` = manager-gated; see [Owner Policies](#owner-policies)) |
| `expiry` | 22–27 | `uint48` Unix timestamp (seconds); the owner is invalid once `block.timestamp > expiry`. `0` = no expiry |
| reserved | 28–31 | Reserved for future use (must be zero) |

When `policyType == 0x01`, a second slot holds the manager address:

```
owner_manager(account, ownerId) → address policyManager
```

This slot is read only during execution (see [Owner Policies](#owner-policies)); validity is still decided by the single `owner_config` SLOAD.

**Implicit EOA authorization**: An unregistered owner (`owner_config` slot is empty) is implicitly authorized if `ownerId == bytes32(bytes20(account))`. The empty slot's scope byte and `policyType` are both `0x00`, and `expiry` is `0` (never expires), granting unrestricted, non-expiring permissions and no protocol-enforced policy by default. This allows every existing EOA to send AA transactions immediately without prior registration. When the implicit rule applies, the protocol verifies using native ecrecover rather than calling an external verifier contract. The implicit authorization is revoked by writing `REVOKED_VERIFIER` (`type(uint160).max`) to the verifier field, making the slot non-empty and blocking re-authorization. The EOA owner can also be explicitly registered with `ECRECOVER_VERIFIER` (`address(1)`) to set a custom scope or `policyType` while retaining native ecrecover verification.
...
The protocol validates signatures by reading `owner_config` directly and delegating authentication to [Verifiers](#verifiers) — see [Validation](#validation) for the full flow. Owner enumeration is performed off-chain via `OwnerAuthorized` and `OwnerRevoked` event logs. No owner counts are enforced on-chain — gas costs naturally bound them.

#### Owner Policies

The `policyType` byte selects an optional protocol-enforced call restriction for an owner. The protocol rejects unrecognized values to keep the selector extensible.

| `policyType` | Name | Description |
|--------------|------|-------------|
| `0x00` | none | No protocol-enforced restriction. |
| `0x01` | manager-gated | Under SENDER scope, every `call.to` MUST equal the owner's policy manager (`owner_manager(account, ownerId)`), enforced during execution. |
| `0x02`–`0xFF` | reserved | Reserved for future modes; a future ERC may lift specific values. |

A `policyType == 0x01` owner — typically a session key — may call exactly one address: its **policy manager**. The manager interprets *what* the key may do (token spend limits, call gating, lending actions, etc.), then relays approved actions back to the account for execution. The protocol's only policy responsibility is the single-target gate; all policy semantics live in the manager.

**The manager is itself an owner.** To relay actions, the manager calls back into the account, so it must be an owner with execute authority (SENDER scope). A wallet cannot be forced to honor relayed calls from an arbitrary address, so the manager is registered like any other owner. The canonical deployment registers an enshrined policy manager as a default owner of the account, so provisioning a manager-gated key adds no owners in the common case; advanced wallets MAY register a custom manager instead.

**Sub-policies live in the manager, not the protocol.** The manager holds each key's configuration (spend limits, allowed selectors/targets, lending parameters) in its own storage, installed by the account, keyed by the authenticated `ownerId` it reads from the [Transaction Context](#transaction-context) precompile. Complex behavior is composed as sub-policy modules behind the manager (a pattern popularized by wallet policy frameworks): the manager either enforces a primitive natively or dispatches to a sub-policy contract that returns a call plan, which the manager relays. Sub-policy contracts need not be owners — the manager relays on their behalf.

**Enforcement is at execution, not validation.** The protocol does not read `owner_manager` when checking transaction validity, so policy state is never part of the mempool validity surface — an owner's single `owner_config` SLOAD still decides validity. During [Call Execution](#call-execution), each call made under a `policyType == 0x01` owner is gated against the manager address. Enforcement applies only under SENDER scope; PAYER, SIGNATURE, and CONFIG authentication ignore `policyType`.

**Lifecycle.** The protocol-level binding (`policyType` + `owner_manager`) and the manager-held sub-policy state are separate layers. Revoking or expiring the key clears its `owner_config` (and the `owner_manager` slot) — one or two slots, with no per-target entries to enumerate — and immediately stops the key from reaching the manager. The manager's own per-key state is the manager's concern (it exposes install/uninstall of sub-policies), so the protocol never has to enumerate or clear policy entries.
...
**`sender_auth_cost`**: For EOA signatures (`sender` empty) or `ECRECOVER_VERIFIER` (`address(1)`) signatures: 6,000 gas (ecrecover + 1 SLOAD + overhead). For other configured owner signatures (`sender` set, `address(2+)` verifier): 1 SLOAD (`owner_config`) + actual gas consumed by verifier execution (the Account Configuration Contract and the sender account are pre-warmed, see [Warm Access](#warm-access)). Policy enforcement (`policyType == 0x01`) adds no validation cost — the `owner_manager` SLOAD occurs during call execution and is charged to `gas_limit` there.
...
| `account_changes_cost` | Per applied config change entry: auth verification cost (same model as `sender_auth_cost`) + storage write costs for each mutated owner slot (`owner_config`, and `owner_manager` when `policyType == 0x01`). Per applied delegation entry: code deposit cost (200 × 23 bytes for the delegation indicator). Per skipped config change entry (already applied): 2,100 (SLOAD to check sequence). 0 if no config change or delegation entries in `account_changes` |
...
Every 8130 transaction touches a fixed set of system addresses during validation and execution. To avoid charging [EIP-2929](./eip-2929.md) cold-access penalties for accesses the protocol already requires, the following addresses MUST be pre-added to the transaction's `accessed_addresses` set before execution begins (warm on first touch), following the [EIP-3651](./eip-3651.md) precedent for warming `COINBASE`:

- the `sender` account
- the `payer` account (when `payer` is non-empty)
- `ACCOUNT_CONFIG_ADDRESS`
- `NONCE_MANAGER_ADDRESS`
- `TX_CONTEXT_ADDRESS`
- `POLICY_MANAGER_ADDRESS` (the enshrined policy manager)

Pre-warming covers only the access-list (cold-vs-warm) cost; storage slots, code execution, and value transfers are charged normally. This makes intrinsic cost independent of whether a given system contract was touched earlier in the block and reflects that the protocol itself reads these contracts on the validation path. For a manager-gated key, the enshrined manager is warm whether or not the transaction uses it; custom managers are not pre-warmed and follow normal access rules.
...
Validity does not depend on `policyType`. When the owner has `policyType == 0x01` under SENDER scope, the manager gate is enforced later, during [Call Execution](#call-execution) — not here.
...
rlp([
  ...
  initial_owners      // Array of [verifier, ownerId, scope, policyType, expiry, manager] tuples
])
```

Initial owners are registered with their specified scope, `policyType`, and `expiry` (`0` = no expiry). When `policyType == 0x01`, `manager` is the owner's policy manager address, written to `owner_manager(sender, ownerId)`; otherwise `manager` MUST be `address(0)`. Wallet initialization code can lock the account via `calls` in the execution phase (e.g., calling `lock()` on the Account Configuration Contract).
...
owners_commitment = keccak256(
    ownerId_0 || verifier_0 || scope_0 || policyType_0 || expiry_0 || manager_0 ||
    ...
    ownerId_n || verifier_n || scope_n || policyType_n || expiry_n || manager_n
)
...
The per-owner contribution is `ownerId || verifier || scope || policyType || expiry || manager` (80 bytes; `expiry` is the 6-byte `uint48`, `manager` the 20-byte address, `address(0)` when `policyType != 0x01`). Sorting owners by `ownerId` makes the commitment order-independent.
...
1. Parse `[0x00, user_salt, code, initial_owners]` where each entry is `[verifier, ownerId, scope, policyType, expiry, manager]`
...
4. Reject any owner with unrecognized `policyType`, a non-zero `manager` when `policyType != 0x01`, or a zero `manager` when `policyType == 0x01`
5. Sort owners by `ownerId`
...
|-------------|------|--------|-------------|
| `0x01` | `authorizeOwner` | `rlp([verifier, scope, policyType, expiry, manager])` | Authorize a new owner. Writes `owner_config` with `verifier`, `scope`, `policyType`, and `expiry` (`0` = no expiry). When `policyType == 0x01`, writes `manager` to `owner_manager(account, ownerId)`; otherwise `manager` MUST be `address(0)`. Emits `OwnerAuthorized`. |
| `0x02` | `revokeOwner` | `rlp([])` | Revoke an existing owner. Deletes `owner_config` (and `owner_manager`) for non-EOA owners; for the implicit EOA owner (`ownerId == bytes32(bytes20(account))`), overwrites with `verifier = REVOKED_VERIFIER` (`type(uint160).max`) to prevent implicit re-authorization. The manager's own per-key sub-policy state is the manager's concern and is not touched. Emits `OwnerRevoked`. |
...
Both paths share the same signed owner changes and `change_sequence` counters. `applySignedOwnerChanges()` parses the verifier address from `auth`, calls the verifier to get the `ownerId`, and checks `owner_config`. `authorizeOwner` writes `owner_config` and, for `policyType == 0x01`, the `owner_manager` slot; `revokeOwner` clears both. Anyone can call these functions; authorization comes from the signed operation, not the caller. All owner modification paths are blocked when the account is locked (see [Account Lock](#account-lock)).
...
1. **Create entry** (if present): Register `initial_owners` in Account Config storage for `sender` — for each `[verifier, ownerId, scope, policyType, expiry, manager]` tuple, write `owner_config` (verifier address, scope byte, `policyType`, and `expiry`) and, when `policyType == 0x01`, write `owner_manager(sender, ownerId) = manager`. Initialize lock state to safe defaults: `locked = false`, `unlock_delay = 0`, `unlocks_at = 0`.
...
**Policy gate**: When the transaction's authenticating owner has `policyType == 0x01` (SENDER scope), each call is gated before dispatch. The protocol reads the owner's manager address from `owner_manager(sender, ownerId)`; if `call.to` is not that address, the call is **not dispatched** and fails deterministically with the protocol revert `PolicyViolation(bytes32 ownerId, address target)`:

```solidity
error PolicyViolation(bytes32 ownerId, address target);
```

The frame's return data is `abi.encodeWithSelector(PolicyViolation.selector, ownerId, call.to)`. This is a consensus-level result — identical on every node — not a node-local heuristic and not a validity error. Standard atomicity then applies: the enclosing phase's state changes roll back, later phases are skipped, and the phase is reported as failed in `phaseStatuses`. Only work already performed is charged — intrinsic gas plus this gate SLOAD (`owner_manager`, charged once to `gas_limit`); the undispatched call body costs nothing, and the transaction is still included with its nonce consumed. A `policyType == 0x01` owner can therefore reach exactly one address — its manager — which enforces all further policy and relays approved actions back to the account. Owners with `policyType == 0x00` skip the gate entirely.
...
If the wallet needs the verifier address or scope, it calls `getOwnerConfig(account, ownerId)` on the Account Configuration Contract. A policy manager reached as a `call.to` identifies which key it is acting for by combining `getSender()` and the authenticated `getOwnerId()` from this precompile, then loads that key's sub-policy configuration from its own storage. Per-key policy state lives in the manager (where it is installed and uninstalled), not the precompile, keeping the precompile to immutable transaction context.
...
The protocol reads everything it needs to decide transaction validity — verifier binding, scope, and policy selection — in one SLOAD on `owner_config`. The manager address is kept in a separate slot (`owner_manager`) that is read only during execution, so policy adds nothing to the validity surface: owners with `policyType == 0x00` pay nothing, and owners with `policyType == 0x01` pay one `owner_manager` SLOAD at execution time when authenticating as SENDER (see [Why a Single Policy Manager?](#why-a-single-policy-manager)).

### Why a Single Policy Manager?

Session keys and gas-payer hot wallets often need narrow authority — only this token, only this much per day, only this lending action. The `policyType == 0x01` mode expresses that by binding the key to exactly one **policy manager** address and letting the manager enforce everything else. The protocol's only policy responsibility is a single-target gate: under SENDER scope, a restricted key may call only its manager. The manager interprets *what* the key may do and relays approved actions back to the account.

This keeps the *validity* surface untouched: the gate is enforced during execution (a disallowed `call.to` reverts), not during mempool validation. The single-`owner_config`-SLOAD validity model is preserved, and changing a key's policy — installing or removing a sub-policy in the manager — never invalidates an in-flight transaction. The tradeoff is that a compromised restricted key can still get policy-violating transactions included — they revert and consume gas — rather than being filtered before execution; self-pay bounds the loss to the account's own ETH, and nodes rate-limit as usual.

**Why one manager rather than a per-target allowlist?** An earlier design let a restricted key carry an array of authorized targets, each with its own opaque commitment, so the key could call self-enforcing resources directly. A single manager is simpler on every axis that matters here:

| Concern | Array of targets | Single manager |
|---------|------------------|----------------|
| Protocol storage per key | One entry per target (non-enumerable) | One `owner_manager` slot |
| Revocation / clearing | Entries can't be bulk-cleared; stale entries resurrect on re-authorization | Clear one slot; no per-target state in the protocol |
| Owners to register | Each self-enforcing target may need to be an owner | One manager owner (enshrined manager pre-installed by default) |
| Where policy logic lives | Spread across each target (logic-in-token smell) | Centralized in the manager, composed from sub-policies |
| Composition (e.g. token + lending) | Must allowlist every touched target up front | Manager dispatches to sub-policies; no protocol change |
| Gas | Direct calls, but every action still relays through `account.execute` once self-pay/ownership is required | One extra hop through the manager; common path uses the enshrined manager |

Once every action relays through `account.execute` (required so the manager can act as the account), the array's "direct call" gas advantage mostly disappears, leaving its extra protocol state and owner-management burden with little upside. The single manager centralizes policy logic, needs no enumerable protocol state, and — via an enshrined default manager — adds no owners in the common case.

**Enshrined vs. custom managers.** `policyType == 0x01` names a manager address but does not constrain which contract it is. The canonical deployment registers an enshrined policy manager as a default owner, so the common policies — native token spend limits and external call gating (by target and selector) — are available with no deployment and no extra owner. Wallets that need richer behavior register a custom manager instead; the protocol treats both identically. Reserved `policyType` values (`0x02`–`0xFF`) leave room for future modes (e.g. a fixed enshrined-manager selector that needs no `owner_manager` slot) without burning scope bits.

**Sub-policies, not protocol primitives.** Encoding a fixed policy vocabulary — selector lists, recipient allowlists, spend limits with reset periods — into consensus would freeze that vocabulary: each new primitive would need a network upgrade and lockstep adoption. Instead the manager holds per-key configuration in its own storage and composes behavior from sub-policy modules (the pattern popularized by wallet policy frameworks): the manager enforces a primitive natively or dispatches to a sub-policy contract that returns a call plan, which the manager relays. New policy types are new sub-policy contracts — shippable permissionlessly, versioned independently — without touching consensus. Sub-policies need not be owners; the manager relays on their behalf, so adding capability to a key never adds owners to the account.

### Reference Policy Manager

*Informative.* A manager identifies the acting key from the [Transaction Context](#transaction-context) precompile (`getSender()` + `getOwnerId()`), loads that key's sub-policy configuration from its own storage, validates the requested action, and relays it back to the account via `account.execute`. The example below shows the shape; real managers persist spend state and dispatch to sub-policies.

```solidity
contract ReferenceManager {
    // Per-key config installed by the account, keyed by (account, ownerId):
    //   allowed selector + recipient allowlist + spend cap, plus persisted spend state.
    function relay(address target, bytes calldata callData) external {
        address account = ITxContext(TX_CONTEXT).getSender();
        bytes32 ownerId = ITxContext(TX_CONTEXT).getOwnerId();
        Policy storage p = policyOf[account][ownerId];      // manager-held state

        // ... check target/selector/recipient against p; update p.spentToday ...
        // (spend caps require persisted spend state in the manager)

        IAccount(account).execute(target, 0, callData);     // relay approved action
    }
}
```

A wallet provisions such a key by authorizing the owner with `policyType == 0x01` and the manager address, then installing the key's sub-policy in the manager. Wallets needing only common policies point the key at the enshrined manager and skip deployment entirely.

#### Worked Example: B20 Spend Limit + Lending

A wallet wants a session key that can (a) spend at most a fixed amount of a native token (B20) per day and (b) adjust a lending position, but nothing else.

1. **Provision.** Authorize the session-key owner with `policyType == 0x01`, `scope = SENDER`, an `expiry`, and `manager = enshrinedManager` (a default owner) — or a custom manager. Install two sub-policies in the manager for `(account, ownerId)`: a B20 daily spend cap and a lending-action policy.
2. **Use.** The session key signs a transaction whose single top-level call is to the manager. The protocol validates the key (one `owner_config` SLOAD), and at execution the gate confirms `call.to == owner_manager(account, ownerId)`.
3. **Enforce + relay.** The manager reads `getSender()`/`getOwnerId()`, looks up the two sub-policies, checks the B20 amount against the remaining daily cap (updating persisted spend state) and the lending action against its policy, then relays both actions back through `account.execute`.
4. **Expire/revoke.** When the key expires (or is revoked), its `owner_config` and `owner_manager` slots stop it from reaching the manager. The manager's per-key config is the wallet's to uninstall; the protocol holds no per-target state to clear.

### Why Owner Expiry?

Session keys are intended to be short-lived, but revocation requires a transaction, and a forgotten key lingers indefinitely. The per-owner `expiry` lets a wallet provision a key that lapses automatically at a chosen time — no revoke transaction, no lingering authority. Because `expiry` is packed into the same `owner_config` slot the protocol already reads for validity, enforcement costs nothing extra and happens at validation: an expired owner's transactions are rejected before execution rather than reverting. For manager-gated keys it also caps how long the key can reach its manager — once expired, the key can no longer relay actions even if the manager's per-key sub-policy state is never explicitly uninstalled. A value of `0` preserves the existing always-valid behavior for owners that should not expire.
...
- **Simplicity**: One primary storage slot per owner (`owner_config`). No variable-length public key encoding, no multi-slot public key reads, no length fields. Registration is a single SSTORE in the common case, with one extra SSTORE for the `owner_manager` slot when an owner uses `policyType == 0x01`.
...
interface IAccountConfiguration {
    ...
    struct OwnerConfig {
        ...
        uint8 policyType;  // 0x00 = none, 0x01 = manager-gated
        uint48 expiry;     // Unix seconds; 0 = no expiry. Owner invalid once block.timestamp > expiry
    }

    struct Owner {
        bytes32 ownerId;
        OwnerConfig config;
        address manager;   // policy manager; nonzero iff config.policyType == 0x01
    }
    ...
    event OwnerAuthorized(address indexed account, bytes32 indexed ownerId, OwnerConfig config, address manager);
    ...
    function getOwnerManager(address account, bytes32 ownerId) external view returns (address manager);
...
**Validation Surface**: For pure verifiers, invalidators are `owner_config` changes and nonce consumption. The manager binding (`owner_manager`) and the manager's per-key sub-policy state are **not** validity inputs — they are read only during execution, so changing a key's policy never invalidates a pending transaction; a call that is no longer permitted simply reverts at execution. Stateful verifiers additionally depend on traced state; invalidation tracking is a mempool concern. Policy managers MUST treat their per-key configuration as part of their own authorization state.
...
**Owner Scope and Policy**: Scope is protocol-enforced after verifier execution during validation — a verifier cannot bypass it. The manager gate is protocol-enforced during execution (a `call.to` other than the manager reverts with `PolicyViolation`) and is likewise outside verifier control; because it is an execution-time gate rather than a validity input, it is not a mempool filter for compromised keys (see [Why a Single Policy Manager?](#why-a-single-policy-manager)).

**Manager as Trust Anchor**: For a `policyType == 0x01` owner, the manager is fully trusted to enforce the key's intended limits — it is an account owner with execute authority and the key can only reach the manager. A buggy or malicious manager can relay anything the account can do. Accounts SHOULD prefer the enshrined manager or audited managers, and treat installing a custom manager with the same care as adding any execute-capable owner. The manager's per-key sub-policy state is the manager's own authorization surface; the protocol does not validate it.

**Policy State on Revocation**: `revokeOwner` clears `owner_config` and `owner_manager`, which immediately stops the key from reaching its manager — there are no per-target protocol entries to enumerate or resurrect. The manager's per-key sub-policy state is not protocol state and is not auto-cleared; wallets uninstall it through the manager when retiring a key, and an `expiry` bounds the window during which a not-yet-uninstalled key could otherwise be used.
