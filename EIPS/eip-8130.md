...
LOCK_CHANGE_TYPEHASH = keccak256(
  "SignedLockChange(address account,uint256 chainId, uint64 sequence,"
  "uint8 op, uint16 unlockDelay)")
// op: 1 = lock, 2 = unlock
...
Lock operates using the local account channel, applying an op advances `local_sequence`. `auth` is a standard `authenticator \|\| data` blob (see [Signature Format](#signature-format)) validated as **admin** (`scope == 0x00`) against `account`; Anyone may relay; authorization comes from the signature.
...
5. **Check nonce scope** (sender-context only, when `nonce_key != NONCE_KEY_MAX`): require the resolved actor be admin (`scope == 0x00`) or carry `NONCE`; otherwise reject (a restricted actor without `NONCE` may only use `nonce_key == NONCE_KEY_MAX`).
...
3. **Delegation entries** (if any): Require the sender was authenticated via the native secp256k1 path (EOA path or `K1_AUTHENTICATOR`), with `actorId == bytes32(bytes20(sender))` and admin (`scope == 0x00`). Reject if account is locked. For each entry, set `code(sender) = 0xef0100 || target` (or clear if `target` is `address(0)`). Reject if account has non-delegation bytecode.
...
1. If `account_changes` contains config change or delegation entries, read lock state for `sender`. Reject transaction if account is locked. If delegation entries are present, require the sender was authenticated via the native secp256k1 path (EOA path or `K1_AUTHENTICATOR`), with `actorId == bytes32(bytes20(sender))` and admin (`scope == 0x00`).
...
A restricted (non-admin) actor without `NONCE` is confined to `NONCE_KEY_MAX` (nonce-free), whose replay protection is a short `expiry` window (`NONCE_FREE_EXPIRY_WINDOW`): the transaction dies if not included in that window. This is deliberate it makes a freshly authorized session key usable immediately, with no nonce-channel coordination, while bounding replay exposure. An actor that instead needs an ordered, replay-protected, non-expiring sequence (e.g. a high-throughput automation key) opts in with `NONCE` and may then use any `nonce_key` in the full 2D space. Making sequenced channels a grant, rather than the default, keeps the common session-key case coordination-free and reserves ordered channels for actors that explicitly want them.
...
interface IAccountConfiguration {
    ...
    // Local channel only: the contract binds the digest to block.chainid and local_sequence + 1,
