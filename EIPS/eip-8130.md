...
8130 is specified once but adopted under one of two **profiles**. A chain MUST declare which profile it activates; the profile fixes every consensus-critical parameter that this specification otherwise leaves open — the [intrinsic-gas schedule](#intrinsic-gas), the [canonical authenticator set](#canonical-authenticator-set), and how canonical authenticators are priced. The profiles differ only in *what is allowed to vary and how it changes*, never in transaction semantics: a wallet targeting the canonical set behaves identically under both.
...
- The [canonical authenticator set](#canonical-authenticator-set) is **fixed by the protocol**: the accepted set is exactly the one enshrined at the active fork. Adding or removing an authenticator (including a post-quantum scheme) is a hard fork — not a node-configuration or companion-ERC change — so block validity never turns on an off-chain allowlist that nodes could disagree on.
- Canonical authenticators are **enshrined** and priced at their fixed `sender_auth_cost` / `payer_auth_cost` component values; the native and pure-EVM implementations MUST produce identical results (see [Portability](#portability)).
- No non-canonical authenticator is accepted on the 8130 transaction path.
...
- choose how canonical authenticators are priced — metered pure-EVM execution or a fixed enshrined cost (see [Authenticators](#authenticators));
- extend or narrow the accepted authenticator set through the [companion ERC](#canonical-authenticator-set) process and node configuration.
...
How authenticator execution is priced depends on the chain's [adoption profile](#adoption-profiles). Under the **L1 profile**, canonical authenticators are enshrined and charged a fixed, standard gas cost per authenticator (the `sender_auth_cost` / `payer_auth_cost` component values). Under the **L2 profile**, a chain MAY instead call directly into the EVM and meter as ordinary EVM execution (see [Mempool Acceptance](#mempool-acceptance) for rules). When an authenticator is enshrined, its execution MUST produce identical results to the corresponding authenticator contract.
...
The canonical authenticator set and corresponding contract addresses are deployed at deterministic CREATE2 addresses across chains and catalogued in a companion ERC (number TBD). How the set evolves depends on the chain's [adoption profile](#adoption-profiles). Under the **L1 profile** the accepted set is fixed by the protocol and changes only through a hard fork, so block validity never depends on an off-chain allowlist that nodes could disagree on. Under the **L2 profile** the set MAY be extended as new algorithms are adopted (e.g., post-quantum) through the companion ERC process and node configuration.

Nodes MUST include all canonical authenticators in their allowlist. Under the L1 profile a node MUST NOT extend the allowlist with non-canonical authenticators — the accepted set is exactly the enshrined one. Under the L2 profile a node MAY extend it, but the 8130 path is intended to use a small, standard set of signature algorithms; wallets that need cross-chain portability SHOULD rely only on the canonical set.
...
1. **Account changes.** Apply `account_changes` in order: a create entry calls `createAccount`, config-change entries call `applySignedAccountChanges` — both metered as ordinary Keystore contract calls — and a delegation entry performs a standard [EIP-7702](./eip-7702.md) delegation, metered at the standard delegation cost.
...
`max_payer_auth_gas` is a configurable node/chain setting that caps how much a payer's authenticator may add on top of the sender-signed `gas_limit`. Its **suggested value is 100,000 gas**; a chain that enshrines the canonical authenticator set natively MAY instead set it to the largest fixed authentication cost across that set, since no accepted payer authenticator can exceed it. A transaction whose resolved `payer_auth_cost` exceeds `max_payer_auth_gas` is rejected at [Mempool Acceptance](#mempool-acceptance) rather than metered, so `gasUsed <= effective_gas_limit` always holds and block-gas accounting stays bounded. For self-pay transactions `payer_auth_cost` is `0` and `effective_gas_limit == gas_limit`. Under the L1 profile `max_payer_auth_gas` is fixed under consensus (the enshrined set's maximum); under the L2 profile a chain MAY choose its own value (see [Adoption Profiles](#adoption-profiles)).
...
6. Resolve payer from `payer` and `payer_auth`:
   ...
   - In every mode, reject the transaction if the resolved `payer_auth_cost` exceeds `max_payer_auth_gas`, so total gas stays within `effective_gas_limit = gas_limit + max_payer_auth_gas` (see [Intrinsic Gas](#intrinsic-gas)).
...
| `max_payer_auth_gas` | configurable (suggested 100,000) | Cap on payer-authentication gas metered outside `gas_limit`; bounds `effective_gas_limit = gas_limit + max_payer_auth_gas`. Suggested 100,000, or the largest fixed authentication cost in the canonical set when enshrined natively. See [Intrinsic Gas](#intrinsic-gas) |
