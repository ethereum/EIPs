...
| `K1_VERIFIER_ADDRESS` | TBD <!-- TODO --> | secp256k1 (ECDSA) verifier contract address (used for implicit EOA authorization) |
...
| `NONCE_KEY_MAX` | `2^192 - 1` | Nonce-free mode sentinel (expiry-only replay protection) |
...
**Implicit EOA authorization**: An unregistered owner (`owner_config` slot is empty) is implicitly authorized if `ownerId == bytes32(bytes20(account))` and the verifier is `K1_VERIFIER_ADDRESS`. The empty slot's scope byte is `0x00` (unrestricted), granting full permissions by default. This allows every existing EOA to send AA transactions immediately without prior registration. The implicit authorization is revoked if the owner is explicitly revoked (verifier set to `address(0)`).
...
Nodes SHOULD reject `NONCE_KEY_MAX` transactions from the mempool if `expiry` exceeds a short window (e.g., 10 seconds from current time). Deduplication is handled by transaction hash.
...
Nodes MAY implement equivalent verification logic natively for well-known verifier addresses, bypassing EVM execution. The native implementation MUST produce identical results to the onchain contract.
...
| **New Accounts (No EOA)** | Created via a create entry in `account_changes` with CREATE2 address derivation; code placed at address (delegation indicator or runtime bytecode), owners + verifiers configured, `calls` handles initialization | Wallet-defined |
...
**`sender_auth_cost`**: For EOA signatures (`from` empty): 6,000 gas (ecrecover + 1 SLOAD + overhead). For configured owner signatures (`from` set): 1 SLOAD (`owner_config`) + cold code access + actual gas consumed by verifier execution.
...
The first 20 bytes identify the verifier contract address. The `data` portion is verifier-specific — each verifier defines its own wire format.

##### Validation

1. **Resolve sender**: If `from` empty, ecrecover derives the sender address (EOA path).
2. **Parse verifier**: Read the first 20 bytes of `sender_auth` as the verifier contract address.
...
5. **Authorize**: SLOAD `owner_config(from, ownerId)`. Require that the verifier address matches. **Implicit EOA rule**: if the slot is empty and `ownerId == bytes32(bytes20(from))` and verifier is `K1_VERIFIER_ADDRESS`, treat as authorized.
...
Create entries are authorized by the transaction's `sender_auth` — there is no separate create-specific authorization field. The initial `ownerId`s are salt-committed to the derived address. Config change and delegation entries carry their own `authorizer_auth` and use a sequence counter for deterministic cross-chain ordering. Nodes SHOULD enforce a configurable per-transaction limit on the number of config change and delegation entries (mempool rule).
...
rlp([
  ...
  code,               // bytes: Code placed at account address (bytecode or delegation indicator)
...
The `code` field specifies exactly what code is placed at the account address. It accepts both runtime bytecode and [EIP-7702](./eip-7702.md) delegation indicators:

- **Delegation account**: `code = 0xef0100 || walletImpl` — creates an account that delegates to `walletImpl`. The common case for new accounts using a shared wallet implementation.
- **Bytecode account**: `code = <runtime bytecode>` — creates an account with deployed bytecode. Used for accounts with custom contract code.

```
// Delegation account (default wallet)
[0x00, user_salt, 0xef0100 || DEFAULT_ACCOUNT_ADDRESS, initial_owners]

// Delegation account (custom wallet)
[0x00, user_salt, 0xef0100 || customWalletImpl, initial_owners]

// Bytecode account (full contract)
...
`DEPLOYMENT_HEADER(n)` is a fixed 14-byte EVM loader that returns the trailing code (see [Appendix: Deployment Header](#appendix-deployment-header) for the full opcode sequence). On non-8130 chains, `createAccount()` constructs `deployment_code` and passes it as init_code to CREATE2. On 8130 chains, the protocol constructs the same `deployment_code` for address derivation but places `code` directly (no execution). Both paths produce the same address for bytecode accounts — callers only provide `code`; the header is never user-facing.

For delegation accounts (`code = 0xef0100 || address`), the address is only reproducible on 8130 chains. On non-8130 chains, [EIP-3541](./eip-3541.md) prevents CREATE2 from deploying code starting with `0xef`. Cross-chain portable accounts should use runtime bytecode.
...
Delegation entries set [EIP-7702](./eip-7702.md)-style code delegation for accounts, replacing the need for an `authorization_list` in the transaction.

Delegation authorization is **EOA-only** in this version: `authorizer_auth` MUST resolve to the account's EOA owner identity (`ownerId == bytes32(bytes20(account))`) using `K1_VERIFIER_ADDRESS`, and MUST satisfy CONFIG authorization checks. Non-EOA owners are rejected for delegation operations. Future revisions MAY expand this to additional K1 owner patterns.
...
rlp([
  ...
  chain_id,           // uint64: 0 = valid on any chain
  sequence,           // uint64: monotonic ordering (shares sequence channel with config changes)
  target,             // address: delegate to this contract, or address(0) to clear
  authorizer_auth     // EOA authorization (K1, account-bound ownerId) with CONFIG authorization
])
```

The delegation is only permitted when:

- `code_size(account) == 0` (empty account), or
- `code(account)` starts with the delegation designator `0xef0100` (updating an existing delegation)
...
```
TYPEHASH = keccak256("Delegation(address account,uint64 chainId,uint64 sequence,address target)")

digest = keccak256(abi.encode(TYPEHASH, account, chainId, sequence, target))
```

Domain separation from transaction signatures (`AA_TX_TYPE`, `AA_PAYER_TYPE`) is structural — transaction hashes use `keccak256(type_byte || rlp([...]))`, which cannot produce the same prefix as `abi.encode(TYPEHASH, ...)`.

The `authorizer_auth` follows the same [Signature Format](#signature-format) as `sender_auth` (`verifier || data`). For delegation entries, it MUST resolve to the account's EOA owner identity via `K1_VERIFIER_ADDRESS` and pass CONFIG authorization checks.

##### Delegation Paths

On 8130 chains, code delegation is managed via `account_changes` delegation entries.

The Account Configuration Contract does not expose an `applyDelegation()` API in this version.

On non-8130 chains, delegation uses standard [EIP-7702](./eip-7702.md) transactions (ECDSA authority), which preserves portability for EOAs.
...
3. **Delegation entries** (if any): For each entry, set `code(account) = 0xef0100 || target` (or clear if `target` is `address(0)`). Reject transaction if account is locked. Reject if account has non-delegation bytecode.
4. **Code placement** (if create entry present): Place `code` at `from`. The code is placed directly — whether it is runtime bytecode or a delegation indicator (`0xef0100 || address`).
...
1. Parse and structurally validate `sender_auth`. Verify `account_changes` contains at most one create entry (type `0x00`, must be first) and at most one delegation entry (type `0x02`) per account. Nodes SHOULD enforce a configurable limit on the number of config change entries (type `0x01`) and delegation entries (type `0x02`).
...
4. If config change or delegation entries present in `account_changes`: reject if account is locked (see [Account Lock](#account-lock)). Simulate applying operations in sequence, skip already-applied entries. For delegation entries: verify `code_size(account) == 0` or existing delegation designator, and verify `authorizer_auth` resolves to `ownerId == bytes32(bytes20(account))` with `K1_VERIFIER_ADDRESS` and passes CONFIG authorization checks.
...
1. If `account_changes` contains config change or delegation entries, read lock state for `from`. Reject transaction if account is locked.
...
On non-8130 chains, when `code` is a delegation indicator (`0xef0100 || address`), the init_code will attempt to return it as runtime code, which [EIP-3541](./eip-3541.md) rejects. Delegation accounts are therefore only creatable via this path on 8130 chains where the protocol places code directly.
...
The default path is key `0`, which behaves like a standard sequential nonce and preserves familiar ordering semantics.

Additional `nonce_key` values allow parallel transaction lanes without nonce contention between independent workflows.

`NONCE_KEY_MAX` enables nonce-free transactions where replay protection comes from short-lived `expiry` and node-level deduplication by transaction hash. This is useful for time-sensitive operations where nonce ordering is undesirable.

This provides both durable ordering (`nonce_key` channels) and high-throughput short-lived flow (`NONCE_KEY_MAX`) in one model.
...
3. **Portability**: For bytecode accounts, same `deployment_code` produces the same address on both 8130 and non-8130 chains (see [Address Derivation](#address-derivation)). Delegation accounts (`code = 0xef0100 || address`) are 8130-only due to [EIP-3541](./eip-3541.md)
...
[EIP-7702](./eip-7702.md) introduced `authorization_list` as a transaction-level field for code delegation, with ECDSA authority. This proposal keeps delegation in `account_changes` so delegation uses the same sequencing and portable operation model as other account configuration actions.

In this version, delegation authorization is intentionally limited to the account's EOA identity (K1, account-bound `ownerId`) so accounts remain portable across non-8130 chains using standard [EIP-7702](./eip-7702.md) transactions.

Using `account_changes` on 8130 chains also decouples delegation ordering from transaction nonce — 7702 authorization tuples must match the authority's current nonce, while `account_changes` uses an independent `change_sequence` counter. Future revisions MAY expand 8130-chain delegation authorization beyond EOA-only to any verifier type.
...
**Validation Surface**: For pure verifiers, primary invalidators are `owner_config` revocation and nonce consumption. For `NONCE_KEY_MAX` transactions, validity additionally depends on the expiry window and dedup horizon. Stateful verifiers additionally depend on traced state; invalidation tracking is a mempool concern.

**Replay Protection**: Transactions include `chain_id`, 2D nonce (`nonce_key`, `nonce_sequence` packed into `nonce`), and `expiry`. For `NONCE_KEY_MAX` (nonce-free mode), replay protection relies on short-lived `expiry` and node-level deduplication by transaction hash. The mempool enforces a tight expiry window (e.g., 10 seconds) to bound the dedup window. Block builders MUST NOT include duplicate `NONCE_KEY_MAX` transactions with the same hash.
...
**Delegation Security**: Delegation entries are EOA-authorized only in this version: `authorizer_auth` must resolve via `K1_VERIFIER_ADDRESS` to `ownerId == bytes32(bytes20(account))` and pass CONFIG authorization checks. The delegation is only permitted on accounts with no code or with an existing delegation designator, preventing accidental overwrite of deployed contract bytecode. Cross-chain delegation operations (chain_id 0) share the same sequence channel as config changes, ensuring deterministic ordering.
