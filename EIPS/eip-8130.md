...
operation = rlp([
  ...
  policy,               // uint8: bit 0 disableKeyAdmin, bits 1-2 spendMode, bits 3-7 reserved
...
Each entry represents a set of key operations authorized at a specific sequence number. The `authorizer_auth` must be valid against the account's key configuration *at the point after all previous entries in the list have been applied*. The authorizer key must have `disableKeyAdmin == 0`; keys with `disableKeyAdmin == 1` cannot sign key change operations.
...
4. If `key_changes` non-empty: reject if account is locked (see [Account Lock](#account-lock)). For each entry, verify the authorizer key has `disableKeyAdmin == 0`; reject if non-admin. Otherwise, simulate applying operations in sequence, skip already-applied entries.
...
6. Apply `key_changes` to Account Config storage (if non-empty). Reject transaction if account is locked (see [Account Lock](#account-lock)). Reject if any authorizer key has `disableKeyAdmin == 1`.
...
Key struct (per keyId):
  keccak256(keyId || base_slot) + 0: key_config (uint256, packed)
    ...
    byte 20:    policy (uint8) — bit 0: disableKeyAdmin (0=admin, 1=non-admin); bits 1-2: spendMode (00=unlimited, 01=off, 10=limit, 11=reserved); bits 3-7: reserved
...
All key policy fields — verifier, expiration, spend mode, gas limit, key admin, and verifier data — fit in a single 32-byte storage slot (see [Appendix: Storage Layout](#appendix-storage-layout)). This means the protocol reads everything it needs for authorization, expiration, gas policy, and key management authority in one SLOAD — zero additional storage cost for policy enforcement. The `disableKeyAdmin` bit (bit 0) controls whether a key can sign portable key change operations; non-admin keys (`disableKeyAdmin == 1`) are restricted to transaction signing only. The `verifierData` byte gives verifiers 1 byte of mutable configuration at no extra read cost, while the reserved bits in the `policy` byte provide an extension path for future protocol flags.
...
**Key Management**: Only `msg.sender` can modify keys via local EVM calls; portable key changes require signature authorization from a key with `disableKeyAdmin == 0`. Non-admin keys (`disableKeyAdmin == 1`) cannot sign key change operations — this prevents a compromised session key from escalating privileges by adding new keys or revoking existing ones. EOA key is implicitly authorized by default; revocable via `revokeKey` (cross-chain portable). Accounts SHOULD have at least one configured key before revoking the EOA key. Per-key `expiration`, `spendMode`, and `disableKeyAdmin` provide protocol-enforced time limits, gas spending controls, and key management authority (see [Appendix: Storage Layout](#appendix-storage-layout)); cumulative spend tracking is handled by wallet code. `MAX_KEY_CHANGES` bounds per-transaction processing cost; `KEY_CHANGE_TYPE` domain separation prevents authorizer signatures from being reused as transaction signatures.
