...
| `expiry` | 21–26 | `uint48` Unix timestamp (seconds); the owner is invalid once `block.timestamp > expiry`. `0` = no expiry |
| `policyType` | 27 | Policy selector: `0x00` = no policy; any non-zero value gates the owner to its stored `policy_manager`. The specific non-zero value is interpreted by the manager, not the protocol (see [Owner Policies](#owner-policies)) |
...
```
policy_commitment(account, ownerId) → bytes32   // set when policyType != 0x00
policy_manager(account, ownerId)    → address   // set when policyType != 0x00
```
...
The `policyType` byte selects whether an owner is gated. `0x00` means no policy; any non-zero value gates the owner to a stored `manager` and carries an opaque 32-byte **commitment** (`keccak256` of the policy parameters). The protocol gates identically on every non-zero value and never interprets the specific byte; it is carried in `owner_config` for the `manager` to read as a sub-type if useful.

| `policyType` | Gate: `call.to` MUST equal | `policyData` |
|--------------|----------------------------|---------------|
| `0x00` | (no gate) | — |
| non-zero | the owner's `manager` | `manager` (20 bytes) ‖ `commitment` (32 bytes) |
...
**Scope.** The policy gate constrains only SENDER-context calls, so a policy-bearing owner MUST be authorized with a restricted `scope` that is neither unrestricted (`0x00`) nor includes CONFIG (`0x08`): a CONFIG-scoped key could authorize new, unrestricted owners and escape its policy entirely. PAYER and SIGNATURE scopes are permitted but are not policy-constrained — a policy-bearing owner with PAYER scope can sponsor gas up to the transaction's limits, and with SIGNATURE scope can produce ERC-1271 signatures the policy never sees — so wallets SHOULD grant a restricted key only the scopes it needs, typically SENDER alone.
...
**Lifecycle.** Policy state is keyed by `(account, ownerId)`; clearing on revocation and target-held parameters are covered under [Policy State on Revocation](#security-considerations).
...
owners_commitment = keccak256(
    ownerId_0 || verifier_0 || scope_0 || expiry_0 || policyType_0 || manager_0 || commitment_0 ||
    ...
    ownerId_n || verifier_n || scope_n || expiry_n || policyType_n || manager_n || commitment_n
)
...
The per-owner contribution is `ownerId || verifier || scope || expiry || policyType || manager || commitment` (112 bytes; `expiry` is the 6-byte `uint48`, `manager` the 20-byte address, and `commitment` the 32-byte value, with both `manager` and `commitment` zero unless `policyType != 0x00`). Sorting owners by `ownerId` makes the commitment order-independent.
...
1. Parse `[0x00, user_salt, code, initial_owners]` where each entry is `[verifier, ownerId, scope, expiry, policyType, policyData]`
...
4. Reject any owner whose `len(policyData)` does not match its `policyType` (`0` for `0x00`, `52` for non-zero), or any policy-bearing owner (`policyType != 0x00`) whose `scope` is unrestricted (`0x00`) or includes CONFIG (`0x08`)
...
|-------------|------|--------|-------------|
| `0x01` | `authorizeOwner` | `rlp([verifier, scope, expiry, policyType, policyData])` | Authorize a new owner. Writes `owner_config` with `verifier`, `scope`, `expiry` (`0` = no expiry), and `policyType`. Slices `policyData` into `policy_commitment`/`policy_manager`: empty for `0x00`, `manager ‖ commitment` when non-zero (see [Owner Policies](#owner-policies)); rejects mismatched `len(policyData)`, and rejects a policy-bearing owner whose `scope` is unrestricted (`0x00`) or includes CONFIG (`0x08`). Emits `OwnerAuthorized`. |
...
1. **Create entry** (if present): Register `initial_owners` in Account Config storage for `sender` — for each `[verifier, ownerId, scope, expiry, policyType, policyData]` tuple, write `owner_config` (verifier address, scope byte, `expiry`, and `policyType`) and the policy slots from `policyData` (see [Owner Policies](#owner-policies)). Initialize lock state to safe defaults: `locked = false`, `unlock_delay = 0`, `unlocks_at = 0`.
...
**Validation Surface**: For pure verifiers, invalidators are `owner_config` changes and nonce consumption. Stateful verifiers additionally depend on traced state; invalidation tracking is a mempool concern.
