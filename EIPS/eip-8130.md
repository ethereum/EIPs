...
- **Account Changes**: how accounts are created (including transaction-scoped ephemeral accounts) and how actors are added, revoked, or delegated (see [Account Changes](#account-changes)).
...
| **Ephemeral Accounts** | Created via a create-ephemeral entry in `account_changes`; address is `keccak256("EPHEMERAL" \|\| salt \|\| actors \|\| delegation)`, code is a transaction-scoped [EIP-7702](./eip-7702.md) delegation (cleared at settlement), no Keystore write. The transaction MUST be nonce-free. Recover leftover funds by submitting another create-ephemeral with the same parameters | Same `user_salt` + `initial_actors` + `delegation` recreates the address |
...
A [create-ephemeral](#create-ephemeral-entry) entry additionally **requires** `nonce_key == NONCE_KEY_MAX` (the transaction is invalid otherwise) and **forbids** the `NONCE` bit on every `initial_actor`. Admin (`scope == 0x00`) and every other grant remain expressible; sequenced nonce use is not.
...
| `account_changes` | **Empty**: No account changes. **Non-empty**: Array of typed entries: create (type `0x00`) for account deployment, create-ephemeral (type `0x03`) for a transaction-scoped account, config change (type `0x01`) for actor management, and delegation (type `0x02`) for code delegation. See [Account Changes](#account-changes) |
...
1. **Account changes.** Apply `account_changes` in order: a create entry calls `createAccount` and config-change entries call `applySignedAccountChanges` (both metered as ordinary Keystore contract calls), a delegation entry performs a standard [EIP-7702](./eip-7702.md) delegation, metered at the standard delegation cost, and a create-ephemeral entry installs a transaction-scoped delegation (cleared at settlement) without a Keystore write. Create-ephemeral gas is TBD.
...
| `bytecode_cost` | 0 if no create entry in `account_changes`. Create-ephemeral does not deploy bytecode and does not charge here. Otherwise: 32,000 (deployment base) + code deposit cost (200 gas per deployed byte). Byte costs for `code` are covered by `tx_payload_cost`; the create entry's initial-actor slot writes are covered by `account_changes_cost` |
| `account_changes_cost` | Per applied **create** entry: one `actor_config` slot write per initial actor (22,100 gas each: cold SLOAD + SSTORE set); a `POLICY` actor also writes its `policy_manager`/`policy_commitment` slots (~3 slot-sets, ~66,300 gas). Per applied **create-ephemeral** entry: TBD (no Keystore writes; transaction-scoped indicator is installed and cleared in the same transaction). Per applied **config change** entry: auth authentication cost (same model as `sender_auth_cost`) plus the storage writes for each mutated slot (`actor_config`, plus policy slots when `POLICY` is set); self-actor changes mutate the packed account-state slot instead. Per applied **delegation** entry: 4,600 gas (indicator deposit, 200 × 23 bytes). Per **skipped** config-change entry: 2,100 gas (sequence SLOAD). `0` when there is no create, create-ephemeral, config-change, or delegation entry. Expiry is not expressible at create; `policyData` bytes are charged through `tx_payload_cost` |
...
3. **Authorize**: If a create or create-ephemeral entry is present, resolve `scope` (and `policyData`) from the matching `initial_actors` entry rather than Keystore storage; require the resolved `actorId` and authenticator to match that entry. Otherwise: **Self-actor (native secp256k1) rule**: if authentication in step 2 used the native secp256k1 path (the EOA path or `K1_AUTHENTICATOR`) and `actorId == bytes32(uint256(uint160(sender)))`, resolve the self-actor from the inline default-EOA config in the packed account-state slot: reject if `DEFAULT_EOA_REVOKED` is set; otherwise take `scope` and `expiry` from the inline fields (all-zero = unrestricted, non-expiring full owner, i.e. admin). Otherwise SLOAD `actor_config(sender, actorId)`, reject if its reserved bytes are nonzero (version gate, see [Storage Layout](#storage-layout)), and require that the stored authenticator address matches the effective authenticator (this covers every other actor, including a non-secp256k1 self). In either case, if the resolved `expiry` is non-zero, also require `block.timestamp <= expiry`; an expired actor is rejected. Create and create-ephemeral actors have no expiry.
...
| `0x03` | Create ephemeral | Transaction-scoped account: same `initial_actors` shape as create, 7702 delegation for this transaction only, no Keystore write (must be first, at most one, exclusive of `0x00`/`0x01`/`0x02`) |

Create, create-ephemeral, and delegation entries are authorized by the transaction's `sender_auth` and there is no separate authorization field. The initial `actorId`s for create and create-ephemeral entries are salt-committed to the derived address. Delegation requires the sender to be the account's implicit EOA actor and admin (`scope == 0x00`). Config change entries carry their own admin `auth` and a `channel`/`sequence` for deterministic ordering and cross-chain replay (see [Config Change Authorization](#config-change-authorization)). Nodes SHOULD enforce a configurable per-transaction limit on the number of config change entries (mempool rule).
...
A create-ephemeral entry produces a **transaction-scoped** account for privacy and one-shot flows: the same `initial_actors` tuple as create, a 7702 delegation that exists only for this transaction, and **no Keystore write**. There is no `createEphemeral` Keystore function and no portable EVM equivalent of transaction-scoped delegation.
...
`initial_actors` uses the same tuple, sorting, `policyData`, and `authorizeActor` rules as [Create Entry](#create-entry). `expiry` is not expressible. Every `scope` grant is allowed **except** `NONCE` (`0x04`): reject the entry if any initial actor has `scope & NONCE != 0`. Admin (`scope == 0x00`) and `SENDER` / `POLICY` / `SELF_PAYER` / `SPONSOR_PAYER` remain expressible. The transaction itself MUST use `nonce_key == NONCE_KEY_MAX` and `nonce_sequence == 0` (see [Nonce-Free Mode](#nonce-free-mode-nonce_key_max)); a create-ephemeral entry on a sequenced `nonce_key` is invalid.

The entry MUST be the first (and only) `account_changes` entry. Combining it with create, config-change, or delegation is invalid: those persist Keystore or code state.

##### Address Derivation (Ephemeral)

Addresses are the trailing 20 bytes of a domain-separated hash over the entry. The `initial_actors` MUST be provided already sorted by `actorId` in strictly ascending order (same canonical-commitment rule as create):
...
address = keccak256(EPHEMERAL_PREFIX || user_salt || actors_commitment || delegation)[12:]
```

`EPHEMERAL_PREFIX` is the 9-byte ASCII string `"EPHEMERAL"` (see [Constants](#constants)). `user_salt` and `actors_commitment` are 32 bytes each; `delegation` is the 20-byte address. The prefix domain-separates this space from CREATE2 create addresses; `delegation` is committed so a prefunded counterfactual cannot be pointed at a different implementation. Off-chain address prediction MUST apply the same initial-actor validation as create-ephemeral, including the `NONCE`-bit ban, so a predicted address always corresponds to an entry the protocol will accept.

The same `(user_salt, initial_actors, delegation)` therefore always produces the same address. That is the recovery path: leftover ETH or tokens at the address are swept by a later create-ephemeral transaction with the same parameters. The protocol does not increment the account's EVM nonce and does not require CREATE2 freshness (`code_size == 0 && nonce == 0`); uniqueness comes from the hash, and reuse is intentional.

##### Validation (Create Ephemeral Entry)

When a create-ephemeral entry is present in `account_changes`:

1. Parse `[0x03, user_salt, delegation, initial_actors]` where each actor is `[actorId, authenticator, scope, policyData]`. Apply create's `policyData` rule. Reject if `delegation` is `address(0)`.
2. Require `initial_actors` sorted by `actorId` in strictly ascending order; reject unsorted or duplicate `actorId`s.
3. Reject if any initial actor has `scope & NONCE != 0`.
4. Require `nonce_key == NONCE_KEY_MAX` and `nonce_sequence == 0`.
5. Require `account_changes` contains no create, config-change, or delegation entry.
6. Compute `actors_commitment` and `expected` per [Address Derivation (Ephemeral)](#address-derivation-ephemeral).
7. Require `sender == expected` (`sender` MUST be set; this is the configured-actor path).
8. Validate `sender_auth` against one of `initial_actors` (resolved `actorId` and authenticator MUST match that entry). Scope and `policyData` for the rest of the transaction are taken from that entry, not from Keystore storage: there is no `actor_config` / `policy_manager` write, so the [policy gate](#call-phases) MUST read `manager` from the authenticating actor's in-entry `policyData` when `POLICY` is set.

##### Execution (Create Ephemeral)

During [Block Execution](#block-execution), a create-ephemeral entry:

1. Suppresses auto-delegation to `DEFAULT_ACCOUNT_ADDRESS`.
2. Sets `code(sender) = 0xef0100 || delegation` before `calls` (the same indicator as [Delegation Entry](#delegation-entry)).
3. Does **not** write Keystore storage, set `CONTRACT_ESTABLISHED`, initialize lock/channels, or emit `AccountCreated` / `DelegationApplied` / `ActorAuthorized`.
4. After `calls` settle (every phase succeeded or any phase reverted), clears the indicator and resets the account's code hash to the empty code hash.

The temporary code is visible to `EXTCODESIZE` / `EXTCODECOPY` / `CALL` / `DELEGATECALL` / `STATICCALL` targeting `sender` for the remainder of this transaction only. Subsequent transactions see an empty-code account unless another create-ephemeral with the same parameters is included.
...
2. **Create-ephemeral entry** (if present): apply the create-ephemeral entry (see [Execution (Create Ephemeral)](#execution-create-ephemeral)) — set `code(sender) = 0xef0100 || delegation`. No Keystore write.
3. **Config change entries** (if any): Apply operations in entry order. Reject transaction if account is locked.
4. **Delegation entries** (if any): Require admin EOA-actor delegation authority (see [Delegation Entry](#delegation-entry)). Reject if account is locked. For each entry, set `code(sender) = 0xef0100 || target` (or clear if `target` is `address(0)`). Reject if account has non-delegation bytecode.
...
| **Create ephemeral** | Type `0x03` in `account_changes`: transaction-scoped 7702, no Keystore write | Not available; there is no EVM equivalent of clearing delegation at settlement |
...
1. Parse and structurally validate `sender_auth`. Verify `account_changes` contains at most one create entry (type `0x00`) or one create-ephemeral entry (type `0x03`), must be first, and not both; and at most one delegation entry (type `0x02`). A create-ephemeral entry MUST be the sole `account_changes` entry. Nodes SHOULD enforce a configurable limit on the number of config change entries (type `0x01`).
...
3. Determine effective actor state:
   ...
   b. If create-ephemeral entry present: verify ephemeral address derivation, `NONCE`-bit ban, and `nonce_key == NONCE_KEY_MAX`; use `initial_actors` (no Keystore read)
   c. Else: read from Keystore storage
...
4. If `code_size(sender) == 0` and no create entry, no create-ephemeral entry, and no delegation entry is present in `account_changes`, auto-delegate `sender` to `DEFAULT_ACCOUNT_ADDRESS` (set code to `0xef0100 || DEFAULT_ACCOUNT_ADDRESS`). This delegation persists. A delegation entry that clears the indicator (`target = address(0)`) returns the account to code-less state, but auto-delegation re-applies on its next transaction: a code-less account cannot permanently opt out of auto-delegation without placing non-delegation bytecode (e.g. via a create entry).
...
| `EPHEMERAL_PREFIX` | ASCII `"EPHEMERAL"` (9 bytes) | Domain separator for create-ephemeral address derivation |
...
### Why Create Ephemeral?

Privacy and one-shot flows need an account that can originate `calls` (so it needs delegated wallet code for the duration of the transaction) without leaving Keystore state, persistent code, or a nonce slot. Create writes all three. Create-ephemeral is the same `initial_actors` envelope with those writes removed: a domain-separated `keccak256("EPHEMERAL" || …)` address, a 7702 indicator that is cleared at settlement, and a mandatory `NONCE_KEY_MAX` so the only protocol footprint is the existing nonce-free ring buffer.

The `NONCE` grant is banned on `initial_actors` so the entry cannot express sequenced-nonce authority; admin and every other grant remain available. Recovery of leftover ETH or tokens is another create-ephemeral with the same `user_salt`, `initial_actors`, and `delegation` — the address is stable and the protocol does not consume CREATE2 freshness. Application-level nullifiers (privacy pools) live in `calls`, not in this entry.

Create-ephemeral is 8130-native. There is no Keystore function and no non-8130 equivalent of "delegate for this transaction, then forget."
...
**Create Ephemeral**: The address commits `initial_actors` and `delegation` under the `"EPHEMERAL"` prefix, so a different authenticator or implementation cannot claim a prefunded counterfactual. No Keystore write means there is no durable `(address → actors)` map; unlinkability across transactions is the wallet's (unique salt / one-shot actor), not the protocol's. The mandatory nonce-free path avoids a permanent Nonce Manager slot; `replay_id` only suppresses the same logical transaction inside `NONCE_FREE_EXPIRY_WINDOW`. A later create-ephemeral with the same parameters is the intended recovery path and a new `replay_id` if `calls` differ. Delegated code that `SSTORE`s to the account leaves storage after the indicator is cleared; implementations used as `delegation` SHOULD be stateless. High-volume ephemeral traffic shares `REPLAY_BUFFER_CAPACITY` with every other nonce-free transaction.
