...
Account lock state is stored in a single packed 32-byte account-state slot that also holds the change sequences and an account-flags byte (so the default-EOA revoke costs no additional slot — see [Implicit EOA authorization](#account-configuration)):
...
On success, `importAccount` sets the `DEFAULT_EOA_REVOKED` flag (parity with `createAccount`), disabling the implicit native-secp256k1 owner. The flag MUST be set **after** the ERC-1271 check, not before: an EIP-7702 EOA delegated to a default account proves authorization with its own implicit default-EOA signature — the only authenticator available to it at import time — so that path must still be live while the signature is verified. An owner who wants to keep using that key past import includes the self-actorId as an explicit `K1_AUTHENTICATOR` entry in `initialActors` (lossless: still a full owner, now resolved through its config).
...
| `K1_AUTHENTICATOR` | `address(1)` | Native secp256k1 (ECDSA) authenticator — the single canonical k1 path (implicit default EOA and explicitly registered k1 actors) |
| `DEFAULT_EOA_REVOKED` | `0x01` | Account-state `flags` bit that disables the implicit default-EOA path (replaces the former `REVOKED_AUTHENTICATOR` sentinel) |
