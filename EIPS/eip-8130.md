...
The byte layout of `metadata` is not constrained by this specification; producers MAY use any encoding. To enable interoperable indexing, producers SHOULD use a deterministic [CBOR](https://www.rfc-editor.org/rfc/rfc8949) (RFC 8949, §4.2 deterministic encoding) array of **records**, where each record is a map with integer keys:

| Key | Name | Description |
| --- | --- | --- |
| `0` | type | Optional format hint for `data`. Absent or `0` = opaque/application-defined. Other values are defined by companion specifications. |
| `1` | scope | Optional. Absent = the whole transaction. A `uint` = the `calls` phase at that index. A two-element array `[phase, call]` = a specific call within a phase. |
| `2` | data | The payload byte string, opaque to this specification. |

This shape supports the common cases directly:

- **Attribution** (whole transaction): one record with the attribution payload, `scope` absent.
- **Per-phase or per-call annotation** (e.g. a payment reference on one transfer in a batch): a record whose `scope` selects that phase or call.
- **Multiple independent contributors** (e.g. application and wallet attribution): multiple records; each producer appends its own without merging into a shared blob.
- **Privacy-preserving data**: a record whose `data` is a commitment (for example a 32-byte hash) to off-chain content, revealing nothing on-chain.

A consumer that does not recognize a record's `type`, or that cannot validate its `data` against the named format, SHOULD treat that record's `data` as opaque rather than failing. Companion ERCs MAY define `type` values and their payload formats (for example, transaction attribution codes).
