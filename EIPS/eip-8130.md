...
| `0x01` | per-target allowlist | Under SENDER scope, every `call.to` MUST have a non-zero commitment in `owner_policy(account, ownerId, callTo)`, enforced during execution. |
...
A non-zero `commitment` authorizes calls to `callTo`; a zero value means the target is not authorized. The commitment is opaque to the protocol — it is policy configuration that the contract at `callTo` reads and interprets.
...
**Indexing and clearing.** Every policy write emits `PolicySet(account, ownerId, callTo, commitment)` — including clears, where `commitment == bytes32(0)` — so an indexer can reconstruct an owner's exact `owner_policy` state from logs alone, alongside `OwnerAuthorized` / `OwnerRevoked`. `revokeOwner` does **not** enumerate and clear an owner's `owner_policy` entries; a non-enumerable mapping cannot be bulk-cleared on-chain, so those entries persist in storage. They are unreachable while the owner is revoked, but would apply again if the same `ownerId` is later re-authorized with `policyType == 0x01`. Clearing is therefore wallet-managed: to retire a key's authority, a wallet issues `setPolicy(callTo, bytes32(0))` for each target it previously set (enumerated from its `PolicySet` history), or treats re-authorization as deliberately inheriting the prior policy set. The protocol never auto-clears.
...
The per-owner contribution is `ownerId || verifier || scope || policyType || expiry || policies_root` (92 bytes; `expiry` is the 6-byte `uint48`). Sorting owners by `ownerId` and policies by `callTo` makes the commitment order-independent.
...
|-------------|------|--------|-------------|
| `0x01` | `authorizeOwner` | `rlp([verifier, scope, policyType, expiry, policies])` where `policies = [[callTo, commitment], ...]` | Authorize a new owner. Writes `owner_config` with `verifier`, `scope`, `policyType`, and `expiry` (`0` = no expiry), and writes each policy entry to `owner_policy(account, ownerId, callTo)`. `policies` MUST be empty unless `policyType == 0x01`. Emits `OwnerAuthorized` and a `PolicySet` per policy entry. |
| `0x02` | `revokeOwner` | `rlp([])` | Revoke an existing owner. Deletes `owner_config` for non-EOA owners; for the implicit EOA owner (`ownerId == bytes32(bytes20(account))`), overwrites with `verifier = REVOKED_VERIFIER` (`type(uint160).max`) to prevent implicit re-authorization. Per-target policy entries are not enumerated or cleared; they persist in storage (unreachable while revoked) and would apply again if the same `ownerId` is re-authorized — clearing is wallet-managed via `setPolicy(callTo, 0)` (see [Owner Policies](#owner-policies)). Emits `OwnerRevoked`. |
| `0x03` | `setPolicy` | `rlp([callTo, commitment])` | Write `owner_policy(account, ownerId, callTo) = commitment`. `commitment == bytes32(0)` clears the entry. The owner referenced by `ownerId` MUST already exist with `policyType == 0x01`. Emits `PolicySet`. |
...
**Policy gate**: When the transaction's authenticating owner has `policyType == 0x01` (SENDER scope), each call is gated before dispatch. The protocol reads `owner_policy(sender, ownerId, call.to)`; if the commitment is zero, the call is **not dispatched** and fails deterministically with the protocol revert `PolicyViolation(bytes32 ownerId, address to)`:

```solidity
error PolicyViolation(bytes32 ownerId, address to);
```
...
```solidity
// Commitment stored in owner_policy(account, ownerId, callTo):
