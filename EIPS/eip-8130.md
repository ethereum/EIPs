...
Account-level policy is stored in a single packed 32-byte slot. Policy flags are modified via portable account change operations (`setAccountPolicy`, op_type `0x03` — see [Account Change Entry](#account-change-entry)).
...
New verifiers can be permissionlessly deployed allowing for signature algorithms without protocol changes. The protocol executes unknown verifiers in a sandbox that enforces purity at runtime. Verifier addresses MUST NOT be [EIP-7702](./eip-7702.md) delegations — reject if the code at the verifier address starts with the delegation designator (`0xef0100`). Sandbox execution is metered; nodes SHOULD enforce a configurable per-verifier gas cap. See [Appendix: Sandbox Execution Environment](#appendix-sandbox-execution-environment) for the runtime rules.
...
4. **Verify**: Run the native or sandboxed verifier.
...
Nodes MAY restrict mempool acceptance to transactions using native verifiers or a node-configured allowlist of known sandbox verifiers. With this gating, validation requires only Account Configuration state (`key_config`), nonce state and eth balances. Once a transaction validates, only two state changes can invalidate it: key revocation (`key_config` set to `address(0)`) or nonce consumption — a single SLOAD confirms continued validity.
...
**Storage layout is consensus-critical**: Because the protocol reads storage slots directly on 8130 chains, the deployed contract's storage layout becomes a consensus rule. The layout is intentionally simple — one slot per key — to minimize the likelihood of future changes. The final deployed bytecode at `ACCOUNT_CONFIG_ADDRESS` is the canonical reference.
