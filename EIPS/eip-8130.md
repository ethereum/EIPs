...
Account creation, import, signed actor changes, and locking are ordinary EVM entry points on the Account Configuration Contract, so an external factory can compose them to mint accounts into a desired end state. This is typically not expected to ever be needed, however there is one case where it is expected should [EIP-7819](./eip-7819.md) be adopted. A canonical factory would likely be designated that does a delegation deploy flow and uses the account configuration contract and checks for admin signature. This will also likely extend the CREATE flow on the native 8130 path to use this canonical factory to enable any key the 7702 delegation, not just EOA as today. 
