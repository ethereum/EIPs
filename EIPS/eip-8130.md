...
Authenticators are executed via STATICCALL. The 8130 transaction path accepts only canonical authenticators from the node allowlist (see [Canonical Authenticator Set](#canonical-authenticator-set)); these are enshrined contracts at deterministic CREATE2 addresses and are never delegated accounts, so no separate check of the authenticator's code for the delegation indicator (`0xef0100`) is required on that path. Non-canonical authenticators are reachable only within EVM execution and are registered at the account's own discretion.
