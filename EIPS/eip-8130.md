...
**Policy gate**: When the transaction's authenticating owner has `policyType == 0x01` (SENDER scope), each call is gated before dispatch. The protocol reads `owner_policy(sender, ownerId, call.to)`; if the commitment is zero, the call fails as if it had reverted (its phase is discarded and later phases skipped, per the atomicity rules above). The commitment is otherwise opaque to the protocol — the contract at `call.to` reads it to enforce its own semantics. This SLOAD is charged to `gas_limit` per distinct `call.to`. Owners with `policyType == 0x00` skip the gate entirely.
