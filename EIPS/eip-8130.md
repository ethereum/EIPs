...
When `policyType == 0x01`, a second slot holds the authorization manager address:

```
owner_authorization_manager(account, ownerId) → address authorizationManager
```
...
| `0x01` | manager-gated | Under SENDER scope, every `call.to` MUST equal the owner's authorization manager (`owner_authorization_manager(account, ownerId)`), enforced during execution. |
| `0x02`–`0xFF` | reserved | Reserved for future modes; a future ERC may lift specific values. |

A `policyType == 0x01` owner — typically a session key — may call exactly one address: its **authorization manager**. The protocol's only policy responsibility is this single-target gate; it does not interpret what the call does. The authorization manager is an application- or wallet-defined contract that decides *what* the key may do (e.g. token spend limits, call gating, lending actions). How the manager is built and how it carries out approved actions on behalf of the account are out of scope for this specification.

The manager holds each key's configuration in its own storage, keyed by the authenticated `ownerId` it reads from the [Transaction Context](#transaction-context) precompile — not in protocol storage. This keeps the protocol agnostic to the policy vocabulary: new policy behaviors are new manager logic, requiring no protocol change.

**Enforcement is at execution, not validation.** The protocol does not read `owner_authorization_manager` when checking transaction validity, so policy state is never part of the mempool validity surface — an owner's single `owner_config` SLOAD still decides validity. During [Call Execution](#call-execution), each call made under a `policyType == 0x01` owner is gated against the manager address. Enforcement applies only under SENDER scope; PAYER, SIGNATURE, and CONFIG authentication ignore `policyType`.

**Lifecycle.** The protocol-level binding (`policyType` + `owner_authorization_manager`) and any manager-held configuration are separate layers. Revoking or expiring the key clears its `owner_config` (and the `owner_authorization_manager` slot) — one or two slots, with no per-target entries to enumerate — and immediately stops the key from reaching the manager. Any per-key state inside the manager is the manager's concern; the protocol never has to enumerate or clear policy entries.
...
| `account_changes_cost` | Per applied config change entry: auth verification cost (same model as `sender_auth_cost`) + storage write costs for each mutated owner slot (`owner_config`, and `owner_authorization_manager` when `policyType == 0x01`). Per applied delegation entry: code deposit cost (200 × 23 bytes for the delegation indicator). Per skipped config change entry (already applied): 2,100 (SLOAD to check sequence). 0 if no config change or delegation entries in `account_changes` |
...
Validity does not depend on `policyType`. When the owner has `policyType == 0x01` under SENDER scope, the authorization-manager gate is enforced later, during [Call Execution](#call-execution) — not here.
...
rlp([
  ...
  initial_owners      // Array of [verifier, ownerId, scope, policyType, expiry, authorizationManager] tuples
])
```

Initial owners are registered with their specified scope, `policyType`, and `expiry` (`0` = no expiry). When `policyType == 0x01`, `authorizationManager` is the owner's authorization manager address, written to `owner_authorization_manager(sender, ownerId)`; otherwise `authorizationManager` MUST be `address(0)`. Wallet initialization code can lock the account via `calls` in the execution phase (e.g., calling `lock()` on the Account Configuration Contract).
...
owners_commitment = keccak256(
    ownerId_0 || verifier_0 || scope_0 || policyType_0 || expiry_0 || authorizationManager_0 ||
    ...
    ownerId_n || verifier_n || scope_n || policyType_n || expiry_n || authorizationManager_n
)
...
The per-owner contribution is `ownerId || verifier || scope || policyType || expiry || authorizationManager` (80 bytes; `expiry` is the 6-byte `uint48`, `authorizationManager` the 20-byte address, `address(0)` when `policyType != 0x01`). Sorting owners by `ownerId` makes the commitment order-independent.
...
1. Parse `[0x00, user_salt, code, initial_owners]` where each entry is `[verifier, ownerId, scope, policyType, expiry, authorizationManager]`
...
4. Reject any owner with unrecognized `policyType`, a non-zero `authorizationManager` when `policyType != 0x01`, or a zero `authorizationManager` when `policyType == 0x01`
...
|-------------|------|--------|-------------|
| `0x01` | `authorizeOwner` | `rlp([verifier, scope, policyType, expiry, authorizationManager])` | Authorize a new owner. Writes `owner_config` with `verifier`, `scope`, `policyType`, and `expiry` (`0` = no expiry). When `policyType == 0x01`, writes `authorizationManager` to `owner_authorization_manager(account, ownerId)`; otherwise `authorizationManager` MUST be `address(0)`. Emits `OwnerAuthorized`. |
| `0x02` | `revokeOwner` | `rlp([])` | Revoke an existing owner. Deletes `owner_config` (and `owner_authorization_manager`) for non-EOA owners; for the implicit EOA owner (`ownerId == bytes32(bytes20(account))`), overwrites with `verifier = REVOKED_VERIFIER` (`type(uint160).max`) to prevent implicit re-authorization. Any per-key state held inside the manager is the manager's concern and is not touched. Emits `OwnerRevoked`. |
...
1. **Create entry** (if present): Register `initial_owners` in Account Config storage for `sender` — for each `[verifier, ownerId, scope, policyType, expiry, authorizationManager]` tuple, write `owner_config` (verifier address, scope byte, `policyType`, and `expiry`) and, when `policyType == 0x01`, write `owner_authorization_manager(sender, ownerId) = authorizationManager`. Initialize lock state to safe defaults: `locked = false`, `unlock_delay = 0`, `unlocks_at = 0`.
...
**Policy gate**: When the transaction's authenticating owner has `policyType == 0x01` (SENDER scope), each call is gated before dispatch. The protocol reads the owner's authorization manager address from `owner_authorization_manager(sender, ownerId)`; if `call.to` is not that address, the call is **not dispatched** and fails deterministically with the protocol revert `PolicyViolation(bytes32 ownerId, address target)`:

```solidity
error PolicyViolation(bytes32 ownerId, address target);
```

The frame's return data is `abi.encodeWithSelector(PolicyViolation.selector, ownerId, call.to)`. This is a consensus-level result — identical on every node — not a node-local heuristic and not a validity error. Standard atomicity then applies: the enclosing phase's state changes roll back, later phases are skipped, and the phase is reported as failed in `phaseStatuses`. Only work already performed is charged — intrinsic gas plus this gate SLOAD (`owner_authorization_manager`, charged once to `gas_limit`); the undispatched call body costs nothing, and the transaction is still included with its nonce consumed. A `policyType == 0x01` owner can therefore reach exactly one address — its authorization manager — which enforces all further policy. Owners with `policyType == 0x00` skip the gate entirely.
...
If the wallet needs the verifier address or scope, it calls `getOwnerConfig(account, ownerId)` on the Account Configuration Contract. An authorization manager reached as a `call.to` identifies which key it is acting for by combining `getSender()` and the authenticated `getOwnerId()` from this precompile, then loads that key's configuration from its own storage. Per-key policy state lives in the manager (where it is installed and uninstalled), not the precompile, keeping the precompile to immutable transaction context.
...
The protocol reads everything it needs to decide transaction validity — verifier binding, scope, and policy selection — in one SLOAD on `owner_config`. The authorization manager address is kept in a separate slot (`owner_authorization_manager`) that is read only during execution, so policy adds nothing to the validity surface: owners with `policyType == 0x00` pay nothing, and owners with `policyType == 0x01` pay one `owner_authorization_manager` SLOAD at execution time when authenticating as SENDER (see [Why a Single Authorization Manager?](#why-a-single-authorization-manager)).

### Why a Single Authorization Manager?

Session keys and gas-payer hot wallets often need narrow authority — only this token, only this much per day, only this action. The `policyType == 0x01` mode expresses that by binding the key to exactly one **authorization manager** address and letting that contract enforce everything else. The protocol's only policy responsibility is a single-target gate: under SENDER scope, a restricted key may call only its manager. What the manager does with the call — and how it ultimately effects actions for the account — is left to the application; the protocol does not define it.

This keeps the *validity* surface untouched: the gate is enforced during execution (a disallowed `call.to` reverts), not during mempool validation. The single-`owner_config`-SLOAD validity model is preserved, and changing a key's policy inside the manager never invalidates an in-flight transaction. The tradeoff is that a compromised restricted key can still get policy-violating transactions included — they revert and consume gas — rather than being filtered before execution; self-pay bounds the loss to the account's own ETH, and nodes rate-limit as usual.

**Why one manager rather than a per-target allowlist?** An earlier design let a restricted key carry an array of authorized targets, each with its own opaque commitment. A single manager is simpler on every axis the protocol cares about:

| Concern | Array of targets | Single manager |
|---------|------------------|----------------|
| Protocol storage per key | One entry per target (non-enumerable) | One `owner_authorization_manager` slot |
...
| Where policy logic lives | Spread across each target | Centralized in one manager the key points at |
| Composition (e.g. token + lending) | Must allowlist every touched target up front | Manager composes behavior; no protocol change |
| Gate cost | One SLOAD per distinct `call.to` | One SLOAD |

The single manager keeps the protocol's footprint to one extra slot and one extra SLOAD, leaves no enumerable per-target state to clear, and moves all policy expressiveness to the application layer.

**Policy vocabulary lives in the manager, not consensus.** Encoding a fixed policy vocabulary — selector lists, recipient allowlists, spend limits with reset periods — into consensus would freeze that vocabulary: each new primitive would need a network upgrade and lockstep adoption. Instead the manager holds per-key configuration in its own storage, keyed by the authenticated `ownerId` from the [Transaction Context](#transaction-context) precompile. New policy behaviors are new manager logic — shippable permissionlessly and versioned independently — without touching consensus. Reserved `policyType` values (`0x02`–`0xFF`) leave room for future protocol-level modes without burning scope bits.

This specification deliberately stops at the gate. It does not prescribe how the manager is implemented, how a wallet installs per-key configuration, or how the manager executes resulting actions for the account — those are wallet/application concerns layered on top of the single-target binding.

### Why Owner Expiry?

Session keys are intended to be short-lived, but revocation requires a transaction, and a forgotten key lingers indefinitely. The per-owner `expiry` lets a wallet provision a key that lapses automatically at a chosen time — no revoke transaction, no lingering authority. Because `expiry` is packed into the same `owner_config` slot the protocol already reads for validity, enforcement costs nothing extra and happens at validation: an expired owner's transactions are rejected before execution rather than reverting. For manager-gated keys it also caps how long the key can reach its authorization manager — once expired, the key can no longer act even if its per-key configuration inside the manager is never explicitly uninstalled. A value of `0` preserves the existing always-valid behavior for owners that should not expire.
...
- **Simplicity**: One primary storage slot per owner (`owner_config`). No variable-length public key encoding, no multi-slot public key reads, no length fields. Registration is a single SSTORE in the common case, with one extra SSTORE for the `owner_authorization_manager` slot when an owner uses `policyType == 0x01`.
...
interface IAccountConfiguration {
    ...
    struct Owner {
        bytes32 ownerId;
        OwnerConfig config;
        address authorizationManager;   // nonzero iff config.policyType == 0x01
    }
    ...
    event OwnerAuthorized(address indexed account, bytes32 indexed ownerId, OwnerConfig config, address authorizationManager);
    ...
    function getOwnerAuthorizationManager(address account, bytes32 ownerId) external view returns (address authorizationManager);
...
**Validation Surface**: For pure verifiers, invalidators are `owner_config` changes and nonce consumption. The manager binding (`owner_authorization_manager`) and any manager-held per-key state are **not** validity inputs — they are read only during execution, so changing a key's policy never invalidates a pending transaction; a call that is no longer permitted simply reverts at execution. Stateful verifiers additionally depend on traced state; invalidation tracking is a mempool concern. Authorization managers MUST treat their per-key configuration as part of their own authorization state.
...
**Owner Scope and Policy**: Scope is protocol-enforced after verifier execution during validation — a verifier cannot bypass it. The authorization-manager gate is protocol-enforced during execution (a `call.to` other than the manager reverts with `PolicyViolation`) and is likewise outside verifier control; because it is an execution-time gate rather than a validity input, it is not a mempool filter for compromised keys (see [Why a Single Authorization Manager?](#why-a-single-authorization-manager)).

**Manager as Trust Anchor**: For a `policyType == 0x01` owner, the authorization manager is fully trusted to enforce the key's intended limits — the key can only reach the manager, and the manager decides what happens next. A buggy or malicious manager can do anything its own authority over the account allows. Accounts SHOULD point restricted keys only at audited managers and treat installing a manager with the same care as granting that contract authority over the account. The manager's per-key state is its own authorization surface; the protocol does not validate it. How the manager obtains authority to act for the account is out of scope for this specification.

**Policy State on Revocation**: `revokeOwner` clears `owner_config` and `owner_authorization_manager`, which immediately stops the key from reaching its manager — there are no per-target protocol entries to enumerate or resurrect. Any per-key state inside the manager is not protocol state and is not auto-cleared; wallets uninstall it through the manager when retiring a key, and an `expiry` bounds the window during which a not-yet-uninstalled key could otherwise be used.
