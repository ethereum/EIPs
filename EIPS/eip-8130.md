...
> **Canonical contracts.** This EIP specifies the protocol surface: the transaction type, node validation and execution rules, intrinsic gas, the authenticator model, address derivation, and the account state a node reads directly. The **Keystore** and the canonical authenticator/account contracts — including exact storage packing, EIP-712 typehashes, event ABIs, and function bodies — live in the canonical repository at [`base/eip-8130`](https://github.com/base/eip-8130) (`src/Keystore.sol`), which is authoritative for all contract internals. Where this document gives a byte layout or digest, it is a normative summary; the repository is the source of truth, and any implementation MUST match it (either by calling the deployed contracts or by reproducing their behavior with static/precomputed gas values).
...
| **Existing Smart Contracts** | Already-deployed accounts (e.g., ERC-4337 wallets) register actors via `importAccount()` on the Keystore contract | Wallet-defined |
...
Each actor occupies a single `actor_config` slot containing the authenticator address, an optional expiry, and the scope bitmask. The protocol reads this slot directly during validation, so its packing is normative: `authenticator(20) ‖ expiry(6) ‖ scope(2) ‖ reserved(4)`, where `expiry` is a `uint48` Unix timestamp in **seconds** (`0` = no expiry; the actor is invalid once `block.timestamp > expiry`) and `scope` is the `uint16` permission bitmask (`0x0000` = unrestricted, also the admin predicate; see [Actor Scope](#actor-scope)). The trailing reserved bytes MUST be zero on write and act as an implicit version gate; `scope` occupies the last populated bytes so a future widening consumes reserved space and shifts nothing. The exact slot derivation and byte offsets are defined in the canonical repository (`src/Keystore.sol`).
...
|-----|-------|------|---------|
| 0 | `0x01` | SENDER | Ungated initiation: `sender_auth`; may originate transactions to any `call.to`. Carries operational authority (unless combined with `POLICY`), which also governs account-level signing (ERC-1271); see [Operational authority](#actor-scope) below and [Signature Verification](#signature-verification) |
...
- `IncrementLocalEpoch` (a local-only account change) increments `local_epoch` and resets `local_sequence` to `0`. Every local batch signed at the prior epoch — sequenced or unsequenced/JIT — no longer matches the current `local_epoch` and is rejected with `StaleEpoch`; it can never land.
...
Account lock state is stored in a single packed 32-byte account-state slot that also holds the change channels, an account-flags byte, and the inline default-EOA (self-actor) config. The protocol reads this slot's raw layout directly for mempool rate-limit tiering, so its field order and widths are normative:

| Field | Description |
|-------|-------------|
| `multichain_sequence` | `uint64` change counter for the multichain channel (`chain_id 0`); a plain monotonic counter |
| `local_sequence` | `uint32` change counter for the local channel; the low half of the signed local word |
| `local_epoch` | `uint32` local-channel epoch; the high half of the signed local word. Incrementing it invalidates every unlanded local signature (see [Config Change Authorization](#config-change-authorization)) |
...
| `lock_union` | `uint48` union field. While `UNLOCK_INITIATED` is clear it holds `unlock_delay` (seconds, `uint16` range): the notice required before config can change, which nodes read for rate-limit tiering. While `UNLOCK_INITIATED` is set it holds `unlocks_at` (the timestamp at which unlock takes effect) |
...
| reserved | Remaining byte in the packed slot; MUST be zero |

`local_epoch` and `local_sequence` are stored as two adjacent `uint32` fields but are committed together as a single `uint64` `sequence = local_epoch(high 32) ‖ local_sequence(low 32)` in a signed local batch (read via `getChangeSequences().local`). The combined word is non-zero once the account is initialized (creation/import set `local_sequence = 1`), so it doubles as the initialized flag. Exact byte offsets are defined in the canonical repository (`src/Keystore.sol`).

The packed slot is exactly 32 bytes, so the inline self-actor config and lock state cost no extra SLOAD/SSTORE beyond the account-state access already performed for the change-channel fields.
...
The `signature` is validated against the account via [ERC-1271](./eip-1271.md) `isValidSignature(digest, signature)`, binding the initial actor set to the account's existing authorization logic. `digest` is a typed (EIP-712-style) `ActorInitialization` struct hash that intentionally omits the EIP-712 domain separator (mitigating phishing via standard wallet signing flows). It binds:

- the account's own actorId (`bytes32(bytes20(account))`), so the digest cannot be replayed against another account;
- `chainId` (`0` = multichain, else the current chain), matching the `applySignedAccountChanges` replay domain;
- each initial actor structurally as `(actorId, ActorConfig{authenticator, expiry = 0, scope}, keccak256(policyData))`, under the per-actor `Actor`/`ActorConfig` typehashes.

This typed style is distinct from the packed style used for [Address Derivation](#address-derivation); that split is intentional. Imported actors are always non-expiring: implementations MUST hash `expiry = 0` into every actor and MUST NOT accept an actor-provided expiry at import. `policyData` is validated with `authorizeActor`'s frozen rule (well-formed `manager ‖ commitment` when `POLICY` is set, empty otherwise), written to `policy_manager`/`policy_commitment`, and hashed into the actor; unlike create, `manager = account` is expressible here. The exact typehash strings and digest construction are defined in the canonical repository (`src/Keystore.sol`).
...
`DEFAULT_ACCOUNT_ADDRESS` SHOULD implement [ERC-1271](./eip-1271.md) `isValidSignature` on top of the Keystore's typed-envelope `validateSignature`, returning the magic value only for an operational signer (see [Signature Verification](#signature-verification)), and SHOULD implement token receiver hooks (ERC-721, ERC-1155) to safely receive assets.
...
| `valid_after` | `uint64` Unix timestamp in **milliseconds**; encoded as a canonical minimal-length RLP integer. Transaction invalid when `block.timestamp * 1000 < valid_after`. A value of `0` means no lower bound. |
...
|-------------|------|-----------|-------------|
| `0` | `AuthorizeActor` | `abi.encode(bytes32 actorId, ActorConfig config, bytes policyData)` (see [`ActorConfig`](#ikeystore)) | Upsert an actor. Writes `actor_config` with `authenticator`, `expiry` (`0` = no expiry; a non-zero expiry MUST be strictly in the future), and `scope` verbatim (unknown scope bits stored as-is), reserved bytes zeroed. If `POLICY` is set, requires `policyData = manager ‖ commitment` (exactly 52 bytes) and writes those slots; otherwise requires empty `policyData` and clears policy slots. Does **not** enforce scope-combination exclusivity (that is use-time / protocol). Emits `ActorAuthorized` whose `actorData` is 32 bytes (`authenticator ‖ expiry ‖ scope ‖ reserved`) when `POLICY` is unset, or 84 bytes (appending `manager ‖ commitment`) when `POLICY` is set. |
...
| `2` | `IncrementLocalEpoch` | empty | **Local only.** Increments `local_epoch` and resets `local_sequence` to `0`, invalidating every unlanded local signature (sequenced and unsequenced) at the prior epoch. Consumes no sequence itself. See [Local Epoch](#local-epoch). |
...
- **Multichain** (`chain_id 0`): a plain monotonic `uint64` counter, valid on any chain, for synchronizing actor state across chains. Each applied batch consumes the counter (`sequence` MUST equal the current value, then it increments). There is no epoch and no unsequenced mode on this channel.
...
Both paths carry the same batch, share the same channels and counters, and are equally portable (Multichain for cross-chain, Local for chain-local). They differ only in transport. `applySignedAccountChanges` parses the authenticator from `auth`, resolves the admin `actorId`, verifies the batch digest, and applies each change in order: `AuthorizeActor` writes `actor_config` (and policy slots when `POLICY` is set); `RevokeActor` clears them; `IncrementLocalEpoch`, `Lock`, and `Unlock` mutate the packed account-state slot. Anyone may call it; authorization comes from the signed batch, not the caller. Authority changes and delegation are blocked while the account is locked; `Lock`/`Unlock` are the only permitted lock-state transitions (see [Account Lock](#account-lock)).
...
The protocol reads storage directly from the Keystore contract (`KEYSTORE_ADDRESS`). The storage layout is defined by the deployed contract bytecode; slot derivation follows from the contract's Solidity storage declarations. The final deployed contract source serves as the canonical reference for slot locations.
...
Authentication in this proposal *authenticates* an actor (an authenticator maps a hash + signature to an `actorId`) and scope then *authorizes* what that actor may do. 8130 defines a **typed signature envelope** that reuses this same machinery for account-level message signing, adding two things ordinary ERC-1271 lacks: **granular authority** and **auditability of who signed**. The Keystore's `validateSignature(account, hash, auth)` resolves the account/chain-scoped envelope digest, runs the named authenticator, and returns the verified `(actorId, scope)` — the identity of the signing key and its capability set — rather than a bare boolean. A consumer can therefore attribute a signature to a specific actor and make a finer-grained authorization decision than "valid / invalid".

The envelope is `sigType(1) ‖ authenticator(20) ‖ authenticator-specific data`. The leading `sigType` byte selects the replay domain the digest binds — **Local** (`0x01`, `block.chainid`) or **Multichain** (`0x02`, `chainId = 0`, all chains) — mirroring the [config-change channels](#config-change-authorization). The signed digest is `replaySafeHash(account, chainId, hash) = keccak256(SIGNED_MESSAGE_TYPEHASH, account, chainId, hash)`, a typed hash that deliberately omits the EIP-712 domain separator (anti-phishing, consistent with the other 8130 signed-message typehashes) and binds the account and chain so a signature cannot be replayed across accounts or (for Local) chains.

**Backwards compatible with ERC-1271.** This is a new signing standard for 8130 accounts that supersedes plain ERC-1271, but 8130 accounts remain ERC-1271 compatible by implementing `isValidSignature(hash, signature)` on top of `validateSignature`: they call it, return the ERC-1271 magic value when the resolved actor is **operational** (`Scopes.isOperator`: admin `scope == 0x00`, or `SENDER` without `POLICY`; see [Actor Scope](#actor-scope)), and treat any authentication revert as invalid. Operational gating lives at the account layer, not in the Keystore, so `validateSignature` stays a pure authority-reporting primitive while ERC-1271 callers still see a plain boolean. Apps that want the finer-grained path call `validateSignature` directly and gate on the returned `actorId`/`scope` (resolving a policy-gated actor's target via `getPolicy(account, actorId)`).

Signing is treated as an encoding of operational authority rather than a separate grant: an operational key can already originate calls, so letting it produce a `Permit`/order signature grants nothing new, while a `POLICY`-gated key is non-operational and its signature is rejected for ERC-1271 (a raw-hash signature would act off its gate). The [delegate authenticator](#delegate-authenticator) and replay-safe hashing extend this to nested and cross-account signing without new protocol surface. Exact typehashes and the account-side wrapper are defined in the canonical repository (`src/Keystore.sol`, `src/accounts/DefaultAccount.sol`).

**Native implementation.** Because the canonical authenticator set is small and enshrinable (see [Canonical Authenticator Set](#canonical-authenticator-set)), a chain that implements 8130 natively can evaluate this whole check — resolve the envelope digest, run the enshrined authenticator, and read the actor's `scope`/`expiry` from the packed `actor_config` slot — entirely in native code, with no STATICCALL into the EVM. Exposed as a precompile, that gives contracts and system components a native 8130 signature-verification primitive returning the signer's `(actorId, scope)` at a fixed, metered cost, and lets L2s specialize: enshrine a different authenticator subset, add native algorithms (e.g. post-quantum), or price signature verification independently, all without EVM round-trips. On non-8130 chains the identical check runs as ordinary EVM through the Keystore contract, so results match either way — the native path is a performance/customization lever, not a behavioral fork.

### Why a Local Epoch?

Session keys and just-in-time (JIT) authorizations want two things that pull against a shared counter: they should be usable in any order (no coordination), and the account should be able to cancel any it has handed out that have not yet landed. A single monotonic `local_sequence` gives ordering but forces every signature to burn the next slot, so parallel keys contend and a stale signature can still land later. Splitting the local word into `local_epoch || local_sequence` separates the two concerns: sequenced changes still consume `local_sequence` for ordered, replay-once semantics, while an epoch bump is a single blunt "cancel everything outstanding on the local channel" that resets the counter and invalidates every not-yet-landed local signature at the prior epoch (`StaleEpoch`). Crucially it cancels *signatures*, not *authority* — actors from landed batches survive — so bumping the epoch is cheap to reason about and cannot brick a live actor set. That is what makes the unsequenced/JIT mode (`UNSEQUENCED`) safe: a JIT batch is intentionally replayable within its epoch, and durable retirement is just an epoch bump batched with the reducing change (see [Local Epoch](#local-epoch)).
...
The Keystore contract is the canonical ABI surface. Its full source — exact storage packing, typehashes, event ABIs, and function bodies — lives in [`base/eip-8130`](https://github.com/base/eip-8130) (`src/Keystore.sol`) and is authoritative. The sketch below mirrors only the protocol-relevant shape; consult the repository for the complete interface.
...
**Local Epoch and Outstanding Signatures**: `IncrementLocalEpoch` cancels every unlanded **local** signature at the prior epoch (`StaleEpoch`), including unsequenced/JIT batches; it does **not** revoke live actors or affect the multichain channel (see [Local Epoch](#local-epoch)). Because an unsequenced (JIT) batch is replayable within its epoch, wallets MUST treat epoch increment — not mere non-inclusion — as the durable retirement of a JIT authorization, batching the reducing `RevokeActor`/`AuthorizeActor` with `IncrementLocalEpoch`. Since the epoch does not walk authorizer lineage, an actor added by a landed batch survives later epoch bumps and must be removed with an explicit `RevokeActor`.
