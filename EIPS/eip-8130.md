...
ERC-1271 signing (`verifySignature()`) is **not** a grant: it is authorized only for admin (`scope == 0x00`). Raw signing authority is off any policy gate and effectively equivalent to asset control (a signed approval or `Permit` moves tokens), so it is reserved to the account root. Scoped signing is expressed at the account layer via an approved-hash / approve-typed-data pattern driven by a `POLICY` key (see [Actor Policies](#actor-policies)), not by a signing scope bit.
...
**Scope.** `POLICY` is the gated initiation grant. How it composes with `SENDER` is defined in [Actor Scope](#actor-scope). It MAY combine with `SELF_PAYER` so a session key can self-pay gas (bounded by balance and `expiry`). It MAY also combine with `NONCE` so a policy key can use a sequenced `nonce_key`. A `POLICY` key cannot sign ERC-1271 (signing is admin-only); scoped signing SHOULD use an account-level approved-hash / approve-typed-data pattern (Safe `approveHash` precedent) driven by the `POLICY` key.
...
4. **Check scope**: Read the resolved `scope` byte (from the inline default-EOA config for the secp256k1 self-actor, or `actor_config` otherwise). For `sender_auth`, require `scope == 0x00 || (scope & (SENDER | POLICY)) != 0`; when `POLICY` is set the actor is gated to its `manager` regardless of `SENDER`. For sponsored `payer_auth` require SPONSOR_PAYER (or unrestricted); for `verifySignature()` and for config `auth` require admin (`scope == 0x00`). In self-pay (no `payer_auth`), additionally require SELF_PAYER (or unrestricted) on the resolved sender actor.
...
| address | `authenticator (20) \|\| data` | `payer` field | Sponsored: any authenticator. Reads payer's `actor_config`, validates against `payer` address, and requires SPONSOR_PAYER scope |
...
6. Resolve payer from `payer` and `payer_auth`:
   ...
   - `payer` = 20-byte address (sponsored): `payer_auth` uses any authenticator. Validate `payer_auth` against the `payer` address's `actor_config`. Require SPONSOR_PAYER scope on the resolved actor.
...
Session keys often need narrow authority: only this token, only this much per day, only this action. `POLICY` makes gated initiation a first-class grant, distinct from ungated `SENDER`: the key may originate transactions, but only to a single call target, bound to a signed opaque **commitment**. The protocol's only policy responsibility is the single-target gate plus storing the commitment. Allowing `POLICY | SELF_PAYER` lets a session key self-pay but has access to the full account's eth for gas.
...
interface IAccountConfiguration {
    ...
    struct ActorConfig {
        ...
        uint8 scope;        // grants bitmask; 0x00 = unrestricted (admin); 0x01 = SENDER; 0x02 = POLICY; 0x04 = NONCE; 0x08 = SELF_PAYER; 0x10 = SPONSOR_PAYER; ERC-1271 signing is admin-only (not a grant)
