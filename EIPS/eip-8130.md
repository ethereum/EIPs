...
- The account already has 8130 state — **either** change-sequence channel is non-zero. Import is a one-time bootstrap, so it requires both the local and multichain channels empty. The local channel is set by a prior [Create Entry](#create-entry) or `importAccount`; the multichain channel is advanced by any applied global (`chain_id 0`) config change, which can disable the default EOA and establish state without touching the local channel. Gating on the local channel alone would leave import open on such an account and re-open a previously revoked native key.
