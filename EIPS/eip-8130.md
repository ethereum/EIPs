...
An account manages its authentication configuration via the keystore. By default, the Keystore maps an account to its configured authenticators and stores their authorizations onchain. An account MAY instead permanently opt into [Checkpointed Keystore Mode](#checkpointed-keystore-mode), where the Keystore stores a configuration commitment and actors are supplied as authenticated transaction witnesses. Each actor is bound to an **authenticator**, an onchain contract that checks a signature and returns the actor's identity (`actorId`).
...
Unless this subsection states otherwise, every rule in this EIP applies unchanged. For a checkpointed account, an authenticated actor witness replaces reads from `actor_config`, `policy_manager`, and `policy_commitment`; references elsewhere to stored actor configuration MUST be interpreted accordingly. Scope, expiry, policy, nonce, sponsorship, batching, call execution, and signature-payload semantics are identical. `AuthorizeActor` and `RevokeActor` are invalid for checkpointed accounts; actors and policies change through successor image hashes.
...
The fields have the same meanings as their onchain equivalents. When `scope & POLICY == 0`, `policy_manager` and `policy_commitment` MUST both be zero. The actor leaf is:
...
The checkpoint is strictly increasing and MUST fit in `uint56`. The checkpointer MAY be `address(0)`, which disables snapshot validation. Multisignature or weighted authentication remains expressible through an authenticator.
...
The returned checkpoint MUST NOT exceed `2^56 - 1`. A zero returned image hash disables the snapshot for that validation and activates the chained fallback. Validation MUST invoke only the stored `settled_checkpointer`, and every configuration reconstructed under it MUST commit to the same address.

A nonzero snapshot is an authority anchor: the effective configuration MUST equal it or descend from it through a valid chained proof. An effective checkpoint below the snapshot is invalid, as is a different image hash at the same checkpoint. This permits a chain settled at checkpoint `0` to validate checkpoint `100` without replaying the intervening history. A checkpointer able to attest arbitrary snapshots is therefore trusted as configuration authority.

##### Configuration Updates and Authorization
...
`imageHash` is the successor and `wallets` MUST be empty. The struct hash is:
...
The update digest is `keccak256("\x19\x01" || domain_separator || struct_hash)`. A live admin actor from the current configuration authenticates the update. The successor checkpoint MUST be greater than the current checkpoint. Using `chainId = 0` makes the update chain-independent.

A chained proof represents `C[0] -> C[1] -> ... -> C[n]`, where `C[0]` is the snapshot or settled configuration and `C[n]` is effective. Each link reveals the current header and admin witness, authenticates its successor image hash, and strictly increases the checkpoint. The wire order is reversed: transaction authorization by `C[n]` appears first, followed by update signatures from `C[n-1]` through `C[0]`. Each element is prefixed by its three-byte length. Nested chains are invalid. The oldest recovered image hash MUST equal the active snapshot or settled image hash.

Checkpointed authorization retains the configured-actor prefix:

```text
authenticator (20 bytes) || checkpointed_authorization
```

The prefix MUST equal the authenticator in the revealed actor. A regular authorization contains:
...
Bit `0x40` indicates checkpointer data, bit `0x01` indicates a chained proof, and bit `0x02` selects the chain-independent update digest. A nonzero `settled_checkpointer` requires the outer authorization to set `0x40`, encode exactly that address, and supply the proof passed to `snapshotFor`; a zero address requires these fields to be absent. When `0x01` is set, the bytes after optional outer checkpointer data are the reverse-ordered chained list instead of a regular body. Inner elements omit checkpointer fields and inherit the outer address. Rotating the checkpointer requires `SettleImageHash`.

For a checkpointed account, the protocol replaces [configured-actor validation](#validation) with: reconstruct the effective image hash; validate it against the snapshot, settled hash, or chained proof; authenticate the revealed actor over the unchanged sender, payer, or account-change digest; require the returned `actorId` and authenticator to match the witness; then apply the existing expiry and scope rules. For a policy actor, the witnessed manager and commitment are snapshotted into the [Transaction Context](#transaction-context) for execution.

While an account is locked, checkpointed validation accepts only its settled image hash and MUST NOT advance through a snapshot or chained proof. A wallet MUST settle its desired effective image before locking.
...
For checkpointed accounts, phase 2 replaces the `actor_config` SLOAD with checkpoint proof decoding, topology hashing, optional checkpointer execution, chained-update authentication, and the acting authenticator.
...
A checkpointed create charges the writes for its settled image hash, checkpointer, and account state instead of initial actor slots. `SettleImageHash` charges its authentication and checkpointed-state writes. All checkpoint authorization bytes remain included in the existing transaction payload costs.
...
Validation applies the existing create code, address, freshness, and initial-call rules, but authenticates `sender_auth` against `initial_configuration`. On success the Keystore stores the image hash and checkpointer, sets `CHECKPOINTED` and `DEFAULT_EOA_REVOKED`, initializes the change sequences, and writes no actor or policy slots.
...
| `5` | `SettleImageHash` | `abi.encode(bytes32 imageHash, bytes32 actorsRoot, uint56 checkpoint, address checkpointer)` | **Checkpointed accounts only.** Stores a successor image hash authorized by the effective admin authenticating the batch. The header MUST reconstruct `imageHash`, and its checkpoint MUST be greater than the settled checkpoint unless that image is already settled. Updates the image hash and checkpointer atomically. |

`AuthorizeActor` and `RevokeActor` are valid only for the default onchain mode. `SettleImageHash` is valid only for checkpointed mode and is the only operation that can rotate its stored checkpointer.
...
3. Determine effective actor state:
   a. If a create entry is present: verify its mode-specific address derivation and freshness, then use its initial actors or initial checkpointed configuration
...
| `CHECKPOINTER_FLAG` | `0x40` | Authorization contains checkpointer data |
| `CHAINED_FLAG` | `0x01` | Authorization contains chained configuration updates |
...
For checkpointed accounts, a newer accepted snapshot can also invalidate a pending transaction without changing its nonce. Nodes MUST revalidate affected transactions when the settled checkpointer's accepted latest-state commitment changes.
...
**Policy State on Revocation**: In onchain mode, `revokeActor` clears `actor_config`, `policy_commitment`, and `policy_manager`. In checkpointed mode, a newer accepted image hash omits the actor. Any parameters a target keeps in its own storage are not protocol state and are not auto-cleared; wallets uninstall them through the target when retiring a key, and an `expiry` bounds the window during which a not-yet-uninstalled key could otherwise be used.

**Actor Management**: Config change authorization requires the authorizing actor be admin (`scope == 0x00`). The EOA actor is implicit only in onchain mode; checkpointed mode must commit every authority. While locked, onchain actor changes are blocked and checkpointed validation accepts only the settled image hash.
...
**Checkpointer Authority**: A checkpointer able to attest arbitrary snapshots can select an image hash containing attacker-controlled actors. Trust-minimized deployments SHOULD verify owner-authorized transitions, a validity proof, or an equivalent commitment inside `snapshotFor`.
...
**Checkpointer Liveness and Data Availability**: Wallets MUST retain configuration data and signed transitions. An unavailable checkpointer or missing witness can make a checkpointed account unusable; the chained fallback helps only when the wallet retains the required history.

**Proof Bounds**: Nodes MUST charge every proof byte, topology hash, checkpointer call, and chained authenticator execution. Proof work above `MAX_AUTHENTICATION_GAS` is invalid.

**Settlement Rollback**: `SettleImageHash` MUST accept only a successor authorized by the effective admin and MUST reject a checkpoint below the currently settled checkpoint.
