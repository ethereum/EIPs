...
Sponsorship enables arbitrary gas payment mechanisms, including ERC-20 token payments, without protocol-level token awareness.
...
Account-level policy is stored in a single packed 32-byte slot. Policy is managed via EVM methods on the Account Configuration Contract — the account calls `lock()`, `requestUnlock()`, and `unlock()` directly.

##### Account Lock

An account can lock itself to freeze its owner configuration. When locked:

- Config changes (config change entries in `account_changes`, `applyConfigChange()` via EVM) are rejected
- The lock cannot be removed without a timelock delay

**Lifecycle**:

1. **Lock**: Account calls `lock(unlockDelay)` on the Account Configuration Contract. Sets `locked = true` with the specified `unlockDelay` (seconds).
2. **Request unlock**: Account calls `requestUnlock()`. Records `unlock_requested_at = block.timestamp`.
3. **Unlock**: Account calls `unlock()`. Requires `block.timestamp >= unlock_requested_at + unlock_delay`. Clears the lock and resets `unlock_requested_at`.

All lock operations are chain-local — they use `block.timestamp` and affect only the chain where they execute. Authorization is via the account itself (`msg.sender == account`); the wallet's execution logic determines which owners can trigger lock operations.
...
| **New Accounts (No EOA)** | Created via a create entry in `account_changes` with CREATE2 address derivation; runtime bytecode placed at address, owners + verifiers configured, `call_phases` handles initialization | Wallet-defined |
...
AA_TX_TYPE || rlp([
  ...
  call_phases,        // Array of phases, each phase is an array of calls [[{to, data, value}, ...], ...] | empty
...
call = rlp([to, data, value])   // to: address, data: bytes, value: uint256
```
...
| `call_phases` | **Empty**: No calls. **Non-empty**: Array of phases, where each phase is an array of calls (`[to, data, value]`). Phases execute in order; each phase is atomic. If a phase reverts, its state is discarded and remaining phases are skipped. Completed phases persist. See [Call Execution](#call-execution) |
...
| Component | Value |
|-----------|-------|
| `tx_payload_cost` | Standard per-byte cost over the entire RLP-serialized transaction: 16 gas per non-zero byte, 4 gas per zero byte, consistent with [EIP-2028](./eip-2028.md). Ensures all transaction fields (`account_changes`, `authorization_list`, `sender_auth`, `call_phases`, etc.) are charged for data availability |
...
keccak256(AA_TX_TYPE || rlp([
  ...
  authorization_list, account_changes, call_phases,
  payer
]))
```

**Payer signature hash** — all tx fields through `call_phases`, excluding `payer`, `sender_auth`, and `payer_auth`:
...
New smart contract accounts can be created with pre-configured owners in a single transaction. The `bytecode` is the runtime code placed directly at the account address — it is not executed during deployment. The account's initialization logic runs via `call_phases` in the execution phase that follows:
...
Initial owners are registered and account policy is initialized to safe defaults (see [Execution (Account Changes)](#execution-account-changes) for exact values). No user-specified account policy is accepted during initialization — this prevents front-running attacks from setting unsafe policy. Wallet initialization code can modify policy via `call_phases` in the execution phase (e.g., calling `lock()` on the Account Configuration Contract).
...
**Common patterns**:
- **Simple call**: `[[{to, data, value}]]` — one phase, one call
...
1. ETH gas deduction from payer (sponsor for sponsored, `from` for self-pay). Transaction is invalid if payer has insufficient balance.
...
7. Execute `call_phases` in order — for each phase, execute its calls atomically (all calls in the phase succeed or all revert). If a phase reverts, discard that phase's state changes and skip remaining phases. Completed phases persist.
...
- `status` (uint8): `0x01` = all phases succeeded (or `call_phases` was empty), `0x00` = one or more phases reverted. Existing tools checking `status == 1` remain correct for the success path.
- `phaseStatuses` (uint8[]): Per-phase status array. Each entry is `0x01` (success) or `0x00` (reverted). Phases after a revert are not executed and reported as `0x00`. Empty if `call_phases` was empty.
...
- **Atomic batching**: One phase with multiple calls — all-or-nothing, works for any account including codeless EOAs.
...
The protocol dispatches each call directly to the specified `to` address with `msg.sender = from`. This gives every account — including codeless EOAs — the ability to make arbitrary calls (ETH transfers, token transfers, contract interactions) without requiring wallet code to interpret calldata. All owners are equal and fully authorized; there is no wallet-level call gating to enforce.

### Why a Default Account?

EOAs benefit from code for receiving external calls, [ERC-1271](./eip-1271.md) `isValidSignature` support, and account lock operations (which require `msg.sender == account`). Without a default, every EOA's first AA transaction requires an `authorization_list` entry to delegate to a wallet — unnecessary friction. The protocol instead auto-delegates to `DEFAULT_ACCOUNT_ADDRESS` via a standard [EIP-7702](./eip-7702.md) designator, giving every EOA a working account immediately. Custom implementations can override via `authorization_list` at any time. Signing an 8130 transaction with no code at your address implicitly approves designating to the default account.

### Why an Account Policy Slot?

Single read for account-level policies. The `locked` flag gives nodes guarantees to grant high throughput to certain accounts, which is needed for permissioned payer infrastructure or high throughput accounts. Lock operations are chain-local EVM methods rather than portable config changes because they depend on `block.timestamp` and affect only local chain state.

### Why One Slot Per Owner?

All owner data fits in a single 32-byte storage slot — just the verifier address (20 bytes) with 12 bytes reserved. The protocol reads everything it needs for authorization in one SLOAD — the entire per-owner cost is a single storage read. All authorized owners are equal, keeping the protocol simple and the storage layout minimal. Reserved bytes in the slot provide an extension path if future protocol-level owner policy is needed.

### Why These Reference Verifiers?

The reference verifier set is intentionally minimal: K1, P256_RAW, P256_WEBAUTHN, and DELEGATE.

- **K1**: Already built into the protocol for EOA signature recovery. The K1 verifier uses ecrecover internally and derives an address-based `ownerId`, enabling EOA compatibility.
...
interface IAccountConfig {
    ...
    // Account lock (chain-local, called by the account itself)
...
**Owner Management**: All authorized owners are equal — every owner has full authority over the account. Owner modifications require cryptographic authorization — either a signed config change in `account_changes` or direct verification via `owner_config` (with `isValidSignature` fallback for migration). No `msg.sender`-gated owner management functions exist, eliminating an entire class of wallet code vulnerabilities (delegatecall exploits, reentrancy, hidden multicall payloads) from escalating to owner compromise. The EOA owner is implicitly authorized by default; revocable via portable config change (cross-chain). Accounts SHOULD have at least one configured owner before revoking the EOA owner. Nodes enforce a configurable per-transaction limit on config change entries as a mempool rule.
