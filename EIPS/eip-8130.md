...
`DEFAULT_ACCOUNT_ADDRESS` SHOULD implement [ERC-1271](./eip-1271.md) by delegating account-level signature verification to the Keystore (see [Signature Verification](#signature-verification)), and SHOULD implement token receiver hooks (ERC-721, ERC-1155) to safely receive assets.
...
Authentication in this proposal *authenticates* an actor (an authenticator maps a hash + signature to an `actorId`) and scope then *authorizes* what that actor may do. Account-level ERC-1271 signing — the account answering "is this signature valid for me?" — reuses that same machinery: the Keystore's `verifySignature(account, hash, signature)` runs the named authenticator, resolves the actor, and accepts the signature when the actor is **operational** (admin, or `SENDER` without `POLICY`; see [Actor Scope](#actor-scope)). Signing is treated as an encoding of operational authority rather than a separate grant: an operational key can already originate calls, so letting it produce a `Permit`/order signature grants nothing new, while a `POLICY`-gated key must not sign raw hashes (the signature would act off its gate). The [delegate authenticator](#delegate-authenticator) and ERC-7739 replay-safe hashing extend this to nested and cross-account signing without new protocol surface.

The exact signing envelope and return types are still being finalized (see [base/eip-8130#63](https://github.com/base/eip-8130/pull/63)); a follow-up will pin the typed signature envelope. This EIP therefore specifies only that account-level signing is gated on operational authority and defers the wire encoding to the canonical repository.
...
interface IKeystore {
    ...
    // Account-level signature verification (ERC-1271; see Signature Verification) and actor authentication.
    function verifySignature(address account, bytes32 hash, bytes calldata signature) external view returns (bool);
    function authenticateActor(address account, bytes32 hash, bytes calldata auth) external view returns (uint16 scope, address policyTarget);
