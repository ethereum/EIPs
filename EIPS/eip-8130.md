...
This model is normative: two implementations that meter phases 1–3 as ordinary EVM produce identical gas and identical block validity. A chain MAY implement phases 1 and 2 (account-change application and authentication) as **native** code where applicable — for the enshrined canonical authenticator set and the Keystore reads it depends on — provided the native path produces results and prices identical to the Full-EVM model (see [Portability](#portability)). Whether phases 1–2 are metered pure-EVM or priced natively is decided per chain (see [Adoption Profiles](#adoption-profiles)); phase 3 is always ordinary EVM execution.

The per-component values below are the fixed prices a native implementation charges for phases 1–2. They reflect the EVM access and data-availability costs ([EIP-2929](./eip-2929.md) / [EIP-2028](./eip-2028.md)) at the time of writing and MUST equal what the Full-EVM model would meter for the same work. Their normative status follows the chain's [adoption profile](#adoption-profiles):
