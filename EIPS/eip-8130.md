...
- **Revocation & defaults.** Revoking the self-actor sets `DEFAULT_EOA_REVOKED` (leaving `actor_config(self)` empty and reusable); `createAccount`/`importAccount` set it by default, so a new or imported account leaves no native secp256k1 owner live unless one is among `initial_actors` (a quantum-safe default).
...
| `flags` | Account flags byte: bit 0 (`DEFAULT_EOA_REVOKED`) disables the secp256k1 self-actor; bit 1 (`LOCKED`) freezes actor configuration; bit 2 (`UNLOCK_INITIATED`) selects how the `lock_union` field is interpreted; bit 3 (`CONTRACT_ESTABLISHED`) marks the account as keystore-established (see [Account Establishment](#account-establishment)) |
...
**On the millisecond window.** The transaction window (`valid_after`/`valid_before`) is denominated in milliseconds, while on-chain actor and lock expiries are seconds; this split is deliberate and there is no second-denominated variant of these fields. Milliseconds are used only on the wire and in the signature payload — nodes do not track sub-second time. `block.timestamp` stays in seconds and is scaled (`* 1000`) solely for the two window comparisons, so a seconds-based implementation reasons entirely in seconds by treating the fields as `seconds * 1000`. Because `block.timestamp` has second granularity, the **effective** activation/expiry resolution equals the chain's block-timestamp resolution (1 second on most chains today); the millisecond field exists for off-chain clock compatibility (e.g. `Date.now()`, sequencer clocks) and forward-compatibility with sub-second block times, not for finer resolution than the chain itself provides.
...
| `account_changes_cost` | Per applied **create** entry: one `actor_config` slot write per initial actor (22,100 gas each: cold SLOAD + SSTORE set); a `POLICY` actor also writes its `policy_manager`/`policy_commitment` slots (~3 slot-sets, ~66,300 gas). Per applied **config change** entry: auth authentication cost (same model as `sender_auth_cost`) plus the storage writes for each mutated slot (`actor_config`, plus policy slots when `POLICY` is set); self-actor changes mutate the packed account-state slot instead (see [Self-Actor](#self-actor)). Per applied **delegation** entry: 4,600 gas (indicator deposit, 200 × 23 bytes). Per **skipped** config-change entry: 2,100 gas (sequence SLOAD). `0` when there is no create, config-change, or delegation entry. Expiry is not expressible at create; `policyData` bytes are charged through `tx_payload_cost` |
...
| `2` | `IncrementLocalEpoch` | empty | **Either channel.** Increments `local_epoch` and resets `local_sequence` to `0`, invalidating every unlanded local signature (sequenced and unsequenced) at the prior epoch. Always targets the local epoch, even when carried in a Multichain batch. Consumes no sequence itself. See [Local Epoch](#local-epoch). |
...
**Precompiles and native signature verification.** A precompile calling back into the EVM is undesirable and unimplemented at the time of writing, so [ERC-1271](./eip-1271.md) — which verifies a signature by calling the account's `isValidSignature` — is unavailable to chains that want signature checks inside custom precompile logic. 8130 removes that dependency: because the canonical authenticator set is small and enshrinable (see [Canonical Authenticator Set](#canonical-authenticator-set)) and authority lives in the flat `actor_config` slot, a native implementation can evaluate `validateSignature` — envelope digest, enshrined authenticator, and the actor's `scope`/`expiry` — entirely in native code and expose it as a precompile returning `(actorId, scope)`, with no EVM re-entry. On non-8130 chains the identical check runs as ordinary EVM through the Keystore, so results match either way.
