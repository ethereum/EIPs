...
To let one signed authority change verify across account models and across chains, a wallet MAY also produce a model-neutral change digest that binds only the outcome-relevant fields: a fixed domain separator, the resolved `chainId` (`0` for any chain), the `account`, the `actorId`, the prior `actor_config`, and the new `actor_config`. Such a digest omits the signer identity, the transport, and the transaction envelope, so the same signature authorizes the identical actor change whether it is applied here, through another native account-abstraction model that reads the same record, or on another chain. This is optional and does not replace the sequenced batch digest above; it is the portable authority-change shape described by the account authority lifecycle proposal (ethereum/ERCs#1979).

When such a change is applied unsequenced (just-in-time), binding the prior `actor_config` value does not by itself prevent replay, because a configuration can recur: an actor removed and later re-added returns the record to a prior value, so a stored value or a state-root membership check cannot tell a first application from a replay. To make an unsequenced change single-effect, the digest also binds a per-actor monotone counter that the keystore advances on every accepted change to that actor and preserves through revocation and expiry, so the signature names a counter value that cannot become current a second time on the same chain. This counter is stored per actor and is not part of the transaction signature, so it MUST be named as a signed field in the change for the check to bind. Sequenced changes are already single-use on each chain through the sequence counter and do not need it.

The counter starts again on a chain where the actor has never been written, so a change with `chainId` `0` applies there once even after a later change has undone it elsewhere, and anyone may apply it. A change with `chainId` `0` MUST therefore set the new `actor_config` either to empty or to a non-admin scope with a non-zero `expiry`, so that on such a chain it grants at most authority that cannot sign account changes and lapses at a fixed time. A change that narrows an existing actor is included in this rule, because on a chain without that actor it installs the record instead of narrowing it. A grant of admin scope or with no `expiry` MUST bind the executing `chainId`.
