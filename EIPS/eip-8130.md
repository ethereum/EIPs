...
4. Reject any owner whose `len(policyData)` does not match its `policyType` (`0` for `0x00`, `52` for non-zero)
...
|-------------|------|--------|-------------|
| `0x01` | `authorizeOwner` | `rlp([verifier, scope, expiry, policyType, policyData])` | Authorize a new owner. Writes `owner_config` with `verifier`, `scope`, `expiry` (`0` = no expiry), and `policyType`. Slices `policyData` into `policy_commitment`/`policy_manager`: empty for `0x00`, `manager ‖ commitment` when non-zero (see [Owner Policies](#owner-policies)); rejects mismatched `len(policyData)`. Emits `OwnerAuthorized`. |
...
Because the Account Configuration Contract is a single immutable deployment, the protocol can never grow new handling for specific `policyType` values, so it does not reserve any. It gates identically on every non-zero value and never interprets the byte; the value is carried in `owner_config` for a `manager` to read as a sub-type if useful (managers needing none simply use `0x01`). New policy behaviors are new manager logic, not new protocol values.
