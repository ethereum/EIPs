...
**`sender_auth_cost`**: For EOA signatures (`from` empty): 6,000 gas (ecrecover + 1 SLOAD + overhead). For configured owner signatures (`from` set): 1 SLOAD (`owner_config`) + cold code access + metered verifier execution gas. Nodes MAY use native implementations for known verifier addresses with fixed gas costs (e.g., K1 = 6,000, P256 = 7,000). Nodes SHOULD enforce a configurable per-verifier gas cap.
...
| **Verifier Contracts** | Protocol calls verifiers in sandbox (may use native implementations for known addresses) | Same onchain contracts callable by account config contract and wallets |
...
Enables permissionless extension of signature algorithms — deploy a new verifier contract, no protocol changes required. Pure-function verifiers allow heavy node-level optimization: nodes can use native implementations for known verifier addresses (same result, better performance) following the same pattern as precompiles, and cache verification outcomes permanently since sandbox results are fully determined by `(codehash, calldata)`. The verifier returns the `ownerId` rather than accepting it as input, so the protocol never needs algorithm-specific logic — routing, derivation, and validation are all handled by the verifier.
