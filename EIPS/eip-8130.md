...
|--------------|--------------|--------------|
| **Existing Smart Contracts** | Already-deployed accounts (e.g., ERC-4337 wallets) register keys via `addKey()` on the system contract | Wallet-defined |
...
Each account can configure an ordered array of authorized keys through the Account Configuration Contract at `ACCOUNT_CONFIG_ADDRESS`. Only `msg.sender` can modify their own configuration. This enables existing ERC-4337 smart accounts to register keys and migrate to native AA without redeployment.
...
The protocol validates `0xFF || 0x02 || ecdsa_sig` against Account A's config at index 2. Note this could also be its standard EOA signature (not required to be in the system contract).
...
Addresses are derived using CREATE2 with the Account Configuration Contract (`ACCOUNT_CONFIG_ADDRESS`) as the deployer. The `initial_keys` are incorporated into the salt to prevent front-running attacks where an attacker deploys with different keys:
...
- **On non-8130 chains**: The contracts operate as standard smart contracts. `createAccount()` works as an unstaked factory compatible with ERC-4337. Key management functions (`addKey()`, `removeKey()`, etc.) and nonce queries (`getNonce()`) operate normally through EVM execution.
...
3. Compute `expected = keccak256(0xff || ACCOUNT_CONFIG_ADDRESS || effective_salt || keccak256(init_code))[12:]`
...
2. Register `initial_keys` in system contract storage for `from`
...
7. Verify nonce (read from Nonce Manager), payer ETH balance, expiry
...
4. Process account_initialization (if non-empty): deploy `init_code`, register `initial_keys`
...
1. **Register keys**: Call `addKey()` on the system contract to add existing signing keys (K1, R1, WebAuthn, etc.) to auth config
