...
// Per-actor. Imported actors carry scope/expiry like create; policyData is empty (policy is added post-import):
...
rlp([
  ...
  initial_actors      // Array of [actorId, authenticator, scope, expiry] tuples
])
```

Each initial actor carries an `actorId`, `authenticator`, `scope` (`uint8`; `0x00` = unrestricted admin), and `expiry` (`uint48`; `0` = no expiry), all committed to the derived address (see [Address Derivation](#address-derivation)) so the counterfactual address binds each initial actor's authority. Policy is not expressible here; a policy-bearing actor is authorized with a config change entry, which MAY accompany the create entry in the same `account_changes` array for atomic setup and, running after creation, resolves `manager = account` to a concrete address. For example, a subaccount is created with an admin actor (a delegate to the primary account, `scope = 0x00`) and a restricted app key (`SENDER | PAYER | SIGNER`, or `POLICY` via an accompanying config change).
...
actors_commitment = keccak256(
    actorId_0 || authenticator_0 || scope_0 || expiry_0 ||
    ...
    actorId_n || authenticator_n || scope_n || expiry_n
)
...
The per-actor contribution is `actorId || authenticator || scope || expiry` (59 bytes: the 32-byte `actorId`, 20-byte `authenticator`, 1-byte `scope`, and 6-byte `uint48` `expiry`); policy fields do not participate. The required strictly-ascending `actorId` ordering makes the commitment canonical.
...
1. Parse `[0x00, user_salt, code, initial_actors]` where each entry is `[actorId, authenticator, scope, expiry]`. `scope` and `expiry` are stored verbatim (unknown scope bits allowed, per [Actor Scope](#actor-scope)); no `policyData` is accepted in the create entry
...
|-------------|------|--------|-------------|
| `0x01` | `authorizeActor` | `abi.encode(ActorConfig config, bytes policyData)` (see [`ActorConfig`](#iaccountconfiguration)) | Authorize a new actor. Writes `actor_config` with `authenticator`, `scope`, and `expiry` (`0` = no expiry) verbatim (unknown scope bits stored as-is). If `POLICY` is set, requires `policyData = manager ‖ commitment` (exactly 52 bytes; neither field need be nonzero) and writes those slots; otherwise requires empty `policyData` and clears policy slots. Rejects nonzero reserved bytes. Does **not** enforce scope-combination exclusivity (that is use-time / protocol). Emits `ActorAuthorized`. |
...
1. **Create entry** (if present): Register `initial_actors` in Account Config storage for `sender`, for each `[actorId, authenticator, scope, expiry]` tuple writing `actor_config` with the given `authenticator`, `scope`, and `expiry` verbatim (no policy slots; `policyData` is not expressible at create). A tuple naming the self-actorId (`actorId == bytes32(bytes20(sender))`) with `K1_AUTHENTICATOR` writes its `scope`/`expiry` into the inline default-EOA fields instead. Mark the account initialized by setting its local change-sequence channel to `1` (see [Config Change Authorization](#config-change-authorization)). Initialize lock state to safe defaults: `locked = false`, `unlock_delay = 0`, `unlocks_at = 0`. Set the `DEFAULT_EOA_REVOKED` flag so a freshly created account does not leave a native secp256k1 owner live unless one is among `initial_actors`. Place `code` at `sender`.
...
Account creation, import, signed actor changes, and locking are ordinary EVM entry points on the Account Configuration Contract, so an external factory can compose them to mint accounts into a desired end state — for example a locked account whose sole actor is `POLICY`-gated to itself and exposes a batched relay. This works on non-8130 chains and suits special, non-user accounts such as permissionless payers and privacy pools; ordinary user accounts and subaccounts use the [create entry](#create-entry) flow. The bootstrap SHOULD be atomic so the transient admin used during setup cannot be hijacked, and a sealed end state holds only when the account's own code never reaches the Configuration Contract's unlock path (unlock is gated on `msg.sender == account`, not on scope).

A canonical factory MAY standardize a user-facing variant: with [EIP-7819](./eip-7819.md) (`SETDELEGATE`), the create entry can set an [EIP-7702](./eip-7702.md)-style delegation indicator pointing at the factory, deploying delegated accounts of any key type with factory-controlled permanence. This is an optional layer over the primitives above and adds no protocol surface.
...
interface IAccountConfiguration {
    ...
    // Actor used for account creation and import. Carries scope/expiry (0x00 scope = unrestricted admin);
    // policy (manager/commitment) is NOT expressible here and is added post-deployment via config changes.
    struct InitialActor {
        bytes32 actorId;
        address authenticator;
        uint8 scope;        // 0x00 = unrestricted (admin)
        uint48 expiry;      // 0 = no expiry
    }
