...
There is exactly one privilege level that can rewrite an account's own authorization surface: the unrestricted root, `scope == 0x00`. That is the same level every other check already special-cases as admin, and every account is born with it (the implicit EOA, or an unrestricted actor named at create/import). Grants below that root (`SENDER`, `PAYER`, `SIGNATURE`, `SCOPE_POLICY`, `SCOPE_NONCE`, …) name what is actually separable; config authority is not among them, because rewriting `actor_config` includes the ability to authorize a new unrestricted actor. Admin is therefore a predicate on the existing root, not an additional grant bit.

### Why SCOPE_NONCE?

Parallel session keys should not, by default, need to coordinate over the owner's (or each other's) nonce state at all. `SCOPE_NONCE` makes nonceless (`nonce_key == NONCE_KEY_MAX`) the default for every restricted actor: a session key can be authorized and used immediately with zero nonce-channel bookkeeping, protected by `expiry` and the [Replay Identifier](#replay-identifier). A key that specifically needs an ordered, replay-protected sequence (for example, a high-frequency automation actor issuing many transactions in flight) opts in with `SCOPE_NONCE` and can then use any `nonce_key` in the full 2D key space, exactly as an admin actor can. The grant is a single bit: choose a `nonce_key` freely once it is set.
...
**Admin Compromise**: Compromise of any admin actor (`scope == 0x00`) is immediately a full compromise of the account: the attacker can authorize new admin actors, revoke existing ones, and rewrite every other actor's scope. Defense in depth for the admin key (multi-factor confirmation, time-delayed recovery, social recovery, etc.) is expressed at the *verifier* layer of that scope-0 actor — for example a custom authenticator or a `manager`-style contract behind a delegate actor. A dormant recovery actor authorized with `scope == 0x00` is a dormant root and SHOULD be protected, monitored, and revoked with the same care as any other live admin key.
