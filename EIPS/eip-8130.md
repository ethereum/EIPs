...
When `policyType != 0x00`, the owner's policy is held in up to two additional slots:

```
policy_commitment(account, ownerId) → bytes32   // set for 0x01 and 0x02
policy_manager(account, ownerId)    → address   // set for 0x02 only
```

These slots are read only during execution (see [Owner Policies](#owner-policies)); validity is still decided by the single `owner_config` SLOAD.
...
The `policyType` byte selects how the protocol restricts the calls an owner may make and where its signed policy lives. The protocol rejects unrecognized values to keep the selector extensible. Each policy-bearing type carries an opaque 32-byte **commitment** (`keccak256` of the policy parameters), signed as part of the owner change; the protocol enforces only a single-target call gate and never interprets the commitment.

| `policyType` | Name | Gate: `call.to` MUST equal | Policy `data` |
|--------------|------|----------------------------|---------------|
| `0x00` | none | (no gate) | — |
| `0x01` | self | `sender` (the account itself) | `commitment` (32 bytes) |
| `0x02` | custom | the owner's `manager` | `manager` (20 bytes) ‖ `commitment` (32 bytes) |
| `0x03`–`0xFF` | reserved | — | Reserved for future enshrined/predetermined managers (a fixed address implied by the value, reclaiming the `policy_manager` slot) or other modes; a future ERC may lift specific values without changing this layout. |

A policy-bearing owner — typically a session key — may call exactly one target: itself (`0x01`) or its configured `manager` (`0x02`). The contract at that target reads the owner's `commitment` (via [`getPolicy`](#iaccountconfiguration)), validates the presented policy parameters against it (`keccak256(params) == commitment`), enforces *what* the call may do, and carries out the approved action. The protocol's only responsibility is the single-target gate; how the target enforces the commitment and how it acts on behalf of the account are out of scope for this specification.

**The commitment is signed, opaque, and protocol-stored.** Because it is part of the signed owner change, one signature fully describes the key's authority — manager (if any) and exact policy — and travels with the portable, multichain owner-change path. Because it is opaque (one word), the protocol stays agnostic to the policy vocabulary: new policy behaviors are new target logic, requiring no protocol change.

**Enforcement is at execution, not validation.** The protocol does not read `policy_commitment` or `policy_manager` when checking transaction validity, so policy state is never part of the mempool validity surface — an owner's single `owner_config` SLOAD still decides validity. During [Call Execution](#call-execution), each call made under a policy-bearing owner is gated against the resolved target. Enforcement applies only under SENDER scope; PAYER, SIGNATURE, and CONFIG authentication ignore `policyType`.

**Lifecycle.** The protocol-level policy (`policyType`, `policy_commitment`, `policy_manager`) is keyed by `(account, ownerId)`, so revoking or expiring the key clears its `owner_config` and those one-or-two policy slots exactly — no per-target map to enumerate, full storage reclaim, and no resurrection on re-authorization. Any per-key state a target keeps in its own storage is the target's concern; the protocol never has to enumerate or clear policy entries.
...
| `account_changes_cost` | Per applied config change entry: auth verification cost (same model as `sender_auth_cost`) + storage write costs for each mutated owner slot (`owner_config`; `policy_commitment` when `policyType` is `0x01`/`0x02`; `policy_manager` when `policyType == 0x02`). Per applied delegation entry: code deposit cost (200 × 23 bytes for the delegation indicator). Per skipped config change entry (already applied): 2,100 (SLOAD to check sequence). 0 if no config change or delegation entries in `account_changes` |
...
Validity does not depend on `policyType`. When the owner has `policyType != 0x00` under SENDER scope, the policy gate is enforced later, during [Call Execution](#call-execution) — not here.
...
rlp([
  ...
  initial_owners      // Array of [verifier, ownerId, scope, policyType, expiry, data] tuples
])
```

Initial owners are registered with their specified scope, `policyType`, and `expiry` (`0` = no expiry). The `data` field carries the owner's policy, sliced by `policyType`: empty for `0x00`; `commitment` (32 bytes) for `0x01`, written to `policy_commitment(sender, ownerId)`; `manager` (20 bytes) ‖ `commitment` (32 bytes) for `0x02`, written to `policy_manager` and `policy_commitment` (see [Owner Policies](#owner-policies)). Wallet initialization code can lock the account via `calls` in the execution phase (e.g., calling `lock()` on the Account Configuration Contract).
...
owners_commitment = keccak256(
    ownerId_0 || verifier_0 || scope_0 || policyType_0 || expiry_0 || manager_0 || commitment_0 ||
    ...
    ownerId_n || verifier_n || scope_n || policyType_n || expiry_n || manager_n || commitment_n
)
...
The per-owner contribution is `ownerId || verifier || scope || policyType || expiry || manager || commitment` (112 bytes; `expiry` is the 6-byte `uint48`, `manager` the 20-byte address — `address(0)` unless `policyType == 0x02` — and `commitment` the 32-byte value — `bytes32(0)` unless `policyType` is `0x01`/`0x02`). Sorting owners by `ownerId` makes the commitment order-independent.
...
1. Parse `[0x00, user_salt, code, initial_owners]` where each entry is `[verifier, ownerId, scope, policyType, expiry, data]`
...
4. Reject any owner with unrecognized `policyType`, or with `len(data)` not matching the type (`0` for `0x00`, `32` for `0x01`, `52` for `0x02`)
...
|-------------|------|--------|-------------|
| `0x01` | `authorizeOwner` | `rlp([verifier, scope, policyType, expiry, data])` | Authorize a new owner. Writes `owner_config` with `verifier`, `scope`, `policyType`, and `expiry` (`0` = no expiry). Slices `data` by `policyType` (empty for `0x00`; `commitment` for `0x01`; `manager` ‖ `commitment` for `0x02`) and writes `policy_commitment`/`policy_manager` accordingly; rejects mismatched `len(data)`. Emits `OwnerAuthorized`. |
| `0x02` | `revokeOwner` | `rlp([])` | Revoke an existing owner. Deletes `owner_config` (and `policy_commitment`/`policy_manager`) for non-EOA owners; for the implicit EOA owner (`ownerId == bytes32(bytes20(account))`), overwrites with `verifier = REVOKED_VERIFIER` (`type(uint160).max`) to prevent implicit re-authorization. Any per-key state held inside the target is the target's concern and is not touched. Emits `OwnerRevoked`. |
...
Both paths share the same signed owner changes and `change_sequence` counters. `applySignedOwnerChanges()` parses the verifier address from `auth`, calls the verifier to get the `ownerId`, and checks `owner_config`. `authorizeOwner` writes `owner_config` and, for policy-bearing types, the `policy_commitment` (and `policy_manager` for `0x02`) slots; `revokeOwner` clears them all. Anyone can call these functions; authorization comes from the signed operation, not the caller. All owner modification paths are blocked when the account is locked (see [Account Lock](#account-lock)).
...
1. **Create entry** (if present): Register `initial_owners` in Account Config storage for `sender` — for each `[verifier, ownerId, scope, policyType, expiry, data]` tuple, write `owner_config` (verifier address, scope byte, `policyType`, and `expiry`) and, for policy-bearing types, write `policy_commitment(sender, ownerId)` (and `policy_manager(sender, ownerId)` for `0x02`) from `data`. Initialize lock state to safe defaults: `locked = false`, `unlock_delay = 0`, `unlocks_at = 0`.
...
**Policy gate**: When the transaction's authenticating owner has `policyType != 0x00` (SENDER scope), each call is gated before dispatch. The protocol resolves the owner's allowed target — `sender` for `0x01` (self), or `policy_manager(sender, ownerId)` for `0x02` (custom) — and if `call.to` is not that address, the call is **not dispatched** and fails deterministically with the protocol revert `PolicyViolation(bytes32 ownerId, address target)`:

```solidity
error PolicyViolation(bytes32 ownerId, address target);
```

The frame's return data is `abi.encodeWithSelector(PolicyViolation.selector, ownerId, call.to)`. This is a consensus-level result — identical on every node — not a node-local heuristic and not a validity error. Standard atomicity then applies: the enclosing phase's state changes roll back, later phases are skipped, and the phase is reported as failed in `phaseStatuses`. Only work already performed is charged — intrinsic gas plus the gate's resolution (no SLOAD for `0x01`, since the target is `sender`; one `policy_manager` SLOAD for `0x02`, charged once to `gas_limit`); the undispatched call body costs nothing, and the transaction is still included with its nonce consumed. A policy-bearing owner can therefore reach exactly one address — itself (`0x01`) or its manager (`0x02`) — which reads the owner's `commitment` and enforces all further policy. Owners with `policyType == 0x00` skip the gate entirely.
...
If the wallet needs the verifier address or scope, it calls `getOwnerConfig(account, ownerId)` on the Account Configuration Contract. A policy target reached as a `call.to` identifies which key it is acting for by combining `getSender()` and the authenticated `getOwnerId()` from this precompile, then reads the owner's signed `commitment` via `getPolicy(account, ownerId)` and validates the presented policy parameters against it. The commitment lives in Account Configuration storage (where it is written and revoked), not the precompile, keeping the precompile to immutable transaction context.
...
The protocol reads everything it needs to decide transaction validity — verifier binding, scope, and policy selection — in one SLOAD on `owner_config`. The policy commitment and manager are kept in separate slots (`policy_commitment`, `policy_manager`) read only during execution, so policy adds nothing to the validity surface: owners with `policyType == 0x00` pay nothing; `0x01` (self) and `0x02` (custom) pay their policy SLOADs at execution time when authenticating as SENDER (see [Why Policy Types?](#why-policy-types)).
...
The type is a **resolution mode** — where the call must go and where the policy lives — forming a cost ladder:

| `policyType` | Target | Protocol slots | Policy SLOADs |
|--------------|--------|----------------|---------------|
| `0x00` none | — | 0 | 0 |
| `0x01` self | `sender` | 1 (`commitment`) | 1 |
| `0x02` custom | stored `manager` | 2 (`commitment` + `manager`) | 2 |
| `0x03`+ | reserved | — | — |

`0x01` (self) costs no stored target — the account is its own enforcer, so the gate is free and a single commitment slot describes the policy. `0x02` (custom) routes to a wallet-chosen manager, which needs its own plaintext slot for the gate (a full 32-byte commitment cannot share a slot with a 20-byte address), hence two slots. Reserved values (`0x03`+) are intended for future *enshrined* managers — a fixed address implied by the type, reclaiming the `manager` slot so the common case is one slot with a full commitment — addable by a future ERC with no layout change. Enshrined managers are deliberately left out of this version: they require a standardized, audited, protocol-aware contract, and the `0x02` path already lets any wallet point a key at such a contract once it exists.

**The commitment is signed, opaque, and protocol-stored.** It is part of the signed owner change, so one signature fully describes the key's authority — target and exact policy — and rides the portable, multichain owner-change path. It is opaque (one word), so the protocol never interprets the policy vocabulary; new behaviors are new target logic, shippable permissionlessly without touching consensus. Encoding a fixed vocabulary (selector lists, recipient allowlists, spend limits) into consensus would instead freeze it, requiring a fork and lockstep adoption per primitive.

**Why not a per-target allowlist?** An earlier design let a restricted key carry an array of authorized targets, each with its own commitment. Keying policy by `(account, ownerId)` instead is strictly better on the axes the protocol cares about: revocation clears one-or-two known slots (vs. a non-enumerable map that can't be bulk-cleared and resurrects on re-authorization), there is no enumerable per-target state, and the gate is a single resolution rather than one SLOAD per distinct `call.to`. Multi-target composition (token + lending) moves into the single target's own logic, where it belongs.

This specification deliberately stops at the gate and the commitment. It does not prescribe how a target enforces the commitment, how a wallet installs the corresponding parameters, or how the target executes resulting actions for the account — those are wallet/application concerns layered on top of the signed single-target binding.

### Why Owner Expiry?

Session keys are intended to be short-lived, but revocation requires a transaction, and a forgotten key lingers indefinitely. The per-owner `expiry` lets a wallet provision a key that lapses automatically at a chosen time — no revoke transaction, no lingering authority. Because `expiry` is packed into the same `owner_config` slot the protocol already reads for validity, enforcement costs nothing extra and happens at validation: an expired owner's transactions are rejected before execution rather than reverting. For policy-bearing keys it also caps how long the key can reach its target — once expired, the key can no longer act even if the policy parameters held by the target are never explicitly uninstalled. A value of `0` preserves the existing always-valid behavior for owners that should not expire.
...
- **Simplicity**: One primary storage slot per owner (`owner_config`). No variable-length public key encoding, no multi-slot public key reads, no length fields. Registration is a single SSTORE in the common case, with one extra SSTORE for `policy_commitment` (and a second for `policy_manager` under `policyType == 0x02`) when an owner carries a policy.
...
interface IAccountConfiguration {
    ...
    struct OwnerConfig {
        ...
        uint8 policyType;  // 0x00 = none, 0x01 = self, 0x02 = custom
    ...
    struct Owner {
        bytes32 ownerId;
        OwnerConfig config;
        bytes policyData;  // sliced by policyType: empty (0x00); commitment[32] (0x01); manager[20] || commitment[32] (0x02)
    }
    ...
    event OwnerAuthorized(address indexed account, bytes32 indexed ownerId, OwnerConfig config, address policyManager, bytes32 policyCommitment);
    ...
    // Resolves the policy gate target and signed commitment by policyType:
    //   0x00 -> (address(0), bytes32(0));  0x01 -> (account, commitment)
...
**Validation Surface**: For pure verifiers, invalidators are `owner_config` changes and nonce consumption. The policy slots (`policy_commitment`, `policy_manager`) and any target-held per-key state are **not** validity inputs — they are read only during execution, so changing a key's policy never invalidates a pending transaction; a call that is no longer permitted simply reverts at execution. Stateful verifiers additionally depend on traced state; invalidation tracking is a mempool concern. Policy targets MUST treat the committed parameters as part of their own authorization state.
