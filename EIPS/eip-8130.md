...
Each actor occupies a single `actor_config` slot containing the authenticator address, scope byte, optional expiry, and an execution `lane`. When `scope` includes `SCOPE_POLICY` (`0x10`), the actor also carries a signed policy commitment and a manager address in the separate policy slots `policy_commitment`/`policy_manager` (see [Actor Policies](#actor-policies)). Actors are revoked by deleting the `actor_config` slot. The self-actor (`actorId == bytes32(bytes20(account))`) is the one exception: its native secp256k1 config is held **inline in the packed account-state slot**, the default-EOA `scope`/`expiry` fields plus the `DEFAULT_EOA_REVOKED` bit (see [Account Lock](#account-lock)), so the account's own key resolves in a single SLOAD, including the scoped-self case. The inline self has no `lane` field and always reports `lane = 0`. Its `actor_config` slot is instead reserved for a *non-secp256k1* self authenticator (e.g. a authenticator that returns the self-actorId); the inline secp256k1 self and a non-k1 self are mutually exclusive (registering one clears the other). Revoking the self-actor writes the account-state slot and leaves the `actor_config` slot empty and reusable.
...
| `lane` | 27–28 | `uint16` execution lane assignment (see [Actor Lanes](#actor-lanes)). `0` = unpinned / owner channel |
| reserved | 29–31 | Reserved for future protocol use (MUST be zero) |
...
**Implicit EOA authorization**: A native secp256k1 signature recovering to the account authenticates as the self-actor whenever the account's `DEFAULT_EOA_REVOKED` flag is not set, resolved from the inline default-EOA config. For a fresh account that inline config is all-zero, `scope` is `0x00` and `expiry` is `0` (full permissions, `lane = 0`). This allows every existing EOA to send AA transactions immediately without prior registration. When this path applies, the protocol authenticates using native ecrecover rather than calling an external authenticator contract.
...
| 1 | `0x02` | SENDER | Ungated initiation: `sender_auth` validation; may originate transactions to any `call.to` |
...
| 4 | `0x10` | POLICY | Gated initiation: `sender_auth` validation; may originate transactions only to the actor's `policy_manager` (see [Actor Policies](#actor-policies)) |

**Initiation modes.** `SENDER` and `POLICY` are mutually exclusive initiation modes, not combinable privilege bits. An actor MAY set at most one of them. `authorizeActor` MUST reject `scope` values that set both (`SENDER | POLICY`), and MUST reject any `POLICY` combination with `SIGNATURE`, `PAYER`, `CONFIG`, or unrestricted (`0x00`). In other words, a policy-bearing actor's `scope` MUST be exactly `SCOPE_POLICY` (`0x10`).

Scope authorization is applied only after an authenticator authenticates the signature and returns an `actorId`: the protocol loads that actor's `scope` and checks it against the context being authorized: `sender_auth` (SENDER or POLICY), gas payment by either the sender or a sponsor (PAYER), `verifySignature()` (SIGNATURE), or config change `auth` (CONFIG). The actor is authorized for that context when it is unrestricted (`scope == 0x00`) or the context's bit is set (`(scope & context_bit) != 0`); for `sender_auth`, POLICY satisfies the initiation check the same way SENDER does (`(scope & (SENDER | POLICY)) != 0`). Otherwise the actor is not authorized and the request is rejected.
...
The `lane` field (`uint16`) pins an actor to an execution channel. Wallets learn the assignment from the signed `authorizeActor` payload, from `ActorAuthorized`, or from `getActorConfig(account, actorId)`.

| `lane` | Meaning |
|--------|---------|
| `0` | Unpinned / owner channel. Typical for unrestricted and CONFIG keys. The inline secp256k1 self always reports `0`. |
| `1`–`65535` | Actor is permanently assigned to that lane for as long as it remains authorized. |

**Uniqueness.** Among live actors on an account, a non-zero `lane` MUST be unique. `authorizeActor` MUST reject a non-zero lane already held by another live actor. `revokeActor` frees the lane for reuse. This yields up to 65,535 concurrent pinned session keys per account without shared-sequence coordination.

**Relation to 2D nonces.** [2D Nonce Storage](#2d-nonce-storage) already provides parallel `nonce_key` channels. `lane` is the actor-config binding that lets a session key be authorized onto one of those channels and never use another: the key carries its lane in config, and clients SHOULD submit that key's transactions on the matching `nonce_key` (for example `nonce_key == uint256(lane)`). Exact mempool and validity coupling between `lane` and `nonce_key` MAY be tightened in a later revision; this specification defines the assignment and uniqueness rules in `actor_config`.
...
`SCOPE_POLICY` selects whether an actor is gated. When set, the actor is gated to a stored `manager` and carries an opaque 32-byte **commitment** whose meaning is defined by the `manager` (commonly `keccak256` of the policy parameters). Policy vocabulary lives in the commitment and manager logic, not in a protocol sub-type byte.

| `scope` | Gate: `call.to` MUST equal | `policyData` |
|---------|----------------------------|--------------|
| no `SCOPE_POLICY` | (no gate) | empty |
| exactly `SCOPE_POLICY` (`0x10`) | the actor's `manager` | `manager` (20 bytes) ‖ `commitment` (32 bytes) |
...
**Scope.** `SCOPE_POLICY` *is* the initiation capability for a gated key: it replaces ungated `SCOPE_SENDER` rather than stacking on top of it. Because SIGNATURE, PAYER, and CONFIG are not policy-constrained, a policy-bearing actor MUST NOT hold them (see [Actor Scope](#actor-scope)); otherwise a key could pass ERC-1271, sponsor gas, or mint new owners outside the manager.

**Reference flow** (non-normative). One construction of a session-key policy:

1. **Authorize.** The account authorizes the key with `scope = SCOPE_POLICY`, a non-zero `lane`, the `manager`, and a `commitment` (here `keccak256(params)`). The key holds no authority of its own, only the gate to its `manager`.
...
4. **Retire.** `revokeActor` zeroes the commitment (and frees the lane); `expiry` rejects further authentication. Either way step 3 fails for that key.

**Example** (non-normative). A subscription session key limited to **5 USDC per 30-day period** with a **two-target allowlist**: USDC (`transfer` only) and one application contract, encoded as the key's `params`. The wallet authorizes the key with exactly `SCOPE_POLICY` on its own `lane`, so it cannot pay its own gas or widen its own authority; a sponsor covers gas via `payer`/`payer_auth`. The key's **first transaction** installs the `params` at the manager and makes its first call in the same flow (steps 2 and 3); **later transactions** go straight to step 3, drawing down the same monthly counter. A `transfer` over the remaining budget, a call to any other target, or any call after the key expires or is revoked is rejected.
...
| reserved | Remaining bytes in the packed slot; MUST be zero. The inline self has no `lane` field and always reports `lane = 0` |
...
ACTORCONFIG_TYPEHASH =
  keccak256("ActorConfig(address authenticator,uint8 scope,uint48 expiry,uint16 lane)")

ACTOR_TYPEHASH =
  keccak256("Actor(bytes32 actorId,ActorConfig config,bytes policyData)"
            "ActorConfig(address authenticator,uint8 scope,uint48 expiry,uint16 lane)")
...
// Per-actor. For imported actors scope, expiry, and lane are 0 and policyData is empty:
configHash_i = keccak256(abi.encode(ACTORCONFIG_TYPEHASH, authenticator_i, scope_i, expiry_i, lane_i))
...
Initial actors are registered as unrestricted owners (`scope = 0x00`, `lane = 0`, no expiry, no policy). Only `authenticator` and `actorId` are required; scope, expiry, lane, and policy are not expressible in the create entry and default to their zero values. Additional actors with restricted scope, lanes, or policies are configured after deployment via config change entries.
...
|-------------|------|--------|-------------|
| `0x01` | `authorizeActor` | `abi.encode(ActorConfig config, bytes policyData)` (see [`ActorConfig`](#iaccountconfiguration)) | Authorize a new actor. Writes `actor_config` with `authenticator`, `scope`, `expiry` (`0` = no expiry), and `lane`. Slices `policyData` into `policy_commitment`/`policy_manager`: empty unless `scope` is exactly `SCOPE_POLICY`, in which case `manager ‖ commitment` (see [Actor Policies](#actor-policies)); rejects mismatched `len(policyData)`, rejects reserved bytes that are non-zero, rejects a non-zero `lane` already held by another live actor, and rejects any `scope` other than exactly `SCOPE_POLICY` when policy data is present (and rejects `SCOPE_POLICY` without valid policy data). Emits `ActorAuthorized`. |
...
Session keys and gas-payer hot wallets often need narrow authority: only this token, only this much per day, only this action. `SCOPE_POLICY` expresses that by making gated initiation a first-class scope mode, distinct from ungated `SCOPE_SENDER`: the key may originate transactions, but only to a single call target, and is bound to a signed, opaque **commitment** that the target enforces. The protocol's only policy responsibility is the single-target gate plus storing the commitment; what the target does with the call, and how it ultimately effects actions for the account, is left to the application.

Because the Account Configuration Contract is a single immutable deployment, policy vocabulary is not encoded as protocol sub-type bytes. New policy behaviors are new manager logic, not new consensus values. Encoding policy primitives (selector lists, allowlists, spend limits) into consensus would freeze the vocabulary and require a fork per new primitive; an opaque commitment keeps the protocol agnostic and new behaviors shippable permissionlessly.

### Why Actor Lanes?

Parallel session keys should not share a single account nonce or rely on nonce-free inclusion to avoid coordinating with each other. Assigning a `lane` at authorize time pins each key to one of up to 65,535 execution channels; the key learns its lane from the signed actor config and cannot migrate to another while authorized. Revocation frees the lane. This complements [2D nonces](#2d-nonce-storage): `nonce_key` is the channel in the transaction, and `lane` is the durable actor-config binding onto that channel.
...
interface IAccountConfiguration {
    ...
    struct ActorConfig {
        ...
        uint8 scope;       // 0x00 = unrestricted; 0x10 = SCOPE_POLICY (exactly, when gated)
        uint48 expiry;     // Unix seconds; 0 = no expiry. Actor invalid once block.timestamp > expiry
        uint16 lane;       // execution lane; 0 = unpinned / owner. Non-zero MUST be unique among live actors
    }

    // Minimal actor used for account creation and import (always unrestricted: scope=0x00, lane=0, no expiry)
    struct InitialActor {
        bytes32 actorId;
        address authenticator;
    }
    ...
    function authenticateActor(address account, bytes32 hash, bytes calldata auth) external view returns (uint8 scope, address policyTarget);
    ...
    // Resolves the gate target and signed commitment:
    //   no SCOPE_POLICY -> (address(0), bytes32(0)); SCOPE_POLICY -> (manager, commitment)
...
**Actor Scope and Policy**: Scope is protocol-enforced after authenticator execution during validation. The policy gate is protocol-enforced during execution (a `call.to` other than the resolved target reverts with `ActorPolicyViolation`). Because it is an execution-time gate rather than a validity input, it is not a mempool filter for compromised keys (see [Why Actor Policies?](#why-actor-policies)). `SCOPE_POLICY` is an initiation mode mutually exclusive with `SCOPE_SENDER` and MUST NOT be combined with SIGNATURE, PAYER, or CONFIG; those privileges are not policy-constrained and would otherwise escape the manager (see [Actor Policies](#actor-policies)).
