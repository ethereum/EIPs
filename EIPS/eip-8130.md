...
When `policyType != 0x00`, the owner's policy is held in two additional slots:

```
policy_commitment(account, ownerId) → bytes32   // set for policyType 0x01
policy_manager(account, ownerId)    → address   // set for policyType 0x01
```
...
The `policyType` byte selects whether an owner is gated. `0x00` means no policy; `0x01` gates the owner to a stored `manager` and carries an opaque 32-byte **commitment** (`keccak256` of the policy parameters). Values `0x02`–`0xFF` are reserved and MUST be rejected.

| `policyType` | Gate: `call.to` MUST equal | Policy `data` |
|--------------|----------------------------|---------------|
| `0x00` | (no gate) | — |
| `0x01` | the owner's `manager` | `manager` (20 bytes) ‖ `commitment` (32 bytes) |
| `0x02`–`0xFF` | reserved | rejected |

A policy-bearing owner — typically a session key — may call exactly one target: its configured `manager`. The contract at that target reads the owner's `commitment` (via [`getPolicy`](#iaccountconfiguration)), validates the presented policy parameters against it (`keccak256(params) == commitment`), enforces *what* the call may do, and carries out the approved action. The protocol's only responsibility is the single-target gate; how the target enforces the commitment and how it acts on behalf of the account are out of scope for this specification.

A key that should be enforced by the account's own code rather than a separate contract sets `manager = account` — self-enforcement needs no dedicated `policyType`.

**Reserved values.** The protocol implements a single policy type (`0x01`); `0x02`–`0xFF` are rejected today and reserved for future protocol-level resolution modes introduced by a companion ERC — for example an *enshrined* manager whose address is implied by the value, reclaiming the `manager` slot for a full commitment. Keeping the range protocol-owned (rather than letting managers assign ad-hoc meaning) leaves these values free to standardize later.

**Reference flow** (non-normative). A session-key policy with a custom manager:

1. The account authorizes the key with `policyType = 0x01`, a `manager`, and `commitment = keccak256(params)`.
2. The account installs `params` at the manager, which checks that `getPolicy(account, ownerId)` resolves to itself with a matching `commitment`.
3. On each use, the protocol gate routes the key's call to the manager; the manager re-reads `getPolicy(account, ownerId)`, requires `keccak256(presented params) == commitment`, enforces them, and acts for the account.
4. `revokeOwner` (or `expiry`) clears the slots, so step 3 then fails for that key.

**The commitment is signed, opaque, and protocol-stored.** Because it is part of the signed owner change, one signature fully describes the key's authority — its manager and exact policy — and travels with the portable, multichain owner-change path. Because it is opaque (one word), the protocol stays agnostic to the policy vocabulary: new policy behaviors are new target logic, requiring no protocol change.
...
**Lifecycle.** Policy state (`policyType`, `policy_commitment`, `policy_manager`) is keyed by `(account, ownerId)` and cleared exactly on revoke or expiry; any parameters the target keeps in its own storage are the target's concern (see [Policy State on Revocation](#security-considerations)).
...
Initial owners are registered with their specified scope, `policyType`, and `expiry` (`0` = no expiry). The `data` field carries the owner's policy and is sliced by `policyType` into `policy_commitment`/`policy_manager` exactly as for [`authorizeOwner`](#owner-policies). Wallet initialization code can lock the account via `calls` in the execution phase (e.g., calling `lock()` on the Account Configuration Contract).
...
The per-owner contribution is `ownerId || verifier || scope || policyType || expiry || manager || commitment` (112 bytes; `expiry` is the 6-byte `uint48`, `manager` the 20-byte address, and `commitment` the 32-byte value — both `manager` and `commitment` are zero unless `policyType == 0x01`). Sorting owners by `ownerId` makes the commitment order-independent.
...
4. Reject any owner with a reserved `policyType` (`0x02`–`0xFF`), or whose `len(data)` does not match its `policyType` (`0` for `0x00`, `52` for `0x01`)
...
|-------------|------|--------|-------------|
| `0x01` | `authorizeOwner` | `rlp([verifier, scope, policyType, expiry, data])` | Authorize a new owner. Writes `owner_config` with `verifier`, `scope`, `policyType`, and `expiry` (`0` = no expiry). Slices `data` into `policy_commitment`/`policy_manager` — empty for `0x00`, `manager ‖ commitment` for `0x01` (see [Owner Policies](#owner-policies)); rejects reserved `policyType` and mismatched `len(data)`. Emits `OwnerAuthorized`. |
| `0x02` | `revokeOwner` | `rlp([])` | Revoke an existing owner. Deletes `owner_config` (and `policy_commitment`/`policy_manager`) for non-EOA owners; for the implicit EOA owner (`ownerId == bytes32(bytes20(account))`), overwrites with `verifier = REVOKED_VERIFIER` (`type(uint160).max`) to prevent implicit re-authorization. Emits `OwnerRevoked`. |
...
1. **Create entry** (if present): Register `initial_owners` in Account Config storage for `sender` — for each `[verifier, ownerId, scope, policyType, expiry, data]` tuple, write `owner_config` (verifier address, scope byte, `policyType`, and `expiry`) and the policy slots from `data` (see [Owner Policies](#owner-policies)). Initialize lock state to safe defaults: `locked = false`, `unlock_delay = 0`, `unlocks_at = 0`.
...
**Policy gate**: When the transaction's authenticating owner has `policyType != 0x00` (SENDER scope), each call is gated before dispatch. The protocol resolves the owner's allowed target — `policy_manager(sender, ownerId)` — and if `call.to` is not that address, the call is **not dispatched** and fails deterministically with the protocol revert `PolicyViolation(bytes32 ownerId, address target)`:

```solidity
error PolicyViolation(bytes32 ownerId, address target);
```

The frame's return data is `abi.encodeWithSelector(PolicyViolation.selector, ownerId, call.to)`. This is a consensus-level result — identical on every node — not a node-local heuristic and not a validity error. Standard atomicity then applies: the enclosing phase's state changes roll back, later phases are skipped, and the phase is reported as failed in `phaseStatuses`. Only work already performed is charged — intrinsic gas plus the gate's resolution (one `policy_manager` SLOAD, charged once to `gas_limit`); the undispatched call body costs nothing, and the transaction is still included with its nonce consumed. A policy-bearing owner can therefore reach exactly one address — its `manager` (which may be the account itself) — which reads the owner's `commitment` and enforces all further policy. Owners with `policyType == 0x00` skip the gate entirely.
...
The protocol distinguishes only no policy, a policy, and reserved values:

| `policyType` | Target | Protocol slots | Policy SLOADs |
|--------------|--------|----------------|---------------|
| `0x00` (none) | — | 0 | 0 |
| `0x01` | stored `manager` | 2 (`commitment` + `manager`) | 2 |
| `0x02`–`0xFF` | reserved | — | — |

A policy-bearing owner stores both a `commitment` and a `manager`: the gate resolves the target with one `policy_manager` SLOAD and the target reads the `commitment` with another. Collapsing every policy to this single shape — rather than carving out a cheaper "self" type — keeps the consensus gate branch-free and loses no expressiveness: a key enforced by the account's own code is just `manager = account`, trading one SLOAD for the account's own address against a separate mode every integrator must understand.

The protocol implements exactly one policy type (`0x01`) and reserves `0x02`–`0xFF`. Reserved values are rejected today; a future ERC may give specific values protocol-level meaning — for example an *enshrined* manager whose fixed address is implied by the value, reclaiming the `manager` slot for a full commitment — without changing this layout. Keeping the range protocol-owned (rather than manager-defined) ensures those values stay free to standardize later, and the `0x01` stored-manager path already lets any wallet point a key at such a contract once it exists.

**Why an opaque commitment rather than a consensus-level policy vocabulary?** Encoding policy primitives (selector lists, recipient allowlists, spend limits) into consensus would freeze the vocabulary, requiring a fork and lockstep adoption per new primitive. A single opaque commitment keeps the protocol agnostic — new policy behaviors are new target logic, shippable permissionlessly — while still riding the signed, portable owner-change path so one signature fully describes a key's authority.

**Why not a per-target allowlist?** An earlier design let a restricted key carry an array of authorized targets, each with its own commitment. Keying policy by `(account, ownerId)` is strictly better on the axes the protocol cares about — bounded, enumerable-free revocation (see [Policy State on Revocation](#security-considerations)) and a single gate resolution rather than one SLOAD per distinct `call.to`. Multi-target composition (token + lending) moves into the single target's own logic, where it belongs.
...
- **Simplicity**: One primary storage slot per owner (`owner_config`). No variable-length public key encoding, no multi-slot public key reads, no length fields. Registration is a single SSTORE in the common case, with two extra SSTOREs (`policy_commitment` and `policy_manager`) when an owner carries a policy.
...
interface IAccountConfiguration {
    ...
    struct Owner {
        bytes32 ownerId;
        OwnerConfig config;
        bytes policyData;  // sliced by policyType: empty (0x00); manager[20] || commitment[32] (0x01)
    }
...
**Validation Surface**: For pure verifiers, invalidators are `owner_config` changes and nonce consumption. The policy slots (`policy_commitment`, `policy_manager`) are **not** validity inputs — they are read only during execution, so changing a key's policy never invalidates a pending transaction (enforcement consequences are covered under [Owner Scope and Policy](#security-considerations)). Stateful verifiers additionally depend on traced state; invalidation tracking is a mempool concern.
