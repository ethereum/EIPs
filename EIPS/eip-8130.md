...
| `payer_auth` | **65 bytes**: K1 signature, payer recovered via ecrecover. **Empty**: `from` pays ETH. See [Payer Modes](#payer-modes) |
...
| **Open sponsor** | `0x00` (placeholder) | 65-byte K1 signature | Payer (ecrecover) | Any K1 payer can sponsor. |
| **Committed sponsor** | `<payer_address>` (20 bytes) | 65-byte K1 signature | Payer (ecrecover) | Sender commits to specific payer. Recovered payer must match. |

Future EIPs may extend `payer_auth` to support additional formats..
...
7. Resolve payer from `payer_auth` and validate against sender's signed marker:
   - If `payer_auth` is empty: verify sender signed `0x80`. Payer is `from`.
   - If `payer_auth` is 65 bytes: recover payer address via ecrecover (using `AA_PAYER_TYPE`). Verify sender signature against committed marker (`<payer_address>` or `0x08`).
