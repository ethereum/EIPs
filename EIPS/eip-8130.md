...
The `policyType` byte selects how the protocol restricts the calls an owner may make and where its signed policy lives. The protocol rejects unrecognized values to keep the selector extensible. Each policy-bearing type carries an opaque 32-byte **commitment** (`keccak256` of the policy parameters).
...
|-------------|------|--------|-------------|
| `0x01` | `authorizeOwner` | `rlp([verifier, scope, policyType, expiry, data])` | Authorize a new owner. Writes `owner_config` with `verifier`, `scope`, `policyType`, and `expiry` (`0` = no expiry). Slices `data` by `policyType` into `policy_commitment`/`policy_manager` (see [Owner Policies](#owner-policies)); rejects mismatched `len(data)`. Emits `OwnerAuthorized`. |
