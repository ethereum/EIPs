...
- **Actors and Verifiers** — the authentication and authorization model(see [Verifiers](#verifiers)).
...
Each actor is associated with a verifier, a contract that performs signature verification. In the protocol's terms, the verifier *authenticates* the actor (it returns the actor's `actorId`); scope and policy then *authorize* what that authenticated actor may do. The verifier address is stored in `actor_config` (see [Account Configuration](#account-configuration)). All verifiers implement `IVerifier.verify(hash, data)`. After the verifier authenticates the signature, the protocol validates the returned `actorId` against `actor_config` and authorizes the actor by checking its scope against the authorization context; the policy gate (`policyType != 0x00`) is enforced later, during execution.
...
| delegate | Signature delegation | Onchain contract | `bytes32(bytes20(delegated_address))` — signatures from `delegated_address` are valid for the registering account (see [Delegate Verifier](#delegate-verifier)) |
...
The delegate verifier lets one account act on behalf of another. An account **A** registers a *delegate actor* that points at another account **B**; thereafter any key that can authenticate as **B** may authenticate as **A**, bounded by the scope **A** grants that delegate actor.

**Registration.** **A** authorizes an actor with `verifier = DELEGATE_VERIFIER` and `actorId = bytes32(bytes20(B))`. That actor's `scope` in **A**'s config governs what **B** may do for **A** under the normal [Actor Scope](#actor-scope) rules.

**Wire format.** When authenticating through the delegate verifier, the auth blob's `data` (the bytes following the 20-byte verifier address, per [Signature Format](#signature-format)) is:

```
data = delegated_account (20 bytes)   // B
    || nested_auth                     // verifier (20 bytes) || data — a normal auth blob for B
```

**Constraints**:

- **No nesting (depth-1):** the nested verifier MUST NOT be the delegate verifier; delegation cannot chain.
- **Nested verifier MUST be canonical:** nodes apply the verifier allowlist to the nested verifier just as to the outer one, keeping total work bounded and enshrinable.
...
AA_TX_TYPE || rlp([
  ...
  sender_auth,        // raw ECDSA r||s||v (EOA, sender empty) | verifier || data (configured actor)
  payer_auth          // empty = self-pay | verifier || data = sponsored (same format as sender_auth)
])
...
2. **Verify**: Route by verifier address. For the EOA path (`sender` empty), ecrecover was already performed in step 1. For `ECRECOVER_VERIFIER` (`address(1)`), the protocol natively ecrecovers from `data` (as `r || s || v`), returning `actorId = bytes32(bytes20(recovered_address))`. For all other verifiers, call `verifier.verify(hash, data)` via STATICCALL, returning `actorId` (or `bytes32(0)` for invalid). Reject `REVOKED_VERIFIER` as a verifier address.
3. **Authorize**: SLOAD `actor_config(sender, actorId)`. **Implicit EOA rule**: if the slot is empty, `actorId == bytes32(bytes20(sender))`, and verification in step 2 used the native secp256k1 path (the EOA path or `ECRECOVER_VERIFIER`), treat as implicitly authorized with `scope = 0x00` and `policyType = 0x00`. Otherwise, require that the stored verifier address matches the effective verifier. If the stored `expiry` is non-zero, also require `block.timestamp <= expiry`; an expired actor is rejected.
...
rlp([
  ...
  initial_actors      // Array of [actorId, verifier] tuples
])
```
...
The per-actor contribution is `actorId || verifier` (52 bytes: the 32-byte `actorId` and the 20-byte `verifier` address). Because initial actors are always unrestricted owner keys, no scope, expiry, or policy fields participate in the commitment. The required strictly-ascending `actorId` ordering makes the commitment canonical.
...
1. Parse `[0x00, user_salt, code, initial_actors]` where each entry is `[actorId, verifier]`
