...
This proposal defines a new [EIP-2718](./eip-2718.md) transaction type and an onchain system contract that together provide account abstraction — batching, gas sponsorship, and authentication using any cryptographic system. Accounts configure authorized keys and relevant verifier in the system contract; the protocol validates transactions using native implementations for recognized algorithms, and via sandboxed pure-function contracts for any other scheme.

## Motivation

Provide account abstraction in a performant, highly optimizable way. Pure-function verifiers with known state inputs let nodes determine validity from state alone — no staking or reputation systems required. Being specific about what verification requires allows aggressive optimization and high-throughput accounts.

Extensible, permissionless deployment of authentication mechanisms. New signature schemes deploy as EVM contracts without protocol changes, providing a native path to post-quantum secure authentication.

Permissionless pay-during-execution flows enable ERC-20 gas payments and encourage FOCIL compatibility.

Enable programmable accounts while maintaining protocol separation. Wallet code layers additional logic; the protocol handles authentication, gas payment, and calldata delivery.

Provide real-world features today and extensibility for tomorrow.
