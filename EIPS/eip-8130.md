...
The byte layout of `metadata` is not constrained by this specification; producers MAY use any encoding. A companion ERC defines a recommended structured encoding so that off-chain consumers can index metadata interoperably (covering whole-transaction, per-phase, and per-call annotation, multiple independent contributors, and commitments to off-chain data). Producers that do not need interoperable indexing MAY treat `metadata` as wholly opaque bytes.
