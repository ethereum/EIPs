...
| `KNOWN_VERIFIER_ADDRESSES` | TBD <!-- TODO --> | Well-known verifier addresses (K1, P256, DELEGATE) |
...
| P256 | `P256_VERIFIER` | secp256r1 / P-256 (raw or WebAuthn) | 33/65 bytes | 64 bytes (raw) or Variable (WebAuthn) | `keccak256(x \|\| y)[12:]` |
...
**P256 dual-format**: The P256 verifier accepts both raw P-256 signatures and WebAuthn-wrapped P-256 signatures for the same registered key. Gas is charged based on the detected format (see [Intrinsic Gas](#intrinsic-gas)).
...
| `P256_VERIFIER` (raw) | 7000 | P256 verify + reads public key |
| `P256_VERIFIER` (WebAuthn) | 12000 + calldata_gas | P256 verify + WebAuthn parsing + reads public key |
...
All types read `key_config` (1 SLOAD) for authorization, policy checks, and verifier address. Non-K1 types require additional SLOADs to read the public key. For `P256_VERIFIER`, the protocol determines the gas cost by inspecting the `signature_data` length: exactly 64 bytes indicates raw P-256, longer indicates WebAuthn framing.
...
| P256 (raw) | `keyId (20) \|\| r,s (64)` | 84 bytes | reads key_config + public key |
| P256 (WebAuthn) | `keyId (20) \|\| authenticatorData \|\| cDJ_len (2) \|\| clientDataJSON \|\| r,s (64)` | Variable | reads key_config + public key |
...
**Local key changes** (`authorizeKey()` / `revokeKey()` / `setPolicy()` / `setExpiration()` / `setVerifierData()`) require `msg.sender` and do **not** increment `key_change_sequence`. Portable key changes may overwrite local changes at the same keyIds.
...
The well-known verifier set is intentionally minimal: K1, P256, and DELEGATE. Other algorithms (BLS, post-quantum, etc.) start as sandbox verifiers.

- **K1**: Already built into the protocol for EOA signature recovery. Zero marginal implementation cost.
- **P256**: The high-volume path for passkey-based accounts. The ~200-600 gas savings per transaction over a sandbox verifier wrapping the P256VERIFY precompile (RIP-7212) compounds meaningfully at scale.
...
Post-quantum schemes (ML-DSA), BLS12-381, ZK-proof-based auth, and exotic curves are deployed as [sandbox verifier contracts](#sandbox-verifiers) — no protocol change required. Any algorithm that becomes widely adopted can be promoted to a well-known verifier via protocol upgrade for optimal gas. This promotion is seamless: the verifier contract address in existing `key_config` entries remains unchanged, and the protocol simply begins using a native implementation for that address.
