...
| `0x01` | per-target allowlist | Under SENDER scope, every `call.to` MUST have a non-zero commitment in `owner_policy(account, ownerId, target)`, enforced during execution. |
...
```
owner_policy(account, ownerId, target) → bytes32 commitment
```

A non-zero `commitment` authorizes calls to `target`; a zero value means the target is not authorized. The commitment is opaque to the protocol — it is policy configuration that the contract at `target` reads and interprets.

**Enforcement is at execution, not validation.** The protocol does not read `owner_policy` when checking transaction validity, so policy state is never part of the mempool validity surface — an owner's single `owner_config` SLOAD still decides validity. Instead, during [Call Execution](#call-execution), each call made under a `policyType == 0x01` owner is gated: the protocol reads `owner_policy(sender, ownerId, call.to)` and, if the commitment is zero, the call fails (its phase reverts). This restricts a restricted owner — e.g. a session key — to calling only its configured policy contracts; each such contract reads the commitment and the authenticated `ownerId` to enforce *what* the call may do, then typically forwards the action back to the account (see [Why Per-Call Policies?](#why-per-call-policies)).

Enforcement applies only under SENDER scope; PAYER, SIGNATURE, and CONFIG authentication ignore `policyType`.

**Indexing and clearing.** Every policy write emits `PolicySet(account, ownerId, target, commitment)` — including clears, where `commitment == bytes32(0)` — so an indexer can reconstruct an owner's exact `owner_policy` state from logs alone, alongside `OwnerAuthorized` / `OwnerRevoked`. `revokeOwner` does **not** enumerate and clear an owner's `owner_policy` entries; a non-enumerable mapping cannot be bulk-cleared on-chain, so those entries persist in storage. They are unreachable while the owner is revoked, but would apply again if the same `ownerId` is later re-authorized with `policyType == 0x01`. Clearing is therefore wallet-managed: to retire a key's authority, a wallet issues `setPolicy(target, bytes32(0))` for each target it previously set (enumerated from its `PolicySet` history), or treats re-authorization as deliberately inheriting the prior policy set. The protocol never auto-clears.
...
Validity does not depend on `policyType`. When the owner has `policyType == 0x01` under SENDER scope, the per-target call restriction is enforced later, during [Call Execution](#call-execution) — not here.
...
rlp([
  ...
  initial_owners      // Array of [verifier, ownerId, scope, policyType, expiry, policies] tuples
])

policies = [[target, commitment], ...]   // empty unless policyType == 0x01
```

Initial owners are registered with their specified scope, `policyType`, and `expiry` (`0` = no expiry). Entries in `policies` populate `owner_policy(sender, ownerId, target)` for each new owner. `policies` MUST be empty unless `policyType == 0x01`. Wallet initialization code can lock the account via `calls` in the execution phase (e.g., calling `lock()` on the Account Configuration Contract).
...
policies_root_i = keccak256(
    target_0 || commitment_0 || ... || target_m || commitment_m   // policies_i sorted by target; bytes32(0) if empty
)

owners_commitment = keccak256(
    ownerId_0 || verifier_0 || scope_0 || policyType_0 || expiry_0 || policies_root_0 ||
    ...
    ownerId_n || verifier_n || scope_n || policyType_n || expiry_n || policies_root_n
)
...
The per-owner contribution is `ownerId || verifier || scope || policyType || expiry || policies_root` (92 bytes; `expiry` is the 6-byte `uint48`). Sorting owners by `ownerId` and policies by `target` makes the commitment order-independent.
...
1. Parse `[0x00, user_salt, code, initial_owners]` where each entry is `[verifier, ownerId, scope, policyType, expiry, policies]`
...
4. Reject any owner with unrecognized `policyType`, non-empty `policies` when `policyType != 0x01`, a policy entry with `commitment == bytes32(0)`, or duplicate `target` within a single owner's `policies`
5. Sort owners by `ownerId` and each owner's `policies` by `target`
...
|-------------|------|--------|-------------|
| `0x01` | `authorizeOwner` | `rlp([verifier, scope, policyType, expiry, policies])` where `policies = [[target, commitment], ...]` | Authorize a new owner. Writes `owner_config` with `verifier`, `scope`, `policyType`, and `expiry` (`0` = no expiry), and writes each policy entry to `owner_policy(account, ownerId, target)`. `policies` MUST be empty unless `policyType == 0x01`. Emits `OwnerAuthorized` and a `PolicySet` per policy entry. |
| `0x02` | `revokeOwner` | `rlp([])` | Revoke an existing owner. Deletes `owner_config` for non-EOA owners; for the implicit EOA owner (`ownerId == bytes32(bytes20(account))`), overwrites with `verifier = REVOKED_VERIFIER` (`type(uint160).max`) to prevent implicit re-authorization. Per-target policy entries are not enumerated or cleared; they persist in storage (unreachable while revoked) and would apply again if the same `ownerId` is re-authorized — clearing is wallet-managed via `setPolicy(target, 0)` (see [Owner Policies](#owner-policies)). Emits `OwnerRevoked`. |
| `0x03` | `setPolicy` | `rlp([target, commitment])` | Write `owner_policy(account, ownerId, target) = commitment`. `commitment == bytes32(0)` clears the entry. The owner referenced by `ownerId` MUST already exist with `policyType == 0x01`. Emits `PolicySet`. |
...
1. **Create entry** (if present): Register `initial_owners` in Account Config storage for `sender` — for each `[verifier, ownerId, scope, policyType, expiry, policies]` tuple, write `owner_config` (verifier address, scope byte, `policyType`, and `expiry`) and, for each entry in `policies`, write `owner_policy(sender, ownerId, target) = commitment`. Initialize lock state to safe defaults: `locked = false`, `unlock_delay = 0`, `unlocks_at = 0`.
...
**Policy gate**: When the transaction's authenticating owner has `policyType == 0x01` (SENDER scope), each call is gated before dispatch. The protocol reads `owner_policy(sender, ownerId, call.to)`; if the commitment is zero, the call is **not dispatched** and fails deterministically with the protocol revert `PolicyViolation(bytes32 ownerId, address target)`:

```solidity
error PolicyViolation(bytes32 ownerId, address target);
```

The frame's return data is `abi.encodeWithSelector(PolicyViolation.selector, ownerId, call.to)`. This is a consensus-level result — identical on every node — not a node-local heuristic and not a validity error. Standard atomicity then applies: the enclosing phase's state changes roll back, later phases are skipped, and the phase is reported as failed in `phaseStatuses`. Only work already performed is charged — intrinsic gas plus this gate SLOAD (charged to `gas_limit` per distinct `call.to`); the undispatched call body costs nothing, and the transaction is still included with its nonce consumed. The commitment is otherwise opaque to the protocol — the contract at `call.to` reads it to enforce its own semantics. Owners with `policyType == 0x00` skip the gate entirely.
...
If the wallet needs the verifier address or scope, it calls `getOwnerConfig(account, ownerId)` on the Account Configuration Contract. A policy contract reached as a `call.to` reads its own commitment in one call: `getPolicyCommitment(getSender(), getOwnerId(), address(this))` — combining `sender` and the authenticated `ownerId` from this precompile with the contract's own address as `target`. The commitment stays in Account Configuration storage (where it is written and revoked) rather than the precompile, keeping the precompile to immutable transaction context.
...
The protocol reads everything it needs to decide transaction validity — verifier binding, scope, and policy selection — in one SLOAD on `owner_config`. Per-target policy commitments are kept in a separate map (`owner_policy`) that is read only during execution, so policy adds nothing to the validity surface: owners with `policyType == 0x00` pay nothing, and owners with `policyType == 0x01` pay one SLOAD per distinct `call.to` at execution time when authenticating as SENDER (see [Why Per-Call Policies?](#why-per-call-policies)).

### Why Per-Call Policies?

Session keys and gas-payer hot wallets often need narrow authority — only this token, only this swap router, only this policy contract. The `policyType == 0x01` mode expresses that as a per-target allowlist while keeping the *validity* surface untouched: the protocol enforces the allowlist during execution (a disallowed `call.to` reverts), not during mempool validation. This preserves the single-`owner_config`-SLOAD validity model and means a policy change never invalidates an in-flight transaction. The tradeoff is that a compromised restricted key can still get policy-violating transactions included — they revert and consume gas — rather than being filtered before execution; self-pay bounds the loss to the account's own ETH, and nodes rate-limit as usual.

The commitment is an opaque `bytes32`. A policy contract reached as a `call.to` retrieves it from the Account Configuration Contract (`getPolicyCommitment`) together with the authenticated `ownerId` (Transaction Context precompile) and decides what it means: a hash of allowed-method selectors, a hash of spend-limit data initialised on first use, a Merkle root, a tag-and-hash, etc. The protocol enforces *that* a target is authorised; the policy contract enforces *what* the call may do, then typically forwards the action back to the account.

Keeping the commitment opaque is deliberate. Encoding a fixed policy vocabulary — selector lists, recipient allowlists, spend limits with reset periods — directly into the protocol would freeze that vocabulary into consensus: each new policy primitive would require a network upgrade, every node would carry schema-decoding logic, and chains would have to adopt each revision in lockstep. By storing only a commitment and delegating interpretation to an ordinary contract, the policy vocabulary lives at the application layer. New policy types are new contracts — shippable permissionlessly, versioned independently, and reusable across accounts — without touching consensus or coordinating a fork. The protocol's job stays minimal (authorize the target); expressiveness is unbounded.

This subsumes simpler patterns. "Calls must go to the account itself" is a single entry with `target == sender`; "calls must go to one address X" is a single entry for `X`. It also extends to richer use cases such as native token spend limits, where a `transferWithAuthorization(policyData, amount, to)` call proves `keccak256(policyData)` matches the stored commitment before the policy updates spend state.
...
*Informative.* Because policy semantics live entirely in the contract that interprets the commitment, a single reusable policy contract can serve most session-key needs without any per-wallet deployment. A canonical reference policy — deployable once at a deterministic address and shared by all accounts — covers the common case: an allowlisted function selector on the target plus an optional recipient allowlist and spend cap. The commitment binds the parameters; the contract verifies them and forwards the action back to the account.

```solidity
// Commitment stored in owner_policy(account, ownerId, target):
//   commitment = keccak256(abi.encode(selector, recipients, spendCap))
contract ReferencePolicy {
    function execute(bytes calldata params, address target, bytes calldata callData) external {
        address account = ITxContext(TX_CONTEXT).getTransactionSender();
        bytes32 ownerId = ITxContext(TX_CONTEXT).getTransactionSenderOwnerId();
        bytes32 commitment = IAccountConfiguration(ACCOUNT_CONFIG)
            .getPolicyCommitment(account, ownerId, address(this));
        require(commitment == keccak256(params), "policy mismatch");

        (bytes4 selector, address[] memory recipients, uint256 spendCap) =
            abi.decode(params, (bytes4, address[], uint256));
        // ... check callData selector/recipient/amount against decoded params ...
        // ... then forward the approved action back to `account` ...
    }
}
```

A wallet provisions such a key with one `setPolicy(referencePolicy, commitment)` entry and no contract deployment of its own. Wallets needing richer logic (custom rate limits, Merkle-gated targets, off-chain attestations) deploy their own policy contract; the protocol treats both identically.

### Why Owner Expiry?

Session keys are intended to be short-lived, but revocation requires a transaction, and a forgotten key lingers indefinitely. The per-owner `expiry` lets a wallet provision a key that lapses automatically at a chosen time — no revoke transaction, no lingering authority. Because `expiry` is packed into the same `owner_config` slot the protocol already reads for validity, enforcement costs nothing extra and happens at validation: an expired owner's transactions are rejected before execution rather than reverting. It also bounds the blast radius of the unenumerated `owner_policy` entries (see [Owner Policies](#owner-policies)) — once a key has expired, its stale policy entries are unreachable even if the same `ownerId` is never explicitly cleared. A value of `0` preserves the existing always-valid behavior for owners that should not expire.
...
- **Simplicity**: One primary storage slot per owner (`owner_config`). No variable-length public key encoding, no multi-slot public key reads, no length fields. Registration is a single SSTORE in the common case, with one extra SSTORE per per-target policy entry when an owner uses `policyType == 0x01`.
...
interface IAccountConfiguration {
    ...
    struct OwnerConfig {
        ...
        uint8 policyType;  // 0x00 = none, 0x01 = per-target policy
    ...
    event PolicySet(address indexed account, bytes32 indexed ownerId, address indexed target, bytes32 commitment);
    ...
    function getPolicyCommitment(address account, bytes32 ownerId, address target) external view returns (bytes32 commitment);
...
**Validation Surface**: For pure verifiers, invalidators are `owner_config` changes and nonce consumption. Per-target policy state (`owner_policy`) is **not** a validity input — it is read only during execution, so changing or clearing a policy never invalidates a pending transaction; a call that is no longer allowlisted simply reverts at execution. Stateful verifiers additionally depend on traced state; invalidation tracking is a mempool concern. Policy contracts that read the commitment during execution MUST treat it as part of their own authorization state.
...
**Owner Scope and Policy**: Scope is protocol-enforced after verifier execution during validation — a verifier cannot bypass it. The per-target call policy is protocol-enforced during execution (a disallowed `call.to` reverts with `PolicyViolation`) and is likewise outside verifier control; because it is an execution-time gate rather than a validity input, it is not a mempool filter for compromised keys (see [Why Per-Call Policies?](#why-per-call-policies)).

**Policy Persistence on Revocation**: `revokeOwner` does not clear an owner's `owner_policy` entries — a non-enumerable mapping cannot be bulk-cleared on-chain. The entries are unreachable while the owner is revoked, but re-authorizing the same `ownerId` with `policyType == 0x01` makes them live again. Wallets MUST treat re-authorization as inheriting any prior policy set and explicitly clear stale targets via `setPolicy(target, 0)` — enumerated from `PolicySet` logs — when a fresh policy set is intended. This EIP deliberately omits an on-chain policy epoch/version: clearing is cheap relative to the storage cost of versioning every entry, and `PolicySet` / `OwnerRevoked` logs give wallets and indexers everything needed to compute the live set off-chain.
