...
- The account is already initialized (either change-sequence channel — local or multichain — is non-zero; see [Config Change Authorization](#config-change-authorization)). This rejects accounts already created via a [Create Entry](#create-entry) as well as accounts that have applied any config change (including a multichain `chain_id 0` change that leaves the local channel at `0`). A locked account is necessarily already initialized, so this precondition also covers the locked case — `importAccount` needs no separate lock check.
...
The change-sequence channels double as the initialized flag. Creation and import set the local channel to `1`, and any applied config change bumps whichever channel it used (local for a chain-specific `chain_id`, multichain for `chain_id 0`). An account is therefore considered initialized when **either** channel is non-zero; `isInitialized()` reports `true` exactly in that case. Checking both channels is required because an account can first acquire registered config through a multichain config change — which leaves the local channel at `0` — and such an account must not be treated as a fresh, importable account. Never-initialized accounts (such as implicit EOAs that have done no config change) have both channels at `0`. Because signers read the current values via `getChangeSequences()` before signing, client implementers MUST NOT assume either channel always starts at `0`; this is observable but not breaking.
...
### Why Does Only Create/Import Mark an Account Initialized?

The `initialized` flag tracks whether an explicit initial-actor set has been registered in Account Config storage, so only `createAccount` and `importAccount` set it; auto-delegation places wallet code only and leaves the account uninitialized, operating through its implicit EOA actor. EOAs therefore never use `importAccount` (it is for existing contract accounts and is gated by ERC-1271) — they add actors by signing `authorizeActor` changes with the implicit EOA actor, a path that works identically on 8130 and non-8130 chains.
...
interface IAccountConfiguration {
    struct ChangeSequences {
        ...
        uint64 local;      // chain_id == block.chainid; set to 1 on create/import. Initialized when local or multichain is non-zero
    }
    ...
    // Storage views
    function isInitialized(address account) external view returns (bool); // true when either change-sequence channel (local or multichain) is non-zero
