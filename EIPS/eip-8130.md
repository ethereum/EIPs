...
When `scope & SCOPE_POLICY != 0`, the actor's policy is held in two additional slots:

```
policy_commitment(account, actorId) → bytes32   // set when SCOPE_POLICY is set
policy_manager(account, actorId)    → address   // set when SCOPE_POLICY is set
```
...
| 3 | `0x08` | | (spare) |
| 4 | `0x10` | SCOPE_POLICY (POLICY) | Gated initiation: `sender_auth`; may originate transactions only to the actor's `policy_manager` (see [Actor Policies](#actor-policies)) |
| 5 | `0x20` | SCOPE_NONCE (NONCE) | Permits a restricted actor to use sequenced `nonce_key`s for sender-context transactions (see [Actor Nonce Scope](#actor-nonce-scope)) |
| 6–7 | | (spare) | Reserved for future pure grants |

There is no CONFIG grant: config-change authority (`authorizeActor`, `revokeActor`, `applySignedActorChanges`, delegation) requires the admin predicate `scope == 0x00` directly, not a separate bit. A grant bit for config authority would be separable from admin, but any actor that can rewrite `actor_config` can also authorize itself an unrestricted actor, so a "config-only" grant is unrestricted in every way that matters; the scope byte only names what is actually separable. See [Why CONFIG Is Not a Grant](#why-config-is-not-a-grant).

**Initiation modes.** `SCOPE_POLICY` and `SENDER` are not mutually exclusive at write time, but at **use** time `SCOPE_POLICY` always wins: if the resolved `scope` sets `SCOPE_POLICY`, the actor is call-gated regardless of whether `SENDER` is also set — `SENDER` does not ungate a policy-bearing actor. The protocol (native validation / call execution) MUST reject a `sender_auth` whose resolved `scope` sets `SCOPE_POLICY` together with `SIGNATURE`; this combination fails closed rather than falling back to ungated `SENDER` behavior, since it would otherwise reopen the permit-style signing escape hatch `SCOPE_POLICY` is meant to close (see [Actor Policies](#actor-policies)). `SCOPE_POLICY | PAYER` and `SCOPE_POLICY | SCOPE_NONCE` are both permitted: the gate constrains calls, not gas or nonce channel selection. An invalid combo stored on-chain is inert (fails authorization) rather than unwritable, so `authorizeActor` does not enumerate combinations.
...
Scope authorization is applied only after an authenticator authenticates the signature and returns an `actorId`: the protocol loads that actor's `scope` and checks it against the context being authorized. For `sender_auth`, require `scope == 0x00 || (scope & SENDER) != 0 || (scope & SCOPE_POLICY) != 0`, then apply the precedence rule above (`SCOPE_POLICY` wins over `SENDER`; `SCOPE_POLICY` combined with `SIGNATURE` is rejected outright). For `payer_auth` / self-pay require PAYER (or unrestricted); for `verifySignature()` require SIGNATURE (or unrestricted); for config change `auth` require admin (`scope == 0x00`).
...
`SCOPE_NONCE` (`0x20`) governs which `nonce_key`s a restricted actor may use for **sender-context** sequenced transactions. It is a grant, not a channel reservation: no exclusivity among live actors is promised, and it does not bind an actor to any particular `nonce_key`.

| Actor | `nonce_key` allowed |
|-------|----------------------|
| Admin (`scope == 0x00`) | Any `nonce_key`, including `NONCE_KEY_MAX` (nonceless), freely |
| Restricted, `SCOPE_NONCE` unset | `nonce_key == NONCE_KEY_MAX` only (nonceless) |
| Restricted, `SCOPE_NONCE` set | Any `nonce_key`, including `NONCE_KEY_MAX` — the full 2D key space, with no per-actor binding to a specific key |

This makes nonceless (`NONCE_KEY_MAX`) the default for every restricted (non-admin) actor: a freshly authorized session key can send sequenced-looking transactions without ever touching a shared or dedicated nonce channel. An actor that needs an ordered, replay-protected sequence of its own (e.g. a high-throughput automation key) opts in with `SCOPE_NONCE` and then may pick any `nonce_key` in the full 2D space, including keys already used by other actors on the account; the protocol does not track or enforce per-actor ownership of a `nonce_key`.

`SCOPE_NONCE` constraints apply to **sender-context** transactions only. `payer_auth` consumes no payer-side sequence and is unaffected. The check (reject a sequenced `nonce_key` from a restricted actor lacking `SCOPE_NONCE`) is enforced by the protocol; see [Validation](#validation).
...
`SCOPE_POLICY` selects whether an actor is gated. When set at authorize time, `policyData` MUST be `manager` (20 bytes) ‖ `commitment` (32 bytes) with both nonzero, and those values are written to `policy_manager` / `policy_commitment`. When unset, `policyData` MUST be empty and any prior policy slots are cleared. Policy vocabulary lives in the commitment and manager logic, not in a protocol sub-type byte.

| `SCOPE_POLICY` | Gate: `call.to` MUST equal | `policyData` at authorize |
|----------------|----------------------------|---------------------------|
| unset | (no gate) | empty |
...
**Scope.** `SCOPE_POLICY` is the gated initiation grant. At use time it takes precedence over `SENDER` (a policy-bearing actor is always call-gated, whether or not `SENDER` is also set) and it MUST NOT be combined with `SIGNATURE` (see [Actor Scope](#actor-scope)). It MAY combine with `PAYER` so a session key can self-pay gas; that gas spend is not policy-gated and is bounded by balance and `expiry`. It MAY also combine with `SCOPE_NONCE` so a policy key can use a sequenced `nonce_key`; the gate governs `call.to`, not nonce channel selection. `SIGNATURE` is excluded so permit-style ERC-1271 bypasses are structurally closed; session-key signing SHOULD use an account-level approved-hash / approve-typed-data pattern (Safe `approveHash` precedent) rather than granting `SIGNATURE` on the policy key.

**Reference flow** (non-normative). One construction of a session-key policy:

1. **Authorize.** The account authorizes the key with `SCOPE_POLICY` (optionally `| PAYER` and/or `| SCOPE_NONCE`), the `manager`, and a `commitment`. The key's call authority is only the gate to its `manager`.
...
**Example** (non-normative). A subscription session key limited to **5 USDC per 30-day period** with a two-target allowlist. Authorized with `SCOPE_POLICY` (nonceless by default, and without `PAYER` if a sponsor pays gas). Over-budget transfers, wrong targets, expiry, or revoke all fail.
...
Restrictions the Account Configuration contract must enforce are complete at deployment: the signer is admin (`scope == 0x00`), reserved-bytes-zero, and well-formed `policyData` when `SCOPE_POLICY` is set. Grants can grow because reads fail closed on unknown bits — `SCOPE_NONCE` itself is such a grant, added to the spare bit space without changing the frozen surface above. Signing semantics (e.g. approved typed data for session keys) evolve at the account layer, not in this contract.
...
Every AA transaction, nonce-free or not, has a **replay identifier** that names the *logical* transaction independent of its fees and authorization blobs:

```
REPLAY_ID_TYPE = 0x7B01

replay_id = keccak256(REPLAY_ID_TYPE || rlp([
  chain_id, resolved_sender, nonce_key, nonce_sequence, expiry,
...
`replay_id` also drives normative fee-bump replacement for any AA transaction, independent of `nonce_key`:

- Two pending transactions from the same `sender` with the same `replay_id` are **replacement candidates**.
- A replacement MUST increase `max_priority_fee_per_gas` by at least the node's configured minimum bump (e.g., ≥10%), mirroring standard replace-by-fee conventions.
- A replacement MUST be independently fully valid, including a fresh `payer_auth` when sponsored: `payer_auth` commits to `max_fee_per_gas`, `max_priority_fee_per_gas`, and `gas_limit` (see [Signature Payload](#signature-payload)), so the payer must re-sign to authorize the bumped fees even though `payer` and `replay_id` are unchanged.
- Changing `payer` yields a new `replay_id` (a new logical transaction) rather than a replacement of the old one.
- Block builders MUST NOT include two transactions with the same `(sender, replay_id)` in the same block.
...
4. **Check scope**: Read the resolved `scope` byte (from the inline default-EOA config for the secp256k1 self-actor, or `actor_config` otherwise). For `sender_auth`, require `scope == 0x00 || (scope & (SENDER | SCOPE_POLICY)) != 0`, then apply the precedence rule (`SCOPE_POLICY` wins over `SENDER`; reject outright if `SCOPE_POLICY` is combined with `SIGNATURE`). For `payer_auth` require PAYER (or unrestricted); for `verifySignature()` require SIGNATURE (or unrestricted); for config `auth` require admin (`scope == 0x00`). In self-pay (no `payer_auth`), additionally require PAYER (or unrestricted) on the resolved sender actor.
5. **Check nonce scope** (sender-context only, when `nonce_key != NONCE_KEY_MAX`): require the resolved actor be admin (`scope == 0x00`) or carry `SCOPE_NONCE`; otherwise reject (a restricted actor without `SCOPE_NONCE` may only use `nonce_key == NONCE_KEY_MAX`). Reject nonzero reserved bytes on the loaded config.
...
|-------------|------|--------|-------------|
| `0x01` | `authorizeActor` | `abi.encode(ActorConfig config, bytes policyData)` (see [`ActorConfig`](#iaccountconfiguration)) | Authorize a new actor. Writes `actor_config` with `authenticator`, `scope`, and `expiry` (`0` = no expiry) verbatim (unknown scope bits stored as-is). If `SCOPE_POLICY` is set, requires `policyData = manager ‖ commitment` (both nonzero) and writes those slots; otherwise requires empty `policyData` and clears policy slots. Rejects nonzero reserved bytes. Does **not** enforce scope-combination exclusivity (that is use-time / protocol). Emits `ActorAuthorized`. |
...
5. Validate `sender_auth` against resulting actor state (see [Validation](#validation)). Require SENDER scope (or `SCOPE_POLICY`, subject to the precedence rule in [Actor Scope](#actor-scope)) on the resolved actor. If `nonce_key != NONCE_KEY_MAX`, also require the resolved actor be admin (`scope == 0x00`) or carry `SCOPE_NONCE`. If the transaction is self-pay, also require PAYER scope on the resolved sender actor. If delegation entries are present, also require `actorId == bytes32(bytes20(sender))` (EOA actor) and admin (`scope == 0x00`).
...
9. Apply [Mempool Replacement](#mempool-replacement) rules when a pending transaction from the same `sender` shares this transaction's `replay_id`.
...
Session keys often need narrow authority: only this token, only this much per day, only this action. `SCOPE_POLICY` makes gated initiation a first-class grant, distinct from ungated `SCOPE_SENDER`: the key may originate transactions, but only to a single call target, bound to a signed opaque **commitment**. The protocol's only policy responsibility is the single-target gate plus storing the commitment.

Because Account Configuration is a single immutable deployment, policy vocabulary is not encoded as protocol sub-type bytes. Excluding `SIGNATURE` on policy keys structurally closes permit-style ERC-1271 bypasses; session-key signing belongs at the account layer (approved-hash / approve-typed-data). Allowing `SCOPE_POLICY | PAYER` lets a session key self-pay a small ETH budget without escaping the call gate.

### Why CONFIG Is Not a Grant?

An earlier design gave config-change authority its own scope bit, so a key could rewrite `actor_config` without being a full owner in every other sense. That separation is not actually achievable: any actor that can call `authorizeActor` can authorize a brand-new actor with `scope == 0x00`, i.e. it can always self-escalate to unrestricted. A "config-only" grant is therefore unrestricted in every way that matters, it just takes one extra transaction to prove it. Naming that bit `CONFIG` implied a containment the protocol was never actually enforcing.

Making admin a predicate (`scope == 0x00`) rather than a grant bit says this plainly: there is exactly one privilege level that can rewrite an account's own authorization surface, and it is the same unrestricted level every other check already special-cases. Every account is born with a scope-0 root (the implicit EOA, or an explicit unrestricted actor named in a create/import), so "admin" is not a new concept introduced by this section, it is the same root the rest of the specification already depends on for validity.

### Why SCOPE_NONCE?

Parallel session keys should not, by default, need to coordinate over the owner's (or each other's) nonce state at all. `SCOPE_NONCE` makes nonceless (`nonce_key == NONCE_KEY_MAX`) the default for every restricted actor: a session key can be authorized and used immediately with zero nonce-channel bookkeeping, protected instead by `expiry` and the [Replay Identifier](#replay-identifier). A key that specifically needs an ordered, replay-protected sequence (for example, a high-frequency automation actor issuing many transactions in flight) opts in with `SCOPE_NONCE` and can then use any `nonce_key` in the full 2D key space, exactly as an admin actor can, with no separate per-actor lane field to provision, learn, or coordinate. This is deliberately simpler than binding each actor to a specific channel: there is nothing to reserve, nothing to look up before choosing a `nonce_key`, and no new field in `actor_config` beyond the single grant bit.
...
interface IAccountConfiguration {
    ...
    struct ActorConfig {
        ...
        uint8 scope;        // grants bitmask; 0x00 = unrestricted (admin); 0x10 = SCOPE_POLICY; 0x20 = SCOPE_NONCE
    ...
    struct Actor {
        bytes32 actorId;
        ActorConfig config;
        bytes policyData;  // empty unless SCOPE_POLICY set; then manager[20] || commitment[32]
    }
    ...
    //   and, only when scope & SCOPE_POLICY != 0, manager(20) || commitment(32) (84 bytes total).
...
**Replay Protection**: Transactions include `chain_id`, 2D nonce (`nonce_key`, `nonce_sequence`), and `expiry`. For `NONCE_KEY_MAX` (nonce-free mode), replay protection relies on short-lived `expiry` and deduplication by the fee- and signature-invariant [Replay Identifier](#replay-identifier) (`replay_id`). The mempool enforces a tight expiry window (e.g., 10-30 seconds) to bound the window. Block builders MUST NOT include two transactions with the same `(sender, replay_id)`, nonce-free or not (see [Mempool Replacement](#mempool-replacement)).
...
All three cases resolve to the same `replay_id` (fee bumps included, since `replay_id` excludes the fee fields), so deduplicating and replacing on it collapses them to a single mempool slot and, ultimately, a single includable transaction. `expiry` cannot be extended via a fee-bump replacement: `expiry` is part of `replay_id`, so changing it produces a different `replay_id` — a new logical transaction, not a replacement of the old one — and must independently satisfy nonce/sequence and mempool acceptance rules.

**Actor Scope and Policy**: Scope grants are protocol-enforced after authenticator execution during validation (fail closed). The policy gate is protocol-enforced during execution (`ActorPolicyViolation`). The gate constrains calls, not gas or nonce selection: `SCOPE_POLICY | PAYER` may self-pay, with drain bounded by balance and `expiry`; `SCOPE_POLICY | SCOPE_NONCE` may use a sequenced `nonce_key`; sponsored gas is unchanged. At use time `SCOPE_POLICY` takes precedence over `SENDER` (a policy-bearing actor is always gated) and is rejected outright when combined with `SIGNATURE`, structurally closing permit-style bypasses; session-key signing SHOULD use an account-level approved-hash / approve-typed-data pattern (Safe `approveHash` precedent) (see [Actor Scope](#actor-scope)).
...
**Actor Management**: Config change authorization requires the authorizing actor be admin (`scope == 0x00`); there is no separate config-change grant. The EOA actor is implicitly authorized with unrestricted (admin) scope; revocable via portable config change. All actor modification paths are blocked when the account is locked.

**Admin Compromise Is Total**: Because config authority is the admin predicate itself rather than a separable grant, compromise of any admin actor (`scope == 0x00`) is immediately and completely a compromise of the account: the attacker can authorize new admin actors, revoke existing ones, and rewrite every other actor's scope, all in one transaction. There is no intermediate "can change config but is otherwise limited" compromise to detect or contain; a previous design's separable CONFIG grant offered only the appearance of such a tripwire, since a CONFIG-only actor could always self-authorize an unrestricted one anyway. Wallets that want defense in depth for the admin key (multi-factor confirmation, time-delayed recovery, social recovery, etc.) express it at the *verifier* layer of that scope-0 actor, for example a custom authenticator or a `manager`-style contract behind a delegate actor, rather than relying on the protocol's scope byte to partition admin authority. A dormant recovery actor authorized with `scope == 0x00` is a dormant root: it should be protected, monitored, and revoked when no longer needed with the same care as any other live admin key, since it is one.
