...
The verifier interface is the extension point. This proposal defines pure-function verifiers for high-performance transaction validation. Future verifier types — with access to `call_phases` and arbitrary state — can enable permissionless paymasters, session key policies, and other programmable account features as follow-up EIPs.
...
Also, since calls do not carry value (see [Call Execution](#call-execution)), ETH can only leave an account through its own bytecode. This feature is used by nodes for [High-Rate Mempool Acceptance](#high-rate-mempool-acceptance).
...
AA_TX_TYPE || rlp([
  ...
  call_phases,        // Array of phases, each phase is an array of calls [[{to, data}, ...], ...] | empty
...
| `call_phases` | **Empty**: No calls. **Non-empty**: Array of phases, where each phase is an array of calls (`[to, data]`). Phases execute in order; each phase is atomic. If a phase reverts, its state is discarded and remaining phases are skipped. Completed phases persist. See [Call Execution](#call-execution) |
...
The protocol dispatches calls directly from `from` to each call's `to` address with `value = 0`:
...
2. **Bytecode is compatible**: Account code is an [ERC-1167](./eip-1167.md) minimal proxy to a recognized implementation that refuses outbound value transfers when locked, or other bytecode the node recognizes as compatible. Since calls do not carry value, the wallet bytecode is the sole controller of ETH outflows.
...
The protocol dispatches each call directly to the specified `to` address with `msg.sender = from` and `value = 0`. This gives every account — including codeless EOAs — the ability to make arbitrary contract calls (token transfers, DeFi interactions, etc.) without requiring wallet code to interpret calldata. ETH transfers route through the account's wallet code via a self-call, keeping the wallet bytecode as the sole controller of ETH outflows — critical for the [Account Lock](#account-lock) guarantee and saving calldata. All owners are equal and fully authorized; there is no wallet-level call gating to enforce.
...
Single read for account-level policies. The `locked` flag gives nodes the guarantee that owners cannot change, enabling elevated mempool throughput for locked accounts — critical for permissioned payer infrastructure and high-frequency senders. The node only needs to read this slot if the account is attempting to send more transactions than the mempool allows per sender/payer by default. 
...
**Signature Size Limits**: Signatures exceeding `MAX_SIGNATURE_SIZE` May be rejected to prevent DoS via oversized signatures.
