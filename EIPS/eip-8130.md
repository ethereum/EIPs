...
**`sender_auth_cost`**: For EOA signatures (`sender` empty) or native ecrecover signatures (`ECRECOVER_VERIFIER`): 6,000 gas (ecrecover + 1 SLOAD + overhead). For other configured owner signatures (`sender` set): 1 SLOAD (`owner_config`) + cold code access + actual gas consumed by verifier execution.
...
1. **Resolve sender**: If `sender` empty, ecrecover derives the sender address (EOA path). The effective verifier is `ECRECOVER_VERIFIER` and `ownerId = bytes32(bytes20(sender))`. If `sender` set, read the first 20 bytes of `sender_auth` as the verifier address.
...
4. **Authorize**: SLOAD `owner_config(sender, ownerId)`. **Implicit EOA rule**: if the slot is empty and `ownerId == bytes32(bytes20(sender))`, treat as implicitly authorized with scope `0x00`. Otherwise, require that the stored verifier address matches the effective verifier.
