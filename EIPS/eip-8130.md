...
Both paths share the same signed operations and `change_sequence` counters. `applyConfigChange()` parses the verifier address from `authorizerAuth`, calls the verifier (native ecrecover for `ECRECOVER_VERIFIER`, STATICCALL otherwise) to get the `ownerId`, and checks `owner_config` (including the implicit EOA rule). Anyone (including relayers) can call these functions; authorization comes from the signed operation, not the caller. All owner modification paths are blocked when the account is locked (see [Account Lock](#account-lock)).
...
**Delegation Security**: Delegation entries are authorized by the transaction's `sender_auth` and require the sender to be the account's implicit EOA owner (`ownerId == bytes32(bytes20(from))`) with CONFIG scope. The delegation is only permitted on accounts with no code or with an existing delegation designator, preventing accidental overwrite of deployed contract bytecode. Delegation is blocked when the account is locked.
