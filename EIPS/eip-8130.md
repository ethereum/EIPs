...
AA_TX_TYPE || rlp([
  ...
  sender_auth,
  payer_auth          // empty = sender-pay, ECRECOVER_VERIFIER || data = sponsored
])
...
| `gas_limit` | Maximum gas budget for intrinsic costs, sender authentication, payer authentication, account changes, and call execution (see [Intrinsic Gas](#intrinsic-gas)) |
...
| `payer_auth` | Payer authorization. **Empty**: self-pay. **Non-empty**: `ECRECOVER_VERIFIER || data`. See [Payer Modes](#payer-modes) |
...
Intrinsic gas consumes `gas_limit` before call execution. The remaining gas is available to `calls`, and unused gas is refunded to the payer. The payer is charged for total gas consumed, capped by `gas_limit * effective_gas_price`.

The sender verifier runs first, and its metered cost is included in `gas_limit`. This lets the payer sign a single maximum gas exposure without committing to the sender's verifier choice. Payer authentication is restricted to native secp256k1 so the payer cannot choose a variable-cost verifier that changes the gas available for call execution.
...
**`payer_auth_cost`**: 0 for self-pay (`payer` empty). For sponsored transactions, `payer_auth` MUST use `ECRECOVER_VERIFIER` and costs 6,000 gas (ecrecover + 1 SLOAD + overhead). Other payer verifier addresses are invalid.
...
**`payer_auth`** — uses the `verifier || data` format with `verifier == ECRECOVER_VERIFIER`:

| `payer` | `payer_auth` | Payer Address | Validation |
|---------|-------------|---------------|------------|
| empty | empty | `sender` | Self-pay — no payer validation |
| address | `ECRECOVER_VERIFIER (20) \|\| data` | `payer` field | Sponsored — native secp256k1 payer authorization. Reads payer's `owner_config`, validates against `payer` address |
...
| `getMaxCost()` | `uint256` — `gas_limit * max_fee_per_gas` | Validation + Execution |
| `getGasLimit()` | `uint256` — total gas budget (`gas_limit`) before intrinsic costs and call execution | Validation + Execution |
...
6. Resolve payer from `payer` and `payer_auth`:
   ...
   - `payer` = 20-byte address (sponsored): `payer_auth` MUST use `ECRECOVER_VERIFIER`. Validate `payer_auth` against the `payer` address's `owner_config`. Require PAYER scope on the resolved owner.
...
**Payer Security**: `AA_TX_TYPE` vs `AA_PAYER_TYPE` domain separation prevents signature reuse between sender and payer roles. The `payer` field in the sender's signed hash binds to a specific payer address. Scope enforcement adds a second layer — PAYER-only owners cannot be used as `sender_auth`, and vice versa. The payer's maximum gas exposure is bounded by signed fee fields because `gas_limit` includes sender authentication, payer authentication, intrinsic costs, account changes, and call execution. Sponsored payer authentication is restricted to native secp256k1, giving it a fixed protocol cost and preventing the payer's verifier choice from changing the gas available for execution.
