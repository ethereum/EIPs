...
When such a change is applied unsequenced (just-in-time), binding the prior `actor_config` value does not by itself prevent replay, because a configuration can recur: an actor removed and later re-added returns the record to a prior value, so a stored value or a state-root membership check cannot tell a first application from a replay. To make an unsequenced change single-effect, the digest also binds a per-actor monotone counter that the keystore advances on every accepted change to that actor and preserves through revocation and expiry, so the signature names a counter value that cannot become current a second time. This counter is stored per actor and is not part of the transaction signature, so it MUST be named as a signed field in the change for the check to bind. Sequenced changes are already single-use through the sequence counter and do not need it.
