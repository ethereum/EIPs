...
| `lane` (`nonceLane`) | 27–28 | `uint16` nonce-lane restriction (see [Actor Lanes](#actor-lanes)) |
| reserved | 29–31 | MUST be zero on write (native and EVM paths). Implicit version discriminator: new-format configs with nonzero reserved fail to apply through legacy deployments rather than applying with restrictions dropped |
...
| 4 | `0x10` | POLICY | Gated initiation: `sender_auth`; may originate transactions only to the actor's `policy_manager` (see [Actor Policies](#actor-policies)) |
| 5–7 | | (spare) | Reserved for future pure grants |

**Initiation modes.** `SENDER` and `POLICY` are mutually exclusive at **use** time, not at write time. The protocol (native validation / call execution) MUST reject a `sender_auth` whose resolved `scope` sets both, or sets `POLICY` together with `SIGNATURE` or `CONFIG`. `POLICY | PAYER` is permitted: the gate constrains calls, not gas, so a session key may self-pay within the account's ETH balance and its `expiry`. An invalid combo stored on-chain is inert (fails authorization) rather than unwritable, so `authorizeActor` does not enumerate combinations.

**Verbatim reporting.** `getActorConfig` and `authenticateActor` MUST return stored scope (and lane) bytes unmodified — no normalization or masking of unrecognized bits.

Scope authorization is applied only after an authenticator authenticates the signature and returns an `actorId`: the protocol loads that actor's `scope` and checks it against the context being authorized. For `sender_auth`, require `scope == 0x00 || (scope & (SENDER | POLICY)) != 0`, then apply the exclusivity rules above. For `payer_auth` / self-pay require PAYER (or unrestricted); for `verifySignature()` require SIGNATURE (or unrestricted); for config change `auth` require CONFIG (or unrestricted).
...
The `nonceLane` field (`uint16`, stored as `lane` in `actor_config`) is a **restriction** on which nonce channel a key may use for sender-context sequenced transactions — not a reservation of that channel. No exclusivity among live actors is promised. Wallets learn the value from the signed `authorizeActor` payload, `ActorAuthorized`, or `getActorConfig`. The inline secp256k1 self has no lane field and always reports `0`.

| `nonceLane` | Meaning |
|-------------|---------|
| `0x0000` | Unbound: any `nonce_key`, including nonceless (`NONCE_KEY_MAX`) |
| `0x0001`–`0xFFFE` | Bound: that lane (`nonce_key == uint256(lane)`) **or** nonceless |
| `0xFFFF` | Nonceless only (`nonce_key` MUST be `NONCE_KEY_MAX`) |

Lane constraints apply to **sender-context** transactions only. `payer_auth` consumes no payer-side sequence and is unaffected. Exact validity coupling (reject mismatched `nonce_key` at validation) is enforced by the protocol; see [Validation](#validation).
...
**Scope.** `SCOPE_POLICY` is the gated initiation grant. It MUST NOT be combined with `SENDER`, `SIGNATURE`, or `CONFIG` at use time (see [Actor Scope](#actor-scope)). It MAY combine with `PAYER` so a session key can self-pay gas; that gas spend is not policy-gated and is bounded by balance and `expiry`. `SIGNATURE` is excluded so permit-style ERC-1271 bypasses are structurally closed; session-key signing SHOULD use an account-level approved-hash / approve-typed-data pattern (Safe `approveHash` precedent) rather than granting `SIGNATURE` on the policy key.

**Reference flow** (non-normative). One construction of a session-key policy:

1. **Authorize.** The account authorizes the key with `SCOPE_POLICY` (optionally `| PAYER`), a `nonceLane`, the `manager`, and a `commitment`. The key's call authority is only the gate to its `manager`.
...
**Example** (non-normative). A subscription session key limited to **5 USDC per 30-day period** with a two-target allowlist. Authorized with `SCOPE_POLICY` on a bound `nonceLane` (and without `PAYER` if a sponsor pays gas). Over-budget transfers, wrong targets, expiry, or revoke all fail.
...
Restrictions the Account Configuration contract must enforce are complete at deployment: the signer's CONFIG grant, reserved-bytes-zero, and well-formed `policyData` when `SCOPE_POLICY` is set. Restrictions it merely stores (`nonceLane`) can grow via fork-level validation without rewriting historical configs. Grants can grow because reads fail closed on unknown bits. Signing semantics (e.g. approved typed data for session keys) evolve at the account layer, not in this contract.
...
ACTOR_TYPEHASH =
  keccak256("Actor(bytes32 actorId,ActorConfig config,bytes policyData)"
            "ActorConfig(address authenticator,uint8 scope,uint48 expiry,uint16 nonceLane)")
...
// Per-actor. For imported actors scope, expiry, and nonceLane are 0 and policyData is empty:
configHash_i = keccak256(abi.encode(ACTORCONFIG_TYPEHASH, authenticator_i, scope_i, expiry_i, nonceLane_i))
...
4. **Check scope**: Read the resolved `scope` byte (from the inline default-EOA config for the secp256k1 self-actor, or `actor_config` otherwise). For `sender_auth`, require `scope == 0x00 || (scope & (SENDER | POLICY)) != 0`, then reject illegal POLICY combinations (`POLICY` with `SENDER`, `SIGNATURE`, or `CONFIG`). For `payer_auth` require PAYER (or unrestricted); for `verifySignature()` require SIGNATURE (or unrestricted); for config `auth` require CONFIG (or unrestricted). In self-pay (no `payer_auth`), additionally require PAYER (or unrestricted) on the resolved sender actor.
5. **Check nonce lane** (sender-context only): Read `nonceLane` from `actor_config` (inline self ⇒ `0`). If `nonceLane == 0x0000`, accept any `nonce_key`. If `nonceLane == 0xFFFF`, require `nonce_key == NONCE_KEY_MAX`. Otherwise require `nonce_key == uint256(nonceLane)` or `nonce_key == NONCE_KEY_MAX`. Reject nonzero reserved bytes on the loaded config.
...
|-------------|------|--------|-------------|
| `0x01` | `authorizeActor` | `abi.encode(ActorConfig config, bytes policyData)` (see [`ActorConfig`](#iaccountconfiguration)) | Authorize a new actor. Writes `actor_config` with `authenticator`, `scope`, `expiry` (`0` = no expiry), and `nonceLane` verbatim (unknown scope bits stored as-is). If `SCOPE_POLICY` is set, requires `policyData = manager ‖ commitment` (both nonzero) and writes those slots; otherwise requires empty `policyData` and clears policy slots. Rejects nonzero reserved bytes. Does **not** enforce scope-combination exclusivity (that is use-time / protocol). Emits `ActorAuthorized`. |
...
A bound actor (`nonceLane` in `0x0001..0xFFFE`) on a locked account reduces mempool invalidation to a single `(account, lane)` counter, enabling per-lane caching and rate limits (the lane dividend).
...
Because Account Configuration is a single immutable deployment, policy vocabulary is not encoded as protocol sub-type bytes. Excluding `SIGNATURE` on policy keys structurally closes permit-style ERC-1271 bypasses; session-key signing belongs at the account layer (approved-hash / approve-typed-data). Allowing `POLICY | PAYER` lets a session key self-pay a small ETH budget without escaping the call gate.

### Why Actor Lanes?

Parallel session keys should not share the owner's nonce channel. `nonceLane` is a per-actor **restriction** (not a unique reservation): `0` unbound, `1..0xFFFE` that lane or nonceless, `0xFFFF` nonceless only. That protects owner sequencing from session-key traffic, enables coordination across many keys, and still allows nonceless when durable state growth is undesirable. Combined with account lock, a bound actor reduces mempool invalidation to a single `(account, lane)` counter, enabling per-lane caching and rate limits.
...
interface IAccountConfiguration {
    ...
    struct ActorConfig {
        ...
        uint8 scope;        // grants bitmask; 0x00 = unrestricted; 0x10 = SCOPE_POLICY
        uint48 expiry;      // Unix seconds; 0 = no expiry. Actor invalid once block.timestamp > expiry
        uint16 nonceLane;   // protocol-side; stored verbatim. 0 = unbound; 1..0xFFFE = lane or nonceless; 0xFFFF = nonceless only
    }
    ...
    //   authenticator(20) || scope(1) || expiry(6) || nonceLane(2) — 29 bytes —
    //   and, only when scope & SCOPE_POLICY != 0, manager(20) || commitment(32) (81 bytes total).
    ...
    function authenticateActor(address account, bytes32 hash, bytes calldata auth)
        external view returns (uint8 scope, uint16 nonceLane, address policyTarget);

    // Storage views
    function isActor(address account, bytes32 actorId) external view returns (bool);
    // Returns stored bytes unmodified (verbatim reporting). Inline k1 self reports nonceLane 0.
...
**Actor Scope and Policy**: Scope grants are protocol-enforced after authenticator execution during validation (fail closed). The policy gate is protocol-enforced during execution (`ActorPolicyViolation`). The gate constrains calls, not gas: `POLICY | PAYER` may self-pay, with drain bounded by balance and `expiry`; sponsored gas is unchanged. `POLICY` excluding `SIGNATURE` structurally closes permit-style bypasses; session-key signing SHOULD use an account-level approved-hash / approve-typed-data pattern (Safe `approveHash` precedent). Illegal `POLICY` combinations with `SENDER` / `SIGNATURE` / `CONFIG` are rejected at use time (see [Actor Scope](#actor-scope)).
