...
Stateful verifiers can read from the Transaction Context precompile (see [Transaction Context](#transaction-context)). When called outside of an 8130 transaction (e.g., `verifySignature()` in a legacy transaction), the precompile returns zero/default values, so stateful verifiers that require context naturally reject those calls.
