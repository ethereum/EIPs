...
| **Deferred pay** | `0x80` (empty) | `0x03` | `from` (after committed_calldata) | Gas deducted after `committed_calldata` executes. Rejected if `key_policy.requireSponsor` set. |
...
1. ETH gas deduction from payer. **Deferred pay**: skip — deferred to step 8.
...
8. Execute `committed_calldata` via `ENTRY_POINT_ADDRESS` if non-empty (committed independently). **Deferred pay**: ETH gas deduction from `from` after this step. Transaction is invalid and cannot be included if `from` has insufficient ETH at this point.
9. Execute `calldata` via `ENTRY_POINT_ADDRESS` if non-empty

`committed_calldata` (step 8) is committed independently — its state changes persist if step 9 reverts. Unused gas is refunded to the payer. For steps 4–6, the protocol SHOULD inject log entries into the transaction receipt (e.g., `KeyAuthorized`, `AccountCreated`) matching the events defined in the [IAccountConfig](#iaccountconfig) interface, following the protocol-injected log pattern established by [EIP-7708](./eip-7708.md).
...
The wallet fully interprets both payloads — batching, multicall, or any other execution pattern is the wallet's responsibility, not the protocol's.
...
- Accounts gain AA capabilities by configuring keys and deploying code (via [EIP-7702](./eip-7702.md) or `account_initialization`) to interpret calldata delivered by the protocol
...
**Deferred Pay**: The deferred pay mode defers gas deduction, exposing block builders to unpaid computation if `committed_calldata` fails to produce sufficient ETH. This risk is borne by the builder, not the protocol — the transaction is invalid if the sender has insufficient ETH after `committed_calldata`. Builders/mempools can choose to mitigate risk by tracing mechanisms and restrictions, risk systems, gas limits to the committed_calldata phase or allowlists.
