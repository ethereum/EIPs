...
The per-component gas *values* in this section reflect the EVM access and data-availability costs ([EIP-2929](./eip-2929.md) / [EIP-2028](./eip-2028.md)) at the time of writing. Their normative status depends on the chain's [adoption profile](#adoption-profiles):

- **L1 profile:** the schedule is **normative**. The values below are protocol constants — the gas schedule *is* part of the specification — identical for every node and changeable only through a hard fork. A chain MUST NOT adopt a different intrinsic-gas schedule under this profile.
- **L2 profile:** the schedule is a **recommended default**. A chain MAY adopt a different intrinsic-gas schedule (for example to track a future EVM repricing or a local cost model), provided it is fixed under that chain's consensus and identical across its nodes.
