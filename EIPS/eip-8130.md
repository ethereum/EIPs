...
4. **Check scope**: Read the resolved `scope` byte (from the inline default-EOA config for the secp256k1 self-actor, or `actor_config` otherwise). For `sender_auth`, require `scope == 0x00 || (scope & (SENDER | POLICY)) != 0`; when `POLICY` is set the actor is gated to its `manager` regardless of `SENDER`. Self-pay is defined relationally (payer account == sender account): require SELF_PAYER (or unrestricted) on the actor that authorizes payment — the resolved sender actor when `payer` is empty, or the `payer_auth`-resolved actor when `payer` explicitly equals `sender`. For sponsorship (payer account ≠ sender) require SPONSOR_PAYER (or unrestricted) on the `payer_auth`-resolved actor. For `verifySignature()` and for config `auth` require admin (`scope == 0x00`).
...
**`payer_auth`** uses the same `authenticator || data` format as `sender_auth`. The governing distinction is **relational**: a transaction is *self-pay* when the payer account equals the sender account, and *sponsored* when it does not. Self-pay requires SELF_PAYER on whichever actor authorizes payment; sponsorship requires SPONSOR_PAYER on the `payer_auth`-resolved actor.
