...
| `NATIVE_K1_VERIFIER` | `address(1)` | Native secp256k1 (ECDSA) verifier sentinel — protocol handles ecrecover directly, no contract call |
...
**Implicit EOA authorization**: An unregistered owner (`owner_config` slot is empty) is implicitly authorized if `ownerId == bytes32(bytes20(account))`. The empty slot's scope byte is `0x00` (unrestricted), granting full permissions by default. This allows every existing EOA to send AA transactions immediately without prior registration. When the implicit rule applies, the protocol verifies using native ecrecover (`NATIVE_K1_VERIFIER`) rather than calling an external verifier contract. The implicit authorization is revoked by writing a revocation marker to the slot (`verifier = address(0)` with non-zero scope), making it non-empty. The EOA owner can also be explicitly registered with `NATIVE_K1_VERIFIER` to set a custom scope while retaining native ecrecover verification.
...
`NATIVE_K1_VERIFIER` (`address(1)`) is a protocol-reserved sentinel for native secp256k1 verification. When the protocol encounters this address as a verifier in auth data, it performs ecrecover directly rather than making a STATICCALL. The `data` portion is interpreted as raw ECDSA `(r || s || v)`, and the returned `ownerId` is `bytes32(bytes20(recovered_address))`. `NATIVE_K1_VERIFIER` is not a deployed contract — it is handled entirely by the protocol.
...
| **EOAs** | EOAs send AA transactions using their existing secp256k1 key (native ecrecover via `NATIVE_K1_VERIFIER`). If the account has no code, the protocol auto-delegates to `DEFAULT_ACCOUNT_ADDRESS` (see [Block Execution](#block-execution)). Accounts MAY override with a delegation entry in `account_changes` or a standard [EIP-7702](./eip-7702.md) transaction | Wallet-defined; EOA recoverable via 1559/7702 transaction flows |
...
**`sender_auth_cost`**: For EOA signatures (`from` empty) or `NATIVE_K1_VERIFIER` signatures: 6,000 gas (native ecrecover + 1 SLOAD + overhead). For other configured owner signatures (`from` set): 1 SLOAD (`owner_config`) + cold code access + actual gas consumed by verifier execution.
...
The first 20 bytes identify the verifier address. When the verifier is `NATIVE_K1_VERIFIER`, `data` is raw ECDSA `(r || s || v)` and the protocol handles ecrecover natively. For all other verifiers, `data` is verifier-specific — each verifier defines its own wire format.

##### Validation

1. **Resolve sender**: If `from` empty, ecrecover derives the sender address (EOA path). The effective verifier is `NATIVE_K1_VERIFIER` and `ownerId = bytes32(bytes20(sender))`. If `from` set, read the first 20 bytes of `sender_auth` as the verifier address.
...
3. **Verify**: If the verifier is `NATIVE_K1_VERIFIER`, the protocol natively ecrecovers from `data` (as `r || s || v`), returning `ownerId = bytes32(bytes20(recovered_address))`. For the EOA path, ecrecover was already performed in step 1. For all other verifiers, call `verifier.verify(hash, data)` via STATICCALL. Returns `ownerId` (or `bytes32(0)` for invalid).
...
Both paths share the same signed operations and `change_sequence` counters. `applyConfigChange()` uses a two-tier authorization model: the Account Configuration Contract parses the verifier address from `authorizerAuth`. If the verifier is `NATIVE_K1_VERIFIER`, the contract uses ecrecover to get the `ownerId`; otherwise it calls the verifier contract. It then checks `owner_config` (including the implicit EOA rule). If direct verification fails (owner not found in `owner_config`), the contract falls back to the account's `isValidSignature` ([ERC-1271](./eip-1271.md)) for authorization. This fallback enables migration for existing wallets that have not yet registered owners. Anyone (including relayers) can call these functions; authorization comes from the signed operation, not the caller. All owner modification paths are blocked when the account is locked (see [Account Lock](#account-lock)).
...
rlp([
  ...
  authorizer_auth     // EOA authorization (NATIVE_K1_VERIFIER, account-bound ownerId) with CONFIG authorization
])
```
...
The `authorizer_auth` follows the same [Signature Format](#signature-format) as `sender_auth` (`verifier || data`). For delegation entries, it MUST resolve to the account's EOA owner identity via `NATIVE_K1_VERIFIER` and pass CONFIG authorization checks.
...
4. If config change or delegation entries present in `account_changes`: reject if account is locked (see [Account Lock](#account-lock)). Simulate applying operations in sequence, skip already-applied entries. For delegation entries: verify `code_size(account) == 0` or existing delegation designator, and verify `authorizer_auth` resolves to `ownerId == bytes32(bytes20(account))` with `NATIVE_K1_VERIFIER` and passes CONFIG authorization checks.
...
[EIP-7702](./eip-7702.md) introduced `authorization_list` as a transaction-level field for code delegation, with ECDSA authority. This proposal keeps delegation in `account_changes`, aligning it with the same sequencing and portability model as other account configuration actions.

In this version, delegation authorization is intentionally limited to the account's EOA identity (`NATIVE_K1_VERIFIER`, account-bound `ownerId`) so accounts remain portable across non-8130 chains using standard [EIP-7702](./eip-7702.md) transactions.
...
**Owner Management**: Config change authorization requires CONFIG scope. The EOA owner is implicitly authorized with unrestricted scope; revocable via portable config change (revocation overwrites the slot with `verifier = address(0)` and non-zero scope to prevent implicit re-authorization). All owner modification paths are blocked when the account is locked.
...
**Delegation Security**: Delegation entries are EOA-authorized only in this version: `authorizer_auth` must resolve via `NATIVE_K1_VERIFIER` to `ownerId == bytes32(bytes20(account))` and pass CONFIG authorization checks. The delegation is only permitted on accounts with no code or with an existing delegation designator, preventing accidental overwrite of deployed contract bytecode. Cross-chain delegation operations (chain_id 0) share the same sequence channel as config changes, ensuring deterministic ordering.
