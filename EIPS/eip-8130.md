...
8130 is specified once but adopted under one of two **profiles**. A chain MUST declare which profile it activates; the profile fixes the parameters this specification otherwise leaves open: the [intrinsic-gas schedule](#intrinsic-gas), how the enshrined [canonical authenticator set](#canonical-authenticator-set) evolves, and which [authenticator classes](#authenticator-classes) the 8130 transaction path accepts. The **canonical set is accepted on the transaction path under both profiles**, always at a constant enshrined validation cost, so a wallet targeting the canonical set behaves identically everywhere; the profiles differ only in what else is allowed and how it changes, never in transaction semantics.

**L1 profile (normative schedule, CANONICAL through ARBITRARY).** For a base layer, where the gas schedule *is* the specification and consensus validity MUST NOT depend on any out-of-band configuration. Under this profile:
...
- Authenticator acceptance covers tiers **CANONICAL through ARBITRARY** (see [Authenticator Classes](#authenticator-classes)). Consensus validity is unchanged: any authenticator whose `STATICCALL` returns within `MAX_AUTHENTICATION_GAS` remains valid in a block. Tiers govern mempool propagation, revalidation, and rate limits only. No allowlist is maintained; a base layer can afford this bounded, single-call validation.
...
Authenticator acceptance is **tier CANONICAL only**: the 8130 transaction path accepts only the enshrined canonical set (fixed validation cost, no arbitrary `STATICCALL` on the hot path). Non-canonical authenticators remain registrable and usable through the EVM (config-change calls, recovery flows, [ERC-4337](./eip-4337.md)), just not on the 8130 transaction path. An L2-profile chain is still internally consistent; wallets that need cross-chain portability SHOULD rely only on the canonical set and the recommended schedule.

The gas-schedule policy and the acceptance policy are independent choices; the two profiles name the common pairings, and a chain MAY combine them differently. What is universal is the **canonical set**: every compliant chain accepts it on the 8130 transaction path at a constant enshrined cost, so a wallet can rely on canonical authenticators working everywhere. Additional authenticators may be registered on any chain (for example, recovery flows); they are usable on the 8130 transaction path only where a chain's policy includes their [authenticator class](#authenticator-classes), otherwise they are reached through the EVM.

A chain MAY accept any downward-closed prefix of the ordered tiers CANONICAL, PURE, BOUNDED, ARBITRARY (for example CANONICAL+PURE, or CANONICAL+PURE+BOUNDED) as its mempool policy without any protocol change. Consensus validity is unaffected: under the L1 profile an authenticator returning within `MAX_AUTHENTICATION_GAS` remains valid in a block regardless of the node's mempool prefix.
...
Nodes MUST accept all canonical authenticators on the 8130 transaction path; this is the universal baseline wallets can rely on everywhere. Whether an authenticator **outside** the canonical set is accepted on the transaction path is the chain's acceptance policy (see [Adoption Profiles](#adoption-profiles)): under the L1 profile any authenticator returning within `MAX_AUTHENTICATION_GAS` remains consensus-valid and is classified into PURE, BOUNDED, or ARBITRARY for mempool treatment (see [Authenticator Classes](#authenticator-classes)); under the L2 profile it is not accepted on the transaction path, so it must be reached through the EVM (config-change calls, [ERC-4337](./eip-4337.md), direct Keystore). The 8130 path is intended to use a small, standard set of signature algorithms, and wallets that need cross-chain portability SHOULD rely only on the canonical set.

#### Authenticator Classes

Mempool acceptance of a declared authenticator is classified into one of four tiers. Consensus validity is unchanged: under the L1 profile any authenticator whose `STATICCALL` returns within `MAX_AUTHENTICATION_GAS` remains valid in a block. Tiers govern mempool propagation, revalidation, and rate limits only.

- **CANONICAL**: the enshrined set, including canonical wrappers such as the [delegate authenticator](#delegate-authenticator). Fixed validation cost, no EVM, no tracing. Accepted on the 8130 transaction path on every compliant chain.
- **PURE**: non-canonical bytecode that reads no state. Classified once per code hash: statically when the runtime bytecode contains no state-observing opcodes (`SLOAD`, `BALANCE`, `SELFBALANCE`, `EXTCODE*`, `CALL` / `STATICCALL` / `DELEGATECALL` / `CALLCODE`, `TIMESTAMP`, `NUMBER`, `BLOCKHASH`, `PREVRANDAO`, `BASEFEE`, `BLOBBASEFEE`, `GASPRICE`, `COINBASE`, `GASLIMIT`, `ORIGIN`, `CREATE*`), otherwise by a single classification trace on first sight. Classification is cacheable and stable: pure code cannot become impure. Dependency set: none beyond the standard slots (`nonce`, `actor_config`, payer balance).
- **BOUNDED**: non-canonical bytecode whose classification trace touches only enumerated dependency classes, each with a defined revalidation key: (a) Keystore `actor_config` slots (key: `(account, actorId)`); (b) a single sender-keyed slot in a caller-named contract, for example an ERC-20 `balances[sender]` or a nullifier slot (key: `(contract, slot)`); (c) the recent-root registry ring (key: `(source_id, slot)`), when that companion is active. Classification records the dependency template; nodes index pending transactions by the concrete keys and revalidate only on writes to them. Classes are extensible by amending this list (mempool policy, not consensus).
- **ARBITRARY**: anything else that returns within `MAX_AUTHENTICATION_GAS`. Traced per transaction; subject to node-local rate limits; nodes MAY decline to propagate.

An authenticator is classifiable per code hash because it holds no per-account authority (authority lives in `actor_config`), so it can be genuinely stateless. Validation code that reads its own account's storage is a function of `(code, storage)` and admits no stable per-code classification.
...
Nodes SHOULD classify non-canonical authenticators per code hash on first sight and cache the result (see [Authenticator Classes](#authenticator-classes)); reclassification is never required for PURE. Nodes SHOULD apply per-tier pending and rate limits and MAY grant BOUNDED transactions the indexed-revalidation treatment described there. Authenticator tiers are node policy: a block MUST NOT be considered invalid because a node's mempool would not have propagated one of its transactions.

Nodes MAY apply higher pending transaction rate limits based on account lock state. A node can cheaply read the packed account-state slot and grant the higher tier when the account is `LOCKED`, has **no pending unlock** (`UNLOCK_INITIATED` clear), and carries an `unlock_delay` at or above the node's threshold (e.g. ≥ 6 hours) — the actor set is then frozen for at least that window. The lock-based higher-rate tiers compose with authenticator classes: the stable-signature condition is a locked sender together with a PURE or CANONICAL authenticator.

- **Locked sender**: A locked `sender` account has a stable signature if combined with a PURE or CANONICAL authenticator. Nodes can safely allow a higher sender rate.
...
The two [adoption profiles](#adoption-profiles) make that split explicit rather than implicit. The **L1 profile** enshrines every consensus-critical parameter (the gas schedule, the canonical authenticator set, and the pricing of enshrined authenticators), so that two independent implementations agreeing on this document produce identical block validity, and changing any of them requires a hard fork rather than an off-chain configuration or companion-ERC rollout. The [authenticator classes](#authenticator-classes) are mempool policy, not one of those parameters: under the L1 profile block validity still depends only on `MAX_AUTHENTICATION_GAS` and the enshrined canonical costs. The **L2 profile** keeps the flexibility that lets a faster-moving chain track a local cost model or a future EVM repricing, at the cost of cross-chain uniformity. Because transaction semantics are identical across both, a wallet targeting the canonical set and the recommended schedule is portable regardless of which profile a chain runs.
...
**Validation Surface**: For CANONICAL authenticators, invalidators are `actor_config` changes and nonce consumption. CANONICAL authenticators are pure functions of `(hash, data)` and the resolved `actor_config`, so a validated transaction is invalidated only by those two events, keeping the mempool invalidator set small and enumerable (this is why a CANONICAL-only acceptance policy suits high-throughput chains). PURE authenticators share that invalidator set (standard slots only: nonce, `actor_config`, payer balance). BOUNDED authenticators add the enumerated dependency keys recorded at classification; nodes revalidate only on writes to those keys. ARBITRARY authenticators may read mutable state, so an accepted transaction can be invalidated by state beyond those keys, and a node MUST run additional mempool-validation machinery to bound that (for example, tracing per transaction and applying node-local rate limits). A base layer typically wants this larger surface; the execution cost is already bounded per call by `MAX_AUTHENTICATION_GAS` and each authentication is a single `STATICCALL` returning an `actorId`, so the extra machinery is about *invalidation tracking*, not unbounded execution. 8130 makes this an opt-in, allow-only-what-you-need extension rather than an all-or-nothing choice: the declared-authenticator model lets a chain enshrine the CANONICAL set and admit PURE, BOUNDED, or ARBITRARY authenticators only where a use case needs them (and a chain that needs none runs no such machinery at all), instead of having to admit arbitrary wallet-defined validation code wholesale. Tiers never affect block validity: under the L1 profile an authenticator returning within `MAX_AUTHENTICATION_GAS` remains valid in a block.
