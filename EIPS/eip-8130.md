...
Authenticators are executed via STATICCALL. The 8130 transaction path accepts only canonical authenticators from the node allowlist (see [Canonical Authenticator Set](#canonical-authenticator-set)); these are enshrined contracts at deterministic CREATE2 addresses whose code is fixed rather than an [EIP-7702](./eip-7702.md) code delegation (`0xef0100`), so no separate delegation-indicator check on the authenticator is required on that path. (This concerns the authenticator's own code, not the canonical [delegate authenticator](#delegate-authenticator), which is itself such an enshrined contract.) Non-canonical authenticators are reachable only within EVM execution and are registered at the account's own discretion.
...
| `1` | `RevokeActor` | `abi.encode(bytes32 actorId)` | Revoke an actor. Deletes `actor_config` (and `policy_commitment`/`policy_manager`). For the self-actor (`actorId == bytes32(bytes20(account))`) it additionally sets `DEFAULT_EOA_REVOKED`, disabling the inline secp256k1 self; the slot delete then also clears a non-k1 self if one was authorized under the self-actorId (a k1 self is held inline and has no `actor_config` slot, so only the flag takes effect). Emits `ActorRevoked`. |
