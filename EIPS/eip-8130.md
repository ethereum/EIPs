...
`POLICY` selects whether an actor is gated. When set at authorize time, `policyData` MUST be `manager` (20 bytes) ‖ `commitment` (32 bytes) with both nonzero, and those values are written to `policy_manager` / `policy_commitment`. When unset, `policyData` MUST be empty and any prior policy slots are cleared. Policy vocabulary lives in the commitment and manager logic.
...
|-------------|------|--------|-------------|
| `0x01` | `authorizeActor` | `abi.encode(ActorConfig config, bytes policyData)` (see [`ActorConfig`](#iaccountconfiguration)) | Authorize a new actor. Writes `actor_config` with `authenticator`, `scope`, and `expiry` (`0` = no expiry) verbatim (unknown scope bits stored as-is). If `POLICY` is set, requires `policyData = manager ‖ commitment` (both nonzero) and writes those slots; otherwise requires empty `policyData` and clears policy slots. Rejects nonzero reserved bytes. Does **not** enforce scope-combination exclusivity (that is use-time / protocol). Emits `ActorAuthorized`. |
