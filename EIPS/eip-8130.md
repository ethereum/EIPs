...
This proposal introduces a new [EIP-2718](./eip-2718.md) transaction type and an on-chain account configuration system that together provide minimal native account abstraction: custom authentication, call batching, and gas sponsorship. Accounts register actors with authenticator contracts, and transactions declare the authenticator used for validation. Policy actors, account locking, keyed nonces, and nonce-free transactions are left to a companion extension.
...
| 2 | `0x04` | (reserved) | Reserved for a companion extension |
...
Admin authority is exactly `scope == 0x00`. For `sender_auth`, require admin or `SENDER`. For self-payment, require admin or `SELF_PAYER`. For sponsorship, require admin or `SPONSOR_PAYER`. Configuration changes require admin.
...
| `nonce_key` | `uint256` nonce channel selector. MUST be `0`. Other values are reserved for extensions. |
| `nonce_sequence` | `uint64` expected transaction sequence for `sender`. Must match the current sequence for `(sender, 0)` and is incremented after inclusion regardless of execution outcome. |
...
| `nonce_cost` | 22,100 gas when the sender sequence changes from zero (cold SLOAD + SSTORE set), otherwise 5,000 gas (cold SLOAD + warm SSTORE reset) |
...
5. Validate `sender_auth` against resulting actor state (see [Validation](#validation)) and require admin or `SENDER` for the sender context. Payer scope (SELF_PAYER / SPONSOR_PAYER) is checked in step 6, not here — the sender actor is not required to carry a payer bit. If delegation entries are present, the resolved actor must additionally hold admin EOA-actor delegation authority (see [Delegation Entry](#delegation-entry)).
...
3. Increment the Nonce Manager sequence for `(sender, 0)`.
...
**`eth_getTransactionCount`**: Returns the AA transaction sequence for the account from the Nonce Manager precompile at `NONCE_MANAGER_ADDRESS`.
...
The transaction retains `nonce_key` in its wire format but requires it to be zero. Scope bits `0x02` and `0x04` are stored but grant no authority. Companion proposals may define those reserved values without changing the base actor representation, address derivation, or transaction envelope.
...
Read-only. This proposal only reads `nonceKey == 0`; other values are reserved for extensions. The protocol manages nonce storage directly; there are no state-modifying functions. Gas is charged as a base cost plus a cold SLOAD (2,100 gas) for the first read of `(account, 0)` in the transaction, or a warm SLOAD (100 gas) for subsequent reads of the same slot, consistent with [EIP-2929](./eip-2929.md) access rules.
...
**Replay Protection**: Transactions include `chain_id`, `nonce_key`, `nonce_sequence`, and a validity window (`valid_after`, `valid_before`). This proposal requires `nonce_key == 0`. Inclusion increments `(sender, 0)`, so a given `(sender, nonce_sequence)` can be included at most once on a chain.

**Actor Scope**: Scope grants are protocol-enforced after authenticator execution during validation and fail closed. Unknown and reserved bits grant no authority. See [Actor Scope](#actor-scope).
