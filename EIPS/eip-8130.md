...
10. Require `sender` is fresh: `code_size(sender) == 0`, `nonce(sender) == 0`, and no existing `owner_config` entries (including no revoked EOA owner). This prevents replay against a used EOA or an account whose owners were previously set via `applySignedOwnerChanges()` on another chain.
