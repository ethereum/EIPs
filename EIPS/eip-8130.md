...
description: Enable account abstraction through onchain or checkpointed keystore accounts and a new transaction type
...
An account manages its authentication configuration via the keystore. By default, the Keystore maps an account to its configured authenticators and stores their authorizations onchain. An account MAY instead permanently opt into [Checkpointed Keystore Mode](#checkpointed-keystore-mode), where admins remain defined onchain and non-admin actors are supplied as authenticated witnesses against an admin-approved configuration commitment. Each actor is bound to an **authenticator**, an onchain contract that checks a signature and returns the actor's identity (`actorId`).
...
Checkpointed mode is an opt-in alternative representation of the same actor and policy model. An account selects checkpointed mode during creation or import, and the choice is permanent. Accounts that do not opt in use the existing onchain representation without modification.

Unless this subsection states otherwise, every rule in this EIP applies unchanged. For a checkpointed account, admin actors (`scope == 0`) are defined exclusively onchain. An authenticated witness replaces reads from `actor_config`, `policy_manager`, and `policy_commitment` only for non-admin actors; references elsewhere to stored configuration MUST be interpreted accordingly. Scope, expiry, policy, nonce, sponsorship, batching, call execution, and transaction-signature payloads are unchanged. `AuthorizeActor` and `RevokeActor` manage onchain admins; non-admin actors and policies change through successor image hashes.
...
`settled_checkpointer` and `settled_checkpoint` are the address and `uint56` checkpoint committed by `settled_image_hash`; they MAY share one storage slot. Storing them explicitly prevents a proposed configuration from selecting its own checkpointer or lowering its checkpoint floor. The Keystore stores admin actors using the existing onchain representation, including the inline self-actor rules, but MUST NOT store non-admin actor or policy slots for a checkpointed account. Checkpoint witnesses cannot add, replace, or revoke these admins. Transaction nonces remain in the Nonce Manager and are not part of the checkpointed configuration.
...
The fields have the same meanings as their onchain equivalents. A revealed actor MUST have a nonzero `actor_id`, nonzero `authenticator`, and nonzero `scope`; an admin leaf is invalid. A witness MUST NOT resolve the self-actorId or an actorId with an onchain record, even if that record is expired or has a different authenticator. When `scope & POLICY == 0`, `policy_manager` and `policy_commitment` MUST both be zero. The actor leaf is:
...
The item contributes weight `1` only when `actor_auth` authenticates the enclosing payload through the encoded `authenticator` and returns the encoded `actor_id`; otherwise it contributes weight `0`. The signature bytes do not participate in `actor_leaf`. The recovered topology root is `actors_root`.

##### Image Hash and Checkpointer
...
Successor checkpoints MUST strictly increase and fit in `uint56`. The checkpointer MAY be `address(0)`, which explicitly selects admin-only state approval without an additional checkpointer attestation. Multisignature or weighted authentication remains expressible through an authenticator.
...
The `wallet` argument MUST be the account being authenticated. The proof format is opaque to the Keystore. The call MUST use `STATICCALL`, and its complete cost is authentication gas. Under the L1 profile any checkpointer completing within `MAX_AUTHENTICATION_GAS` is accepted. Under the L2 profile a chain MAY restrict checkpointed accounts to checkpointers it enshrines with fixed validation costs.

The returned checkpoint MUST NOT exceed `2^56 - 1`. Validation MUST invoke only the stored `settled_checkpointer`, and every reconstructed image MUST commit to that address. When it is nonzero, `snapshotFor` MUST return exactly the effective image hash and checkpoint. A zero image hash, revert, malformed return, or mismatch is invalid; none enables a fallback that bypasses the checkpointer.

The effective image MUST equal the settled image or have a checkpoint strictly above the settled checkpoint. A different image at the settled checkpoint is invalid. These checks apply independently of the checkpointer's answer. The checkpointer accepts a state, but cannot confer its authority: an unsettled image additionally requires the onchain admin approval below. A chain settled at checkpoint `0` can therefore validate an approved checkpoint `100` directly without replaying the intervening history.

##### Onchain Admin Co-Authorization

An onchain admin co-signs a state transition by approving its complete successor image hash. The Keystore computes:

```text
approval_hash = keccak256(abi.encode(
    keccak256("CheckpointedStateApproval(address keystore,bytes32 settledImageHash,address checkpointer,bytes32 imageHash,uint56 checkpoint)"),
    KEYSTORE_ADDRESS, settled_image_hash, settled_checkpointer,
    image_hash, checkpoint
))
```

`adminAuth` is an ordinary `sigType || authenticator || data` message envelope over `approval_hash`. The Keystore MUST apply the existing `replaySafeHash(account, chainId, approval_hash)` domain, with `chainId` selected by the Local or Multichain `sigType`, and authenticate the signer exclusively against the account's onchain configuration. It MUST require the signer's configured authenticator to match, its expiry to be live, and its scope to be zero. A checkpointed wrapper is invalid in `adminAuth`; neither `snapshotFor` nor a checkpoint witness can supply the approver's authority. The Keystore MUST perform this check independently of the checkpointer.

The approval binds the account, chain domain, Keystore, settled state, checkpointer, successor checkpoint, and every leaf in the successor image. The Keystore independently reconstructs the image from the actor witness, so a malicious checkpointer cannot substitute a leaf or expand its permissions. Admins cannot be changed by this approval or any checkpointed state transition; they change only through ordinary onchain account changes.

For an unsettled image, `adminAuth` MUST be present and valid on every use. The same approval MAY be reused for transactions under that image; the admin need not sign each transaction. Revoking, expiring, or changing the approving admin can invalidate these witness-carried approvals. Settlement changes the approval's bound base state, so later unsettled images require approval against the new base. A current onchain admin can approve a complete successor directly; historical offchain admin chains are not an alternative authorization path.

These approvals are message authorizations, not account-change batches: `IncrementLocalEpoch` does not revoke them. Restoring an admin can make its prior approvals valid again while the bound settled state and other validity conditions still hold.

For the settled image, `adminAuth` MUST be empty: creation, import, or the onchain admin-authorized `SettleImageHash` has already installed that state. This is a durable onchain grant and does not depend on the continued authority of the admin who installed it. Checkpointer acceptance is still required when a nonzero checkpointer is configured. Onchain admins authenticate independently of the checkpointer and can settle a replacement state or rotate an unavailable checkpointer while the account is unlocked.
...
```text
address(0) (20 bytes) || authenticator (20 bytes) ||
abi.encode(uint56 checkpoint, bytes actorTopology, bytes checkpointerProof, bytes adminAuth)
```

The ABI encoding MUST be canonical with no trailing bytes. `authenticator` MUST be nonzero and equal the authenticator in the single revealed `0xb0` actor item in `actorTopology`. It is the effective authenticator for acceptance and gas accounting; the zero selector is never called as a contract. The topology MUST authenticate exactly that actor with weight `1`. `checkpointerProof` is passed to the stored checkpointer, or MUST be empty when the checkpointer is zero. Rotating the checkpointer requires `SettleImageHash`.

The wrapper is permitted wherever a configured non-admin actor's `authenticator || data` authorization is accepted, including sender and payer authentication and Keystore signature verification. In a `validateSignature` envelope, the existing `sigType` precedes the complete wrapper. The wrapper is invalid for the empty-sender EOA path and for admin authorization. Onchain admins retain the ordinary signature format and validation path.

For the wrapper, replace [configured-actor validation](#validation) with: reconstruct the effective image hash using the stored checkpointer; enforce the settled checkpoint floor and checkpointer acceptance; verify the independent admin approval when the image is unsettled; authenticate the revealed actor over the unchanged transaction or message digest; require the returned actorId and authenticator to match the witness; then apply the existing expiry and context-specific scope rules. The actor's entire effective configuration, including any policy manager and commitment, is snapshotted into the [Transaction Context](#transaction-context) for execution.

While an account is locked, checkpointed validation accepts only its settled image hash; no offchain approval or checkpointer response may advance it. A wallet MUST settle its desired image before locking. Checkpointer availability and acceptance remain necessary for checkpointed authentication; ordinary onchain admins remain available for the existing unlock lifecycle.
...
`importCheckpointedAccount(address account, uint256 chainId, bytes32 initialImageHash, bytes initialConfiguration, InitialActor[] initialAdmins, bytes signature)` is the checkpointed equivalent. It applies the existing import eligibility and ERC-1271 validation rules. `initialConfiguration` and `initialAdmins` follow [Checkpointed Create Entry](#checkpointed-create-entry), with checkpoint `0`. The import digest is `keccak256(abi.encode(keccak256("CheckpointedInitialization(address keystore,address account,uint256 chainId,bytes32 imageHash,bytes32 adminsCommitment)"), KEYSTORE_ADDRESS, account, chainId, initialImageHash, initial_admins_commitment))`. On success it installs both the checkpointed state and the onchain admin set, with the same initialization flags and sequences as checkpointed creation. The signature binds the entire initial state, so neither admins nor the root can be substituted by the relayer.
...
For checkpointed non-admin authentication, phase 2 replaces the actor lookup with proof decoding, topology hashing, optional checkpointer execution, independent admin-approval verification and its onchain state reads, and the acting authenticator. A canonical-only transaction path accepts a checkpointed wrapper only when every participating authenticator and any checkpointer are enshrined with consensus-defined fixed validation costs.
...
For a checkpointed sender or payer, the same constant additionally caps the aggregate authentication work described above. This additional sender-side cap applies only to checkpointed mode; existing onchain sender authentication is unchanged.
...
A checkpointed create charges the writes for its settled image hash, checkpointer, account state, and initial onchain admins. `SettleImageHash` charges its authentication and checkpointed-state writes. All checkpoint authorization bytes remain included in the existing transaction payload costs.
...
The first 20 bytes identify the authenticator address. When the authenticator is `K1_AUTHENTICATOR`, `data` is raw ECDSA `(r || s || v)` and the protocol handles ecrecover natively. For all other authenticators, `data` is authenticator-specific; each authenticator defines its own wire format. The zero selector is reserved for the [checkpointed wrapper](#checkpointed-authorization), whose effective authenticator is the following 20 bytes.
...
2. **Authenticate**: Route by authenticator address. For the EOA path (`sender` empty), ecrecover was already performed in step 1. For `K1_AUTHENTICATOR` (`address(1)`), the protocol natively ecrecovers from `data` (as `r || s || v`), returning `actorId = bytes32(uint256(uint160(recovered_address)))`. For all other authenticators, call `authenticator.authenticate(hash, data)` via STATICCALL, returning `actorId` (or `bytes32(0)` for invalid). `address(0)` is never called as an authenticator; it selects the checkpointed wrapper only for a checkpointed account, and is invalid otherwise.
...
| `0x03` | Checkpointed create | Deploy a new checkpointed account with an initial image hash (must be first and mutually exclusive with `0x00`) |

Create and delegation entries are authorized by the transaction's `sender_auth` and there is no separate authorization field. An onchain create commits its initial actors to the derived address; a checkpointed create commits its initial image hash and onchain admins. Delegation requires the sender to be the account's implicit EOA actor and admin (`scope == 0x00`). Config change entries carry their own admin `auth` and a `channel`/`sequence` for deterministic ordering and cross-chain replay (see [Config Change Authorization](#config-change-authorization)). Nodes SHOULD enforce a configurable per-transaction limit on the number of config change entries (mempool rule).
...
`initial_configuration` is the canonical ABI encoding `abi.encode(bytes32 actorsRoot, address checkpointer)` with no trailing bytes. Its header uses checkpoint `0` and MUST reconstruct `initial_image_hash`. An empty non-admin set MAY use `actorsRoot = bytes32(0)`; no actor can authenticate without a valid witness. `initial_admins` uses the ordinary initial-actor encoding and validation, including nonempty, strictly ascending nonzero actorIds and valid authenticators, but every entry MUST have scope `0`, implicit expiry `0`, and empty policy data. Admins are stored exclusively onchain, outside the image hash.

`initial_admins_commitment` is the `actors_commitment` defined in [Address Derivation](#address-derivation), computed over `initial_admins`. The sender address commits to both the initial non-admin state and its onchain admins:

```text
effective_salt = keccak256(0x01 || user_salt || initial_image_hash || initial_admins_commitment)
deployment_code = DEPLOYMENT_HEADER(len(code)) || code
address = keccak256(0xff || KEYSTORE_ADDRESS || effective_salt || keccak256(deployment_code))[12:]
```

Validation applies the existing create code, address, freshness, and initial-call rules, but authenticates `sender_auth` against `initial_admins` using ordinary onchain admin authentication. On success the Keystore stores the image hash, checkpointer, and checkpoint `0`, sets `CHECKPOINTED`, initializes the change sequences, and installs the initial admins with the ordinary create rules, including `DEFAULT_EOA_REVOKED` unless a native self-admin is explicitly included. No non-admin actor or policy slots are written. `computeCheckpointedAddress` MUST apply the same initial-admin validation and commitment.
...
| `5` | `SettleImageHash` | `abi.encode(bytes32 imageHash, bytes32 actorsRoot, uint56 checkpoint, address checkpointer)` | **Checkpointed accounts only.** Stores a successor image hash authorized by the onchain admin authenticating the batch. The header MUST reconstruct `imageHash`, and its checkpoint MUST be greater than the settled checkpoint unless that image is already settled. Updates the image hash, checkpointer, and settled checkpoint atomically. |

`AuthorizeActor` and `RevokeActor` retain their ordinary semantics in onchain mode. In checkpointed mode, `AuthorizeActor` MUST require scope `0` and empty policy data, and `RevokeActor` affects only the onchain admin record; it cannot revoke a checkpointed leaf. Non-admin revocation requires an approved successor image or leaf expiry. `SettleImageHash` is valid only for checkpointed mode and is the only operation that can rotate its stored checkpointer. It MUST use a sequenced batch, require the account to be unlocked, and authenticate an onchain admin independently of any checkpoint witness; it needs no checkpointer approval. An already-settled image is a no-op, and all other accepted settlements strictly increase the checkpoint.
...
1. **Create entry** (if present): apply the onchain or checkpointed create entry — place `code` at `sender`, initialize the selected Keystore representation and change sequences, and initialize lock state to safe defaults.
...
**Policy gate**: When the transaction's authenticating actor has `POLICY` set (and exclusivity checks passed), each call is gated before dispatch. The protocol resolves the actor's allowed target once at the start of `calls` execution, from `policy_manager(sender, actorId)` in the default mode or the authenticated witness in checkpointed mode. That snapshot gates every call in every phase; a config change applied by an earlier call does not retarget the gate for later calls. If `call.to` is not that address, the call is **not dispatched** and fails deterministically with the protocol revert `ActorPolicyViolation(bytes32 actorId, address target)`:
...
For an onchain actor, wallets MAY continue reading configuration through the Keystore. A checkpointed policy target reads the actor and commitment from `getTransactionSenderActor()`, whose value is fixed during validation and cannot change during execution.
...
|-----------|-------------|-----------------|
| **Keystore contract** | Protocol reads onchain actor state or checkpointed anchors; EVM interface available | Standard contract (ERC-4337 compatible factory) |
...
| **Checkpointer Contracts** | Protocol calls the stored checkpointer for checkpointed accounts | Same contracts callable by the Keystore and wallets |
...
1. Parse and structurally validate `sender_auth`. Verify `account_changes` contains at most one create entry (`0x00` or `0x03`, mutually exclusive and first) and at most one delegation entry (`0x02`). Nodes SHOULD enforce a configurable limit on config change entries (`0x01`).
2. Resolve sender: if `sender` set, use it; if empty, ecrecover from `sender_auth`
3. Determine effective actor state:
   a. If a create entry is present: verify its mode-specific address derivation and freshness, then use its initial actors or initial onchain admins and checkpointed state
   b. Otherwise read the account mode and resolve the actor from Keystore storage or the checkpointed authorization
...
5. Validate `sender_auth` against the resulting actor state (see [Validation](#validation)) and check scope per [Actor Scope](#actor-scope): SENDER (or `POLICY`) for the sender context, and admin or `NONCE` when `nonce_key != NONCE_KEY_MAX`. For checkpointed mode, reject aggregate authentication work above `MAX_AUTHENTICATION_GAS`. Payer scope (SELF_PAYER / SPONSOR_PAYER) is checked in step 6, not here — the sender actor is not required to carry a payer bit. If delegation entries are present, the resolved actor must additionally hold admin EOA-actor delegation authority (see [Delegation Entry](#delegation-entry)).
...
6. Set the authenticated actor configuration on the Transaction Context precompile. The static sender-signed fields (`sender`, `payer`, `calls`, `gas_limit`) are already exposed from validation onward; actor data becomes readable from this point (execution).
...
| `CHECKPOINTED` | `0x08` | Account-state `flags` bit selecting checkpointed Keystore mode |
| `CHECKPOINTED_THRESHOLD` | `1` | Fixed checkpointed configuration threshold |
| `MAX_CHECKPOINT` | `2^56 - 1` | Largest checkpoint in a checkpointed state |
...
Onchain configuration offers simple inspection and immediate onchain revocation. Checkpointed configuration avoids replicating every key or policy change across every chain, but adds proof data, checkpointer trust or proof-system complexity, and data-availability requirements. A permanent per-account choice preserves the original default while allowing wallets with high-frequency multichain configuration to opt into that tradeoff.

Keeping admins onchain makes the checkpointer a co-authorizer of non-admin state rather than its sole authority. An admin can approve a root containing ten website keys with different seven-day expiries, authenticators, and transaction policies in one offchain signature. Each key still authenticates its own actions, and expiry is enforced without updating the root. Adding another key requires approval of the new root; the checkpointer cannot issue additional authority unattended without an available admin signer. Website login also needs domain and message-policy validation, and existing sessions need their own expiry. Root approvals amortize admin signing across a batch, but changing any leaf requires a new root approval; onchain admin changes still need chain synchronization.

### Why Is the Checkpoint Limited to 56 Bits?

Three authorization-flag bits encode a checkpoint length of zero through seven bytes. The checkpointer ABI uses `uint256`, but values above `2^56 - 1` are invalid.
...
- Existing EIP-8130 accounts retain the onchain create format, actor slots, signature encoding, configuration changes, and validation behavior. Checkpointed mode is selected only through its dedicated create or import path
...
interface IKeystore {
    ...
    function createCheckpointedAccount(bytes32 userSalt, bytes calldata bytecode, bytes calldata initialConfiguration, InitialActor[] calldata initialAdmins) external returns (address);
    function computeCheckpointedAddress(bytes32 userSalt, bytes calldata bytecode, bytes32 initialImageHash, InitialActor[] calldata initialAdmins) external view returns (address);
    ...
    function importCheckpointedAccount(address account, uint256 chainId, bytes32 initialImageHash, bytes calldata initialConfiguration, InitialActor[] calldata initialAdmins, bytes calldata signature) external;

    // Single signed entry point for actor changes, checkpoint settlement, epoch, and lock state.
    ...
    function getCheckpointedState(address account) external view returns (bytes32 settledImageHash, address settledCheckpointer, uint56 settledCheckpoint);
...
Read-only. Gas is charged as a base cost plus 3 gas per 32 bytes of returned data. `getTransactionSender`, `getTransactionPayer`, `getTransactionCalls`, and `getTransactionGasLimit` read static sender-signed fields and are available during both authentication and call execution; the actor getters are available during execution only.
...
For checkpointed accounts, checkpointer-state changes, settlement, and changes to an approving onchain admin can also invalidate pending transactions. Nodes MUST revalidate against inclusion state and track mutable state read by the checkpointer and both the admin and acting authenticators. Admin approval does not remove this freshness dependency.
...
**Policy State on Revocation**: In onchain mode, `revokeActor` clears `actor_config`, `policy_commitment`, and `policy_manager`. In checkpointed mode, onchain admin revocation uses the ordinary actor path, while non-admin revocation requires a newer approved and accepted image hash that omits the actor. Any parameters a target keeps in its own storage are not protocol state and are not auto-cleared; wallets uninstall them through the target when retiring a key, and an `expiry` bounds the window during which a not-yet-uninstalled key could otherwise be used.

**Actor Management**: Config change authorization requires the authorizing actor be admin (`scope == 0x00`). Checkpointed accounts install admins explicitly onchain; checkpoint witnesses cannot grant admin or self-actor authority. While locked, onchain actor changes are blocked and checkpointed validation accepts only the settled image hash.
...
**actorId Binding**: The protocol checks that the authenticator's returned `actorId` maps back to the authenticator in `actor_config` or the authenticated checkpoint witness, preventing a malicious authenticator from claiming another authenticator's actors.

**Checkpointer Authority**: The Keystore independently checks both admin approval of the effective image and membership of the acting leaf. A compromised checkpointer cannot invent actors or expand their signed permissions, but can withhold acceptance or re-present a still-approved state above the settled checkpoint. Co-authorization limits grants; it does not prove that a snapshot is the latest. A zero checkpointer explicitly omits this second acceptance condition, leaving admin approval and the settled floor as the authority rules.

**Stale Snapshots**: A checkpointer MUST provide freshness through expiring proofs, a committed latest-state root, a rollup proof, or another mechanism that prevents acceptance of superseded snapshots.

**Checkpointer Self-Selection**: Validation invokes the checkpointer stored with the settled image hash, and every image reconstructed under that authority commits to the same address. Replacement requires an admin-authorized `SettleImageHash`.

**Checkpointer Liveness and Data Availability**: Wallets MUST retain configuration data and admin approvals. An unavailable checkpointer or missing witness can interrupt checkpointed actors. Onchain admins remain usable and can settle a replacement or rotate the checkpointer after the existing unlock lifecycle. There is no automatic fallback around a configured checkpointer.

**Proof Bounds**: Nodes MUST charge every proof byte, topology hash, checkpointer call, admin-approval verification, state read, and acting authenticator execution. Proof work above `MAX_AUTHENTICATION_GAS` is invalid.

**Settlement Rollback**: `SettleImageHash` MUST accept only a successor authorized by an onchain admin and MUST reject a checkpoint below the currently settled checkpoint.
