...
category: Core
created: 2025-10-14
requires: 155, 170, 712, 1153, 1271, 1559, 2028, 2718, 2929, 4337, 6780, 7702, 7708, 7819
---
...
To enable uncoordinated session-key, subscription, and JIT actor additions there is an epoch system. Both change channels are **symmetric replay tracks** that differ only in what the signature binds: **Local** binds `block.chainid`, **Multichain** binds `chain_id 0` (all chains). Each track carries an **epoch** alongside its sequence counter — `local_epoch`/`local_sequence` for Local, `global_epoch`/`global_sequence` for Multichain — and the signed `sequence` word on either channel is `epoch(high 32) || counter(low 32)` (see [Config Change Authorization](#config-change-authorization)). An epoch is a blunt cancellation control for the account's own outstanding signatures *on that track*, distinct from actor expiry:

- `IncrementEpoch` increments the epoch of the channel it rides on (`Local` → `local_epoch`, `Multichain` → `global_epoch`) and resets that track's counter to `0`. Every batch on that track signed at the prior epoch — sequenced or unsequenced/JIT — no longer matches and is rejected with `StaleEpoch`; it can never land.
- The epoch does **not** revoke live actors or touch `actor_config`. An actor authorized in a landed batch stays authorized across an epoch bump; only *unlanded signatures on that track* are invalidated. Removing an actor is still an explicit `RevokeActor`.
- The two tracks are **independent**: Local activity never advances the global counter or epoch, and vice versa. A Multichain one-shot batch ("install this admin on every chain") therefore stays applicable across chains no matter how much Local churn happens, and a Multichain `IncrementEpoch` — one signature, landed on each chain — is the single revocation handle for every outstanding Multichain (all-chains) signature.

This is what makes the unsequenced/JIT mode (`UNSEQUENCED`) safe to hand out on either channel: an unsequenced batch is intentionally replayable within its track's epoch, and the account durably retires it by bumping that epoch — typically batching the reducing `RevokeActor`/`AuthorizeActor` with `IncrementEpoch` in one signed batch. A **Multichain JIT** batch is a reusable, all-chains grant: any relayer may replay it on every chain (including chains that do not yet exist) until a Multichain `IncrementEpoch` lands on that chain. Wallets SHOULD give every JIT grant a non-zero `expiry` so a leaked or forgotten grant is time-bounded rather than living until an epoch bump reaches every chain, and SHOULD prefer Local JIT for chain-specific hot keys. See [Why a Local Epoch?](#why-a-local-epoch).
...
`AuthorizeTransientActor` installs an actor for the **current transaction only**. It carries the same `(actorId, ActorConfig, policyData)` payload as `AuthorizeActor` and is authorized the same way — by the batch's admin signature and the channel's epoch/sequence machinery — but the record is written to an [EIP-1153](./eip-1153.md) transient tier keyed by `(account, actorId)` instead of persistent `actor_config`, and is cleared automatically at transaction end. No `RevokeActor` is needed or emitted.

Resolution is **persistent-first, transient-fallback**, with one occupancy rule shared by the read surface and authentication: the transient tier is consulted only when the persistent home for `actorId` is *unoccupied* (no `actor_config` entry and, for the self-actorId, the inline key is revoked). A persistent entry that is present but **expired** keeps the tier shadowed — it reads empty and authenticates as `ActorExpired`, and a transient twin does not resurrect or re-scope it. A transient actor is otherwise indistinguishable from a durable one for the rest of the transaction: same `actor_config` packing, same expiry gating, same policy-slot semantics (its `policy_manager`/`policy_commitment` live in the same tier). The self-actorId cannot be installed transiently. Installation is frozen while the account is locked.

Combined with an unsequenced (JIT) batch this yields a **reusable install**: one admin signature that any relayer may include ahead of the transaction's own `sender_auth` to materialize a worker key with no persistent state and no counter burned, bounded by the grant's `expiry` and the track's epoch (Local JIT: this chain; Multichain JIT: every chain). Because `AuthorizeTransientActor` accepts any scope, including `0x00`, a transient install with admin scope is a full admin for that transaction and can sign durable changes; wallets SHOULD scope transient installs narrowly and always give them a non-zero `expiry`.
...
Account lock state is stored in a single packed 32-byte account-state slot that also holds both change tracks, an account-flags byte, and the inline default-EOA config. The protocol may read this slot's raw layout directly for mempool rate-limit tiering, so its field order and widths are normative: `global_sequence(4) ‖ global_epoch(4) ‖ local_sequence(4) ‖ local_epoch(4) ‖ flags(1) ‖ lock_union(6) ‖ default_eoa_expiry(6) ‖ default_eoa_scope(2) ‖ reserved(1)`, low-order first (Solidity struct packing; each track's `sequence ‖ epoch` pair occupies the same 8 bytes as, and reads identically to, the signed `epoch(32) || counter(32)` word). The exact layout is defined in the canonical repository (`src/Keystore.sol`).

When `LOCKED`, all authority changes (authorize/revoke actor) and delegation are rejected on both paths (config entries in `account_changes` and `applySignedAccountChanges`). Three changes remain permitted while locked: `Unlock` (which begins the timed release), `IncrementEpoch` on either channel (which cancels that track's outstanding signatures without touching actor authority — see [Epoch System](#epoch-system)), and `AuthorizeActor` **only as a pure add or an expiry-only re-lease** whose granted `expiry` outlives the unlock floor (`now + unlock_delay` while hard-locked, `unlocks_at` while an unlock is pending; `expiry == 0` always qualifies). A locked authorize MUST NOT change a live actor's `authenticator`, `scope`, or policy — only its `expiry` — so nothing a node cached about an existing actor can be invalidated by it; adding an actor is likewise monotone. `AuthorizeTransientActor` is frozen while locked. Lock and unlock are themselves the only lock-state transitions, carried as standalone changes (below). The stored `unlock_delay` is bounded to the `uint16` range (~18.2h max): lock exists for mempool permissioning, and a short ceiling prevents an account from self-bricking config rotations.
...
rlp([
  ...
  sequence,           // uint64: epoch(high 32) || counter(low 32), against the channel's own track:
                      //         Local = local_epoch || local_sequence, Multichain = global_epoch || global_sequence
  changes,            // Array of account changes
  auth                // Admin (scope == 0x00) signature over the batch digest
])

change = rlp([
  change_type,        // uint8: 0 AuthorizeActor, 1 RevokeActor, 2 IncrementEpoch, 3 Lock, 4 Unlock,
                      //        5 AuthorizeTransientActor
...
| `2` | `IncrementEpoch` | empty | **Either channel.** Increments the epoch of the channel it rides on (`Local` → `local_epoch`, `Multichain` → `global_epoch`) and resets that track's counter to `0`, invalidating every unlanded signature on that track (sequenced and unsequenced) at the prior epoch. Consumes no sequence itself. Permitted while locked. See [Epoch System](#epoch-system). |
...
| `5` | `AuthorizeTransientActor` | `abi.encode(bytes32 actorId, ActorConfig config, bytes policyData)` | **Either channel.** Same payload and validation as `AuthorizeActor`, but the actor is written to a **transient tier** ([EIP-1153](./eip-1153.md) transient storage keyed by `(account, actorId)`) for the current transaction only; it is never persisted and is cleared at transaction end. Rejects the self-actorId. Frozen while locked. On the unsequenced/JIT path an already-lapsed grant is silently skipped, as for `AuthorizeActor`. Emits `TransientActorInstalled` (same `actorData` packing as `ActorAuthorized`). See [Transient Actors](#transient-actors). |
...
The `channel` selects the replay domain; `sequence` is interpreted **identically** on both channels as `epoch(high 32) || counter(low 32)` against the channel's own track:

- **Multichain** (`chain_id 0`): the track is `global_epoch || global_sequence`. Valid on any chain, for synchronizing actor state across chains.
- **Local** (`block.chainid`): the track is `local_epoch || local_sequence`.

On either channel the batch is rejected with `StaleEpoch` unless its high half equals the track's current epoch. The low half selects one of two modes:

- **Sequenced** (low half `< UNSEQUENCED`): the low half MUST equal the track's current counter; on success the counter increments. This is the ordered, replay-once mode.
- **Unsequenced / JIT** (low half `== UNSEQUENCED`, i.e. `uint32` max): consumes no counter. It is permissive — any change may ride an unsequenced batch (Lock/Unlock must still be standalone) — and remains replayable until the track's epoch moves. Because it consumes no counter, ordering between two unsequenced batches is undefined. Retiring an unsequenced (JIT) authorization durably is done by bumping that track's epoch (`IncrementEpoch` on the same channel), typically batched with the reducing change. See [Epoch System](#epoch-system).

The two tracks are independent (see [Epoch System](#epoch-system)). A Multichain `IncrementEpoch` retires outstanding Multichain signatures on the chains where it lands; a chain that has not yet received it still accepts the old-epoch Multichain batches, and Multichain **sequenced** batches signed at the old epoch can never land on a chain that has moved to the new epoch — the wallet MUST re-issue them at the new epoch. Wallets that need a change on many chains SHOULD therefore prefer Multichain JIT (idempotent, order-free) over Multichain sequenced.

**Expiry on `AuthorizeActor` is channel/mode-aware and never reverts.** On the **unsequenced/JIT path**, an already-lapsed grant (a non-zero `expiry` that has passed per [Actor Expiry](#actor-expiry)) is **silently skipped**: the change is not applied and does not revert, and any live sibling changes in the same batch still apply. A JIT grant is replayable, so skipping a lapsed one keeps it from ever clobbering its slot — a renewed grant cannot be overwritten by replaying the old, expired one. A **single-consume** (Sequenced) batch on either channel cannot be replayed, so an already-lapsed grant is instead installed **inert** — the `actor_config` slot is written (the actor is present and revocable) but the actor is not live, since authentication yields expired per [Actor Expiry](#actor-expiry) — and still consumes its sequence. Cross-chain sync relies on this: a chain catching up must replay a historical expiring grant's slot (e.g. a periodically-renewed operator) in order to reach the current live grant, so dropping the stale slot would strand its counter. A zero `expiry` is the "no expiry" sentinel and is always accepted.

The four track fields together double as the initialized flag: creation and import set `local_sequence = 1`, and any non-zero epoch or counter on either track marks the account initialized; all four zero means uninitialized (only the implicit EOA is on the account). A first unsequenced (JIT) batch on a fresh account sets `local_sequence = 1` as well (so a later create/import is rejected) without advancing the global track.
...
Session keys and just-in-time (JIT) authorizations want two things that pull against a shared counter: they should be usable in any order (no coordination), and the account should be able to cancel any it has handed out that have not yet landed. A single monotonic counter gives ordering but forces every signature to burn the next slot, so parallel keys contend and a stale signature can still land later. Splitting each track's word into `epoch || counter` separates the two concerns: sequenced changes still consume the counter for ordered, replay-once semantics, while an epoch bump is a single blunt "cancel everything outstanding on this track" that resets the counter and invalidates every not-yet-landed signature on it at the prior epoch (`StaleEpoch`). The Multichain track carries the same machinery so that a reusable all-chains grant exists and has a revocation handle; the two tracks stay independent so this-chain activity can never desynchronize the global counter across chains. Crucially it cancels *signatures*, not *authority* — actors from landed batches survive — so bumping the epoch is cheap to reason about and cannot brick a live actor set. That is what makes the unsequenced/JIT mode (`UNSEQUENCED`) safe: a JIT batch is intentionally replayable within its epoch, and durable retirement is just an epoch bump batched with the reducing change (see [Epoch System](#epoch-system)).
...
```solidity
interface IKeystore {
    // Packed account state (see Account Lock). Each track's signed word is epoch(high 32) || sequence(low 32).
    struct ChangeSequences {
        uint32 globalEpoch;    // Multichain (chain_id 0) track epoch; a Multichain IncrementEpoch bumps it
        uint32 globalSequence; // Multichain track counter; reset to 0 by a Multichain IncrementEpoch
        uint32 localEpoch;     // Local (block.chainid) track epoch; a Local IncrementEpoch bumps it
        uint32 localSequence;  // Local track counter; reset to 0 by a Local IncrementEpoch
    }
    ...
    enum ChangeType { AuthorizeActor, RevokeActor, IncrementEpoch, Lock, Unlock, AuthorizeTransientActor }
    ...
    struct AccountChange {
        ...
        bytes payload;        // AuthorizeActor: abi.encode(bytes32 actorId, ActorConfig, bytes policyData);
                              // AuthorizeTransientActor: same as AuthorizeActor; RevokeActor: abi.encode(bytes32 actorId);
                              // Lock: abi.encode(uint16 unlockDelay); IncrementEpoch / Unlock: empty
    }

    struct SignedAccountChanges {
        AccountChangeChannel channel;
        uint64 sequence;      // epoch || counter against the channel's track (Local or Multichain/global)
        AccountChange[] changes;
        bytes signature;      // admin (scope == 0x00): authenticator || data
    }

    uint32 constant UNSEQUENCED = type(uint32).max; // JIT sentinel for the low half, either channel
    ...
    // Transient (per-transaction) actor materialized; never persisted, no matching revoke event.
...
**Epochs and Outstanding Signatures**: `IncrementEpoch` cancels every unlanded signature on **its own channel's track** at the prior epoch (`StaleEpoch`), including unsequenced/JIT batches; it does **not** revoke live actors and does not touch the other track (see [Epoch System](#epoch-system)). Because an unsequenced (JIT) batch is replayable within its epoch, wallets MUST treat epoch increment — not mere non-inclusion — as the durable retirement of a JIT authorization, batching the reducing `RevokeActor`/`AuthorizeActor` with `IncrementEpoch` on the same channel. A **Multichain** JIT grant is valid on every chain — including chains that do not yet exist — until a Multichain `IncrementEpoch` lands *on that chain*; durable revocation without a per-chain write is not possible, so wallets SHOULD give every JIT grant a non-zero `expiry` and reserve the epoch bump for compromise. Since the epoch does not walk authorizer lineage, an actor added by a landed batch survives later epoch bumps and must be removed with an explicit `RevokeActor`.
