...
A transaction whose `gas_limit` is below the floor branch is invalid at [Mempool Acceptance](#mempool-acceptance). The `payer_auth` bytes are floored the same way within `payer_auth_cost` (charged to the payer, outside `gas_limit`), closing the equivalent vector on that field.

The floor is normative **structure** under both [adoption profiles](#adoption-profiles), not a repriceable constant: an L2-profile chain MAY change the `10` per-token rate but MUST NOT set it below `4` or drop the floor.
