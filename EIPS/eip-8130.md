...
The byte layout of `metadata` is not constrained by this specification; producers MAY use any encoding. For interoperable indexing, producers SHOULD encode `metadata` using [ERC-8021](./eip-8021.md) (transaction attribution), which defines a marker-less schema 2 wire format for this field. Use of ERC-8021 is RECOMMENDED but not REQUIRED; producers that do not need interoperable indexing MAY treat `metadata` as wholly opaque bytes.

As a convention, a `metadata` field that is exactly 32 bytes and is not a valid [ERC-8021](./eip-8021.md) payload SHOULD be interpreted as a commitment (a digest) to data stored off-chain, rather than as an inline structured payload.
