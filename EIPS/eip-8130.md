...
| **Ephemeral Accounts** | Created via a create-ephemeral entry in `account_changes`; address is `keccak256("EPHEMERAL" \|\| salt \|\| actor \|\| delegation)`, code is a transaction-scoped [EIP-7702](./eip-7702.md) delegation (cleared at settlement), no Keystore write. The transaction MUST be nonce-free. Recover leftover funds by submitting another create-ephemeral with the same parameters | Same `user_salt` + `initial_actor` + `delegation` recreates the address |
...
A [create-ephemeral](#create-ephemeral-entry) entry additionally **requires** `nonce_key == NONCE_KEY_MAX` (the transaction is invalid otherwise) and **forbids** the `NONCE` bit on its `initial_actor`. Admin (`scope == 0x00`) and every other grant remain expressible; sequenced nonce use is not.
...
5. Compute `actor_commitment` and `expected` per [Address Derivation (Ephemeral)](#address-derivation-ephemeral).
6. Require `sender == expected` (`sender` MUST be set; this is the configured-actor path).
7. Validate `sender_auth` against `initial_actor` (resolved `actorId` and authenticator MUST match). Scope and `policyData` for the rest of the transaction are taken from that actor, not from Keystore storage: there is no `actor_config` / `policy_manager` write, so the [policy gate](#call-phases) MUST read `manager` from the in-entry `policyData` when `POLICY` is set.
...
3. Determine effective actor state:
   ...
   b. If create-ephemeral entry present: verify ephemeral address derivation, `NONCE`-bit ban, and `nonce_key == NONCE_KEY_MAX`; use `initial_actor` (no Keystore read)
...
Privacy and one-shot flows need an account that can originate `calls` (so it needs delegated wallet code for the duration of the transaction) without leaving Keystore state, persistent code, or a nonce slot. Create writes all three. Create-ephemeral is create's actor tuple as a single `initial_actor` with those writes removed: a domain-separated `keccak256("EPHEMERAL" || …)` address, a 7702 indicator that is cleared at settlement, and a mandatory `NONCE_KEY_MAX` so the only protocol footprint is the existing nonce-free ring buffer.

The `NONCE` grant is banned on `initial_actor` so the entry cannot express sequenced-nonce authority; admin and every other grant remain available. Recovery of leftover ETH or tokens is another create-ephemeral with the same `user_salt`, `initial_actor`, and `delegation` — the address is stable and the protocol does not consume CREATE2 freshness. Application-level nullifiers (privacy pools) live in `calls`, not in this entry.
