...
We do expect to extend the protocol to a single external factory once [EIP-7819](./eip-7819.md) is adopted. That would extend the create entry to emit an [EIP-7702](./eip-7702.md) delegation prefix and deploy the account from the factory, which uses Account Configuration Contract state to check the signature and upgrade. This would then enable any initial actor set to use delegation. 
