...
The sender verifier runs first, and its metered cost is included in `gas_limit`. This lets the payer sign a single maximum gas exposure without committing to the sender's verifier choice. Payer authentication is chosen by the payer and is metered as part of transaction validation.
