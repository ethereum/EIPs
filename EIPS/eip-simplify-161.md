...
EIP 161 is overly complex and has a number of edgecases that are poorly documented and tested. This EIP takes advantage of the complete removal of all remaining empty accounts in block 14,049,881 to clarify it and allows implementors to not implement various edgecases that never occured and are not possible in future.
...
Retroactively replace EIP 161 with the following rules:
...
If the transaction, `CALL` or `SELFDESTRUCT` is reverted, then the state clearing is also reverted. As a special case, in block 2675119 (tx `0xcf416c536ec1a19ed1fb89e4ec7ffb3cf73aa413b3aa9b77d60e4fd81a4296ba`), an empty call is made to an empty account at `RIPEMD160` (precompile `0x3`), the call fails out of gas, but the empty account is removed anyway.
...
After the start of Byzantium, the only state clearing events are the two listed below. Clients MAY implement post Byzantium state clearing by hardcoding those events.
