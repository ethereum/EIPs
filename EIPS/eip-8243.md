...
Batching also expands the design space for consensus throughput. Because each network byte now carries more attestation information, if we hold traffic constant, we can consolidate subnets (and committees) by the same efficiency factor (e.g. 64 → 16 at 4x efficiency), and in turn shorten the epoch by the same factor (e.g. 32 -> 8 slots). Faster L1 finality follows as a side effect. That said, the right factor is hard to predict ahead of mainnet observations, so we leave it out of this spec.
