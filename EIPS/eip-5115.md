---
eip: 5115
title: Standardized Yield Token
...
- Yield-generating assets that have different input tokens used for minting vs accounting for the pool value.
...
Therefore, we are proposing Standardized Yield (SY), a flexible standard for wrapped yield generating tokens that could cover most mechanisms in DeFi. We foresee that:

- EIP-4626 will still be a popular vault standard, that most vaults should adopt.
- SY tokens can wrap over most yield generating mechanisms in DeFi, including EIP-4626 vaults for projects built on top of interest-bearing tokens.
- Whoever needs the functionalities of SY could integrate with the existing SY tokens or write a new SY (to wrap over the target interest-bearing token).
...
Standardized Yield is a token standard for any yield generating mechanism that conforms to the GYGP model. Each Standardized Yield token represents **shares** in a GYGP and allows for interacting with the GYGP via a standard interface.
...
This function will convert the *amountTokenToDeposit* of input token $i$ into $d_a$ worth of **asset** and deposit this amount into the pool for the *receiver*, who will receive *amountSharesOut* of SY tokens (**shares**).
...
This function will deposit *amountSharesToRedeem* into the SY contract and use the floating amount $d_s$ of SY tokens (**shares**) in the SY contract to redeem to output token $i$ (*tokenOut*). This pattern is similar to UniswapV2 which allows for more gas efficient ways to interact with the contract.
...
- **MUST** return $(rewardIndexes_{current} - rewardIndexes_{user}) \times d_s$
- **MUST NOT** revert.
...
This read-only method returns the underlying yield-generating token (representing a GYGP) address.

- **MUST** return a token address that conforms to the EIP-20 interface, or zero address
- **MUST NOT** revert.
- **MUST** reflect the exact underlying yield-generating token address if the SY token is a wrapped token.
...
- **MUST** be emitted when rewards are claimed from the SY contract via `claimRewards` method.
...
`yieldToken` must strongly reflect the address of the underlying wrapped yield-generating token. For a native implementation wherein the SY token does not wrap a yield-generating token, but natively represents a GYGP share, then the address returned MAY be a zero address. Otherwise, for wrapped tokens, you may introduce confusion on what the SY token represents, or may be deemed malicious.
