...
In its specific implementation, in order to include the role of `BALANCE`, let the codehash of the address without balance be `0x`, and the codehash of the address with balance be `keccak256("")`.

The contract address can be calculated in advance. Whether it is  `CREATE` or `CREATE2`, it is possible that the contract is not created but has a balance. For security, you can actually only use  `keccak256(add.code) == keccak256("")` instead of `add.codehash == 0`, which makes the original intention of EIP-1052 meaningless.
