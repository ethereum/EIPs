...
| maxFeePerGas      | uint256 | max amount of basefee that the `operation` execution is expected to pay, (similar to [EIP-1559](./eip-1559.md) `max_fee_per_gas`)                        |
| priorityFeePerGas | uint256 | fixed amount of fees that the `operation` execution is expected to pay to the bundler (similar to [EIP-1559](./eip-1559.md) `max_priority_fee_per_gas`). |
...
If the call succeeds and returns `ready == true`, then the operation can be kept in the mempool and used when constructing a block.
