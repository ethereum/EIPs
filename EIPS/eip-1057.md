...
We do not recommend implementing this fix at this time. Ethash will not be exploitable for years, and it's not clear ProgPoW will ever be exploitable.

After the completion of the audits a clever finding by [Kik](https://github.com/kik/) disclosed a vulnerablity to [bypassing ProgPoW memory hardness](https://github.com/kik/progpow-exploit).  The vulnerablity is present in Ethash as well but is near-impossible to exploit.  In progPoW it is not practically implementable.  An attacker would need modified block headers, would need customized nodes able to accept modified block headers by the miner, the exploit uses extraNonce/extraData as entropy, which isn’t the standard today, and the required brute-force search would be difficult to accomplish in one blocktime.  And if the exploit was implemented other miners would simply reject the modified blocks.

 To more completely prevent this exploit we could change the condition modifying the input state of the last keccak pass from
* header (256 bits) +
...
Again, we do not recommend implementing this fix at this time.  Kik's vulnerability and others like it cannot be exploited now and likely never will be.
...
### _Notes on Deployment_

>We **Do Not** recommend that this Proposal be deployed at this time.  Rather, we leave open the possibility and threat of future deployment.

>Note that [DAG-size growth](https://investoon.com/tools/dag_size) will defeat the [Antminer E3](https://blog.bitmain.com/en/bitmains-antminer-e3-firmware-update/) (and many Innosilicon ASICs) circa November or December 2020, which will mark one good window to introduce progPoW with minimal disruption to the network and industry.

>Note also that before the cutover from PoW to PoS it might be advantageous to minimize the domination of the network by ASICs.
...
After the completion of the audits a clever finding by Kik disclosed a possibly exploitable condition to bypass ProgPoW memory hardness. The condition is present in Ethash but near-impossible to exploit, and requires the availability of a customized node able to accept modified block headers by the miner. To prevent this exploit a future release could change the condition modifying the input state of the last keccak pass from
```
    header (256 bits) +
    seed for mix initiator (64 bits) +
    mix from main loop (256 bits)
...
to
    ...
    digest from initial keccak (256 bits) +
    mix from main loop (256 bits) +
    padding thus widening the constraint to target in keccak brute force keccak linear searches from 64 to 256 bits.

```
This fix is available as a PR to the reference implementation.

We do not recommend making this change, as this exploit cannot succeedd due to the structure of the block headers.  A similar exploit -- if there is one -- would require custom ASICS and the attacker's own pool and miners.  And even so other miners would reject the attacker's blocks.  Further, the attack only defeats memory hardness and does not allow for denial of service, double spends, or other attacks to the network.
