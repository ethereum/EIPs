...
discussions-to: https://ethereum-magicians.org/t/implementing-account-abstraction-as-part-of-eth1-x/4020
status: Draft
...
Transaction validity is defined rigidly by the protocol, forcing a particular set of validity rules based on ECDSA signatures and nonces onto the protocol. This precludes innovation in a number of important areas, particularly including:
...
In most of the above cases, it is still possible to do these applications using intermediaries, most notably the [Gas Station Network](https://www.opengsn.org/) and application-specific alternatives, but these alternatives are (i) technically inefficient, due to the extra 21000 gas to pay for the relayer, (ii) economically inefficient, as relayers need to make a profit on top of the gas fees that they pay. Additionally, use of intermediary protocols means that these applications cannot simply rely on base ethereum infrastructure and need to rely on extra protocols that have smaller userbases and higher risk of no longer being available at some future date.

Account abstraction allows for the validity conditions of a transaction to include execution of arbitrary EVM code (with some limits regarding what state can be accessed), allowing the above use cases.

We split account abstraction into two tiers: **single-tenant AA**, which is intended to support abstracted accounts that represent wallets or otherwise are only meant to be accessed by one or a few participants, and **multi-tenant AA**, which is intended to allow abstracted accounts to also directly be applications (eg. tornado.cash, Uniswap). Out of the four use cases above, it can be approximately said that single-tenant AA supports (1) and (2), and multi-tenant AA supports (3) and (4). We get into the differences between the two tiers in the specification and rationale section below.
...
#### Step 1: Transactions with Fixed Nonces
...
#### Extensions (can be done simultaneously with step 1 or after)

If the [indestructible contracts EIP](https://hackmd.io/@HWeNw8hNRimMm2m2GH56Cw/SyNT3Cdmw) is added, this version of AA can be adapted to allow for `DELEGATECALL` during transaction verification: During execution of a new AA transaction, external state access that reads code (`EXTCODESIZE`, `EXTCODEHASH`, `EXTCODECOPY`, `CALLCODE`, `DELEGATECALL`) to any contract whose first byte is the `SET_INDESTRUCTIBLE` opcode is no longer banned. However, calls other than `CALLCODE` and `DELEGATECALL` (ie. calls that change the `callee`) to anything but the `target` or a precompile are still not permitted.

The list of allowed prefixes can over time be extended beyond the `AA_PREFIX` listed above, to enable other benign use cases (eg. logging).
...
#### Later stages: multiple transactions, full multi-tenant AA support
...
Here is a sketch of a strategy for mitigating this problem. We would require incoming transactions to contain an [EIP-2930](https://github.com/ethereum/EIPs/blob/master/EIPS/eip-2930.md)-style access list detailing the storage slots that the transaction reads or modifies, and make it binding; that is, acccesses outside the access list would be invalid. A transaction would only be included in the mempool if its access list is disjoint from the access lists of other transactions in the mempool (or if its gasprice is higher). An alternative way to think about this is to have per-storage-slot mempools instead of just per-account mempools, except a transaction could be part of multiple per-storage-slot mempools (if desired it could be capped to eg. 5 storage slots).
...
### Dependency on 1344 (CHAINID exposed to the EVM)

This is done to allow signature checking done in validation code to implement replay protection.
...
Note that fully implementing this functonality requires AA to be structured in a way that supports multiple users sending withdrawals at the same time (requiring nonces would make this difficult), and that allows a single account to support both AA transactions (the withdrawals) and externally-initiated calls (the deposits).
...
Much of the work on this EIP has been focused on addressing the security concerns that arise in the mempool. Although the miner strategies laid out here are not required in a hard fork, they are important for maintaining the network's resilience.
