...
description: Adds an indexed mask encoding for heterogeneous blob cell requests.
...
This EIP introduces limits of 64 transaction hashes and 64 masks for indexed messages. These limits apply only to the indexed forms. The shared forms retain their existing `eth/72` semantics as defined by [EIP-8070](./eip-8070.md). A mask identifier remains one byte, allowing a future protocol version to raise the mask-table limit to at most 256 without changing the identifier encoding.
...
An indexed response containing no transaction data MUST be encoded as `[request_id: P, [], [], [], mask_ids: B]`, where `mask_ids` is the empty byte string. This empty response is exempt from mask-table requirements 1 and 3. Its empty `mask_ids` satisfies requirement 2, and the remaining requirements apply vacuously. An indexed request MUST NOT be empty.
