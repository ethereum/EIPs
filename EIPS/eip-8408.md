...
This EIP introduces limits of 64 transaction hashes and 64 masks for indexed messages. These limits apply only to the indexed forms. The shared forms retain their existing `eth/72` semantics. A mask identifier remains one byte, allowing a future protocol version to raise the mask-table limit to at most 256 without changing the identifier encoding.
