...
There are cases where the `ModExp` precompile is underpriced for it's resource consumption. By modifying the `ModExp` pricing formula these scenarios would be covered. The target is to make `ModExp` at least as fast as EcRecover precompile in all cases.
...
### 1. Increase minimal price from 200 to 500 and triple general price
