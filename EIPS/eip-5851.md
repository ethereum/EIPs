...
- Verifiable credentials (VC): A Verifiable Credential is a collection of claims made by an issuer. These are temper evident credentials that allow the holders to prove that they posses certain characteristics (for example, passport verification, constraints like value of tokens in your wallet, etc) as demanded by the verifier entity.

- Claims: Decentralised Identity(DID) Claims are a set of assertions that the DID Holder must fulfill to be verified.
...
- Issuer: An entity creates a verifiable credential from claims about one or more subjects to a holder. Example issuers include governments, corporations, non-profit organizations, trade associations, and individuals.
...
Claims MUST be stored in the following structures:

#### 1. Metadata information

Each claim requirement MUST be exposed using the following struct:
...
This structure will be used to define the actual claim information, based on the description of the `Metadata` structure, the structure is similar to `Values` structure of [EIP-3475](./eip-3475.md).
...
Defines the condition encoded for the identity index 1 (i.e the holder must be equal or more than 18 years old).
...
```solidity
    ...
    /// @dev it MUST be Defining the logic to fetch the result of the ownership verification of NFT(with metadata generated using ZK schemes).
