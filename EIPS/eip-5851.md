...
- ZK-ID NFT: NFT representing the self-sovreign federated identity for providing ZK based  Decentralised identity from trusted public certificates (e.g national identity cards, password and eSIM).

- W3C DID specification: Its the specification defined by world-wide-web org in order to assign the identifier and the verification condition without determining the person/ organisation Private identification inforamtion.

### Diagram

![diagram](../assets/eip-5851/architecture-diagram.png)

The following is an workflow using preimage verification:

* The KYC contract is an oracle that assigns user identities with SBT certificates.
* During the issuance stage, the process to generate the off-chain computation of the Merkle root from the various primary details is calculated and then assigned chain to the given wallet address with the identity.
* The verifier is a shared node of the nodes and the Merkle root, in order to verify the Merkle leaf information.
...
* Once the certificate is obtained, the functions that assign the SBTID to the given user will be enabled.
...
For the standard interface functions, we followed the implementation of the  [W3C specification for DID standard](https://www.w3.org/TR/did-core/), which defines the components of data registry, DID identifier. thus we represented the same properties in our standard using the Verifier and SBT standard. Also in order to define the metadatas (URL, topics, subjects and the identifier), we followed the metadata definition as [eip-3475](./eip-3475.md) with possiblity to define context metadata and verification condition within the single contract. 
