---
eip: 5851
title: Onchain verifiable claims.
...
This EIP defines the interface for contracts that verify on-chain attestations of credentials by the holder of the wallet as a soulbound token. It considers the use of ZK schemes  under the hood in order to certify the given wallet owner with the claims abstractly defined on-chain. 
...
- W3C DID specification: Its the specification defined by world-wide-web org in order to assign the  decentralized identifier (DID) and the verification condition without to any entity which is owner to the given wallet (eg: person/ organization Private identification information). its defined as a JSON document with the DID string, schema details, etc.
...
Currently W3C specifications is the way to create the verifiable credentials by define the format of DID components (like URI, document details) along with claims that given identity posses to be able to use a given service(ie VC). they are gaining traction in use-cases like supply chains, digital government services on web2 platforms etc. But they have the following challenges in terms of their adoption for web3 applications:

- significant overhead: currently integrating the information needed to represent the W3C VC credentials onchain are costly due to the data needed ot store the details onchain. for instance if the entity needs to just get the validation that user has attained the certain age, it will not have to sift through the other claims and their proofs provided by the current verifiable credentials.

- technical constraints: In order to integrate these standards based on the various categories of ZKProofs, the third parties in the process (either issuer, holder and verifier) have to collaborate regarding the functional interface in order to implement the solution, making it significantly long process to integrate the standard.
 

Thus we want to propose a standard for the identity issuers and verifier entities to create their smart contract in order to provide on chain commitment of the off-chain verification process, and once the given address is associated with the given attestation of the identity verification off-chain, the issuers can then onboard other verifiers (ie web3 based services like DeFI protocol, governance platform) to define the condition of the ownership of the user.
...
1. Issuer: Entity that attests the verification onchain by issuing SBT onchain to the wallet holder, it will also store the cryptographic proof string onchain (for eg merkle root).

2. Holder: Corresponds to the wallet that is issued the  SBT. once their wallet is approved to have attestation with the wallet, they can 


3. Verifier: This can be any entity that wants to validate the user which has ownership of the given SBT. for that it will co-ordinate with the issuer to know regarding the 
