...
```solidity
 string jurisdiction = IERC5851.stringValue("CH");
```

2. Requirement metadata: 

The below example defines the condition that the holder of this SBT is certified to be more more than 18 yrs of age.
...
```suggestion
For the standard interface functions, we followed the implementation of the W3C DID specification, which defines the components of the data registry and DID identifier. We represented the same properties in our standard using the Verifier and SBT standards.

To define the metadata (URL, topics, subjects, and the identifier), we followed the metadata definition in a manner similar to the one used in [EIP-3475](./eip-3475.md) with the possibility of defining context metadata and verification conditions within single contracts.
```

## Backwards Compatibility

```suggestion
- This EIP is backward compliant for previous versions for cases that only do the changes in the schema of the requirements structure.
```
...
```suggestion
There is also an [example script](../assets/eip-5851/script/createProof.js) that allows the creation of proofs off-chain which can then be initialized by an admin contract in order to validate the `isVerified()` function for a given contract.
```
