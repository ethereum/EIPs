...
|-----------|-------------|
| `test_simple_transfer_emits_log` | Verifies basic EOA-to-account transfer emits a LOG3 event |
| `test_zero_value_transfer_no_log` | Confirms zero-value transfers do NOT emit logs |
| `test_call_with_value_emits_log` | Verifies CALL opcode with value emits transfer log |
| `test_selfdestruct_with_value_emits_log` | Verifies SELFDESTRUCT emits transfer log to beneficiary |
| `test_selfdestruct_to_self_emits_finalization_log` | Tests selfdestruct-to-self emits finalization log for remaining ETH |
| `test_zero_value_operations_no_log` | Confirms zero-value CALL/SELFDESTRUCT don't emit logs |
| `test_failed_inner_operation_no_log` | Verifies failed/reverted inner operations don't emit logs |
| `test_nested_calls_log_order` | Tests nested CALLs emit logs in chronological order |
| `test_reverted_transaction_no_log` | Confirms failed transactions emit no logs |
| `test_transfer_to_special_address` | Tests transfers to precompiles/system addresses emit logs |
| `test_transfer_with_all_tx_types` | Verifies all transaction types emit transfer logs |
