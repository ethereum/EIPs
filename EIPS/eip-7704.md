...
discussions-to: https://ethereum-magicians.org/t/create-a-separate-basefee-and-gaslimit-for-calldata/19998
status: Draft
...
The EIP also introduces a new transaction type which includes the three types of max-basefees and priority fees as a vector, allowing the same code paths to handle all three types of gas. We also make the basefee adjustment for all three types of gas work using the same approach as in EIP-4844, to remove the complexity of having two separate adjustment algorithms, and ensure that the stronger mathematical properties of the newer basefee adjustment algorithm cover all three types of gas.
