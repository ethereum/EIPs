...
Furthermore, many [ERC-721](./eip-721.md) contracts include time-based metadata changes or changes based on off-chain conditions.  In such cases, a scheduled refresh mechanism would allow NFT smart contracts to dictate a configurable polling interval without the need to emit future `MetadataUpdate` events unnecessarily. 
...
interface IERC4906 is IERC165{
    ...
    /// Alternately, this event may be emitted to request that third-party
    /// platforms automatically refresh the metadata on a specified schedule.
    event MetadataUpdate(uint256 _fromTokenId, uint256 _toTokenId, uint256 _refreshIntervalSeconds);
}
```

The `MetadataUpdate` event MUST be emitted when the JSON metadata of a token, or a consecutive range of tokens, is changed.
...
The `MetadataUpdate` event MAY be emitted to schedule or to cancel an automatic refresh interval.  To specify an interval at which the JSON metadata should be refreshed, specify a non-zero value for _refreshIntervalSeconds.  To cancel previously scheduled metadata refreshes, specify zero for _refreshIntervalSeconds. 
...
After capturing the `MetadataUpdate` event, a third party can update the metadata with information returned from the `tokenURI(uint256 _tokenId)` of ERC-721.  When a range of token ids is specified, the thrid party can query each token URI individually.  

It is RECOMMENDED for the third party to capture the refresh interval at the same time, and schedule or unschedule subsequent polling of the token URI for each specified token id.
