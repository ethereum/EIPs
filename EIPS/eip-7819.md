...
The delegation indicators created by the `SETDELEGATE` opcode are the same objects as the one created using EIP-7702. Therefore, all the behavior specificities documented in EIP-7702 (`CODESIZE`, `CODECOPY`, targeting precompile, or chaining delegation indicator) also apply here. Any future EIP that modify the behavior of EIP-7702 delegation indicators should also apply to the delegation indicators created using `SETDELEGATE`. They are not contracts, and cannot be deleted using `SELFDESTRUCT`, even if the `SELFDESTRUCT` happens in the same transaction that created the account at `location`. Destruction of the delegations SHOULD only be possible by the same caller (that created them) doing a new `SETDELEGATE` call with the same salt and a zero target.

Step 9 ensures that an account that was created using `SETDELEGATE` never returns to an empty state (forbidden by [EIP-7523](./eip-7523.md)). Even if the balance is zero, no state was ever set, and the code is removed by a `SETDELEGATE` operation (with zero target), then the nonce being (at least) one would guarantee the account is not empty.
