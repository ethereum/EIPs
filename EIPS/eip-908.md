...
Further discussion is at [this link](https://ethresear.ch/t/incentives-for-running-full-ethereum-nodes/1239).
...
1. **Basic Client Reward Test**:
   - Verify that a client receives 0.15 ETH when a block containing a transaction with the PREV_BLOCK_VERIF opcode is processed
   - Confirm that the client address in the access list receives the reward

2. **Block Age Validation Test**:
   - Verify that transactions with blockhashes older than 400 blocks are rejected
   - Verify that transactions with blockhashes within the 400 block window are accepted

3. **Access List Tests**:
   - Verify that only addresses in the access list can receive rewards
   - Test that transactions revert if the client address doesn't match an address in the access list

4. **Anti-Exploitation Tests**:
   - Test that miners cannot include their own client address in multiple transactions
   - Verify that the slashing condition works when a miner includes multiple transactions with blockhashes they've previously proposed
   - Test that the state correctly removes blockhashes older than the defined window

5. **Reward Calculation Tests**:
   - Verify that the client reward calculations work correctly with different ETH price assumptions
   - Test the distribution of transaction fees between miners and clients

These test cases should cover the core functionality and security aspects of the proposal.
...
1. **Protocol Changes**:
   - Modify the block structure to include the new `PrevBlockVerifications` field
   - Implement the `ClientReward` field in the block structure
   - Add the `PREV_BLOCK_VERIF` opcode to the EVM

2. **Access List Management**:
   - Implement a secure and decentralized access list for client addresses
   - Create mechanisms for adding and removing addresses from the access list
   - Implement verification that client addresses in transactions match addresses in the access list

3. **Block Validation**:
   - Add validation for blockhash age (within 400 blocks)
   - Implement the slashing mechanism for miners who include their own client in multiple transactions

4. **Reward Distribution**:
   - Implement the 0.15 ETH reward distribution to client addresses
   - Modify transaction fee distribution to allocate a portion to clients

5. **State Management**:
   - Implement efficient state tracking of previously validated blockhashes
   - Create a mechanism to prune expired blockhashes from state

Implementation should be developed and tested across multiple client implementations to ensure compatibility and security.
