...
A standardized URI for transaction hash with complete information about transaction.
...
if `method` or `events` are not present, this could either mean that it was a "simple" ETH transfer, or that the URI is not specifying any details. 
