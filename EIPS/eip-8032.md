...
1. **Select account**  
   If `cursor_account_hash == 0x00…00`, set it to the smallest account hash **strictly greater than** `0x00…00` that is present in the state trie. Otherwise, continue with `cursor_account_hash`.
...
3. **Finalize account (if exhausted)**  
   ...
   - Else: 
        ...
        `cursor_account_hash = next account hash`  
...
5. **Completion**  
   The transition is complete when there is no account present in the state trie whose account hash is lexicographically greater than the current `cursor_account_hash`.
