...
- slot `0x01`: `cursor_slot_hash` (32-byte slot hash last processed, or 0x00 if none)
...
4. **Persist**  
   If `TRANSITION_SLOTS_PER_BLOCK` is reached, write `cursor_account_hash`, `cursor_slot_hash`, and `cursor_accum` to the registry in the post-state of the block.
...
Transition reference: [EIP7612](https://eips.ethereum.org/EIPS/eip-7612)
...
During a transition, the transition is advanced first before applying the storage deltas. Update semantics when a transition is occuring:
...
`storage_count` is RLP-encoded as a minimal-length big-endian byte string (no leading zeros). The per-account overhead is therefore small and not a fixed 32 bytes. The field is present only for accounts with non-empty code hash. If `storage_count == 0`, the field may be omitted.
...
If counting began only at `FORK_TIMESTAMP`, every existing contract would start with an implicit `storage_count = 0` and accrue counts solely from *new* writes. This would misprice precisely the workloads we intend to regulate: large, pre-fork contracts would continue paying unscaled base costs until they happen to rewrite a substantial portion of their historical slots. Gas pricing would under-reflect their actual storage footprint.
...
- **Cons:** Late upgraders, snap-syncing nodes, or nodes that were offline cannot reconstruct pre-fork counts deterministically. This approach has to be coordinated so that all users upgrade their nodes and must finish counting prior to the fork activation.
