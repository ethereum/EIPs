...
The transition process involves reading all accounts and counting the number of storage slots for accounts with non-empty storage root hashes, which in turn populates the `storage_count` field in the account.
