---
title: Keystore Signatures for All Tx Types
description: Verify legacy transaction signatures through the Keystore so every transaction type can use post-quantum authenticators
...
category: Core
created: 2026-08-21
requires: 155, 2718, 2930, 1559, 4844, 7702, 8130
---

## Abstract

On [EIP-8130](./eip-8130.md) chains, every non-8130 transaction type authenticates through the
Keystore instead of treating secp256k1 `ecrecover` as authority. A 65-byte signature is always k1
and is checked against the account's **default EOA**, with `from` unspecified — the same empty-sender
path EIP-8130 uses for AA transactions. Any other signature is `account || authenticator || data` and
is checked against that account's Keystore actors. Old wallets keep working until the default EOA is
revoked; after that, every transaction type (legacy, [EIP-1559](./eip-1559.md), blobs,
[EIP-7702](./eip-7702.md), …) can be post-quantum, and aggregatable if the authenticator allows it.
...
type. Type-0, [EIP-2930](./eip-2930.md), [EIP-1559](./eip-1559.md), [EIP-4844](./eip-4844.md), and
[EIP-7702](./eip-7702.md) transactions still treat `ecrecover(hash, v, r, s) == from` as sufficient
authority. An account that has rotated to a PQ key — or revoked its default EOA — cannot originate a
blob transaction or a 1559 transfer with that key, and a k1 key that the account intended to retire
can still spend via those types.

This EIP closes that hole: on 8130 chains, **every** transaction signature is a Keystore
authentication. secp256k1 remains the 65-byte default-EOA path so unmodified wallets continue to work
for accounts that have not revoked it. Configured-actor signatures use the same authenticator
interface as EIP-8130, so a PQ (or aggregatable) authenticator that is valid on the AA path is valid
on every other path.
...
After activation, every [EIP-2718](./eip-2718.md) transaction type other than `AA_TX_TYPE`, and
legacy (type-0) transactions, carry a signature as a byte string `sig` over the type's existing
signing hash `H` (the hash secp256k1 currently signs; `sig` is not part of `H`).
...
- **Initiation:** `scope == 0x00` or `(scope & (SENDER | POLICY)) != 0`. If `POLICY` is set, require
  `tx.to == policy_manager(account, actorId)` (the EIP-8130 policy gate). A contract-creation
  transaction (`to` empty) fails the gate.
- **Payment:** `scope == 0x00` or `(scope & SELF_PAYER) != 0`.
...
The rule applies to every transaction type except `AA_TX_TYPE`, including:
...
| `0x03` | [EIP-4844](./eip-4844.md) | type-3 payload hash | `yParity, r, s` |
| `0x04` | [EIP-7702](./eip-7702.md) | type-4 payload hash | `yParity, r, s` |

Future [EIP-2718](./eip-2718.md) types on an 8130 chain MUST use this signature rule unless they
define their own Keystore authentication (as EIP-8130 does).
...
**Why Keystore on every type, not a new type.** A new PQ transaction type would leave 1559, blobs,
and 7702 as k1 backdoors. Rotating off secp256k1 only works if *every* origin path consults the same
authority.
...
default EOA). Start originating 1559/blob/7702 transactions with `account || pq_authenticator || data`.
Revoke the default EOA. 65-byte k1 signatures fail on every type. The account address does not change.
...
**Scope.** A `SENDER` actor without `SELF_PAYER` cannot self-pay an EIP-8130 AA transaction and MUST
NOT self-pay a type-2 transaction either. `POLICY` actors are gated to `tx.to == manager`, including
on blob and 7702 transactions.
...
type 3; a Keystore-authorized actor cannot replay across types. Chain id remains in `H`.
