...
class LogIndexMinimalState:
    next_index: uint64                                                              # next free index where the block entry of the previous head will be added
    ...
    index_entries_branch: Vector[Bytes32, log2(MAPS_PER_EPOCH * VALUES_PER_MAP)]    # merkle branch of index entry where next_index points
```
...
Note that initialization is also possible on an epoch boundary, in which case the log index is very cheap to initialize (only requires the epoch branch and the last _block entry_ branch from the last epoch) but in this case all blocks and receipts between the boundary and the current head are required to generate the last unfinished epoch.
