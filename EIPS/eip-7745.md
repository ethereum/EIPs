...
These definitions use the `ProgressiveList` SSZ type as defined in [EIP-7916](./eip-7916.md).
...
type FilterRow = ProgressiveList[MAX_BASE_ROW_LENGTH * log2(MAP_WIDTH) // 8 * MAPS_PER_EPOCH, LAYER_COMMON_RATIO]   # assumes that MAP_WIDTH is a power of 256
...
class Log(Container):
    ...
    data: ProgressiveList[MAX_LOG_DATA_SIZE, 4]
