...
category: ERC
created: 2022-07-10
requires: 137, 165, 4804, 4834
---

## Abstract

Ethereum is the most-established blockchain for building decentralized applications (referred to as `DApp`s). Due to this, the Ethereum DApp ecosystem is very diverse. However, one issue that plagues DApps is the fact that they are not fully decentralized. Specifically, to interface a "decentralized" application, one first needs to access a *centralized* website containing the DApp's front-end code, presenting a few issues (discussed in the Motivation). To solve these issues, this EIP introduces an interface allowing clients to make HTTP-like requests (containing a method, a resource to request, a request body, and headers) to smart contracts, and to receive HTTP-like responses (containing headers and a body).
...
It is RECOMMENDED for clients implementing this EIP to recognize the following standards:
...
Clients MAY also support regular DNS, as defined in RFC 1034 and RFC 1035.
...
- Other EIPs, such as [EIP-4804](./eip-4804.md), can be used to request state changing operations in conjunction with a `307 Temporary Redirect` status code.
