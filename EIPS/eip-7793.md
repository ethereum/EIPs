...
- If `evm.message.depth != 0 or evm.message.gas - evm.gas_left > costof(PUSHN) + costof(ASSERTTXINDEX)`
- If `evm.message.tx_env.index_in_block is None` (i.e. for system transactions)
- If `expected_tx_index != evm.message.tx_env.index_in_block and expected_tx_index != -1`
