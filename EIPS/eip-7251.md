...
The consolidation smart contract uses fee mechanism to rate limit a number of requests per block. Details of the fee mechanism are available in [EIP-7002](./eip-7002.md#fee-update-rule).
...
Note: the above code reverts if the fee is too high, the fee can change significantly between creation of a consolidation request transaction and its inclusion into a block, thus, this check is very important to avoid overpayments.
