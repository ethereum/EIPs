...
`"Burn"`MUST trigger when the bonds are being redeemed via staking  by the bank contract.

`"Burn"` MUST trigger when Bonds are burned (for instance during redemption or invalidation of bonds ). This SHOULD not include zero value burning.
