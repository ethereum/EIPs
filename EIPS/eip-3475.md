...
This API standard allows for the creation of arbitrary type of bonds  in a single contract. Existing LP token standards like [ERC-20](./eip-20.md) require deployment of separate factory and token contracts per token type. The need of issuing bonds with multiple redemption data can’t be achieved with existing token standards without relying of external contracts , hence increasing the complexicity. ERC-3475 Multiple Callable Bonds Standard allows for each bond class ID to represent a new configurable token type, and for each bond nonce to represent an issuing date or any other forms of data in uint256. Every single nonce of a bond class may have its own metadata, supply and other redemption conditions.

## Motivation

Current LP token is a simple ERC-20 token, which only stores the balances in its contract storage logic , not further info about pricing , interest , redemption logic etc that is needed by Bonds for  complex reward and redemption logic to be built in order issue decentralized bonds.Thus  we need a new LP token standard that can manage multiple bonds, stores much more data and gas efficient. ERC-3475 standard interface allows any entity (smart contracts or EOA) on solidity compatible blockchains to create its own bond. These bonds with this standard addresses  the following: 

- compatible and  can be exchanged in secondary market.  
- allows any 3rd party wallet applications or exchanges to read the balance / redemption conditions . 
- defines approvals for addresses that can handle the transfer /burn and other write operations .
-  defines the metadata to unwrap and read the underlying supply and redemption logic , making the standard compatible to all the secondary bond markets.
...
Trading and burning of ERC-3475 Bonds will also better maintain the tokenomics of the market and providing more composablity for buiding other defi derivatives market, helping it to recover from recession period. Existing structures, such as AMM exchanges or lending platform can be updated to recognize ERC-3475 Bonds.
...
**definitions**
- Issuer (Bank) : is the enitity that issues the bond after getting the necessary amount of liquidity. generally its an single entity having access to the pool.
- Exchange : its the contract / factory contracts that manages the  secondary market .
- Redeemers : this will be entities that redeem the bonds (generally these can be the single entity as issuer or can be ones that are whitelisted) .
...
This example shows the transfer from `_from` address, to `\_to` address,  amount of active supply of `500000000`  of bond class`1` nonce `42`

**important** :  before transferring the assets , it MUST need  to be approved by the owner using the function approve (explained below).
...
**important** It SHOULD be issued by single entity  (for instance role based ownable contract that has integration with the liquidity pool).
...
those input mean “redeem from wallet address(0x2d03B6C79B75eE7aB35298878D05fe36DC1fE8Ef), 500000000 of bond class1 nonce 42. this means corresponding active supply will be reduced and the ERC20 token will be transferred to the given owner .



**important** redeem function for given class and nonce category MUST BE  done only after certain conditions for maturity (can be end time , total active liquidity , etc).


**Burn**

`burn(address from, uint256 classId, uint256 nonceId, uint256 amount)` allows the reduction  of any number of s from the bond type defined by classId / nonceId and  a given  user.
...
The `"nonce"` is the list of nonce of the given bond class. This param is for distinguishing of the issuing conditions of the bond.

The `"_amount"` is the  amount of the bond, that will be reduced from `"_from"`address 
e.g.

```
burnBond(0x82a55a613429Aeb3D01fbE6841bE1AcA4fFD5b2B,1,25, 150000000);  
// means that the bond holder of address 0x82a55a613429Aeb3D01fbE6841bE1AcA4fFD5b2B , of class 1 and nonce type of 25 wants to nullify  150000000 bonds , thus increasing the burnedSupply 150000000.
```

**important** burning of the tokens must be done only by the owner of the bonds .
...
The `"classId"` is the classId nonce of bond to be approved to be spend  by the owner or issuing authority(starting from 0)  .
...
Returns the class information description of the class info as key. *Every bond contract can have their own list of info description.*
