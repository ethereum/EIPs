...
### Metadata: 
the information of these bonds are stored as an array of JSON objects , consisting of following  example : 
...
the Key value for indexing the inforlation is class field . following  are the rules : 
- the title can be any alpha neumeric  different type (although can dependent in certain jurisdiction based redictions).
-  the title SHOULD not non EMPTY .

also for example  there can be possiblity to have localisation of the issuance / redemption of the bond , by describing the jurisdiction code (fr, de , cn rtc). by defining the "localisation" attribute as follows  :
