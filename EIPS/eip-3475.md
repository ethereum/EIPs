...
description: Interface for tokenized obligations with abstract on-chain metadata storage
...
**Note that the this standard interface is also compatible with [ERC-20](./eip-20.md) and [ERC-721](./eip-721.md) interface.**
...
Test case for the minimal reference implementation are available [here](../assets/eip-3475/ERC3475.test.ts). developers need to run this script with truffle dependency in an npm package.
...
## Security Considerations
    - The `function setApprovalFor`  gives the operator role in this standard. It has all the permissions to transfer, burn and redeem bonds by default. If the owner wants to give a one-time allocation to an address, he should call the approve function giving the transaction[] allocated.
    
    - The issue function can only be called by the bank contract (what we call issuer in this EIP).
