...
The reason behind the standard's name (abstract storage bond) is its ability to store all the specifications (metadata/values and transaction as defined in the following sections) without needing external storage onchain/offchain. 

The functions defined in this EIP provide a gas-efficient solution for creating, trading, and burning bonds. They offer more optimisation options for existing AMM pools.

The bonds can also be batched for issuance / redemption condition for efficiency on gas costs and UX side .And finally, bonds created from this standard can be divided and exchanged in a secondary market.
...
Test-case for the minimal reference implementation is available [here](../assets/eip-3475/ERC3475.test.ts). to test them, compile and test the contracts in truffle box.
...
- If the owner wants to give a one-time allocation to an address, he should call the `function approve()` giving the `Transaction[]` allocated rather than giving approval to all the classes using `setApprovalFor`.

- The `function issue()` can only be called by the bank contract (what we call issuer in this EIP). this can be either an smart contract managing the logic of liquidity management and routing the collateral , but also EOA multisig.

- Nonce generation should be implemented by the developer in a way its as much pseudo random as possible.
