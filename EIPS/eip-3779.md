...
The runtime adressing of `JUMP` has long been an obstacle to statically proving this sort of safety, but that very feature has been considered necessary to implement the return jump from a subroutine.  But consider this example of calling a minimal subroutine.
```
ADD:
    RTN_ADD 
    0x02
...
ADDITION:
    ...
    jump 
```
Note that the return address and the destination address are pushed on the stack as constants, so the `JUMP` instruction are static, not dynamic – they jump to the same `PC` with every run.  We do not need (nor typically use) dynamic jumps to implement subroutines.
...
* `advance_pc()` to advance skipping any immediate data.
* `imm_data() to get the immediate data.
* `add_items()` and `remove_items()` to push or pop items on the `const_stack` and return the effect of each instruction on the `stack pointer`.  `add_items()` will `PUSH` the value of constants – all other stack items are zeroed.
```
