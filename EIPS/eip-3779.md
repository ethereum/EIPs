...
description: Introduces contract creation-time validation rules for some important safety properties,.
status: Draft
...
* `imm_data()` returns the immediate data.
* `valid_jumpdest` tests whether destination is a JUMPDEST byte and not in immediate data.
* `add_items()` and `remove_items()` push or pop the `const_stack` items for each instruction, and return its effect on the `stack pointer`.
...
```
    ...
    func validate(PC :=0, stack_depth:=0) boolean {
        ...
        if PC > code_len
                || instruction == STOP ||  instruction == RETURN || instruction == SUICIDE {
...
_Rule 1_ – requiring constant destinations for `JUMP` and `JUMPI` – forbids dynamic jumps.  This rule is already required by `EIP-3540`.  Jump destinations are currently checked at runtime, but static jumps can be validated at creation time.   
...
These changes affect the semantics of existing EVM code – the [EVM Object Format](./eip-3540.md) deprecates dynamic use of JUMP and JUMPI.   Most contracts do not use dynamic jumps.  Contracts that make only static use of JUMP and JUMPI and cannot underflow or overflow stack remain valid  – most contracts probably meet those conditions.
