...
We validate _safety_ at initialization time.  We define a _safe_ EVM contract as one that cannot encounter an exceptional halting state.
...
*We would like to consider EVM programs valid iff they are safe.  And a program is safe iff no execution can lead to an exceptional halting state.*

However, we must be able to validate *code* in linear time to avoid denial of service attacks. And we must support dynamically-priced instructions, loops, and recursion, which can use arbitary amounts of gas and stack  
