...
ADDITION:
    ...
    returnsub         ; 3 gas
```
Note that the return address and the destination address are pushed on the stack as constants, so the JUMP instruction are static, not dynamic – they jump to the same instruction with every run.  Since the jumps are static we can validate their safety with a static analysis of the code.  We do no need dynamic jumps to implement subroutines. 
...
We need to define `stack depth`.  The Yellow Paper has the `stack pointer` or `SP` pointing just past the top item on the `data stack`.   We define the `stack base` as where the `SP` pointed before the most recent `JUMP` or `JUMPI`, or `0` on program entry.  So we can define the `stack depth` as the number of stack elements between the current `SP` and the current `stack base`.  
...
* We assume `add_items()` and `remove_items()` functions that push or pop items and return the effect of each instruction on the stack pointer.  `add_items()` will _PUSH_ the value of constants – all other stack items are zeroed.
