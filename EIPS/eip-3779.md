...
The `RJUMP`, `RJUMPI` and `RJUMPSUB`instructions take their destination as an immediate argument, so they are *static*. The `RJUMPV` instructions take an index on the stack at runtime, and proceed via the `jumpdest` indexed in the jump vector, or else via its first, default `jumpdest`. In this way `RJUMPV` is constrained to be *static*, as the programmer must provide a default case, and all cases are validated.
