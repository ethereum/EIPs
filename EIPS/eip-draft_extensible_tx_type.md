...
This EIP introduces a new [EIP-2718](./eip-2718.md) transaction type called the "Amsterdam transaction". The base transaction fields follow [EIP-1559](./eip-1559.md) with the access list removed. Instead of fixed-position signature fields, all signatures are encoded as typed capabilities in a trailing RLP list. Each capability carries a capability type and a signature type, providing domain separation and future-proofing for alternative signature schemes. A payer signature capability enables native transaction sponsorship. The signing hash for each capability includes only the capabilities that precede it, allowing both sender-first and payer-first signing flows without circular dependencies.

This is a simpler alternative to [EIP-8141](./eip-8141.md) (Frame Transaction) which requires new opcodes, frame modes, and per-frame gas accounting. The Amsterdam transaction achieves sponsorship with zero new opcodes and minimal changes to transaction processing.
...
### Amsterdam transaction

A new [EIP-2718](./eip-2718.md) transaction is introduced where the `TransactionType` is `AMSTERDAM_TX_TYPE` and the `TransactionPayload` is the RLP serialization of:
...
The address is verified by checking the signature against the account's Ed25519 public key, following the verification requirements from [EIP-8164](./eip-8164.md).
...
```
signing_hash(sender_or_payer) = keccak256(AMSTERDAM_TX_TYPE || rlp([
    chain_id,
    nonce,
...
A valid Amsterdam transaction MUST satisfy the following:
...
6. For ED25519 signatures, the signature MUST follow the verification requirements from [EIP-8164](./eip-8164.md).
...
[EIP-8141](./eip-8141.md) introduces frames, new opcodes, per-frame gas accounting, and multiple execution modes to achieve sponsorship. The Amsterdam transaction achieves the same result by adding a single payer signature capability to the transaction model, with no new opcodes or execution modes. The payer simply co-signs the transaction and their account is charged for gas, minimizing implementation burden and attack surface.
