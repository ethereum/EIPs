...
#### EIP-6372: clock and CLOCK_MODE

An EIP-5805 contract may optionally implement [EIP-6372: Contract clock](./eip-6372.md) to announce the clock that is used for vote tracking.

If the contract does not implement EIP-6372, it is assumed to operate according to a block number clock, exactly as if EIP-6372's `CLOCK_MODE` returned `mode=blocknumber&from=default`.

In the following specification, "the current clock" refers to either the result of EIP-6372's `clock()`, or the `block.number` if interface is not implemented.
...
While timestamps produced by EIP-6372's `clock` are represented as `uint48`, `getPastVotes`'s timepoint argument is `uint256` for backward compatibility. Any timepoint `>=2**48` passed to `getPastVotes` SHOULD cause the function to revert, as it would be a lookup in the future.
...
Both implementations use `block.number` for their checkpoints and do not implement EIP-6372, which is compatible with this EIP.
