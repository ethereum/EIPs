...
Additionally, the existing (non-standardized) implementations are limited to `block.number` based checkpoints. This choice causes many issues in a multichain environment, where some chains (particularly L2s) have an inconsistent or unpredictable time between blocks. This EIP uses the clock interface defined in [EIP-6372](./6372.md) to specify the clock used for tracking votes.
...
Both timestamps, block numbers, and other possible modes use the same external interfaces. This allows transparent binding of third-party contracts, such as governor systems, to the vote tracking built into the voting contracts. For this to be effective, the voting contracts must, in addition to all the vote-tracking functions, expose the current value used for time-tracking using EIP-6372.

### Methods

#### clock & CLOCK_MODE

Contracts implementing this EIP SHOULD also implement EIP-6372.

If vote tracking is achieved using **block number**, inheritance from EIP-6372 MAY be skipped. Any other vote tracking mode MUST implement EIP-6372.

In the absence or the `clock()` or `CLOCK_MODE()` functions should be considered as a marker of the contract operating using block number. (This makes this EIP compatible with pre-existing voting contracts).
...
- As specified in EIP-6372, the `clock()` function MUST be non-decreasing.
...
Both implementations use `block.number` for their checkpoints and do not implement the `clock()` method defined in EIP-6372, which is compatible with this EIP.
