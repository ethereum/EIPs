...
We add a new global variable, `current_frame_role`. During non-AA transactions, it is always set to `ROLE_SENDER_EXECUTION`. During AA transactions, it is set depending on the current position in the transaction flow, described below.
...
The `ACCEPT_ROLE` opcode is equivalent to `RETURN` or `REVERT` in the sense that it copies a memory slice, ends execution, and pastes the memory slice onto parent returndata and parent memory, and except:

* It does not revert
* It accepts the `current_frame_role` as the additional input parameter

The `ACCEPT_ROLE` opcode with a correct `current_frame_role` input is expected by AA transaction processing flow; if any of the transaction validation frames does not perform a valid exit via the `ACCEPT_ROLE` opcode, the transaction fails the validity check.
