...
The `2` on the stack is expected by AA transaction processing flow; if any of the transaction validation parts does not exit via the `ACCEPT_ROLE` opcode, the transaction fails.
