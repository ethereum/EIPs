...
The `ACCEPT_ROLE` opcode with a correct `current_frame_role` input is expected by AA transaction processing flow; if any of the transaction validation frames does not perform a single valid exit via the `ACCEPT_ROLE` opcode, the transaction fails the validity check.
