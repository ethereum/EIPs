...
1. `TXPARAM` opcode pushes onto the stack the 32-byte value of the transaction parameter value defined by the input `n`.
   Using `TXPARAM` for the parameters `n` with a dynamic data size is considered to be an invalid opcode and reverts the execution.
2. `TXPARAMDATASIZE` opcode puts the byte size of the transaction parameter value defined by the input `n`.
3. `TXPARAMDATALOAD` opcode takes the byte offset in the specified transaction parameter from stack and puts the
...
4. `TXPARAMDATACOPY` opcode copies data from the appropriate dynamically sized transaction parameter to memory.\
...
| 0x14   | `executionStatus`             | 32          |
| 0x15   | `actualGasCost`               | 32          |
...
The `executionStatus` and `actualGasCost` parameters are only accessible in the `role_paymaster_post_tx` section.
...
If the `validUntil` field is non-zero, the transaction is only valid for inclusion in a block with a timestamp less than the `validUntil` value.
Similarly, the transaction is only valid for inclusion in blocks with a timestamp greater than the `validAfter` value.
...
In order for the transaction to be considered valid the
paymaster validation frame MUST return without reverting.
...
The `executionStatus` and `actualGasCost` values are accessible via the `TXPARAM` opcode.
...
def block_transition_function(block):
    for index, tx in enumerate(block.txs):
        if is_type_aa(tx):
            try:
                if tx.validUntil < block.timestamp or tx.validAfter > block.timestamp:
