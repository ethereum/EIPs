...
The TXPARAM* opcodes are allowed only at the correct frame role. Calling these opcodes in another context, or with invalid `txparam_id`, returns zero value and zero size.

| role                                                                            | allowed txparam_id                                                                 | 
|---------------------------------------------------------------------------------|------------------------------------------------------------------------------------|
| 0 (none)                                                                        | none. all return zero length and zero value                                        |
| role_sender_deployment<br/>role_sender_validation<br/>role_paymaster_validation | all tx parameters, `current_frame_role` and `tx_hash_for_signature`                |
| role_sender_execution                                                           | only `current_frame_role`                                                          |
| role_paymaster_post_op                                                          | all the above parameters,<br/>and also `execution_status` and `execution_gas_cost` |
