...
* It does not revert
* It pushes the number `2` onto the stack (instead of 0 or 1).

The `2` on the stack is expected by AA transaction processing flow; if any of the transaction validation frames does not perform a valid exit via the `ACCEPT_ROLE` opcode, the transaction fails the validity check.
