...
### New opcodes `AATXPARAM` and `AATXPARAMDATACOPY`

Accessing transaction details within call frames is performed using the new `AATXPARAM` opcode.
The instructions are followed by an 8-bit immediate value, which we call `n`, and can have a value of 0 to 255.

The `AATXPARAM` opcode pushes onto the stack the transaction parameter value defined by the input `n`.

The `AATXPARAMDATACOPY` opcode copies data from the appropriate dynamically sized transaction parameter to memory.
...
|-----|-----------------------------|
| 0   | `sender`                    |
| 1   | `senderValidationData` size |
| 2   | `senderValidationData`      |
