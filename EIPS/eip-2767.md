...
A standard interface for Governance contracts that administratively decentralize the ownership of dApp smart contracts.

## Abstract

<!--A short (~200 word) description of the technical issue being addressed.-->

The following standard allows for the implementation of a standard API for a Governance smart contract. This standard provides basic functionality for on-chain and off-chain governance inheriting equal or privileged voting rights. Existing EIP-173 compatible dApps can upgrade from private key wallet ownership to a Governance smart contract. A smart contracts that implement such a mechanism, makes them more administratively decentralised and adhering to a standard API enables tools to populate governance information to dApp users.
...
At present, there are various implementation ideas for handling governance. Adhering to a standard API would enable general dApp tools (like EtherScan) to populate governance information, thus increasing transparency to dApp users. This can result in a wide adoption for dApp governance.
...
- Governor: The board member who has voting rights.

### Governance Type

A governance contract can have equal or privileged voting rights.
...
interface IGovernanceEqual {
    ...
    function required() external view returns (uint256 numerator, uint256 denominator);
...
interface IGovernancePrivileged {
    ...
    /// @param _newPowers New power for the governor
    function updatePower(address _governor, uint256 _newPower)
        external;
}
```

### Transaction execution consensus

There are two types of proposed transaction execution consensus interfaces: On-chain and Off-chain. A `EIP-2767` compatible governance contract should inherit at least one of them and optionally implement both.
...
interface IGovernanceOnchain {
    ...
    /// @dev Emits everytime a governor confirms a transaction
...
Anyone can take an initiative to execute a transaction by preparing a `EIP-191` signed data `0x 19 00 <20 bytes address> <32 bytes nonce> <20 bytes to-address> <input data>` and collect signatures from required governors. The contract logic should expect signatures to be sorted, this is to save the expenditure gas to prevent duplicate signatures. If signatures are sorted, valid, more than required, the governance will make a call to the dApp contract to perform the administrative task.

### Interface Identification

An `EIP-2767` Governance Contract should also implement `EIP-165`. This helps general tools to indentify the type of governance.
...
- standardize API of Governance contracts for dApps
...
dApp smart contracts which are `EIP-173` compatible can deploy a `EIP-2767` Governance contract and transfer dApp's ownership to it. This enables such existing dApps become compatible with `EIP-2767`.

However, there are some existing projects having governance implementations and most of them have custom APIs, since a standard did not exist. Such projects would require deploying a new governance contract and transfer ownership to it to be `EIP-2767` compatible. Not having `EIP-2767` compatible governance contract means only that general tools might be able to populate their governance information.
...
The signatures in off-chain governance implementation should follow recommendations of `EIP-191`.
