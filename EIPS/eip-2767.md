...
/// @dev ERC-165 InterfaceID: 0x4ca6c101
interface ERC2767 {
...
Implementations can have a simple number of required votes value or any complex logic that depends on other factors (for e.g. `50%+` or `66%+` of `totalPower()`). The `required()` should return the calculated number of votes using the complex logic.

### Interface Identification

An `EIP-2767` Governance Contract should also implement `EIP-165`. This helps general tools to identify whether a contract is a `EIP-2767` Governance contract.
...
There are two optional interfaces for transaction execution: On-chain and Off-chain. A `EIP-2767` compatible governance contract can optionally implement one of them or both. The contract should specify the interfaceID(s) using `EIP-165`'s `supportsInterface` method.
...
Smart contracts that are `EIP-173` compliant can transfer their ownership to a Governance contract. This enables existing contracts to become compatible with `EIP-2767`.

However, there are some existing projects with governance implementations and most of them have custom APIs, since a standard did not exist. Such projects need to deploy a new governance contract and transfer ownership to it to be `EIP-2767` compatible. Not having an `EIP-2767` compatible governance contract means only that general tools might be able to populate their governance information.
