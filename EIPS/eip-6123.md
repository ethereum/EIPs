...
Every process state is specified; therefore, the entire post-trade process is known in advance and is deterministisc over the trade's live cycle. An [EIP-20](./eip-20.md) token can be used for frictionless decentralized settlement, see reference implementation. We do provide an own interface and implementation for a specific "Settlement Token" derived from [EIP-20](./eip-20.md).
