...
The `SIGRECOVER` precompile is used from [EIP-7932](./eip-7932.md).

- Retrieve the `mode` with `TXPARAMLOAD`.
- If `mode` is `VERIFY`:
  - Load first byte from `frame.data` as `scope`.
  - Load remaining data from `frame.data` as `signature`.
  - If `SIGRECOVER(signature || sighash || scope) != frame.target`, revert.
  - Call `APPROVE(scope)`.
- If `mode` is `SENDER`:
...
def default_code(frame, tx):
    ...
    if mode == VERIFY:
        scope          = frame.data[0]             # First byte for the scope of the verification
        signature      = frame.data[1:]            # The remaining data is the signature information
        sig_hash       = compute_sig_hash(tx)      # equivalent to TXPARAMLOAD(0x08)

        if SIGRECOVER(signature + sig_hash + scope) != frame.target:
            revert()
...
| Frame | Caller      | Target        | Data                   | Mode    |
| ----- | ----------- | ------------- | ---------------------- | ------- |
| 0     | ENTRY_POINT | Null(sender)  | (0x0, 0xFF, r, s, v)   | VERIFY  |
