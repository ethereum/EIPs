...
Frames in this mode will have their data elided from signature hash calculation and from introspection by other frames, except for [expiry verifier frames](#expiry-verifier-frame).
...
2. If frame has mode `VERIFY` and the frame reverted, the transaction is invalid.
