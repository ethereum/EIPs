...
frames = [[mode, target, gas_limit, data, value, *signature], ...]
```

`signature` is only present in `VERIFY` frames (see below).
...
Frames in this mode have a `signature` field which is elided from signature hash calculation and from introspection by other frames.
...
In the canonical signature hash any frame with mode `VERIFY` will have its signature elided:

```python
def compute_sig_hash(tx: FrameTx) -> Hash:
    for i, frame in enumerate(tx.frames):
        if frame.mode == VERIFY:
            tx.frames[i].signature = Bytes()
...
| 0x11  | must be 0   | `signature` of the current frame (revert if the mode is not `VERIFY`)      | 32      |
| 0x12  | frame index | `target`                             | 32      |
| 0x13  | frame index | `data`                               | dynamic |
| 0x14  | frame index | `value`                              | 32      |
| 0x15  | frame index | `gas_limit`                          | 32      |
| 0x16  | frame index | `mode`                               | 32      |
| 0x17  | frame index | `status` (exceptional halt if current/future) | 32 |
...
1. Execute a call with the specified `mode`, `target`, `gas_limit`, `data`, and `value`.
   ...
   - If mode is `VERIFY` and `frame.target` has no code:
       - Read `frame.signature` as `(v, r, s)`.
       - Verify that `frame.target == ecrecover(hash, v, r, s)`, where `hash` is the transaction payload hash.
           - If verification fails, revert the frame.
       - Read `frame.data` as `scope`, and call `APPROVE(scope)`.
...
| Frame | Caller      | Target        | Data                   | Signature | Mode    |
| ----- | ----------- | ------------- | ---------------------- | ------- |
| 0     | ENTRY_POINT | Null(sender)  | 0                      | Signature (v, r, s) | VERIFY  |
| 1     | ENTRY_POINT | Sponsor       | Sponsor data           | Sponsor signature | VERIFY  |
| 2     | Sender      | ERC-20        | transfer(Sponsor,fees) | N/A | SENDER  |
| 3     | Sender      | Target addr   | Call data              | N/A | SENDER  |
