...
```
[chain_id, signer, nonce, sender, frames, max_priority_fee_per_gas, max_fee_per_gas, max_fee_per_blob_gas, blob_versioned_hashes, valid_after, valid_before]

frames = [[mode, flags, target, gas_limit, value, data], ...]
```

`signer`, `nonce`, `valid_after`, and `valid_before` are `uint64`. `nonce` keeps its existing field name, type, and position relative to upstream; its meaning depends on `signer`. When `signer == 0`, `nonce` is the sender's legacy account nonce (byte-identical to upstream behavior) and no `AUTH_MANAGER` lookup runs. When `signer != 0`, `signer` selects a registered signer entry in `AUTH_MANAGER` for `tx.sender` and `nonce` is the sequence within that signer's authentication stream. The `signer` field is a small protocol-managed identifier into `AUTH_MANAGER`; the underlying pubkey is fetched via `getSigner(tx.sender, signer)` and may be kilobyte-scale (lattice or multivariate). A zero validity bound means no bound.
...
The approval-scope field expands from two bits in the upstream spec to three to accommodate `APPROVE_GUARANTEE` (see [APPROVE](#approve-opcode-0xaa)). The atomic-batch flag therefore moves from bit 2 to bit 3.
...
assert tx.valid_after < 2**64
assert tx.valid_before < 2**64
assert tx.valid_after == 0 or tx.valid_before == 0 or tx.valid_after < tx.valid_before
...
# Atomic batch flag (bit 3 of flags) is only valid with SENDER mode, and next frame must also be SENDER.
...
5. `APPROVE_GUARANTEE = 0x4`
6. `APPROVE_SCOPE_MASK = APPROVE_PAYMENT | APPROVE_EXECUTION | APPROVE_GUARANTEE`
...
4. `APPROVE_GUARANTEE` (`0x4`): Approval of a guarantor commitment - the contract commits to paying the maximum tx cost if sender validation later fails. See [Guarantors](#guarantors).

Any other value, including `APPROVE_SCOPE_NONE` and combinations involving `APPROVE_GUARANTEE` with other scopes (e.g. `0x5`, `0x6`, `0x7`), results in an exceptional halt.
...
- If `allowed_scope == APPROVE_GUARANTEE`, only `APPROVE_GUARANTEE` is allowed.
...
- For scopes `APPROVE_EXECUTION`, `APPROVE_PAYMENT`, and `APPROVE_PAYMENT_AND_EXECUTION`, execute the following:
    ...
    - `APPROVE_PAYMENT`: Collect the transaction's maximum cost (`TXPARAM(0x06)`) from `resolved_target` and set `payer_approved = true`.
    ...
    - `APPROVE_PAYMENT_AND_EXECUTION`: Set `sender_approved = true`, collect the transaction's maximum cost (`TXPARAM(0x06)`) from `resolved_target`, and set `payer_approved = true`.
...
- For `APPROVE_GUARANTEE`:
    - Set `guarantor_approved = true` and `guarantor = resolved_target`.
    ...
    - The guarantor's reservation is released and re-collected from the payer if a later `APPROVE_PAYMENT` overrides it; see [Guarantors](#guarantors).
...
Perform pre-frame consensus checks:

```python
# Validity window (see Validity Windows section)
if tx.valid_after  != 0 and block.timestamp <= tx.valid_after:  invalid
if tx.valid_before != 0 and block.timestamp >= tx.valid_before: invalid
...
Initialize transaction-scoped variables:
...
Consecutive `SENDER` frames where all but the last have the atomic batch flag (bit 3 of `flags`) set form an **atomic batch**. Within a batch, if any frame reverts, all preceding frames in the batch are also reverted and all subsequent frames in the batch are skipped.
...
After executing all frames, verify the validity rule above. If `payer_approved == true`, refund any unpaid gas to the gas payer. Otherwise if `guarantor_approved == true`, the guarantor pays. Otherwise the whole transaction is invalid.

Note:

- It is implied by the handling that the sender must approve the transaction *before* the payer (when not guarantor-backed) and that once `sender_approved`, `payer_approved`, or `guarantor_approved` become `true` they cannot be re-approved or reverted.
...
- Signature types beyond `0x0` and `0x1` are sourced from the protocol-level `signature_type` registry alongside their address-derivation rules. The default code resolves PQ pubkeys exclusively via `AUTH_MANAGER.getSigner`; inline envelope pubkeys are not supported.

Here's the secp256k1 and P256 logic above implemented in Python:
...
APPROVE_SCOPE_MASK = APPROVE_PAYMENT | APPROVE_EXECUTION | APPROVE_GUARANTEE
ATOMIC_BATCH_FLAG = 0x08
...
def default_code(frame, tx):
    ...
    if mode == VERIFY:
        ...
        if signature_type == SECP256K1:
            ...
            # Registered PQ schemes resolve via AUTH_MANAGER; see Signer Binding.
            verify_pq_scheme(frame, tx, resolved_target, signature_type, sig_hash)
...
This refund is returned to the gas payer (the `target` that called `APPROVE(APPROVE_PAYMENT)`, `APPROVE(APPROVE_PAYMENT_AND_EXECUTION)`, or, if no payer was approved, the guarantor) and added back to the block gas pool. *Note: This refund mechanism is separate from EIP-3529 storage refunds.*
...
This section specifies the time-bound semantics surfaced by `tx.valid_after` and `tx.valid_before`.
...
if tx.valid_after  != 0 and block.timestamp <= tx.valid_after:  invalid
if tx.valid_before != 0 and block.timestamp >= tx.valid_before: invalid
if tx.valid_after  != 0 and tx.valid_before != 0 and tx.valid_after >= tx.valid_before: invalid
```

The bounds are exclusive on both ends. A tx in a block whose timestamp violates either bound is invalid; the block is invalid if it includes such a tx. Reverse windows (both non-zero, `valid_after >= valid_before`) MUST be rejected statically.

Three states are deterministic from the envelope and observable without simulating any frame:

- **future-valid**: `block.timestamp <= valid_after`
- **ready**: inside the window
- **expired**: `block.timestamp >= valid_before`

Mempool nodes SHOULD evict expired transactions on each new head and MAY defer gossip for future-valid transactions until near `valid_after`. See [Mempool](#mempool).

`tx.valid_after` and `tx.valid_before` are exposed via `TXPARAM(0x0E)` and `TXPARAM(0x0F)`.
...
The **validation prefix** of a frame transaction is the shortest prefix of frames whose successful execution sets `payer_approved = true` or `guarantor_approved = true`.

Public mempool rules apply only to the validation prefix. Once `payer_approved = true` (or `guarantor_approved = true` with the guarantor structurally identifiable), subsequent frames are outside public mempool validation and may be arbitrary. In particular, `user_op` and `post_op` occur after payment approval and are therefore not subject to the public mempool restrictions below.
...
8. the validity-window envelope fields, deterministic from the envelope,
...
The public mempool recognizes the following validation prefixes, each optionally preceded by one `deploy` frame. Structural rules are enforced only up to and including the frame that sets `payer_approved` or `guarantor_approved` to `true`.
...
6. The validity window MUST permit inclusion at the current head (`valid_after < block.timestamp < valid_before`, with zeros meaning unbounded). Future-valid txs are buffered subject to the deferral horizon below; expired txs are rejected.
7. The sum of `gas_limit` values across the validation prefix must not exceed `MAX_VERIFY_GAS`.
8. Nodes should stop simulation immediately once `payer_approved = true` or `guarantor_approved = true` has been observed.

#### Validity-window deferral

- Public mempool admits future-valid txs whose `valid_after - now <= 1 hour`; expansive tier extends to 24 hours; direct-to-builder is unlimited. Beyond a tier's horizon, nodes MAY reject with a `validity_window_too_far_future` policy error.
- Future-valid txs are held locally and gossiped only once `now >= valid_after - GOSSIP_THRESHOLD` (e.g. 60 seconds).
- Expired txs are dropped on each new head.

#### Replacement and block invalidation

- **RBF**: matches `(sender, signer, nonce)`. A replacement MUST also satisfy its own validity window.
...
A public mempool node must simulate the validation prefix and reject the transaction if any of the following occurs before `payer_approved = true` or `guarantor_approved = true`:
...
The canonical paymaster in this draft authorizes via a single secp256k1 signer through `ecrecover`, supports both paymaster mode (signing `TXPARAM(0x08)`) and guarantor mode (signing `TXPARAM(0x0B)`), and may change in later specifications, in which case a new canonical implementation version would be required.
...
3. The node checks the validity window. Expired txs are rejected; future-valid txs are buffered or rejected per the deferral horizon.
...
7. A node should keep at most one pending frame transaction per `(sender, signer, nonce)` triple in the public mempool. A new transaction with the same triple MAY replace the existing one only if it satisfies the node's fee bump rules and its own validity window.
8. If all checks pass, the transaction may be accepted into the public mempool and propagated to peers.

#### Revalidation

When a new canonical block is accepted, the node removes any included frame transactions from the public mempool, updates paymaster reservations accordingly, and identifies the remaining pending transactions whose tracked dependencies were touched by the block. This includes at least transactions for the same sender, transactions whose recorded sender storage slots changed, transactions whose `(sender, signer)` `AUTH_MANAGER` slot changed, transactions that reference a canonical paymaster (or guarantor) instance whose balance, code, or delayed-withdrawal state changed, and transactions whose validity window has just become future-valid or expired. The node then re-simulates the validation prefix of only those affected transactions against the new head and evicts any transaction that no longer satisfies the public mempool rules.
...
### Validity windows are envelope-level

Today, a tx that wants a deadline can only enforce one inside a `VERIFY` frame reading `block.timestamp`, which is a banned opcode in the public mempool. Moving the bound into the envelope, checked once at tx entry, lets nodes determine future-valid/ready/expired states deterministically without simulating any frame. Both fields are covered by `compute_sig_hash` for free; consensus checks them pre-frame.
...
Using a flag to indicate atomic batches saves us from having to introduce a new mode. Batches are identified purely by consecutive `SENDER` frames with the flag set, terminated by a `SENDER` frame without it. This design enables consecutive atomic batches since the batch boundary is clearly indicated by the `SENDER` frame without the flag. The atomic batch flag moves from bit 2 to bit 3 in this draft because the approval-scope field expanded by one bit to accommodate `APPROVE_GUARANTEE`.
...
| 1     | ENTRY_POINT | Null(sender)  | 0     | APPROVE_PAYMENT_AND_EXECUTION | PQ sigtype + permit   | VERIFY |
...
Notes: Signer = 0 (legacy path) compresses to 1 byte. valid_after / valid_before zero compresses to 1 byte each. Otherwise identical to the upstream total (136 bytes), so the four additions cost ~2 extra bytes for the common EOA case.
...
The envelope adds three fields (`signer`, `valid_after`, `valid_before`). RLP decoders MUST be updated; transactions that omit these fields are not valid `FRAME_TX_TYPE` payloads. For EOAs that do not register any signer, the legacy account-nonce behavior is preserved by setting `signer = 0`, and unbounded validity is preserved by setting `valid_after = valid_before = 0`.
...
Such transactions are valid when submitted but become invalid once the deadline passes, without any on-chain action required from the attacker. The envelope-level `valid_before` field exists in part to give wallets a cheap, public-mempool-safe alternative to this pattern.
...
It's recommended that to *validate* the transaction, a specific frame structure is enforced and the amount of gas that is expended executing the validation phase must be limited. Once the validation prefix reaches payer approval via `APPROVE(APPROVE_PAYMENT)` or `APPROVE(APPROVE_PAYMENT_AND_EXECUTION)`, or guarantor approval via `APPROVE(APPROVE_GUARANTEE)`, the transaction can be included in the mempool and propagated to peers safely.
...
In general, it can be assumed that handling of frame transactions imposes similar restrictions as EIP-7702 on mempool relay, i.e. only a single transaction can be pending for a given `(sender, signer, nonce)` triple that uses frame transactions.
...
Wallets MUST display `valid_after` / `valid_before` to users before signing. A short `valid_before` window reduces stale-execution risk for swap-like flows; a long window approaches the unbounded behavior. Reverse windows (`valid_after >= valid_before` with both non-zero) are statically invalid and rejected at admission.
