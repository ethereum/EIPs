...
A `pay` frame that consumes payer-owned replay state generally executes in `VERIFY` mode and is immediately followed by `DEFAULT` frame to consume the nonce.
