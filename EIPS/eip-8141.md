...
Adds a new transaction whose validity and gas payment can be defined abstractly, therefore enabling *native account abstraction*.  This is achieved by decomposing the transaction into a sequence of *frames* which are contract calls that can approve transaction payment and execution.

## Motivation

Native AA in the form of frame transactions offers UX and security benefits across many areas:
...
- the default EOA account gives the protocol an even stronger guarantee on the lowest common denominator of account functionality
...
3. If frame has mode `SENDER` with the atomic batch flag set, and it fails, unroll the atomic batch.
    - An **atomic batch** is a maximal contiguous sequence of `SENDER` frames `[i, j]` (inclusive on both ends) where `j > i`, frames `i` through `j - 1` have `ATOMIC_BATCH_FLAG` set, and frame `j` does not have `ATOMIC_BATCH_FLAG` set.
...
- `APPROVE_PAYMENT_AND_EXECUTION` (`0x3`): Approval of payment and execution.
...
Thanks to the default code, EOAs today can use frame transactions to reap many benefits of AA, including sending sponsored transactions, paying gas in ERC-20 tokens, batch transactions, and more.
