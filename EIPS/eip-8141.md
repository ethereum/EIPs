...
Notes: Signer = 0 (legacy path) compresses to 1 byte. `expiry == 0` compresses to 1 byte. Otherwise identical to the upstream total (136 bytes), so the two additions cost ~1 extra byte for the common EOA case.
...
Such transactions are valid when submitted but become invalid once the deadline passes, without any on-chain action required from the attacker. The envelope-level `expiry` field exists in part to give wallets a cheap, public-mempool-safe alternative to this pattern.
