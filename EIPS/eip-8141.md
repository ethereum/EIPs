...
#### Concurrent Pending By Keyed-Nonce Domain

The generic single-pending-transaction-per-sender limit exists because pending transactions from one sender share fate: under one account nonce they form a single sequence in which only one transaction can be included, and one change to the sender's validation inputs can invalidate all of them at once.

[EIP-8250](./eip-8250.md) removes the first cause for transactions on disjoint non-zero nonce-key sets. Such transactions are replay-independent: their replay protection lives in the `NONCE_MANAGER` sequence of their own keys, not in the sender's account nonce, so consuming one set's keys leaves every disjoint set unaffected.

A node MAY therefore admit more than one pending frame transaction from a single sender when their non-zero nonce-key sets are pairwise disjoint and each transaction's dependency set, as accumulated under the trace rules below, reaches no external mutable state and no storage slot other than its own `NONCE_MANAGER` key sequence. Each such transaction is then invalidated only by consumption of its own keys; the others remain valid. Transactions whose key sets overlap share fate and count as one pending transaction for this limit.

This bounds a sender's concurrent pending transactions by the number of disjoint keyed-nonce domains it uses, which `MAX_NONCE_KEYS` already caps, and by the per-payer exposure reservation the mempool rules already require: the summed maximum cost of the sender's pending transactions must not exceed the payer's balance. It adds no new constant and defines no account class -- the unit is the keyed-nonce set EIP-8250 already gives every transaction, read directly from the transaction rather than declared by a separate on-chain flag. A sender using the account nonce (`nonce_keys == [0]`) keeps the single-pending sequence.
