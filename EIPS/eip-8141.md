...
Identifies the frame as a validation frame. Its purpose is to *verify* that a sender, payer, or guarantor authorized the transaction.

The execution behaves the same as `STATICCALL` for user code: state cannot otherwise be modified. The `APPROVE` opcode is the only exception and applies its protocol-defined effects, including approval updates and, for payment scopes, gas-charge collection.

Frames in this mode will have their data elided from canonical signature hash calculation and from introspection by other frames.
...
For the canonical signature hash, any frame with mode `VERIFY` will have its data elided:
...
def compute_frame_sig_hash(tx: FrameTx, frame_index: int) -> Hash:
    tx_copy = deep_copy(tx)
    tx_copy.frames[frame_index].data = Bytes()
    return keccak(rlp(tx_copy))
```

The canonical signature hash is useful when sender and payer signatures are independent.  The frame signature hash can be used when either the sender signature or the payer signature depends on the other.
...
4. `APPROVE_PAYMENT_AND_EXECUTION = APPROVE_PAYMENT | APPROVE_EXECUTION = 0x3`
5. `APPROVE_GUARANTEE = 0x4`
6. `APPROVE_SCOPE_MASK = APPROVE_PAYMENT | APPROVE_EXECUTION | APPROVE_GUARANTEE = 0x7`

The `scope` operand must be exactly one of the following values:
...
3. `APPROVE_PAYMENT_AND_EXECUTION` (`0x3`): Approval of payment and execution. Processed atomically within a single `APPROVE` and is not equivalent to invoking `APPROVE` twice.
4. `APPROVE_GUARANTEE` (`0x4`): Approval of payment as a guarantor - the contract unconditionally approves paying for the transaction, even if sender validation subsequently fails. See the [guarantor behavior](#guarantor-behavior) section below. A guarantor's payment can later be overridden by a subsequent `APPROVE_PAYMENT`. `APPROVE_GUARANTEE` cannot be combined with `APPROVE_PAYMENT` or `APPROVE_EXECUTION`.
...
- If scope is `APPROVE_EXECUTION` or `APPROVE_PAYMENT_AND_EXECUTION`:
  - Revert if `sender_approved` is already `true`.
  - Revert if `resolved_target != tx.sender`.
  - Set `sender_approved = true`.

- If scope is `APPROVE_PAYMENT` or `APPROVE_PAYMENT_AND_EXECUTION`:
...
See the [guarantor behavior](#guarantor-behavior) section below for more details on guarantors.
...
- `0x09` returns the frame-elide signature hash for the currently executing frame. Unlike `0x08`, which elides *all* `VERIFY` frame data, `0x09` elides only the current frame's data, leaving every other frame's data (including other `VERIFY` frames) intact. This value depends on the executing frame index and MUST be computed at most once per frame and cached.
...
- Let `resolved_target = frame.target if frame.target is not null else tx.sender`, then execute a call with the specified `mode`, `flags`, `resolved_target`, `gas_limit`, `value`, and `data`.
  - If mode is `SENDER`:
...
In general, any frame may revert without automatically invalidating the transaction (subject to the restrictions above); when a frame reverts, execution continues with the next frame.
...
If the condition is `false`, the whole transaction is invalid. If it is `true`:

1. If `sender_approved == true`, increment the sender's nonce.
2. Refund any unpaid gas to `payer`.

Notes:

- Once any of `sender_approved`, `payer_approved`, or `guarantor_approved` becomes `true`, it cannot be reverted or re-set.
- The sender's nonce is incremented only when the sender authenticated (`sender_approved == true`). In guarantor flows where sender validation fails, the sender's nonce is untouched; replay protection in that case relies on the guarantor's own mechanism (see [canonical paymaster](#canonical-paymaster)).

#### Guarantor behavior

An account is said to be the *guarantor* of a frame transaction if it's the target of a `VERIFY` frame who successfully called `APPROVE` with the `APPROVE_GUARANTEE` scope.  The guarantor unconditionally pays gas for the transaction, even if the sender validation fails.  As such, a transaction is valid as long as there's a valid guarantor.
...
- If sender validation succeeds (`sender_approved == true`), the protocol increments the sender's nonce. If sender validation fails, the sender's nonce is untouched and replay protection falls back to the guarantor's own mechanism — see the [canonical paymaster](#canonical-paymaster) section for the standardized approach.

See [the rationale section](#guarantor-mechanism) for why the guarantor mechanism exists.
...
def default_code(frame, tx):
    mode = frame.mode                        # equivalent to FRAMEPARAM(0x02, TXPARAM(0x0B))
    resolved_target = frame.target if frame.target is not None else tx.sender
    ...
    if mode == VERIFY:
        ...
        if allowed_scope == APPROVE_SCOPE_NONE:
            revert()
        # EOAs cannot safely serve as guarantors (no replay protection).
        if allowed_scope & APPROVE_GUARANTEE != 0:
            revert()
...
The **validation prefix** of a frame transaction is the shortest prefix of frames whose successful execution satisfies the [transaction validity condition](#post-execution) (`guarantor_approved OR (sender_approved AND payer_approved)`).
...
The **validation prefix** ends at the frame that satisfies the [transaction validity condition](#post-execution). The public mempool recognizes three base prefixes, each optionally preceded by a single `deploy` frame:

| Name                | Base prefix           |
|---------------------|-----------------------|
| Self relay          | `self_verify`         |
| Canonical paymaster | `only_verify`, `pay`  |
| Canonical guarantor | `guarantee`           |

When a `deploy` frame is present it must be the first frame, giving e.g. `deploy`, `only_verify`, `pay`.

Frames after these prefixes are outside public mempool validation. For example, a transaction may continue with any number of `user_op`s, or with `only_verify` after a `guarantee` prefix.
...
7. Nodes should stop simulation immediately once the transaction validity condition has been observed.
...
Because the canonical paymaster implementation is explicitly standardized to be safe for public mempool use, nodes do not need to apply the generic validation trace and opcode rules to that `pay` or `guarantee` frame. Instead, they identify it by runtime code match and apply the paymaster-specific accounting and revalidation rules in this section.
...
###### Paymaster mode vs. guarantor mode

The canonical paymaster implementation has two code paths, selected by the currently executing frame's `allowed_scope`.

- **Paymaster mode** (`allowed_scope == APPROVE_PAYMENT`): the paymaster's signer authorizes the canonical signature hash `sig_hash = TXPARAM(0x08)`. The paymaster calls `APPROVE(APPROVE_PAYMENT)`.

- **Guarantor mode** (`allowed_scope == APPROVE_GUARANTEE`): the paymaster authorizes the frame signature hash (instead of the canonical signature hash) and calls `APPROVE(APPROVE_GUARANTEE)`. The sender nonce advances only if sender validation succeeds; otherwise, replay protection falls back to a per-sender nonce mapping in the paymaster's storage:

  ```solidity
  mapping(address sender => uint256 nonce) guarantor_nonce;
  ```

  The expected frame ordering for a guaranteed transaction is: `guarantee` (VERIFY), sender validation (VERIFY), `bump_nonce` (DEFAULT), then any `user_op` frames. Because a `VERIFY` frame executes under `STATICCALL` semantics and cannot write to storage, nonce management is split across the `guarantee` and `bump_nonce` frames.

  **`guarantee` frame (VERIFY).** The paymaster:

  1. Reads the `bump_nonce` frame's shape via `FRAMEPARAM`/`FRAMEDATALOAD` at frame index `current + 2` (skipping the sender validation frame at `current + 1`) and requires:
     - the frame exists (`TXPARAM(0x0B) + 2 < TXPARAM(0x0A)`),
     - `target == ADDRESS()` (the paymaster itself),
     - `mode == DEFAULT`,
     - `gas_limit` is sufficient to complete a cold-slot `SSTORE` to `guarantor_nonce[tx.sender]`,
     - `data` is the ABI encoding of `bumpNonce(tx.sender, payer_nonce)` for some `payer_nonce` (the selector and `tx.sender` argument are explicitly checked; `payer_nonce` is read from `data[36:68]`).
  2. Reads `guarantor_nonce[tx.sender]` via `SLOAD` and requires it to equal the `payer_nonce` read from the `bump_nonce` frame's data.
  3. Authenticates the frame signature hash against the paymaster's signer. The frame signature hash elides only the guarantor's own frame data while preserving every other frame's data — including the sender's `VERIFY` frame data and the `bump_nonce` frame's `data` (which carries `payer_nonce`). So the signature binds the sender's `VERIFY` data (preventing grief via sig mutation) and also binds `payer_nonce` (via the `bump_nonce` frame).
  4. Calls `APPROVE(APPROVE_GUARANTEE)`.

  **`bump_nonce` frame (DEFAULT).** The frame calls `paymaster.bumpNonce(sender, payer_nonce)`. The paymaster:

  1. Requires `msg.sender == ENTRY_POINT`.
  2. Requires the `guarantee` frame at `current - 2` to be a successful self-targeted guarantor `VERIFY`, via `FRAMEPARAM`:
     - `FRAMEPARAM(0x00, current - 2) == ADDRESS()`,
     - `FRAMEPARAM(0x02, current - 2) == VERIFY`,
     - `FRAMEPARAM(0x09, current - 2) == APPROVE_GUARANTEE` (the frame's `approved_scope`),
     - `FRAMEPARAM(0x05, current - 2) == 1` (succeeded).
  3. Reads the sender validation frame's status at `current - 1` via `FRAMEPARAM(0x05, current - 1)`.
     - If sender validation **succeeded** (status == 1): the protocol will increment the sender's nonce, so no guarantor nonce bump is needed. The function returns without modifying state.
     - If sender validation **failed** (status == 0): increments `guarantor_nonce[sender]` as fallback replay protection.

  Because `flags`, `target`, `mode`, `gas_limit`, and DEFAULT-frame `data` are all covered by `TXPARAM(0x09)`, the `bump_nonce` frame is bound to the `guarantee` frame by the frame-elide sig hash: an attacker cannot substitute, remove, or mutate it without invalidating the guarantor's signature. Furthermore, because `TXPARAM(0x09)` preserves the sender's `VERIFY` frame data, the sender's signature is also bound — an attacker cannot replace it with garbage to grief the guarantor.

  Keying the nonce per-sender ensures that a failure of one guaranteed transaction invalidates only the slot `(paymaster, tx.sender)`, leaving guaranteed transactions for other senders at the same paymaster unaffected.

  If sender validation fails and the `bump_nonce` frame reverts (e.g., out of gas), the guarantor nonce is not consumed and the same guarantor signature becomes replayable until the paymaster's balance is drained. This is a paymaster liveness concern, not a mempool-admission concern: mempool correctness and spam prevention do not depend on the nonce bump. The canonical paymaster enforces a minimum `gas_limit` on the `bump_nonce` frame in step (1) to make this impossible for a well-signed transaction; the concrete threshold is fixed by the canonical paymaster implementation and must cover a cold-slot `SSTORE`.

In both modes, the paymaster's storage reads and writes are permitted under the canonical paymaster exception; the mempool tracks the affected slots for revalidation.
...
Sender validation in a frame transaction may use arbitrary EVM code, which in the general case cannot be safely executed by public mempool nodes: it may read disallowed state, use banned opcodes, or have mass-invalidation dependencies.  Accounts whose validation logic is not permitted by the public mempool therefore have to resort to private mempools, which have their own downsides.

The `APPROVE_GUARANTEE` scope addresses this by letting an account commit to paying for a transaction unconditionally — even if sender validation subsequently fails. Mempool nodes simulate only the guarantor's `VERIFY` frame and do not need to understand or execute the sender's validation logic. Senders gain the freedom to implement arbitrary validation schemes while remaining eligible for public mempool propagation.  In practice, guarantors could be either accounts that the users themselves own, or third-parties who have a trust relationship with the user.

#### Replay protection for guarantors

When sender validation fails, the sender's nonce is not incremented, so the guarantor must implement its own replay protection mechanism, otherwise its balance can be trivially drained by replaying a guaranteed transaction whose sender validation fails.

The canonical paymaster implements replay protection by tracking its own sender nonces via `guarantor_nonce[sender]`, and by checking that a `bump_nonce` frame follows the sender validation frame. The `bump_nonce` frame increments this mapping only when the preceding sender validation frame failed, avoiding unnecessary state writes in the success case.

The canonical paymaster also protects itself against replays by signing over the frame signature hash instead of the canonical signature hash.  By doing so, it signs over the sender's `VERIFY` frame data.  If it were to instead sign over the canonical signature hash, which elides the sender's validation signature, it would create a griefing vector: an attacker can observe a pending guaranteed transaction, replace the sender's `VERIFY` frame data with garbage, and resubmit. The sender's validation fails, the guarantor still pays (by design), and the attacker has wasted the guarantor's gas without cost.

#### Payer override

A later `APPROVE_PAYMENT` (or `APPROVE_PAYMENT_AND_EXECUTION`) may override a previous `APPROVE_GUARANTEE`. When this happens, the protocol refunds the transaction's maximum cost to the original guarantor and collects it from the new payer.

This enables a "guarantor as admission bond" pattern: a guarantor approves payment to get the transaction into the mempool, but the sender later takes over payment after performing an in-transaction action (e.g., swapping ERC-20 tokens to ETH). If the sender's takeover fails — for example because the swap did not yield enough ETH — the `VERIFY` frame reverts, its state changes (including the payer switch) are discarded, the guarantor remains the payer, and the transaction stays valid.
...
#### Example 5: Guarantor (arbitrary sender validation through the public mempool)

| Frame | Caller      | Target        | Value | Flags                                    | Data                                  | Mode    |
| ----- | ----------- | ------------- | ----- | ---------------------------------------- | ------------------------------------- | ------- |
| 0     | ENTRY_POINT | Guarantor     | 0     | APPROVE_GUARANTEE                        | Guarantor sig                         | VERIFY  |
| 1     | ENTRY_POINT | Null (sender) | 0     | APPROVE_EXECUTION                        | Sender validation data                | VERIFY  |
| 2     | ENTRY_POINT | Guarantor     | 0     | APPROVE_SCOPE_NONE                       | bumpNonce(tx.sender, payer_nonce)     | DEFAULT |
| 3     | Sender      | Target addr   | 0     | APPROVE_SCOPE_NONE                       | Call data                             | SENDER  |

- Frame 0 (`guarantee`): The canonical guarantor checks that frame 2 is a `DEFAULT` call to itself invoking `bumpNonce(tx.sender, payer_nonce)` with a `gas_limit` sufficient to cover a cold-slot `SSTORE`, reads `payer_nonce` from that frame's data, verifies `guarantor_nonce[tx.sender] == payer_nonce`, authenticates its own signer over `TXPARAM(0x09)` (the frame-elide sig hash, which binds the sender's `VERIFY` data and also the `bump_nonce` frame's `payer_nonce`), and calls `APPROVE(APPROVE_GUARANTEE)`. This sets `guarantor_approved = true`, satisfying the transaction validity condition without requiring `sender_approved`.
- Frame 1: Sender validation runs using whatever scheme the sender's account implements — including schemes that would otherwise be disallowed by the public mempool's trace rules. If this frame fails to call `APPROVE`, its state changes are discarded, execution continues, and the guarantor still pays gas for the transaction.
- Frame 2 (`bump_nonce`): The paymaster's `bumpNonce` function verifies, via `FRAMEPARAM`, that frame 0 was a successful self-targeted `guarantee` frame (checking `approved_scope == APPROVE_GUARANTEE`), then checks frame 1's status. If sender validation failed, it increments `guarantor_nonce[tx.sender]` as fallback replay protection. If sender validation succeeded, it is a no-op (the protocol increments the sender's nonce instead).
- Frame 3: The sender's intended call. Executes if sender validation approved; skipped if it did not.

The mempool simulates only Frame 0 (via the canonical paymaster exception), tracking `guarantor_nonce[tx.sender]` at the paymaster and the paymaster's balance. Frame 2's presence and exact shape are enforced by Frame 0's code, and its `target`, `mode`, `gas_limit`, and `data` are all bound by `TXPARAM(0x09)`; the mempool does not need to simulate its execution. Frames 1–3 are outside the validation prefix and do not participate in public mempool admission.

#### Example 6: Guarantor as admission bond (sender takes over payment)
...
| 1     | ENTRY_POINT | Null (sender) | 0     | APPROVE_EXECUTION                        | Sender validation data                | VERIFY  |
| 2     | ENTRY_POINT | Guarantor     | 0     | APPROVE_SCOPE_NONE                       | bumpNonce(tx.sender, payer_nonce)     | DEFAULT |
| 3     | Sender      | DEX           | 0     | APPROVE_SCOPE_NONE                       | swap(SHIB, ETH, amount)              | SENDER  |
| 4     | ENTRY_POINT | Null (sender) | 0     | APPROVE_PAYMENT                          | Sender payment sig                   | VERIFY  |
| 5     | Sender      | Target addr   | 0     | APPROVE_SCOPE_NONE                       | Call data                             | SENDER  |

- Frame 0 (`guarantee`): Same as Example 5. The guarantor approves payment to get the transaction into the mempool.
- Frame 1: Sender validation. Same as Example 5.
- Frame 2 (`bump_nonce`): Same as Example 5. Since sender validation succeeded, this is a no-op.
- Frame 3: The sender swaps ERC-20 tokens to ETH on a DEX, acquiring the ETH needed to pay gas.
- Frame 4 (payer override): The sender calls `APPROVE(APPROVE_PAYMENT)`, triggering a **payer override**. The protocol refunds the transaction's maximum cost to the guarantor and collects it from the sender instead. If this frame fails (e.g., the swap did not yield enough ETH), its state changes are discarded and the guarantor remains the payer.
- Frame 5: The sender's intended call.

This pattern lets the guarantor act as a mempool admission bond rather than the actual gas payer. The guarantor's balance is reserved at mempool admission time, but during execution the sender takes over payment after acquiring ETH. The guarantor is only charged if the sender's takeover fails.
...
It's recommended that to *validate* the transaction, a specific frame structure is enforced and the amount of gas that is expended executing the validation phase must be limited. Once the validation prefix satisfies the [transaction validity condition](#post-execution) — via `APPROVE(APPROVE_PAYMENT)`/`APPROVE(APPROVE_PAYMENT_AND_EXECUTION)` together with sender approval, or via `APPROVE(APPROVE_GUARANTEE)` alone — the transaction can be included in the mempool and propagated to peers safely.
