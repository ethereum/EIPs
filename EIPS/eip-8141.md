...
A frame with `(mode & 0xFF) == 0` is in `DEFAULT` mode.
...
| Mode bit | Meaning                      |
|----------|------------------------------|
| 8        | APPROVE of execution allowed |
| 9        | APPROVE of payment allowed   |

The execution modes are explained in detail below:
...
1. `0x0`: Approval of execution - the sender contract approves future frames calling on its behalf. Only valid when `frame.target` equals `tx.sender`, and when
...
Usabe `scope` operands are constrained by bits 8 and 9 of the `frame.mode`, and using
a non-allowed scope also results in an exceptional halt.

- If `frame.mode>>8 & 0x3 == 1`, then only scope `0x0` can be used.
- If `frame.mode>>8 & 0x3 == 2`, then only scope `0x1` can be used.
- If `frame.mode>>8 & 0x3 == 3`, scope `0x0`, `0x1` or `0x2` can be used.
