...
|    3-5 | `VERIFY_EOA`   | Verify the frame with `ecRecover`, then call `APPROVE` with `scope = mode - 3` |
...
For backward-compatibility with EOAs, `VERIFY_EOA` runs a [default code](#default-code) against `frame.data` which should be a `(v, r, s)` signature.  The default code verifies the signature using `ecRecover`, then calls `APPROVE` with the correct scope based on the specific mode number.

As a result, EOAs can send/sponsor frame transactions and therefore enjoy the benefits of gas abstraction -- sending sponsored transactions, pay gas in ERC20 tokens, etc.
...
In `VERIFY_EOA` mode, the following default code is executed since EOAs have no code:

```
  // EIP-8141 Default Code  (used for VERIFY_EOA mode frames, modes 3–5)
  //
  // Scope is derived from frame.mode: mode 3 → APPROVE(0x0), 4 → APPROVE(0x1), 5 → APPROVE(0x2)
  ...
  //   [0x80] = ecRecover output (recovered address, left-padded to 32 bytes)
      ...
      // Derive APPROVE scope from frame mode: scope = mode - 3
      PUSH1 0x00          // in2
