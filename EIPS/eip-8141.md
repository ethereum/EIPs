...
3. the sender's nonce or, when `tx.signer != 0`, the relevant `AUTH_MANAGER` storage slot for `(sender, signer)`,
4. the sender's code and storage,
5. when a binding VERIFY frame is present, the relevant `AUTH_MANAGER` storage slot for `(target, tx.signer)`,
6. if a deploy frame is present, the code of the factory targeted by that frame (subject to the deploy-frame trace rules below),
7. if a paymaster frame is present, either a canonical paymaster instance together with explicit paymaster balance reservation, or a non-canonical paymaster being used by less than `MAX_PENDING_TXS_USING_NON_CANONICAL_PAYMASTER` pending transactions,
8. if a guarantor frame is present, the canonical paymaster runtime code at the guarantor target plus the guarantor balance reservation,
...
8. Nodes should stop simulation immediately once `payer` has been set or `guarantor_approved` has been observed.

#### Replacement and block invalidation

- **RBF**: matches `(sender, signer, nonce)`.
...
- a `VERIFY` frame in the validation prefix exits without the required `APPROVE` (expiry verifier frames are exempt)
...
When a new canonical block is accepted, the node removes any included frame transactions from the public mempool, updates paymaster reservations accordingly, and identifies the remaining pending transactions whose tracked dependencies were touched by the block. This includes at least transactions for the same sender, transactions whose recorded sender storage slots changed, transactions whose `(sender, signer)` `AUTH_MANAGER` slot changed, and transactions that reference a canonical paymaster (or guarantor) instance whose balance, code, or delayed-withdrawal state changed. The node then re-simulates the validation prefix of only those affected transactions against the new head and evicts any transaction that no longer satisfies the public mempool rules.
...
Guarantors need to commit to a tx shape that includes the data of *another* VERIFY frame (the sender's). `compute_sig_hash` elides every VERIFY frame's data (except expiry verifier frames), so a guarantor signing it cannot bind the sender VERIFY data. `compute_frame_sig_hash(tx, frame_index)` resolves this by eliding only the frame currently executing and preserving every other VERIFY frame's data, so the guarantor's signature covers the sender's VERIFY data without self-referencing the guarantor's own signature bytes.
...
### Deadlines via expiry verifier frame, not envelope

Earlier drafts of this consolidated proposal included a `uint64 expiry` envelope field with a pre-frame consensus check. The upstream EIP-8141 spec subsequently merged a built-in [expiry verifier frame](#expiry-verifier-frame) at `EXPIRY_VERIFIER = address(0x8141)`: a sender-authored deadline carried as the 8-byte `frame.data` of a special `VERIFY` frame, with the timestamp covered by `compute_sig_hash` and checked deterministically by mempool nodes without simulating any other frame. The verifier-frame mechanism subsumes every load-bearing property of the prior envelope-expiry design: consensus-enforced deadline, deterministic ready/expired classification, no future-valid state, and free coverage by the canonical signature hash. Keeping a redundant envelope field would impose its byte cost on every tx that does not use a deadline and would force two parallel deadline paths through the spec. The envelope `expiry` field, its pre-frame check, the `TXPARAM(0x0E)` exposure, and the expired-eviction RBF rules are therefore dropped from this consolidated proposal; deadline use-cases route through the upstream expiry verifier frame.
...
The envelope adds one field (`signer`). RLP decoders MUST be updated; transactions that omit this field are not valid `FRAME_TX_TYPE` payloads. For EOAs that do not register any signer, the legacy account-nonce behavior is preserved by setting `signer = 0`.
