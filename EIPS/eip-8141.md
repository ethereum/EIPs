...
category: Core
created: 2026-01-29
requires: 1559, 2718, 3607, 4844, 7623, 7702, 7951
---
...
The outer payload MUST be an RLP list of exactly nine elements. Each `frame` MUST be an RLP list of exactly six elements. Each `signature` MUST be an RLP list of exactly four elements. Decoders MUST reject a byte string where a list is required, a list where a byte string is required, and any list with a missing or additional element.

The following fields MUST be canonical minimal-length RLP integers: `chain_id`, `nonce`, `mode`, `flags`, `gas_limit`, `value`, `scheme`, `max_priority_fee_per_gas`, `max_fee_per_gas`, and `max_fee_per_blob_gas`. Integer zero MUST be encoded as the empty byte string. Integer encodings MUST NOT contain leading zero bytes. The value bounds below and those inherited from [EIP-1559](./eip-1559.md) and [EIP-4844](./eip-4844.md) still apply.

`sender`, `target`, `data`, `signer`, `msg`, and `signature` MUST be RLP byte strings. An empty `target` decodes to `frame.target = None`; otherwise, `target` MUST be exactly 20 bytes and decodes to that address. `sender` MUST be exactly 20 bytes. `frames`, `signatures`, and `blob_versioned_hashes` MUST be RLP lists. Each item in `blob_versioned_hashes` MUST be a 32-byte RLP byte string whose first byte is `VERSIONED_HASH_VERSION_KZG` as defined by EIP-4844.
...
def compute_sig_hash(tx: FrameTx) -> Hash:
    signing_signatures = [
        [
            ...
            sig.signer,
            sig.msg,
            Bytes() if len(sig.msg) == 0 else sig.signature,
        ]
        for sig in tx.signatures
    ]
    signing_payload = [
    ...
    return keccak(bytes([FRAME_TX_TYPE]) + rlp(signing_payload))
```

`signing_payload` is constructed from the decoded fields and is used only for hashing. Computing `compute_sig_hash` MUST NOT modify the decoded transaction or its encoded bytes.
...
`P256VERIFY(msg, r, s, qx, qy)` returns `true` if and only if the [EIP-7951](./eip-7951.md) `P256VERIFY` precompile returns the 32-byte encoding of `1` for the 160-byte input `msg || r || s || qx || qy`. EIP-7951 defines only the validation rule; the transaction uses the `P256` cost in the table above.
...
For every block `B` where this EIP applies but does not apply to `B`'s parent, clients MUST initialize `EXPIRY_VERIFIER` from `B`'s parent state before processing any transactions in `B`.

If `EXPIRY_VERIFIER` does not exist, clients MUST create it with balance `0`, nonce `1`, the runtime code above, and empty storage. If it exists with empty code and empty storage, clients MUST install the runtime code, set its nonce to `max(existing_nonce, 1)`, preserve its balance, and leave storage empty. If it has non-empty code or non-empty storage in the parent state, the block is invalid.

For every other block, clients MUST NOT run this initialization. Clients MUST apply or undo the transition when a reorganization crosses the activation boundary.
