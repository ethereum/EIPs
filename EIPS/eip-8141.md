...
- When frame-transaction processing begins, `accessed_addresses` is initialized as in [EIP-2929](./eip-2929.md) and [EIP-3651](./eip-3651.md) (`tx.sender`, the coinbase, and precompiles warm), and `accessed_storage_keys` empty (there is no access list). A frame's `resolved_target` is not pre-warmed by being a target — its warm/cold access is charged within the frame's own `gas_limit` — but this never un-warms an address already in the initial set. The payer is added to `accessed_addresses` when an `APPROVE` with payment scope sets it and collects `max_cost`, as for any protocol-touched account. `ENTRY_POINT` is not pre-warmed, so `ORIGIN` is cold in `DEFAULT` and `VERIFY` frames (where it is `ENTRY_POINT`) and warm in a `SENDER` frame (where it is `tx.sender`).
