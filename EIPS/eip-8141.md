...
The following fields MUST be canonical minimal-length RLP integers: `chain_id`, `nonce`, `mode`, `flags`, `gas_limit`, `value`, `scheme`, `max_priority_fee_per_gas`, `max_fee_per_gas`, and `max_fee_per_blob_gas`. Integer zero MUST be encoded as the empty byte string. Integer encodings MUST NOT contain leading zero bytes.

`sender`, `target`, `data`, `signer`, `msg`, and `signature` MUST be RLP byte strings. `target` MUST be either the empty byte string, representing no explicit target, or exactly 20 bytes. `sender` MUST be exactly 20 bytes. `frames`, `signatures`, and `blob_versioned_hashes` MUST be RLP lists. Each item in `blob_versioned_hashes` MUST be a 32-byte string satisfying [EIP-4844](./eip-4844.md).
...
`copy(tx)` creates a signing projection and MUST NOT modify the decoded transaction or its raw bytes.
...
`P256VERIFY` is the exact input-validation and signature-verification relation specified by [EIP-7951](./eip-7951.md). It returns `true` only when EIP-7951 would return the 32-byte value `1` for `msg || r || s || qx || qy`. This is protocol-level signature validation; the transaction is charged the `P256` cost in the signature table above rather than the EVM precompile call cost.
...
At the first block where this EIP is active, and before any transaction in that block executes, clients MUST initialize `EXPIRY_VERIFIER` against the block's parent state.
