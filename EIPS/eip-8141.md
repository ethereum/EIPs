...
| 0x1        | `P256`       | `r || s || qx || qy` (32 bytes each)           | 6700     |
...
The canonical signature hash is defined by eliding all raw `signature` bytes:
...
The raw `signature` bytes are elided from the canonical signature hash. This is done for three reasons:

1. A signature over `compute_sig_hash(tx)` cannot commit to its own raw bytes.
1. Signatures with explicit 32-byte `msg` values do not induce this circularity, but by commiting to `msg` and `signer`, we indirectly commit to `signature` as well (otherwise entire tx is invalid).
