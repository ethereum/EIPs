...
| `MAX_PENDING_TXS_PER_STABLE_ACCOUNT`   | `16`  | Maximum amount of pending transactions admitted from a single validation-stable sender |
...
The generic per-sender pending-transaction limit exists because a single change to a sender's validation inputs can invalidate every pending transaction from that sender at once. When a validation prefix depends only on inputs that nothing other than the sender's own nonce can move, that concern does not apply and the limit can be relaxed.

A node simulating a validation prefix under the trace rules below already learns its state dependency set. Call a sender **validation-stable** for a pending transaction when, for that sender, the set contains only its code hash and nonce and no storage slot, and the prefix reaches no external mutable state. For such a sender only nonce consumption can invalidate the pending transaction, so a node MAY cache the sender's validation state and admit up to `MAX_PENDING_TXS_PER_STABLE_ACCOUNT` pending transactions from it in place of the generic per-sender limit.

This is the frames-native form of an account lock: stability is read directly from the prefix's dependency set rather than declared by a separate on-chain flag, so any account whose validation is a fixed code path over its signature -- the default-code sender or a wallet that reads no mutable storage while validating -- qualifies with no explicit opt-in.
