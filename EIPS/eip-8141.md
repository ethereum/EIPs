...
A frame transaction is charged nothing until one of its frames approves payment, so a validation prefix that does not approve consumes its validation work with no fee every time a block producer offers it. A block producer SHOULD bound the unpaid validation work it spends on any one pending frame transaction: after a bounded number of consecutive block-building attempts in which the transaction's validation prefix fails to approve, the producer MAY evict it, treating it as invalid against the current head and releasing its payer reservation or exposure as for any other eviction. The bound is a local producer policy and is not consensus. A build-time failure to approve MAY be transient rather than permanent, for example a payer funded by an earlier transaction in the same block, a prefix reading state that a preceding transaction changes, or a referenced recent root not yet committed at the moment of the attempt, so the bound SHOULD permit more than one attempt before eviction. As with any eviction, re-admission requires receiving the transaction again, which lets the prefix become admissible once the state it reads has changed.
