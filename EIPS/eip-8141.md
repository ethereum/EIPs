...
```python
tx_unused_gas = sum(
    (frame.limits.execution - frame_receipt.execution_gas_used)
    + (frame.limits.state - frame_receipt.state_gas_used)
...
Wallets and gas estimators must produce two-dimensional, per-frame gas estimates: each frame's `limits.execution` covers its execution gas and its `limits.state` covers its state gas, and neither budget can borrow from the other or from other frames. Under-provisioned state gas halts a frame exactly like out-of-gas; over-provisioned budgets are refunded at settlement but raise the up-front `max_cost` collected from the payer.
