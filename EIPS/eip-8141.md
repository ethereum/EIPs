...
- `execution_gas_limit` -- the maximum execution gas allowed to be expended in pursuit of the frame.
...
1. Execute a call with the specified `mode`, `flags`, `target`, `execution_gas_limit`, `state_gas_limit`, `value`, and `data`.
    - Initialize the frame's receipt with `state_gas_used = 0`.
    - At frame entry, initialize the frame's gas pools per [Gas Accounting](#gas-accounting): `gas_left = frame.execution_gas_limit` and `state_gas_left = frame.state_gas_limit`.
    ...
    - In the top-level frame call, `CALLVALUE` is `frame.value`.
        ...
        - If `frame.value > 0` and `resolved_target` does not exist, `STATE_BYTES_PER_NEW_ACCOUNT * CPSB` state gas is charged after the balance check and before the frame's code executes, as is [EIP-2780](./eip-2780.md). If `state_gas_left` cannot cover the charge, the frame halts exceptionally.
...
- State-gas ownership records and changes to frame receipts' `state_gas_used` fields journaled alongside the state changes that produced them. Reverting a call, frame, or atomic batch restores all of them to the corresponding checkpoint.
- Discard the `TSTORE` and `TLOAD` transient storage between frames.

##### Gas Accounting

Frame transactions adopt the two-dimensional gas model of [EIP-8037](./eip-8037.md): **execution gas** meters computation, data, and account access, while **state gas** meters durable state growth. Where EIP-8037 retrofits the split onto envelopes carrying a single gas field via the reservoir model, the frame transaction declares both budgets explicitly: `frame.execution_gas_limit` bounds a frame's execution gas and `frame.state_gas_limit` bounds its state gas. The reservoir model — including the `state_gas_reservoir` split and the `state_gas_from_gas_left` accounting — does not apply within frame transactions.
...
```python
gas_left = frame.execution_gas_limit
...
- If `owner != current_frame_index`, reduce the owner frame's `state_gas_used` to return the correct amount during transaction settlement.
...
| 0x01    | frameIndex   | `execution_gas_limit`                                               |
...
| 0x0A    | frameIndex   | `execution_gas_used` (exceptional halt if current/future) |
...
- The `execution_gas_used` (0x0A) and `state_gas_used` (0x0B) fields return the current values recorded in the frame's receipt. Like `status`, they are defined only for frames that have completed; accessing them for the current frame or a subsequent frame results in an exceptional halt. A completed frame's `state_gas_used` may subsequently decrease when another frame refills state gas attributed to it, or be restored if that later change is rolled back. `execution_gas_used` does not change after frame completion.
- Undefined `param` values result in an exceptional halt.
...
Wallets and gas estimators must produce two-dimensional, per-frame gas estimates: each frame's `execution_gas_limit` covers its execution gas and its `state_gas_limit` covers its state gas, and neither budget can borrow from the other or from other frames. Under-provisioned state gas halts a frame exactly like out-of-gas; over-provisioned budgets are refunded at settlement but raise the up-front `max_cost` collected from the payer.
