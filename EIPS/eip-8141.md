...
category: Core
created: 2026-01-29
requires: 1559, 2718, 4844, 7623, 7702
---
...
2. If frame has mode `VERIFY` the following additional requirements are imposed:
    - Execute the frame as a `STATICCALL`, disallowing state manipulation.
      Only `APPROVE` can modify the state or transaction context in
      `VERIFY`.
...
3. If frame has mode `SENDER` and it fails, unroll the atomic batch.
    ...
    - Since skipped frames are not executed, the gas value allotted to them
      is refunded at the end of the transaction.
