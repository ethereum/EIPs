...
One decidable form is a whole-code scan over instruction positions (a linear pass that skips `PUSH` immediates, so a byte inside a `PUSH` datum is never read as an opcode). The target is egress-sealed if no instruction position holds a value-egress or code-mutation opcode: `CALL`, `CALLCODE`, `DELEGATECALL`, `CREATE`, `CREATE2`, `SELFDESTRUCT`, or any operation that re-points the account's executable code. `STATICCALL` is permitted, since it cannot move value. `APPROVE` is not an egress instruction: its `offset`/`length` operands designate return data only, and the protocol collects at most the reserved `max_cost` through `APPROVE_PAYMENT` regardless of the approval scope, so no scan of the `APPROVE` operands is required. A whole-code scan is preferred over a per-transaction trace check because it is a pure function of the runtime code, sound regardless of control flow, and cacheable once per code hash alongside the existing jump-destination analysis. A sealed target's ether can then leave only through collections the node has already reserved, so `pending_withdrawal_amount` is zero by construction.
...
Open items for discussion. First, the exact opcode set for "re-points the account's executable code" under the active fork's account model, which should be pinned against the fork's opcode table before this is normative. Second, the gas budget: the proof machinery itself is inexpensive under the fixed gas schedule (a signature check at 3,000, a keccak Merkle path on the order of a few thousand for a realistic depth, and a handful of field comparisons, so roughly single-digit thousands against the 100,000 `MAX_VERIFY_GAS`), which means the open question is narrower than "does the proof fit". It is the gas drawn by the recent-root verification frame itself, which shares the same budget and is not yet measured, and whether proof calldata is billed against `MAX_VERIFY_GAS` or only at the transaction intrinsic level. None of these is measured end-to-end, since no client implements the sealed pay frame yet.
