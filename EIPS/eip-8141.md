...
1. Execute a call with the specified `mode`, `target`, `gas_limit`, `data`, and `value`.
   ...
   - If mode is `VERIFY` and `frame.target` has no code:
       - Read `frame.data` as `(scope, v, r, s)`.
       - Verify that `frame.target == ecrecover(hash, v, r, s)`, where `hash` is `keccak(sighash, scope)`.
...
- It is implied that for a EOA to send a frame transaction, it needs to format `frame.data` as `(scope, v, r, s)`, where `scope` is the intended approval scope and `(v, r, s)` is the signature over `(sighash, scope)`, where `sighash` is the canonical signature hash of the frame transaction.  It's necessary to hash over the `scope` because `scope` is part of `frame.data` which is elided from the signature hash itself.
