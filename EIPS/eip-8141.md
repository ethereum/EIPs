...
And there are three "precompile modes":

| Mode | Name           | Summary                                                   |
| ---- | -------------- | --------------------------------------------------------- |
|    3 | `PRECOMPILE_VERIFY_SENDER`      | Verify the `sender` using a precompile                            |
|    4 | `PRECOMPILE_VERIFY_PAYER`       | Verify the `payer` using a precompile                |
|    5 | `PRECOMPILE_VERIFY_BOTH`       | Verify both the `sender` and the `payer` using a precompile                                 |
...
Just like the `VERIFY` mode, the precompile modes are validation frames that verify that a sender and/or payer authorized the transaction, but they do so using signature precompiles rather than code.

Just like `VERIFY`, frames in precompile modes will have their data elided from signature hash calculation and from introspection by other frames.

The three precompile modes differ only in what happens when the frame successfully executes:

- `PRECOMPILE_VERIFY_SENDER`: `sender_approved` is set to `true`.
- `PRECOMPILE_VERIFY_PAYER`: `payer_approved` is set to `true`.
- `PRECOMPILE_VERIFY_BOTH`: both `sender_approved` and `payer_approved` are set to `true`.

In precompile modes, `frame.target` must be equal to one of the following supported precompiles.  Other signature precompiles may be added to this EIP.

##### ecRecover

If `target` is the `ecRecover` precompile, `data` is interpreted as `(v, r, s)`, and `ecRecover` is invoked with `(hash, v, r, s)` where `hash` is the hash of the frame transaction.

If the returned `publicAddress` is equal to `tx.sender`, the verification is successful.  Otherwise the frame reverts.

##### P256VERIFY

If `target` is the `P256VERIFY` precompile, `data` is interpreted as `(r, s, qx, qy)` and `P256VERIFY` is invoked with `(hash, r, s, qx, qy)` where `hash` is the hash of the frame transaction.

The verification is successful if:

- The returned `success` value is `1`.
- `tx.sender == keccak(qx|qy)[12:]`
    - In other words, we verify that the sender address is equal to the last 20 bytes of the `keccak` hash of the P256 public key.

Otherwise the frame reverts.
...
Precompile modes serve two purposes:
...
| Frame | Caller      | Target        | Data                   | Mode    |
| ----- | ----------- | ------------- | ---------------------- | ------- |
| 0     | ENTRY_POINT | Null (sender) | Signature (r, s, qx, qy) | PRECOMPILE_VERIFY_SENDER  |
| 1     | ENTRY_POINT | Sponsor       | Signature (v, r, s) | PRECOMPILE_VERIFY_PAYER  |
