...
- When frame-transaction processing begins, `accessed_addresses` is initialized exactly as in [EIP-2929](./eip-2929.md) and [EIP-3651](./eip-3651.md): `tx.sender`, the coinbase, and all precompiles are warm. `accessed_storage_keys` is initialized empty, as a frame transaction carries no access list. Being a frame target does not by itself warm an address — a frame's `resolved_target` is charged for warm / cold access within its own `gas_limit` under the normal EVM accounting — and this rule never un-warms an address already in the initial set, so a frame that targets `tx.sender`, the coinbase, or a precompile still sees it warm. When a frame sets the payer via `APPROVE(APPROVE_PAYMENT)` or `APPROVE(APPROVE_EXECUTION_AND_PAYMENT)`, the protocol touches the payer to collect `max_cost`, so the payer is added to `accessed_addresses` (warm) from that point, consistent with EIP-2929's treatment of protocol-touched accounts and with the `tx.sender` rule. `ENTRY_POINT` is deliberately not pre-warmed; consequently the `ORIGIN` value seen by `DEFAULT` and `VERIFY` frames (which is `ENTRY_POINT`) starts cold, a departure from EIP-2929, which warms the origin because the protocol touched it. In a `SENDER` frame `ORIGIN` is `tx.sender`, which is warm from initialization.
