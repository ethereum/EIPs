...
- When frame-transaction processing begins, `accessed_addresses` is initialized as in [EIP-2929](./eip-2929.md) and [EIP-3651](./eip-3651.md) — `tx.sender`, the coinbase, and all precompiles — except that no frame target is pre-warmed; each frame's `resolved_target` warm / cold access is charged within its own `gas_limit`, and `ENTRY_POINT` is not pre-warmed.
