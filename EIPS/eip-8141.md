...
The delay period in item 1(b) MUST be at least as long as the deepest reorg the node's public mempool rules protect against. The reservation and revalidation rules keep the mempool safe while a withdrawal is pending, because `pending_withdrawal_amount` lowers `available_paymaster_balance` as soon as the withdrawal is queued and the affected pending transactions are revalidated. They do not cover a withdrawal that has already matured and paid out: if the delay is shorter than the reorg depth, that payout is not reorg-stable, so the on-chain balance a node reserved against can change under a reorg after the reservation was released. Pinning the delay at or above the reorg depth keeps a matured withdrawal irreversible relative to the reservation accounting, so a reserved balance stays present when the transactions it backs are included.
