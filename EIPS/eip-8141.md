...
Every public-mempool node MUST admit and propagate a frame transaction whose validation prefix satisfies the bounds in this section evaluated against `MAX_VERIFY_GAS`. This floor is the propagation and inclusion-list eligibility guarantee: a transaction at or below it reaches inclusion-list builders regardless of which node it first reaches. `MAX_VERIFY_GAS` SHOULD be set from the weakest node profile the network intends to keep in that inclusion-list set. The guarantee bounds only the verification work a node must be willing to expend at ingress; it does not exempt the transaction from the mempool's other admission checks, including nonce ordering, fee, balance, and intrinsic validity.

A node MAY additionally admit transactions whose validation work exceeds the floor when its own capacity allows, as local headroom. Such headroom only widens what an individual node accepts; it never lowers the floor for other nodes and never affects block validity. A transaction above the floor stays valid and still propagates, but only across the subset of nodes whose headroom accepts it, so it diffuses more slowly rather than being dropped from the network. Because gas is an imperfect proxy for the real cost of validation at ingress — signature checks parallelize across cores, cold state reads block rather than spin, and succinct-proof verifiers scale with their public inputs — the headroom above the floor is a per-node policy informed by each client's measured worst-case validation cost, not a second protocol constant.

Because acceptance above the floor varies per node, a node MAY advertise a transaction's validation-prefix gas alongside its hash when announcing it to peers, so that a receiving node can skip requesting a transaction whose validation work exceeds its own headroom instead of fetching and then dropping it. This advertisement is an optimization for above-floor transactions only: it never gates block validity, and a transaction at or below the floor MUST be admitted and propagated regardless of whether the announcement carries this value.
