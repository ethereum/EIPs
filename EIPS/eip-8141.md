...
| Mode | Name           | Summary                                                   |
| ---- | -------------- | --------------------------------------------------------- |
|  0-2 | `VERIFY_EOA`   | Verify the frame with `ecRecover`, then call `APPROVE` with `scope = mode` |
|    3 | `DEFAULT`      | Execute frame as `ENTRY_POINT`                            |
|    4 | `VERIFY`       | Frame identifies as transaction validation                |
|    5 | `SENDER`       | Execute frame as `sender`                                 |
...
For backward-compatibility with EOAs, `VERIFY_EOA` runs a [default code](#default-code) against `frame.data` which should be a `(v, r, s)` signature.  The default code verifies the signature using `ecRecover`, then calls `APPROVE` with scope corresponding to the mode number.

Note that mode 0 to 2 are all considered `VERIFY_EOA`; they differ only in the `scope` that `APPROVE` will be called with, where `scope = mode`.

With the `VERIFY_EOA` mode, EOAs can send/sponsor frame transactions and therefore enjoy the benefits of gas abstraction -- sending sponsored transactions, pay gas in ERC20 tokens, etc.
...
1. Execute a call with the specified `mode`, `target`, `gas_limit`, `data`, and `value`.
   ...
   - If mode is `VERIFY_EOA`:
       - If the `code` of `tx.sender` is not empty, revert.
       - Execute the [default code](#default-code).
...
In `VERIFY_EOA` mode, the following default code is executed for the EOA.  The code verifies that `frame.data` is a valid secp256k1 signature of the transaction payload, then calls `APPROVE` with the `scope` corresponding to the `mode` number, where `scope = mode`.

```
  // EIP-8141 Default Code  (VERIFY_EOA modes 0–2, where scope == mode)
  //
  // frame.target must be null (→ tx.sender).
  // Calldata (frame.data): v(32) | r(32) | s(32)
  //
  // Memory layout:
  //   [0x00] = sig_hash
  //   [0x20] = v
  //   [0x40] = r
  //   [0x60] = s
  //   [0x80] = ecRecover output (recovered address, left-padded to 32 bytes)

      // Load APPROVE scope directly from frame.mode (scope == mode)
      PUSH1 0x00          // in2
      PUSH1 0x0a          // in1: currently executing frame index
      TXPARAMLOAD         // [frame_idx]
      PUSH1 0x0f          // in1: mode of frame at in2=frame_idx
      TXPARAMLOAD         // [scope]

      // Load sig_hash and write to mem[0x00]
      PUSH1 0x00          // in2
      PUSH1 0x08          // in1: compute_sig_hash(tx)
      TXPARAMLOAD         // [hash, scope]
      PUSH1 0x00
      MSTORE              // mem[0x00] = hash

      // Copy v | r | s from calldata into mem[0x20..0x7f]
      PUSH1 0x00
      CALLDATALOAD
      PUSH1 0x20
      MSTORE              // mem[0x20] = v
      ...
      // STATICCALL ecRecover(hash, v, r, s): 128-byte input, 32-byte output
      // An invalid signature returns the zero address rather than reverting
      ...
      GAS
      STATICCALL          // [ok, scope]
      POP                 // success is always 1; check the returned address instead

      // Revert if recovered address does not match tx.sender
      PUSH1 0x80
      ...
      PUSH1 0x00          // signature mismatch — revert
      PUSH1 0x00
      REVERT

  approve:
      JUMPDEST            // [scope]
      // APPROVE (0xaa): top=offset, top-1=length, top-2=scope
      PUSH1 0x00          // length = 0
      PUSH1 0x00          // offset = 0
      APPROVE             // (0xaa) — exits frame with the requested approval scope
```
...
With frame transactions, EOA wallets today can reap the key benefit of AA -- gas abstraction, including sending sponsored transactions, paying gas in ERC20 tokens, and more.
...
#### Example 3: EOA paying gas in ERC20s

| Frame | Caller      | Target        | Data                   | Mode    |
| ----- | ----------- | ------------- | ---------------------- | ------- |
| 0     | ENTRY_POINT | Null(sender)  | Signature (v, r, s)    | VERIFY_EOA(mode=3) |
| 1     | ENTRY_POINT | Sponsor       | Sponsor data           | VERIFY  |
| 2     | Sender      | ERC-20        | transfer(Sponsor,fees) | SENDER  |
| 3     | Sender      | Target addr   | Call data              | SENDER  |

- Frame 0: Verify the sender with a EOA signature.  The default code will call `APPROVE(0x0)` to authorize execution.
- Frame 1: Checks that the user has enough ERC-20 tokens, and that the next frame is an ERC-20 send of the right size to the sponsor. Calls `APPROVE(0x1)` to authorize payment.
- Frame 2: Sends tokens to sponsor.
- Frame 3: User's intended call.
