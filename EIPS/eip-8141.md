...
Before processing transactions in the first block where this EIP is active, clients MUST initialize `EXPIRY_VERIFIER` from the block's parent state.
...
For all later blocks, clients MUST NOT run this initialization. Clients MUST apply or undo the transition when a reorganization crosses the activation boundary.
