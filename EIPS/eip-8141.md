...
#### Validation-Stable Accounts

The generic single-pending-transaction-per-sender limit exists because a single change to a sender's validation inputs can invalidate every pending transaction from that sender at once. When a validation prefix depends only on inputs that nothing other than the sender's own nonce can move, that concern does not apply and the limit can be relaxed without introducing a new bound.

A node simulating a validation prefix under the trace rules below already learns its state dependency set. Call a sender **validation-stable** for a pending transaction when, for that sender, the set contains only its code hash and nonce and no storage slot, and the prefix reaches no external mutable state. For such a sender only nonce consumption can invalidate the pending transaction, so a node MAY admit more than one pending transaction from it, bounding their number by the per-payer exposure reservation that the mempool rules already require of every payer rather than by a fixed count: the summed maximum cost of the sender's pending transactions must not exceed the payer's balance. A self-paying validation-stable sender can therefore keep as many pending transactions as its balance funds, and no more.

This reuses the reservation accounting a node already maintains for every payer, so it adds no new constant. Stability is read directly from the prefix's dependency set rather than declared by a separate on-chain flag, so any account whose validation is a fixed code path over its signature -- the default-code sender or a wallet that reads no mutable storage while validating -- qualifies with no explicit opt-in.
