...
| Mode | Name           | Summary                                                   |
| ---- | -------------- | --------------------------------------------------------- |
|    3 | `VERIFY_SENDER_PRECOMPILE`      | Verify the `sender` using a precompile                            |
|    4 | `VERIFY_PAYER_PRECOMPILE`       | Verify the `payer` using a precompile                |
|    5 | `VERIFY_SENDER_AND_PAYER_PRECOMPILE`       | Verify both the `sender` and the `payer` using a precompile                                 |
...
To make 8141 backwards-compatible with EOAs, the precompile modes behave like `VERIFY` except that instead of executing arbitrary EVM code, it verifies the frame using a signature precompile such as `ecRecover`.

The specific precompile to use is identified by the `target`, and input to the precompile is [the hash of the transaction](#signature-hash) plus additional data specified in `data`.  Upon a successful return, `sender_approved` or `payer_approved` or both are set to true, depending on the specific precompile mode.

With the `ecRecover` precompile, regular EOAs can send and sponsor frame transactions.  With a non-ecRecover precompile, the sender effectively becomes a "non-secp256k1 EOA."  For example, with the `P256VERIFY` precompile, the user may control a EOA (an account without code) with a passkey and send/sponsor transactions with it using frame transactions.

The following list defines how frame transactions interact with each supported precompile.  As Ethereum implements more signature precompiles, their behaviors with respect to frame transactions SHOULD be specified in the following list or in another EIP.
...
If the returned `publicAddress` is equal to `sender`, the verification is successful.
...
If the returned `success` value is 1 AND `sender` is `keccak(qx|qy)[12:]`, the verification is successful.
