...
category: Core
created: 2026-01-29
requires: 1559, 2718, 2780, 3529, 3607, 4844, 7594, 7623, 7702, 7708, 7825, 8037, 8038
---
...
frames = [[mode, flags, target, gas_limit, state_gas_limit, value, data], ...]
...
- `gas_limit` -- the maximum execution gas allowed to be expended in pursuit of the frame.
- `state_gas_limit` -- the maximum state gas ([EIP-8037](./eip-8037.md)) allowed to be expended in pursuit of the frame.
...
for i, frame in enumerate(tx.frames):
    ...
    total_frame_execution_gas += frame.gas_limit
    total_frame_gas += frame.gas_limit + frame.state_gas_limit
...
# Intrinsic gas and the sum of all frame execution-gas limits must fit the
# EIP-7825 transaction cap. State-gas limits are excluded from the cap.
# frame_tx_intrinsic_gas and calldata_floor_gas are defined in Gas Accounting.
assert max(
...
1. Initialize transaction-scoped variables:
    ...
    - an empty state-gas journal, including no outstanding storage-charge owners (see [Gas Accounting](#gas-accounting))

Then for each frame:

1. Execute a call with the specified `mode`, `flags`, `target`, `gas_limit`, `state_gas_limit`, `value`, and `data`.
    - Initialize the frame's receipt with `state_gas_used = 0`.
    - At frame entry, initialize the frame's gas pools per [Gas Accounting](#gas-accounting): `gas_left = frame.gas_limit` and `state_gas_left = frame.state_gas_limit`.
    ...
    - In the top-level frame call, `CALLVALUE` is `frame.value`.
        ...
        - If `frame.value > 0` and the resolved target does not exist, per the existence rule of [EIP-8037](./eip-8037.md), `STATE_BYTES_PER_NEW_ACCOUNT * CPSB` state gas is charged after the balance check and before the frame's code executes, mirroring [EIP-2780](./eip-2780.md)'s runtime account-creation charge. If `state_gas_left` cannot cover the charge, the frame halts exceptionally; the transaction remains valid and is included.
        - Once any required account-creation state-gas charge succeeds, a non-zero value transfer to an address other than `tx.sender` emits the transfer log specified by [EIP-7708](./eip-7708.md), with `tx.sender` as `from`, `resolved_target` as `to`, and `frame.value` as the amount. The log is emitted before any logs produced by the target's code and is included in this frame's receipt. It is discarded if the frame reverts or halts exceptionally, or if an atomic batch containing the frame is unrolled.
...
1. If a frame is part of an atomic batch and it fails, unroll the associated atomic batch.
    ...
    - Logs emitted by frames that executed before the failure are discarded together with their state changes when the batch is unrolled. Journaled changes to frame receipts' `state_gas_used` fields are reversed to their values at the start of the batch. This may also restore `state_gas_used` in a frame before the batch if a frame within the batch had refilled state gas attributed to it. Executed frame receipts retain their execution status and execution gas used, with empty logs and the state gas resulting from the rollback.
...
- The *original value* used by `SSTORE` metering, in both gas dimensions, is the slot value at the start of the transaction, as for other transaction types.
- State-gas ownership records and changes to frame receipts' `state_gas_used` fields are transaction-scoped and journaled alongside the state changes that produced them. Reverting a call, frame, or atomic batch restores all of them to the corresponding checkpoint, exactly as changes to the `refund_counter` are restored.
- Discard the `TSTORE` and `TLOAD` transient storage between frames.

##### Gas Accounting

Frame transactions adopt the two-dimensional gas model of [EIP-8037](./eip-8037.md): **execution gas** meters computation, data, and account access, while **state gas** meters durable state growth. Where EIP-8037 retrofits the split onto envelopes carrying a single gas field via the reservoir model, the frame transaction declares both budgets explicitly: `frame.gas_limit` bounds a frame's execution gas and `frame.state_gas_limit` bounds its state gas. The reservoir model — including the `state_gas_reservoir` split and the `state_gas_from_gas_left` accounting — does not apply within frame transactions.
...
def value_cost(frame):
    # Self-transfers pay no value cost, per EIP-2780. A None target resolves
    # to the sender and is therefore also exempt.
...
`frame_tx_intrinsic_gas` is the transaction's intrinsic cost in the sense of [EIP-2780](./eip-2780.md): a sum of primitives computable from the transaction fields alone, with no state access. It is charged entirely in the execution dimension and, together with the sum of the declared frame execution-gas limits, participates in the [EIP-7825](./eip-7825.md) validity check in [Constraints](#constraints). `value_transfer_cost` charges `TX_VALUE_COST` for each value-bearing frame, covering the recipient balance write and transfer log exactly as EIP-2780 prices top-level value transfers. State-dependent costs do not enter this static gas-validity calculation and are charged at runtime inside frame state-gas budgets, as described below.
...
state_gas_left = frame.state_gas_limit
```

The pools are hard-separated:

- Execution-gas charges draw from `gas_left` only, under unchanged EVM semantics: `gas_left` is call-frame-local, forwarded to child calls by the 63/64 rule, returned on success or revert, and consumed on exceptional halt. The `GAS` instruction returns the current call frame's `gas_left` and never observes `state_gas_left`. The `GAS_CALL_STIPEND` pre-state check of EIP-8037 applies to `gas_left` only.
- State-gas charges draw from `state_gas_left` only. `state_gas_left` is frame-scoped: EVM call frames at any depth within the frame draw from it directly; it is not forwarded, split, or divided by the 63/64 rule.
- A charge exceeding its remaining pool is an exceptional halt of the current call frame, with ordinary out-of-gas semantics. Execution gas can never be spent on state charges, and state gas can never be spent on execution charges.

Charge points follow EIP-8037 unchanged: `SSTORE` state gas at the end of the opcode, the conditional account-creation charge of the `CALL*` and `CREATE`/`CREATE2` families in the creating frame, and code-deposit gas at deposit time. In addition, the frame-entry account-creation charge for value-bearing frames ([Behavior](#behavior)) and the account-creation charge applied by [`APPROVE`](#approve-instruction-0xaa) mirror EIP-2780's runtime account-creation charges. A charge that exhausts either pool fails the frame rather than changing the transaction's static validity; as specified above, failure of a `VERIFY` frame nevertheless makes the transaction invalid.
...
frame_receipts[current_frame_index].state_gas_used += amount
```
...
frame_receipts[owner].state_gas_used -= amount
```

The destination of the spendable refill remains frame-isolated:

- If `owner == current_frame_index`, also credit the amount to the current frame's `state_gas_left`. A frame's spendable refills can never exceed its own outstanding charges, so `state_gas_left` never exceeds `frame.state_gas_limit`.
- If `owner != current_frame_index`, do not credit any frame's `state_gas_left`. The earlier frame's pool is closed, and the currently executing frame cannot gain state-gas budget by clearing state another frame paid for. Reducing the owner's `state_gas_used` returns the amount to the payer during transaction settlement.

In both cases the storage slot's outstanding-charge ownership entry is cleared, so a subsequent re-creation is charged and attributed to whichever frame performs it.
...
EIP-8037 assigns no state-gas refill to `SELFDESTRUCT`, including when it deletes a contract created earlier in the transaction. Such an operation therefore does not modify the creating frame's `state_gas_used`.
...
```python
frame_receipt.gas_used = frame.gas_limit - gas_left
```
...
Blocks account for the two dimensions separately, per EIP-8037. A frame transaction contributes:

```python
tx_state_gas = sum(fr.state_gas_used for fr in frame_receipts)
...
All intrinsic costs are execution gas, so the execution dimension is the transaction total less its net state gas. The block header `gas_used`, the block validity condition, and the base fee update rule follow EIP-8037 unchanged.
...
execution_reservation = max(
    frame_tx_intrinsic_gas + sum(frame.gas_limit for frame in tx.frames),
    calldata_floor_gas,
)
...
| 0x09    | frameIndex   | `state_gas_limit`                                         |
| 0x0A    | frameIndex   | `gas_used` (exceptional halt if current/future)           |
| 0x0B    | frameIndex   | `state_gas_used` (exceptional halt if current/future)     |

Notes:

- The `status` field (0x05) returns `0` for failure, `1` for success, or `2` for a frame skipped due to a failed atomic batch.
- The `gas_used` (0x0A) and `state_gas_used` (0x0B) fields return the current values recorded in the frame's receipt. Like `status`, they are defined only for frames that have completed; accessing them for the current frame or a subsequent frame results in an exceptional halt. A completed frame's `state_gas_used` may subsequently decrease when another frame refills state gas attributed to it, or be restored if that later change is rolled back. `gas_used` does not change after frame completion.
- Undefined `param` values result in an exceptional halt.
...
```
receipt = [tx-type, cumulative-gas, payer, [[status, gas-used, state-gas-used, logs], ...]]
```
...
[EIP-8037](./eip-8037.md) introduces separate metering for execution gas and state gas, but retrofits it onto transaction envelopes that carry a single gas field: the split is derived at runtime by capping `gas_left` at the [EIP-7825](./eip-7825.md) limit and treating the excess as a `state_gas_reservoir`, with state charges falling back to `gas_left` when the reservoir is empty. The reservoir is a compatibility mechanism, and its costs follow from the single-field constraint: the split is invisible below the EIP-7825 cap, per-dimension usage cannot be bounded by the sender, and block inclusion must reserve a transaction's full gas in both dimensions, because either could end up consuming it.

The frame transaction is a new envelope, so it declares the split instead: each frame carries an explicit `state_gas_limit` beside its `gas_limit`, and the two pools are hard-separated. This removes the reservoir and its `state_gas_from_gas_left` accounting from frame transactions entirely, makes the EIP-7825 cap a static transaction-level validity rule over intrinsic gas plus the sum of all frame execution-gas limits, gives the mempool fully static per-dimension bounds on the validation prefix, and makes block-level reservations exact in each dimension. State gas remains outside the EIP-7825 cap, so a frame whose `state_gas_limit` is tens of millions of gas — such as a deploy frame carrying state gas for a large code deposit — can express directly what the reservoir model could express only by overflow.

The budgets are frame-scoped rather than transaction-scoped for the same reason unused execution gas does not roll over between frames: frames belong to mutually distrusting parties, and each frame's resources must be predictable regardless of what earlier frames did. A transaction-scoped state pool would let a user operation drain the state gas a paymaster's `post_op` frame requires — the class of problem [ERC-4337](./eip-4337.md) solves by having the `EntryPoint` reserve `postOp` gas, solved here by consensus. It would also leave the mempool unable to statically bound the state gas consumable by the validation prefix, since the whole pool would be legally available to a `deploy` frame.

Because `GAS` returns only execution `gas_left`, contracts cannot observe state-gas consumption through `gasleft()` deltas — the attribution concern EIP-8037 raises for bundlers. Frame transactions instead expose the state dimension directly: `state_gas_limit` of any frame and `state_gas_used` of completed frames via `FRAMEPARAM`, and the current frame's remaining state gas via `TXPARAM`. A paymaster can therefore verify, before approving payment, that every frame it depends on carries the state budget it needs, and can attribute actual state-gas usage per frame afterwards.

### Cross-frame state-gas refills

EIP-8037's refill conditions are transaction-scoped — a slot's *original value* is its value at transaction start — while frame budgets are frame-scoped. The only EIP-8037 refill that a later successfully completing frame can trigger for a charge paid by an earlier frame is an `SSTORE` clearing a slot that the earlier frame created. A contract created by an earlier frame can be deleted by a later `SELFDESTRUCT` under EIP-6780, but EIP-8037 explicitly assigns no state-gas refill to `SELFDESTRUCT`. Account-creation and code-deposit charges can otherwise be undone only by rolling back the state changes that produced them, in which case the state-gas journal restores the creating frame's receipt.

Routing such refills into the executing frame's `state_gas_left` would break frame isolation: a frame would gain spendable budget it never declared by touching state another party paid for, and its pool could exceed its declared limit, breaking per-frame conservation. Suppressing the refill entirely would instead overcharge common patterns — an approve-and-swap batch creates an allowance slot in one frame and clears it in the next — and would make `block_state_gas_used` overstate durable state growth, undermining the utilization assumptions behind EIP-8037's `CPSB` derivation. Reducing the earlier frame's `state_gas_used` takes the middle path: the payer is made whole at settlement, the frame receipts sum to the transaction's net state gas, and no other frame receives spendable budget. Journaling these receipt mutations also handles atomic-batch unrolls consistently with the existing removal of logs from unrolled frames.

### Intrinsic cost decomposition

`FRAME_TX_INTRINSIC_COST` is set equal to [EIP-2780](./eip-2780.md)'s `TX_BASE_COST` rather than derived independently. `TX_BASE_COST` prices one ECDSA recovery, the sender's account access and write, and inclusion of the transaction's bytes in a block. For frame transactions, signature recovery is priced per entry in `tx.signatures`, and the recovery-sized component instead covers the payer settlement — collection and refund of the maximum cost — which standard transactions do not price separately. As in EIP-2780 and [EIP-8038](./eip-8038.md), these values are provisional targets derived from client benchmarks and may be refined together.

Each value-bearing frame is charged `TX_VALUE_COST` intrinsically, covering the recipient balance write and transfer log exactly as EIP-2780 charges top-level value transfers; the charge is computable statically since `frame.value` and `frame.target` are transaction fields, and self-transfers are exempt for the same reason as in EIP-2780. Frame targets are deliberately *not* charged EIP-2780's unconditional cold-rate recipient touch: frames behave like internal calls, whose accesses EIP-2780 leaves on the standard [EIP-2929](./eip-2929.md) warm/cold model, and frames repeatedly targeting the same account would otherwise overpay.

More broadly, frame transactions retain EIP-2780's split between intrinsic and runtime gas. Intrinsic gas is state-independent and participates in the static validity check together with the transaction's declared execution budget; state-dependent charges never enter that check. Every state-dependent charge is applied inside a frame's `state_gas_limit`. This includes frame-entry account creation for value-bearing frames and sender-account creation by `APPROVE`. Exhausting state gas fails the frame: for a post-approval `SENDER` frame the transaction remains included and pays for the gas consumed, while failure in a `VERIFY` frame follows the validation semantics above and makes the transaction invalid.

### Transaction execution gas cap

The [EIP-7825](./eip-7825.md) cap applies to the frame transaction's complete execution budget. The sum of `frame_tx_intrinsic_gas` and every frame's `gas_limit` must not exceed `TX_MAX_GAS_LIMIT`; the calldata-floor validity check applies independently through the maximum of those values. A frame's `state_gas_limit` is excluded from this cap and is bounded by the transaction's aggregate encoding constraint and available block state-gas capacity, matching EIP-8037's treatment of state gas.
...
Wallets and gas estimators must produce two-dimensional, per-frame gas estimates: each frame's `gas_limit` covers its execution gas and its `state_gas_limit` covers its state gas, and neither budget can borrow from the other or from other frames. Under-provisioned state gas halts a frame exactly like out-of-gas; over-provisioned budgets are refunded at settlement but raise the up-front `max_cost` collected from the payer.
