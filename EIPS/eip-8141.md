...
Every public-mempool node MUST admit and propagate a frame transaction whose validation prefix satisfies the bounds in this section evaluated against `MAX_VERIFY_GAS`. This floor is the propagation and inclusion-list eligibility guarantee: a transaction at or below it reaches inclusion-list builders regardless of which node it first reaches. `MAX_VERIFY_GAS` SHOULD be set from the weakest node profile the network intends to keep in that inclusion-list set.

A node MAY additionally admit transactions whose validation work exceeds the floor when its own capacity allows, as local headroom. Such headroom only widens what an individual node accepts; it never lowers the floor for other nodes and never affects block validity. Because gas is an imperfect proxy for the real cost of validation at ingress — signature checks parallelize across cores, cold state reads block rather than spin, and succinct-proof verifiers scale with their public inputs — the headroom above the floor is a per-node policy informed by each client's measured worst-case validation cost, not a second protocol constant.
