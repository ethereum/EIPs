...
|---|---|---|
| `MAX_VERIFY_GAS`   | `100_000`  | Shared floor of public-mempool validation work: every node MUST admit and propagate a frame transaction whose validation prefix stays within this bound. See [Verification Gas Floor](#verification-gas-floor) |
...
#### Verification Gas Floor

A public-mempool node MUST NOT refuse to admit or propagate a frame transaction solely because the validation work of its prefix, evaluated against `MAX_VERIFY_GAS`, is too high; within that bound the verification cost is never grounds for rejection. This floor is the propagation and inclusion-list eligibility guarantee: a transaction at or below it reaches inclusion-list builders regardless of which node it first reaches, subject to the mempool's other admission checks such as nonce ordering, fee, balance, and intrinsic validity. `MAX_VERIFY_GAS` SHOULD be set from the weakest node profile the network intends to keep in that inclusion-list set.

Validation work above `MAX_VERIFY_GAS` is not by itself a violation of the public mempool rules. A node MAY additionally admit transactions whose validation work exceeds the floor when its own capacity allows, as local headroom. Such headroom only widens what an individual node accepts; it never lowers the floor for other nodes and never affects block validity. A transaction above the floor stays valid and still propagates, but only across the subset of nodes whose headroom accepts it, so the heavier its validation work the fewer nodes carry it and the slower it diffuses, without ever being dropped network-wide. This is a natural gradient rather than a cutoff: the floor is the minimum propagation every node guarantees, and everything above it is best-effort by each node's own capacity. Because gas is an imperfect proxy for the real cost of validation at ingress (signature checks parallelize across cores, cold state reads block rather than spin, and succinct-proof verifiers scale with their public inputs), the headroom above the floor is a per-node policy informed by each client's measured worst-case validation cost, not a second protocol constant.

Letting a node advertise a transaction's validation-prefix gas alongside its hash, so a peer can skip fetching an above-floor transaction it would only drop, is an eth-wire concern rather than part of this specification and belongs to a separate devp2p protocol change. A transaction at or below the floor MUST be admitted and propagated regardless of any such announcement.
...
- total public-mempool validation work, including signature validation, exceeds `MAX_VERIFY_GAS` and the node does not admit it as headroom (see [Verification Gas Floor](#verification-gas-floor))
