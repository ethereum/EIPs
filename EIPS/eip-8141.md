...
category: Core
created: 2026-01-29
requires: 1559, 2718, 2780, 3529, 3607, 4844, 7594, 7623, 7702, 7708, 7778, 7825, 8037, 8250
---
...
### Co-signer-funded value legs

A `SENDER` frame MAY perform a bare value transfer funded by a *co-signer* other than `tx.sender`. This lets a single frame transaction atomically settle payments from several independent externally-owned accounts, each spending its own balance, with no shared account and no pre-deposit. Call authority is unchanged: `msg.sender`, `ORIGIN`, execution approval, and gas payment all remain `tx.sender`; a co-signer contributes value only.

The following parameter is added:

| Constant | Value | Description |
| - | - | - |
| `LEG_AUTH_DOMAIN` | `keccak256("frame-tx/co-signer-leg")[:4]` | Domain separator for a co-signer leg authorization |

#### Wire shape

A `SENDER` frame gains three OPTIONAL trailing elements, present together or not at all:

```
frame = [mode, flags, target, gas_limit, value, data]                                        # legacy, 6 elements
frame = [mode, flags, target, gas_limit, value, data,
         payer_sig_index, cosigner_nonce_key, cosigner_nonce_seq]                             # co-signer-funded, 9 elements
```

List arity selects the shape, as the envelope's `nonce_keys` presence does; a 6-element frame is byte-identical to today's. A decoder MUST accept only 6 or 9 elements on a `SENDER` frame and MUST reject any trailing element on a non-`SENDER` frame. `payer_sig_index`, `cosigner_nonce_key`, and `cosigner_nonce_seq` MUST be canonical minimal-length RLP integers, with `0 <= payer_sig_index < len(tx.signatures)` and `cosigner_nonce_seq < MAX_NONCE_SEQ`. `data` MUST be empty on a co-signer-funded leg.

#### Authorization

When a `SENDER` frame carries the co-signer fields, let `s = tx.signatures[payer_sig_index]`. `s` MUST be a `SECP256K1` or `P256` entry whose `msg` is an explicit 32-byte digest (not the empty canonical-hash form), whose `signer` is a 20-byte address, and whose `signer != tx.sender`. Then `payer = s.signer`, and `s.msg` MUST equal:

```
keccak256(
    LEG_AUTH_DOMAIN || chain_id || tx.sender
    || cosigner_nonce_key || uint64_be(cosigner_nonce_seq)
    || batch_digest
)
```

where `batch_digest` is `keccak256` over the ordered static content of every frame in the co-signer leg's atomic batch, in frame order, each frame contributing `mode || flags || target || u256_be(value) || keccak256(data) || uint64_be(limits.execution) || uint64_be(limits.state)`. Because `s.msg` is non-empty, its bytes remain committed by `compute_sig_hash(tx)`, so the sender's outer signature transitively commits to the co-signer entry. `batch_digest` binds the co-signer's own leg and every sibling leg and atomic-batch membership, so a lifted `s` is valid only inside the identical batch a co-signer authorized: a relay cannot re-target, reprice, re-order, strip, or gas-starve the leg without invalidating `s`.

#### Execution

A `SENDER` frame carrying the co-signer fields performs a bare value transfer and does not execute `target` code, regardless of `target`'s code. The frame:

- checks `NONCE_MANAGER[payer][cosigner_nonce_key] == cosigner_nonce_seq`; if not current, the frame halts exceptionally;
- checks `payer` balance `>= value`; else the frame halts exceptionally;
- transfers `value` from `payer` to `target` and emits the EIP-7708 transfer log with `from = payer`, `to = target`, `value`, including when `target == payer` or `target == tx.sender`;
- if `target` does not exist and `value > 0`, charges `STATE_BYTES_PER_NEW_ACCOUNT * CPSB` state gas from the frame's `state_gas_left` before the transfer;
- increments the co-signer sequence in the co-signer's `NONCE_MANAGER` domain. This write is journaled with the atomic-batch snapshot and is therefore rolled back if the batch fails, so a failed batch costs the co-signer nothing and can be retried; a committed inclusion blocks replay, and a reorg that drops it restores the nonce (per [EIP-8250](./eip-8250.md)) so the leg executes at most once on the canonical chain. First use of a co-signer key charges `KEYED_NONCE_FIRST_USE_GAS` per first-use key from the frame's `state_gas_left`.

Because the co-signer halt conditions are exceptional halts inside the atomic batch, a failed co-signer leg unrolls the batch exactly as any other failed batch frame. Running no code means no `CALLVALUE` reaches contract code under a foreign payer, so no target can credit `tx.sender` in error for a co-signer's value.

#### Mempool

A co-signer leg is a bare, direct-evaluable value transfer with all inputs on the wire, so a node validates it at admission without simulating the batch. Each `payer` is a recognized payer for its leg's `value`: `reserved_pending_cost` is tracked per co-signer address, folding into the existing per-payer exposure rule. At most one pending frame transaction is admitted per co-signer address, reusing the one-pending-per-sender rule (per co-signer address, not per `(co-signer, key)`). Co-signer signature validation and the keyed-nonce read count against `MAX_VERIFY_GAS`, which bounds the number of co-signer legs per transaction with no new constant.
