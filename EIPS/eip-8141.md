...
The reads permitted above bound the state that any admitted validation prefix can depend on. A node MUST accumulate this state dependency set during simulation: `tx.sender`'s code hash and nonce, and every storage slot the prefix reads under `tx.sender`; the payer's code hash and balance, or a canonical paymaster's tracked reservation state; the runtime code of every contract reached via `CALL*` or `EXTCODE*`; and, if a `deploy` frame is present, the runtime code of the targeted factory. Nodes SHOULD index each pending transaction by this set and, on a new head, revalidate it by checking whether any member changed, re-simulating the prefix only for a transaction whose set was touched. This generalizes to every admitted prefix the dependency set that [Direct Evaluation of Protocol-Defined Frames](#direct-evaluation-of-protocol-defined-frames) enumerates for the protocol-defined case. Once a member of a transaction's set has changed, a node MUST treat that transaction as requiring re-simulation until a re-simulation completes successfully, and MUST NOT return it to the revalidated set on a later head merely because that head's changes do not intersect its set, since the earlier change may already have invalidated the prefix.
