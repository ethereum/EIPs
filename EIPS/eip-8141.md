...
|------|-----------|----------|---------------|
| `0x01` | initiate withdrawal | amount (non-zero) | caller is `signer`; no action pending |
| `0x02` | initiate rotation | new signer (non-zero) | caller is `signer`; no action pending |
| `0x03` | cancel pending action | - | caller is `signer` |
| `0x04` | finalize matured action | - | anyone, once `block.timestamp >= slot 2` |

Finalizing a withdrawal sends the amount to `signer` and reverts if the balance is insufficient (the sponsor cancels and re-initiates a smaller amount). Finalizing a rotation writes slot 3 into slot 0. Both clear the pending state before acting.
...
if CALLDATASIZE == 0:                   # validation path (pay frame)
    if SIGPARAM(scheme, 1) != SECP256K1: REVERT
...
if op == 0x01:                          # initiate withdrawal
    require CALLER == SLOAD(0) and SLOAD(2) == 0
