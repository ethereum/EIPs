...
category: Core
created: 2026-01-29
requires: 1559, 2718, 4844, 7623
---

## Abstract

Adds a new transaction whose validity and gas payment can be defined abstractly. Instead of relying solely on a single ECDSA signature, accounts may freely define and interpret their signature scheme using any cryptographic system. Additionally, *frames* are introduced to facilitate the new functionality and make it legible to the protocol. Frame transactions consist of one or more frames. Different modes and flags lead to different execution semantics and future increases the legibility of intended operation to the protocol.

## Motivation

This new transaction offers a lot across many areas:
...
- smart accounts become simpler and therefore safer by natively providing batch call proccessing
...
Ultimately, this transaction realizes the original vision of account abstraction. The assumption of an account simply becomes an address with code. It leverages the EVM to support arbitrary *user-defined* definitions of validation and gas payment.
...
Below are high-level definitions of each field in the transaction defintion. Detailed behavior is defined in subsequent sections.
...
- `value` -- the amount in wei that should transfered from the `sender` as part of the frame execution.
...
The canonical signature is defined such that any frame with mode `VERIFY` will have its data elided:
...
3. If frame has mode `SENDER` and it fails, unroll the atomic batch.
    - Consecutive `SENDER` frames where all but the last have the atomic batch flag (bit 2 of `flags`) set form an **atomic batch**.
    - When a frame in the batch fails, the state must be rolled back to the condition it was in after the last frame before the atomic batch began. All remaining frames in the atomic batch are skipped.
...
This refund is returned to the gas payer (the `resolved_target` that called `APPROVE(APPROVE_PAYMENT)` or `APPROVE(APPROVE_PAYMENT_AND_EXECUTION)`) and added back to the block gas pool.

##### Default code

Frame transactions can be used by EOAs when they do not have code nor an [EIP-7702](./eip-7702.md) delegation indicator via the "default code" mechanism. The behavior of the default code is defined below:
...
1. `APPROVE_NONE` (`0x0`): No approval scope allowed - the current frame is not allowed to approve any scope.
1. `APPROVE_PAYMENT` (`0x1`): Approval of payment - the contract approves paying the total gas cost for the transaction.
1. `APPROVE_EXECUTION` (`0x2`): Approval of execution - the sender contract approves future frames calling on its behalf.
   - Note this is only valid when `resolved_target` equals `tx.sender`.
1. `APPROVE_PAYMENT_AND_EXECUTION` (`0x3`): Approval of payment and execution.

`APPROVE_SCOPE_MASK` is defined as an alias for `APPROVE_PAYMENT_AND_EXECUTION`.
...
- Ensure that `scope` is one of the caller's allowed scopes in `frame.flags`, otherwise revert.
    - To check this, evaluate that `frame.flags & APPROVE_SCOPE_MASK & scope` is non-zero.
- Depending on the value of `scope`, perform the following:
    ...
    - For `APPROVE_PAYMENT`
        ...
        - Increment the sender's nonce, collect the transaction's maximum cost from `resolved_target`, and set `payer_approved = true`.
    - For `APPROVE_PAYMENT_AND_EXECUTION`: 
        - If `sender_approved` or `payer_approved` was already set, revert the frame.
        - If `resolved_target` != `tx.sender`, revert the frame.
        - If `resolved_target` has insufficient balance, revert the frame.
        - Set `sender_approved = true`, increment the sender's nonce, collect the transaction's maximum cost (`TXPARAM(0x06)`) from `resolved_target`, and set `payer_approved = true`.

### Introspection

The instructions in this section provide the needed instrospection capabilities of the new frame transaction type and the frame objects themselves, including some runtime information like the execution result status.
