...
description: Verify EIP-2930 and EIP-1559 signatures through the Keystore so those types can use post-quantum authenticators
...
category: Core
created: 2026-08-21
requires: 1271, 1559, 2718, 2930, 4844, 7702, 8130
---
...
**Why the 7702 *transaction type* is deprecated, and why the indicator stays.** The alternative to
deprecation is keeping type `0x04` and Keystore-checking `authorization_list` (same 65-byte /
`account || authenticator || data` shapes as origin). That would close the k1 hole on inner
authorities, but it would keep a second writer of the indicator next to 8130, and a second
envelope wallets have to implement. 8130 already has the Keystore-authenticated replacement:
auto-delegation and the delegation entry in `account_changes` (authorized by `sender_auth`).
This EIP takes deprecation. Type `0x04` is invalid after activation. The indicator
(`0xef0100 || target`) and code-loading remain; 8130 is the sole writer. Non-8130 chains keep
using type `0x04` as today. A Keystore-checked `authorization_list` is out of scope.
...
**No `authorization_list`.** Type `0x04` is invalid. This EIP does not Keystore-upgrade inner
7702 tuples; it removes the type that carried them. Delegation on these chains is only 8130
auto-delegation and `account_changes`, which already Keystore-authenticate the sender. There is
no remaining k1-only path to set or clear code delegation.
...
account authorize this hash" MUST use EIP-8130 `validateSignature` / [ERC-1271](./eip-1271.md). Enshrining that as a
