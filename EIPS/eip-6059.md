...
Since the current owner of the token is allowed to manage the token, there is a possibility that when selling the parent token, the seller might remove a child token post-factum. This is a risk that is inherent to the design of this standard. Marketplaces should take this into account and provide a way to verify the expected child tokens are present when the parent token is being sold or to guard against such a malicious behaviour in another way.
