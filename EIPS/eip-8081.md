...
eip: 8081
title: Private ERC20
description: Private burn and re-mint of ERC20 tokens using storage proofs and zero-knowledge circuits
author: Jake <jake@mayan.finance>
status: Draft
...
This EIP extends the zero-knowledge proof-of-burn mechanism from [EIP-7503](./eip-7503.md) to ERC20 tokens. Users burn tokens by transferring them to cryptographically unspendable addresses derived from secret preimages. Using Merkle Patricia Trie (MPT) storage proofs and zero-knowledge circuits, users can later prove the burn occurred without revealing the original transaction, enabling private re-minting of equivalent tokens.

## Motivation

[EIP-7503](./eip-7503.md) introduced Zero-Knowledge Wormholes for native ETH, enabling privacy-preserving burns and re-mints. However, the majority of value transferred on Ethereum exists in ERC20 tokens. Extending this mechanism to ERC20 tokens requires additional considerations:

1. **Storage vs Account State**: ETH balances are stored in account state, while ERC20 balances are stored in contract storage, requiring a two-level proof structure.

2. **Variable Storage Layouts**: Different ERC20 implementations store balances at different storage slots, requiring slot discovery mechanisms.
...
Current privacy solutions for ERC20 tokens have significant drawbacks:
...
This EIP enables permissionless, trustless privacy for any ERC20 token without modifying the token contract.
...
- **Mapping Slot**: The base storage slot of the `balances` mapping in an ERC20 contract
...
For an ERC20 token with `mapping(address => uint256) balances` at storage slot $S$, the storage slot for address $A$ is:
...
A complete ERC20 burn proof consists of:
...
Different ERC20 tokens store balances at different slots. Common patterns:

| Pattern | Mapping Slot | Examples |
|---------|--------------|----------|
| OpenZeppelin ERC20 | 0 | Most tokens |
| OpenZeppelin ERC20 (older) | 2 | Legacy tokens |
...
Unlike ETH balances stored in account state, ERC20 balances exist in contract storage. This requires:
...
This EIP introduces no changes to existing ERC20 tokens or the Ethereum protocol. It defines an external verification layer that works with any standard ERC20 token.
...
A reference implementation in Python for proof generation is available at:
[https://github.com/jakeolo/private-erc20](https://github.com/jakeolo/private-erc20)

The implementation includes:
- MPT proof generation using `eth_getProof` RPC
- MPT proof verification
- Storage slot calculation for ERC20 balances
...
This EIP is designed to complement EIP-7503. The same secret and burn address pattern can be used for both ETH and ERC20 burns, though separate nullifiers are generated due to the token address inclusion.

## Citation

This EIP builds directly upon the concepts introduced in:

- [EIP-7503: Zero-Knowledge Wormholes](./eip-7503.md) - Keyvan Kambakhsh (@keyvank), Hamid Bateni (@irnb), Amir Kahoori, Nobitex Labs. "Enable minting of secretly burnt Ethers as a native privacy solution for Ethereum." 2023.
