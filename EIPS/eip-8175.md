...
author: Dragan Rakita (@rakita), Giulio Rebuffo (@Giulio2002) <giulio.rebuffo@gmail.com>
...
**FALCON-512 (`SIG_FALCON512` = `0x03`):**

Format: `[signature_type, signature]`

- `signature`: A variable-length Falcon-512 signature (typically ≤ 666 bytes, compressed format).

Falcon-512 is a lattice-based post-quantum signature scheme standardized by NIST (FIPS 206). It provides security against both classical and quantum adversaries. The address is verified by checking the signature against the account's Falcon-512 public key. Falcon-512 public keys are 897 bytes; the account address is derived as `keccak256(public_key)[12:]`.
...
- `signature_type`: `SIG_SECP256K1`, `SIG_ED25519`, or `SIG_FALCON512` (see [Signature Schemes](#signature-schemes)).
...
- `SIG_SECP256K1`, `SIG_ED25519`, or `SIG_FALCON512` with the actual signature payload if the capability has already been signed
...
4. The `signature_type` in each capability MUST be `SIG_SECP256K1` (`0x01`), `SIG_ED25519` (`0x02`), or `SIG_FALCON512` (`0x03`).
...
6a. For FALCON-512 signatures, the signature MUST be valid and the compressed signature length MUST NOT exceed 666 bytes.
...
The inclusion of Falcon-512 provides a migration path for accounts that require post-quantum security. Falcon-512 signatures are larger than SECP256K1 or Ed25519 (up to 666 bytes vs 64-65 bytes), resulting in higher intrinsic gas costs. However, the composable capability model allows accounts to adopt post-quantum signatures without requiring a new transaction type. Accounts can transition to Falcon-512 independently, and mixed signing flows (e.g. a Falcon-512 sender with a SECP256K1 payer) are natively supported.
