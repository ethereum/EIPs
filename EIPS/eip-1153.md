...
A test suite for this EIP can be found in the `ethereum/execution-spec-tests` repository, under `tests/cancun/eip1153_tstore` (commit `1983444`).
...
A concrete instance of this risk occurred on March 30, 2025, when the DeFi protocol SIR.trading lost its entire ~$355,000 in total value locked. A transient storage slot was used to verify that a callback's caller was a legitimate Uniswap v3 pool; later in the same call path, that same slot was overwritten (`tstore(1, amount)`) for an unrelated purpose without being cleared first. By brute-forcing a CREATE2 address whose numeric value matched the overwritten slot's contents, an attacker satisfied the stale authorization check on a subsequent invocation and repeatedly drained the protocol's collateral. Implementers should treat any transient slot used for access control or state verification as sensitive: either clear it immediately after the check that depends on it, or ensure the slot cannot be reused for an unrelated purpose within the same transaction.
