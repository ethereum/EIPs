...
If the `TSTORE` opcode is called within the context of a `STATICCALL`, the call must revert. `TLOAD` is allowed within the context of a `STATICCALL`.
