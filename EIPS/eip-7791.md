...
A new quantity is added to the calling context, the `gas2eth` limit. It is analogous to the gas counter, but is transaction-global and is maintained separately from the gas limit. At the beginning of the transaction, it is initialized to the same value as the tx gas limit.
...
- If `gas_amount` is greater than the remaining `gas2eth` limit, the calling context should fail with "out-of-gas" and any state changes reverted.
- Pushes `wei_val` onto the stack.

On reverts (e.g., through the `REVERT` opcode or invalid opcodes), any gas paid out via `GAS2ETH` should be returned to the gas2eth budget.
...
- Limit being 100% of the gas limit: 100% of the gas limit is a "natural" bound on the gas used and provides some protection to the user from unbounded `GAS2ETH` spending. In the future, a new transaction type could be added which includes an explicitly user-specified `GAS2ETH` limit.
