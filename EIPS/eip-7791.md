...
- Deducts `gas_amount` from both the current call frame's local `GAS2ETH` allowance and the transaction-global `GAS2ETH` budget.
  - If `gas_amount` is greater than the current call frame's local `GAS2ETH` allowance or the remaining transaction-global `GAS2ETH` budget, set `gas_amount` to `min(forwarded_gas, remaining_local_GAS2ETH_budget, remaining_global_GAS2ETH_budget)`.
