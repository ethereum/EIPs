...
* do static checks for each transaction in `check_transaction_static`:
...
At present, block's need to be fully executed before attesting to them. By allowing skipped transactions in a block, the protocol no longer requires each transaction to be validated at the moment a block is attested to. Instead, only the overall block validity must be confirmed initially—any invalid transactions skip execution rather than causing the entire block to be invalid. This MAY allow us to target a longer block execution time, giving us of some of the time between the attestation deadline (4s) and the beginning of the next slot (12s) for asynchronous execution. In contrast, most blocks today take well below a second to execute, mostly closer to 100ms.
