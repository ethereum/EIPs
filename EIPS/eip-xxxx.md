---
eip: xxxx
title: Contracts Registry the Dependency Injector
description: Management of smart contracts and their dependencies
...
The EIP proposes a way to manage smart contracts within the decentralized application/ecosystem. It enables protocols to become agile and reduces their maintenance threshold. The EIP demonstrates smart contracts dependency injection mechanism to organize the usage of their dependencies, especially in large projects.
...
The `ContractsRegistry` SHOULD implement the following interface:
...
interface IContractsRegistry {
    ...
    *  MUST call the setDependencies() with address(this) as an argument on the substituted contract
    *  @param name the name of the contract
    */
...
- The `ContractsRegistry` SHOULD deploy the `ProxyUpgrader` contract in the constructor that MUST be set as an admin of `Transparent` proxies deployed via `addProxyContract` method.
- It SHOULD NOT be possible to add zero addresses to the `ContractsRegistry`.
- The `ContractsRegistry` MUST use the `IDependant` interface in the `injectDependencies` method.
...
interface IDependant {
    ...
    */
   function setDependencies(address contractsRegistry) external;
...
- The `Dependant` contract MAY store the dependency injector address in the special slot `...` (obtained as `bytes32(uint256(keccak256(eipxxxx.dependant.slot) - 1))`).

## Rationale

The EIP proposes a way to manage system's smart contracts and maintain a clear usage of their dependencies. There are few design decisions that have to be specified explicitely:

### ContractsRegistry

**Usage**

The `ContractsRegistry` SHOULD be the topmost smart contract in the system. It SHOULD NOT depend on other contracts. The extensions of this EIP SHOULD add proper access control check to the described non-view methods. 

The `getContract` and `getImplementation` methods SHOULD revert if the nonexisting contracts are asked. This logic MAY be used to ensure that proper dependencies are provided to the dependants.
...
Only the injector MUST be able to call the `setDependencies` and `setInjector` methods. The initial injector will be a zero address, in that case, the call MUST NOT revert on access control checks. The `setInjector` function is made `public` to support dependency injection mechanism for factory-made contracts. However, the method SHOULD be used with extra care.

The injector address MAY be stored in the dedicated slot `...` to exclude the chances of storage collision.

## Reference Implementation

*0xdistributedlab-solidity-library dev-modules* provides a reference implementation.

- MIT licensed, no constraints on the usage.
- Test cases provided.
...
- The `ContractsRegistry` does not perform any upgradeability checks between the proxy upgrades. It is a user responsibility to make sure that the new implementation is compatible with the old one.
...
- The `Dependant` contract MUST set its dependency injector not later than the first call to the `setDependencies` function is made. That being said, it is possible to front-run the first dependency injection.
