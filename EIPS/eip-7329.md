...
Describes the motivation and rationale for splitting the EIP repositories into an
...
particulars of how each repository will govern itself are out of scope for this
...
1. All ERCs and Interface-category EIPs are removed from this repository and
   ...
   forked from this one with the non-ERCs removed. 
...
sub-groupings that include core protocol changes would be associated with the
EIP repository and other sub-groupings are associated with the ERC repository.
Any such process change is out of scope of this EIP and is independent of the
...
more sub-groupings. Such proposals are out of scope for this EIP.
...
chains native currency) can have a limited to zero impact on the rest of the
...
new semantics to gas estimation would not need a uniform rollout across the
...
covers a wide variety of topics such as elliptic curve cryptography, VM
...
working group process, however, it underscores how differently they would handle
it. For the core protocol change, the working group would be one of the two
...
process with known governance checkpoints.
...
* Changing the workflow to add the Eligible for Inclusion/Considered for
