---
eip: 5453
title: Endorse, Permit for Extendable Methods
description: A general protocol for arbitrary number of approvers giving ECDSA permit in the same TX for any EIP-5750 conforming extendable methods.
...
This EIP establish A general protocol for arbitrary number of approvers giving ECDSA permit in the same TX for any [EIP-5750](./eip-5750.md) conforming extendable methods. Unlike a few prior art ([EIP-2612](./eip-2612.md) for [EIP-20](./eip-20.md), `EIP-4494`(draft) for [EIP-721](./eip-721.md) that usually only permit for a single behavior (`transfer` for EIP-20 and `safeTransferFrom` for EIP-721) and a single approver in two transactions (first a `permit(...)` TX, then a `transfer`-like TX), this EIP provides a way to permit arbitrary behaviors and aggregating multiple approvals from arbitrary number of approvers in the same transaction, allowing for Multi-Sig or Threshold Signing behavior.
...
could balance a wider range of use cases. E.g. the same use cases of EIP-2612 and `EIP-4494`(draft) can be supported by `ERC5453_TYPE_A`. And threshold approvals can be done via `ERC5453_TYPE_B`. More complicated approval types can also be extended by defining new `ERC5453_TYPE_?`

1. We chose to include both `validSince` and `validBy` to allow maximum flexibility in expiration. This can be also be supported by EVM natively at  if adopted `EIP-5081`(draft) but it is not gaining consensus anytime soon, we choose to add these two numbers in this EIP to allow
...
Here is a reference implementation of `EndorsableERC721` that achieves similar behavior of `EIP-4494`(draft).
