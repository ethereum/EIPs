...
This EIP establishes a general protocol for an arbitrary number of approvers to give ECDSA permits in the same transaction as any [EIP-5750](./eip-5750.md) conforming extendable methods. Unlike prior standards ([EIP-2612](./eip-2612.md) for [EIP-20](./eip-20.md) and `EIP-4494` for [EIP-721](./eip-721.md) that usually only permit a single behavior (`transfer` for EIP-20 and `safeTransferFrom` for EIP-721) and a single approver using two transactions (first a `permit(...)` transaction, then a `transfer`-like transaction), this EIP provides a way to permit arbitrary behaviors and aggregate multiple approvals from an arbitrary number of approvers in the same transaction, allowing for multisig and threshold signing behavior.
