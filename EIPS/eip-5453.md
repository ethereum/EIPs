...
usually only permit for a single behavior (`transfer` for EIP-20 and `safeTransferFrom` for EIP-721) and a single approver in two transactions, this EIP provides a way to permit arbitrary behaviors and aggregating multiple approvals from arbitrary number of approvers in the same transaction, allowing for Multi-Sig or Threshold Signing behavior.
...
```solidity
// See the entire code files in ../assets/eip-5453
...
See more code files in [asset folder of EIP-5453](../assets/eip-5453).
...
### Reference Implementation of Endorsable ERC721
