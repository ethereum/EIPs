---
eip: 5453
title: Endorsement, Permits for Extendable Methods
...
This EIP establishes a general protocol for an arbitrary number of approvers to give ECDSA permits in the same transaction as any [EIP-5750](./eip-5750.md) conforming extendable methods. Unlike prior standards ([EIP-2612](./eip-2612.md) for [EIP-20](./eip-20.md) and [EIP-4494](./eip-4494.md) for [EIP-721](./eip-721.md) that usually only permit a single behavior (`transfer` for EIP-20 and `safeTransferFrom` for EIP-721) and a single approver using two transactions (first a `permit(...)` transaction, then a `transfer`-like transaction), this EIP provides a way to permit arbitrary behaviors and aggregate multiple approvals from an arbitrary number of approvers in the same transaction, allowing for multisig and threshold signing behavior.
...
1. We chose to include both `validSince` and `validBy` to allow maximum flexibility in expiration. This can be also be supported by EVM natively at if adopted by [EIP-5081](./eip-5081.md). These two parameters allow for
...
Here is a reference implementation of `EndorsableERC721` that achieves similar behavior as EIP-4494.
