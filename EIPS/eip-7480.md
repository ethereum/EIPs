...
4. If `offset + size` is greater than data section size of the active container, execution results in exceptional halt.
5. Reads `[offset:offset+size]` segment from the data section and writes it to memory starting at offset `mem_offset`.
