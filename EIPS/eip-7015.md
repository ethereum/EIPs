...
Creator consent is given by signing an [ERC-712](./eip-712.md) compatible message; all signatures compliant with this EIP MUST include all fields defined. The struct signed can be any arbitrary data that defines how to create the token; it must hashed in an ERC-712 compatible format with a proper ERC-712 domain. 
