...
- The `EXTCALL` opcode (introduced in [EIP-7069](./eip-7069.md)) does not provide a way to restrict the amount of gas being forwarded to the recipient, which means sending ether via `EXTCALL` is always subject to the reentrancy attack vector.
...
- Push `1` on the stack if the `PAY` operation was succesful, or `0` if it failed
  - The only reason for failure is currently attempting to `PAY` more `val` than is available in the balance
...
It is also possible to involuntarily fund an account by funding `block.coinbase` via priority fees.
