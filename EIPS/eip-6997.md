...
When `approve` is called by the owner, the approval MUST be frozen and an `ApproveValidation` MUST be created.

When `setApprovalForAll` is called by the owner, the approval MUST be frozen and an `ApproveValidation` MUST be created.
...
The standard only defines the validation functions, but not how they should be used.

Like the [ERC-721](./eip-721.md) standard with the `_mint` and `_burn` functions, it defines the validations as internal and lets the user decide how to manage them.
...
It could also be used as an wrapped Smart Contract for existing ERC-721, allowing 1/1 conversion with existing NFTs.
...
The `_validateTransfer` and `_validateApprove` functions are created as internal, just as the [ERC-721](./eip-721.md) `_mint` function, tries to create a secure system by which they can be called.
