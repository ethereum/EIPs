...
When `_transfer` is called by the owner, the transfer MUST be frozen and a `TransferValidation` MUST be created.

When `_approve` is called by the owner, the approval MUST be frozen and an `ApprovalValidation` MUST be created.

When `_setApprovalForAll` is called by the owner, the approval MUST be frozen and an `ApprovalValidation` MUST be created.
...
The `_validateTransfer` and `_validateApproval` functions are created as internal, just as the [ERC-721](./eip-721.md) `_mint` function, tries to create a secure system by which they can be called.
