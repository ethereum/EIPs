...
| `ts > 0 AND block.timestamp >= ts AND account has EIP-7702 delegation designation` | **DEACTIVATED** | `2`                     |
| `ts > 0 AND block.timestamp >= ts AND account has no EIP-7702 delegation`          | **DORMANT**     | `3`                     |
