...
For any sender or authority address `S`, clients MUST read `ts` from the system contract's storage at `SYSTEM_CONTRACT_ADDRESS` using the key defined in [Storage Layout](#storage-layout) (i.e., `keccak256(abi.encode(S, uint256(0)))`), interpreted as a `uint64` timestamp. The key is considered **deactivated** when `ts != 0` AND `block.timestamp >= ts`.

This check MUST be applied in the following contexts:

- **Block processing**: During transaction validity checks, if the recovered sender's key is deactivated, the transaction MUST be treated as invalid.
