...
description: Allows users to refund before escrow finalized
...
There are 3 contracts for the escrow process: `Buyer Contract`, `Seller Contract`, and `Escrow Contract`.
 ...
 - Escrow Contract: Will be created by the seller. The contract source code allows users(seller and buyers) based on constraint rules. Instead of a simple address mapped balance variable in [EIP-20](./eip-20.md) tokens, the user’s balance should be [Seller Token, Buyer Token].

**Every EIP-5528 compliant contract must implement the `ERC5528` interfaces**
```solidity
...
By design, EIP-5528 is fully backward compatible with [EIP-20](./eip-20.md).
