...
Batching attestations at origin solves this through two cooperating mechanisms. First, two cryptographic primitives protect the integrity of each batch: every batched validator signs a seal authorizing the specific batcher (preventing non-consensual inclusion and signature theft), and the batcher signs the specific composition (preventing third-party reuse and providing structural evidence of any composition equivocation). Second, gossip dedup admits a message only if it conveys at least one previously unseen vote for the duty, capping attacker amplification at O(k) for k controlled keys regardless of how the attacker partitions them across messages.
...
Batch efficiency scales with the number of validators under an operator concurrently assigned to the same committee. Reducing subnets increases density, making batching more attractive for medium-sized operators.
