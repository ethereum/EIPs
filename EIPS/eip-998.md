...
The `transferChild/safeTransferChild` transfer functions do have a **from** because it is already known that the calling contract is the **from**.
...
The `onERC721Received` function is how a top-down composable contract is notified that an ERC721 token has been transferred to it and what tokenId in the composable is the parent tokenId.
