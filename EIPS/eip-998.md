...
If the token is a bottom-up composable and has a parent token then call rootOwnerOf for the parent token.
  If the call was successful then the returned address is the rootOwner address.
  Otherwise call rootOwnerOfChild for the parent token.
	  If the call was successful then the returned address is the rootOwner address
	  Otherwise the token owner address is the rootOwner address
Otherwise call rootOwnerOfChild for the token
  If the call was successful then the returned value is the rootOwner.
  Otherwise get the token owner of the token and that is the rootOwner.
```

Calling `rootOwnerOfChild` for a token means the following logic:
```solidity
// call rootOwnerOfChild for tokenId
address tokenOwner = ownerOf(tokenId);
address childContract = address(this);
address rootOwner = ERC998ERC721(tokenOwner).rootOwnerOfChild(childContract, tokenId);
```
