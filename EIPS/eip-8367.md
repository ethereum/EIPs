...
Reference tests covering ceiling progression across the sunset window, clamp application to exited `0x00` balances, top-ups absorbed by the ceiling, credential rotation mid-window releasing the remaining balance, and the pre-start/post-end boundary epochs will be provided with the consensus-specs implementation.
