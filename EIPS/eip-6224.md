...
discussions-to: https://ethereum-magicians.org/t/eip-6224-contractsregistry-the-dependency-injector/12316
status: Draft
...
The EIP proposes a way to manage the system's smart contracts and maintain a clear usage of their dependencies. There are a few design decisions that have to be specified explicitly:
...
Only the injector MUST be able to call the `setDependencies` and `setInjector` methods. The initial injector will be a zero address, in that case, the call MUST NOT revert on access control checks. The `setInjector` function is made `public` to support the dependency injection mechanism for factory-made contracts. However, the method SHOULD be used with extra care.
