...
- `mix_in_active_fields`: Given a Merkle root `root` and an `active_fields` configuration return `hash(root, mix_in_length(merkleize_progressive(pack_bits(active_fields)), len(active_fields))`.
...
If `Optional` support becomes desirable, and it is not feasible to model the valid field combinations as a `CompatibleUnion`, the `active_fields` concept should be reused by updating `get_active_fields` to only emit `1` for fields that are always required as well as for optional fields that are present, and emit `0` for fields that are always excluded as well as for optional fields that are absent, retaining compatibility across the proposed `ProgressiveContainer(active_fields)` type. As for serialization, a `BitVector[O]` with `O` being the number of optional fields would be prepended to the value to indicate presence or absence of optional fields.

For lists of optionals, a more compact serialization should be considered, depending on whether the list is sparse (prepend each entry with its index), or dense (prepend bitlist that indicates optionality). `CompatibleUnion` serialization currently reserves several values (0, and 0x80-0xFF) for its selector tag.
