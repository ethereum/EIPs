...
Fields with a `None` value SHALL be represented as `null`.
...
`Profile[B]` also defines an ordered heterogeneous collection of fields, a subset of fields of a base `StableContainer[N]` `B`. This type has
...
- Fields in the base type `B` may be absent in `Profile[B]` (to disallow their presence in the sub-type)
- For each field in the base type `B` of type `Optional<T>`, the corresponding field in `Profile[B]` MUST have type `T`, `Optional<T>`, `Profile[T]`, or `Optional<Profile[T]>` (`Profile[T]` and `Optional<Profile[T]>` are only available if `T` is a `StableContainer[N]`)
- For fields in the base type `B`, the corresponding fields in `Profile[B]` may have type `T` / `Profile[T]` (to make them required in the sub-type), or `Optional[T]` / `Optional[Profile[T]]` (to keep them optional in the sub-type)

#### Serialization

The leading `Bitvector` is replaced by a sparse representation that only includes information about optional fields in `Profile[B]`. Bits for required fields of `Profile[B]` as well as the zero-padding to capacity `N` are not included. If there are no optional fields in `Profile[B]`, no `Bitvector` is omitted.
