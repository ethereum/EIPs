...
Precompiled smart contracts, does not execute opcodes, so there is no need to pass a gas parameter as a normal `CALL` (`0xf1`).  If the available gas is less than 2 plus the required gas required for the specific precompiled smart contract, the context just STOPS executing with an "Out of Gas" error.
