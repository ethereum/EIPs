...
## Abstract
Removing trust from upgradeability proxy is required for anonymous developers. To achieve that, disallowing instant, potentially malicious upgrades is required. This EIP introduces a time lock, Zero Trust Period, before already defined logic implementation becomes an active logic implementation.
...
// calldata is ignored here if zero trust period, described below, is not at zero
...
Zero trust period in blocks, can only be set once. While it is at default value(0), the proxy operates exactly as standard EIP-1967 transparent proxy. After zero trust period set, all above specification is enforced.
...
Propose block adds to convenience if used, so should be kept. An ability to cancel next logic can also be important for the same reason.
...
Users must ensure that a trust-minimized proxy they interact with does not allow overflows, ideally represent the exact copy of the code in implementation section above, and also they must ensure that Zero Trust Period length is reasonable.
