...
This specification is an extension on the ERC-20 token that provides basic functionality for depositing
...
MUST support a withdraw flow where the principal tokens are burned from `owner` directly where `owner` is `msg.sender` or `msg.sender` has ERC-20 approval over the principal tokens of `owner`.
...
ERC-5089 is fully backward compatible with the ERC-20 specification and has no known compatibility issues with other standards.
