...
MUST support a withdraw flow where the principal tokens are burned from `owner` directly where `owner` is `msg.sender` or `msg.sender` has [EIP-20](./eip-20.md) approval over the principal tokens of `owner`.
