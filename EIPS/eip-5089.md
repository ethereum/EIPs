...
description: Principal tokens (zero-coupon tokens) are redeemable for a single underlying ERC-20 token at a future timestamp.
...
The following specification is an API for principal tokens --
representing ownership of an underlying [ERC-20](./eip-20.md) token at a future timestamp.
This specification is an extension on the [ERC-20](./eip-20.md) token that provides basic functionality for depositing
and withdrawing tokens and reading balances and the ERC-2612 specification that provides
...
All ERC-5089 Principal Tokens MUST implement [ERC-20](./eip-20.md) to represent ownership of future underlying redemption (PTs).
If a PT is to be non-transferrable, it MAY revert on calls to `transfer` or `transferFrom`.
The [ERC-20](./eip-20.md) operations `balanceOf`, `transfer`, `totalSupply`, etc. operate on the Principal Token balance.

All ERC-5089 Principal Tokens MUST implement [ERC-20](./eip-20.md)'s optional metadata extensions.
...
- underlying: The token that Principal Tokens are redeemable for at maturity.
  Has units defined by the corresponding [ERC-20](./eip-20.md) contract.
...
MUST support a redeem flow where the Principal Tokens are burned from `owner` directly where `owner` is `msg.sender` or `msg.sender` has ERC-20 approval over the principal tokens of `owner`.
...
MUST support a withdraw flow where the principal tokens are burned from `owner` directly where `owner` is `msg.sender` or `msg.sender` has [ERC-20](./eip-20.md) approval over the principal tokens of `owner`.
...
The Principal Token interface is designed to be optimized for integrators with a core minimal interface alongside optional interfaces to enable backwards compatability. Details such as accounting and management of underlying are intentionally not specified, as Principal Tokens are expected to be treated as black boxes on-chain and inspected off-chain before use.

[ERC-20](./eip-20.md) is enforced as implementation details such as token approval and balance calculation directly carry over. This standardization makes Principal Tokens immediately compatible with all [ERC-20](./eip-20.md) use cases in addition to ERC-5089.
...
ERC-5089 is fully backward compatible with the [ERC-20](./eip-20.md) specification and has no known compatibility issues with other standards.
