...
**`Influencer`**: **`Influencer`** contract stores influence configurations measures the contribution of a user from his activities done in registered **`Account Bound Token(ABT)`** contract. The contract puts lock on **`Account Bound Token(ABT)`** until proposal is finalized.
