...
## Abstract
This EIP proposes a form of smart contract design pattern and a new type of account abstraction on how one's finance should be managed, ensuring transparency of managing investments and protection with self-sovereignty even from its financial operators.
...
The pattern empowers more self-sovereignty of one's asset via having a personal finance contract for each individual. Smart contracts should be a promise between peer to peer written in code, but current defi contracts are formed of 6 or less smart contracts to manage their whole investors' fund and often have owner key to control them arbitrarily. This implies that the current defi smart contracts are not fully self-owned, and it implies the risk that the investors have to trust contract operators not to start ponzi with mixed funds. The pattern draws a trustless baseline to remove the risk. Smart contracts without one's full control is basically a proxy with multisig with smart contract operators. One of members can easily run away with the operating funds as 0xSifu from Magic Internet Money(MIM).

The pattern with personal finance contract also gives more credentials(obviously with an Account-bound token) than storing mixed fund financial data in the operating team's contract. With a personal finance contract, an account's activity is easier to track than one global smart contract's activity. The pattern introduces an non-fungible token called ABT to store credentials from the personal finance contract.

Segregation between investor's fund and operation fee is clearly specified in the personal smart contract, so investors can ensure safety from arbitrary loss of funds by the operating team's control.	

#### Offchain-identity vs Soul-bound token on credentials 

This EIP opposes the off-chain identity solution to take over the whole system because their backends eventually rely on the trust of the operator, not cryptographic proof (e.g. Proof-of-work, Proof-of-stake, etc). Off-chain identity as credentials denies the whole premise of crypto's existence. Soulbound tokens are a better, verifiable, credential, and data stored off-chain is only to store token metadata.
...
Operating gas cost with one contract vs multiple contracts is actually gas saving if the clone factory pattern was applied. One contract storing users' state globally means each user is actually paying for storage cost on other previous users after interacting with the contract. This, for example, means that MakerDAO's contract operating cost is sometimes over 0.1 ETH, and often limits users' minimum deposit for CDP to save gas costs. To solve inefficient n-times charging gas cost interaction for future users, one contract per user is needed.

#### Separation between investor's and operation fund
The separation between an investor's fund and operation fee is clearly specified in the smart contract, so investors can ensure safety from arbitrary loss of funds by the operating team's control.
