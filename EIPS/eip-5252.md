...
Operating gas cost with one defi contract vs multiple contract was actually gas saving if clone factory pattern was applied. One defi contract storing m users' state globally means each user is actually paying for storage cost on other previous users after interacting with contract. This makes MakerDAO's contract operating cost to be sometimes over 0.1 ETH.
