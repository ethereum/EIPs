...
1. It is non-fungible and must satisfy [ERC-721](./eip-721.md).
2. Credentials are represented with its metadata
3. It should only reference factory to verify its minting. 
4. If it is transferrable, it is account-bound. If not, it is soul-bound.

A manager contract is defined with these properties:
1. It is the only contract which calls factory to create.
2. It stores all related configuration for financial parameters.

A factory contract is defined with these properties:
1. It clones the finance contract with uniform implementation.
2. It is the only contract that can mint account-bound token.
3. It must keep an recent id of account bound token.

A finance contract is defined with these properties:
1. A finance contract can only be initialized once from factory contract in constructor.
2. Funds in the contract cannot be transferred to other contracts unless sender who owns soul-bound or account bound token signs to do so.
3. Every state-changing function of the smart contract only accept sender who owns soul-bound or account bound-token except global function(e.g. liquidation).
...
5. If soul-bound token is used for access, the finance contract should be able to represent transaction that has happened only with whom had the private key.
