...
A `lender` smart contract implementing a flash lending feature MUST implement a `maxFlashAmount` function:
```
function maxFlashAmount(address token) external external returns (uint256);
```
...
The `flashLoan` function MUST transfer `amount` `token` to `receiver` before calling `FlashBorrowerLike(receiver).onFlashLoan(...)`.

The `lender` smart contract MUST include `msg.sender` as the first argument to `onFlashLoan`.

The `lender` smart contract MUST NOT modify the `data` parameter received to `flashLoan`, and must include it as the last argument to `onFlashLoan`.

`amount + fee` tokens MUST be returned to the `lender` before the end of the `flashLoan` function, reverting otherwise.

The return of the loan MUST be initiated by the `receiver`. The `lender` MUST NOT take the `amount + fee` from the `receiver` or any other contract.

### Receiver Specification

A `receiver` of flash loans MUST implement an `onFlashLoan` callback:
...
For the transaction to not revert, `receiver` MUST send `amount + fee` of `token` to `msg.sender` before the end of `onFlashLoan`.
...
=======
>>>>>>> ece611fbdf5e7aa3dc897068c13f535cf39b736d
