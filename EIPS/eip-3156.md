...
```
function maxFlashAmount(address token) external returns (uint256);
```
...
After the callback, the `flashLoan` function MUST take the `amount + fee` `token` from the `receiver`, or revert if this is not succesful.
...
```
function batchFlashLoan(address receiver, address[] calldata token, uint256[] calldata amount, bytes calldata data) external {
  ...
  IERC3156BatchFlashBorrower(receiver).onBatchFlashLoan(msg.sender, tokens, amounts, fees, data);
  ...
}
```
...
After the callback, for each `token` in `tokens`, the `batchFlashLoan` function MUST take the `amounts[i] + fees[i]` of `tokens[i]` from the `receiver`, or revert if this is not succesful.
...
contract FlashLender is IERC3156FlashLender {
     ...
     * @param receiver The contract receiving the tokens, needs to implement the `onFlashLoan(address user, uint256 amount, uint256 fee, bytes calldatadata)` interface.
     * @param token The loan currency.
     * @param amount The amount of tokens lent.
     * @param data A data parameter to be passed on to the `receiver` for any custom use.
     */
