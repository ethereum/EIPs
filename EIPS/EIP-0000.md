...
The mechanism only applies mean reversion when $f_g<\bar{f}_{g}$. While there can be benefits from mean reversion in both directions, the case when $f_g<\bar{f}_{g}$ is far more important to address. Furthermore, the update schedule from [EIP-7691](./eip-7691.md), applied when $f_g \geq \bar{f}_{g}$, already decreases more with 0 blobs than it increases with the maximum.
