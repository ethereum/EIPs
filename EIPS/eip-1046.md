...
This ERC adds a `tokenURI` function to [ERC-20](./eip-20.md), and extends [ERC-721](./eip-721.md) and [ERC-1155](./eip-1155.md) to enable interoperability between all three types of token URIs.
...
1. What type of token a contract is (if any);
2. How to display a token to a user, either in an asset listing page or on a dedicated token page; and
3. Determining the capabilities of the token
 ...
 * **Not every field has to be an optional boolean.**
 * Any optional JSON-serializable object is acceptable.
 */
