...
The Ethereum community has expressed a need for non-transferrable, non-fungible tokens. Popular contracts implicitly implement account-bound interaction rights today. A principled normation helps interoperability and improves on-chain data indexing. ABTs also enable new use cases including the tracking an individual's achievements (which can be used as credentials), or items that cannot be transferred (like certain loot in multiplayer online games).
