...
- The number of row subnets is a fixed constant, `ROW_SUBNET_COUNT = 128`, independent of the blob count. Blob rows are mapped to subnets with a per-slot pseudo-random permutation, reusing the consensus-layer swap-or-not shuffle:
