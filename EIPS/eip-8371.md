...
- The number of row subnets is a fixed constant, `ROW_SUBNET_COUNT = 128`, independent of the blob count. Blob rows are mapped to subnets with a rotating function: `subnet_id = (blob_index + slot) % ROW_SUBNET_COUNT`. The rotation distributes load evenly across the network over time, including which subnets are idle when the blob count is below `ROW_SUBNET_COUNT`. If the blob count exceeds `ROW_SUBNET_COUNT`, a subnet carries multiple rows in a slot; the bitmap of a row subnet covers the cells of all rows mapped to it, ordered by `blob_index`.
...
**Why rotate the blob-to-subnet mapping every slot?**

With fewer rows than subnets, some subnets are idle in a given slot. The rotation spreads idleness evenly, so every node contributes equally over time instead of high-index subnets being permanently unused.
