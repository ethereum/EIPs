...
In this document, a *supernode* is any node subscribed to 64 or more of the 128 column subnets, and thus able to reconstruct the entire blobspace on its own. Note that this is intentionally broader than the customary use of the term for nodes custodying all 128 columns: the requirements of this EIP attach to any node meeting the 64-subnet threshold.
...
Similar to PeerDAS, a supernode MUST contribute to reconstruction. Reconstruction of a row is REQUIRED only once a node holds at least 64 distinct verified cells of that row; subscription alone does not imply possession, so all reconstruction obligations below are conditional on this. Reconstruction becomes a two-phase process:

- 1st phase: a supernode MUST reconstruct each row mapped to its designated row subnet for which it holds sufficient cells, and it MUST send updated bitmap states to its peers. A small random delay (recommended range TBD) before reconstruction is allowed to desynchronise nodes in the network and reduce overall load.
- 2nd phase: after a slightly longer random delay (recommended range TBD), during which cells are collected from `getBlobs`, columns, and rows, a supernode MUST do a second reconstruction phase, reconstructing all missing rows for which it holds sufficient cells, and sharing the results as defined above.

Both delays are measured from the moment the node first obtains the block root of a valid block for the slot. A node MUST cancel a pending reconstruction of a row that completes through other means before the timer fires. The delays MUST be bounded by a maximum (value TBD), so that the recovery path is not postponed indefinitely under network degradation. Reconstruction SHOULD be prioritized for blocks on the node's current head branch.
