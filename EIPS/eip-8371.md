...
- 2nd phase: after a slightly longer random delay (recommended range TBD), during which cells are collected from `getBlobs`, columns, and rows, a supernode MUST — and any other row reconstructor SHOULD — do a second reconstruction phase, reconstructing all missing rows for which it holds sufficient cells, and sharing the results as defined above. The 2nd-phase obligation is limited to supernodes because only a node subscribed to all column subnets can observe rows completing elsewhere and cancel the redundant work; row reconstructors with fewer subscriptions lack this signal.
...
A node MAY defer reconstruction and cell collection for a block while any of the block's rows is not yet known to be recoverable — that is, while fewer than 64 distinct cells of some row are advertised across the topics the node observes — since completed rows of an unavailable block serve no purpose. Supernodes, observing the bitmaps of all column subnets, can assess the recoverability of every row before committing resources.
