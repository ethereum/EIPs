...
This standard is an extension of [ERC-721](./eip-721.md). It proposes some additional properties (`startTime`, `endTime`) and function (`isValidNow`) to help with on-chain time management.
...
By introducing these properties (`startTime`, `endTime`) and function (`isValidNow`), it is possible to enable and disable NFT automatically on chain.
