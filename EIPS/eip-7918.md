...
An `if`-clause is added to `calc_excess_blob_gas()` from [EIP-4844](./eip-4844.md). The function no longer subtracts `target_blob_gas` when updating `excess_blob_gas`, if the price of a `blob` is below the price of `BLOB_BASE_COST` execution gas. The `blobSchedule` for referencing target and max blobs was introduced in [EIP-7840](./eip-7840.md). The *current block's* `blobSchedule` is used during processing. Thus, when calculating the `base_fee_per_blob_gas` via `get_base_fee_per_blob_gas(parent)` after a hard fork, the `fake_exponential()` function is called with the *new* `blobSchedule.baseFeeUpdateFraction`. Likewise, the *new* `blobSchedule.max` and `blobSchedule.target` are used in `calc_excess_blob_gas()`.
