...
```solidity
function previewDeposit(uint256 value) public view returns (uint256 shares)
```
...
```solidity
function previewMint(uint256 shares) public view returns (uint256 value)
```
...
```solidity
function previewWithdraw(uint256 value) public view returns (uint256 shares)
```
...
```solidity
function previewRedeem(uint256 shares) public view returns (uint256 value)
```
...
ERC-20 inheritance is required because implementation details like token approval and balance calculation directly carry over to the shares accounting. This standardization makes the vaults immediately compatible with all ERC-20 use cases in addition to ERC-4626.
...
There are a number of security/safety considerations developers implementing and integrating ERC-4626 vaults should be aware of.

### Choosing the Right Preview

Integrators must ensure they are using the appropriate "preview" function for their use case when performing calculations related to a vault. The amount of underlying tokens a user may receive from redeeming their vault shares (`previewRedeem`) can be significantly different than the amount that would be taken from them when minting the same quantity of shares (`previewMint`). The differences may be small (like if due to rounding error), or very significant (like if a vault implements withdrawal or deposit fees, etc). Therefore integrators should always take care to use the preview function most relevant to their use case, and never assume they are interchangeable. 

Similarly it is important to note that while an `exchangeRate()` function is provided for convenience, it only matches the behavior of `previewRedeem(10 ** vault.decimals())`, so it should not be used in any situation the integrator would not wish to use `previewRedeem`. For example, `exchangeRate()` should not be used to compute the amount of vault shares that will be minted to a user on deposit, as due to rounding error and potential withdrawal/deposit fees the computed amount may be significantly incorrect.

### Rounding Error/Direction

Vault implementers should be aware of the need for specific, opposing rounding directions in some of the "preview" functions. It is considered best practice to always favor the Vault itself during calculations. For example, in the `previewDeposit` function, the amount of shares returned should round down towards 0, preferring to give users too little share tokens rather than too many. However, note that this can lead to cases where users receive 0 shares and lose their deposits, which is why it is recommended to revert in cases where the amount of shares minted for a deposit would be 0. However unlike in `previewDeposit`, in `previewWithdraw` the amount of shares returned should round up towards 1, preferring to burn more of the user's vault shares than strictly needed rather than too few.

In summary, the rounding direction should be dependent on whether the vault is calculating an amount that will go into the vault – in which case it should round up – or whether it is calculating an amount that goes out of the vault to a user – in which case it should round down.

- No matter whether (1) it is calculating how many shares to issue to a user for a certain amount of the underlying tokens they provide or whether (2) it is determining the amount of the underlying tokens to transfer to them for returning a certain amount of shares, it should round down. 

- No matter whether (1) it is calculating the amount of shares a user has to supply to receive a given amount of the underlying tokens or whether (2) it is calculating the amount of underlying tokens a user has to provide to receive a certain amount of shares, it should round up.

### Share Token Standard
