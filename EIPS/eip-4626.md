...
- fee: An amount of assets or shares charged to the User by the Vault. Can be a fee on deposits, yield, AUM, withdrawal, or any other kind of fee.
...
MUST *NOT* return 0, unless `totalAssets` is 0.
MUST *NOT* revert.
...
MUST account for any Vault fees charged on converting the shares into assets.
...
`assets` MUST match the increase of `totalAssets`, and `shares` MUST match the increase of `totalSupply`.
...
`assets` MUST match the decrease of `totalAssets`, and `shares` MUST match the decrease of `totalSupply`.
