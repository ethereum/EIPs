...
Vault implementers should be aware of the need for specific, opposing rounding directions in the "preview" functions, as it is considered most secure to favor the vault itself during calculations over its users. For example, in the `previewDeposit` function, the amount of shares returned should round _down_ towards 0, preferring to give users too little share tokens rather than too many. However, note that this can lead to cases where users receive 0 shares and lose their deposits, which is why it is recommended to revert in cases where the amount of shares minted for a deposit would be 0. However unlike `previewDeposit`, the amount of shares returned from `previewWithdraw` should round _up_ towards 1, preferring to burn more of the user's vault shares than strictly necessary rather than too few.

In summary, the rounding direction used should be dependent on whether the vault is calculating an amount that will go into the vault, in which case it should round up, or whether it is calculating an amount that goes out of the vault to a user, in which case it should round down.
...
- If (1) it's calculating the amount of shares a user has to supply to receive a given amount of the underlying tokens or (2) it's is calculating the amount of underlying tokens a user has to provide to receive a certain amount of shares, it should round _up_.
