...
MUST return the *exact* amount of Vault shares that would be obtained if `depositor` were to deposit
...
MUST return the *exact* amount of underlying assets that would be deposited by `depositor` in exchange
...
MUST return the *exact* amount of Vault shares that would be redeemed by `redeemer` if withdrawing
...
MUST return the *exact* amount of underlying assets that would be withdrawn by `redeemer` if redeeming
