...
In the ERC4626 contract, the calculateShares function rounds down, and the calculateUnderlying function rounds up. Hence, the rounding direction is dependent on whether we convert shares to the underlying token or vice versa. However, the rounding direction should be dependent on whether we calculate an amount that has to go into the contract – in which case we should round up – or whether we calculate an amount that goes out of the contract to the user – in which case we should round down. And this is independent of the asset: No matter whether (1) we calculate how many shares to issue to the user for a certain amount of the underlying token they provide or whether (2) we determine the amount of the underlying token to transfer to them for returning a certain amount of shares, we should round down. No matter whether (1) we calculate the amount of shares a user has to supply to receive a given amount of the underlying token or whether (2) we calculate the amount of underlying a user has to provide to receive a certain amount of shares, we should round up.
