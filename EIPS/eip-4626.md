...
MUST NOT return a value higher than the actual maximum that would be accepted by `deposit` if `receiver` is subject to some deposit limit, it should underestimate if necessary.
...
MUST NOT return a value higher than the actual maximum that would be accepted by `mint` if `receiver` is subject to some deposit limit, it should underestimate if necessary.
...
MUST return an estimation of the maximum amount of assets `owner` could call `withdraw` with and not experience a revert if `owner` is not subject to any withdrawal limit or timelock.

MUST NOT return a value higher than the actual maximum that would be accepted by `withdraw`, it should underestimate if necessary.
...
MUST NOT return a value higher than the actual maximum that would be accepted by `redeem`, it should underestimate if necessary.
