...
If `target_address.code` contains an [EIP-7702](./eip-7702.md) delegation, the result of `HASCODE` should follow the delegation and return a result according to the delegation designator account. Additional gas costs and rules concerning `accessed_addresses` apply as specified in EIP-7702 for code reading instructions like `EXTCODESIZE`.
...
A new opcode is preferred by a general policy to not reuse opcodes. Also `HASCODE` can be rolled out in legacy EVM if desired.
