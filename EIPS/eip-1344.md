...
**NOTE**: A 64-bit value was chosen as a reasonably-sized value that works with the EIP-155 formatting for the `v` field (where `v = CHAIN_ID * 2 + 36`). The max value that can be used for chain ID is `MAX_CHAIN_ID = MAX_UINT256 / 2 - 36` in order to have the math fit inside the RLP encoding for the `v` field in a transaction (as is currently expected by all production RLP encoders).
