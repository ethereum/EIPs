...
### ORIGIN and CALLER
There was a discussion about making the ORIGIN and CALLER opcodes dependent on the transaction type, allowing each transaction type to define the values returned by these opcodes. However, there is a strong preference to keep transaction types opaque to contracts, discouraging them from treating different transaction types differently. Additionally, concerns were raised about maintaining backward compatibility with existing contracts that have assumptions about the ORIGIN and CALLER opcodes.

Moving forward, it is assumed that all transaction types will provide an address that reasonably represents the CALLER of the first EVM frame, with ORIGIN being the same address in all cases. If a transaction type needs to provide additional information to contracts, a new opcode will be required.

### Backwards Compatibility
Clients can distinguish between legacy transactions and typed transactions by examining the first byte. If the first byte is in the range [0, 0x7f], it indicates a new transaction type. If it falls within the range [0xc0, 0xfe], it indicates a legacy transaction type. The value 0xff is not used for RLP-encoded transactions and is reserved for future use as an extension sentinel value.

### Security Considerations
When designing a new 2718 transaction type, it is STRONGLY recommended to include the transaction type as the first byte of the signed payload. Failure to do so could result in a transaction being signature-compatible with other transaction types, potentially leading to security vulnerabilities for users.
