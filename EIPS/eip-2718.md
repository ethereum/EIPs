...
The first new transaction type will be a wrapped legacy transactions with the format `0 || rlp(nonce, gasPrice, gasLimit, to, value, data, v, r, s)`.
...
As of `FORK_BLOCK_NUMBER`, `0 || rlp(nonce, gasPrice, gasLimit, to, value, data, v, r, s)` will be a valid transaction where the transaction `Payload` will be signed/processed/handled exactly the same as legacy transactions were signed/processed/handled.
