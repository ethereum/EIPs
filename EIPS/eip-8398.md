...
category: Core
created: 2026-08-27
requires: 155, 170, 1559, 2028, 2718, 2929, 7702, 7708, 8401
---

## Abstract

This proposal introduces an [EIP-2718](./eip-2718.md) transaction type for accounts using the portable Keystore defined by [EIP-8401](./eip-8401.md). It provides native custom authentication, call batching, gas sponsorship, account-change transport, and keyed sequential transaction nonces without requiring nodes to execute arbitrary wallet validation code.

## Motivation

[EIP-8401](./eip-8401.md) separates account authority from transaction transport. Its actors, authenticators, deterministic accounts, and signed configuration changes work through ordinary EVM calls, including [ERC-4337](./eip-4337.md) deployments.
...
A transaction authenticates its sender and optional payer against the [EIP-8401](./eip-8401.md) Keystore. The protocol invokes the declared authenticator, resolves the returned actor configuration, applies this proposal's scope rules, consumes a keyed sequential nonce, and executes the declared calls.
...
This proposal owns the transaction envelope, transaction-specific scope assignments, native validation and execution, keyed nonces, gas accounting, account-change transport, transaction context, receipts, and RPC behavior. EIP-8401 owns the persistent actor and configuration model.
...
| Existing smart account | The account first imports or configures actors through EIP-8401 |
| New smart account | A create entry invokes the EIP-8401 deterministic creation semantics before call execution |
...
The authenticator interface, actor binding, expiry, implicit address-bound actor, and persistent configuration are defined by [EIP-8401](./eip-8401.md).

#### Canonical Authenticator Set

Canonical authenticators are enshrined on the validation path with constant validation costs. Their native execution MUST produce the same result as their EIP-8401 authenticator contracts.
...
EIP-8401 stores a `uint16` scope word and defines `scope == 0x0000` as admin. This proposal assigns:
...
The `account_changes` field transports EIP-8401 operations and native code delegation before calls execute. Entries are applied in order and are atomic per phase.

| Type | Name | Meaning |
|---|---|---|
| `0x00` | Create | Create the sender through EIP-8401 |
| `0x01` | Config change | Apply an EIP-8401 signed configuration batch |
...
Each initial actor is `[actorId, authenticator, scope]`. The actor set, account address, deployment header, and storage initialization MUST follow [EIP-8401](./eip-8401.md#account-creation).

Validation requires:

1. A canonical, strictly sorted EIP-8401 initial actor set.
...
The protocol places the supplied runtime code directly and initializes the EIP-8401 state. Its result MUST match the EIP-8401 `createAccount` EVM path.
...
This is the transaction encoding of EIP-8401 `SignedAccountChanges`. The operation payload is ABI-encoded as required by EIP-8401. Native application MUST match `applySignedAccountChanges`, including authorization, atomicity, expiry handling, storage, sequence consumption, events, and reverts.

The batch may use the EIP-8401 Local or Multichain channel. Its admin authorization is independent from `sender_auth`.
...
2. Apply create entries through EIP-8401 semantics.
...
The same EIP-8401 account can use different transaction transports on different chains:

| Component | Chain implementing this proposal | Chain without this transaction type |
|---|---|---|
| Authority | EIP-8401 Keystore | Same EIP-8401 contract |
...
| Creation | Native create entry | EIP-8401 CREATE2 factory |
...
All EIP-8401 contracts SHOULD use deterministic addresses across chains. The native path changes transport and pricing, not account authority.
...
The EIP-8401 constants `KEYSTORE_ADDRESS`, `K1_AUTHENTICATOR`, `CONTRACT_ESTABLISHED`, and `DEFAULT_EOA_REVOKED` are used unchanged.
...
### Why Reuse EIP-8401 Account Changes?
...
EIP-8401 accounts are usable without this proposal. Adoption adds a native transaction path but does not require account migration.
...
**Validation surface:** Canonical authenticators are pure functions of the transaction hash, signature data, and EIP-8401 actor configuration. Their enumerable invalidators are actor changes and sequential nonce consumption. Permissive acceptance of non-canonical authenticators widens that surface to state read by those contracts.
...
**Authenticator binding:** The actor ID returned by an authenticator must resolve to an EIP-8401 configuration bound to that authenticator.

**Implicit actor:** Only the raw EOA path or `K1_AUTHENTICATOR` may use EIP-8401's implicit address-bound actor.
...
**Account changes:** Native creation and configuration must exactly match EIP-8401 contract results. Divergence would produce transport-dependent authority.
