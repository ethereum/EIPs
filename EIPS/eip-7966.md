...
- The handler function MUST check if the transaction is ready for immediate execution BEFORE adding it to the mempool:
  - If the transaction has a nonce gap (transaction nonce is higher than the expected nonce), the handler function MUST NOT add the transaction to the mempool and MUST return error code `6` with the expected nonce in the `data.expected_nonce` field.
...
| Code | Error Type     | Description                                                          | Data Fields      |
|------|----------------|----------------------------------------------------------------------|------------------|
| 4    | Timeout        | Transaction was added to mempool but not processed within timeout    | `hash`           |
| 5    | Unknown/Queued | Transaction is NOT added to mempool (not ready for execution)        | `expected_nonce` |
| 6    | Nonce Gap      | Transaction is NOT added to mempool (nonce gap detected)             | `expected_nonce` |
...
- `data`: An object containing:
  - `hash`: The transaction hash (only present for error code 4, when transaction was added to mempool)
  - `expected_nonce`: The expected nonce for the next transaction in hex format (only present for error code 6, when transaction was NOT added to mempool)

### Sync Semantics

- **Sequential Submission**: Users MUST wait for the response before submitting the next transaction from the same account.
- **No Mempool Queueing**: Transactions not ready for immediate execution are rejected and NEVER added to the mempool.
- **Immediate Feedback**: Returns `expected_nonce` for nonce gaps, eliminating additional RPC calls.
...
Note: No transaction hash is returned because the transaction was never added to the mempool.
...
The structured error response format with specific error codes (4, 5, 6) and actionable data provides several benefits:

- **Immediate Feedback**: Developers receive specific error information without additional RPC calls.
- **Nonce Management**: Error code 6 includes the `expected_nonce` field, allowing applications to immediately construct the next transaction with the correct nonce.
- **Reduced Round-Trips**: Applications no longer need to query account state separately when encountering nonce gaps.
        ...
        match reason {
            QueuedReason::NonceGap => {
                let expected_nonce = self
                    ...
                    .ok()
                    .map(|account| account.map(|acc| acc.nonce).unwrap_or(0));

                return Err(ErrorObject::owned(
                    ERROR_CODE_NONCE_GAP,
                    ERROR_MSG_NONCE_GAP,
                    expected_nonce.map(|nonce| {
                        json!({
                            "expected_nonce": format!("0x{:x}", nonce)
                        })
                    }),
                ));
            }
