...
The `Portal wire protocol` [EIP-X](./eip-x) is used as wire protocol for the history network.


##### Protocol Identifier

As specified in the [Protocol identifiers](./eip-x#protocol-identifiers) section of the Portal wire protocol, the `protocol` field in the `TALKREQ` message **MUST** contain the value of `0x500B`.
