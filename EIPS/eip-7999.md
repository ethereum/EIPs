...
```python
def get_priority_fee(tx, gas_used: list[int], base_fees: list[int], remaining_fee: int) -> int:
    ...
    if tx.type == NEW_TX_TYPE: # New tx type: Single tip or full vector of tips
        if len(tx.max_priority_fee_per_gas) == 1:
            tmax_fee = tx.max_fee - gas_used[1] * base_fees[1]  # Ignore blobs (as now)
            tpriority_fee = tmax_fee - tfee if tmax_fee > tfee else 0
            max_priority_fee = min(tx.max_priority_fee_per_gas[0] * tgas, tpriority_fee)
        else:
            max_priority_fee = sum(vector_mul(tx.max_priority_fee_per_gas, gas_used))
...
**At the end of processing a transaction**:
  ...
  * Transfer to the coinbase `priority_fee_paid = get_priority_fee(tx, tx_gas_consumed, base_fees, max_fee - base_fee_paid)`
