...
* When atomicity is set to `strict`:
  * MUST execute all calls atomically in a single transaction.
...
* If a wallet executes multiple calls **atomically** in a multiple transactions (loose atomicity), `wallet_getCallsStatus` MUST return an object with a `receipts` field that contains **an array of receipts** for all transactions. It also MUST be explicit and return a `atomicity` field set to `"loose"`.
...
* `strict` means that the wallet will execute the calls atomically as part of a single transaction.
...
* `none` means that the wallet will execute the each call as an individual transaction.
