...
author: Moody Salem (@moodysalem), Derek Rein (@arein)
...
Reducing the amount of clicks end-users need to make to interact with Ethereum applications is crucial to improving the end-user experience. This is especially important for mobil-first applications where a signature usually required end-users to switch context into their app.
...
Wallets can advertise their support for these methods via the [CAIP-25](https://github.com/ChainAgnostic/CAIPs/blob/main/CAIPs/caip-25.md) standard which most wallets already implement.
...
### Advertising support via [CAIP-25](https://github.com/ChainAgnostic/CAIPs/blob/main/CAIPs/caip-25.md)

In order for dapps to be able to determine whether to use these methods or whether they need to fall back to `eth_sendTransaction` and `eth_getTransactionReceipt` they can advertise via CAIP-25, which most wallets already implement, that they support these RPC methods as follows:
  ...
  "result": {
    ...
    "sessionScopes": {
      "eip155": {
        ...
        "methods": ["eth_sendTransaction", "eth_signTransaction", "get_balance", "eth_sign", "personal_sign", "wallet_sendFunctionCallBundle", "wallet_getBundleStatus", "wallet_showBundleStatus"]
...
In the above code snippet you can see the CAIP-25 response advertises the support of 
...
Dapps may only use the new methods if the wallet advertises its support. Else they shall fall back to using  `eth_sendTransaction`.
