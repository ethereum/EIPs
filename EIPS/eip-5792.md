...
An app calling the above-defined methods without prior knowledge that the wallet supports them risks degrading user experience considerably, yet overtly or indirectly querying the wallet for interfaces risks eroding the anonymity of the user-agent (see Privacy Considerations below). The semantics of requesting a capability supports security and privacy by treating unsupported features and unpermitted features the same way, reducing fingerprinting surface. An exemplary wallet RPC is defined below for applications to request the capability to call the RPC methods above, assuming the wallet will process them atomically if the wallet grants that capability.
