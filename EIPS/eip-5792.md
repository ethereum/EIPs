...
Like the illustrative examples given above and other capabilities to be defined in future EIPs, the capability to specify how calls delivered via the above-defined methods should be executed can be expressed  as `atomic` or `nonAtomic`.
...
```typescript
type AtomicityCapability = "atomic" | "nonAtomic";
```

The only valid JSON-RPC values for this capability's only member, `atomicity`, are `"atomic"` and `"nonAtomic"`.
