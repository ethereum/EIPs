...
The current APIs to send transactions and check their status are `eth_sendTransaction` and `eth_getTransactionReceipt`.
These methods are not flexible enough for dapps to provide a good user experience for sending transactions and showing
...
where the actual transaction hash may not be known at the time of signing, but must still be returned
by `eth_sendTransaction`. The transaction hash returned by `eth_sendTransaction` in these cases is unlikely
to be relevant to the transaction hash of the included transaction. Dapps often link to the block explorer for
