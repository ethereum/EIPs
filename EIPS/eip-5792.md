...
If a wallet does not support a chain that is requested in the `wallet_getCapabilities` method, the wallet MUST NOT respnond with an error, and MUST instead not include a key for the unsupported chain(s) in the response.
...
| 5720 | Malformed request | The wallet cannot parse this request (e.g., missing 0x prefix, leading zeros in chain id, request fails schema validation) | `wallet_sendCalls`, `wallet_getCallsStatus`, `wallet_showCallsStatus`, `wallet_getCapabilities` |
| 5730 | Duplicate ID | There is already a bundle submitted with this id | `wallet_sendCalls` |
| 5740 | Unknown bundle id | This bundle id is unknown / has not been submitted | `wallet_getCallsStatus`, `wallet_showCallsStatus` |
| 5750 | Bundle too large | The call bundle is too large for the wallet to process | `wallet_sendCalls` |
| 5760 | Atomic-ready wallet rejected upgrade | The wallet can support atomicity after an upgrade, but the user rejected the upgrade | `wallet_sendCalls` |
| 5770 | Atomicity not supported | The wallet does not support atomic execution but the request requires it | `wallet_sendCalls` |
