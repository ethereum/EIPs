...
author: Moody Salem (@moodysalem), Lukas Rosario (@lukasrosario), Wilson Cusack (@wilsoncusack), Dror Tirosh (@drortirosh)
...
Defines new JSON-RPC methods for apps to send batches of calls from a user's wallet, as well as check on the status of those calls. The new methods have a simple core while enabling additional transaction features (e.g. sponsored transactions with a paymaster) via the introduction of capabilities. To support this, there is also a new method for querying for the capabilities a given wallet supports. The new methods are more abstract in regard to the underlying transactions than the existing transaction sending APIs to allow for differences between wallet implementations, e.g. smart accounts utilizing ERC-4337 or EOA wallets that support batched transactions via EIP-3074. Apps may use this more abstract interface to support different kinds of wallets without additional effort, as well as provide a better UX for sending batches of function calls (e.g. ERC-20 `approve` followed by a contract call).
...
Today, developers want to send batch transactions, which many smart accounts can support atomically. Developers also want to make use of features afforded by new transaction formats, like paymasters in ERC-4337 transactions. `eth_sendTransaction` offers no way to do these things. 

In updating to a new set of `wallet_` RPCs, our main goals are to enforce a clean separation of wallet and app concerns, and create a clear way for more features to be added over time with minimal coordination.
