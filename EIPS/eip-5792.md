...
* Dapps MUST NOT rely on the calls being sent in a single transaction, i.e. other untrusted calls may be
...
DATA, 32 Bytes - an identifier that the wallet uses to represent the bundle of calls.
...
value
returned from the `wallet_sendFunctionCallBundle` RPC. Note this method only returns a subset of fields
