...
Today, developers want to send batched transactions, which many smart accounts can receive and execute atomically. Developers also want to make use of features afforded by new transaction formats, like paymasters in [ERC-4337](./eip-4337.md) transactions. `eth_sendTransaction` offers no way to do these things. 
...
Three new JSON-RPC methods are added: two for handling batches of onchain calls, and one for querying support for wallet capabilities, such as the other two novel methods. Apps may begin using these first two methods immediately, falling back to `eth_sendTransaction` and `eth_getTransactionReceipt` when they are not available.

We define one capability expression for use with the third method, specifically for expressing a guarantee of call execution atomicity pertinent to the first two methods.
...
An app calling the above-defined methods without prior knowledge that the wallet supports them risks degrading user experience considerably, so an additional RPC for querying the wallet's capabilities is provided.

In addition to, or instead of, querying the wallet directly for capabilities, the same capability objects could be expressed out-of-band (such as in the `sessionProperty.capabilities` object persisted in a CAIP-25-conformant wallet provider interface) or in a well-known location (such as a URL derived from an [EIP-6963](./eip-6963.md) `rdns` identifier). Provider abstractions may also cache capabilities from previous requests or otherwise inject them from out-of-band. If querying multiple ways leads to different results, or to override caches or provider injections, direct RPC communication with the wallet SHOULD always be taken as the canonical and current expression of capabilities.

#### `wallet_getCapabilities` RPC Specification

The key of each item per chain is names a capability and the value can be of any shape. Capabilities are returned per chain because wallets may support different capabilities across chains.

We expect the community to align on the syntax and semantics of additional capabilities in separate ERCs over time.
