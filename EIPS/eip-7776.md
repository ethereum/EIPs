...
- `SSTORE` operations that revert a storage slot to its original value.
...
2. **Block Gas Accounting:**
   ...
   - **`SSTORE` Operations:**
     ...
     - **Reverting Storage to Original Value:**
       - For block gas accounting, `SSTORE` operations that revert a storage slot to its original value receive the same gas discount as in user gas costs, reflecting the reduced workload (no actual write to storage).
