...
Currently, Ethereum bytecode has no syntatic structure.  We propose to add named procedures -- delimited blocks of code that can be entered only by calling into them via defined interfaces.  Syntactic contraints on the use of procedures must be validated at contract initialization time to maintain the safety properties of [EIP-3779](./eip-3779.md): Valid programs will not halt with an exception unless they run out of gas or recursively overflow stack.

We also propose to support procedures with a stack of automatically allocated and deallocated stack frames in memory.
...
#### ADDRESSLOCAL (0x??) offset: int16
```
*SP++ = FP + offset
```
> Push the address `FP + offset` onto the data stack.

Local data is addressed at an immediate `offset` relative to `FP`.

Typical usage includes storing data on the call stack (32 bytes above the `frame pointer` in the following cases)
```
PUSH 0xdada
ADDRESSLOCAL 32
MSTORE
...
The design model here is the subroutines and procedures of the Intel x86 architecture. Semantically,
* `JUMPSUB` and `RETURNSUB` (from [EIP-2315](./eip-2315)) -- like `CALL` and `RET` -- jump to and return from subroutines.
* `ENTERPROC`  -- like `ENTER` -- sets up a stack frame for a procedure.
...
* `RETURNPROC` amounts to an `RJUMP` to a `LEAVEPROC` and
* `ADRESSLOCAL` addresses data local to the current procedure's call frame.
