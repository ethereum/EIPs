...
discussions-to: https://ethereum-magicians.org/t/eip-4573-entry-points-for-eof/4573
created: 
...
Memory can be addressed relative to the frame point `FP` or  by absolute address.  `FP` starts at 0, and moves to point to the start of the input arguments for each `CALLPROC`, and to the start of the output values for each `RETURNPROC`.
...
*Note:  In order to save space the inputs and outputs overlap.  When the inputs are used up in the course of producing the outputs this should not be a problem.  (The space beyond the end of the call frame can be used for temporaries.)*

### Instructions

#### ENTERPROC (0x??) n_inputs: uint2, n_locals: uint2, n_outputs: uint2
> Marks the entry point to a procedure.  Procedures can only be entered via a `CALLPROC` to their entry point.
>
...
```
> Prepare to return to the current procedure, then call the new procedure.  The required `n_inputs` words are validated to be available on the frame stack, with the start of the input addressed by `FP`.
>
...
```
   ...
   PC = return_stack.pop()
```
> Leave the current procedure and return to the calling produre, with PC set to one past its previous value, and FP restored to its previous value.  The promised `n_outputs` words are validated to be available on the frame stack, with the start of the output addressed by FP.
...
This proposal uses the [EIP-2315](./eip-2315) return stack to manage calls and returns, and steals and simplifies ideas from [EIP-615](./eip-615) and [EIP-3337](./eip-3337). `ENTERPROC` is modeled on `BEGINSUB`, and per EIP-3337 it moves call frames from the data stack to a new in-memory stack. The `FP` register automatically tracks call frame addresses as procedures are entered and left, and cannot be otherwise modified.  The aliasing of the frame stack with ordinary memory supports languages that provide pointers to variables on the stack.
