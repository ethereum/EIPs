...
If a previously-existing account in the state trie would have its code modified by the the `CREATE` or `CREATE2` opcodes, or by transferring ether to the zero address, instead, the call or ether transfer MUST revert. The `CREATE` and `CREATE2` opcodes MUST cost an additional `100` gas.
