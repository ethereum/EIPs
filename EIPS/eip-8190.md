...
9. The transaction MUST have sufficient gas to cover the intrinsic gas including `scheme_gas_surcharge`.
...
Post-quantum signatures are 10–20× larger than ECDSA signatures. Without a gas surcharge, a block full of Falcon transactions would consume significantly more bandwidth and storage than a block of ECDSA transactions at the same gas cost. The Falcon surcharge of 28,008 gas (25,008 for calldata-equivalent data cost + 3,000 for verification) ensures economic parity: nodes that must download, validate, and store 1,563 extra bytes per Falcon transaction are compensated proportionally.
