...
The base fee is updated by aggregating the regular gas and "normalized state gas", where a net change of `MAX_STATE_BYTES_PER_BLOCK` corresponds to block's gas limit, thus giving state gas the same range as regular gas. The change is still restricted to the range ±12.5%. Ignoring conditioning against negative values, the `gas_used_delta` used in the base fee update can for a EIP-1559 style mechanism be computed as:
