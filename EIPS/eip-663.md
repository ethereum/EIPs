...
"Operand", `n`, and `m` always refer to values in the semantic domain. In assembly written using mnemonics, operands must be written and interpreted as values in the semantic domain. For example, the assembly `DUPN 20` represents the instruction `DUPN` with operand `n = 20`.

We now detail the encoding and decoding procedures. Instruction encoding is mainly relevant for assemblers and compilers, and decoding mainly for EVM implementations and disassemblers.
...
This function maps pairs in the domain onto `[0, 79] ∪ [128, 255]`.
