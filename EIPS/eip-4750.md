...
Rules 5, 6, and 7 require a symbolic execution of the code.  This algorithm does not doesn't consider the code’s data and computations, only its control flow and stack use.  This means we will reject programs with invalid code paths, even if those paths are not reachable.
