...
3. If data stack has more than `caller_stack_height + type[code_section_index].outputs` items, discards the items between `caller_stack_height` and top `type[code_section_index].outputs`, so that there are exactly `caller_stack_height +  type[code_section_index].outputs` items left.
4. Charges 4 gas.
5. Pops nothing and pushes nothing to data stack.
