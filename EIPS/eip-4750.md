...
eip: 4750
title: EOF - Functions
description: Individual sections for functions with `CALLF`, `TAILCALLF` and `RETF` instructions
...
1. Has one immediate argument,`code_section_index`, encoded as a 16-bit unsigned big-endian value.
2. If data stack has less than `caller_stack_height + type[code_section_index].inputs` items, execution results in exceptional halt.
3. If data stack has more than `caller_stack_height + type[code_section_index].inputs` items, discards the items between `caller_stack_height` and top `type[code_section_index].inputs`, so that there are exactly `caller_stack_height +  type[code_section_index].inputs` items left.
4. Charges 6 gas.
...
2. List of allowed *terminating instructions* in EIP-3670 is extended to include `RETF` and `TAILCALLF`. (*Note that `CALLF` and `TAILCALLF`, like other instructions with immediates, cannot be truncated.*)
3. Code section is invalid in case an immediate argument of any `CALLF` or `TAILCALLF` is greater than or equal to the total number of code sections.
4. Code section is invalid in case an immediate argument of any `TAILCALLF` is such that `type[callee_section_index].outputs != type[caller_section_index].outputs`, i.e. it is allowed to tail-call only the function with the same output type.
5. `RJUMP` and `RJUMPI` immediate argument value (jump destination relative offset) validation:
    1. Code section is invalid in case offset points to a position outside of section bounds.
    2. Code section is invalid in case offset points to one of two bytes directly following `CALLF` or `TAILCALLF` instruction.
...
# STOP, RETF, TAILCALLF, RETURN, REVERT, INVALID
...
immediate_sizes[0xb2] = 2  # TAILCALLF
...
def validate_code_section(func_id: int, code: bytes, types: list[FunctionType] = [FunctionType(0, 0)]):
    ...
    while pos < len(code):
        ...
        if opcode == 0x5c or opcode == 0x5d:
            ...
            if pos + 2 > len(code):
                raise ValidationException("truncated TAILCALLF immediate")
            section_id = int.from_bytes(code[pos:pos+2], byteorder = "big", signed = False)
            ...
            if types[section_id].outputs != types[func_id].outputs:
                raise ValidationException("incompatible function type for tail call")
