...
Furthermore it aims to improve analysis opportunities by encoding the number of intputs and outputs for each given function, and isolating the stack of each function (i.e. a function cannot read the stack of the caller/callee).
...
We have decided in favor of always having at least one item in the return stack, becasue it allows to avoid having a special case for empty stack in the interprer loop stack underflow check. We keep the stack underflow rule general by having `caller_stack_height = 0` in the top frame.
