...
1. Has one immediate argument,`code_section_index`, encoded as a 16-bit unsigned big-endian value.
2. If data stack does not equal `caller_stack_height + type[code_section_index].inputs` items, execution results in exceptional halt.
4. Charges 4 gas.
...
2. If data stack does not equal `caller_stack_height + type[code_section_index].outputs` items, execution results in exceptional halt.
4. Charges 4 gas.
5. Pops nothing and pushes nothing to data stack.
