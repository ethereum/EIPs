...
4. If the `outputs` does not designate a non-returning function (set to `0x80`). The `output` value must be less than or equal to the sum of the `inputs` and `max_stack_increase`. An EOF container with an `outputs` value not set to `0x80` and that has a value greater than `inputs` and `max_stack_increase` is invalid.
