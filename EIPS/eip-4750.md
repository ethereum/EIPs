...
author: Andrei Maiboroda (@gumb0), Alex Beregszaszi (@axic), Paweł Bylica (@chfast), Greg Colvin, Brooklyn Zelenka (@expede)
...
If the execution of an instruction would violate a condition the execution is in an exceptional halting state.  The [Yellow Paper](https://ethereum.github.io/yellowpaper/paper.pdf) defines five such states.
1. Insufficient gas
...
4. `RJUMP` and `RJUMPI` immediate argument value (jump destination relative offset) validation:
   ...
   * Code section is invalid in case offset points to one of two bytes directly following `CALLF` instruction.
...
Rules 5, 6, and 7 require a symbolic execution of the code.  This algorithm does not consider the code’s data and computations, only its control flow and stack use.  This means we will reject programs with invalid code paths, even if those paths are not reachable.  This algorithm can also be seen as an acyclic traversal of the directed graph formed by calls, returns, and jumps.  Cycles are broken when a vertex which has already been visited is reached.  The complexity of this traversal is `O(|E|+|V|)`: The sum of the number of edges and number of vertices in the graph.

_Note: a similar algorithm is sketched out in EIP-615, and will eventually be given here._
...
execution, because there is arguably no caller for the starting function. This would mean that return stack is initialized as empty, and `RETF` exceptionally aborts when return stack is empty.
...
This change poses no risk to backwards compatibility, as it is introduced only for EOF1 contracts, forAlternative logic for executing `RETF` in the top frame could be to exceptionally halt  which deploying undefined instructions is not allowed, therefore there are no existing contracts using these instructions. The new instructions are not introduced for legacy bytecode (code which is not EOF formatted).
...
The following pseudo-C function lays out an algorithm for validating that EVM code satisfies rule 5, 6, and 7  above, assuming that all preceding rules are followed.  It is linear in time and space.

A call to `validate_subroutine(0, 0, 0)`, will traverse all of the reachable `bytecode`, recursing when `JUMP` and `JUMPI` instructions are encountered.  When a destination has been visited before it returns, thus breaking cycles.  It returns true if the subroutine is valid, false otherwise.
...
```
    ...
    validate_subroutine(PC, return_pc, SP)
    {
        ...
        while true
        {
            ...
            // check for stack underflow (many can be missed)
            if SP < n_removed
                return false
            ...
            // return to top or from recursion to CALLF
            if instruction is RETURNF
                return true;

            if instruction is CALLF
            {
...
For our purposes we define a safe EVM contract as one that cannot encounter an exceptional halting state. From the standpoint of security it would be best if unsafe contracts were never placed on the blockchain. Unsafe code can attempt to overflow stack, underflow stack, execute invalid instructions, and jump to invalid locations.  We cannot guarantee that all valid code is safe, but we can guarantee that valid contracts will not halt with an exception unless they either throw out of gas or overflow stack.
