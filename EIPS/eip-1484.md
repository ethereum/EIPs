...
Adds an array of `Providers` to the `Identity` referenced by the passed `EIN`. This must be called by a `Provider`. Requires a signature from the `approvingAddress`.
...
Removes an array of `Providers` to the `Identity` referenced by the passed `EIN`. This must be called by a `Provider`. Requires a signature from the `approvingAddress`.
