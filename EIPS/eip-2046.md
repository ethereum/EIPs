...
This motivation became more relevant after later repricing proposals
such as [EIP-1108](./eip-1108.md), which adjusted gas costs for elliptic
curve cryptography operations used by several precompiled contracts.

## Specification

After block `HF` the `STATICCALL` (`0xfa`) instruction charges a different
basic gas cost (`G_call` in [Yellow Paper]'s notation) depending on the
