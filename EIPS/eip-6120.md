...
2. For each `token` in `action.tokens`:
   * if `token.offset > 0`, transfer the following `amountOut` of token from `UTR` contract to `token.recipient`:
     * if `token.offset >= 32`, `amountOut = lastInputResult.slice(offset-32, offset)`
     * if `token.offset == 1`:
       * if `token.amount > 0`, `amountOut = token.amount`,
       * if `token.amount == 0`, `amountOut` is the current token balance owned by the `UTR` contract.
...
All application contracts that accept `recipient` (or `to`) argument instead of using `msg.sender` as the beneficiary (or recipient) are compatible with the UTR out of the box.
...
UniversalTokenRouter.exec([{
    ...
    tokens: [{
        ...
        id: 0,
        amount: 123,
        offset: 0,              // use the amount specified above
    ...
    tokens: [{
        ...
        adr: WETH.address,
        id: 0,
        amount: 0,  // entirely WETH balance of this UTR contract
...
Applications can also deploy additional adapter contracts to add a `recipient` to contract functions.

```solidity
// sample adapter contract for WETH
contract WethAddapter {
...
Application contracts that use `msg.sender` as the beneficiary using internal storage without any function for ownership transfer are incompatible with the UTR.
