...
The Universal Token Router separates the token allowance from the application logic, so users only need to approve tokens to one contract address and all applications can use it for their own logic. Universal Token Router transactions signed by users have clearly visible token transfer behavior, including token types (ETH, [EIP-20](./eip-20.md), [EIP-721](./eip-721.md) or [EIP-1155](./eip-1155.md)), `amountInMax`, `amountOutMin` and `recipient`.

The Universal Token Router contract is counter-factually deployed using `CREATE2` at address \<TBD\> across all EVM-compatible networks, so new token contracts can pre-configure it as a trusted spender and no approval transaction is necessary ever again.

## Motivation

Most application router contracts have the following pattern: approve/permit, (optional) calculation, transferFrom, action, and (optionally) verify the token output. This requires `n*m*k` `allow` (or `permit`) transactions, for `n` applications, `m` tokens and `k` user addresses. Even though user approves a contract to spend their tokens, it's the front-end code that they trust, not the contract itself. Anyone can create a front-end code and trick the users to sign a transaction interacting with the Uniswap Router contract and steal all their tokens that have been approved.

The Universal Token Router separates token allowance logic from application logic, and acts as a manifest for users when signing a transaction. It saves `(n-1)*m*k` approval transactions for old tokens and **ALL** approval transactions for new tokens.

With the support from wallets, users can see and review their expected token behavior instead of blindly trusting the application contracts and front-end code. Phishing sites will be much easier to detect and avoid for users.
...
Actions with `action.output == 0` declare which and how many tokens are transferred from `msg.sender` to `action.recipient`.

1. If the `action.data` is not empty, `action.code.call(action.data)` is invoked and the value returned is recorded in `amountIns` as a `bytes` for subsequence token transfer amounts.
...
Note: `amountIns` is the last value returned by an input action's code contract call. It can be shared by multiple input actions until another action's code is invoked. Using `amountIns` before any input action's code invocation can produce unexpected `amountIn` value.

### Output Action

Actions with `action.output > 0` declare the main application action to call, and optionally verify the output tokens after all is done.
...
The last `amountIns` bytes can be passed to an output action code call by:
...
EIP-721 has a special id `EIP_721_ALL = keccak256('UniversalTokenRouter.EIP_721_ALL')`, which can be used in output actions to verify the total amount of all ids owned by the `recipient` address.
...
The result of input action's `getAmountIns` will replace the `AMOUNT_INS_PLACEHOLDER` bytes, save the transaction from calculating twice of the same data.
...
The `tokens` verification of the last output action is not performed by Uniswap's legacy function and can be skipped. But the output token verification SHOULD always be done for the `UniversalTokenRouter` so user can see and review the behavior of transaction.
        ...
        for (uint i = 0; i < actions.length; ++i) {
            ...
            if (action.output > 0) {
                ...
                if (action.data.length > 0) {
                    ...
                    // ignore output action error if output == 1
