...
description: Universal Token Router generalizes transfer-and-call pattern so any token can be spent in a contract call the same way with ETH, without approving any other application contracts.
...
When users sign transactions to spend their approved tokens, they trust the front-end entirely to encode those transactions honestly and correctly. This puts them at great risk of phishing sites.
