...
* Freely update/upgrade/change their helper contract logic.
...
1. If the `action.code` is `0x0`, the `action.amount` is used for `amountIn`, otherwise `amountIn` is returned by the contract call `action.code.call(action.data)`. This contract function MUST return a single `uint256` value and this value MUST NOT greater than `action.amount`, otherwise reverts with "EXCESSIVE_INPUT_AMOUNT".
...
After all the actions are handled as above, every token balance tracked in Output Action #1 is queried again for comparison. The balance change MUST NOT be less than `action.amount` of each output action, otherwise reverts with "INSUFFICIENT_OUTPUT_AMOUNT".
