...
1. For each `token` in `action.tokens`, the current token balance of `recipient` is recorded for later verification.
...
* using a function with the last param is a `bytes` type.
...
`AMOUNT_INS_PLACEHOLDER` SHOULD only be used in output action for gas optimization, not as trusted conditions. Application contract code MUST always expect arbitruary, malformed or mallicious `bytes` data can be passed in where the `AMOUNT_INS_PLACEHOLDER` is expected.
