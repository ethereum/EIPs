...
Two consequences follow from the comparison. Legacy entries, which [EIP-8188](./eip-8188.md) decodes with `last_written_block = 0`, are below any valid `CUTOFF_BLOCK` and are therefore Inactive. State created in the window between [EIP-8188](./eip-8188.md)'s activation and `CUTOFF_BLOCK` is also Inactive, so its next write pays the surcharge even though it was written not long before the cutoff.
