...
A transfer between two unmodified accounts costs `8000` gas, matching the current effective cost; a transfer between two already-modified accounts costs only the base of `4056` gas.
...
* **Forwarders and routers** that receive `msg.value` and pass it on pay twice for what is, in state, a single transfer. Under this EIP the pass-through hop nets `4056` gas, since the intermediary's balance returns full circle to its original value.
* **Refund patterns** where a contract returns excess ether to the payer pay full price for balance changes that partially or fully cancel. A complete round trip (deposit and return within one transaction) drops from an effective `16000` to a net `8112` gas.
...
An `N`-hop pass-through chain costs an effective `8000 * N` today; under this EIP it nets `8000 + 4056 * (N - 1)`, a 37% reduction at `N = 4`.
...
| Constant | Value |
| - | - |
| `CALL_VALUE_BASE_GAS` | `CALL_STIPEND + TRANSFER_LOG_COST` (= `4056`) |
| `CLEAN_BALANCE_CHANGE_GAS` | `(ACCOUNT_WRITE - CALL_VALUE_BASE_GAS) / 2` (= `1972`) |
| `BALANCE_RESET_REFUND` | `= CLEAN_BALANCE_CHANGE_GAS` (= `1972`) |
| `CALL_STIPEND` | `2300` (unchanged) |

`ACCOUNT_WRITE` (currently `8000`) is the parameter of [EIP-8038](./eip-8038.md), and `TRANSFER_LOG_COST` (currently `1756`) is the parameter of [EIP-2780](./eip-2780.md); retuning them there retunes this schedule.
...
Two costs recur on every transfer regardless of prior account changes and form the base: the stipend granted to the callee and the [EIP-7708](./eip-7708.md) transfer log.
...
The remainder, `ACCOUNT_WRITE - CALL_VALUE_BASE_GAS = 3944`, is the first-change write premium, split evenly across the two modified accounts as `CLEAN_BALANCE_CHANGE_GAS = 1972` and charged only when the modified account is clean — giving `ACCOUNT_WRITE` the first-change semantics its definition claims.
...
The clean-clean total equals the current effective cost of `8000` exactly, and the dirty-to-dirty cost is the bare base of `4056` — the stipend and the log, the only resources that transfer consumes anew.
...
With an [EIP-7702](./eip-7702.md) delegated sender this becomes a loop: value out through the delegated account, value back from a cooperating contract, collecting `1972` of refund per round trip backed by no per-account charge at all.
...
* the clean branch of the balance change charge: `1972` charged, equal to the `1972` refunded at most once per divergence;
...
| `A→B x; A→B x` | 12056 | 0 | 12056 | 16000 |
| `A→B x; B→C x` | 14028 | 1972 | 12056 | 16000 |
| `A→B x; B→C y` | 14028 | 0 | 14028 | 16000 |
| `A→B x; B→A x` | 12056 | 3944 | 8112 | 16000 |
| `A→A x` (self-transfer) | 4056 | 0 | 4056 | 8000 |
| `CALLCODE` with value `x` | 4056 | 0 | 4056 | 8000 |
| `A→B x`, balance of `A` less than `x` | 8000 | 0 | 8000 | 8000 |

Worked example for `A→B x; B→C x`:
the first call charges `4056 + 1972 + 1972 = 8000` (both accounts clean), and its unused stipend is consumed rather than returned.
The second call charges the `4056` base, nothing for dirty `B`, and `1972` for clean `C`, totaling `6028`; because `B`'s new balance equals its original balance, `1972` is added to the refund counter.
...
* A transaction carrying an authorization for `B`, whose execution then performs `A→B x`: the transfer charges `4056 + 1972 = 6028`, since the applied authorization left `B` dirty.
...
* Contract `A` performs a `CREATE` and then `A→B x`: `A`'s nonce differs from its original, so the transfer charges `4056 + 1972 = 6028`.
...
Consequently a transaction with gas limit `G` can never cause more than `G` gas of execution, with no dependence on the size of the per-account charge — the invariant that must be preserved under any retuning is only `CALL_VALUE_BASE_GAS > CALL_STIPEND`, which holds by construction since the base contains the stipend plus the strictly positive `TRANSFER_LOG_COST`.
...
This EIP prices the log explicitly: `TRANSFER_LOG_COST` is a component of `CALL_VALUE_BASE_GAS`, charged and consumed on every transfer, so even the cheapest log-emitting transfer — the bare base of `4056` gas between two changed accounts — pays more than the `1756` cost of the equivalent `LOG3` with 32 data bytes.
