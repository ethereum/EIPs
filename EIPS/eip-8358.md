...
The additive stipend becomes a minimum callee gas limit: a value-bearing call runs its callee with at least `CALL_STIPEND` gas, and gas granted above the forwarded amount is not returned.
...
| `CALL_STIPEND` | `2300` (unchanged value; now a minimum callee gas limit) |

`CALL_VALUE_BASE_GAS` is `CALL_STIPEND + TRANSFER_LOG_COST` (currently `2300 + 1756 = 4056`, per [EIP-2780](./eip-2780.md)) rounded to `4000`; `ACCOUNT_WRITE` (currently `8000`) is the parameter of [EIP-8038](./eip-8038.md), and retuning it there retunes this schedule.
...
When the transferred value is nonzero, the call enforces a minimum callee gas limit of `CALL_STIPEND`: the callee frame starts with `max(forwarded_gas, CALL_STIPEND)`, where *forwarded gas* is the gas the caller makes available to the callee.
This floor replaces the additive stipend — no gas is added on top of *forwarded gas*.
Gas granted above *forwarded gas* is prepaid by `CALL_VALUE_BASE_GAS` and is not returned: when the callee frame completes, whether by success or by revert, the gas returned to the caller is `min(callee_gas_remaining, forwarded_gas)`, and if the call fails without creating a callee frame (insufficient balance or call depth limit) *forwarded gas* is returned.
When *forwarded gas* is at least `CALL_STIPEND`, a value-bearing call therefore has ordinary call semantics with no adjustment.
...
### Stipend as a minimum callee gas limit
...
Restating the stipend as a minimum callee gas limit preserves the floor while shrinking the special case: for forwarded gas of at least `CALL_STIPEND` a value call has ordinary call semantics, and only below it does the base-prepaid top-up exist, as use-it-or-lose-it gas.
...
For a value call forwarding at least `CALL_STIPEND` and whose callee consumes `s` gas, the caller's remaining gas afterwards is `G - charge - s` compared to `G - 8000 - s` today; since `charge <= 8000`, the caller is always left with at least as much gas, with equality exactly for a transfer between two unmodified accounts.
Below `CALL_STIPEND` of forwarded gas the caller does better still, as part of the callee's consumption is drawn from the prepaid floor.
...
As shown in [Rationale](#stipend-as-a-minimum-callee-gas-limit), the caller's remaining gas after a value call is always greater than or equal to what it would be under current rules, with equality only for transfers between unmodified accounts.
...
A callee's available gas becomes `max(forwarded_gas, CALL_STIPEND)` instead of `forwarded_gas + CALL_STIPEND`: a callee invoked with an explicitly forwarded budget receives up to `2300` less gas than before, while the floor guarantee that deployed zero-forwarding patterns (`transfer`/`send`) rely on is preserved exactly.
And a caller no longer receives unused floor gas back, which arises only when it forwarded less than `CALL_STIPEND`; contracts that measure `gasleft()` around value calls are never left with less gas than under current rules, because the reduced charge more than compensates.
...
Minimum callee gas limit cases, for a value call forwarding `g` gas to a callee that consumes `s` gas:

* `g = 0` (e.g. Solidity `transfer`): the callee runs with `2300` gas; no gas returns to the caller.
* `0 < g < 2300`: the callee runs with `2300` gas; `min(2300 - s, g)` gas returns to the caller.
* `g >= 2300`: the callee runs with `g` gas and unused gas returns in full — ordinary call semantics, with no additive stipend.
...
Tests should additionally cover: reverted sub-frames restoring cleanliness and rolling back refunds; the minimum callee gas limit under callee revert; transfers interleaved with `CREATE` endowments and `SELFDESTRUCT` sweeps; cleanliness of the transaction sender and recipient at the start of execution, including a self-sponsored authorization dirtying the sender; and the [EIP-3529](./eip-3529.md) refund cap.
...
A value-bearing call runs its callee with at least `CALL_STIPEND` gas.
Gas above what the caller forwarded exists only when the caller forwards less than `CALL_STIPEND`; it is bounded by `CALL_STIPEND`, prepaid by the base charge, and never returned.
