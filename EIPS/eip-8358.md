...
The stipend remains available to the callee, but unused stipend gas is no longer returned to the caller.
A transfer between two unmodified accounts costs `8000` gas, matching the current effective cost; a transfer between two already-modified accounts costs `4356` gas.
...
* **Forwarders and routers** that receive `msg.value` and pass it on pay twice for what is, in state, a single transfer. Under this EIP the pass-through hop nets `4356` gas, since the intermediary's balance returns full circle to its original value.
* **Refund patterns** where a contract returns excess ether to the payer pay full price for balance changes that partially or fully cancel. A complete round trip (deposit and return within one transaction) drops from an effective `16000` to a net `8712` gas.
...
An `N`-hop pass-through chain costs an effective `8000 * N` today; under this EIP it nets `8000 + 4356 * (N - 1)`, a 34% reduction at `N = 4`.
...
| `CLEAN_BALANCE_CHANGE_GAS` | `(ACCOUNT_WRITE - CALL_VALUE_BASE_GAS) / 2` (= `1922`) |
| `DIRTY_BALANCE_CHANGE_GAS` | `= WARM_ACCESS` (= `100`) |
| `BALANCE_RESET_REFUND` | `CLEAN_BALANCE_CHANGE_GAS - DIRTY_BALANCE_CHANGE_GAS` (= `1822`) |
...
The stipend is prepaid by the base and consumed in full.
The callee still receives *forwarded gas* `+ CALL_STIPEND`, where *forwarded gas* is the gas made available to the callee before the stipend is added, but stipend gas left unused is not returned to the caller: when the callee frame completes, whether by success or by revert, the gas returned to the caller is `min(callee_gas_remaining, forwarded_gas)`, and if the call fails without creating a callee frame (insufficient balance or call depth limit) only *forwarded gas* is returned.
The gas available inside the callee frame, including as observed via the `GAS` opcode, is unchanged.
...
`CALL_VALUE_BASE_GAS = CALL_STIPEND + WARM_ACCESS + TRANSFER_LOG_COST` collects the three costs every transfer incurs regardless of prior account changes: the stipend granted to the callee, the account touch, and the [EIP-7708](./eip-7708.md) transfer log.
The remainder is the first-change write premium, split evenly across the two modified accounts as `CLEAN_BALANCE_CHANGE_GAS = 1922`, and it is this component alone that collapses to `DIRTY_BALANCE_CHANGE_GAS` once an account is already changed — giving `ACCOUNT_WRITE` the first-change semantics its definition claims.
The clean-clean total equals the current effective cost of `8000` exactly, and every component of the dirty-to-dirty cost of `4356` maps to a real recurring resource: stipend, access, log, and two in-memory balance updates.
...
### The stipend is prepaid and consumed

Under current rules the stipend is conjured on top of the caller's charge and its unused portion is returned to the caller, so a parent frame ends a value call with up to `2300` gas it never paid for.
With the flat `CALL_VALUE` charge this rebate is harmless, but combined with per-account charges as low as `100` it becomes an exploit surface: the parent would recover most of a small charge, and the effective cost of a dirty transfer would collapse to `charge - 2300`.
This EIP instead prepays the stipend inside `CALL_VALUE_BASE_GAS` and consumes it in full — the callee receives it as use-it-or-lose-it gas, and the caller's payment is final.
...
With an [EIP-7702](./eip-7702.md) delegated sender this becomes a loop: value out through the delegated account, value back from a cooperating contract, collecting `1822` of refund per round trip against only `400` of dirty charges.
...
* the clean branch of the balance change charge: `1922` charged, versus at most `1822` later refunded;
...
As shown in [Rationale](#the-stipend-is-prepaid-and-consumed), the caller's remaining gas after a value call is always greater than or equal to what it would be under current rules, with equality only for a first-change transfer whose callee consumes no gas.
...
| `A→B x; A→B x` | 12356 | 0 | 12356 | 16000 |
| `A→B x; B→C x` | 14178 | 1822 | 12356 | 16000 |
| `A→B x; B→C y` | 14178 | 0 | 14178 | 16000 |
| `A→B x; B→A x` | 12356 | 3644 | 8712 | 16000 |
| `A→A x` (self-transfer) | 4256 | 0 | 4256 | 8000 |
| `CALLCODE` with value `x` | 4256 | 0 | 4256 | 8000 |
| `A→B x`, balance of `A` less than `x` | 8000 | 0 | 8000 | 8000 |

Worked example for `A→B x; B→C x`:
the first call charges `4156 + 1922 + 1922 = 8000` (both accounts clean), and its unused stipend is consumed rather than returned.
The second call charges `4156` base, `100` for dirty `B`, and `1922` for clean `C`, totaling `6178`; because `B`'s new balance equals its original balance, `1822` is added to the refund counter.
...
* A transaction carrying an authorization for `B`, whose execution then performs `A→B x`: the transfer charges `4156 + 1922 + 100 = 6178`, since the applied authorization left `B` dirty.
...
* Contract `A` performs a `CREATE` and then `A→B x`: `A`'s nonce differs from its original, so the transfer charges `4156 + 100 + 1922 = 6178`.
...
Under current rules this gas is conjured on top of the caller's charge; if any value call could be charged less than the stipend, stipend gas could recursively fund further value calls, each level of such a self-sustaining chain receiving a fresh conjured stipend and producing call frames, balance updates and [EIP-7708](./eip-7708.md) transfer logs paid for by gas the transaction never supplied.
This EIP closes the concern structurally:

* The stipend is prepaid — `CALL_VALUE_BASE_GAS > CALL_STIPEND`, and the base charge is applied on every value call — so the callee's stipend gas is always paid for by the caller.
* The stipend is consumed in full: the caller's gas after the call never exceeds its gas before the call minus the full charge.
...
This EIP prices the log explicitly: `TRANSFER_LOG_COST` is a component of `CALL_VALUE_BASE_GAS`, charged and consumed on every transfer, so even the cheapest log-emitting transfer — `4356` gas between two changed accounts — pays more than the `1756` cost of the equivalent `LOG3` with 32 data bytes.
