...
the first call charges `4000 + 2000 + 2000 = 8000` (both accounts clean), and its unused stipend is consumed rather than returned.
...
Consequently a transaction with gas limit `G` can never cause more than `G` gas of execution, with no dependence on the size of the per-account charge — the invariant that must be preserved under any retuning is only `CALL_VALUE_BASE_GAS > CALL_STIPEND`, currently `4000 > 2300` with a wide margin.
