...
If `v > 0xFF` then the modified version of the precompile MUST take precedence. Implementations MUST not parse `r` and `s` values, it MUST instead parse all of the remaining call data as `signature_info` and MUST calculate `alg_type` using the following method: `alg_type = v >> 8` where `>>` is a logical right-shift.
