...
<!-- Finish `### Algorithmic Transaction (RLP)` -->
<!-- Check `ALGORITHMIC_TX_TYPE` is not already defined, it shouldn't be. But I am paranoid.-->
<!-- Finish https://github.com/ethereum/EIPs/pull/10373 -->
...
<!-- TODO: RLP support -->
<!-- TODO: Decide whether signature data should still be `bytes` -->
<!-- TODO: Figure out how EIP-155/legacy should work -->
<!-- TODO: Write spec based on [sig_type, sig_data, tx_type, tx_data, additional_signatures] -->
<!-- TODO: Figure out whether to just nullify existing sig params or remove them. I am leaning towards nullifying for compat -->
...
The verify function MUST return either an error or return the full public key of the sender.
...
The below function MUST be used when deriving an address from a public key:
...
The penalty MUST be added onto the `21000` base gas of each transaction before the transaction is processed. A penalty MUST also be added for every instance of an `additional_signatures` object. If the payload's `gas_limit` is less than `21000 + penalty` then the transaction MUST be invalidated.
...
`secp256k1_validate` and `SECP256K1_SIGNATURE_SIZE` are defined from [EIP-6404](./eip-6404.md).
```python
def verify(signature_info: bytes, payload_hash: Hash32) -> Bytes
