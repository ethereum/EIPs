...
| `SIGRECOVER_PRECOMPILE_ADDRESS`| `Bytes20(0x0B)` |
...
This EIP also introduces a new precompile located at `SIGRECOVER_PRECOMPILE_ADDRESS`, this precompile MUST cost `SIGRECOVER_PRECOMPILE_BASE_GAS` when calling even if the precompile fails, this price MUST be aggregated with algorithm specific `GAS_PENALTY` and the output of `calculate_penalty` for the signature data.

The precompile logic executes the following logic:
```python

def sigrecover_precompile(input: Bytes) -> Bytes:
  # Recover signature length & type
  assert(len(input) >= 64)
  hash = input[:32]
  alg_type = input[32]
  sig_length = int.from_bytes(input[33:64], "little")

  # Ensure the algorithm exists & signature is correct size
...
The NULL algorithm (0xFF) MUST be present if the signature parameters inside the wrapped transaction are still valid for `secp256k1`, however there are more parameters in the transaction such as [EIP-7702](./eip-7702.md)'s `authorization_list` that require swapping out for different algorithm transactions.

The NULL algorithm MUST NOT be present if the transaction does not contain additional signatures, if it is the transaction MUST be treated as invalid.
