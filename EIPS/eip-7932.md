...
The precompile MUST either output:

- The address the signer if validation was successful and the first input byte of the precompile is 0x00
- The full public key of the signer if validation was successful and the first input byte of the precompile is 0x01
- No data if validation has failed
...
def sigrecover_precompile(input: Bytes) -> Bytes:
    ...
    should_return_public_key = input[0] == 1
    input = input[1:]
