...
The field `algorithm_type` represents the algorithm used to sign the transaction in the `payload` field which MUST still contain the `r`, `s`, and `y_parity` fields which MUST be set to `0x0`.
