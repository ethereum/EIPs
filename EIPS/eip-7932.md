...
Note: The "verification function" referred to by this EIP is per specified algorithm. When explicitly calling the "verification function" implementations MUST NOT call the precompile, but the function exposed by each algorithm.
...
def sigrecover_precompile(input: Bytes) -> Bytes:
  ...
  hash = input[:32]
  alg_type = input[32]
  sig_length = int.from_bytes(input[33:64], "big")
