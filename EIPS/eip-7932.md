...
The first byte of `signature_info` (`signature_info[0]`) represents the algorithm used to sign the transaction in the `payload` field. The `payload` transaction MUST still contain the `r`, `s`, and `y_parity` fields which MUST be set to `0x0`.
