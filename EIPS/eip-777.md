...
The token-contract MUST register the `ERC777Token` interface via EIP-820.
...
This call MUST:
 - call the `tokensToSend` method on the contract implementing `ERC777TokensSender` as returned by a EIP-820 lookup on the `from` address, regardless if `from` is a regular address or a contract.
 - call the `tokensReceived` method on the address implementing `ERC777TokensRecipient` as returned by a EIP-820 lookup on the `to` address, regardless if `to` is a regular address or a contract.
 - fire the `Sent` event

If `to` is a contract which is not prepared to receive tokens. Specifically, it is a contract which does not register an address (its own or another) via EIP-820 implementing the `ERC777TokensRecipient` interface. Then `send` MUST throw.
...
This call MUST:
 - call the `tokensToSend` method on the contract implementing `ERC777TokensSender` as returned by a EIP-820 lookup on the `from` address
 - call the `tokensReceived` method on the contract implementing `ERC777TokensRecipient` as returned by a EIP-820 lookup on the `to` address
 - fire the `Sent` event

If `to` is a contract which is not prepared to receive tokens. Specifically, it is a contract which does not register an address (its own or another) via EIP-820 implementing the `ERC777TokensRecipient` interface. Then `operatorSend` MUST throw.

The method MUST throw if:
 - `from` account balance does not have enough tokens to spend.
 - `to` is a contract which does not register an address (its own or another) via EIP-820 which implement the `ERC777TokensRecipient` interface.
 - `to` is a contract which is not prepared to receive tokens.
...
Every mint MUST call `tokensReceived` on the address implementing `ERC777TokensRecipient` for the `to` address as returned by a EIP-820 lookup.

Minting MUST follow the same rules as `send`/`operatorSend`. I.e. if `to` is a contract which is not prepared to receive tokens. Specifically, it is a contract which does not register an address (its own or another) via EIP-820 implementing the `ERC777TokensRecipient` interface. Then the minting MUST throw.

If the `to` register an address via EIP-820 implementing the `ERC777TokensRecipient` interface, `tokensReceived` MUST be called.
...
Any address (contract or regular account) CAN register a contract (itself or an other) implementing the `ERC777TokensSender` interface via the EIP-820 registry.
...
When tokens are sent as a result of burning, the `to` address MUST be `0x0` and the `userData` MUST be empty. `operator` MUST be the address who initiated the burning and `operatorData` MAY contain data.

When tokens are sent as a result of sending (`send` or `operatorSend`), the `to` address MUST be the one to which the tokens will be sent and MUST NOT be `0x0`. If it is a direct `send` (i.e. not an `operatorSend`) the `operator` MUST the address from which the tokens originate. That is the `from` and `operator` addresses MUST be equals.


### ERC777TokensRecipient

Any address (contract or regular account) CAN register a contract (itself or an other) implementing the `ERC777TokensRecipient` interface via the EIP-820 registry.
...
If the token-contract implements ERC-20, it MUST be register the `ERC20Token` interface via EIP-820.

The only difference for new contracts implementing ERC-20 is that registration of `ERC777TokensSender` and `ERC777TokensRecipient` via EIP-820 takes precedence over ERC-20. This means that even with on a ERC-20 `transfer` call, the token contract MUST check via EIP-820 if the `from`/`to` address implements `tokensToSend`/`tokensReceived` and call it if available. Note that when calling ERC-20 `transfer` on a contract, if the contract does not implement `tokensReceived`, the `transfer` call SHOULD still be accepted even if this means the tokens will probably be locked.
