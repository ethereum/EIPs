...
be $10^18$ of the internal denomination.
...
E.g. if a token contract returns a balance of `500,000,000,000,000,000` (0.5&times;$10^18$) for a user,
the user interface MUST show `0.5` tokens to the user.
If the user wishes to send `0.3` tokens,
the contract MUST be called with an amount of `300,000,000,000,000,000` (0.3&times;$10^18$).
...
|----------------------------------------|-----------------|---------------------------------|---------------------------------|
| `ERC777TokensRecipient` registered     | regular address | MUST call `tokensReceived`      | MUST call `tokensReceived`      |
| `ERC777TokensRecipient` registered     | contract        | MUST call `tokensReceived`      | MUST call `tokensReceived`      |
| `ERC777TokensRecipient` not registered | regular address | Continue                        | Continue                        |
