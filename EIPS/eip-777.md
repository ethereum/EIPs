...
6. Every token transaction contains `data` and `operatorData` `bytes` fields to be used freely to pass data from the token holder and operator, respectively.
...
- The *operator* MAY include information in `operatorData`.
...
*NOTE*: The `data` field contains information provided by the token holder similar to the data field in a regular ether send transaction. `data` is passed to both the `tokensToSend()` and `tokensReceived()` hooks, and either or both can use the information to decide if they wish to reject the transaction.  Alternatively, `data` can be used by the token holder to provide accountancy, audit or similar clerical information as part of the record of the transaction.  The `operatorData` field is analagous to the `data` field except it is provided by the *operator*.
...
> <small>`data`: Information attached to the send provided by the token holder.</small>
> <small>`operatorData`: Information attached to the send provided by the operator.</small>
...
*NOTE*: The `data` field contains information provided by the token holder similar to the data field in a regular ether send transaction. `data` is passed to both the `tokensReceived()` hook, which can use the information to decide if it wishes to reject the transaction.  Alternatively, `data` can be used by the token minter to provide accountancy, audit or similar clerical information as part of the record of the transaction.  The `operatorData` field is analagous to the `data` field except it is provided by the *operator*.
...
> <small>`data`: Information attached to the mint provided by the token holder.</small>
> <small>`operatorData`: Information attached to the mint provided by the operator.</small>
...
> <small>`operatorData`: Information attached to the burn provided by the operator.</small>
...
> <small>`data`: Information attached to the send provided by the *token holder* for a send; MUST be empty for a mint.</small>
> <small>`operatorData`: Information attached to the send provided by the operator.</small>
