...
If an account’s balance changes during a transaction, but its post-transaction balance is equal to its pre-transaction balance, then the change **MUST NOT** be recorded in `balance_changes`. However the corresponding addresses **MUST** still be included in `AccountChanges`.
