...
4. **Overhead analysis**: Historical data shows ~28 KiB average BAL size with ~4.5 KiB for balance diffs - reasonable for performance gains.
...
- Average: ~28 KiB (compressed)
- Balance diffs: ~4.5 KiB (compressed)
- Storage diffs: ~21.22 KiB (compressed)
...
Increased block size impacts propagation but overhead (~27 KiB average) is reasonable for performance gains.
