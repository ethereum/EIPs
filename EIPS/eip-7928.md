...
Note: For all address-accessing opcodes, the target address is only included in the BAL if all gas checks pass before the address would be warmed per [EIP-2929](./eip-2929.md). If gas shortage causes the operation to fail before address warming occurs, the target address MUST NOT be included. See [Gas Boundaries for Address Inclusion](#gas-boundaries-for-address-inclusion) for the specific gas boundary for each opcode.
...
### Gas Boundaries for Address Inclusion

For deterministic BAL generation, all clients MUST agree on when an address is considered "accessed". An address is included in the BAL if and only if all gas checks pass before the point where the address would be warmed per [EIP-2929](./eip-2929.md).

The following table specifies the gas boundary for each address-accessing opcode. An address is included in the BAL only if the operation has sufficient gas to cover all listed costs:

| Opcode | Gas costs that must be satisfiable before address inclusion |
|--------|-------------------------------------------------------------|
| `BALANCE` | Access cost (100 warm / 2600 cold) |
| `EXTCODESIZE` | Access cost (100 warm / 2600 cold) |
| `EXTCODEHASH` | Access cost (100 warm / 2600 cold) |
| `EXTCODECOPY` | Memory expansion cost, then access cost (100 warm / 2600 cold) |
| `CALL` | Memory expansion cost (input + output), access cost (100 warm / 2600 cold), positive value transfer cost (9000 if value > 0) |
| `CALLCODE` | Memory expansion cost (input + output), access cost (100 warm / 2600 cold), positive value transfer cost (9000 if value > 0) |
| `DELEGATECALL` | Memory expansion cost (input + output), access cost (100 warm / 2600 cold) |
| `STATICCALL` | Memory expansion cost (input + output), access cost (100 warm / 2600 cold) |
| `CREATE` | Memory expansion cost, initcode cost (2 gas per byte, [EIP-3860](./eip-3860.md)), then fixed creation cost (32000) |
| `CREATE2` | Memory expansion cost, initcode cost (2 gas per byte), hash cost (6 gas per word), then fixed creation cost (32000) |
| `SELFDESTRUCT` | Access cost for beneficiary (2600 if cold) |

The costs are evaluated in the order shown (left to right). If any cost check fails, subsequent checks are not performed and the target address is NOT included in the BAL.

**Example:** If `BALANCE` is called with 100 gas available and the target address is cold (requiring 2600 gas), the operation fails OOG before warming the address. The target address MUST NOT be included in the BAL.
