...
- CALL **senders** (value).
- CALL **recipients** (only if `value > 0`).
...
Consider an account with a pre-transaction balance of `x`. If during the course of a single transaction, the balance changes from `x`->`y`->...->`x` then this **MUST NOT** be recorded in `balance_changes`. However the corresponding addresses **MUST** still be included in `AccountChanges`.
