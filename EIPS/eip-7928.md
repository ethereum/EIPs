...
Transaction execution cannot be efficiently parallelized without knowing in advance which addresses and storage slots will be accessed. While [EIP-2930](./eip-2930.md) introduced optional transaction access lists, they are not enforced.

This EIP enforces access lists at the block level, enabling:
...
- Unchanged writes (same value written, gas consumed) - Note: These are rare in practice but MUST be included as reads in `storage_reads` when they occur
...
Implementations MUST pass test vectors that will be provided in the EIP repository, ensuring consistent SSZ encoding across all clients.

## Implementation Requirements

1. **SSZ**: Per [Ethereum SSZ spec](https://github.com/ethereum/consensus-specs/blob/dev/ssz/simple-serialize.md)
2. **Deterministic encoding**: Sorted as specified
3. **Hash**: `bal_hash = Keccak256(ssz.encode(BlockAccessList))`
