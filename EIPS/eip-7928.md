...
If an account’s balance changes during a transaction, but its post-transaction balance is equal to its pre-transaction balance, then the change **MUST NOT** be recorded in `balance_changes`. However all sender and recipient addresses **MUST** be included in `AccountChanges`.

This includes the special cases of a zero-value transfer, and calling SELFDESTRUCT on an account that had a zero pre-transaction balance.
