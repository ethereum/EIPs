...
The EL MUST retain BALs for at least `HISTORY_PRUNE_EPOCHS` (`=33024 epochs`), the retention window of block bodies, so that consensus clients can serve full execution payloads for as long as the p2p network requires them.
