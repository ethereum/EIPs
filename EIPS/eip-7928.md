...
| - | - | - |
| `BAL_RETENTION_EPOCHS` | 33024 | The consensus-layer block retention window in epochs |

Execution clients MUST retain block access lists that are not older than `BAL_RETENTION_EPOCHS` epochs.

Execution clients MAY locally prune block access lists that are older than `BAL_RETENTION_EPOCHS` epochs.
...
This proposal sets `BAL_RETENTION_EPOCHS` to 33,024 epochs, matching the consensus layer's block retention window and the history expiry window for headers, block bodies, and receipts. Some consensus clients rely on their paired execution client to store execution payloads and retrieve block access lists through `engine_getPayloadBodiesByHashV2` and `engine_getPayloadBodiesByRangeV2`. This ensures those clients can serve complete execution payloads throughout the period in which the consensus-layer p2p network requires them to be available. If the consensus layer changes its block retention window, `BAL_RETENTION_EPOCHS` is expected to be updated to match.

The consensus-layer retention window is derived from the maximum weak subjectivity period. It is therefore also long enough to support synchronization with re-execution after being offline for less than the weak subjectivity period. At roughly 70 KiB per block, retaining block access lists for 33,024 epochs adds about 76 GB of storage, compared with roughly 200 GB of block bodies retained for the same window.
