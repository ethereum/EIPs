...
1. **Size vs parallelization**: BALs include all accessed addresses (even unchanged) for complete parallel IO and execution. Omitting unchanged values would reduce size while maintaining execution parallelization benefits. 
...
4. **Transaction independence**: 60-80% of transactions access disjoint storage slots, enabling effective parallelization. The remaining 20-40% can only be parallelized by having post-transaction state diffs.
