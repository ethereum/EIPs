...
- **SENDALL:** For positive-value calls, the sender and beneficiary are recorded with a balance change.
- **SELFDESTRUCT (in-transaction):** Accounts destroyed within a transaction **MUST** be included in `AccountChanges` with empty changes. Storage keys within the self-destructed contracts that were modified or read **MUST** be included as a `storage_read`.
