...
2. Balance Diffs that track every address touched by value transfers along with their post-transaction balance values.
...
TxIndex = uint16
Balance = ByteVector(12)  # post balance value, unsigned
Nonce = uint64
...
- Each `BalanceChange` MUST include the transaction index and the post-transaction balance value
- 12 bytes are sufficient to represent the total ETH supply as an unsigned integer
...
def state_transition(block):
    ...
    for tx_index, tx in enumerate(block.transactions):
        ...
        for (addr, post_balance) in balance_changes:
            ensure_account(addr)
            account_changes[addr]['balance_changes'].append((tx_index, post_balance))
