...
The `NonceDiffs` structure MUST record the post-transaction nonce values for all `CREATE` and `CREATE2` deployer accounts and the deploayed contracts in the block. Contract deployments that revert based on [EIP-7610](./eip-7610.md) MUST NOT be included.
