...
`ITEM_COST` reflects the sum of `TX_ACCESS_LIST_STORAGE_KEY_COST` (1900, as defined in [EIP-2930](./eip-2930.md)) and `WARM_STORAGE_READ_COST` (100). The `storage_keys` is the total number of storage keys across all accounts, and `addresses` is the total number of unique addresses accessed in the block.
