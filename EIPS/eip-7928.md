...
The `NonceDiffs` structure MUST record the post-transaction nonce values for all `CREATE` and `CREATE2` deployer accounts and the deployed contracts in the block. This includes nonce increases that occur at the deployer contract even when deployments using `CREATE` or `CREATE2` revert, as specified in [EIP-7610](./eip-7610.md).
