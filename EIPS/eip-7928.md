...
`ITEM_COST` reflects the sum of `ACCESS_LIST_STORAGE_KEY_COST` (1900, as defined in [EIP-2930](./eip-2930.md)) and `WARM_STORAGE_READ_COST` (100, as defined in [EIP-2929](./eip-2929.md)). The `storage_keys` is the total number of storage keys across all accounts, and `addresses` is the total number of unique addresses accessed in the block.

With [EIP-7981](./eip-7981.md), the cheapest way to add an item to the BAL is a cold `SLOAD` at `COLD_SLOAD_COST` (2100). Using `ITEM_COST` = 2000 — deliberately below this minimum — creates a buffer of approximately `block_gas_limit / 42000` extra items, which absorbs BAL entries contributed by system contract execution (e.g., [EIP-2935](./eip-2935.md), [EIP-4788](./eip-4788.md), [EIP-7002](./eip-7002.md), [EIP-7251](./eip-7251.md)) and withdrawal recipients ([EIP-4895](./eip-4895.md)) that do not consume block gas.
