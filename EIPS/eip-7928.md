...
The EL MUST retain the BAL of every block for at least `BAL_RETENTION_EPOCHS = 33024` epochs after the block is imported, and MAY prune older BALs. This is the consensus layer's minimum block request window, `compute_min_epochs_for_block_requests()` in the consensus specs, which is 33,024 epochs (about 147 days) on mainnet, and the same window after which headers, block bodies and receipts may be expired from execution client storage under history expiry. It lets consensus clients that store only the BAL root, as permitted above, serve full execution payloads to peers via `engine_getPayloadBodiesByHashV2` and `engine_getPayloadBodiesByRangeV2` for as long as the consensus layer p2p network requires them, and it covers synchronization with re-execution after being offline for less than the weak subjectivity period.
...
### BAL Retention Window

An earlier revision required BALs to be retained only for the weak subjectivity period (3,533 epochs). That is enough for an execution client to re-execute recent blocks after downtime, but not for consensus clients that rely on their paired execution client to serve historical execution payloads. Consensus clients must serve blocks, and execution payload envelopes once payloads are separated from blocks, over `compute_min_epochs_for_block_requests()` epochs, and history expiry sets the execution layer's retention window for headers, bodies and receipts to the same 33,024 epochs for exactly this reason. A shorter BAL window would make the BAL the only part of an execution payload that a consensus client cannot recover from its execution client, forcing consensus clients to keep a redundant copy. At roughly 70 KiB per block, retaining BALs for 33,024 epochs adds about 76 GB to execution client storage, compared with roughly 200 GB of block bodies already retained for the same window.
