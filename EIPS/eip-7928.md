...
The EL MUST retain the BAL of every block for at least `BAL_RETENTION_EPOCHS = 33024` epochs after the block is imported, and MAY prune older BALs. This is the consensus layer's minimum block request window, `compute_min_epochs_for_block_requests()` in the consensus specs, and the window after which headers, block bodies and receipts may be expired from execution client storage under history expiry.
...
Consensus clients must serve blocks, and execution payload envelopes once payloads are separated from blocks, to their peers for `compute_min_epochs_for_block_requests()` epochs, which is 33,024 epochs (about 147 days) on mainnet. A consensus client that stores only the BAL root, as the Engine API section permits, depends on `engine_getPayloadBodiesByHashV2` and `engine_getPayloadBodiesByRangeV2` to rebuild full execution payloads for that whole window, in the same way it depends on its execution client for transactions and withdrawals. History expiry sets the execution layer's retention window for headers, bodies and receipts to the same 33,024 epochs for this reason, and the BAL window matches it so that no part of an execution payload leaves the execution client before the consensus layer stops serving it. The weak subjectivity period, which bounds how far back an execution client re-executes after downtime, is shorter than this window and is therefore covered as well. At roughly 70 KiB per block, retaining BALs for 33,024 epochs adds about 76 GB of execution client storage, compared with roughly 200 GB of block bodies retained for the same window.
