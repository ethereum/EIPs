...
This change removes gas observability from the new instructions and opening the door for new classes of contracts that are not affected by repricings. Furthermore, the legacy call instructions are rejected within EOF contracts, making sure they are mostly unaffected by changes in gas fees. Because these operations are required for removing gas observability they are required for EOF in lieu of the existing legacy instructions.
...
2. Performs memory expansion to `destOffset + size` and deducts memory expansion cost.
...
Current call instructions return a boolean value to signal success: 0 means failure, 1 means success.
...
Status code `2` is used for to signal exceptional failure in callee execution. Which means that an error which results in burning all gas occurred in callee frame.  
