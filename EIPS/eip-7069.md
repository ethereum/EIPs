...
Lastly, the introduction of the `RETURNDATA*` instructions ([EIP-211](./eip-211.md)) has obsoleted the output parameters of calls, mostly rendering them unused. An interesting problem to mention is the case of [ERC-20](./eip-20.md) where conflicting implementations caused a lot of trouble, where some would return something, while others wouldn't.
...
The functionality of specifying output buffer address is removed, because it is predominantly unused and implementers prefer to use `RETURNDATACOPY` instead. One exception is the case when the expected return size is known (i.e. non-dynamic return values), Solidity still uses the output buffer.
