...
12. Push a status code on the stack:
    12a. `0` if the call was successful.
    12b. `1` if the call has reverted.
    12c. `2` if the call has failed.
...
1. Use the 63/64th rule, but
    1a. ensure that at least `MIN_RETAINED_GAS` gas is retained prior to executing the callee,
    1b. ensure that at least `MIN_CALLEE_GAS` gas is available to the callee.
