...
9. Fail with status code `1` returned on stack, if the balance of the current account is less than `value`. Only gas charged until this point is consumed.
10. Fail with status code `1` returned on stack, if current call stack depth equals `1024`. Only gas charged until this point is consumed.
