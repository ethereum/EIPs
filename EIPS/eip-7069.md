...
Lastly, the introduction of the `RETURNDATA*` instructions ([EIP-211](./eip-211.md)) have obsoleted the output parameters of calls, mostly rendering them unused. An interesting problem to mention is the case of [ERC-20](./eip-20.md) where conflicting implementations caused a lot of trouble, where some would return something, while others wouldn't.
