...
See [`IERC5269.sol`](../assets/eip-5269/contracts/IERC5269.sol).
...
6. For any `EIP-XXX` that is NOT in `Final` status, when querying `supportEIP((any caller), xxx, (any minor identifier), [])`, it MUST NOT return `keccak256("FINAL")`, it is RECOMMENDED to return `0`.
