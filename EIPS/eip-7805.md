...
Each committee member is assigned an id from `0` to `f`. This is derived from the validator index, the lowest validator index is mapped to `0` and the highest to `f`.
...
- Nondeterminism: Introducing randomness can reduce overlap between ILs, leading to a larger IL when they are combined.
...
The selection process continues until the IL reaches its maximum size of `MAX_BYTES_PER_INCLUSION_LIST = 8 KiB`.
