...
- In each slot, a set of validators is selected as inclusion list (IL) committee members. Each member builds and gossips one IL containing transaction hashes according to their subjective view of the mempool.
- The proposer and all attesters of the next slot monitor, store and forward available ILs and their corresponding transaction sidecars.
- The proposer (or the builder if the block is not built locally by the proposer) includes transactions corresponding to transaction hashes from all collected ILs in its block. The proposer then broadcasts the block including IL transactions to the rest of the network.
- Attesters only vote for the proposer's block if it includes transactions corresponding to all transaction hashes from stored ILs.
...
- **`Slot N`, `t=0 to 8s`**:
IL committee members construct their ILs by selecting transaction hashes from transactions pending in the public mempool, and broadcast them (along with transaction sidecars) over the P2P network after processing the block for `slot N` and confirming it as the head. If no block is received by `t=7s`, they should run `get_head` and build and release their ILs based on the node’s local head.
...
6. The size of the IL sidecar data does not exceed the maximum size allowed (`MAX_BYTES_PER_INCLUSION_LIST = 8 KiB`).
7. The number of transaction hashes does not exceed `MAX_TRANSACTION_HASHES_PER_INCLUSION_LIST`.
8. All transaction hashes in the IL have corresponding transactions in the sidecar.
9. Each sidecar transaction's hash matches the corresponding hash in the inclusion list.
...
For each transaction `T` corresponding to the transaction hashes in ILs (obtained from sidecars), perform the following:
...
- Add `engine_updatePayloadWithInclusionListV1` endpoint to update a payload with the IL that should be used to build the block. This takes as an argument an 8-byte `payloadId` of the ongoing payload build process, along with the IL itself and corresponding transaction sidecars.
- Modify `engine_newPayload` endpoint to include a parameter for transaction hashes in ILs determined by the IL committee member. If the IL is not satisfied an `INVALID_INCLUSION_LIST` error must be returned.

#### IL Building

The rules for building ILs are left to the discretion of implementers. For instance, they may select transactions from the public mempool in various ways such as at random, by priority fee, or based on how long they have been pending. 

IL committee members construct inclusion lists containing transaction hashes (not full transactions) with a maximum of `MAX_TRANSACTION_HASHES_PER_INCLUSION_LIST` hashes per list. The full transactions corresponding to these hashes are provided as sidecars during network propagation. The overall IL size limit of `MAX_BYTES_PER_INCLUSION_LIST = 8 KiB` applies to the full sidecar data, not just the hash list.
...
#### Consensus layer sidecar handling

On the consensus layer, inclusion lists reference transaction hashes but not full transaction data. Instead of embedding full transactions in beacon blocks, they are propagated separately as "sidecars" during the P2P networking phase, similar to how blob data is handled in EIP-4844.

This sidecar design provides forward compatibility and reduces consensus layer bandwidth requirements by separating the signed commitments (transaction hashes) from the actual transaction data. The consensus layer is tasked with ensuring transaction hash availability and validation, while the execution layer handles the full transaction processing using the corresponding sidecars.
...
| `MAX_TRANSACTION_HASHES_PER_INCLUSION_LIST` | `uint64(2**8)` (=256) | 
...
class InclusionList(Container):
    ...
    transaction_hashes: List[Hash32, MAX_TRANSACTION_HASHES_PER_INCLUSION_LIST]
```
...
- Full transactions are provided as sidecars during P2P propagation, similar to blob sidecars in EIP-4844.

### Networking

Inclusion lists have two network representations. During IL gossip, the `SignedInclusionList` is wrapped with its corresponding transaction sidecars to become:

```
rlp([signed_inclusion_list, transactions])
```
...
The node MUST validate the relationship between the `SignedInclusionList` and transaction sidecars. To do so, ensure that:

- There are an equal number of `signed_inclusion_list.message.transaction_hashes` and `transactions`
- Each transaction hash matches its corresponding full transaction, i.e. `keccak256(rlp(transactions[i])) == signed_inclusion_list.message.transaction_hashes[i]`

For consensus layer propagation, only the `SignedInclusionList` objects are included in beacon blocks, while full transactions remain as sidecars, similar to how blobs are handled in EIP-4844.
