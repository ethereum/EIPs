...
Under Altair-and-later scoring with `INACTIVITY_PENALTY_QUOTIENT_BELLATRIX = 2^24`, a continuously offline validator's effective balance evolves as `balance(N)/balance(0) ≈ exp(-N²/(2Q))`. For finality to resume from an initial offline-stake fraction `f`, the offline share must fall below `1/3`, giving `N > sqrt(2 · Q · ln(2f/(1-f)))` epochs:
...
`8,192` epochs (`2^13`, ~36.4 days) covers leak-driven non-finality up to roughly 70% initial offline stake; beyond that envelope, recovery is expected to rely on external sync rather than local state. Encoding it as `8192 × 32 = 262,144` blocks (`2^18`) gives EL clients a single block-denominated value with no dependency on finalized beacon state.
