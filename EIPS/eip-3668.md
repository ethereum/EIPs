...
In step 1, a standard blockchain call operation is made to the contract. The contract reverts with an error that specifies the data to complete the call can be found offchain, and provides the url to a service that can provide the answer, along with additional contextual information required for the callback.

In step 2, the client calls the gateway service with the `callData` from the revert message. The gateway responds with an answer `response`, whose content is opaque to the client.
...
`url` specifies the URL to a service (known as the gateway) that implements the CCIP read protocol and can formulate an answer to the query. `url` can be the empty string `''`, in which case the client MUST specify the URL.
...
If the client successfully calls the gateway, the callback function specified in the `OffchainLookup` error will be invoked by the client, with `response` set to the value returned by the gateway, and `extraData` set to the value returned in the contract's `OffchainLookup` error. In a call context (as opposed to a transaction), the return data from this call will be returned to the user as if it was returned by the function that was originally invoked.
...
### Gateway Interface
The URL returned by a contract may be of any schema, but this specification only defines how clients should handle HTTPS URLs. Compliant gateways MUST accept POST requests with a Content-Type of `application/json`, matching the following schema:
```
{
    id: string,
    data: {
...
// Error result
{
    ...
    "error": {
        ...
        "error": "Could not decode input data"
    }
}
```
...
A client that supports CCIP read MUST make contract calls using the following process:
 ...
 1. Call the contract function normally. If the function returns a successful result, return it to the caller and stop.
 2. If the function returns an error other than `OffchainLookup`, return it to the caller in the usual fashion.
 3. Otherwise, decode the `sender`, `url`, `callData`, `callbackFunction` and `extraData` arguments from the `OffchainLookup` error.
 4. If the `sender` field does not match the address of the contract that was called, return an error to the caller and stop.
 5. Make an HTTP POST request to the URL specified in `url` or an override URL specified by the caller, if available, formatted as specified in [Gateway Interface](#gateway-interface).
 6. If the response code from step (5) is <200 or >299, return an error to the caller and stop.
 7. Otherwise, call the contract function specified by the 4-byte selector `callbackFunction`, supplying `data.result` from step (5) and `extraData` from step 3. Return the result of this call to the caller.

The lookup protocol for a client is described with the following pseudocode:

```javascript
async function durin_call(contract, functionName, args, gateway_url=undefined) {
    try {
        ...
        const {sender, url, callData, callbackFunction, extraData} = error.data;
        ...
        return await contract[callbackFunction](data.result, extraData);
    }
}
```

Where:
 - `contract` is an object representing the contract to call.
 - `functionName` is the name of the function to call on the contract.
 - `args` is an array of arguments to pass to the specified function.
 - `gateway_url` is an optional URL for the gateway, which overrides that returned by the contract if supplied.

If the function being called is a standard contract function, the process terminates after the original call, returning the same result as for a regular call. Otherwise, the gateway at `url` (or `gateway_url` if provided) is called with the `data` returned by the `OffchainLookup` error, and is expected to return a valid response. The response and the `extraData` are then passed to the specified callback function.
