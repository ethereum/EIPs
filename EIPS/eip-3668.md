...
In step 2, the client calls the gateway service with the `data` from the revert message. The gateway responds with an answer, whose content is opaque to the client but must start with the `prefix` specified by the contract.
...
#### `durin_call`
Accepts data provided by a supported contract, and returns a message (encoded as a call to the same smart contract) that can be used to generate and validate the response.
...
##### Parameters
 1. Object - The call object
   ...
   - abi: DATA - Optional. A JSON format ABI for the function being invoked, as specified in the [Solidity ABI encoding specification](https://docs.soliditylang.org/en/latest/abi-spec.html#json).
...
```
def durin_call(address, abi, args, gateway_url=None):
    ...
    try:
        ...
        response = json_rpc_call(gateway_url or url, 'durin_call', {'to': address, 'data': data, 'abi': abi})
...
If the function being called is a standard contract function, the process terminates after the original call, returning the same result as for a regular `eth_call`. Otherwise, the gateway at `url` (or `gateway_url` if provided) is called with the `data` returned by the `OffchainLookup` error, and is expected to return a valid response. The response is first checked against `prefix`, then used to call the original contract to produce the final result of the operation.
...
Using a revert, and conveying the `url` and `prefix` in the revert data, allows any function to be retrofitted to support lookups via CCIP read so long as the client understands the specification, and so facilitates translation of existing specificatiosn to use offchain data.

### Passing address and ABI to `durin_call`
