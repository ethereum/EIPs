...
While `POST_TX` frames dictate how assertions are executed, there is no explicit protocol-level requirement that a transaction *must* include a `POST_TX` frame. Smart accounts aiming to enforce a permanent on-chain policy must explicitly configure their validation logic (e.g., inside their `VERIFY` frame) to require the presence of their specific `POST_TX` frame.

Because a transaction can contain multiple `POST_TX` frames for different participants, and they are guaranteed to form a contiguous suffix at the end of the transaction, the smart account can iterate backwards to locate its required assertion frame. 

It is critical that the expected target of this frame (`this.postTxEnforcer`) is an immutable, non-upgradeable contract. If the assertion contract is upgradeable, a malicious transaction could potentially upgrade it during execution to bypass the security checks. Because the `VERIFY` frame executes *before* any state changes are made, checking the target during validation guarantees the transaction will be evaluated against the intended immutable policy.

Assuming future high-level language support for inspecting EIP-8141 frames (e.g., a `tx.frames` array), this can be enforced as follows:
