...
| 0x14    | must be 0                     | `gas_pre_charge` - the total amount deducted from the gas payer at transaction start for gas fees (`gas_limit × gas_price + blob_count × GAS_PER_BLOB × blob_base_fee`; zero blob term for non-blob transactions) |
...
Exposing `gas_pre_charge` directly lets callers subtract it with a single opcode call. It covers all gas-related deductions including the blob fee for [EIP-4844](./eip-4844.md) transactions, so the same subtraction isolates pure ETH transfers regardless of transaction type, including [EIP-8141](./eip-8141.md) frame transactions.
