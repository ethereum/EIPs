...
`TXTRACE` reports an event's topics and the byte length of its non-indexed data, but not the data itself, which does not fit in a single stack value. We introduce an `EVENTDATACOPY` opcode to copy that data into memory.
