...
* `asserter_address` - the address of an existing contract that will be called for the [Assertion Frame](#assertion-frame).
* `asserter_data` - the data to be passed as a call data to the Assertion Frame. In case the `asserter_address` is not set, this field acts as a [One-Time Restrictions Code](#one-time-restrictions-code).
...
The format of the returned transaction stack trace is described in [Transaction Trace Data Structure](#transaction-trace-data-structure).
