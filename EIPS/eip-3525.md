...
ERC-3525 introduces new token transfer models than ERC-721, including value transfer between two tokens of the same slot, as well as value transfer from a token to an address.
...
Both approaches have significant drawbacks. In the fungible way, one needs to create a separate ERC-20 contract for each different value or combination of customizable properties, which can easily require an unacceptable number of ERC-20 contracts in practice. On the other hand, there are no quantitative feature in an ERC-721 token, hence significantly reducing its computability, liquidity, and manageability. For example, if we create some different financial instruments like bonds, insurance policy, vesting plans using ERC-721, there are no standard interfaces to manipulate the value of them, e.g. if you want to transfer part of the equity contained in these contracts.
...
Since ERC-3525 is designed for representing semi-fungible assets, which most suits for digital assets like financial instruments, rather than arts or gaming items, the implementation should give out the metadata directly from contract code, rather than give a URL of a server for returning metadata.
