...
Tokenization is one of the most important patterns to manipulate digital assets in crypto. Normally there are two approches: fungible and non-fungible. The first one generally uses the ERC-20 standard, in the case that every unit of assets is identical to each other, ERC-20 standard provides a flexible and efficient way to manipulate fungible tokens. The second one predominately uses the ERC-721 token standard, for that digital asset needs to be distinguished from each other by an identity. 

Both approches have significant drawbacks. In the fungible way, one needs to create a separate ERC-20 contract for each different value or combination of customizable properties, which can easily require an unacceptable number of ERC-20 contracts in practice. On the other hand, there is no quantitative feature in an ERC-721 token, hence significantly reducing its computability, liquidity, and manageability. For example, if we create different bonds, inssurance policy, vesting plans using ERC-721, there is no standard interfaces to manipulate the value of them, e.g. if you want to transfer part of the equity contained in these contracts.
...
So that from the implementation perspective, this funtion can be done by creating a new token, transfer the value from the source token to the new one, and then transfer the new token to the recipient address. So that this transfer method is not fully independent from the ID to ID transfer method, it can be viewed as syntactical sugar that wraps the above three-step process.

However, for maximum compatibility with today's wallet software, value transfer to an address destination is needed, or else it wuold be too complex for wallet softwares to implement the desired transfer function, as well as too complex for human users to do the transfer mannuly using an graphical interface.
