...
This EIP is an ERC-721 compatible token standard, where each ERC-3525 token, like an ERC-721 token, contains an ID property to identify itself as a universally unique entity, so that the tokens can be transferred between addresses and approved to be operated in ERC-721 compatible way.
