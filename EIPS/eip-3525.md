...
ERC-3525 is ERC-721 compatible, which means, as an ERC-721 token, each ERC-3525 token contains an ID property to identify itself as a universally unique entity, so that tokens can be transferred be tween addresses and approved for manipulation in ERC-721 compatible way.

ERC-3525 token also contains an 'value' property, representing the quantitative nature of the token. The meaning of the 'value' property is quite like that of the 'balance' property of an ERC-20 token. Nevertheless, each ERC-3525 token has a 'SLOT' attribute, which is the logical catagory of tokens. This ensures the fungibility of the value property of tokens, that is, the value of two ERC-3525 tokens with the same slot can be treated as fungible. With these mechanics, a new transfer model is introduced, which enables transferring values between two tokens of same slot.
...
An intuitive and direct way to solve the problem is to add a property to represent the quantitative nature directly to an ERC-721 token, making it best for both property customization and fungibility. Furthermore, the ERC-721 compatibility would help the new standard easily utilize existing infrastructures and gain fast adoption.
...
The first one allows value transfers from one token (specified by fromTokenId) to another token (specified by toTokenId) with the same slot; 

The second one allows value transfers from one token (specified by fromTokenId) to an address. From the implementation perspective, this funtion can be done by creating a new token, transfer the value from one token to the new token, and then transfer the new token to the recipient address. So this transfer method is not fully independent from the first one. However, for maximum compatibility with today's wallet software, value transfer to an address destination is needed. This function can be viewed as syntactical sugar that wraps the above three-step process.

ERC-3525 also provides unsafe transfer mechanism, four maximum compatibility with ERC-721.
