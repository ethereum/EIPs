...
**TODO: recaculate identifier for this interface**
...
interface ERC3525 /* is ERC-721 */{
     ...
     */
    function splitTo(uint256 _tokenId, address[] calldata _to, uint256[] calldata _units) external returns (uint256[] memory);
     ...
     */
    function transferUnitsFrom(address _from, uint256 _tokenId, uint256 _targetTokenId, uint256 _units) external;
...
interface ERC3525SlotEnumerable {
    ...
    function supplyOfSlot() external view returns (uint256);
     ...
     */
    function supplyInSlot(uint256 _slot) external view returns (uint256);
...
interface ERC3525SlotManageable {
    ...
    function balanceOfManager(address manager) external view returns (uint256);
...
The underlying asset container extension is OPTIONAL for ERC-3525 smart contracts. This allows your contract have been a container for undelrying asset.

```solidity
interface ERC3525UnderlyingContainer {
    function totalUnderlyingAmount() external view returns (uint256);
    function underlying() external view returns (address);
}

interface ERC3525ERC20Container is ERC3525UnderlyingContainer {
    function units2UnderlyingAmount(uint256 units) external view returns (uint256 underlyingAmount);

    function underlyingAmount2Units(uint256 underlyingAmount) external view returns (uint256 units);
}

interface ERC3525ERC721Container is ERC3525UnderlyingContainer {
    function toERC721TokenId(uint256 erc3525TokenId) external view returns (uint256);
    function toERC3525TokenId(uint256 erc721TokenId) external view returns (uint256);
}
```
