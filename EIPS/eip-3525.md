...
This is a standard for semi-fungible tokens. The set of smart contract interfaces described in this document defines an ERC-721 extension/compatible token standard, by introducing an <ID, SLOT, VALUE> triple scalar model to represent the semi-fungible structure of a token. The standard also introduces new transfer models as well as approval models to reflect the nature of semi-fungibility.

## Abstract

ERC-3525 is an ERC-721 compatible token standard, which means, as an ERC-721 token, each ERC-3525 token contains an ID property to identify itself as a universally unique entity, so that tokens can be transferred between addresses and be approved for manipulation in ERC-721 compatible way.

Besides ERC-721 compatibility, ERC-3525 tokens also contain an 'value' property, representing the quantitative nature of the token. The meaning of the 'value' property is quite like that of the 'balance' property of an ERC-20 token. Nevertheless, each ERC-3525 token has a 'SLOT' attribute, which is the logical category of tokens. The slot mechanic ensures the fungibility of the value property of tokens, that is, the value of two ERC-3525 tokens with the same slot can be treated as fungible. 

ERC-3525 introduces new token transfer models than ERC-721, including value transfer between two tokens of same slot, as well as value transfer from a token to an address.

## Motivation

Tokenization is one of the most important patterns to manipulate digital assets in crypto. Normally there are two approaches: fungible and non-fungible. The first one generally uses the ERC-20 standard, in the case that every unit of assets is identical to each other, ERC-20 standard provides a flexible and efficient way to manipulate fungible tokens. The second one predominately uses the ERC-721 token standard, for that digital asset needs to be distinguished from each other by an identity. 

Both approaches have significant drawbacks. In the fungible way, one needs to create a separate ERC-20 contract for each different value or combination of customizable properties, which can easily require an unacceptable number of ERC-20 contracts in practice. On the other hand, there is no quantitative feature in an ERC-721 token, hence significantly reducing its computability, liquidity, and manageability. For example, if we create some different financial instruments like bonds, insurance policy, vesting plans using ERC-721, there is no standard interfaces to manipulate the value of them, e.g. if you want to transfer part of the equity contained in these contracts.

An intuitive and direct way to solve the problem is to make a token semi-fungible, having both identity and quantity, hence best fits for both property customization and fungibility. Furthermore, the ERC-721 compatibility would help the new standard easily utilize existing infrastructures and gain fast adoption.
...
Besides ERC-721 compatible token transfer methods, ERC-3525 introduce two new transfer models: value transfer from ID to ID, and value transfer from ID to address.
...
The first one allows value transfers from one token (specified by `_fromTokenId`) to another token (specified by `_toTokenId`) with the same slot; 

The second one allows value transfers from one token (specified by `_fromTokenId`) to an address (specified by `_to`), the value is actually transferred to a token owned by the address. 
...
- The value level `approve` function, overloaded with a `_value` parameter, SHOULD indicate that the authorized operator is capable of managing the specified maximum value of the specified token owned by the owner.
...
Since ERC-3525 is designed for representing semi-fungible assets, which most suites for digital assets like financial instruments, rather than arts or gaming items, the implementation should give out the metadata directly from contract code, rather than give a URL of a server for returning metadata.


### Design decision: Value transfer from token to address

The value of an ERC-3525 token is a property of the token, it cannot be directly held by an address. So that the value transferred to an address is actually transferred to a token owned by the address.

From the implementation perspective, this function can be done by creating a new token, transfer the value from the source token to the new one, and then transfer the new token to the recipient address. So that this transfer method is not fully independent from the ID to ID transfer method, it can be viewed as syntactic sugar that wraps the above three-step process.

However, for maximum compatibility with today's wallet software, value transfer to an address destination is needed, or else it wuold be too complex for wallet softwares to implement the desired transfer function, as well as too complex for human users to do the transfer mannualy using a graphical interface.
