...
author: Will Wang (@will-edge), Mike Meng <myan@solv.finance>, Ethan Y. Tsai <yee.tsai@gmail.com>, Ryan Chow <ryanchow@solv.finance>, Zhongxin Wu <wuzhongxin@solv.finance>, AlvisDu <dujincai@solv.finance>
 ...
 */
interface ERC3525 /* is ERC721 */{

    /**
     * @dev MUST trigger when the value of token ids are transferred, including zero value transfers.
     * @param `_from` The address of the owner of `_fromTokenId`
     * @param `_to` The address of the owner of `_toTokenId`
     * @param `_fromTokenId` The token to partially transfer
     * @param `_toTokenId` The token to receive the units transferred
     @ @param `_value` The amount of from token ID to transfer
     */
    event TransferValue(address indexed _from, address indexed _to, uint256 indexed _fromTokenId, uint256 _toTokenId, uint256 _value);

    /**
     * @dev This emits when a token is split into two.
     * @param `_from` The address of the owner of both `_fromTokenId`
     * @param `_fromTokenId` The tokens to be split
     * @param `_to` The list of address of the recipient
     * @param `_newTokenIds` The list of new tokens created after split
     @ @param `_values` The list of amount to be split from `fromTokenId` to `newTokenIds`
     */
    event SplitTo(address indexed _from, uint256 indexed _fromTokenId, address[] _to, uint256[] _newTokenIds, uint256[] _values);

    /**
     * @dev This emits when a token is merged into another.
     * @param `_owner` The address of the owner of both `_fromTokenId` and `_toTokenId`
     * @param `_fromTokenId` The token to be merged into `_toTokenId`
     * @param `_toTokenId` The token to receive all units of `_fromTokenId`
     @ @param `_mergeUnits` The amount of units to be merged from `_fromTokenId` to `_toTokenId`
     */
    event Merge(address indexed _owner, uint256 indexed _fromTokenId, uint256 indexed _toTokenId, uint256 _value);
     ...
     */
    event ApprovalValue(address indexed _owner, address indexed _approved, uint256 indexed _id, uint256 _value);
     ...
     * @return The number of decimals for value of a token id
     */
     ...
     * @return The amount of `_tokenId`
     */
    function balanceOfId(uint256 _tokenId) external view returns (uint256);

    /**
     * @dev Set or change the approved units of an operator for a token.
     * @param _to The address of the operator to be approved
     * @param _tokenId The token to approve
     * @param _value The amount of approved units for the operator
     */
    function approve(address _to, uint256 _tokenId, uint256 _value) external;
     ...
     */
    function split(uint256 _tokenId, uint256[] calldata _values) external returns (uint256[] memory);

    function splitTo(uint256 _fromTokenId, address[] calldata _to, uint256[] calldata _values) external returns (uint256[] memory);
     ...
     */
    function merge(uint256[] calldata _fromTokenIds, uint256 _toTokenId) external;

    /**
     * @dev Transfer units from a token to another token.
     ...
     */
    function transferFrom(uint256 _fromTokenId, uint256 _toTokenId, uint256 _value) external;
     ...
     */
    function transferFrom(uint256 _fromTokenId, address _to, uint256 _value) external returns (uint256);
     ...
     * @return The token created after transfer containing the transferred units
     */
    function safeTransferValueFrom(uint256 _fromTokenId, address _to,  uint256 _value, bytes calldata _data) external returns (uint256);
}
```
...
```solidity
interface ERC3525SlotEnumerable {

    function totalSlotSupply() external view returns (uint256);
...
The slot manageable extension is OPTIONAL for ERC-3525 smart contracts. This allows owner of slot manage slot.

```solidity
interface ERC3525SlotManageable {

    function slotCountOfManager(address manager) external view returns (uint256);

    function slotOfManagerByIndex(address manager, uint256 index) external view returns (uint256);

    function managerOfSlot(uint256 slot) returns (address);
}
```
 ...
 */
interface ERC3525Receiver {
    function onERC3525Received(address operator, address from, uint256 fromTokenId,
