...
The underlying assets container extension is OPTIONAL for ERC-3525 smart contracts. This allows your contract have been a container for undelrying assets.
```javascript
interface IERC3525UnderlyingAssetsContainer {
    /**
     * @dev Count all underlying assets holding in contract.
     * 
     * @return The number of underlying assets
     */
    function supplyOfUnderlyinyAssets() external view returns (uint256);
    
    /**
     * @dev Enumerate all underlying of the contract.
     * @param _index The index in the underlying list of the contract.
     * @return The address for the `_index` underlying in the underlying list of the contract
     */
    function underlyingAssetByIndex(uint256 index) external view returns (address);

    function totalAmountOfUnderlyingAsset(address underlying) external view returns (uint256);
    
    function balanceOfUnderlyingAsset(address owner, address underlying) external view returns (uint256);
}
```

The slot managable extension is OPTIONAL for ERC-3525 smart contracts. This allows owner of slot manage slot.
```javascript
interface SlotManagable {
    ownerOfSlot(uint256 slot) returns (address);
    modifySlot(uint256 slot, bytes slotData) onlySlotOwner;
}
```
