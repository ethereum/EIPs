...
A `block_access_list` field is added:
...
class Store(object):
    ...
    # [New in EIP-8143]
    ptc_bal_vote: Dict[Root, Vector[boolean, PTC_SIZE]] = field(default_factory=dict)
```
...
store.ptc_bal_vote[block_root] = [False] * PTC_SIZE
```
...
def on_payload_attestation_message(
    ...
    ptc_bal_vote = store.ptc_bal_vote[data.beacon_block_root]
    ptc_bal_vote[ptc_index] = data.block_access_list_present
```
...
BAL availability is required before processing the execution payload:
...
def on_execution_payload(store: Store, signed_envelope: SignedExecutionPayloadEnvelope) -> None:
    ...
    # [New in EIP-8143] Check BAL availability
    assert envelope.beacon_block_root in store.block_access_lists

    # Make a copy of the state to avoid mutability issues
    state = copy(store.block_states[envelope.beacon_block_root])

    # [Modified in EIP-8143] Process the execution payload with BAL
    process_execution_payload(
        state, signed_envelope, EXECUTION_ENGINE,
        block_access_list=store.block_access_lists[envelope.beacon_block_root],
    )
...
The function accepts a `block_access_list` parameter and passes it to the execution engine via `NewPayloadRequest`:
...
def process_execution_payload(
    ...
    verify: bool = True,
    # [New in EIP-8143]
    block_access_list: BlockAccessList = BlockAccessList(),
) -> None:
    ...
    # [Modified in EIP-8143] Include block_access_list in request
...
`engine_newPayloadV5` accepts a `blockAccessList` parameter alongside the `ExecutionPayload`. The EL validates by executing the block, computing the actual BAL, and verifying it matches the provided list and the `block_access_list_hash` in the block header.
...
### Early BAL Delivery to the EL

The BAL is useful to the EL before the execution payload arrives: it enables prefetching storage slots and calculating post-state roots. Two approaches exist for early delivery:

1. **Optional fields in `engine_newPayloadV5`**: The execution payload and related fields become optional, allowing the CL to call with only `blockAccessList` when the sidecar arrives ahead of the envelope. The EL begins prefetching immediately. When the payload arrives later, a second call supplies the remaining fields.
2. **Dedicated method**: A separate `engine_notifyBlockAccessListV1` call delivers the BAL independently. Simpler semantics (no optional fields) but adds a new engine API method.

This question is TBD.
