...
The `image` string should be a URI that resolves to an image.
The following image formats must be supported:
...
If the resolved image is a bitmap, its dimensions should not exceed 512x512 pixels.
Neither the URI nor the image it resolves to should have a size larger than 256kb.

### Returns

`boolean` - `true` if the the asset was added successfully, and an error otherwise. An error might occur in the following cases (but this list is not comprehensive):
