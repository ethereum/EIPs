...
`wallet_watchAsset` SHOULD check the ERC-20 `name()` and `symbol()` methods, and the contract `address` and `chainId` against a list of well-known tokens. If the name and/or symbol are similar to ones on the list, a warning SHOULD be presented to the user.
