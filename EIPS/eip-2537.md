...
The subgroup check **is mandatory** during the pairing call.
Implementations *should* use fast subgroup checks (we can also say that is must be performed by fast subgroup checks to have a speedup that results in a discount over naive implementation based on the `double-and-add` multiplication method that has a clear "worst case" - all bits are equal to one).
