...
discussions-to: https://ethereum-magicians.org/t/eip-XYZ-parent-governed-nestable-non-fungible-tokens/11914
status: Draft
...
![Nestable tokens](../assets/eip-XYZ/img/eip-XYZ-nestable-tokens.png)
...
interface IERCXYZ /* is ERC165 */ {
     ...
     *  child tokens.
     * @dev This method MUST NOT be called directly. It MUST only be called from an instance of `IERCXYZ` as part of a 
...
`nestTransfer` is always "safe" since it MUST check for `IERCXYZ` compatibility on the destination.
...
1. **Reject child token**\
![Reject child token](../assets/eip-XYZ/img/eip-XYZ-reject-child.png)
2. **Abandon child token**\
![Abandon child token](../assets/eip-XYZ/img/eip-XYZ-abandon-child.png)
3. **Unnest child token**\
![Unnest child token](../assets/eip-XYZ/img/eip-XYZ-unnest-child.png)
4. **Transfer the child token to an EOA or an `ERC721Receiver`**\
![Transfer child token to EOA](../assets/eip-XYZ/img/eip-XYZ-transfer-child-to-eoa.png)
5. **Transfer the child token into a new parent token**\
![Transfer child token to parent token](../assets/eip-XYZ/img/eip-XYZ-transfer-child-to-token.png)\
...
Tests are included in [`nestable.ts`](../assets/eip-XYZ/test/nestable.ts).
...
See [`NestableToken.sol`](../assets/eip-XYZ/contracts/NestableToken.sol).
