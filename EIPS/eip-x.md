...
With NFTs being a widespread form of tokens in the Ethereum ecosystem and being used for a variety of use cases, it is time to standardize additional utility for them. Having the ability to store token's attibutes on chain allows for greater utility of tokens as it fosters cross-collection interactivity and provides perpetual store of attributes.
...
/// @dev See https://eips.ethereum.org/EIPS/eip-XYZ
/// @dev Note: the ERC-165 identifier for this interface is 0x003f5b32.
...
- `METHOD_TYPEHASH` - The typehash of the method being called. The supported values, depending on the method are:
  - `SET_UINT_PROPERTY_TYPEHASH` - Used for setting uint attributes
  - `SET_STRING_PROPERTY_TYPEHASH` - Used for setting string attributes
  - `SET_BOOL_PROPERTY_TYPEHASH` - Used for setting bool attributes
  - `SET_BYTES_PROPERTY_TYPEHASH` - Used for setting bytes attributes
  - `SET_ADDRESS_PROPERTY_TYPEHASH` - Used for setting address attributes
...
keccak256(
    abi.encode(
        "ERC-XYZ: Public Non-Fungible Token Attributes Repository",
        "1",
        block.chainid,
        address(this)
    )
...
The `SET_UINT_PROPERTY_TYPEHASH` is generated as follows:
...
The `SET_STRING_PROPERTY_TYPEHASH` is generated as follows:
...
The `SET_BOOL_PROPERTY_TYPEHASH` is generated as follows:
...
The `SET_BYTES_PROPERTY_TYPEHASH` is generated as follows:
...
The `SET_ADDRESS_PROPERTY_TYPEHASH` is generated as follows:
...
The Attributes repository standard is fully compatible with [ERC-721](./eip-721.md) and with the robust tooling available for implementations of ERC-721 as well as with the existing ERC-721 infrastructure.
