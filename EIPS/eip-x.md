...
- Verify that the point formed by (`x`, `y`) values are on the curve.

### Precompiled Contract Specification

The `P256VERIFY` precompiled contract is proposed with the following input and outputs:
...
- **Output data:** 32 bytes of result data and error
    - If the signature verification process succeeds, it returns 1 in 32 bytes format, if not it returns 0 in 32 bytes format for the result with a null error.
    - If the given `x` and `y` coordinates are not valid, it returns null for the result with the given error.
