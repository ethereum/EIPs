...
description: Adds recursive STARK-based aggregation for quantum-resistant signatures and STARKs via an EIP-8141 frame mode
...
A dependency verification frame may appear at any position in the transaction. For [EIP-8141](./eip-8141.md) public mempool purposes it is transparent to the validation prefix: it is skipped when matching that prefix against a recognized shape, and its `limits.execution` does not count toward `MAX_VERIFY_GAS`.
...
### Why dependency frames are transparent to the validation prefix

[EIP-8141](./eip-8141.md) requires the validation prefix to match one of four recognized shapes, all built from `deploy`, `self_verify`, `only_verify` and `pay`. [Test Case 1](#test-case-1-simple-leansphincs-verification) and [Test Case 2](#test-case-2-multiple-leanstark-dependencies) each place a dependency verification frame before the approving `VERIFY` frame. Were such frames counted, both prefixes would be `[dep_verify, self_verify]`, which is none of the four, and neither transaction could be broadcast.

`MAX_VERIFY_GAS` bounds the EVM work a node performs simulating the prefix, and a dependency verification frame performs none of it. What bounds the work of parsing its declarations is `MAX_DEPENDENCIES_PER_FRAME` together with EIP-8141's `MAX_FRAMES`.
