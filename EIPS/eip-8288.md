...
for dep in sorted(all_deps):
    if dep not in filtered_deps and get_deps_hash([dep]) not in discard_hashes:
...
The recursive STARK circuit does not understand the structure of transactions or blocks beyond iterating over a flat list of `(scheme, data_hash, verification_key)` triples, their corresponding witnesses, and nested recursive STARKs.
...
3. Each transaction has `<= MAX_SIGS_PER_TX` leanSPHINCS dependency frames and `<= MAX_STARKS_PER_TX` leanSTARK dependency frames.
4. All transactions satisfy the normal (EIP-8141 or other) validity rules.
...
- `deps` = concatenation of all `(scheme, data_hash, verification_key_hash)` triples from all transactions in the wrapper
