...
In case of an error, a json is returned of this format based on the [GraphQL spec on error handling](https://graphql.github.io/graphql-spec/June2018/#sec-Errors):
...
The `message`, `locations`, and `path` fields are REQUIRED by the GraphQL spec (the latter two only if they can be specified).  To have more specific and machine-readable information on the error, clients are REQUIRED to provide an `errorCode` and an `errorMessage`.  If a client wants to provide extra information on the error, it can do so under the key of its name (e.g. "geth" or "parity").  The error codes are in accord with the [JSON RPC error codes improvement proposal](https://github.com/ethereum/wiki/wiki/JSON-RPC-Error-Codes-Improvement-Proposal).

There are cases where it's debatable if we want to consider them an error or a degenerate edge case, for example getting a block for a greater index than the current block height or getting blocks for a range where the lower index is greater than the upper index.  The rule of thumb is that if the client doesn't have to check the underlying data, it's enough to check certain assumption about the arguments, then it's an error.  If the client queries the underlying data to conclude it can't find some data (e.g. no block or account for a given hash), then it's considered a degenerate case and *"data"* gets returned.

What follows is a list of erronous queries and expected values of the returned error.

| Query                             | Error code | Error message | Main message                                                                        |
|-----------------------------------|------------|---------------|-------------------------------------------------------------------------------------|
| { block { account { balance }}}   | -32602     | Missing field | Field "account" argument "address" of type "Address!" is required but not provided. |
| { blocks(from:2,to:1) { hash }}   | -32602     | Invalid range | Invalid range: expectation failed "from" <= "to"                                    |
| { block(hash: "0xffff") { hash }} | -32602     | Wrong param   | Hash too short, expected length = 40, got 4                                         |
