...
description: a GraphQL schema for accessing data stored on an Ethereum node to complete replacement to the read-only information exposed via the present JSON-RPC interface
author: Nick Johnson (@arachnid), Raúl Kripalani (@raulk), Kris Shinn (@kshinn), Zainan Victor Zhou (@xinbenlv)
...
No need security concern introduced in this EIP.
