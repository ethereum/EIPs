...
created: 2018-06-26
requires: 137
discussions-to: https://github.com/ethereum/EIPs/pull/1185
---
...
The arguments for the function is as follows:
  - node: the nodehash of the fully-qualified domain in ENS for which to clear the records.  Node hashes are defined in #137

### dnsRecords(bytes32 node, bytes32 name, uint16 resource) view returns (bytes)

`dnsRecords()` obtains the DNS records for a given node, name and resource.
...
`hasDNSRecords()` reports if there are any records for the provided name in the domain.  This is needed by DNS resolvers when working with wildcard resources as defined in https://tools.ietf.org/html/rfc4592
