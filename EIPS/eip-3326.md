...
discussions-to: https://ethereum-magicians.org/t/eip-3326-wallet-switchethereumchain/5471
status: Draft
...
The method returns `null` if wallet switched its active chain, and an error otherwise.
...
`wallet_switchEthereumChain` is a simple method with a simple specification.
Its purpose is to provide dapps with a way of requesting to switch the wallet's active chain, which they would otherwise have to ask the user to do manually.
...
In the interest of preserving user privacy, implementers of `wallet_switchEthereumChain` should consider displaying user confirmations even in cases where the chain has yet to be added.
