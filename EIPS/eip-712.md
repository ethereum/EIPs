...
**Definition**: The _reference types_ are arrays and structs. Arrays are either fixed size or dynamic and denoted by `Type[n]` or `Type[]` respectively. They may be multidimensional, in which the brackets in the declaration are left-associative (i.e. `Type[][n]` refers to a fixed `n` size array of dynamic arrays `Type[]`). Structs are references to other structs by their name. The standard supports recursive struct types.
...
If the struct type references other struct types (and these in turn reference even more struct types), then the set of referenced struct types is collected, sorted by name and appended to the encoding. An example encoding is `Transaction(Person from,Person to,Asset[] assets)Asset(address token,uint256 amount)Person(address wallet,string name)`.
...
The atomic values are encoded as follows: Boolean `false` and `true` are encoded as `uint256` values `0` and `1` respectively. Addresses are encoded as `uint160`. Integer values are sign-extended to 256-bit and encoded in big endian order. `bytes1` to `bytes31` are arrays with a beginning (index `0`) and an end (index `length - 1`), they are zero-padded at the end to `bytes32` and encoded in beginning to end order. This corresponds to their encoding in ABI v1 and v2. These values must be present on the struct instance.

The dynamic values `bytes` and `string` are encoded as a `keccak256` hash of their contents. These values also must be present on the struct instance.
...
An array of atomic values is encoded as the `keccak256` hash of the concatenated `encodeData` of each element of the array (e.g. the encoding of `uint256[5]` is identical to that of a struct containing five members of type `uint256`).

An array of struct values is encoded as the `keccak256` hash of the concatenated encodings of each element, where each element is encoded as its `hashStruct` or `bytes(0)` if the element is null or missing from its slot.

An array of arrays gets encoded recursively, in which each child array is encoded into a `keccak256` hash, these hashes are concatenated, and the parent array gets encoded as the `keccak256` hash of the concatenated children arrays' hashes.

All specified arrays must be present on the struct instance, but dynamic arrays may be empty.
