...
![System Architecture of EIP-5521](../assets/eip-5521/Arch-1.png)
...
`Relationship`: a structure that contains `referring`, `referred`, `createdTimestamp`, and other customized and optional attributes (i.e.,g not necessarily included in the standard) such as `privityOfAgreement` recording the ownerships of referred NFTs at the time the rNFTs were being created or `profitSharing` recording the profit sharing of `referring`.
