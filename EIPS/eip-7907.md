...
| ----------------------- | ------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------- |
| Cold account and code   | Add `largeContractCost` to `GAS_COLD_ACCOUNT_ACCESS=2600`         | Contract not in access list nor accessed prior in the txn                                                           |
...
| Warm account and code   | No change to existing gas schedule. `WARM_STORAGE_READ_COST = 100` | `CALL`, `STATICCALL`, `DELEGATECALL`, `CALLCODE` and `EXTCODECOPY` previously in the txn (opcodes that load contract code) |
