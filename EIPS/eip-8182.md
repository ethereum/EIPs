...
* **leafIndex_u32**: The final note-tree leaf index assigned by the contract when the note is inserted.
...
* **recipientAddress**: A private witness in the transaction intent digest for private transfers. It identifies the registry entry whose `ownerNullifierKeyHash` must be used for output slot 0.
* **feeRecipientAddress**: A private witness in the transaction intent digest. The optional designated recipient of the private fee note in output slot 2. If `feeAmount > 0` and `feeRecipientAddress == 0`, the prover chooses output slot 2's nonzero owner address at proof generation time.
...
This EIP uses a single 2-input Poseidon primitive, `hash_2(a, b)`, defined as one permutation on state `[0, a, b]` returning output element 0. All generic `poseidon(x_0, ..., x_{n-1})` expressions are defined as an arity-prefixed wrapper over that primitive:

```
poseidon(x_0, ..., x_{n-1}) = hash_2(n, tree(x_0, ..., x_{n-1}))
```

where `tree(...)` is the binary tree obtained by the following recursive rule:

* `tree(x) = x`
* `tree(a, b) = hash_2(a, b)`
* For `n > 2`, the left subtree receives the largest power of 2 strictly less than `n` inputs and the right subtree receives the remainder; the root is `hash_2(tree(left inputs), tree(right inputs))`.

Worked examples:

* `poseidon(a, b, c) = hash_2(3, hash_2(hash_2(a, b), c))` — left subtree takes 2 inputs, right subtree takes 1.
* `poseidon(a, b, c, d) = hash_2(4, hash_2(hash_2(a, b), hash_2(c, d)))`.
* `poseidon(a, b, c, d, e) = hash_2(5, hash_2(hash_2(hash_2(a, b), hash_2(c, d)), e))` — left subtree takes 4 inputs, right subtree takes 1.
...
* all address-valued witnesses (`authorizingAddress`, `tokenAddress`, `recipientAddress`, `feeRecipientAddress`) MUST be constrained to `< 2^160`. Without this, field aliasing could produce commitments or public inputs that pass proof verification but bind to different addresses than the EVM expects.
...
`leafIndex_u32` is the sequential note-tree leaf index assigned by the contract at insertion time. This is the structural uniqueness source for notes.
...
`noteSecret` is the per-note hidden blinder. Wallets MUST NOT reuse `noteSecret` across notes they create, because reuse creates linkability. Nullifier safety does not depend on `noteSecret` uniqueness in this design because structural note uniqueness comes from `leafIndex_u32`.
...
`innerVkHash` is not an inner-circuit output; the outer circuit computes it from the verification key used for recursive verification, as follows. Let `vk[0] .. vk[114]` denote the 115 BN254 field elements of the `UltraHonkVerificationKey` witness in recursive-verifier order. The outer circuit MUST construct the initial list

```
L = [
  poseidon(AUTH_VK_DOMAIN, 115),
  poseidon(vk[0],   vk[1]),
  poseidon(vk[2],   vk[3]),
  ...,
  poseidon(vk[112], vk[113]),
  vk[114]
]
```

(59 elements: one domain-arity header, 57 pair hashes of `vk[0..113]`, and the final odd word `vk[114]`), then recursively fold adjacent pairs with `poseidon(left, right)` — carrying the odd final element upward unchanged at each level — until one element remains. That element is `innerVkHash`.
...
* If `isDummy == 0` (real output):
  * real outputs MUST have `amount > 0`,
  * the circuit MUST compute `ownerCommitment = poseidon(OWNER_COMMITMENT_DOMAIN, ownerNullifierKeyHash, noteSecret)`,
  * the circuit MUST compute `noteBodyCommitment = poseidon(NOTE_BODY_COMMITMENT_DOMAIN, ownerCommitment, amount, tokenAddress, originTag)`,
  * the circuit MUST enforce `noteBodyCommitment == public noteBodyCommitment_i`.
* If `isDummy == 1` (dummy output):
  * `amount == 0`,
  * `tokenAddress == 0`,
  * `originTag == 0`,
  * `ownerNullifierKeyHash == DUMMY_OWNER_NULLIFIER_KEY_HASH`,
  * the circuit MUST still derive `noteSecret` deterministically and compute the dummy `noteBodyCommitment`.

Additional per-mode constraints:

* **Transfer**
  * output slot 0 is the recipient payment: `isDummy == 0`, `ownerNullifierKeyHash` must equal the recipient's registry `ownerNullifierKeyHash`, `amount` must equal the authorized private amount, and `tokenAddress` must equal the authorized private token.
  * output slot 1 is sender change or dummy: if `isDummy == 0`, `ownerNullifierKeyHash` must equal the sender's registry `ownerNullifierKeyHash`.
  * output slot 2 is a fee note or dummy.
* **Withdrawal**
  * output slot 0 is sender change or dummy: if `isDummy == 0`, `ownerNullifierKeyHash` must equal the sender's registry `ownerNullifierKeyHash`.
...
* if slot 2 is real, slot 2's `amount` MUST equal `feeAmount`,
* if `feeAmount > 0` and `feeRecipientAddress != 0`, then output slot 2's `ownerNullifierKeyHash` MUST equal the registry `ownerNullifierKeyHash` of `feeRecipientAddress`,
* if `feeAmount > 0` and `feeRecipientAddress == 0`, the prover chooses a nonzero owner address and proves its registry entry.
...
* **Transfer**
  ...
  * if `feeAmount != 0`, prove the fee-recipient address has a user-registry entry,
...
* `feeRecipientAddress` MAY be zero. See Section 9.5 for how output slot 2's owner is selected when `feeRecipientAddress == 0`.
...
`leafIndex_u32` is the assigned deposit note-tree leaf index and makes tagged deposits unique.
...
| noteCommitment | `NOTE_COMMITMENT_DOMAIN, noteBodyCommitment, leafIndex_u32` | 3 |
...
| Inner VK hash | initial list `[poseidon(AUTH_VK_DOMAIN, 115), poseidon(vk[0], vk[1]), ..., poseidon(vk[112], vk[113]), vk[114]]`, then binary-tree fold with `poseidon(left, right)` carrying the odd final element upward unchanged | 115-word VK |
| Deposit origin tag | `ORIGIN_TAG_DOMAIN, block.chainid, uint160(msg.sender), uint160(token), amount, leafIndex_u32` | 6 |
...
The Merkle tree node row uses `hash_2(left, right)` directly, not the arity-prefixed wrapper.
...
For ordinary `transact` outputs, scheme `1` plaintext is the five note fields in note-commitment-base order, each encoded as a 32-byte big-endian word:
