...
description: A canonical validity layer for private transfers via a system contract and verifier precompile.
...
1. A **system contract** deployed at a protocol-defined address, holding all shielded pool state (e.g., note commitment tree, nullifier set, intent-nullifier set, user registry, delivery-key registry).
...
6. A **public-input interface** for proofs and required contract execution checks.
7. A **per-note label** tracking deposit lineage.
8. A **proof verification precompile** for gas-feasible proof verification.
...
* **`lineageConstraintKind`**: Private witness in the canonical intent digest. A signer-selected coarse lineage policy: `0` means unconstrained and `1` means forbid `MIXED_LABEL` on real outputs.
...
* `MAX_INTENT_LIFETIME = 86400` (subject to change before Review) — maximum allowed `validUntilSeconds` offset from `block.timestamp`, in seconds (24 hours). <!-- TODO: Pin MAX_INTENT_LIFETIME before Review. -->
...
**Third-party proving.** The user signs an intent and delegates proof generation to a specialized proving service. The prover learns all transaction details and retains discretion over coin selection and registry root selection within the valid history window. Lineage discretion is therefore partially constrainable, not eliminated: without `lineageConstraintKind = LINEAGE_FORBID_MIXED`, a malicious prover can intentionally merge differently labeled notes and force `MIXED_LABEL`, degrading future provenance/selective-disclosure utility while still honoring the authorized payment semantics; with `lineageConstraintKind = LINEAGE_FORBID_MIXED`, proofs whose real outputs would carry `MIXED_LABEL` fail. It cannot forge unauthorized operations, redirect payments, or extract funds — these properties are enforced by the proof system regardless of prover behavior. However, because the protocol does not authenticate or validate note-delivery payload semantics, a malicious prover can still choose unusable `outputNoteData` at proving time and render an in-flight transfer's output notes unrecoverable.
...
Constraints: the inner circuit MUST conform to the inner-proof envelope (Section 9.1). The `innerVkHash` encoding follows the Barretenberg canonical serialization order (Section 9.1, step 2; to be pinned before Review). Companion ERCs MUST authenticate all canonical digest fields. Companion ERCs MUST domain-separate authorizations such that an authorization valid for one `innerVkHash` is invalid for any other `innerVkHash`. Auth methods requiring a different proof system need a hard fork that updates the outer circuit. <!-- TODO: Pin canonical innerVkHash encoding before Review. -->
...
Coin selection is delegated to the prover. The intent binds payment semantics (recipient, amount, token, operation type) and MAY bind the coarse lineage policy via `lineageConstraintKind`, but it does not bind exact note selection or exact output labels. Operation-type binding is the inner circuit's responsibility via `operationKind` in the canonical intent digest.
...
* `intentDigest` — the canonical authorization hash (Section 9.11). Because the digest encodes the full semantic authorization (including `policyVersion`, `authorizingAddress`, `operationKind`, `lineageConstraintKind`, `rawNonce`, and `executionChainId`), it uniquely identifies the authorized action. Replay protection is keyed to the canonical intent digest, which includes `policyVersion`. Since `policyVersion` is per `(address, innerVkHash)`, two auth methods with different versions produce different digests and independent intent nullifiers. When two auth methods share the same `policyVersion` (e.g., both at version 1 after initial registration), the same semantic intent produces the same digest and intent nullifier — executing via one method prevents execution via the other.
...
The deposit-specific fields (`depositorAddress`, `recipientAddress`, `amount`, `feeRecipientAddress`, `feeAmount`, `lineageConstraintKind`, `publicAmountIn`, `publicTokenAddress`) are bound through the canonical intent digest and value-conservation rules (Section 9.11).
...
* `lineageConstraintKind` binds only a coarse lineage policy. It does not bind exact note identities or exact output labels.
...
`authorizingAddress`, `policyVersion`, `innerVkHash`, `authDataCommitment`, and `lineageConstraintKind` are NOT public inputs — they are private, authenticated through the inner proof and canonical intent digest, and known only inside the circuit.
...
The precompile verifies proofs for the outer circuit (UltraHONK/BN254; to be pinned before Review). The verification key is fork-defined. Exact proof serialization and verification key formats MUST be pinned before Review. <!-- TODO: Pin outer proof serialization and verification-key formats before Review. -->
...
| Canonical intent digest | `INTENT_DIGEST_DOMAIN, policyVersion, authorizingAddress, operationKind, poolAddress, tokenAddress, recipientAddress, amount, feeRecipientAddress, feeAmount, lineageConstraintKind, rawNonce, validUntilSeconds, executionChainId` | 14 |
...
The Merkle tree node row uses `hash_2(left, right)` directly — not the arity-prefixed `poseidon(...)` construction (Section 3.3). All other rows use the arity-prefixed form. The Inner VK hash row's exact serialization, padding, and arity MUST be pinned before Review (see Inner VK Hash in Section 9.1). <!-- TODO: Pin Inner VK hash serialization, padding, and arity before Review. -->
...
This EIP also supports non-custodial proof delegation: a user can outsource proof generation to a third party without outsourcing spending authority. The prover cannot steal funds, redirect payments, or forge unauthorized transactions. Lineage discretion is partially constrainable via `lineageConstraintKind = LINEAGE_FORBID_MIXED`, but delivery sabotage remains a residual base-protocol risk because `outputNoteData` semantics are not authorized by the proof relation. Rotating `outputSecret` cuts off a former prover's ability to derive future output randomness for that address. Because note delivery is not coupled to the proof system, the X-Wing baseline and any later delivery schemes can evolve without changing the proof relation.

### Coarse Lineage Policy

`lineageConstraintKind` exists to address a real prover-discretion harm without attempting full coin-selection canonicalization. It gives wallets and auth policies a way to say "this spend is invalid if it would produce mixed-lineage real outputs" while leaving exact note identities and exact output labels out of the canonical authorization. This keeps the protocol change narrow: the signer can opt into pure-lineage spends, but the protocol still does not attempt to bind exact input-note selection or otherwise eliminate prover discretion over coin selection.
...
The base protocol hash-binds `outputNoteData` bytes but does not authenticate or validate their semantics. A remote prover with full witness can therefore still emit unusable note-delivery payloads while producing a valid proof. This cannot steal funds or redirect payment, but it can make note recovery fail. Examples of higher-assurance mitigations include wallet-controlled finalization/broadcast, TEE-attested proving, MPC or witness-splitting systems that prevent any one prover from constructing an alternate proof, or future companion standards for authenticated delivery binding. These mitigations are not standardized by this EIP.
