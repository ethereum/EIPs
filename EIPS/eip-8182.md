...
3. A **recursive proof composition** separating auth (permissionless auth circuits) from protocol invariants (hard-fork-managed pool circuit) for note spending.
4. An **auth policy registry** binding `(address, authVkHash)` pairs to auth credentials, supporting multiple auth policies per address.
...
* **Pool circuit**: The hard-fork-managed circuit that enforces protocol invariants for note spending: value conservation, nullifiers, Merkle membership, deterministic note-secret derivation for ordinary outputs, auth proof verification, and auth policy checks.
* **Auth circuit**: A permissionless circuit that handles authentication and intent parsing. Outputs `[authDataCommitment, transactionIntentDigest]`. `authorizingAddress` and `policyVersion` are supplied to the pool circuit as private witnesses and authenticated through `transactionIntentDigest`. See Section 9.1.
* **Auth policy**: A binding of `authVkHash`, `authDataCommitment`, and `policyVersion` in the auth policy registry. Each address may have multiple auth policies, one per auth circuit. See Sections 5.2 and 6.4.
* **Transaction intent digest**: The canonical digest of the contemplated private-note spend. It includes the signer-authenticated transaction fields and signer-selected execution constraints. The auth circuit authenticates this digest from the signed intent fields and any companion-standard constants; the pool circuit recomputes the same formula from witnesses, public inputs, and mode-derived values.
...
* **policyVersion**: A monotonically increasing counter per (address, authVkHash) pair. Authenticated by the auth circuit's signed artifact. See Section 6.4.
...
| `AUTH_VK_DOMAIN` | `auth_vk` | Auth circuit VK hashing |
...
**Pool circuit** (hard-fork-managed). There is exactly one pool circuit; it can only change via hard fork. It enforces all protocol invariants for `transact`: value conservation, nullifier derivation, Merkle membership, deterministic note-secret derivation for ordinary outputs, user-registry checks, auth-policy checks, and token consistency. It also recursively verifies an auth proof as part of its own verification. The pool circuit is the security boundary — a bug here can compromise the entire pool.

**Auth circuit** (permissionless). Anyone can write and deploy an auth circuit. It handles authentication — verifying the user's credential — and intent parsing — computing the transaction intent digest over transaction fields and any signer-selected execution constraints. It outputs two public values: `[authDataCommitment, transactionIntentDigest]`. The pool circuit uses private witnesses for `authorizingAddress` and `policyVersion`, authenticates them through `transactionIntentDigest`, checks `authDataCommitment` against the auth-policy leaf, and checks `transactionIntentDigest` against its own independent recomputation from execution data plus finalized-output bindings. Section 9 specifies the full per-mode constraints.

**How they compose.** A prover supplies the auth proof and the auth verification key (`authVkey`) as private witnesses to the pool circuit. The pool circuit recursively verifies the auth proof, computes `authVkHash` from `authVkey`, and uses it to look up the auth policy registry leaf. Because `authVkey` is a private witness, on-chain observers cannot determine which auth circuit (and therefore which auth method) was used for a given private-note spend. Section 9.1 specifies the full normative interface.

| Responsibility | Circuit | Fork required? |
|----------------|---------|----------------|
| Value conservation | Pool | Yes |
| Nullifier derivation | Pool | Yes |
| Merkle membership | Pool | Yes |
| Deterministic ordinary note-secret derivation | Pool | Yes |
| Auth proof verification | Pool | Yes |
| Auth policy registry check | Pool | Yes |
| Intent replay ID derivation | Pool | Yes |
| Canonical transaction-intent-digest computation | Pool | Yes |
| Signature verification | Auth | No |
| Intent parsing | Auth | No |
| Auth data commitment binding | Auth | No |
| policyVersion authentication | Auth | No |

The pool circuit enforces protocol invariants that protect the entire pool. A weakened pool circuit could drain all funds. The auth circuit handles auth — a weakened auth circuit can only risk the registering user's funds. This separation is what makes permissionless auth circuits safe.

**Auth method anonymity.** All auth methods share a single pool circuit. `authVkHash` is never a public input — it is checked inside the circuit against the auth policy leaf. On-chain observers cannot determine which auth method was used for a given private-note spend. Auth policy registration is public (`authVkHash` appears in the `AuthPolicyRegistered` event); the privacy property is transaction-time only.

**Output note delivery.** `outputNoteData0`, `outputNoteData1`, and `outputNoteData2` are hash-bound to the proof via `outputNoteDataHash0`, `outputNoteDataHash1`, and `outputNoteDataHash2` (public inputs). The signer MAY additionally lock a slot's emitted semantic note commitment to its payload hash through `outputBinding`. The auth circuit has no scheme-specific role in note delivery, and the pool circuit does not enforce any encryption scheme or delivery format. Section 14 defines the registry lookup and the scheme-`1` interpretation.
...
`getAuthPolicy` returns whether the `(user, authVkHash)` pair is currently active plus the current `authDataCommitment` and `policyVersion` for that pair. It MUST reject `authVkHash >= p`. If the pair was never registered, it returns `(false, 0, 0)`. After deregistration, it returns `(false, lastAssignedAuthDataCommitment, lastAssignedPolicyVersion)`.
...
`registerAuthPolicy` is called by `msg.sender` to bind the `(address, authVkHash)` pair to an auth data commitment. `authDataCommitment` is opaque. The caller MUST already have a user-registry entry. A single address may register multiple auth policies (one per `authVkHash`); each has its own independent `policyVersion`.

* MUST reject `authVkHash >= p` or `authDataCommitment >= p` to prevent field aliasing between the Poseidon tree key (which reduces mod `p`) and the keccak-based policy-assignments mapping key (which does not).
* Computes the auth-policy tree key as `uint160(poseidon(AUTH_POLICY_KEY_DOMAIN, msg.sender, authVkHash))` (low 160 bits; see Section 3.4).
* Computes the per-pair assignment key as `keccak256(abi.encodePacked(msg.sender, authVkHash))` and increments `policyVersion` for that pair (starting from 1 on first registration).
...
`deregisterAuthPolicy` is called by `msg.sender` to remove an auth policy. The contract MUST reject `authVkHash >= p` to prevent field aliasing at the auth-policy tree key. The contract writes `0` (the empty leaf) at the auth-policy tree key `uint160(poseidon(AUTH_POLICY_KEY_DOMAIN, msg.sender, authVkHash))`. The policy-assignments mapping entry at the per-pair assignment key MUST NOT be cleared; it retains the last assigned `authDataCommitment` and `policyVersion` for `getAuthPolicy` introspection. Deregistration must be called directly by the account holder. After stale auth-policy roots expire, no proof against that `(address, authVkHash)` pair can succeed. The contract MUST revert if the leaf is already `0`.

After deregistration, the auth-policy tree state is indistinguishable from "never registered" — history is carried by events, not the current leaf. The policy-assignments mapping MUST still expose the last assigned `authDataCommitment` and `policyVersion` for current-state introspection, but the tree leaf is `0` and therefore unusable for proof verification. Re-registration at the same `(address, authVkHash)` pair continues from the existing `policyVersion` counter (which is not reset by deregistration), so old intents signed at pre-deregistration versions cannot match the re-registered leaf.
...
The auth policy registry binds `(address, authVkHash)` pairs to credentials. State layout is specified in Section 5.2. Registration is via `registerAuthPolicy`, called directly by the account holder. See Section 5.3.

Wallets and provers can read the current status/version for a specific `(address, authVkHash)` pair via `getAuthPolicy` and the current accepted roots via `getCurrentRoots`.

The pool circuit proves auth-policy membership at `(authorizingAddress, authVkHash)` as part of `transact`; see Section 9.
...
* Old intents carry the old `policyVersion` in the signed intent → mismatch with the current registry leaf → pool proof failure.
* Re-registering identical credentials at a higher `policyVersion` does NOT resurrect old intents. The pool circuit uses the `policyVersion` authenticated through the old signed artifact, which mismatches the new leaf's incremented version.
* Rotating one auth method does not affect intents signed with other auth methods for the same address — each (address, authVkHash) pair has its own `policyVersion`.

**Adding a new auth method.** To add a new auth method:

1. Publish an auth circuit that verifies the new signature scheme and outputs `[authDataCommitment, transactionIntentDigest]`, while authenticating `authorizingAddress` and `policyVersion` through the signed intent.
2. Users register their credentials via `registerAuthPolicy` with the auth circuit's `authVkHash` and their `authDataCommitment`. Existing auth policies for other auth circuits remain active — the new registration creates a new leaf at a distinct composite key.
3. Done — no hard fork required.

Constraints: the auth circuit MUST conform to the auth-proof envelope in Section 9.1. `authVkHash` MUST be computed from the canonical auth-verification-key encoding defined in Section 9.1. Companion ERCs MUST authenticate all intent-digest fields. Companion ERCs MUST reject any authenticated value `>= p` before interpreting it as a BN254 field element, including `nonce` and any other authenticated input that feeds `transactionIntentDigest` or other field-based commitments. Companion ERCs MUST ensure an authorization valid for one `authVkHash` is invalid for any other; this EIP does not define how. Auth methods requiring a different proof system need a hard fork that updates the pool circuit.

**Cross-circuit note compatibility.** Note commitments bind to `ownerNullifierKeyHash` and do not encode an auth method. A note created when one auth circuit was used is spendable with any other auth circuit, provided the address bound to that `ownerNullifierKeyHash` has registered an auth policy for the spending circuit's `authVkHash`.
...
`ownerCommitment` hides both `ownerNullifierKeyHash` and `noteSecret` from on-chain observers. On `deposit`, the contract treats `ownerCommitment` as an uninterpreted field element — it does not derive `ownerNullifierKeyHash` or `noteSecret` from it and does not verify its construction. On `transact`, it is a private witness reconstructed inside the pool circuit from the spender's `ownerNullifierKeyHash` and the note's `noteSecret`.
...
#### 9.1 Auth and Pool Circuit Interface

The pool circuit MUST:

1. recursively verify an UltraHonk BN254 auth proof,
2. compute `authVkHash` from `authVkey`,
3. compute the auth-policy tree key `uint160(poseidon(AUTH_POLICY_KEY_DOMAIN, authorizingAddress, authVkHash))`,
...
**Auth circuit public output vector** — exactly 2 field elements:

1. `authDataCommitment`
2. `transactionIntentDigest`

`authorizingAddress` and `policyVersion` are private pool-circuit witnesses reused in auth-policy verification.

`authVkHash` is not an auth-circuit output; the pool circuit computes it from `authVkey`, the `UltraHonkVerificationKey` passed to the recursive verifier. Let `vk[0] .. vk[114]` denote the 115 BN254 field elements of `authVkey` in recursive-verifier order. Then:

```
authVkHash = poseidon(AUTH_VK_DOMAIN, vk[0], vk[1], ..., vk[114])
```

— a single 116-input Poseidon2 sponge call over the concatenation of the domain tag and the 115 VK field elements.

**Security property:** the auth circuit MUST NOT have access to `ownerNullifierKey` or `noteSecretSeed`. Neither secret may appear as a witness or public input in the auth proof relation.
...
* `authVkHash` computed from `authVkey` == `authVkHash` used in the auth-policy tree key
* `authDataCommitment` from auth proof == `authDataCommitment` in the auth-policy leaf
* `policyVersion` witness used in `transactionIntentDigest` == `policyVersion` in the auth-policy leaf
* `transactionIntentDigest` from auth proof == pool circuit's recomputed transaction intent digest
...
The pool and auth circuits do not validate encryption scheme semantics or delivery format. Section 14 defines the registry lookup and the interpretation for scheme ID `1`.
...
The transaction intent digest is the canonical digest of the contemplated private-note spend. Both auth and pool circuits compute the same formula independently: the auth circuit from the authenticated intent fields and any companion-standard constants, and the pool circuit from witnesses, public inputs, and mode-derived values. The auth circuit authenticates this digest; the pool circuit recomputes it and binds the authenticated transaction fields and signer-selected execution constraints to the proof.
...
The pool verifier's public-input vector is the 17 fields of `PublicInputs`, in declaration order.
...
The precompile verifies UltraHonk BN254 proofs for the fork-defined pool circuit. The canonical pool verification-key encoding is pinned by [`pool_vk.bin`](../assets/eip-8182/pool_vk.bin), [`pool_vk.sha256`](../assets/eip-8182/pool_vk.sha256), and [`pool_vk.bb_hash.hex`](../assets/eip-8182/pool_vk.bb_hash.hex). The canonical pool proof byte encoding and verifier-specific metadata required to interpret that proof are pinned by [`pool_verifier_metadata.json`](../assets/eip-8182/pool_verifier_metadata.json) together with [`pool_verifier_transcript_vk_hash.hex`](../assets/eip-8182/pool_verifier_transcript_vk_hash.hex).
...
| Auth policy key (truncated to 160 bits) | `AUTH_POLICY_KEY_DOMAIN, authorizingAddress, authVkHash` | 3 |
| Auth policy leaf | `AUTH_POLICY_DOMAIN, authDataCommitment, policyVersion` | 3 |
| Auth VK hash | `AUTH_VK_DOMAIN, vk[0], vk[1], ..., vk[114]` | 116 |
...
This section sketches an example single-circuit auth circuit for ECDSA/secp256k1 authorization using [EIP-712](./eip-712.md) typed data signing.
...
In this example companion standard, the domain binds `executionChainId` and `poolAddress` without repeating them in the struct. This is intentionally a single-circuit companion-standard example, not a general protocol-level mechanism for binding a signed authorization to the actual `authVkHash` selected by the pool circuit. Companion standards that span multiple auth circuits still need to specify how they satisfy the cross-circuit invalidation requirement from Section 6.4. The actual `authVkHash` used by the pool circuit is computed from `authVkey` per Section 9.1.
...
A malicious pool verification key could drain the entire pool, so pool circuit upgrades require the same social consensus as any other protocol change. Auth circuits are permissionless because the pool circuit independently enforces all pool-critical invariants.
...
Recursion separates pool-critical logic (pool circuit, fork-managed) from spend authorization (auth circuits, user-scoped; registry lifecycle operations remain address-gated). This enables permissionless auth extensibility: new signature schemes deploy as auth circuits without a hard fork. A malicious auth circuit can only risk the registering user's funds, not the pool, because the pool circuit independently enforces value conservation, nullifiers, deterministic note-secret derivation, and auth-policy checks — in practice, adding a new auth method is one `registerAuthPolicy` call with no fund transfers, no new addresses, and no anonymity set fragmentation. Existing auth methods remain active; unwanted methods can be deregistered (Section 6.4). The proving overhead vs a monolithic circuit is the cost of these properties. Decoupling the signed authorization format from the protocol lets auth circuits evolve their signing formats independently, without coordination or a protocol change.
...
`authVkHash`, `authDataCommitment`, and `policyVersion` are private inputs, never exposed as public inputs in `transact`. See Section 4 for the full auth-method anonymity model.
...
* auth-policy rejection when `authVkHash` is computed incorrectly or does not match the auth-policy key proven in the pool circuit
...
Every active `(address, authVkHash)` pair is an independent spend-authorization path for notes bound to that address's registry namespace. Registering a weak auth circuit alongside a strong one widens the attack surface, but spending still also requires custody of `ownerNullifierKey` and the relevant proving material.
