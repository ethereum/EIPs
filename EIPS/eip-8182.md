...
description: This EIP defines shielded transfers via an enshrined system contract with recursive proofs and permissionless authentication.
author: Tom Lehman (@RogerPodacter)
discussions-to: https://ethereum-magicians.org/t/eip-8182-protocol-enshrined-privacy-pool/27889
status: Draft
...
This EIP introduces Ethereum's canonical shielded asset pool: a fixed-address system contract and proof-verification precompile that define valid shielded transfers of ETH and ERC-20 tokens. A recursive proof architecture separates protocol invariants enforced by a hard-fork-managed outer circuit from permissionless inner authentication circuits, allowing new auth methods without fragmenting the anonymity set. The system contract has no on-chain upgrade mechanism and can only be replaced by a hard fork.
...
1. A **Privacy Pool System Contract** deployed at a fixed address, holding all privacy state (e.g., note commitment tree, nullifier set, intent-nullifier set, user registry).
...
The following domain tags are defined by this EIP (all use the `privacy_pool.` prefix):
...
### 5. Privacy Pool System Contract
...
* EIP-712 domain: `{ name: "PrivacyPool", version: "1", chainId: block.chainid, verifyingContract: PRIVACY_POOL_ADDRESS }`.
...
* `privacyPoolAddress` (`PRIVACY_POOL_ADDRESS`) makes the authorization target explicit — cheap defense-in-depth against cross-contract replay if another contract ever uses a similar digest scheme. In the outer circuit, `privacyPoolAddress` is a constant hardcoded at compilation time (fork-managed, like the outer verification key). In the inner circuit, it comes from the signed artifact (e.g., the EIP-712 domain's `verifyingContract`).
...
| Canonical intent digest | `INTENT_DIGEST_DOMAIN, policyVersion, authorizingAddress, operationKind, privacyPoolAddress, tokenAddress, recipientAddress, amount, rawNonce, validUntilSeconds, executionChainId` | 11 |
...
EIP-712 domain: `{ name: "PrivacyPool", version: "1", chainId: <executionChainId>, verifyingContract: <privacyPoolAddress> }`. The domain binds `executionChainId` and `privacyPoolAddress` without repeating them in the struct.
