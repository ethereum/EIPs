...
* **`lineageConstraintKind`**: Private witness in the intent digest. `0` means no additional lineage rule; `1` means real outputs MUST NOT carry `MIXED_LABEL`.
...
* `LINEAGE_UNCONSTRAINED = 0` — signer does not constrain output lineage.
* `LINEAGE_FORBID_MIXED = 1` — signer forbids `MIXED_LABEL` on real outputs.
...
* Round constants and MDS matrix: exactly the constants in [`assets/eip-8182/poseidon_bn254_t3_rf8_rp57.json`](../assets/eip-8182/poseidon_bn254_t3_rf8_rp57.json). The corresponding normative vectors are in [`assets/eip-8182/poseidon_vectors.json`](../assets/eip-8182/poseidon_vectors.json).
...
At the activation fork, clients MUST install the account object in [`assets/eip-8182/shielded-pool-state.json`](../assets/eip-8182/shielded-pool-state.json) at `SHIELDED_POOL_ADDRESS`. This state dump is the canonical shielded-pool activation artifact. The installer used to generate it is non-normative tooling and is not part of consensus.
...
After deregistration, the auth-policy tree state is indistinguishable from "never registered" — history is carried by events, not the current leaf. The auxiliary `getAuthPolicy` mapping MAY still expose the last assigned `authDataCommitment` and `policyVersion` for current-state introspection, but the tree leaf is `0` and therefore unusable for proof verification. Re-registration at the same (address, innerVkHash) pair continues from the existing `policyVersion` counter (which is not reset by deregistration), so old intents signed at pre-deregistration versions cannot match the re-registered leaf.
...
Constraints: the inner circuit MUST conform to the inner-proof envelope (Section 9.1). `innerVkHash` MUST be computed from the canonical inner-verification-key encoding defined in Section 9.1. Companion ERCs MUST authenticate all intent-digest fields. Companion ERCs MUST ensure an authorization valid for one `innerVkHash` is invalid for any other; this EIP does not define how. Auth methods requiring a different proof system need a hard fork that updates the outer circuit.
...
Coin selection is delegated to the prover. The intent digest binds payment semantics (recipient, amount, token, operation type), the selected lineage rule via `lineageConstraintKind`, and any signer-selected output-binding locks, but it does not bind exact note selection or exact output labels. Operation-type binding is the inner circuit's responsibility via `operationKind` in the intent digest.
...
Reusing the same `nonce` within the same `(nullifierKey, authorizingAddress, executionChainId)` replay domain makes those authorizations mutually exclusive even when their payment fields or execution constraints differ. Wallets MUST ensure nonce freshness for each new authorization. Companion standards that encode `nonce` as a `uint256` or byte string MUST reject values `>= p` before reducing them to a field element.

`validUntilSeconds` remains independently range-constrained to `< 2^32`. UNIX seconds fit 32 bits until 2106.
...
* If `lineageConstraintKind == LINEAGE_UNCONSTRAINED`, no extra lineage constraint.
* If `lineageConstraintKind == LINEAGE_FORBID_MIXED`, every real output label MUST satisfy `label != MIXED_LABEL`.
* Dummy outputs are exempt from this check because they already enforce `label == 0`.
* If `lineageConstraintKind > LINEAGE_FORBID_MIXED`, the proof MUST fail.
...
* `lineageConstraintKind` binds whether `MIXED_LABEL` is allowed on real outputs. It does not bind exact note identities or exact output labels.
...
`lineageConstraintKind`, `executionConstraintsFlags`, `lockedOutputBinding0`, `lockedOutputBinding1`, `lockedOutputBinding2`, `nonce`, and `intentDigest` are not public inputs. They are private signed/authenticated values checked inside the recursive proof relation.
...
* **One real input** (one phantom): real output notes MUST inherit the real input's label. Phantom slots contribute no label.
* **Two real inputs, same label**: real output notes MUST inherit that label.
* **Two real inputs, different labels**: all real output notes MUST equal `MIXED_LABEL` (Section 3.2). Provenance is lost at the protocol level — the label no longer traces back to a specific deposit.
* **Both inputs phantom** (deposit mode): real output notes use the freshly derived deposit label (Section 12.1).

Dummy outputs are not covered by these propagation rules and remain `label = 0` per Section 9.5.
...
| Intent digest | `INTENT_DIGEST_DOMAIN, policyVersion, authorizingAddress, operationKind, tokenAddress, recipientAddress, amount, feeRecipientAddress, feeAmount, lineageConstraintKind, executionConstraintsFlags, lockedOutputBinding0, lockedOutputBinding1, lockedOutputBinding2, nonce, validUntilSeconds, executionChainId` | 17 |
...
6. Outputs `[authDataCommitment, intentDigest]` where `authDataCommitment = poseidon(ecdsaPubKeyX, ecdsaPubKeyY)`.
...
For scheme `1`, `keyBytes` MUST be a raw 1216-byte X-Wing encapsulation key: the 1184-byte `ML-KEM-768` encapsulation key followed by the 32-byte `X25519` public key. This EIP pins the X-Wing KEM to `draft-connolly-cfrg-xwing-kem-10`, including key generation, encapsulation, decapsulation, and combiner behavior. Scheme `1` is frozen by that draft revision plus the normative vectors in [`assets/eip-8182/delivery_scheme1_vectors.json`](../assets/eip-8182/delivery_scheme1_vectors.json); later IETF changes do not alter EIP-8182 scheme `1`.

The plaintext is the six note fields in note-commitment order, each encoded as a 32-byte big-endian word:

```
amount || ownerAddress || randomness || nullifierKeyHash || tokenAddress || label
```
...
### Coarse Lineage Policy

`lineageConstraintKind` lets the signer require that real outputs not carry `MIXED_LABEL` without binding exact input-note selection or exact output labels.
...
Normative fixture assets for this EIP are under [`assets/eip-8182/`](../assets/eip-8182/). Implementations claiming conformance MUST match the pinned constants and vectors in at least the following asset families:
...
* mixed-lineage rejection when `lineageConstraintKind = LINEAGE_FORBID_MIXED`
...
### Third-Party Prover Lineage Discretion

Because coin selection is delegated, a third-party prover can intentionally merge differently labeled notes and force `MIXED_LABEL`, degrading future provenance/selective-disclosure utility without violating payment semantics. This discretion is partially constrainable: when the signer sets `lineageConstraintKind = LINEAGE_FORBID_MIXED`, proofs whose real outputs would carry `MIXED_LABEL` fail.
