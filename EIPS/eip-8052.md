...
An implementation is provided in `assets`. For the NIST-compliant version, KAT vectors of the NIST submission are valid. Similar vectors are provided for ETHFALCON, with the same format to keep as close as possible to the NIST standard format.
