...
The same applies to top-level contract-creation transactions. If the transaction reverts or halts, the `STATE_BYTES_PER_NEW_ACCOUNT × CPSB` portion of `intrinsic_state_gas` is refunded back to `state_gas_reservoir` and `execution_state_gas_used` decreases by the same amount.
