...
|---|---|---|---|---|
| `GAS_CREATE` | 32,000 | `STATE_BYTES_PER_NEW_ACCOUNT x CPSB` | `CALL_VALUE` + `COLD_ACCOUNT_ACCESS` | `CREATE`, `CREATE2`, contract creation txs |
...
`STORAGE_WRITE` and `ACCOUNT_WRITE` are defined in [EIP-8038](./eip-8038.md). Additionally, `CALL_VALUE` and `COLD_ACCOUNT_ACCESS` are updated [EIP-8038](./eip-8038.md).
...
- Create transaction regular gas cost (`CALL_VALUE` + `COLD_ACCOUNT_ACCESS` if it is a create tx)
...
The "Proposed cost (ETH)" column assumes a base fee of 0.08 Gwei, which corresponds to the average base fee observed after the 60M gas increase in December 2026 and January 2026.
