...
- The reservoir is passed **in full** to child frames (no 63/64 rule). Unused `state_gas_reservoir` is returned to the parent on child completion, including on **revert** and **exceptional halt**. On child **exceptional halt**, only `gas_left` is consumed (zeroed). The reservoir is not consumed because state changes are reverted on failure, so no state was actually grown.
