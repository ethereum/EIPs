...
|---|---|---|---|---|
| `GAS_CREATE` | 32,000 | `STATE_BYTES_PER_NEW_ACCOUNT x CPSB` | `GAS_CREATE_REGULAR` | `CREATE`, `CREATE2` |
...
`STORAGE_WRITE`, `COLD_ACCOUNT_ACCESS`, `ACCOUNT_WRITE` and `GAS_CREATE_REGULAR` are defined and updated in [EIP-8038](./eip-8038.md). The values are not yet final.
...
- Create transaction regular gas cost (`GAS_CREATE_REGULAR` if it is a create tx)
