...
When an account is created and self-destructed within the same transaction (per [EIP-6780](./eip-6780.md)), the state does not persist in the final state. Accordingly, all state gas associated with that account MUST be refunded to `state_gas_reservoir`, and `execution_state_gas_used` MUST be decremented to reflect that no long-term state growth occurred.

The refund scope includes:
...
#### Provisional refund for account and code

To better align state gas accounting with execution-time resource usage, clients MAY apply a **provisional refund** for account creation and code deposit immediately upon execution of `SELFDESTRUCT`, subject to the following constraints:

- The provisional refund MUST be recorded in a reversible manner and MUST be fully reverted if the enclosing frame (or any ancestor frame) reverts.
- The refund MUST be applied at most once per account within a transaction.
- The refund MUST NOT be considered final until transaction completion confirms that the account does not persist.

This approach allows implementations to reduce the gap between reserved gas (e.g., `gas_limit`) and effective gas usage during execution, while preserving correct semantics under reversion. This is particularly relevant for execution environments that meter gas based on upfront limits rather than post-execution usage, where immediate visibility of reclaimed state gas may improve execution efficiency.

#### Deferred refund for storage

Refunds for created storage slots MUST be applied only at the end of the transaction, based on the final state. This avoids the need for O(N) tracking or rollback of per-slot refund state across nested frames and reverts.

#### Final accounting

All refunds—whether provisional or deferred—MUST be reflected before computing `tx_gas_used_before_refund`, ensuring that the sender is not charged for state that does not persist.

Storage slots that were restored to zero during execution (i.e., `0 → X → 0`) already have a final value of zero and are excluded from this refund scope, preventing double counting with the `SSTORE` restoration refund.
