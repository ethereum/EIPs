...
The same unconditional-charge-with-refill model applies to top-level contract-creation transactions. The `STATE_BYTES_PER_NEW_ACCOUNT × CPSB` portion of `intrinsic_state_gas` is charged unconditionally and refilled back to `state_gas_reservoir` (with `execution_state_gas_used` decreased by the same amount) if the destination account was pre-existing or if the transaction reverts or halts.
