...
category: Core
created: 2025-10-01
requires: 2780, 6780, 7702, 7825, 7904, 7976, 7981, 8038
---
...
| `GAS_NEW_ACCOUNT` | 25,000 | `STATE_BYTES_PER_NEW_ACCOUNT x CPSB` | `ACCOUNT_WRITE` | `CALL`, `CALLCODE`, `SELFDESTRUCT` |
| `GAS_STORAGE_SET` | 20,000 | `STATE_BYTES_PER_STORAGE_SET x CPSB` | `STORAGE_WRITE` | `SSTORE` |
...
`STORAGE_WRITE`, `COLD_ACCOUNT_ACCESS`, `ACCOUNT_WRITE` and `CREATE_ACCESS` are defined and updated in [EIP-8038](./eip-8038.md). The values are not yet final.
...
State-gas accounting for `CALL*` and `CREATE`/`CREATE2` operations is performed at the end of successful call frames. If the respective call frame reverts or halts exceptionally, no state-gas is charged for the new account.
...
`SELFDESTRUCT` does not produce a state-gas refill at the point in which it executes. Its accounting is deferred to the end of the transaction. For each account that was created during the transaction, `SELFDESTRUCT`ed, and is now being removed:
...
For an account that existed before the transaction, `SELFDESTRUCT` only transfers its balance and the account is not removed; no state-gas refund applies and no changes to `execution_state_gas_used`. This is consistent with the behavior of [EIP-6780](./eip-6780.md).
