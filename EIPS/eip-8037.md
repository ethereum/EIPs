...
- On **exceptional halt** at the top level (no parent frame), remaining `gas_left` is attributed to `execution_regular_gas_used` and set to zero (all regular gas consumed), consistent with existing EVM out-of-gas semantics. The `state_gas_reservoir` is preserved (not consumed) for the sender refund calculation. Additionally, `execution_state_gas_used` is reset to zero because state changes are fully reverted and no state was actually grown — the sender is refunded the full state gas via the reservoir. This is consistent with child frame behavior where state gas is restored on failure. System transactions are not subject to the `TX_MAX_GAS_LIMIT` cap, their entire `execution_gas` is placed in `gas_left` with `state_gas_reservoir = 0`.
