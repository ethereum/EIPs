...
`SELFDESTRUCT` for accounts created in the same transaction do no produce increases in state size as the account, its data and its storage is not included in the state trie. However, This operation does not produce any a state-gas refills and there are no changes to `execution_state_gas_used`.
...
- If the `authority`'s `codeHash` is not `emptyHash` (i.e., a prior authorization indicator already occupies the 23 bytes) or `authorization.address` is `ZERO` (the authorizations is clearing the delegation), the `STATE_BYTES_PER_AUTH_BASE × CPSB` portion is also refilled directly to `state_gas_reservoir`.
