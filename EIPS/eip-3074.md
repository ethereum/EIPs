...
The arguments memory region shall be encoded as `type || abi.encode(sponsee, nextra, mingas, to, value, data, v, r, s)`, where:
...
`success` shall be zero in the following cases:
 ...
 - The value sent with the call is not equal to `value`
