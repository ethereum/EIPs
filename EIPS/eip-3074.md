...
`AUTH` returns the newly set `authorizedAccount` variable as a stack element.
...
`AUTHCALL` is interpreted the same as `CALL`, except for:
 ...
 - If `authorizedAccount` is the zero address, no call is made and `success` is set to `0`.
