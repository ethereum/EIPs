...
`NAMETBD` requires the following stack arguments:
...
The arguments are:
 ...
 - `mingas: uint256` - minimum gas limit which must be provided with the call into `NAMETBD`;
...
`success` shall be zero in the following cases:
 ...
 - Gas limit supplied with the call into `NAMETBD` is less than the signed `mingas`
...
Including `sponsee` in the arguments to `NAMETBD` is a gas optimization. Without it, invokers would have to do their own `ecrecover` before calling into `NAMETBD` to verify/adjust any state for replay protection.
...
More logic can be implemented around the call into `NAMETBD`, giving more control to invokers and sponsors without sacrificing security or user experience for sponsees.
