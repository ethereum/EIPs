...
This EIP creates an EVM instruction (`IMPERSONATE_CALL`) which forwards a `CALL`, setting `CALLER` according to an ECDSA signature.
...
Reserving a specific revert reason, for example `IMPERSONATE_CALL failed`, is a large departure from other instructions. An invoker would need to inspect the revert reason to determine whether the callee reverted, or the `IMPERSONATE_CALL` pre-conditions were invalidated, which implies reading and comparing memory values. Further, to remain sound when a callee reverts with `IMPERSONATE_CALL failed`, `IMPERSONATE_CALL` would need to replace the return data with some other value.
