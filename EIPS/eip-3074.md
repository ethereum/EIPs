...
`CALL_PRECOMPILE` requires the following eight arguments:
 ...
 - `v`, `r`, `s` - signature for the package, including chain id as specified in [EIP-115](./eip-115.md).
...
Other approaches to sponsored transactions, like [EIP-2711](./eip-2711.md) and [EIP-2733](./eip-2733.md), are incompatible with [EIP-2938](./eip-2938.md) flavored account abstraction (AA). These proposals require a _signed_ transaction from the sponsor's account, which is not possible from an AA contract, because it has no private key to sign with.
