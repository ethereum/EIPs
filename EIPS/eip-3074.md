...
The arguments to `CALL_PRECOMPILE` are encoded as `type || rlp([nextra, gaslimit, to, value, data, v, r, s])`.

The signature (`v`, `r`, `s`) arguments are computed from `secp256k1(keccak256(type || rlp(nextra, gaslimit, to, value, data, invoker, chainid])))`.
