...
Note that `web3` text record may return an Ethereum address or an EIP-3770 chain-specific address.  If the address is an EIP-3770 chain-specific address, then the chainid to call the message will be overriden by the chain-specified by the address.
...
The protocol will find the address of **ensdomains.eth** from ENS in chainid 4 (Rinkeby).  If the `web3` text record of **ensdomains.eth** on chainid 4 is an address, then the protocol will call the address with "From" = "0x..." and "Calldata" = "0x" with chainid = 4.  If the `web3` text record of **ensdomains.eth** on chainid 4 is an EIP-3770 chain-specific address, e.g., "rop:0x...", then the protocol will call the address with "From" = "0x..." and "Calldata" = "0x" with the chainid specified by the `web3` text record (ropsten in the example).
