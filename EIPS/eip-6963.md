...
Browser extensions, and therefore Wallet extensions, are able to modify the contents of the page and the Provider object by design. Given the provider objects of various Wallets are considered high trusted interface to communicate transaction data. In order to prevent the page or various other extensions from modifying the interaction between the DApp and the Wallet in an unexpected way. The expected way to handle this is to utilize `object.freeze()` on the  `EIP1193Provider` object before the wallet dispatches the `eip6963:announceProvider` Event.
