...
author: Pedro Gomes (@pedrouid), Kosala Hemachandra (@kvhnuke), Richard Moore (@ricmoo), Gregory Markou (@GregTheGreek), Kyle Den Hartog (@kdenhartog), Glitch (@glitch-txs), Sam Wilson (@SamWilsn), Micah Zoltu (@MicahZoltu), Jake Moxey (@jxom), Pierre Bertet (@bpierre), Darryl Yeo (@darrylyeo)
...
- icon - uri encoded image (RFC-3986 compliant, image must be squared with 96x96px minimum resolution)
...
To emit events it must use `window.postMessage` and to observe events it must use `window.addEventListener`. There will be two event interfaces:

```typescript
// Requesting an EVM provider
interface EIP6963RequestProviderEvent extends MessageEvent {
  data: {
    eventName: "eip6963:requestProvider";
  };
}

// Annoucing an EVM provider
interface EIP6963AnnounceProviderEvent extends MessageEvent {
  data: {
    eventName: "eip6963:announceProvider";
...
window.postMessage({ eventName: "eip6963:announceProvider", announcedProvider })
```
...
window.addEventListener("message", (event: MessageEvent) => {
  if (event.data.eventName === "eip6963:requestProvider") {
    announceProvider();
  }
});
```
...
function onPageLoad() {
  ...
  function announceProvider() {
    ...
    const announcedProvider = { info, provider };
    window.postMessage({
      eventName: "eip6963:announceProvider",
      announcedProvider,
    });
...
```typescript
function onPageLoad() {
  const providers: EIP6963AnnouncedProvider[] = [];

  window.addEventListener(
    "message",
    (event: MessageEvent) => {
      if (event.data.eventName === "eip6963:announceProvider") {
        providers.push(event.data.announcedProvider);
      }
    }
  );
