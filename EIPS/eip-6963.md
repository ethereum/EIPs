...
A Javascript communication standard that enables multiple wallet providers to be discovered from an EVM-based application using a browser extension injected script through window events.
...
In this proposal, we present a solution that focuses on optimizing the interoperability of multiple wallet providers. This solution aims to foster fairer competition by reducing the barriers to entry for new wallet providers, along with enhancing the user experience on the Ethereum network.
...
Standardizing a provider info interface (`EIP6963ProviderInfo`) allows determining the necessary information to populate a wallet selection popup. This is particularly useful for web3 onboarding libraries such as Web3Modal, RainbowKit, Web3-Onboard, ConnectKit, etc.
