...
Standardizing `Eip6963ProviderInfo`, an interface for Wallet Provider information with icons and names, enables Dapps to collect all information necessary to programatically generate a user-friendly wallet selection flow (or employ a Wallet Provider Discovery Library such as Web3Modal, RainbowKit, Web3-Onboard, or ConnectKit to do so on its behalf).
