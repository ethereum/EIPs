...
To emit events both DApps and Wallets MUST use `window.dispatchEvent` to emit events and MUST use `window.addEventListener` to observe events. There will be two Event interfaces used for the DApp and Wallet to discover each other.

#### Wallet Events

The `EIP6963AnnounceProviderEvent` interface MUST include a `type` property with a string value of `eip6963:announceProvider`.
...
The Wallet MUST emit 
Therefore both Wallet and the DApp  MUST emit events when they initialize.
