...
description: Using JavaScript `window` events to facilitate discovery of browser extension-based Wallet Providers by Dapps
...
Decentralized Application (Dapp): A webpage that relies upon one or more EIP-1193 Provider Objects which are exposed to the webpage by the Wallet Provider.
