...
The creator simply specifies at the time of creation the royalty rate they wish to receive, then each time a token is listed on-chain it will be available for purchase. By calling a specific function `buyToken` the token transfer and payment are made simultaneously and a portion of the payment is returned to the creator according to the rate they have defined.
...
Although we considered integrating the royalties payments in a separate extension, we ultimately felt it was more appropriate to include it here while giving the creator the option to not apply any royalties (override `_defaultRoyalty()` and set the value to 0).

To avoid any mischievous manipulation we decided that it is not possible to increase the royalty % once the contract is deployed but only to decrease it under its current value.
