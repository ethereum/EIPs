...
To support subroutines, `BEGINSUB`, `GOSUB`, and `RETURNSUB` are provided.  Brief descriptions follow, and full semantics are given below.
...
`JUMPV` and `GOSUBV` are not strictly necessary.  They provide O(1) operations that can be replaced by O(n) or O(log n) EVM code using static jumps, but that code will be slower, larger and use more gas for things that can and should be fast, small, and cheap, and that are directly supported in WASM with br_table and call_indirect.
...
Execution of a subroutine begins with `GOSUB` or `GOSUBV`, which
...
(The _main_ routine is not addressable by `GOSUB` instructions.)
...
>4a  `GOTO`, `JUMPIF`, and `JUMPV` address only `JUMPDEST` instructions.

>4b  `GOSUB` and `GOSUBV` address only `BEGINSUB` instructions.
...
>6  For `GOSUB` and `GOSUBV` the frame size is at least the `n_args` of the `BEGINSUB`(s) to jump to.
