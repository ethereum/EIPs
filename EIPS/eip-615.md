...
>In particular, we propose to deprecate two existing instructions—`JUMP` and `JUMPI`.
>
>They take their argument on the stack, which means that unless the value is constant they can jump to any `JUMPDEST`.  (Note: We must nonetheless continue to support them in old code.)
>
