...
[ERC-20](./eip-20) is motivated by the need to use same terminology as with traditional bonds. Furthermore, by adding the `data` input parameter
...
Another reason that has motivated not to extand other standards like the ERC-20 is to allow token tracker platforms like etherscan to represent
