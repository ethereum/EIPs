...
Holds the address of the beacon contract this proxy relies on. This is an alternative to the logic contract slot. See the **Beacons** section below.
...
Some use-cases rely on multiple proxy contracts delegating their calls to the same logic contract. If each contract was to store the address of the logic contract using the logic contract storage slot, upgrading them would require writing to the corresponding storage slot of every instance, which would be very inefficient.

Another approach is to have each proxy retrieve the logic contract's address for a dedicated beacon. Using this pattern, the address of the logic contract can be modified in the beacon and take immediate effect for all the corresponding proxy contracts.

A beacon MUST implements the function:
...
- Call the `implementation()` function on the beacon contract (with `msg.sender` set to be the proxy's address).
