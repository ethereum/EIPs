---
eip: 6122
title: Forbid checks based on timestamps
...
- Forks by timestamp have to be scheduled at or after the forks by block (on mainnet as well as on private networks)
- An implementation of fork-id verification of remote peer needs to filter the incoming forkids first by block then by timestamp
...
Since all block based forks are before time based forks, nodes need to check the block based forks before the time based forks.
