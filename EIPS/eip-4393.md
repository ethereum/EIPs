...
This standard does not specify any functionality for refunds for deposits nor for tip tokens sent, it is left to the implementer to add this to their smart contract(s). The reasoning for this is to keep the interface light and not to enforce upon implementers the need for refunds but to leave that as a choice.
