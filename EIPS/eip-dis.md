...
Smart contract of registry and factory are deployed on pre-determined address. Anyone interacting with this smart contract is able to create own identitiy instance that consists of a: 1) Soul token: an asset holding which is a proof of identity for any public address that holds it and embedds social recovery mechanisms; 2) digital identity proxy ([EIP-2535](./eip-2535.md) diamond) that is controlled by soul token holders majority and can act as identities gateway or oracle in to the blockchain; 3) entry in the registry pointing their lookup name to that diamond, allowing human readible name

### Soul Token

Soul token is a fungable token that is very similar to conventional [EIP-20](./eip-20.md) with three four new properties:
...
This standard will be backwards compatible with commonly used [EIP-20](./eip-20.md) as well as diamond proxy.
