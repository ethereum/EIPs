...
Note: Alternatively clients can choose to do directly write to the storage of the contract. However this could prove problematic on non-mainnet networks e.g. if the contract is not deployed at activation of this EIP.
...
The `BLOCKHASH` opcode semantics remains the same as before. However the clients which want to leverage the history from state may do so making sure the query is within `BLOCKHASH_SERVE_WINDOW` and is available in the contract.
