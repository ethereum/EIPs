...
The first option is recommended until the Verkle fork, to stay consistent with [EIP-4788](./eip-4788.md) and to issues for misconfigured networks where this EIP is activated but history contract hasn't been deployed. The recommendation may be reconsidered at the Verkle fork. The reason would be to avoid the extra cost of attaching the history contract in each block's Verkle witness, or to avoid the complexity of a work-around for the witness in client implementations.
