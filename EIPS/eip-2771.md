...
This EIP defines a contract-level protocol for `Recipient` contracts to accept meta-transactions through trusted `Forwarder` contracts. No protocol changes are made. `Recipient` contracts fetch the effective `msg.sender` (`_msgSender()`) and `msg.data` (`_msgData()`) through custom getter functions. 
