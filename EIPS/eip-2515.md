...
__Rational behind 0.001% Increase__
The algorithm is not purely linear, but slowly increases exponentially every block by .01% of the previous. It still needs to be determined if a pure linear function is prefered, or which value of the scalar if it is exponential.
