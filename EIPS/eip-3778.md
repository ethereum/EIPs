...
description: Enables NFT to map and anchor things with time features, while tpNFT's timestamp proof can be used in third-party smart contracts.
...
---
## Abstract
This is a standard for timestamp provable NFTs, been called Time proof Non-fungible Token and is referred to as tpNFT in the subsequent descriptions, it enables NFT to mapping and anchor things that have time features, and also the timestamp proof of tpNFT can be the condition for smart contracts, this standard is extended from [ERC-721](./eip-721.md).

Based on the `tokenURI` parameter of metadata of ERC-721 , the plain text parameter and the encryption parameter are added, and the timestamp proof is set for the update of these three parameters.

With these improvements, we can provide new possibilities for NFT, enabling NFT to map and capture time-level value.
Here are some brand new applications that can be achieved by tpNFT based on this extension.
...
Summarize: From a philosophical point of view, the ERC-721 standard is mainly used to anchor and map things related to space, while tpNFT is used to relate time.

    
## Motivation
Compared to other possible solutions for on-chain time proof such as CallData or ERC-721 they only solve partial problems.
...
interface TimeProofNonFungibleToken {
     ...
     * The purpose of using tpNFTtokenURI instead of tokenURI is to avoid the function name conflict that comes with ERC-721.
...
2. Why not use ERC1155 instead of ERC-721: In 1155, there are not only 0 or 1 possible states for the balance of each account, and the relationship between the same ID items(tokens) is not non-fungible, so we don't know who has the authority to update token.
...
## Backwards Compatibility
 ...
 2. Based on ERC-721 and fully compatible, it is a superset of ERC-721. If you only fill in the tokenURI and do not fill in the description and hash in the mint, and then set tpNFT to be constant, you will get a tpNFT that only has more tokenURI update timestamps than the standard NFT with tokenURI.
