...
| `expand_evm_memory` | `func(new_size_evm_words: int) -> int` | EVM memory expansion cost function, modified according to this EIP |
...
if not mod_id in evmmax_state.mods:
    cost += cost_evm_expand_memory(num_vals_used * val_size_multiplier * 8)
````
##### Execution
```
