...
| # | Function | Input / Output | Description |
|---|----------|----------------|-------------|
| 1 | AddNftURIRecord | **Inputs** _string calldata nftURI_ - human readable name _address ethereumAddr_ - Ethereum address **Outputs** none emits **HashtagNFTCollectiveRecord** event (senderAddr, nftURI, contractAddress, tokenId) | Adds a keccak256 hash mapping of a human readable name (e.g. "hashtag" or "nftURI") to an Ethereum address. A human readable name is here defined as case insensitive and **MUST** contain 1 or more non-white space characters. The implementation of this method enforces the nftURI **MUST** be unique otherwise the transaction will be reverted. |
| 2 | ResolveURI | **Inputs** _string calldata nftURI_ - human readable name **Outputs** address - Ethereum address. | Returns the Ethereum address associated with a human readable name. |
| 3 | ResolveAddress | **Inputs** _address ethereumAddr_ - Ethereum address **Outputs** string calldata - Human readable name | Returns the human readable name associated with the Ethereum address. |
