...
The wrapper format is detected by checking if the signature ends in `magicBytes`, which MUST be defined as `0x69696969`.
...
- checking for contract code MUST happen before checking for `magicBytes`, to avoid possible collisions, as [ERC-1271](./eip-1271.md) allows any arbitrary values for signatures
