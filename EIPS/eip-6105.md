...
Out of consideration for the safety and efficiency of buyer' asserts, it does not provide bidding functions and auction functions, but only adds listing funcitons.

The `price` in the `listItem` function cannot be set to zero. On the one hand, a caller rarely sets the price to 0, and when this behavior occurs, it is often the caller’s operation error and will cause assets loss. On the other hand, a caller needs to spend gas to call this function, so if a caller can set the token price to 0, his or her income is actually negative at this time, which does not conform to the "economic man" thinking in economics. What's more, when a token's price is 0, we assume that it is not for sale, which will make the reference implementation more concise.

Setting `expires` in the `listItem` function will allow callers to better manage their listings. In addition, if a listing expires automatically, a token owner no longer needs to `delistItem`, thus saving gas.
