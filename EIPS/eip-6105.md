...
interface IERC6105 {
  ...
  event UpdateListing(uint256 indexed tokenId, address indexed from, address to, uint256 indexed price ,uint64 expires);
...
The `listItem(uint256 tokenId, uint256 price, uint64 expires, address to)` function MAY be implemented as `public` or `external`.And `price` MUST NOT be set to zero.
