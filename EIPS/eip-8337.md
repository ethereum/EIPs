...
discussions-to: https://ethereum-magicians.org/t/eip-8337-validated-evm-code/29013
status: Draft
...
* ***subroutine***: the code an *entry* owns: everything reachable
  ...
  Constraint 5 requires every reachable instruction to belong to
...
### Is the validation linear in space and time?
...
### Is the validation algorithm sound and complete?

A validator is *sound* if it accepts only code that meets the constraints and *complete* if it accepts all code that does, where the constraints are read, as this proposal reads them throughout, over every path of the *control-flow graph*, whether or not data would ever take it.  The reference algorithm is both, constraint by constraint.

* *Constraints 1, 2 and 3* are properties of single instructions: a defined opcode; a `PUSH` immediately before each jump and call, whose value is a `JUMPDEST` or `CALLDEST` outside immediate data.  The traversal visits exactly the reachable instructions and checks each on the spot, so it accepts precisely the code that satisfies them.

* *Constraint 5* is what the traversal records: each instruction's *stack offset* and *entry* on first arrival, matched on every later arrival, and each entry's *net stack effect* on the first `RETURNSUB` that closes a frame begun there, matched by every later one.  A mismatch is a violation of one of the three clauses, and every violation is a mismatch on some arrival, so the check is exact.  The third clause is what lets the rest be computed per subroutine: nets and *demands* are kept per entry, and an instruction charged to two entries would be accounted for in neither.

* *Constraint 4* has two halves.

  * *The data stack.*  Under path-independent offsets, an instruction's depth relative to its entry is a single number, so "finds at least as many items as it removes, on every path" reduces to the *demand* computation: the deepest an entry's own code reaches below its base, less the items each parent supplies at the point of entrance, propagated to the top level, which has nothing to supply.  This is a restatement, not an approximation; the limit only turns away demands that no stack could satisfy, which underflow on every path.

  * *The return stack.*  Call an entry *unframed* if it is the top level or is reached by a jump or fall-through from an unframed entry; a `CALLSUB` frames its callee, and a return point keeps its caller's framing.  The algorithm rejects exactly when an unframed entry contains a `RETURNSUB`.  If it rejects, a chain of jumps from the top level reaches that entry with no `CALLSUB` on it, so a path reaches the `RETURNSUB` on which every `CALLSUB` has returned: a violation.  Conversely, if some path reaches a `RETURNSUB` with no unreturned `CALLSUB`, delete its returned call–return pairs; what remains is a chain of jumps and fall-throughs from the start into the entry containing the `RETURNSUB`, through entries reached the same way, each unframed by definition — so that entry is unframed and contains a `RETURNSUB`, and the algorithm rejects.  Code with no `RETURNSUB` may be shared between framed and unframed entries, as the constraint allows.
