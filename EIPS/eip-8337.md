...
### Why is framing a property of entries?

Constraint 4 requires every `RETURNSUB` to find a return address.  The obvious way to prove it is to carry a flag along each path — is there an unreturned `CALLSUB` on it? — and reject any instruction reached with the flag both set and clear.  That is what an earlier draft of the reference validator did, and it over-rejects: a block reached both from top-level code and from inside a subroutine is rejected even when it contains no `RETURNSUB`, and such blocks are what compilers emit.  Compiled with EIP-7979 subroutines, every one of Vyper's example contracts and a fifth of Solidity's semantic-test contracts share a revert block between the dispatcher and the internal functions — the path-flag rule rejected 23% of 1,756 Solidity contracts, almost all of them for this.

Framing as a property of entries proves the same thing without the over-rejection.  A `RETURNSUB` can only execute with an empty *return stack* on a path that reached it without a `CALLSUB` — that is, in an *unframed entry* — so it is enough to reject a `RETURNSUB` in an unframed entry, and unframed entries are found by propagating from the top level along the jump and fall-through links the traversal already records.  The per-instruction record loses a field, the check is a small fixpoint beside the demand propagation, and the shared revert block is admitted.  On the same 1,756 contracts the rejection rate falls to 4%, none of it for framing; the remainder is code shared across subroutines without being an *entry*, which the compiler can fix by marking it `CALLDEST` at no cost.
