...
We should also consider the treatment [EIP-7702](./eip-7702) delegation designations receive if they show up as code in a contract creation transaciton.  The transaction is valid, is executed as plain bytecode, ad the first operation executed is `0xEF`, which as per [EIP-141](eip-141) is the designated invalid opcode and would cause the whole transction to abort.

Letting the inputdata of a type 0, 1, and 2 contract creation transaction execue as plain EVM code irrespective of the magic `0xEF00` byes keeps a consistent behavior in client code and keeps block builders from having to update their logic in older transaction types.
