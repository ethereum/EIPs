...
created: 2019-10-17
requires: [EIP-3540: EVM Object Format (EOF)](./eip-3540.md)
---
...
Several important safety properties, including
* valid jump destinations,
* valid instructions,
* no stack underflows, and
* no stack overflows without recursion
are ensured by following a few simple rules.

Valid contracts follow the rules.  Valid contracts will not halt with an exception unless they run out of gas or overflow stack during a recursion.

These rules are validated at contract creation time — not runtime — by the provided algorithm.  This is a one-pass algorirhm, linear in the size of the bytecode, so as not to enable DoS attacks.
