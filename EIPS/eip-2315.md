...
*Complete*.  Provides a minimal set of control structures for implementing programs -- jumps, conditional jumps, and subroutines.
...
Over the years native subroutine operations have proven optimal for efficient implementations and simple code.

The [concept](https://academic.oup.com/comjnl/article/20/3/269/751954) goes back to [Turing's Automatic Computing Engine of 1946](http://www.alanturing.net/turing_archive/archive/p/p01/P01-001.html):
...
This proposal only facilitates validation. [EIP-3779](./eip-3779.md) specifies creation-time validation to detect invalid contracts.

*Note: Only the safety properties depend on validation.  Without it the efficiency advantages remain.*
...
This design became and remains popular in many machines since the 1970's, including the PDP 11, VAX, M68000, SPARC, and Intel architectures.  These are all register machines, not stack machines.  The registers are used for computation, and subroutine calls use the stack for return addresses, arguments, and locals.  The USCD Pascal p-machine also uses this design.
