...
Facilities to directly support subroutines are provided by all but one of the real and virtual machines we have programmed. Including physical machines like the Burroughs 5000, CDC 7600, IBM 360, DEC PDP 11 and VAX, Motorola 68000, Sun SPARC, ARM, and a few generations of Intel silicon; as well as virtual machines like the UCSD p-machine, Sun JVM, Wasm, and the sole exception -- the EVM.  In whatever form, these operations provide for
...
The [first machine specification](https://academic.oup.com/comjnl/article/20/3/269/751954) of subroutines goes back to [Turing's Automatic Computing Engine of 1946](http://www.alanturing.net/turing_archive/archive/p/p01/P01-001.html):
...
Turing's machine held data in delay lines made of mercury-filled crystals.  We have better hardware now, but the concept is simple and by now familiar.  As his detailed specifications make clear, Turing's "subsidiary operations" are subroutines, and his "list of notes" is a stack of return addresses.  When a subroutine is called the current program location is pushed on the stack, and when it returns it is popped off of the stack.

We propose to follow Turing's specification, as detailed below.  See the _Rationale_ for further discussion.
...
We assume that `RJUMPSUB` costs 5 (_low_) gas and `RETURNSUB` costs 3 (_verylow_) gas.  We justify these costs below, but they do not make a large difference in this analysis, as much of the improvement is due to `PUSH` and `SWAP` operations that are no longer needed.
...
We can also consider an alternative call mechanism -- call it `ALTSUB` -- that pushes its return address on the `data_stack`:
```
TEST_SQUARE:
    jumpdest        ; 1 gas
    0x02            ; 3 gas
...
SQUARE:
    ...
    returnsub       ; 3 gas
```
*Total 29 gas*, compared to *22 gas* for the `RJUMPSUB` version.  (Setting the `ALTSUB` gas cost to _6_ because using the wide-integer `data_stack` is less efficient than using a stack of native integers.)
...
To support calls and returns we specify a `return stack` of return addresses. The `return stack`, like the EVM `data stack`, is limited to `1024` items. The EVM `return stack` is accessed on via two new subroutine instructions.
...
[EIP-3779: Safer Control Flow for the EVM](./eip-3779.md) specifies creation-time validation to detect invalid contracts.

## Rationale

There are at least two designs for a subroutine facility.

**Turing's design** -- as described above -- keeps return addresses on a dedicated `return stack`.  For a stack machine like the EVM that means that the `data stack` will be used only for computation, not for control flow.
...
We have chosen Turing's design, as have Forth, the JVM, Wasm, and others.  These are all stack machines.
...
This design became popular in the 1970's and remains so.  Examples including the PDP 11, VAX, M68000, SPARC, and Intel architectures.  These are all register machines, not stack machines.  The registers are used for computation, and subroutine calls use the stack for return addresses, arguments, and locals.  The USCD Pascal p-machine also uses this design, although it is in other respects a stack machine.

We give an example of this alternative design above -- running on the EVMs wide stack machine it does not perform well.
...
* It improves performance by
  ...
  * and not imposing a need to swap arguments, return values, and the return address into place.
* It has a 76-year history of working well, especially for stack machines.
