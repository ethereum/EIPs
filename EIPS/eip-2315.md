...
Subroutines were first proposed by Alan Turing (1945) as a means of structuring code for his [Automatic Computing Engine](https://doi.org/10.1093/comjnl/20.3.269):
...
Facilities vary, but all support
* calling a subroutine by saving the current context of execution and transferring control,
* calling further, nested subroutines, and
* restoring the most recently saved context to return to each caller.

These operations have proven their value for efficient implementation and straightforward coding of subroutines across a long line of machines going back over 75 years.  This includes all of the real and virtual machines we have programmed or implemented. Physical machines like the Burroughs 5000, CDC 7600, IBM 360, PDP-11, VAX, M68000, Sun SPARC, ARM, and a few generations of Intel x86. And virtual machines for Lisp, Forth, Pascal, Java, Wasm, and others.

The Ethereum Virtual Machine does not provide subroutine operations. Instead, subroutines can be synthesized by pushing the return address and subroutine address on the data stack and executing an indirect `JUMP` to the subroutine; returns can be synthesized by getting the return address to the top of the stack and doing another indirect `JUMP` back to it.  These conventions cost gas, increase program size, and use stack slots unnecessarily.  And they create unnecessary complexity that is borne by the humans and programs writing, reading, and analyzing EVM code.

### Present Proposal 

We propose -- together with [EIP-4200](./eip-4200.md) -- to provide a _complete, static, efficient_ and _safe_ control-flow facility.
...
* _Efficient_.  There can be substantial reductions in the costs and complexity of calling and optimizing simple subroutines.  We give examples below.
* _Safe_. Establishes rules and an algorithm to validate that programs will not underflow stack, jump to invalid locations, or execute invalid instructions.
...
Turing's design is often used in stack machines, including those for Forth, Java, Wasm, and others.  The data stack is use almost entire for computation, and the dedicated return stack keeps the data stack uncluttered wih return addresses.

#### *Keep return addresses on the data stack.* 

This design is used by all of the register machines we have programmed.  The registers are used primarily for computation, and the stack is used, along with dedicated registers, for subroutine return addresses and call frames for local variables. For all of these machines instruction addresses fit efficiently into machine words on the stack and in registers.

On the EVM this design is currently supported via the dynamic jumps,`JUMP` or `JUMPI`.  They are an obstacle to proving validity in linear time -- Because jumps can be to any destination in the code, quadratic "path explosions" are possible when attempting symbolic execution.  This is why we propose to deprecate `JUMP` and `JUMPI`.
...
We can see that these instructions provide a simpler and more gas-efficient subroutine mechanism than using `JUMP` -- in our examples they cut gas use in half. 
