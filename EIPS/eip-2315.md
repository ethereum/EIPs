...
Conditional jumps and subroutines were first proposed by Alan Turing in 1945 as a means of organizing the logic of the code and the design of the memory crystals for his Automatic Computing Engine:
...
The following is a quasi-Python implementation of an algorithm for predicating code validity.  An equivalent algorithm must be run at initialization time.  
...
For simplicity's sake we assume that all jumpdest analysis and validation has been done, including EIP-4200, and that we have some helper functions.
