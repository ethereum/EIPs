...
Facilities to directly support subroutines are provided by all but one of the real and virtual machines we have programmed. This includes physical machines like the Burroughs 5000, CDC 7600, IBM 360, DEC PDP 11 and VAX, Motorola 68000, Sun SPARC, ARM, and a few generations of Intel x86; as well as virtual machines for Scheme, Pascal, Java, Wasm, and the sole exception -- the EVM.  The details and complexity vary, but in whatever form these facilities provide for
...
* When a subroutine is called
  * the current program adress is pushed on the stack, and
...
We propose to follow Turing's original specification.  We support calls and returns with a `return stack` of return addresses. The `return stack`, like the EVM `data stack`, is limited to `1024` items. The EVM `return stack` is accessed via two new subroutine instructions.
...
We have chosen Turing's design, as have Forth, Java, Wasm, and others.  These are all stack machines, some with stack frames wider and more complex than a machine word. On a stack machine almost all computation is done with the top items on the stack. 

For these machines, an advantage of keeping a separate return stack is that it leaves to the impementation the representation of the data stack, which can remain opaque to the user.  All that matters is that calls and returns work.
...
For all of these machines instruction addresses fit effiently into machine words on the `data stack`.
