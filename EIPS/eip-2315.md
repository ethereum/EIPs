...
**Turing's design** -- as descibed above -- keeps return addresses on a dedicated `return stack`.  For a stack machine like the EVM that means that the `data stack` will be used only for computation, not for control flow.
