...
If [EIP-3337 BEGINDATA](https://github.com/ethereum/EIPs/blob/master/EIPS/eip-3337.md) is implemented then the indirect jumps from EIP-615 -- `JUMPV` and `JUMPSUBV` -- can be implemented.  These would take two arguments on the stack: a constant offset relative to `BEGINDATA` to a jump table, and variable index into that table.  Detailed specifications can await the acceptance EIP-3337.
