...
* The data stack MUST be consistently aligned:
   * The data stack height
     * the absolute difference between the current `stack pointer` and the `stack pointer` on entry to the current subroutine
   * MUST be the same for every reachable path through a given `PC` and 
   * MUST NOT exceed 1024.

## Rationale

This is a purely semantic specification, placing no constraints on the syntax of code sections beyond being a sequence of opcodes and immediate data – a subroutine is not a contiguous sequence of bytecode, it is a subgraph of the control-flow graph.  The EVM is a simple state machine.  We only promise that valid code will not, as it were, jam up the gears of the machine.
...
Using `RJUMPSUB` versus `JUMP` saves _53 - 22 = 31 gas_ — a _58%_ improvement.
...
Clearly, the benefits of this efficiency are greater for programs that have been factored into smaller subroutines.  How small?  Wrapping code in a subroutine costs only _8 gas_ using `RJUMPSUB` and `RETURNSUB` versus _30 gas_ using `JUMP` as above.
...
* `removed_items(opcode,sp)` returns the number of items removed from the `data_stack` by the `opcode`.
* `added_items(opcode,sp)` returns the number of items added to the `data_stack` by the `opcode`.

```
# returns true iff code is valid
def validate_code(code: bytes, pc: int, sp: int, bp: int) -> boolean:
    continuations = [(pc, sp, bp)]
    while continuations:
        pc, sp, bp = continuations.pop()
        while pc < len(code):
            opcode = code[pc]
            if !is_valid(opcode):
                return false
            ...
            sp -= removed_items(opcode, sp)
            if sp < 0
                return false
            sp += added_items(opcode, sp)
