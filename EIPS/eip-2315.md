...
description: Two opcodes safe and efficient subroutines.
...
In concert with [EIP-3540](./eip-4200.md) and [EIP-3670](./eip-3670.md) it ensures that valid code will not execute invalid instructions, jump to invalid locations, or underflow stack. It will maintain consistent numbers of inputs and outputs for subroutines, and loops will not grow the stack.
...
* ensures that stack height can be bounded, (in the absence of recursion) and
...
These changes are compatible with [EIP-4750: EOF - Functions](https://eips.ethereum.org/EIPS/eip-4750#new-execution-state-in-evm)
...
For simplicity's sake we assume that all jumpdest analysis and prior validation has been done, including EIP-3540, EIP-3670, and EIP-4200, so there are no invalid instructions or jumps, and that we have some helper functions.
