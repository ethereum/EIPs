...
At the end of processing any execution block starting from the `FORK_BLOCK` (i.e. after processing all transactions and after performing the block body withdrawal requests validations), the implementation must invoke a call to the contract at `WITHDRAWAL_REQUEST_PREDEPLOY_ADDRESS` as `SYSTEM_ADDRESS` with no calldata. This invocation triggers the following behavior:
...
Additionally, the system call and subsequent block processing must conform to the following:
...
* Both the gas limit assigned to the call and the gas consumed must be excluded from any checks against the block’s gas limit.
* The call does not follow [EIP-1559](./eip-1559.md) fee burn semantics—no value should be transferred as part of this call.
* If there is no code at `WITHDRAWAL_REQUEST_PREDEPLOY_ADDRESS`, the call must NOT fail silently. The corresponding block must be marked invalid.
* If the call to the `WITHDRAWAL_REQUEST_PREDEPLOY_ADDRESS` contract fails or returns an error, the block must be invalidated
...
Note: the above code reverts if the fee is too high, the fee can change significantly between creation of a withdrawal request transation and its inclusion into a block, thus, this check is very important to avoid overpayments.
