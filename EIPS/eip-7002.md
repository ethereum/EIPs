---
eip: 7002
title: Execution layer triggerable exits and partial withdrawals
...
def add_withdrawal_request(Bytes48: validator_pubkey, uint64: amount):
    ...
    sstore(WITHDRAWAL_REQUEST_PREDEPLOY_ADDRESS, queue_storage_slot + 2, validator_pubkey[32:48])
...
* New function in `process_execution_layer_withdrawal_request` that has similar functionality to `process_voluntary_withdrawal_request` but that can fail validations (e.g. validator is already exited) without the block failing (similar to deposit coming from EL)
* `process_execution_layer_withdrawal_request` called in `process_operations` for each `ExecutionLayerWithdrawalRequest` found in the `ExecutionPayload`
