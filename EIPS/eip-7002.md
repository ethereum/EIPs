...
Each withdrawal request must appear in the EIP-7685 requests list in the order they are returned from `dequeue_withdrawal_requests()`. If, however, at the end of processing an execution block the System Call to the `WITHDRAWAL_REQUEST_PREDEPLOY_ADDRESS` contract fails, the block must be considered invalid.
