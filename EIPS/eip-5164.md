...
The `BasicMessageDispatcher` is an extension of `MessageDispatcher` that defines a method, `dispatchMessage`, for dispatching an individual message to be executed on the `toChainId`.
...
`BasicMessageDispatcher`s MUST emit the `MessageDispatched` event when a message is dispatched.

`BasicMessageDispatcher`s MUST forward the message to a `MessageExecutor` on the `toChainId`.

`BasicMessageDispatcher`s MUST use a unique `messageId` for each message.

`BasicMessageDispatcher`s MUST return the `messageId` to allow the message sender to track the messages.

`BasicMessageDispatcher`s MAY require payment.

```solidity
interface BasicMessageDispatcher is MessageDispatcher {
...
```solidity
interface MultiChainMessageDispatcher is MessageDispatcher {
