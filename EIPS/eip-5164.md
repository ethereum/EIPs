...
The specification defines two components: the "Cross Chain Relayer" and the "Cross Chain Executor". The Cross Chain Relayer lives on calling side, and the executor lives on the receiving side. Calls sent to Cross Chain Relayers will move through a transport layer to the Cross Chain Executor, where they are executed. Implementations of this specification must implement both components.

Developers will use this specification by writing smart contracts that can pass calls into a Cross Chain Relayer and receive calls from a Cross Chain Executor.
...
The `CrossChainRelayer` lives on the chain from which messages are sent. The Relayer's job is to broadcast messages through a transport layer to one or more `CrossChainExecutor` contracts.

#### Methods

**relayCalls**

Will relay a batch of Call structs to be executed by any `CrossChainExecutor`(s) bound to this Relayer. The gasLimit is used as a limit on the executing side.
...
`CrossChainExecutor`s SHOULD use the `nonce` to verify that the message being relayed has not been executed yet.

`CrossChainExecutor`s SHOULD append the `nonce` to the call data being executed.

`CrossChainExecutor`s SHOULD use [EIP-2771](./eip-2771.md) for authentication and append the address of the Transaction Signer on the origin chain to the end of the call data being executed.

#### Error handling

**CallFailure**

`CrossChainExecutor`s SHOULD revert with `CallFailure` error if a call fails.
...
Additionally, the `nonce` is appended to the call, so applications can ensure that messages are being executed in order.
