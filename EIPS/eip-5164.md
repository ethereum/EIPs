...
The `CrossChainExecutor` will execute a nonce only once, but may execute nonces in any order. This specification makes no ordering guarantees, because messages may travel async through the transport layer.
...
`CrossChainExecutor`s SHOULD revert with `CallsAlreadyExecuted` error if a batch of calls has already been executed.
...
The `ExecutorAware` interface allows applications to verify that the call has been executed by a trusted `CrossChainExecutor`.

Additionally, the `nonce` and `sender` appended to the call, respectively, allow applications to ensure that messages are being executed in order and to verify the true sender of the message on the origin chain.

#### Methods

**isTrustedExecutor**

`ExecutorAware` SHOULD verify that `msg.sender` is a trusted `CrossChainExecutor`.

```solidity
interface ExecutorAware {
  function isTrustedExecutor(address executor) public view returns (bool isTrusted);
}
```
