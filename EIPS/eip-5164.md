...
Will relay a single `Call` struct as a batch of one to be executed by any `CrossChainExecutor`(s) that executes the calls from this Relayer. The `gasLimit` is used as a limit on the executing side.
...
interface CrossChainRelayer {
  function relayCall(Call calldata call, uint256 toChainId, uint256 gasLimit) external payable returns (uint256 nonce);
}
```
...
interface CrossChainRelayer {
  function relayCalls(Call[] calldata calls, uint256 toChainId, uint256 gasLimit) external payable returns (uint256 nonce);
}
```
...
`CrossChainRelayer`s MAY revert with `GasLimitTooHigh` if the `gasLimit` passed to `relayCall` or `relayCalls` is higher than the maximum gas limit accepted by the bridge being used.
