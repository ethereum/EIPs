...
There are also extensions of `MessageExecutor`, each defining a method for initiating a message or message batch:

- `SingleChainMessageDispatcher`
- `MultiChainMessageDispatcher`
...
The `SingleChainMessageDispatcher` is an extension of `MessageDispatcher` that defines a method, `dispatchMessage`, for dispatching an individual message to a predetermined `MessageExecutor` on the destination chain.

#### Methods

**dispatchMessage**

Will dispatch a message to be executed by the `MessageExecutor` that executes messages from this `MessageDispatcher`.

`SingleChainMessageDispatcher`s MUST emit the `MessageDispatched` event when a message is dispatched.

`SingleChainMessageDispatcher`s MUST forward the message to the predetermined `MessageExecutor`.

`SingleChainMessageDispatcher`s MUST use a unique `messageId` for each message.

`SingleChainMessageDispatcher`s MUST return the `messageId` to allow the message sender to track the messages.

`SingleChainMessageDispatcher`s MAY require payment.

```solidity
interface SingleChainMessageDispatcher {
  function dispatchMessage(address to, bytes calldata data) external payable returns (bytes32 messageId);
}
```
...
The `MultiChainMessageDispatcher` is an extension of `MessageDispatcher` that defines a method, `dispatchMessage`, for dispatching an individual message to be executed on the `toChainId`.
...
`MultiChainMessageDispatcher`s MUST emit the `MessageDispatched` event when a message is dispatched.

`MultiChainMessageDispatcher`s MUST forward the message to the `MessageExecutor` on the `toChainId`.

`MultiChainMessageDispatcher`s MUST use a unique `messageId` for each message.

`MultiChainMessageDispatcher`s MUST return the `messageId` to allow the message sender to track the messages.

`MultiChainMessageDispatcher`s MAY require payment.

```solidity
interface MultiChainMessageDispatcher {
...
`MultiChainMessageDispatcher`s MUST emit the `MessageBatchDispatched` event when a message batch is dispatched.

`MultiChainMessageDispatcher`s MUST forward the message batch to the `MessageExecutor` on the `toChainId`.

`MultiChainMessageDispatcher`s MUST use a unique `messageId` for each batch of messages.

`MultiChainMessageDispatcher`s MUST return the `messageId` to allow the message sender to track the message batches.
...
`MessageExecutor`s MUST revert if any message in a message batch fails and SHOULD emit a `MessageBatchFailure` custom error.
