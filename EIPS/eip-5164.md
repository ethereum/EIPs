...
The specification defines two components: the "Cross Chain Relayer" and the "Cross Chain Executor". The Cross Chain Relayer lives on the calling side, and the executor lives on the receiving side. Calls sent to Cross Chain Relayers will move through a transport layer to the Cross Chain Executor, where they are executed. Implementations of this specification must implement both components.
...
`CrossChainExecutor`s MUST NOT execute a nonce more than once.

`CrossChainExecutor`s MUST append the ABI-packed (`sender`, `nonce`) to the calldata for each Call being executed. This follows [EIP-2771](./eip-2771.md), and allows the receiver of the call to check the true `sender` of the transaction, and use the `nonce` to apply any transaction ordering logic.
