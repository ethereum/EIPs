...
- The Provider and Wallet programs are sandboxed from each other.
...
In order to protect user privacy, the authors recommend not exposing any accounts by default, and implementing the `eth_requestAccounts` Ethereum RPC method (see [EIP-1102](https://eips.ethereum.org/EIPS/eip-1102)) to grant consumers the ability to request the user's accounts.
