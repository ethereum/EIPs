...
This way, we could use accounts as personas. Also if we don't harden the level indexes after the account index, we could compute all public keys
...
If we don't harden the application path we could get all app keys for every applications from persona key.
