...
If `FACTORY_ADDRESS` does not hold the runtime code above when this EIP activates, client software MUST set its code to the runtime code above and its nonce to `1` if zero, as the first state modification of the activation block. An account already holding the runtime code MUST NOT be modified. Where block-level access lists ([EIP-7928](./eip-7928.md)) are active, the activation block's block access list MUST include `FACTORY_ADDRESS`: with the changes made by this transition at `block_access_index = 0` when it is installed, or as an accessed account otherwise.
