...
**Every contract compliant with this ERC must implement the `ERC721` and `ERC165` interfaces** (subject to "caveats" below):
...
The **metadata extension** is OPTIONAL for smart contracts compliant with this ERC (see "caveats", below). This allows your smart contract to be interrogated for its name and for details about the assets which your NFTs represent.
...
The **enumeration extension** is OPTIONAL for smart contracts compliant with this ERC (see "caveats", below). This allows your contract to publish its full list of NFTs and make them discoverable.
...
The 0.4.20 Solidity interface grammar is not expressive enough to document this ERC standard. A contract which complies with this ERC MUST also abide by the following:
...
- Solidity issue #3419: A contract that implements `ERC721Metadata` or `ERC721Enumerable` SHALL also implement `ERC721`.
...
Every NFT is identified by a unique `uint256` ID inside the `ERC721` smart contract. This identifying number SHALL NOT change for the life of the contract. The pair `(contract address, uint256 tokenId)` will then be a globally unique and fully-qualified identifier for a specific asset on an Ethereum chain. While some smart contracts implementing this ERC may find it convenient to start with ID 0 and simply increment by one for each new NFT, callers SHALL NOT assume that ID numbers have any specific pattern to them, and MUST treat the ID as a "black box". Also note that NFTs MAY become invalid (be destroyed). Please see the enumeration functions for a supported enumeration interface.

The choice of `uint256` allows a wide variety of applications because UUIDs and sha3 hashes are directly convertible to `uint256`.

**Transfer Mechanism**

The `ERC721` interface standardizes a safe transfer function `safeTransferFrom` (overloaded with and without a `bytes` parameter) and an unsafe function `transferFrom`. Transfers may be initiated by:
...
Failed transactions will throw, a best practice identified in [ERC-223](./eip-223.md) and OpenZeppelin's implementation of SafeERC20.sol. ERC-20 defined an `allowance` feature, this caused a problem when called and then later modified to a different amount, as on OpenZeppelin issue \#438. In `ERC721`, there is no allowance because every NFT is unique, the quantity is none or one. Therefore we receive the benefits of ERC-20's original design without problems that have been later discovered.
...
**[ERC-165](./eip-163.md) Interface**

We chose Standard Interface Detection ([ERC-165](./eip-165.md)) to expose the interfaces that smart contracts implementing this ERC support.

A future EIP may create a global registry of interfaces for contracts. We strongly support such an EIP and it would allow your implementation of this ERC your are reading to implement `ERC721Enumerable`, `ERC721Metadata`, or other interfaces by delegating to a separate contract.
...
This illustration makes clear: this non-fungible token standard scales.
...
We have required `name` and `symbol` functions in the metadata extension. Every token EIP and draft we reviewed ([ERC-20](./eip-20.md), [ERC-223](./eip-223.md), and [ERC-777](./eip-777.md)) included these functions.

We remind implementation authors that the empty string is a valid response to `name` and `symbol` if you protest to the usage of this mechanism. We also remind everyone that any smart contract can use the same name and symbol as *your* contract. How a client may determine which smart contracts implement ERC are well-known (canonical) is outside the scope of this standard.
...
A significant amount of discussion occurred on the original GitHub issue number 721, additionally we held a first live meeting on Gitter that had good representation and well advertised (on Reddit, in the Gitter #ERC channel, and the original issue number 721). Thank you to the participants:
...
`nibbstack/erc721` Token includes test cases written using Truffle.

## Implementations

`nibbstack/erc721` -- a reference implementation
...
1. The Original Issue Number 721. https://github.com/ethereum/eips/issues/721
...
1. Live Gitter discussion (announcement of first live discussion). https://github.com/ethereum/eips/issues/721#issuecomment-358369377
1. ETHDenver 2018. https://ethdenver.com

**NFT Implementations and Other Projects**

1. CryptoKitties. https://www.cryptokitties.co
1. Nibbstack ERC721. https://github.com/0xcert/ethereum-erc721
