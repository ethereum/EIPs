...
This standard is inspired by the [ERC-20](./eip-20) token standard and builds on two years of experience since EIP-20 was created. EIP-20 is insufficient for tracking NFTs because each asset is distinct (non-fungible) whereas each of a quantity of tokens is identical (fungible).
...
We chose Standard Interface Detection (ERC-165) to expose the interfaces that smart contracts implementing this ERC support.
