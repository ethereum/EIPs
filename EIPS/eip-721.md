...
interface ERC721 /* is ERC165 */ {
    ...
    /// @return The approved address for this NFT, or the zero address if there is no approved address
...
- **Charge a fee to both parties of a transaction** — require payment when calling `approve` with a non-zero `_approved` if it was previously address(0), refund payment if calling `approve` with address(0) if it was previously not address(0), require payment when calling any transfer function, require transfer parameter `_to` to equal `msg.sender`, require transfer parameter `_to` to be the approved address for the NFT
