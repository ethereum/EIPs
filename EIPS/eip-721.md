...
interface ERC721 /* is ERC165 */ {
    ...
    ///  This event emits when NFTs are created (`from` == address(0)) and destroyed
    ///  (`to` == address(0)). Exception: during contract creation, any number of NFTs
    ///  may be created and assigned without emitting Transfer. At the time of
    ///  any transfer, the approved address for that NFT (if any) is reset to none.
    event Transfer(address indexed _from, address indexed _to, uint256 indexed _tokenId);

    /// @dev This emits when the approved address for an NFT is changed or
    ///  reaffirmed. address(0) indicates there is no approved address.
    ...
    /// @dev NFTs assigned to address(0) are considered invalid, and this
    ///  function throws for queries about address(0).
    /// @param _owner An address for whom to query the balance
    /// @return The number of NFTs owned by `_owner`, possibly zero
    function balanceOf(address _owner) external view returns (uint256);

    /// @notice Find the owner of an NFT
    /// @dev NFTs assigned to address(0) are considered invalid, and queries
    ///  about them do throw.
    ...
    ///  not the current owner. Throws if `_to` is address(0). Throws if
    ...
    /// @dev address(0) indicates there is no approved address.
    ...
    /// @return The approved address for this NFT, or address(0) if there is no approved address
...
interface ERC721Enumerable /* is ERC721 */ {
    ...
    ///  them has an assigned and queryable owner not equal to address(0)
    ...
    ///  `_owner` is address(0), representing invalid NFTs.
...
- **Charge a fee to both parties of a transaction** — require payment when calling `approve` with a non-zero `_approved` if it was previously address(0), refund payment if calling `approve` with address(0) if it was previously a non-zero address, require payment when calling any transfer function, require transfer parameter `_to` to equal `msg.sender`, require transfer parameter `_to` to be the approved address for the NFT
