...
interface IDomain is IERC165 {
    ...
    /// @dev        This should return `false` if `hasDomain(name)` is `true`
    ...
    /// @dev        This should return `false` if `hasDomain(name)` is `true` and
...
#### Mitigation
Before acquiring a domain that has been pre-linked, it is reccomended to always have the contract **and** all the parents up to the root audited.
