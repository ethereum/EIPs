...
As per [EIP-165](./eip-165.md), `supportsInterface(type(IDomain).interfaceId)` MUST return `true`.
...
As per [EIP-165](./eip-165.md), `supportsInterface(type(IDomainEnumerable).interfaceId)` MUST return `true` if `IDomainEnumerable` is used.
...
As per [EIP-165](./eip-165.md), `supportsInterface(type(IDomainAccessControl).interfaceId)` MUST return `true` if `IDomainAccessControl` is used.
