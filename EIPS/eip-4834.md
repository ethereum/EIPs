...
This is a standard for generic name resolution with arbitrarily complex access control and resolution. It permits a contract that implements this EIP (referred to as a "domain" hereafter) to be addressable with a more human-friendly name, with a similar purpose to [EIP-137](./eip-137.md) (also known as "ENS").
...
As per [EIP-165](./eip-165.md), `supportsInterface(0xTODO)` MUST return `true`.
...
Solidity Interface with NatSpec & OpenZeppelin v4 Interfaces (also available at [IDomainRegisterable.sol](../assets/eip-4834/IDomainEnumerable.sol)):
...
As per [EIP-165](./eip-165.md), `supportsInterface(0xTODO)` MUST return `true` if `IDomainRegisterable` is used.
