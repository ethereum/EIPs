...
For example, a PBM creator may want to ensure only 50% of PBM Series A can be spent in supermarkets, while there are no restriction on proportion of PBM Series B that can be spent in supermarkets. For such a usecase, he can implement a plugin smart contract to keep track of supermarket spendings by PBM users, and implement a hook to validate that less than 50% of the PBM Series A issued a user is spent in a supermarket. This allows him to use the same generic PBM wrapper and sovToken for both PBM Series A and B. In addition, PBM series A will register the plugin module and hook module for additional data tracking and validations.

### Auditability

PBM Wrapper **SHOULD** provide the public easily accessible mechanism(s) to verify the smart contract logic for unwrapping a PBM if proof of trust is required. Such mechanisms could be leveraged by automated validation or asynchronous user verifications from transacting parties and/or whitelisted third parties attestations.
...
A PBM Wrapper **SHOULD** be able to wrap multiple types of compatible store of value tokens (e.g. the same PBM Wrapper should be able to wrap USDC and XSGD). Store of value tokens wrapped by the same PBM wrapper may or may not be fungible to one another. The standard does NOT mandate how an implementation must do this.
...
- The blacklist is a compulsory requirement to ensure that accounts which were banned for various reasons (e.g. address owner has re-registered a new account, address owner suspended/withdrawn/expelled due to complaints or law enforcement reason etc) are explicitly flagged to ensure that no form of PBM interaction can occur on the blacklisted address. 

Why we can’t have either a whitelist or a blacklist?

- While the same effect can be obtained by only having a whitelist, the advantage of having both a blacklist and whitelist allows for greater modularity and reuseability. For example, a user may freely transfer PBM tokens to whitelisted and non-whitelisted addresses with different behaviours depending on the PBM wrapper hooks logic. However, a blacklisted address can never be interacted with at all times. 

- A blacklisted address list is also more likely to be referenced among various PBM smart contracts, as opposed to a whitelisted address list, since most creators would want to steer clear of the same bad actors in the space, while each maintains their own unique set of whitelisted merchant requirements.

- Using a blacklist alone to implement purpose-bound money is not practical, as you would need to have a list of all addresses to be excluded and update it whenever a new account is created.
...
- PBM can be transferred freely, but the recipients allowed to unwrap the PBM and take delivery of the underlying sovToken must be limited in order to differentiate it from plain vanilla stablecoins that do not have such restrictions.
...
- This standard adheres to the push transaction model where the transfer of PBM is initiated on the sender's side. Modern wallets can support the required PBM logic by embedding the unwrapping logic within the ERC-1155 `safeTransfer` function.
...
3. The proposed PBM Architecture is _intentionally_ modular to enable greater customisability and reusability of smart contracts.
