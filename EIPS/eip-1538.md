...
contract ExampleTransparentContract {
  ...
  function addFunction(address _erc1538Delegate, address contractAddress, string _functionSignatures, string commitMessage) internal {    
    // 61455567 == bytes4(keccak256("updateContract(address,string,string)"))
    bytes memory calldata = abi.encodeWithSelector(61455567, contractAddress, _functionSignatures, commitMessage);
    bool success;
    assembly {
  ...
  constructor(address _erc1538Delegate) public {
    ...
    bytes memory functions = "delegateAddress(string)";
...
```
As can be seen in the above implementation, every function call is delegated to a delegate contract, unless the function is defined directly in the transparent contract.
...
[ERC1538: Future Proofing Smart Contacts and Tokens](https://coinjournal.net/erc1538-future-proofing-smart-contacts-and-tokens/)
