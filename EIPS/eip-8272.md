...
Two prohibitions in this section need no explicit check in the code. In static context the `sstore` itself fails, so no storage is written. Under `DELEGATECALL` or `CALLCODE` the `sstore` writes to the calling account's storage, which is not recent-root storage, and `RECENT_ROOT_ADDRESS[storage_key]` is unchanged.
