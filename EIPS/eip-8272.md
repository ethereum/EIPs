...
`RECENT_ROOT_ADDRESS` MUST be distinct from EIP-8141's `EXPIRY_VERIFIER` and from every other protocol-installed account activated in the same fork. If [EIP-8250](./eip-8250.md) is active, `RECENT_ROOT_ADDRESS` MUST be distinct from `NONCE_MANAGER`.
...
The post-fork signature hash is `compute_sig_hash(tx)` as defined by EIP-8141, applied to the post-fork ten-field payload. The signing projection elides the raw `signature` bytes of every signature with empty `msg` without modifying the decoded transaction.
