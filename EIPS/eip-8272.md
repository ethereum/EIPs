...
| ------------------------------------ | -------------------------------------------------------------------------------: |
| `FORK_TIMESTAMP`                     |                                                                            `TBD` |
| `RECENT_ROOT_ADDRESS`                 |                     `0x0000E45cBB7961F3A46fDD4db33f0A0167F88272` |
| `RECENT_ROOT_CODE`                    |                                                    see [Bytecode](#bytecode) |
...
Only execution whose storage context is `RECENT_ROOT_ADDRESS` writes recent-root storage. A `DELEGATECALL` or `CALLCODE` to this code, or a call to an [EIP-7702](./eip-7702.md)-delegated account whose delegation target is `RECENT_ROOT_ADDRESS`, executes the code in the calling account's own storage context: the write lands in that account's storage and the call returns successfully. Such a write MUST NOT be treated as a write to recent-root storage.
...
`RECENT_ROOT_CODE` is the following runtime code, 144 bytes:
...
The same code as EVM assembly, with the two `push32` values being `RECENT_ROOT_ENTRY_DOMAIN` and `RECENT_ROOT_STORAGE_DOMAIN`:

```asm
// revert unless call value is zero and calldata is exactly 64 bytes
callvalue
iszero
calldatasize
push1 0x40
...
// source_id = keccak256(msg.sender || salt), over memory [0x0c, 0x40)
caller
push1 0x00
mstore
push1 0x20
...
// over memory [0x40, 0xa8). The slot word is stored before the source_id word
// that follows it, so that only its low eight bytes survive.
dup1
push32 0x8f42481679c8e6fefa040974b3c905e0ce3f2e464ba93acdb074a41181617efc
push1 0x40
mstore
slotnum
push1 0x68
...
// over memory [0xa8, 0xf0). RECENT_ROOT_LENGTH is a power of two, so the
// modulus is a mask.
dup2
push32 0xbdc897da2177d260ff5f4be5d4b2aad43f89c3347a305b584fa5a2546d053daa
push1 0xa8
mstore
push2 0x1fff
slotnum
...
The code reads the current slot with the [EIP-7843](./eip-7843.md) `SLOTNUM` opcode, so a write is bound to the slot of the block that includes it without the contract knowing the slot duration.

Two prohibitions in this section need no explicit check in the code. In static context the `sstore` itself fails, so no storage is written. Under `DELEGATECALL` or `CALLCODE`, or when `RECENT_ROOT_ADDRESS` is the delegation target of an [EIP-7702](./eip-7702.md) account, the `sstore` writes to the executing account's own storage, which is not recent-root storage, so `RECENT_ROOT_ADDRESS[storage_key]` is unchanged. These calls do not revert; they return successfully after writing foreign storage.
...
```json
{
  ...
  "maxFeePerGas": null,
  "value": "0x0",
  "input": "",
  "v": "0x1b",
  "r": "0x539",
  "s": "0xa71bb183a8478f55",
  "hash": "0x82dce0e5ca09b26c8dffd432baf72d89b366e2b28f661b3d94d7d3d7018ce698"
}
```

Note, the input in the transaction has a simple constructor prefixing the desired runtime code.

The sender of the transaction can be calculated as `0x1Dd6Bb430Ce28eCD180bA2CeeB1BD378E63aA7DF`. The address of the first contract deployed from the account is `rlp([sender, 0])` which equals `0x0000E45cBB7961F3A46fDD4db33f0A0167F88272`. This is how `RECENT_ROOT_ADDRESS` is determined. Although this style of contract creation is not tied to any specific initcode like create2 is, the synthetic address is cryptographically bound to the input data of the transaction (e.g. the initcode).
...
This EIP MUST activate at or after [EIP-8141](./eip-8141.md) and [EIP-7843](./eip-7843.md), because `RECENT_ROOT_CODE` reads the current slot with the [EIP-7843](./eip-7843.md) `SLOTNUM` opcode.
