...
* the payload does not contain `recent_root_references` immediately after `blob_versioned_hashes`;
...
* add `recent_root_calldata_cost + recent_root_reference_intrinsic_gas` to `tx_gas_limit`;
* add `recent_root_reference_intrinsic_gas` outside the `max` in `charged_gas`;
* add `recent_root_calldata_cost` to the execution branch of that `max`;
* add `recent_root_calldata_tokens` to `calldata_tokens`, which is used by the EIP-7623 floor; and
* add `recent_root_reference_intrinsic_gas + TOTAL_COST_FLOOR_PER_TOKEN * recent_root_calldata_tokens` to the minimum valid `tx_gas_limit`.

All uses of `charged_gas` use the value after these additions. No other gas term is changed by this EIP.
