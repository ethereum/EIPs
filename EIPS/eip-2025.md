...
```
Beginning with block `ISTANBUL_BLOCK_HEIGHT`, the dev reward is added to the participating addresses within `BENEFICIARY_ADDRESSES` at each block until the end of the `EMISSION_SCHEDULE`.

```
IF (CURRENT_BLOCK - ISTANBUL_BLOCK_HEIGHT  <= EMISSION_SCHEDULE)
