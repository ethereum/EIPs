...
## Simple Summary
Add  `0.0055 ETH` per block for 18 months as a developer block reward reserved for funding Ethereum1.X development.
...
REWARD_DURATION_IN_BLOCKS = 3,100,000
...
```
Beginning with block `ISTANBUL_BLOCK_HEIGHT`, the dev reward is added to the participating addresses within `BENEFICIARY_ADDRESSES` at each block until the end of the `REWARD_DURATION_IN_BLOCKS`.
...
```
 Development Loan: 0.005 ETH per block: 15500 ETH total
 Development Loan Interest 10%: 0.0005 ETH per block: 1550 ETH total
...
* Membership and creation managed by the Core Devs
