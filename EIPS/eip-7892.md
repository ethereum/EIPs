...
This EIP introduces **Blob Parameter Only (BPO) Hardforks**, a lightweight mechanism for incrementally scaling Ethereum’s blob capacity through targeted hard forks that modify only blob-related parameters: `blob target`, `blob limit`, and `baseFeeUpdateFraction`. It also introduces an additional optional limit `blob limit per tranaction`. Unlike traditional hard forks, which require extensive coordination and introduce broader protocol changes, BPO forks enable rapid, low-overhead scaling of blob capacity in response to **real-world demand and network conditions**.
...
- **Blob Limit per transaction (`blob_limit_per_tx`)**: The maximum number of blobs per transaction.
...
- The `maxBlobsPerTx` field is optional and defaults to value of `max` when omitted. It's not significant for the consensus layer.
...
#### Tx pool considerations

A bpo fork may allow less blobs per block or per transaction than previous one, which means that at some point transactions with high blob count located in the pool may become unusable. Such trasnactions **MUST** be stopped from propagation and should be considred for eviction from tx pool.
