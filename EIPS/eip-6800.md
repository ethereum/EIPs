...
def get_tree_key_for_codehash(address: Address32):
    return get_tree_key(address, 0, CODEHASH_LEAF_KEY)
```
...
The `code_hash` and `code_size` fields are set upon contract or EoA creation.
