...
- **Armatures**
For example, every metaverse uses its own armature. There is [a standard](https://cascadeur.com/help/getting_started/import_fbxdae/using_mixamo_rig_in_cascadeur) for humanoids but it is not being used for every metaverse and not all the metaverses use humanoids. For example, Decentraland has a different esthetic than Cryptovoxels and TheSandbox. It means that every metaverse will need a different model and they may have the same extension (GLB, GLTF)

![Screen Shot 2022-04-06 at 14.24.42|690x355](https://ethereum-magicians.org/uploads/default/original/2X/4/4c0c9d1fda4579e97225499d5d1a9c1fd5fda962.jpeg)
...
By using namespaces, every project can access their respective supported files by just doing something like `namespaces["PROJECT_NAME"].uri`.
