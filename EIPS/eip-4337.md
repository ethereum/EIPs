...
Not all block-builders on the network are required to be bundlers, and bundlers are not required to be block builders.
It is highly beneficial to combine these roles as such an actor will have access to both mempools to construct the most
profitable blocks while also having access to the latest state and a full control of the bundle ordering and inclusion.

A standalone bundle cannot broadcast to the public transaction mempool its bundles as a `handleOps` transaction.
If the bundler is not a block builder itself, it MUST rely on a special trusted protective mechanism when interacting with the
block builder to safely include `handleOps` transactions.
For example, `bundler` can rely on a MEV searcher service or a `eth_sendRawTransactionConditional` RPC API if it is available.
...
Transient Storage slots defined in EIP-1153 and accessed using `TLOAD` (`0x5c`) and `TSTORE` (`0x5d`) opcodes
...
#### Ban for State-dependant Precompile Calls

In case the contracts are executed in a version of the EVM that includes any precompile contract whose results may be
dependent on any external state, access to such precompiles must be banned in the verification code.

For example, the Arbitrum optimistic rollup exposes the current Ethereum Mainnet base fee per gas to contracts by
providing the `ArbGasInfo` precompile with a `getL1BaseFeeEstimate` function,
as described here: https://developer.arbitrum.io/arbos/precompiles.
This information can be used to perform a DoS attack on the UserOperations mempool if accessed during validation phase.
...
* Run `debug_traceCall` with maximum possible gas, to enforce the validation opcode banning rules,
...
* If the error is caused by a `factory` (error code is `AA1x`) or a paymaster (error code is `AA3x`), and the `sender`
  of the UserOp **is not** a staked address, then also drop from mempool all other UserOps of the guilty factory or paymaster.
* If the error is caused by a `factory` (error code is `AA1x`) or a paymaster (error code is `AA3x`), and the `sender`
  of the UserOp **is** a staked entity, do not remove all the `factory` / `paymaster` UserOps from the mempool.
  Instead, drop all UserOps related to the staked sender from mempool and ban the sender address.
...
it is critical that the exact same opcode banning rules are enforced for the `handleOps` validation as for individual UserOperations.
