...
However, `handleOps` transactions cannot broadcast to public transaction mempool by the `bundler`.
...
block builder to safely include `handleOps` transactions. For example, `bundler` can rely on a MEV searcher service.
