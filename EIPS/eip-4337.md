...
#### Ban Non-standard Precompile Calls

In case the contracts are executed in a version of the EVM that includes any non-standard precompile contract,
access to such precompiles must be banned in the verification code.

Only the standard Ethereum precompiles defined at addresses from `0x01` to `0x09` are allowed.

As an example of a precompile that breaks the verification sandboxing,
see how the Arbitrum optimistic rollup exposes the current Ethereum Mainnet base fee per gas to contracts by
