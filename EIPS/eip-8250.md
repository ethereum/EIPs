...
`NONCE_MANAGER` MUST be distinct from EIP-8141's `EXPIRY_VERIFIER` and from every other protocol-installed account activated in the same fork. If [EIP-8272](./eip-8272.md) is active, `NONCE_MANAGER` MUST be distinct from `RECENT_ROOT_ADDRESS`.
...
If multiple system-account transitions activate in `B`, clients MUST evaluate every transition's address and precondition against `B`'s parent state before applying any activation transition.
