...
`nonce_keys` is a list of `uint256` values; `nonce_seq` is a `uint64`. Each nonce key and `nonce_seq` MUST be encoded as canonical minimal-length RLP integers. The most significant byte of each nonce key is used to denote the type of the nonce and MUST be `NONCETYPE_GENERAL` or `NONCETYPE_BINARY`; other values are reserved for future EIPs. The frame layout and all other EIP-8141 transaction fields are unchanged.

```python
def get_nonce_type(nonce_key):
    return uint256_to_bytes32(nonce_key)[0]
```
...
- `nonce_seq >= 2**64`;
- for any item in `nonce_keys`, its most significant byte is larger than `NONCETYPE_MAX`;
...
for nonce_key in tx.nonce_keys:
    if get_nonce_type(nonce_key) == NONCETYPE_GENERAL:
        assert tx.nonce_seq == current_nonce_seq(tx.sender, nonce_key)
    elif get_nonce_type(nonce_key) == NONCETYPE_BINARY:
        assert tx.nonce_seq == 0
        assert current_nonce_seq(tx.sender, nonce_key) == 0
    else:
    ...
    if nonce_keys == [0]:
        ...
        for nonce_key in nonce_keys:
            if get_nonce_type(nonce_key) == NONCETYPE_GENERAL:
                ...
                state[NONCE_MANAGER].storage[slot(sender, nonce_key)] = 1  # nonce_seq is guaranteed to be 0.
            else:
                assert False  # unreachable
```
...
The first byte of each `nonce_keys` value is used to denote the type of nonce, with a value of `NONCETYPE_GENERAL` indicating the general case and `NONCETYPE_BINARY` indicating the case of binary nonces. Other values are reserved for future use (e.g. nonce keys valid only in a limited timespan). Binary nonces can only take values 0 and 1 and are sufficient for nullifiers. Making the type of the nonce explicit is done to simplify future changes and optimizations. In particular, special-casing binary nonces allows for significant optimizations, particularly to reduce the amount of VOPS storage that (weak) stateless nodes will need to hold. Making the distinction already now will allow those optimizations to be applied retroactively. This EIP uses the first byte of the nonce key for simplicity; the alternative would be adding an extra field.

`nonce_keys` are `uint256` values because privacy protocols often derive keys from 32-byte nullifiers, commitments, or hash outputs. [ERC-4337](./eip-4337.md) uses the same key/sequence model but has only one 32-byte nonce field, so it packs a 24-byte key and an 8-byte sequence into that field. This EIP uses explicit fields: 31-byte nonce keys + 1 byte for the type and an 8-byte `nonce_seq`. This allows to faithfully represent ERC-4337 (key, sequence) pairs.

This EIP intentionally uses one `nonce_seq` shared by all selected `nonce_keys`, rather than a vector of `(nonce_key, nonce_seq)` pairs. A shared sequence preserves the scalar replay semantics of EIP-8141 and keeps transaction introspection simple. Strictly increasing `nonce_keys` give each selected key set one canonical transaction encoding and one canonical hash. Applications that require single-use capabilities, including privacy nullifiers and expiring authorizations, can derive distinct nonce keys and use `nonce_seq == 0` and binary nonce type. Applications that require atomic advancement of unrelated nonce domains at unrelated sequence numbers are outside the scope of this EIP and may be addressed by a future extension.
...
If the nonce key is derived via hashing, only up to 31 bytes of the hash output can be used, since the first byte of the nonce key must be set to 0 or 1 according to its type. 31 bytes still provide sufficient security against collisions.
...
Each consumed `(sender, nonce_key != 0)` occupies one persistent slot in `NONCE_MANAGER` storage, and entries are never deleted. Creating a slot consumes state gas, so a block can create at most `block_gas_limit // KEYED_NONCE_FIRST_USE_STATE_GAS` new keyed nonce slots. `nonce_seq == MAX_NONCE_SEQ` represents an exhausted key for nonce type `NONCETYPE_GENERAL`; a key at that sequence cannot advance further. Binary nonces cannot advance past 1, so they are only ever written once and then never modified.
