...
```
rlp([chain_id, nonce, max_priority_fee_per_gas, max_fee_per_gas, gas_limit, destination, amount, data, blob_versioned_hashes, access_list, y_parity, r, s])`.
```
...
The `blob_versioned_hashes` field represents a list hash outputs from `kzg_to_versioned_hash`.
...
`keccak256(BLOB_TX_TYPE || rlp([chain_id, nonce, max_priority_fee_per_gas, max_fee_per_gas, gas_limit, destination, amount, data, blob_versioned_hashes, access_list]))`.
...
```
rlp([blob_tx_payload, blob_kzgs, blobs, kzg_aggregated_proof])
```
