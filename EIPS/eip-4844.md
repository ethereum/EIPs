...
The hashes for signature generation/verification and for determining the public transaction identifier are computed according to the [EIP-6493](./eip-6493.md) transaction signature scheme:

```python
def compute_sig_hash(message: BlobTransaction) -> Root:
...
def compute_tx_hash(signed_tx: SignedBlobTransaction) -> Root:
...
def get_origin(tx: SignedBlobTransaction) -> Address:
    ...
    return ecrecover(compute_sig_hash(tx), int(sig.y_parity)+27, sig.r, sig.s)
```
