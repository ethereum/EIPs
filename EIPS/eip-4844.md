...
This makes it possible to implement a policy that these blobs may be deleted after a certain period. The specific value chosen is `MIN_EPOCHS_FOR_BLOB_SIDECARS_REQUESTS` epocs, which is around 18 days, 
...
1. `MIN_EPOCHS_FOR_BLOB_SIDECARS_REQUESTS` - a consensus layer constant responsible for blob retention window: https://github.com/ethereum/consensus-specs/blob/master/configs/mainnet.yaml#L148.
