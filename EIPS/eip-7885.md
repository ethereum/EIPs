...
The gas cost model for EIP-7885 precompiles is designed to target approximately 50-55 mgas/s performance, consistent with existing precompiled contracts like ECRECOVER.

#### NTT Precompiles (0x12, 0x13) - Scheme-Specific Cost Model

**Gas Costs by Cryptographic Scheme**:

| Scheme | Ring Degree | NTT_FW Cost | NTT_INV Cost |
|--------|-------------|-------------|--------------|
| Falcon-512 | 512 | 500 gas | 500 gas |
| Falcon-1024 | 1024 | 1,080 gas | 1,080 gas |
| ML-DSA (Dilithium) | 256 | 256 gas | 340 gas |

**Rationale**: The gas costs are calibrated per cryptographic scheme based on actual execution performance using the liboqs library implementation. NTT computation complexity varies by ring degree and modulus size, with costs optimized to maintain consistent throughput (~53 mgas/s) across different post-quantum schemes.

**Benchmark Validation**: Reference implementation using liboqs demonstrates consistent 52-54 mgas/s performance:
- Falcon-512 NTT_FW: 53.12 mgas/s (~9.4μs), NTT_INV: 53.33 mgas/s (~9.4μs)
- Falcon-1024 NTT_FW: 52.89 mgas/s (~20.4μs), NTT_INV: 53.15 mgas/s (~20.3μs)
- ML-DSA NTT_FW: 53.49 mgas/s (~4.8μs), NTT_INV: 53.15 mgas/s (~6.4μs)
...
A native Go implementation has been developed for op-geth using the [liboqs](https://github.com/open-quantum-safe/liboqs) library via CGO bindings for high-performance post-quantum cryptographic operations. The implementation includes four precompiled contracts deployed at addresses `0x12`, `0x13`, `0x14`, and `0x15`:

**NTT_FW Precompile (0x12)**:
- Scheme-specific gas costs: Falcon-512 (500 gas), Falcon-1024 (1,080 gas), ML-DSA (256 gas)
- Forward NTT transformation using liboqs
- Input format: ring_degree (4 bytes) + modulus (8 bytes) + coefficients (Falcon: uint16×N, ML-DSA: int32×N)

**NTT_INV Precompile (0x13)**:
- Scheme-specific gas costs: Falcon-512 (500 gas), Falcon-1024 (1,080 gas), ML-DSA (340 gas)
- Inverse NTT transformation using liboqs
- Input format: Same as NTT_FW (coefficients in NTT domain)

**VECMULMOD Precompile (0x14)**:
- Dynamic gas cost: `ceil(0.32 × N)`
- Element-wise modular multiplication using Barrett reduction
- Linear scaling with ring degree

**VECADDMOD Precompile (0x15)**:
- Dynamic gas cost: `ceil(0.3 × N)`
- Element-wise modular addition
- Slightly cheaper than multiplication (0.3 vs 0.32 gas per element)
...
The benchmarks demonstrate that all NTT precompiles maintain competitive or better throughput compared to existing precompiled contracts like ECRECOVER, processing approximately 52-56 million gas per second. The liboqs-based implementation provides efficient support for multiple post-quantum cryptographic schemes including Falcon-512/1024 and ML-DSA (Dilithium).
...
- [NTT Precompile Benchmark Results](../assets/eip-7885/op-geth/benchmark_results/BenchmarkPrecompiledNTT)
- [ECRECOVER Benchmark Results](../assets/eip-7885/op-geth/benchmark_results/BenchmarkPrecompiledEcrecover)
