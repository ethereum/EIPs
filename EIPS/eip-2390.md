...
resolvers storing DNS resource records (EIP1185), although the technique could be
used on other interfaces like IPFS content hash storage (EIP1162).
...
ENS can respond to queries with IP addresses (as described in EIP1185)
...
This IP is looked up in a database like GeoIP2 [2] from MaxMind which maps the IP address to a location.
...
Cloud based DNS services like Amazon's Route 53 [3] only allow specifying
geographic regions at the granularity of a State in the United States.
...
### What is a geohash?
Geohashes are typically encoded in base 32 characters, as described
https://en.m.wikipedia.org/wiki/Geohash#Algorithm_and_example.
A geohash is an interleaving of latitude and longitude bits, whose
...
This permits querying by exact geohash of 8 characters to return the contect at that location,
or querying by geographic bounding box described by a geohash of less than 8 character precision.

<!-- hilbert geohash -->

There are more advanced ways to search geographic data using geohashes.
Searching a circular shaped regions is slightly more complex as it requires multiple queries.
Algorithms to do so, like https://github.com/ashwin711/proximityhash, are left to the reader to explore.

## Rationale
The proposed implementation uses a sparse Quadtree [1] trie as an index for
...
[1] https://dl.acm.org/doi/10.1007/BF00288933
[2] https://www.maxmind.com/en/geoip2-services-and-databases
