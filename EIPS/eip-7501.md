...
This specification introduces a new interface that extends [ERC-721](./eip-721.md) and [ERC-1155](./eip-1155.md) to enable onchain and offchain redeemables for NFTs.
...
The token MUST have the following interface and MUST return `true` for [ERC-165](./eip-165.md) supportsInterface for `0x12345678`, the 4 byte interfaceId of the below.
...
interface IERC7501 {
  ...
  struct CampaignParams {
      ...
      address signer; // null address means no ERC-712 signature required
...
If the token would like to enable trait redemptions, the token MUST include the [ERC-7500](./eip-7500.md) Dynamic Traits interface.

### Signer

A signer MAY be specified to provide a signature to process the redemption. If the signer is NOT the null address, the signature MUST recover to the signer address via [ERC-712](./eip-712.md) or [ERC-1271](./eip-1271.md).

The ERC-712 struct for signing MUST be as follows: `SignedRedeem(address owner,address redeemedToken, uint256[] tokenIds,bytes32 redemptionHash, uint256 salt)"`
...
| \*-(+\*) | signature         | if signer != address(0). can be for ERC-712 or ERC-1271          |
...
Coming soon <!-- Test cases can be found in [https://github.com/ProjectOpenSea/redeemables/tree/main/test](https://github.com/ProjectOpenSea/redeemables/tree/main/test) -->

## Reference Implementation

Coming soon <!-- The reference implementation for ERC721Redeemable and ERC1155Redeemable can be found in [https://github.com/ProjectOpenSea/redeemables/tree/main/src](https://github.com/ProjectOpenSea/redeemables/tree/main/src) -->

## Security Considerations

Tokens must properly implement ERC-7500 Dynamic Traits to allow for trait redemptions.
