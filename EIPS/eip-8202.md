...
For non-secp256k1 schemes, `keccak256(scheme_id || pubkey)[12:]` provides address domain separation from secp256k1 and from each other. A secp256k1 key, a P256 key, and a Falcon public key cannot collide by merely sharing the same byte suffix.
...
Post-quantum signature schemes like Falcon-512 are essential for long-term quantum safety, but they face practical deployment barriers: large witness sizes, high gas costs, lack of hardware wallet support, and ongoing standardization. The ephemeral secp256k1 scheme provides an immediately deployable quantum-safe alternative that reuses existing ECDSA infrastructure.
...
Users can start with `SCHEME_EPHEMERAL_K1` for immediate quantum safety and later migrate to `SCHEME_FALCON` or a future post-quantum scheme as those mature — potentially using `EXT_SET_CODE` to delegate their ephemeral account to a contract that bridges the transition.
...
* A user who migrates from secp256k1 to P256, Falcon, or ephemeral secp256k1 will generally have a new address, because non-secp256k1 derivation is domain-separated by `scheme_id` (or, for ephemeral k1, by the Merkle root).
...
Users who want post-quantum protection via `SCHEME_FALCON` MUST migrate before a cryptographically relevant quantum computer exists. Any secp256k1 address whose public key has already been revealed on-chain remains vulnerable once ECDSA can be broken. This EIP enables proactive migration to a post-quantum sender scheme.
...
While a `SCHEME_EPHEMERAL_K1` transaction is pending in the mempool, the current signing public key `pk_i` is visible in the witness. A quantum attacker who can run Shor's algorithm within a single block time could recover `sk_i` and submit a competing transaction at the same nonce. This requires breaking ECDSA within seconds, which is far beyond near-term quantum capabilities. Mitigations include private mempools and L2s with shorter inclusion latencies. For users who require unconditional protection against this vector, `SCHEME_FALCON` provides security without any timing assumption.
...
Falcon witnesses are much larger than ECDSA witnesses. Ephemeral secp256k1 witnesses are intermediate in size (737 bytes vs 65 bytes for plain secp256k1 and up to 1563 bytes for Falcon). The sender surcharges for these schemes exist to avoid systematic under-pricing of bandwidth and storage costs. Extensions with large payloads must likewise carry their own appropriate pricing.
...
A future attacker who can forge secp256k1 signatures could target addresses that continue to authorize transactions with `SCHEME_SECP256K1`. This is not introduced by this EIP. It is the pre-existing quantum threat. Users concerned about it SHOULD migrate to `SCHEME_FALCON` or `SCHEME_EPHEMERAL_K1`.
