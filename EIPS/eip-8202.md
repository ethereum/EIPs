...
category: Core
created: 2026-03-22
requires: 1559, 2718, 2780, 4844, 7702, 7951, 7976
---
...
Scheme agility is expressed as an authorization capability, not a separate transaction family. This EIP initially defines one required top-level authorization role, SENDER, and four sender signature schemes:
...
The sender address is derived deterministically from the public key recovered or verified from the SENDER authorization. For secp256k1, address derivation is exactly the same as legacy Ethereum. For non-secp256k1 schemes, the scheme_id is prefixed before hashing to provide address domain separation. For ephemeral secp256k1, the address is derived from a Merkle root over the full key sequence, providing a stable identity across key rotations.
...
The ephemeral secp256k1 scheme offers a complementary quantum migration path: rather than adopting a new post-quantum signature algorithm, it makes ECDSA quantum-safe by ensuring each key pair is used exactly once, eliminating long-term public key exposure. This requires no new cryptographic primitives beyond Keccak-256 preimage resistance and can serve as a transitional mechanism while post-quantum schemes mature.
...
| `SCHEME_EPHEMERAL_K1`    | `0x03`  | Ephemeral secp256k1 with Merkle-committed key rotation  |
...
This EIP initially defines four `scheme_id` values.
...
#### Ephemeral secp256k1 (`0x03`)

This scheme provides quantum resistance using only secp256k1 ECDSA by committing to a large sequence of one-time keypairs via a Merkle tree. Each keypair is used for exactly one transaction. The sender address is derived from the Merkle root, providing a stable identity across key rotations.

The account holder generates 2^`EPHEMERAL_TREE_DEPTH` secp256k1 keypairs offline. A Merkle tree is built over the leaf hashes `keccak256(pubkey_i)` for each keypair. The Merkle root commits to the entire key sequence.

Witness payload:

```text
rlp([leaf_index, y_parity, r, s, merkle_proof])
```
...
| `r`            | 32 bytes                             |
| `s`            | 32 bytes                             |
| `merkle_proof` | `EPHEMERAL_TREE_DEPTH` * 32 bytes    |

`leaf_index` MUST be less than 2^`EPHEMERAL_TREE_DEPTH`. `merkle_proof` MUST contain exactly `EPHEMERAL_TREE_DEPTH` 32-byte sibling hashes. `s` MUST satisfy `s <= secp256k1n / 2` per [EIP-2](./eip-2.md).

Verification and address derivation:

```python
# Recover the one-time public key from the signature
pubkey = ecrecover(sender_signing_hash(tx), y_parity, r, s)

# Verify the Merkle proof
leaf = keccak256(pubkey)
node = leaf
for i in range(EPHEMERAL_TREE_DEPTH):
    sibling = merkle_proof[i]
    if leaf_index & (1 << i):
...
# Address is derived from the Merkle root
sender = keccak256(SCHEME_EPHEMERAL_K1 || merkle_root)[12:]
```

The quantum resistance property: even if a quantum attacker can derive private keys from revealed public keys, they can only forge signatures for leaves whose public keys have already appeared on-chain. Unused leaves have their public keys hidden behind Keccak-256 preimages in the Merkle tree. As long as the account holder uses each leaf exactly once and has remaining unused leaves, the account remains secure.

**Key management**: the account holder MUST track which leaves have been used and MUST NOT reuse a leaf. Clients SHOULD enforce sequential leaf usage (incrementing `leaf_index`) to simplify tracking. An account with no remaining unused leaves cannot send further transactions under this scheme and MUST migrate to a new address.

Scheme gas surcharge: witness size times 16. For `EPHEMERAL_TREE_DEPTH = 20`, the Merkle proof is 640 bytes, total witness is approximately 705 bytes. Surcharge: **11,000 gas**.
...
Where `ecdsa_witness` is the 65-byte secp256k1 `(y_parity, r, s)` witness, `calldata_cost` follows the [EIP-7976](./eip-7976.md) uniform floor (64 gas/byte), and `scheme_precompile_cost` is the gas cost of the verification precompile for that scheme. For P256 (128-byte witness, P256 verification precompile at 3450 gas), the overhead is modest. For Falcon-512 (up to 1563-byte witness, no existing precompile), the overhead is substantial, reflected in the 25,000 gas surcharge. For ephemeral secp256k1 (~705-byte witness including Merkle proof), the overhead is moderate, reflected in the 11,000 gas surcharge.
...
* Ephemeral PQ transaction = base + sender auth(Ephemeral k1)
...
Falcon and ephemeral secp256k1 witnesses are larger than standard ECDSA witnesses. The sender surcharges for these schemes exist to avoid systematic under-pricing of bandwidth and storage costs. Extensions with large payloads must likewise carry their own appropriate pricing.

### Ephemeral key exhaustion

An ephemeral secp256k1 account has a finite number of one-time keys (2^`EPHEMERAL_TREE_DEPTH`). Once all leaves are used, the account cannot authorize further transactions and MUST migrate assets to a new address. Wallets MUST warn users as they approach key exhaustion. Leaf reuse would reveal the public key for a previously used index, degrading quantum resistance to that of standard secp256k1.

### Ephemeral key storage

The full ephemeral key sequence (2^20 private keys) must be stored securely offline. Loss of this key material means loss of the account. The Merkle tree can be recomputed from the key sequence, but the private keys themselves are the critical secret.
