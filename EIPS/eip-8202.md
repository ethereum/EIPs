...
| `SCHEME_DILITHIUM`       | `0x03`  | ML-DSA-65 (Dilithium)                                   |
...
| `DILITHIUM_SIG_LEN`      | `3309`  | ML-DSA-65 signature length in bytes                     |
| `DILITHIUM_PK_LEN`       | `1952`  | ML-DSA-65 public key length in bytes                    |
...
`len(pubkey)` MUST equal `1952`. `len(signature)` MUST equal `3309`.

ML-DSA-65 (NIST FIPS 204) is a module-lattice-based signature scheme at NIST security level 3. Like Falcon, it does not support public key recovery, so the witness includes the full public key.

Verification and address derivation:

```python
valid = ml_dsa_65_verify(sender_signing_hash(tx), pubkey, signature)
assert valid

sender = keccak256(SCHEME_DILITHIUM || pubkey)[12:]
```

Scheme gas surcharge: **84,000 gas** (`(DILITHIUM_SIG_LEN + DILITHIUM_PK_LEN) * 16`).
...
* Dilithium transaction = base + sender auth(Dilithium)
...
Falcon and ML-DSA-65 witnesses are much larger than ECDSA witnesses. The sender surcharges for these schemes exist to avoid systematic under-pricing of bandwidth and storage costs. Extensions with large payloads must likewise carry their own appropriate pricing.
