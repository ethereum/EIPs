...
Where `ecdsa_witness` is the 65-byte secp256k1 `(y_parity, r, s)` witness, `calldata_cost` follows the [EIP-7976](./eip-7976.md) uniform floor (64 gas/byte), and `scheme_precompile_cost` is the gas cost of the verification precompile for that scheme. For P256 (128-byte witness, EIP-7212 precompile at 3450 gas), the overhead is modest. For Falcon-512 (up to 1563-byte witness, no existing precompile), the overhead is substantial, reflected in the 25,000 gas surcharge.
