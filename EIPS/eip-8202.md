...
Where `ecdsa_witness` is the 65-byte secp256k1 `(y_parity, r, s)` witness and `calldata_cost` follows the [EIP-7976](./eip-7976.md) uniform floor (64 gas/byte). For P256 (128-byte witness, P256 verification precompile at 3450 gas), the overhead is modest. For Falcon-512 (up to 1563-byte witness, no existing precompile), the overhead is substantial, reflected in the 25,000 gas surcharge. For ephemeral secp256k1 (737-byte witness including Merkle root and proof), the calldata overhead is `(737 - 65) * 64 = 43,008` gas plus the 5,000 gas surcharge; verification is a standard ecrecover plus `EPHEMERAL_TREE_DEPTH` Keccak-256 hashes for the Merkle proof.
...
* ephemeral k1 + blobs = base + sender auth(ephemeral k1) + blob extension
