...
## Abstract
The "Permit" approval flow outlined in [EIP-2612](./eip-2612.md) has proven a very valuable advancement in UX by creating gasless approvals for ERC20 tokens. This EIP extends the pattern to ERC721 NFTs. This EIP borrows heavily from EIP-2612.
...
interface IERCXXXX is IERC165 {
  ...
  ///
  /// _INTERFACE_ID_ERCXXXX = 0x5604e225
  ...
  /// @returns the bytes32 domain separator
...
Whereas EIP-2612 splits signatures into their `v,r,s` components, this EIP opts to instead take a `bytes` array of variable length in order to support [EIP_2098](./eip-2098) signatures (64 bytes), which cannot be easily separated or reconstructed from `r,s,v` components (65 bytes).
