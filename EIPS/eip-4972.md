...
interface IERC1155NOA is IERC1155, INameOwnedAccount  {
    /**
     * @notice Transfers `_value` amount of an `_id` from the `_from` NOA to the `_to` address specified (with safety call).
     * @dev  MUST revert if `msg.sender` is not the owner of _from
     * MUST revert if `_to` is the zero address.
     * MUST revert if the balance of the holder for token `_id` is lower than the `_value` sent.
     * MUST revert on any other error.
     * MUST emit the `TransferSingle` event to reflect the balance change (see "Safe Transfer Rules" section of the IERC1155 standard).
     * After the above conditions are met, this function MUST check if `_to` is a smart contract (e.g. code size > 0). If so, it MUST call `onERC1155Received` on `_to` and act appropriately (see "Safe Transfer Rules" section of the IERC1155 standard).        
     * @param _from    Source NOA
     * @param _to      Target address
     * @param _id      ID of the token type
     * @param _value   Transfer amount
     * @param _data    Additional data with no specified format, MUST be sent unaltered in call to `onERC1155Received` on `_to`
     */
    function safeTransferFromName(bytes memory _from, address _to, uint256 _id, uint256 _value, bytes calldata _data) external;
    
    /**
     * @notice Transfers `_values` amount(s) of `_ids` from the `_from` NOA to the `_to` address specified (with safety call).
     * @dev MUST revert if `msg.sender` is not the owner of _from
     * MUST revert if `_to` is the zero address.
     * MUST revert if the balance of the holder for token `_id` is lower than the `_value` sent.
     * MUST revert on any other error.
     ...
     * @param _data    Additional data with no specified format, MUST be sent unaltered in call to `onERC1155Received` on `_to`
     */
    function safeBatchTransferFromName(bytes memory _from, address _to, uint256[] calldata _ids, uint256[] calldata _values, bytes calldata _data) external;

    /**
     * @notice Enable or disable approval for a third party ("operator") to manage all of the "_owner" NOA tokens.
     * @dev MUST emit the ApprovalForAll event on success.
     * MUST revert if `msg.sender` is not the owner of _owner
     * @param _owner  NOA of token owner
     * @param _operator  Address to add to the set of authorized operators
     * @param _approved  True if the operator is approved, false to revoke approval
     */
    function setApprovalForAllFromName(bytes memory _owner, address _operator, bool _approved) external;
    
    /// @notice This function returns true if interfaceId is the id of IERC1155NOA
    /// @dev see {IERC165-supportsInterface}
    function supportsInterface(bytes4 interfaceId) external view returns(bool);
}
```
