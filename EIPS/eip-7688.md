...
This EIP defines the changes needed to adopt [`StableContainer`](./eip-7495.md) in consensus data structures.
...
class StableExecutionPayload(StableContainer[MAX_EXECUTION_PAYLOAD_FIELDS]):
    ...
    withdrawals: Optional[List[Withdrawal, MAX_WITHDRAWALS_PER_PAYLOAD]]
...
class StableBeaconBlockBody(StableContainer[MAX_BEACON_BLOCK_BODY_FIELDS]):
    ...
    blob_kzg_commitments: Optional[List[KZGCommitment, MAX_BLOB_COMMITMENTS_PER_BLOCK]]
    ...  # Additional features from the fork that introduces this EIP
...
class StableBeaconState(StableContainer[MAX_BEACON_STATE_FIELDS]):
    ...
    # Execution
    latest_execution_payload_header: Optional[ExecutionPayloadHeader]
    # Withdrawals
