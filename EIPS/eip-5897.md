...
Both the publisher and subscriber contracts **MUST** register in a specific register contract, similarly to how smart contracts register an interface in the [EIP-1820](https://eips.ethereum.org/EIPS/eip-1820) contract.
...
This approach is similar in some ways to the [EIP-777](https://eips.ethereum.org/EIPS/eip-777) standard, which defines 'Hooks' that are fired when a contract receives [EIP-20](https://eips.ethereum.org/EIPS/eip-20) tokens, however this approach is a more generalized and scalable approach that allows hooks to be fired on any event, for any reason.
