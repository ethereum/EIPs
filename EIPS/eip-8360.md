...
category: Core
created: 2026-08-03
requires: 161, 684, 1014, 1153, 3529, 7708, 7928, 8037, 8038
---
...
`SELFDESTRUCT` does not mark a `TCREATE` account for deletion according to its own semantics. Instead, the removal of a `TCREATE` account's code, nonce, and storage at the end of the transaction is performed exclusively by the transaction-finalization mechanism defined by this EIP.

The execution of `SELFDESTRUCT` does not alter the gas accounting associated with a temporary account created by `TCREATE`. `SELFDESTRUCT` continues to follow its existing gas-accounting rules independently of the temporary-account finalization mechanism defined by this EIP.

### Interaction with [EIP-7708](./eip-7708.md)

This EIP inherits the Ether-transfer logging requirements defined by [EIP-7708](./eip-7708.md).

In particular, when `TCREATE` is executed with a non-zero `value`, the corresponding Ether transfer MUST emit the log specified by [EIP-7708](./eip-7708.md).
