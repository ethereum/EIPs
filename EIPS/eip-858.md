...
To ensure a constant Ether issuance, adjust the block reward to `new_block_reward`, where
    ...
    new_block_reward = 1_000_000_000_000_000_000 if block.number >= CNSTNTNPL_FORK_BLKNUM else block.reward
...
## Rationale
This will delay the difficulty bomb by 42 million seconds (approximately 1.4 years). The difficulty bomb remains a community supported mechanism to aid a future transition to POS.
