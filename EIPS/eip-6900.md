...
type HookAssignment is bytes32;
...
Variables of the type `FunctionReference` MUST be interpreted as an address in the first 20 bytes and a function selector or function ID (as used in `IPlugin`) in the remaining 4 bytes.

Variables of the type `HookAssignment` MAY be used in the following manner to store and retrieve different types of hooks:

- Packed function selector and a validator `FunctionReference`, with the last byte set to 0.
  - For pre user operation validation hooks, pre runtime validation hooks, and execution hooks on standard execution functions.
- Packed function selector and a plugin address, with the last byte set to 1.
  - For execution hooks that apply only to the `executeFromPlugin` and `executeFromPluginExternal` flows.
- Packed function selector, with the last byte set to 2.
  - For execution hooks that apply to all calls to the account that match the selector except those of standard execution functions.

#### `IPluginManager.sol`

Plugin manager interface. Modular Smart Contract Accounts SHOULD implement this interface to support installing and uninstalling plugins.

```solidity
interface IPluginManager {
    struct PluginInstallConfig {
        address plugin;
    ...
    function installPlugin(PluginInstallConfig calldata config) external;

    function uninstallPlugin(PluginUninstallConfig calldata config) external;
}
```
...
interface IPluginLoupe {
    ...
    function getExecutionHooks(HookAssignment hookAssignment) external view returns (ExecutionHook[] memory);

    function getPreUserOpValidatorHooks(HookAssignment hookAssignment) external view returns (FunctionReference[] memory);

    function getPreRuntimeValidatorHooks(HookAssignment hookAssignment) external view returns (FunctionReference[] memory);
...
interface IStandardExecutor {
    function execute(address target, uint256 value, bytes calldata data, FunctionReference validator) external payable;
...
interface IPlugin {
     ...
     */
    function preUserOpValidationHook(uint32 functionId, UserOperation memory userOp, bytes32 userOpHash) external view returns (uint256);
     ...
     * @return Packed validation data for validAfter (6 bytes), validUntil (6 bytes), and authorizer (20 bytes).
     */
    function userOpValidator(uint32 functionId, UserOperation memory userOp, bytes32 userOpHash) external returns (uint256);
     ...
     */
    function preRuntimeValidationHook(uint32 functionId, address sender, uint256 value, bytes calldata data) external;
     ...
     * @param data The calldata sent.
     */
    function runtimeValidator(uint32 functionId, address sender, uint256 value, bytes calldata data) external;
     ...
     */
    function preExecutionHook(uint32 functionId, address sender, uint256 value, bytes calldata data) external returns (bytes memory);
     ...
     */
    function postExecutionHook(uint32 functionId, bytes calldata preExecHookData) external;
...
// This struct is akin to a "dynamic" `FunctionReference`, which is resolved at install time.
...
struct PluginManifest {
    ...
    string version;
    // The author field SHOULD be the GitHub handle of the user or organization that created this plugin.
    string author;
...
The function `installPlugin` accepts a parameter `PluginInstallConfig calldata config`, which contains the address of the plugin to install, the Keccak-256 hash of the plugin's manifest, RLP-encoded data to pass to the plugin's `onInstall` callback, and an array of addresses that represent the plugin's install dependencies.
...
- Revert if `config.manifestHash` does not match the computed Keccak-256 hash of the plugin's manifest. This prevents installation of plugins that attempt to install a different plugin configuration than the one that was approved by the client.
...
- Each execution selector MUST be added as a valid execution function on the modular account. If the execution selector has already been added or matches the selector of a native function, the function MUST revert.
...
Plugins MAY configure associated functions on the `installPlugin` function. If present, the validation function and hooks MUST be called in the appropriate sequence.

#### Calls to `uninstallPlugin`

The function `uninstallPlugin` accepts a parameter `PluginUninstallConfig calldata config`, which contains the address of the plugin to uninstall and RLP-encoded data to pass to the plugin's `onUninstall` callback.

The function MUST perform the following preliminary checks:

- Revert if the plugin is not installed on the modular account.
...
The function SHOULD remove any metadata stored for the plugin. Each dependency's record SHOULD also be updated to reflect that it has no longer has this plugin as a dependent.
...
Plugins MAY configure associated functions on the `uninstallPlugin` function. If present, the validation function and hooks MUST be called in the appropriate sequence. Notably, the post execution hooks defined prior to the uninstall must run after the uninstall.
...
Alternatively, when the modular account natively implements functions in `IPluginManager` and `IStandardExecutor`, the same following steps MUST be performed for those functions. Other native functions MAY perform these steps.
...
The `executeFromPlugin` function MUST allow plugins to call functions (either native or execution functions installed by plugins) on the MSCA. Hooks matching the function selector provided in `data` and hooks associated with the function selector and the calling plugin address MUST be called. If the calling plugin's manifest did not include the provided function selector within `permittedExecutionSelectors` at the time of installation, execution MUST revert.
...
This proposal includes several interfaces that build on ERC-4337 and are inspired by ERC-2535. First, we standardize a set of modular functions that allow smart contract developers greater flexibility in bundling validation, execution, and hook logic. We also propose the `IPluginLoupe` interface that takes inspiration from the diamond standard and provides methods for querying execution functions, validation functions, and hooks.
