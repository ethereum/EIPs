...
The following diagram shows how a plugin execution flows. This happens when the execution step (Step4) from above triggers the "(Plugin) Execution Function". Two native functions (`executeFromPlugin` and `executeFromPluginExternal`) can be called by plugins to execute on the account's context. One handles calls to other installed plugin's execution function. One handles calls to external contracts.
...
Standard execution functions SHOULD check whether the call's target implements the `IPlugin` interface via ERC-165.
