...
interface IMultiResource {
    /**
     * @notice Used to notify listeners that a resource object is initialised at `resourceId`.
     * @param resourceId ID of the resource that was initialised
     */
     ...
     * @param priorities An array of priorities of active resources. The succession of items in the priorities array
...
The `getResourceMetadata` function returns the resource's metadata URI. The metadata, to which the metadata URI of the resource points, MAY contain a JSON response with the following fields:
...
While this is the suggested JSON schema for the resource metadata, it is not enforced and MAY be structured completely differently based on implementer's preference.
...
1. **Why are [EIP-712](./eip-712.md) permit-style signatures to manage approvals not used?**

For consistency. This proposal extends EIP-721 which already uses 1 transaction for approving operations with tokens. It would be inconsistent to have this and also support signing messages for operations with resources.
