...
- `DUPN`:
    1. Charge 3 gas.
    2. If `code[pc + 1] != 0x60`, halt with exceptional failure.
    3. Let `x = code[pc + 2]`.
    4. Let `n = x + 17`.
    5. If `n > len(stack)`, halt with exceptional failure.
    6. Push `stack[top - n + 1]` on the stack.
    7. Set `pc = pc + 3`.
- `SWAPN`:
    1. Charge 3 gas.
    2. If `code[pc + 1] != 0x60`, halt with exceptional failure.
    ...
    6. Swap `stack[top - n]` and `stack[top]`.
    7. Set `pc = pc + 3`.
- `EXCHANGE`:
    1. Charge 3 gas.
    2. If `code[pc + 1] != 0x60`, halt with exceptional failure.
    2. Let `x = code[pc + 2]`.
    3. Let `n, m = decode_pair(x)`.
    4. If `m + 1 > len(stack)`, halt with exceptional failure.
    5. Swap `stack[top - n]` and `stack[top - m]`.
    6. Set `pc = pc + 3`.

`code[i]` evaluates to `0` if beyond the end of the code, matching `PUSH` behavior.
...
The auxiliary function `decode_pair` is defined by the following Python code:
...
Assemblers and compilers must emit a 1-byte immediate for each of these opcodes that decodes to the required `n, m` operands. For reference, this can be done by `encode_single` and `encode_pair`:
...
### Immediate preceded by `PUSH1`

A property of the EVM is that it is not possible to jump to the middle of an instruction. In order to guarantee this property, the immediate operands of `PUSH*` instructions are masked from jumpdest analysis given that they may include the byte `0x5b` (`JUMPDEST`), which would otherwise be considered a valid jump target.

The new instructions in this EIP are encoded such that their immediates are preceded by `0x60` (`PUSH1`), preserving this property of the EVM without requiring changes to jumpdest analysis, as the logic for masking `PUSH1` is reused.

Changing jumpdest analysis to introduce new masked opcodes without bytecode versioning (EOF) would create a backwards incompatibility. Consider the bytecode sequence `e6 5b` that decodes to `INVALID JUMPDEST` prior to this EIP, and thus contains a valid jump target. If an instruction `DUPN x` were introduced with encoding `e6 {x}` (opcode immediately followed by operand), and jumpdest analysis changed to mask the operand to `e6`, the same bytecode sequence `e6 5b` would then decode to `DUPN 0x5b`. As a result of the upgrade, `e6` would become an invalid jump target, breaking contracts that include this sequence. The opposite can happen with push opcodes, for example, `e6 60 5b` would transform from `INVALID PUSH1 0x5b` to `DUPN 0x60 JUMPDEST`, creating a new valid jump target. Furthermore, this may have a cascading effect on arbitrarily many subsequent instructions.

Since the EVM doesn't offer a dedicated section for data, arbitrary data may be embedded in code and serve as immutable data storage accessed via `CODECOPY`. Therefore, we must assume that arbitrary byte sequences may be found in contracts that have been deployed or are "counterfactual" deployments (yet to be published on chain).
...
In addition, the exact formulation was chosen to make `decode_pair` a simple function using only basic arithmetic, bitwise operations (division by 16), and few branches.
...
- `e66000` is `[DUPN 17]`
- `e7605b` is `[SWAPN 108]`
- `e660005b` is `[DUPN 17, JUMPDEST]`
- `e75b` is `[INVALID_SWAPN, JUMPDEST]`
- `e6615b00` is `[INVALID_DUPN, PUSH2 0x5b00]`
...
- `60016000808080808080808080808080808080e66000` results in 18 stack items, the top of the stack valued 1, the bottom of the stack valued 1, the rest valued 0
- `600160008080808080808080808080808080806002e76000` results in 18 stack items, the top of the stack valued 1, the bottom of the stack valued 2, the rest valued 0
- `600060016002e86001` results in 3 stack items, from top to bottom: [2, 0, 1]
- `e75b` reverts 
- `600456e65b` executes successfully (`PUSH1 04 JUMP INVALID_DUPN JUMPDEST`)
- `600060006000e8600115` results in 3 stack items, the top of the stack valued 1, the rest valued 0
- `6000808080808080808080808080808080e66000` results in exceptional halt
