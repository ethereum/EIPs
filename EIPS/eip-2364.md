...
From a macro perspective, keeping incompatible nodes partitioned from one another ensues that disjoint clusters retain more resources for maintaining their own chain, thus raising the quality of service for all networks globally.

## Specification

The specification is tiny due to the crux already being specced in EIP-2124.
...
- Advertise a new `eth` protocol capability (version) at `eth/64`.
  - The old `eth/63` protocol must still be kept alive side-by-side.
...
This EIP defines no consensus changes, so it does not require a hard fork.

## Test Cases

For calculating and validating fork IDs, you can find test cases in [EIP-2124](https://eips.ethereum.org/EIPS/eip-2124).
