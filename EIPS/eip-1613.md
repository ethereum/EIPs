...
author: Yoav Weiss <yoav@tabookey.com>, Dror Tirosh <dror@tabookey.com>, Alex Forshtat <forshtat1@gmail.com>
...
pay for transactions and get compensated by that contract or a 3rd party. The trusted contract can be verified by anyone, and the system is otherwise trustless.
...
* a `RelayHub` singleton contract
* participating contracts inheriting the `RelayRecipient` contract
* 3rd party contracts willing to provide the gas fees inheriting `Paymaster` contract (can be implemented by the `RelayRecipient` contract)
* a decentralized network of `Relay` nodes, a.k.a. Gas Stations
* and, of course, user applications (e.g. mobile or web) interacting with contracts via relays.
...
* Hold ETH stakes placed by relays. A minimum stake size is enforced. The stake can be withdrawn after a relay unregisters and waits for a cooldown period.
...
* Provide a cost-free way for relays to know whether they'll be compensated for a future transaction.

Roles of a `Relay` node:

* Maintain a hot wallet with a small amount of ETH, to pay for gas.
* Provide a public interface for client apps to send gasless transactions via channels such as https or whisper.
* Publish its public interfaces and its price (as a multiplier of the actual transaction gas cost) in `RelayHub`.
...
These are all the changes the contract needs to start accepting relayed calls. But in order for a `Relay` node to pay for a transaction, it must be compensated, so dapps are expected to also provide an implementation of the `Paymaster` contract. It can be either a separate contract or the same as `RelayRecipient`. The `Paymaster` contract must:

* Maintain a small balance of ETH gas prepayment deposit in `RelayHub`. Can be paid directly by the `Paymaster` contract, or by the dapp's owner on behalf of the `Paymaster` address.
  The `Paymaster` owner is responsible for ensuring sufficient balance for the next transactions and can stop depositing if something goes wrong, thus limiting the potential for abuse of system bugs. In DAO use-cases it will be up to the DAO logic to maintain a sufficient deposit.
* Implement a `getGasLimits()` view function that returns a `GSNTypes.PaymasterLimits` struct. This struct contains gas limits that the Paymaster requests for its methods (discussed). The `RelayHub` will always give this exact gas limits accordingly.
* Implement an `acceptRelayedCall(GSNTypes.RelayRequest calldata relayRequest, bytes calldata approvalData, uint256 maxPossibleGas)` view function that returns **zero** if and only if it is willing to accept a transaction and pay for it. 
  `acceptRelayedCall` is called by `RelayHub` as a view function when a `Relay` inquires it, and also during the actual transaction. Transactions are reverted if **non-zero**, and `Relay` only gets compensated for transactions (whether successful or reverted) if `acceptRelayedCall` returns **zero**. It can also optionally return `context` bytes array to communicate some data to the non-view `preRelayedCall()` and `postRelayedCall()` methods. Some examples of `acceptRelayedCall()` implementations:
    * Whitelist of trusted dapp members.
    * Make a call to some method in the `RelayRecipient` and allow the Recipient to decide whether the relayed call should be accepted or not.
...
* Implement `preRelayedCall(bytes context) returns (bytes32)`. This method is called before a transaction is relayed. By default, it does nothing.
  
* Implement `postRelayedCall(bytes context, bool success, bytes32 preRetVal, uint256 gasUseWithoutPost, uint256 txFee, uint256 gasPrice)`. This method is called after a transaction is relayed. By default, it does nothing.
  
  These two methods can be used to charge the user in a dapp-specific manner.

Glossary of terms used in the processes below:

* `RelayHub` - the RelayHub singleton contract, used by everyone.
* `Recipient` - a contract implementing `RelayRecipient`, accepting relayed transactions from the RelayHub contract
* `Paymaster` - a contract implementing `Paymaster`, paying for the relayed transactions
...
![Sequence Diagram](https://bit.ly/2PlTyEa)
...
* Relay calls `RelayHub.registerRelay(uint transactionFee, string memory url)` with the relay's `transaction fee` (as a base charge plus a multiplier on transaction gas cost), and a URL for incoming transactions.
...
* `Sender` selects a live `Relay` from RelayHub's list by looking at `RelayAdded` events from `RelayHub`, and sorting based on its own criteria. Selection may be based on a mix of:
    ...
    * Optionally, reputation/blacklist/whitelist held by the sender app itself, or its backend, on a per-app basis (not part of the gas stations network).
* Sender prepares the transaction with Sender's address, the recipient address, the actual transaction data, Relay's transaction fee, gas price, gas limit, its current nonce from `RelayHub.nonces`, RelayHub's address, Paymaster's address, and Relay's address, and then signs it using an [EIP-712](https://eips.ethereum.org/EIPS/eip-712)\-compatible format.
* Sender verifies that `RelayHub.balances[paymaster]` holds enough ETH to pay Relay's fee.
...
* `Relay` verifies that:
    * The transaction's paymaster contract will accept this transaction when submitted, by calling `RelayHub.canRelay()`, a view function,
      which checks the paymaster's `acceptRelayedCall`, also a view function, stating whether it's willing to accept the charges).
    * The transaction nonce matches `RelayHub.nonces[sender]`.
    * The relay address in the transaction matches Relay's address.
    * The transaction's `Paymaster` has enough ETH deposited in `RelayHub` to pay the transaction fee.
...
* `Sender` receives the wrapped transaction and verifies that:
    * It's a valid relay call to the `RelayHub` from Relay's address.
...
* If any of the sender's checks fails, it goes back to selecting a new Relay. The sender may also file a report on the unresponsive relay to its backend or save it locally, to down-sort this relay in future transactions.
...
* `Sender` monitors the blockchain, waiting for the transaction to be mined. 
  ...
  If mining fails due to such attack, the sender may call `RelayHub.penalizeRepeatedNonce` through another relay, to collect his reward and burn the remainder of the offending relay's stake, and then go back to selecting a new Relay for the transaction.
  See the discussion in the attacks/mitigations section below.
* `RelayHub` receives the transaction:
    ...
    * Calls Paymaster's `acceptRelayedCall` function, asking whether it's going to accept the transaction. If not, the `CanRelayFailed` will be emitted and `reason` will contain the return value of `acceptRelayedCall`. In this case, Relay doesn't get paid, as it was its responsibility to check `RelayHub.canRelay` before releasing the transaction.
    * Calls Paymaster's `preRelayedCall` function. If this call reverts the `TransactionRelayed` will be emitted with status `PreRelayedFailed`.
    * Sends the transaction to the recipient.  If this call reverts the `TransactionRelayed` will be emitted with status `RelayedCallFailed`.
      When passing gas to `call()`, enough gas is preserved by `RelayHub` for post-call handling. The recipient may run out of gas, but `RelayHub` never does.
...
* `RelayHub` checks call's return value of the call, and emits `TransactionRelayed(address relay, address from, address to, address paymaster, bytes4 selector, RelayCallStatus status, uint256 charge)`.
* `RelayHub` increases `RelayHub.nonces[sender]`.
* `RelayHub` transfers ETH balance from paymaster to `Relay.owner`, to pay the transaction fee, based on the measured transaction cost. 
...
* `Relay` keeps track of transactions it sent and waits for `TransactionRelayed` events to see the charge.
  If a transaction reverts and goes unpaid, which means the paymaster's `acceptRelayedCall()` function was inconsistent, `Relay` refuses service to that paymaster for a while (or blacklists it indefinitely, if it happens often). 
...
* `Relay` shuts down.
* Once the owner's unstake delay is over, the owner calls `RelayHub.unstake()`, and withdraws the stake.
...
The sender doesn't need to wait for the transaction to be mined and knows immediately whether its request has been served. 
If a relay doesn't return a signed transaction within a couple of seconds, the sender cancels the operation, drops the connection, and switches to another relay. 
...
##### Attack: Relay attempts to censor a transaction by signing it, returning it to the sender, but never put it on the blockchain.
...
The sender has a signed transaction from Relay with nonce N, and also gets a mined transaction from the blockchain with nonce N, also signed by Relay. 
This proves that Relay performed a DoS attack against the sender. 
The sender calls `RelayHub.penalizeRepeatedNonce(bytes transaction1, bytes transaction2)`, which verifies the attack, confiscates Relay's stake and sends half of it to the sender who delivered the `penalizeRepeatedNonce` call. The other half of the stake is burned by sending it to `address(0)`. Burning is done to prevent cheating relays from effectively penalizing themselves and getting away without any loss.
The sender then proceeds to select a new relay and send the original transaction.

The result of such an attack is a delay of a few blocks in sending the transaction (until the attack is detected) but the relay gets removed and loses its entire stake. 
Scaling such an attack would be prohibitively expensive, and actually quite profitable for senders and honest relays.
...
When the sender receives a transaction signed by a Relay he validates that the nonce used is valid, and if it is not, the client will ignore the given relay and use other relays to relay given transaction. Therefore, there will be no actual delay introduced by such an attack.
...
Each offending contract can only cause small damage (e.g. the cost of 2-3 transactions) to a relay, before getting blacklisted.
...
The attack can be made even more impractical by setting RelayHub to require a stake from dapps before they can be served, and enforcing an unstaking delay so that attackers will have to raise a vast amount of ETH in order to simultaneously create enough malicious contracts and attack relays. 
...
or to log in with a Google/Facebook account, which limits the rate of the attack to the attacker's ability to open many Google/Facebook accounts. 
Only a user that passed that process is given credit in RelayRecipient. The rate of such a Sybil attack would be too low to cause any real damage.
...
Each unreliable relay can only cause a couple of seconds of a delay to senders, once, and then it gets blacklisted by them, as described in the first attack above. 
...
2. The relay doesn't execute the transaction, but the user does. Similarly to 1, the transaction is executed, so no problem.
...
Dapps adding gas station network support remain backward compatible with their existing apps/users. The added methods apply on top of the existing ones, so no changes are required for existing apps.

## [EIP-712](https://eips.ethereum.org/EIPS/eip-712)\-compatible signatures

While the relayed transactions do not require users to hold any ETH, they still need to have a private key and are required to sign each relay request message. In order to allow existing web3 wallets to present the data being signed in a human-readable format, the GSN implements EIP-712 typed structured data signing.

The structured data is defined as follows:
...
A working implementation of the [**gas stations network**](https://github.com/tabookey-dev/tabookey-gasless) is being developed by **TabooKey**. It consists of `RelayHub`, `RelayRecipient`, `Paymaster`, `web3 hooks`, an implementation of a gas station and sample dapps using the Gas Stations Network.
