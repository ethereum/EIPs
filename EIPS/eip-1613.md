...
* Implement a `may_relay(address relay, address from, bytes transaction)` view function that returns `true` if and only if it is willing to accept a transaction and pay for it. 
