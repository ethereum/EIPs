...
Assuming that the role was granted and is not expired, the following table shows the result the `hasRole` function MUST
return:
...
this field is used in the `hasRole` function (refer back to 
