...
Merely _drawing_ this graph costs the square of the code size: every block's `jump` might reach every block, so the edges number the blocks times one less than the blocks — O(N²) — and any tool that must consider where jumps can go pays that price whether it draws the graph or not.  _Walking_ the paths costs far more.  A tool that must follow the paths forks its walk at every jump whose destination it cannot know, and the forks compound: the number of walks grows exponentially in the size of the code, and can grow worse.
