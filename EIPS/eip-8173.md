...
This document serves as background material for proposals for static control flow, including [EOP-615](./eip-615): Subroutines and Static Jumps for the EVM, [EIP-7979](./eip-7979): Call and Return Opcodes, [EIP-8013](./eip-8013): Static Relative Jumps, [EIP-4750](./eip-4750): EOF Functions, andZdiscussions around RISC-V migration and ZK verification infrastructure.
...
* BL (Branch with Link) — which loads the return address into LR and branches to its operand
...
An good example is the JVM.  Subroutines are implemented as class methods, and calls and returns are handled by

* INVOKE* — which pushes the return address on an internal return stack and branches to its operand
...
A _complete and sound CFG_ represents all and only the possible paths of program execution and is a fundamental starting point for many downstream tasks, including
