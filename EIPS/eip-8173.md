...
For a detailed explanation and examples of pathological programs, see the proposal to be  .
...
The following is a Python implementation of recursive algorithm for validating EVM bytecode. This algorithm traverses the code, emulating its control flow and stack use, and checking for violations of the rules above. It runs in time proportional to the size of the code, and the depth of the stack is at most proportional to the size of the code.  An equivalent validation MUST be run at `CREATE` time.
...
class Validator:
    ...
    def get_instr_info(self, pc: int) -> Optional[Tuple[int, int, int, int, bool]]:
        ...
        if op == EVMOpcode.PUSH0.value:
            size = 1
        elif EVMOpcode.PUSH1.value <= op <= EVMOpcode.PUSH32.value:
            size = 1 + (op - EVMOpcode.PUSH1.value)
        else:
            size = 1
    ...
    def validate(self) -> bool:
        ...
        """
        # Start the recursion from PC 0, height 0, and no last push value
    ...
    def _validate_recursive(self, pc: int, height: int, last_push: Optional[int]) -> bool:
        ...
        if op in (EVMOpcode.JUMP.value, EVMOpcode.JUMPI.value, EVMOpcode.JUMPSUB.value):
            ...
            # Recursively validate the jump destination
            if not self._validate_recursive(last_push, new_height, None):
                return False

            # For conditional jump (JUMPI), also check the next sequential instruction
            if op == EVMOpcode.JUMPI.value:
                if not self._validate_recursive(pc + size, new_height, None):
                    return False

        # For non-terminating instructions, prepare the next instruction
        elif not is_term:
            # Capture PUSH value for the next instruction
            val = (
            ...
            push_val = val if EVMOpcode.PUSH0.value <= op <= EVMOpcode.PUSH32.value else None

            if not self._validate_recursive(pc + size, new_height, push_val):
                return False

        return True  # If we've processed the instruction without issues
