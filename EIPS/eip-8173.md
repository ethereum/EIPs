...
For a modern register machine, e.g. the ARM, function calls are done with the BL (Branch with Link) instruction -- which loads the return address into LR -- and BLR (Branch with Link to Register) instruction -- which branches to the address in LR.  It is the program's job to save and restore LR values to the stack.  The operand to BL can be direct for static jumps, or indirect for dynamic jumps.  For example:
...
For the JVM all subroutines are implemented as class methods, and calls and returns are handled with the INVOKE* and RETURN* family ofinstructions, which handled return addresses on an internal stack.  For example:
...
A complete and sound CFG represents all and only the possible paths of program execution and is fundamental starting point for many downstream tasks:
...
Dynamic jumps and calls are not a problem for machines which run on physical hardware, but are rare in virtual machines whose code is the source for downstream tools like JIT compilers.  E.g. a JIT compiler that takes quadratic time is of limited practical value. For this reason, Java, Wasm, and .NET VMs do not support dynamic jumps.
