...
An example of a modern register machine is the ARM.  Function calls are done with the BL (Branch with Link) instruction — which loads the return address into LR and branches to it's operand, called function — and BLR (Branch with Link to Register) instruction, which branches back to the address in LR.  It is the program's job to save and restore LR values to the stack during nested calls.  The operand to BL can be direct for static jumps, or indirect for dynamic jumps.  For example:
...
For the JVM all subroutines are implemented as class methods, and calls and returns are handled with the INVOKE* and RETURN* family ofinstructions, which handle return addresses on an internal stack.  For example:
...
Arguments and return values are typically handled by some combination of registers and the stack.

### EVM Control Flow

The EVM is a simple stack machine that interprets a  bytecode instruction.
...
A complete and sound CFG represents all and only the possible paths of program execution and is a fundamental starting point for many downstream tasks:
...
#### Calls, Returns, and Dynamic Jumps
...
* Code size and efficiency: Synthesizing calls with jumps wastes space and gas compared to explicit call opcodes.
* Analysis complexity: Dynamic jumps create fundamental challenges for static analysis that are largely absent in VMs with structured call mechanisms.
...
In general, it is not possible to create a complete, sound CFG for all possible EVM programs -- there is an entire academic literature of complex, incomplete solutions like these that static control-flow renders trivial.

### Static Control Flow and Ethereum Scaling

As laid our above, **Static control flow** means that the destination of every jump or call is determinable at contract creation time (before execution). This has concrete implications for Ethereum's scaling roadmap, particularly around ZK verification, rollups, and future execution layer changes.
