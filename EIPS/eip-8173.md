...
The the control flow graphs for these programs makes the problem clear.  Each block of instructions in a graph is a sequence from the above programs with at most one entry (a `JUMPDEST`) and one exit (a `JUMP`), and each arc is a transfer of control.  Arcs on the left are backwards branches, arcs on the right are forwards branches.  See how the tangle of paths goes up fast as the programs get longer.
