...
The Ethereum Virtual Machine does _not_ provide explicit facilities for calls and returns. Instead, they must be synthesized using the dynamic `JUMP` instruction, which takes its argument from the stack and stores intermediate links on the stack. This creates the quadraditic CFG problems explained above.
