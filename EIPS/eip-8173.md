...
This Informational EIP provides foundational context for understanding control flow in the EVM. It covers the historical development of  mechanisms in computing, the technical foundations of  analysis, and the impact of static control flow on Ethereum's scaling roadmap.
...
A _ graph (CFG)_ is a directed graph representation of a program where:
...
Dynamic jumps make dynamic control flow possible, make quadraticaly complex _CFGs_ possible, and can make static analysis of control difficult to impossible.  Dynamic jumps are not a problem for machines which run on physical hardware.  But they are uncommon in virtual machines whose code is the source for downstream tools because, as we will illustrate below, traversing a dynamic control flow graph can take quadratic time.  For this reason Java and Wasm do not support dynamic jumps and CLI carefully restricts them.

#### EVM: Dynamic Control Flow

For an example, consider these three EVM programs. An indefinite series of longer programs like them can be easily generated, and the really long one make for nice exploits.  It's not important that at runtime `gas` isn't random or that the `jump` will most often fail, what matters is that because the jump destination is taken from the stack it is impossible to know _a priori_ where the jumps go, so every path must be explored.
...
The the  graphs for these programs makes the problem clear.  Each block of instructions in a graph is a sequence from the above programs with at most one entry (a `JUMPDEST`) and one exit (a `JUMP`), and each arc is a transfer of control.  Arcs on the left are backwards branches, arcs on the right are forwards branches.  See how the tangle of paths goes up fast as the programs get longer.

![_ Graphs_](../assets/eip-8173/.svg)
...
It's the historical practice and it's the case for every EIP we have seen – to avoid the problems of dynamic control flow VMs use calls and returns.
...
Static control flow has been a cornerstone of efficient computation since Babbage and Turing. The EVM's reliance on dynamic jumps is an anomaly among virtual machines and a significant barrier to analysis, compilation, and scaling. Proposals to introduce explicit call/return opcodes and enforce static jumps, so far including  bring the EVM in line with industry best practices and unlock a range of optimizations critical to Ethereum's scaling roadmap.
