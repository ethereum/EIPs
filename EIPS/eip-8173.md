...
A _complete and sound CFG_ represents all and only the possible paths of program execution and is a fundamental starting point for many downstream tasks, incuding
...
It should not be a surprise that dynamic control flow makes static analysis difficult.

Consider these three programs.  They make for nice exploits, especially as they get progressively longer.  It's not important that at runtime `gas` isn't random or that the `jump` will most often fail, what matters is that it is impossible to know _a priori_ where the jumps go, so every path must be explored.
...
Dynamic jumps and calls are not a problem for machines which run on physical hardware, but are uncommon in virtual machines whose bytecode is often the source for downstream tools like JIT compilers.  A JIT compiler that takes quadratic time is of limited practical value. For this reason Java and Wasm do not support dynamic jumps and CLR carefully restricts them.
...
This Informational proposal specificies no changes to the protocol.  Therefore it has no direct security implications.  It does not affect the security considerations of the EIPs it references, it helps to explain them.
