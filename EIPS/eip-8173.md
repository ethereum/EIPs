...
This document serves as background material for any proposals current and future proposals for static control flow, including [EIP-7979](./eip-7979): Call and Return Opcodes, [EIP-8013](./eip-8013): Static Relative Jumps and [EIP-4750](./eip-4750): EOF Functions, and discussions around RISC-V migration and ZK verification infrastructure.
...
In 1833 Charles Babbage began the design of the Analytic Engine: a steam-powered, mechanical, Turing-complete computer. Programs were to be encoded on punched cards which controlled a system of rods, gears and other machinery to implement storage, arithmetic, jumps, and conditional jumps. Jumps were supported by cards that shuffled the card deck forwards or backwards a fixed number of the cards.

#### Lovelace 1842: Algorithms in Assembly Language

The first published description of the was by L. F. Menabre, 1842[^1]. Subroutines required manual arrangement of card decks.  The translator, Ada Augusta, Countess of Lovelace, made extensive notes. The notes include her famous program for recursively computing Bernoulli numbers — arguably the world's first complete computer program — which used conditional jumps to implement the required nested loops.
...
An good example is the JVM.  Subroutines are implemented as class methods, and calls and returns are handled with the INVOKE* and RETURN* family of instructions, which handle return addresses on an internal stack.  For example:
...
* and performing formal security analysis

Dynamic jumps exact a high price.
* _With static jumps and explicit calls_, the number of nodes in the CFG is linear in the number of instructions, and creating a _complete, sound CFG_ for any program takes linear time.
* **With _dynamic jumps_ it is not possible to create a _complete, sound CFG_ for all possible EVM programs.**

####  Dynamic Jumps and Computational Complexity

With dynamic jumps, it should not be a surprise that static analysis becomes difficult.

Consider these three programs.  They make for nice exploits, especially as they get progressively longer.  It's not important that at runtime `gas` isn't random or that the `jump` will most often fail, what matters is that it is impossible to know a priori where the jumps go, so every path must be explored.
...
The Ethereum Virtual Machine does _not_ provide explicit facilities for calls and returns. Not even a link register. Instead, they must be synthesized using the dynamic `JUMP` instruction, which takes its argument from the stack and stores intermediate links on the stack. This creates the problems explained above.
...
For Ethereum, this quadratic behavior is a _denial-of-service vulnerability_ for any online static analysis, including validating bytecode and AOT compilation at contract creation time and JIT compilation at runtime.  Even offline, dynamic jumps (and the lack of calls and returns) can cause static analyses of many contracts to become impractical, intractable or even impossible. The following are quotes from the abstracts for just a few recent papers on the problem:
...
As laid our above, _Static control flow_ means that the destination of every jump or call is determinable _a priori,_ before execution. This has concrete implications for Ethereum's scaling roadmap, particularly around ZK verification, rollups, and future execution layer changes.
...
* _Execution Traces:* When a transaction executes, it produces an execution tracea sequence of state changes recording the value of registers, memory, stack, storage, etc. at each step. The trace is a complete record of the computation.
...
A _ZK-Rollup sequencer or prover batches many transactions and constructs a ZK proof that all transactions executed correctly. The proof is then submitted to L1, where it is quickly verified. Key implications:
