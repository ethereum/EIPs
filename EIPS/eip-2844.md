...
An additional property `kid` with the value which represents the DID, and the `keyFragment` that was used to sign the JWS should be added to the potected header ([details](https://github.com/decentralized-identity/did-jose-extensions/issues/2)). It's also sometimes useful to include the `version-id` to refer to a specific version of the DID document, for example if you want the signed JWS to remain valid after the key has been rotated. 
