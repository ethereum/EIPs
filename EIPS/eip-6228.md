...
struct Request {
    ...
    address[] returnToken; // tokens in which funds are returned after call, ETH is address(0)
    uint tip; // tip on top of keeper fee is in ETH wei
    address[] tokenInUse; // tokens in use, must have allowance to address(this)
