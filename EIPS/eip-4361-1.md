...
capgrok-statement-entry = "(" number ")" capgrok-action *("," capgrok-action) "for" ( "any" / capgrok-resource )
...
- If present, set `capgrok-transformed-statement` to the value of the `statement` field of `siwe`, followed by `" "`.
...
- For each entry in `resources` (starting with the first entry), perform the following:
  ...
  - If `defaultActions` is present, concatenate each action in the array with `, `, prepend with `"(numbering)"` and append the string ` for any.`. Append the resulting string to `capgrok-transformed-statement`. Increase `numbering` by 1.
  ...
  - If `targetActions` is non-empty, concatenate each target action in the array with `, `, prepend with `"(numbering)"` and append the string ` for ` concatenated with the resource name concatenated with `"."`. Append the resulting string to `capgrok-transformed-statement`. Increase `numbering` by 1.
  - If `collapsedActions` is non-empty, concatenate each target action in the array with `, `, prepend with `"(numbering)"` and append the string ` for ` concatenated with the resource name concatenated with `"."`. Append the resulting string to `capgrok-transformed-statement`. Increase `numbering` by 1.

Implementers SHOULD NOT rely on a specific ordering of individual CapGrok Statements in the final CapGrok Transformed Statement since the ordering of JSON properties cannot be guaranteed.
