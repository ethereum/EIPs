...
I further authorize https://example.com to perform the following actions on my behalf: (1) example: read for any. (2) example: append, delete for my.resource.1. (2) append for my.resource.2, my.resource.3.
...
- `defaultActions`: (CONDITIONAL) If present, `defaultActions` MUST be a JSON array of string values with at least one entry where each value describes an action the delegee MAY execute in the RS on behalf of the Ethereum account without tying the scope to a particular target.
- `targetedActions`: (CONDITIONAL) If present, `targetedActions` MUST be a JSON object with variable properties where each property is a JSON array of string values each describing an action the delegee MAY execute in the RS on behalf of the Ethereum account over the target resource denoted by the property name.
...
In the example above, the delegee is authorized to perform the action `read` independent of any resource, `append`, `delete` on resource `my.resource.1`, `append` on resource `my.resource.2` and `append` on `my.resource.3`. Note, the delegee can invoke each action invididually and independent from each other in the RS.
...
capgrok-statement-entry = "(" number ")" capgrok-action 
                          *("," capgrok-action) "for" ( "any" / capgrok-resource )
