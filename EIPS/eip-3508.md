...
In detail, the transaction data the `ORIGINDATA*` opcodes operate on will be equivalent to the `calldata` specified in the `commit` of the `AUTH` message format. If a single-call `AUTH` message is defined, the opcodes will retrieve the data of `calldata`. Otherwise, the opcodes will retrieve the `calldata` of each sub-call (i.e. `call 1`, `call 2` ... `call X`) depending on the call execution context.

This interaction ensures full compatibility with [EIP-3074](./eip-3074) and ensures that no form of discrimination is introduced back into the system by this EIP f.e. by contracts entirely relying on `ORIGINDATA*` and thus allowing only EOAs to supply data to them.
