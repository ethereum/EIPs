...
This ERC standardizes an interface for contract-layer consensus-agnostic verifiable cross-chain bridging, through which we can define new kinds of tokens based on [ERC-20](./eip-20.md)/[ERC-721](./eip-721.md) over multichains.  

<p id='architecture' align='center'>Figure.1 Architecture</p> 
...
### Data Structure
A specific data structure SHOULD be designed to discribe the transactions need to be synchronized, and the name of the transactions of this ERC is RECOMMENDED to be referred to as Omniverse Transaction (`o-transaction` for short).  
The data structure MUST be defined as follows:  
...
- Every ERC6358 compliant contract MUST implement the `IERC6358`  

    ```solidity
    /**
    * @notice Interface of the ERC6358
    */
    interface IERC6358 {
        /**
...
- Optional Extension: Fungible Token  
    ...
    /**
    * @notice Interface of the ERC6358 fungible token, which inherits {IERC6358}
    */
...
- Optional Extension: NonFungible Token  
    ...
    /**
    * @notice Interface of the ERC6358 non fungible token, which inherits {IERC6358}
    */
...
As shown in [Figure.1](#architecture), smart contracts deployed on multi-chains execute `o-transactions` of ERC6358 tokens synchronously through the trustless off-chain synchronizers.   

- The ERC6358 smart contracts are referred to as **Abstract Nodes**. The states recorded by the Abstract Nodes that are deployed on different blockchains respectively could be considered as copies of the global state, and they are ultimately consistent.  
- **Synchronizer** is an off-chain execution program responsible for carrying published  `o-transactions` from the ERC6358 smart contracts on one blockchain to the others. The synchronizers work trustless as they just deliver `o-transactions` with others' signatures, and details could be found in the [workflow](#workflow).

### Principle

- The `o-account` has been mentioned [above](#omniverse-account).
- The synchronization of the `o-transactions` guarantees the ultimate consistency of token states across all chains. The related data structure is [ERC6358TransactionDat](#data-structure).

    - A `nonce` mechanism is brought in to make the states consistent globally.
    - The `nonce` appears in two places, the one is `nonce in o-transaction` data structure, and the other is `account nonce` maintained by on-chain ERC6358 smart contracts. 
...
- `A` initiates an `o-transaction` on Ethereum by calling `IERC6358::sendOmniverseTransaction`. The current `account nonce` of `A` in the ERC6358 smart contracts deployed on Ethereum is $k$ so the valid value of `nonce in o-transaction` needs to be $k+1$.  
- The ERC6358 smart contracts on Ethereum verify the signature of the `o-transaction` data. If the verification succeeds, the `o-transaction` data will be published by the smart contracts on the Ethereum side. The verification includes:
...
- Next synchronizers will rush to deliver this message because of a rewarding mechanism. (The strategy of the reward could be determined by the deployers of ERC6358 tokens. For example, the reward could come from the service fee or a mining mechanism.) 
- Finally, the ERC6358 smart contracts deployed on other chains will all receive the `o-transaction` data, verify the signature and execute it when the **waiting time is up**. 
...
The ERC6358 Token could be implemented with the [interfaces mentioned above](#smart-contract-interface). It can also be used with the combination of [ERC-20](./eip-20.md)/[ERC-721](./eip-721.md).  

- The implementation example of the interfaces can be found at:

    - [IERC6358.sol](../assets/eip-6358/src/contracts/interfaces/IERC6358.sol), the basic ERC6358 interface mentioned [above](#smart-contract-interface)
    - [IERC6358Fungible.sol](../assets/eip-6358/src/contracts/interfaces/IERC6358Fungible.sol), the interface for ERC6358 fungible token
    - [IERC6358NonFungible.sol](../assets/eip-6358/src/contracts/interfaces/IERC6358NonFungible.sol), the interface for ERC6358 non-fungible token

- The implementation example of some common tool to operate ERC6358 can be found at:

    - [Common Tools](../assets/eip-6358/src/contracts/libraries/OmniverseProtocolHelper.sol). 
    - The verification for the signature of an `o-transaction` can be found [here](../assets/eip-6358/src/contracts/libraries/OmniverseProtocolHelper.sol#L89)

- [ERC6358 Fungible Token Example](../assets/eip-6358/src/contracts/ERC6358FungibleExample.sol)
    - [Implementation example of interface sendOmniverseTransaction](../assets/eip-6358/src/contracts/ERC6358FungibleExample.sol#L89)
    - [Implementation example of the execution](../assets/eip-6358/src/contracts/ERC6358FungibleExample.sol#L96)
- [ERC6358 Non-Fungible Token Example](../assets/eip-6358/src/contracts/ERC6358NonFungibleExample.sol)  
    - [Implementation example of interface sendOmniverseTransaction](../assets/eip-6358/src/contracts/ERC6358NonFungibleExample.sol#L97)
    - [Implementation example of the execution](../assets/eip-6358/src/contracts/ERC6358NonFungibleExample.sol#L104)
...
- Suppose the current `account nonce` of a **common user** `A` is $k$ on all chains. `A` has 100 token `X`, which is an instance of the ERC6358 token.    
...
- **Response strategy**:
    ...
    - Both the ERC6358 smart contracts or similar mechanisms on Polkadot and Ethereum will find that `A` did cheating after they received both `ot-E-ac` and `ot-P-ab` respectively as the signature of `A` is non-deniable.  
