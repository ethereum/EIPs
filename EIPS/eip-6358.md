...
The core of ERC is synchronization instead of transferring, even if all the other chains break down, as long as Ethereum is still running, user’s assets will not be lost. 
...
First, a global identifier is needed to identify the ownership, which is referred to as Omniverse Account (`o-account` for short) in this EIP. The `o-account` is RECOMMENDED to be expressed as a public key created by the elliptic curve `secp256k1`. A mapping mechanism is RECOMMENDED for different environments.  
