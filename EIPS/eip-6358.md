...
As shown in [Figure.1](#architecture), smart contracts deployed on multi-chains execute `o-transactions` of omniverse tokens synchronously through the trustless off-chain synchronizers.   
...
- **Synchronizer** is an off-chain execution program responsible for carrying published  `o-transactions` from the ERC6358 smart contracts on one blockchain to the others. They are trustless, and details could be found in the [workflow](#workflow).
