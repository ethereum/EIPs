...
signature = key_id || proof
  ...
  authenticated key_id
        ...
        v
  resolved_signer = authenticator, key_id retained
        |
        v
...
def validate_authenticator(sig, sig_hash, state):
    ...
    return VALID(resolved_signer=authenticator, key_id=authenticated_key_id)
```

`SIGPARAM(0x00)` returns the authenticator address because the authenticator is the entry's resolved `signer`. The validated entry's `key_id` attribute (see [Signature entry attributes](#signature-entry-attributes)) is what `SIGPARAM(0x04)` returns. `SIGPARAM(0x01)` and `SIGPARAM(0x02)` retain their EIP-8141 meaning.
...
`AUTHENTICATOR` is an opaque protocol-validated scheme like `SECP256K1` and `P256`: `SIGDATACOPY` and `SIGPARAM(0x03)` are not defined for it and result in an exceptional halt. The authenticated `key_id` is exposed through a new `SIGPARAM` value instead, so that the raw proof never becomes EVM-visible.

#### Signature entry attributes

EIP-8141 associates one derived attribute with each protocol-validated signature entry, `resolved_signer`, which `SIGPARAM(0x00)` returns. This proposal extends the validated signature entry with a second derived attribute:

| Attribute | Type | Set for | Value |
|---|---|---|---|
| `resolved_signer` | 20-byte address | `SECP256K1`, `P256`, `AUTHENTICATOR` | EIP-8141 semantics; the authenticator address for `AUTHENTICATOR` |
| `key_id` | 32 bytes | `AUTHENTICATOR` only | the identifier returned by the authenticator and matched against the claimed `key_id` |

Both attributes are populated during signature validation, before any frame executes, and are immutable for the rest of the transaction. `key_id` is undefined for entries of any other `scheme`.
...
|---|---|---|
| `0x04` | signatureIndex | `key_id` (`AUTHENTICATOR` only) |

`SIGPARAM(0x04)` returns the entry's `key_id` attribute. Requesting `key_id` for a signature entry of any other `scheme` results in an exceptional halt. Account code MAY read it when it needs per-key policy; doing so is optional and does not change the default authenticator-level authorization model.
...
This proposal assigns signature scheme `0x03`, which EIP-8141 currently reserves, and defines `SIGPARAM(0x04)`. Activation requires a coordinated network upgrade. Pre-upgrade nodes reject transactions using these values; existing EIP-8141 transactions retain their prior behavior.
...
4. Account applies optional per-key policy: read `SIGPARAM(0x04)`, assert it equals the authenticated `key_id`, and accept or reject according to account-defined key-level policy.
...
4. Assert that `SIGDATACOPY` and `SIGPARAM(0x03)` on an `AUTHENTICATOR` entry result in an exceptional halt, and that `SIGPARAM(0x04)` on a `SECP256K1`, `P256`, or `ARBITRARY` entry results in an exceptional halt.
