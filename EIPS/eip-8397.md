...
`SIGPARAM(0x00)` returns the authenticator address because the authenticator is the entry's resolved `signer`. The validated entry also retains `key_id`, which `SIGPARAM(0x04)` returns. `SIGPARAM(0x01)` and `SIGPARAM(0x02)` retain their EIP-8141 meaning.
...
`key_id` is the 32-byte value authenticated during signature validation. Requesting `key_id` for a signature entry of any other `scheme` results in an exceptional halt. Account code MAY read it when it needs per-key policy; doing so is optional and does not change the default authenticator-level authorization model.
