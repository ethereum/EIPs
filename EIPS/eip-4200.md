...
Adding new instructions with immediate arguments needs to be carefully considered when implementing EOF container validation algorithm.

Static relative jumps execution does not require runtime check of the jump destination. It greatly reduces execution cost. Therefor the new opcodes gas cost can also be significantly reduced.

`RJUMPV` instruction relative offset table can have up to 256 one-byte entries so reading an offset cannot be a potential attack surface.         
