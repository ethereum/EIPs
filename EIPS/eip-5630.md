...
- to deserialize a point: use [SEC 1, §2.3.3], while _requiring_ point compression; that is:
  ...
  - that the integer represented by the remaining 32 bytes (as in [SEC 1, §2.3.8]) MUST reside in {1, ..., _p_ - 1}, and moreover MUST yield a quadratic residue modulo _p_ under the Weierstrass expression X^3 + 7 (modulo _p_).
