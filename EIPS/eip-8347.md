...
The artifact is a small fixed header followed by one section per zone, in zone order:
...
`pbtRoot` is the claimed PBT root, recomputed and checked by the [dual-check](#verification-dual-check). Each count is the number of records in its section, which are encoded as defined below. The artifact ends immediately after the final storage record, and any trailing byte makes it invalid.

`headerCount` MAY be zero. An anchor state with no accounts converts to exactly that: a snapshot whose `pbtRoot` is the empty-tree hash of [EIP-8297](./eip-8297.md), 32 zero bytes, with no records in any section, and an empty preimage file. Such a snapshot is not a special case for the dual-check: it passes only against an anchor whose `stateRoot` is the empty MPT root, and consensus anchoring rejects it against any other.
...
All integers in the snapshot are big-endian. A field written `name[n]` is `n` bytes wide. A field written `name[≤w]` is a variable-width unsigned integer of at most `w` bytes: a one-byte length `len`, then the integer in `len` bytes without leading zero bytes, so that zero is `len == 0`. A length above `w`, or a leading zero byte, makes the snapshot invalid, so every integer has exactly one encoding.

#### Header record

Each account is written as one header record in the `ACCOUNT_ZONE` section, holding the leaves of its header stem:
...
`addressHash` is `key_hash(address32)`, where `address32` is `address20_to_address32(address)` per [EIP-8297](./eip-8297.md): the tree position of the account's header stem. Header records MUST be sorted strictly ascending by `addressHash`.

`kind` fixes the account's code and selects what `codeRef` holds:

| `kind` | Account | `codeRef` |
| --- | --- | --- |
| `0x00` | no code | empty |
| `0x01` | contract code | `codeHash[32]` followed by `codeSize[≤4]` |
| `0x02` | [EIP-7702](./eip-7702.md) delegation | `target[20]` |

Any other `kind` makes the snapshot invalid. For `kind == 0x00`, `nonce` and `balance` MUST NOT both be zero: that is an empty account, which [EIP-7523](./eip-7523.md) excludes from the anchor state. For `kind == 0x01`, `codeHash` is the account's `code_hash` and `codeSize` its `code_size`, which MUST NOT be zero. For `kind == 0x02`, the account's code is the indicator `0xef0100 || target`.

The `slot` entries are the account's storage slots 0 through 63, with `slot` the slot number. They MUST be strictly ascending by `slot`, and every `slot` MUST be below `HEADER_STORAGE_SLOTS`.

#### Groups and storage records

The `CODE_ZONE` and `STORAGE_ZONE` sections write their leaves one stem at a time, as groups:

```text
group = stemHash[32] | n[1] | (subIndex[1] | value[≤32]) * (n + 1)
```

`n` is the group's leaf count minus one, so a group holds 1 to 256 leaves, and its entries MUST be strictly ascending by `subIndex`.

In the `CODE_ZONE` section each group is one code group of [EIP-8297](./eip-8297.md), and each entry one chunk. `stemHash` is the tree position of the group's stem, `key_hash(code_hash || tree_index)` as in `get_tree_key_for_code_chunk`, and code groups MUST be sorted strictly ascending by `stemHash`.

The `STORAGE_ZONE` section holds one storage record per account with storage slots 64 and above:

```text
storageRecord = addressHash[32] | groupCount[≤8] | group * groupCount
```

`addressHash` is the account's, and MUST equal the `addressHash` of a header record. Storage records MUST be sorted strictly ascending by `addressHash`, and `groupCount` MUST NOT be zero. Each group is one storage group: `stemHash` is the second digest of `storage_tree_position(address32, tree_index)` per [EIP-8297](./eip-8297.md), and the groups of a record MUST be sorted strictly ascending by `stemHash`.

Every `value` MUST be non-zero, since [EIP-8297](./eip-8297.md) requires a zero-valued key to be absent. A chunk whose 32-byte value is all zero is therefore not written.

#### Leaf derivation

A header record yields, under its header stem `ACCOUNT_ZONE || addressHash`:

- at `BASIC_DATA_LEAF_KEY`, the basic data of [EIP-8297](./eip-8297.md), packed from `nonce`, `balance`, and a `code_size` of 0, `codeSize`, or 23 for `kind` `0x00`, `0x01`, or `0x02`, with `version` and the reserved bytes zero;
- at `CODE_HASH_LEAF_KEY`, `keccak256("")` for `kind == 0x00` and `codeHash` for `kind == 0x01`;
- at `DELEGATION_LEAF_KEY`, for `kind == 0x02` only, `0xef0100 || target` followed by nine zero bytes;
- at `HEADER_STORAGE_OFFSET + slot`, each header slot's `value`.

A code group entry yields the leaf `CODE_ZONE || stemHash || subIndex`, and a storage group entry the leaf `STORAGE_ZONE || addressHash || stemHash || subIndex`, each holding the entry's `value`. A `value` is left-padded to 32 bytes; leaf hashing is defined over the full 32-byte value, unchanged from [EIP-8297](./eip-8297.md).
...
1. **Internal PBT consistency.** Rebuild the PBT from the PBT snapshot leaves: derive them per [Leaf derivation](#leaf-derivation), compute every intermediate node by hashing bottom-up, and verify that the resulting root matches the claimed PBT root. The intermediate nodes are a product of this step, not part of the distributed artifact.
2. **Consensus anchoring.** Re-hash the PBT snapshot leaves under the **MPT schema** using the distributed preimages and verify the result against block `ANCHOR_BLOCK`'s header `stateRoot`. That covers every field the MPT commits: nonce, balance, storage, and `code_hash`. A delegated account carries its code hash in its delegation leaf rather than a `code_hash` leaf, so the re-hash recomputes it from that leaf. Each header record and storage record MUST match a preimage address `address` with `key_hash(address20_to_address32(address)) == addressHash`, and each of its header slots and storage-group entries MUST sit at the key `get_tree_key_for_storage_slot(address20_to_address32(address), slotKey)` for one of that address's preimage slot keys. The MPT paths remain `keccak256` of the 20-byte `address` and of the 32-byte `slotKey`. A record or entry that matches no preimage makes the snapshot invalid: the re-hash has no MPT key to place it under, so the root it computes never covers it, and without this rule a surplus leaf, such as a header slot repeated as a storage-group entry, would pass both checks.

   The MPT holds no code, so the re-hash alone leaves the code leaves and `code_size` unchecked. They are anchored instead to the `code_hash` it does cover. Header records that reference the same `code_hash` MUST carry the same `codeSize`: the code is checked once per `code_hash`, so a differing size on another account would go unchecked. For each distinct `code_hash`, reassemble the bytecode from the 31-byte slices of that code's chunk leaves in chunk order, taking the chunk count from `code_size` and reading an absent chunk as 31 zero bytes, truncate the result to `code_size`, and require its `keccak256` to equal that `code_hash`. Then re-chunk the recovered bytecode per [EIP-8297](./eip-8297.md) and require its non-zero chunks to equal the artifact's code leaves byte for byte, which pins each chunk's leading PUSHDATA count as well as its contents and its placement in `CODE_ZONE`. A wrong `code_size` yields different bytes, so the same preimage pins it. The artifact MUST hold no code leaf beyond those of the `code_hash` values its header records reference: internal consistency recomputes the root over a surplus code group, and neither this check nor the re-hash would read it. A `kind == 0x01` account whose recovered bytecode is a delegation indicator makes the snapshot invalid, since it MUST be written with `kind == 0x02`, and both spellings give the re-hash the same `code_hash`. The header record has no field for `version` or the three reserved bytes, the remaining fields the MPT does not commit; derivation writes them as zero, as [EIP-8297](./eip-8297.md) does on every header write.
