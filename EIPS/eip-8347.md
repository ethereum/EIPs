...
Rollback MUST be performed by discarding state, never by reversing writes. [EIP-7928](./eip-7928.md) records post-values only (`new_value`, `post_balance`, `new_nonce`), so a BAL cannot be inverted: undoing a replayed block would require the values it overwrote, and they do not appear in the artifact.
