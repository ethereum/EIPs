...
- **A smaller consensus change.** `PBT_ACTIVATION_FORK` changes the state commitment and nothing else: at one block, one header field starts being computed from a different tree. The online overlay conversion adds a conversion cursor in the state that advances each block during the conversion window and makes the order of MPT traversal consensus-critical.
- **No hybrid proof verifiers.** The canonical state root in the offline approach commits to the entire state (the MPT root before `PBT_ACTIVATION_FORK`, the PBT root after). During an online overlay conversion it commits only to the part already converted, so proving a key still held in the base MPT needs a second proof against the frozen MPT root. This impacts light clients, on-chain bridge contracts and consumers of state proofs.
...
**No hybrid proof verifiers.** Either way, a new tree changes proof formats and the header carries exactly one root, since [EIP-7612](./eip-7612.md) also swaps the header state root at its fork. The difference is how much that root covers. Offline, it covers the whole state at every block: the MPT root before `PBT_ACTIVATION_FORK`, the PBT root after. During an overlay conversion it covers only the part converted so far, so proving a key still held in the base MPT takes a second proof against the frozen MPT root, which no header carries and which the verifier must pin out of band; proving absence takes both. The cost is on proof verifiers: an on-chain bridge verifier needs three deployments under an overlay, an MPT verifier, then a hybrid verifier carrying an out-of-band root, then a PBT verifier, against two switching at a known block. The middle one is the expensive one, the only one that is not a plain tree lookup, built for a bounded window and then maintained anyway.
