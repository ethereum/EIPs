...
| `nonce`         | Integer | Block nonce as a decimal integer.                               | `0` |
| `timestamp`     | Integer | UNIX timestamp as a decimal integer.                            | `1730232000` |
| `extraData`     | String  | Arbitrary extra data as hex string.                             | `0x00` |
| `gasLimit`      | Integer | Block gas limit as a decimal integer.                           | `30000000` |
| `difficulty`    | Integer | Block difficulty as a decimal integer.                          | `1` |
...
| `terminalTotalDifficulty` | Integer | Total difficulty after which to switch from PoW to PoS as a decimal integer.    | `58750000000000000000000` |
...
The `alloc` field defines the initial state at genesis. It maps addresses (as lowercase hex strings without the `0x` prefix) to the following object:

| Field          | Type    | Description                                                         | Example |
|----------------|---------|---------------------------------------------------------------------|---------|
| `balance`      | Integer | Account balance in wei as a decimal integer.                        | `1000000000000000000` (1 ETH) |
...
| `nonce`        | Integer | Account nonce as a decimal integer.                                 | `0` |
...

{
  ...
  "properties": {
    ...
    "nonce": { "$ref": "#/$defs/uint64" },
    "timestamp": { "$ref": "#/$defs/uint64" },
    "extraData": {
      "anyOf": [
        {"type": "string", "const": "" },
    ...
    "gasLimit": { "$ref": "#/$defs/uint64" },
    "difficulty": { "$ref": "#/$defs/uint256" },
