...
This standard defines an interface for [EIP-721](./eip-721.md) or [EIP-1155](./eip-155.md) tokens, known as "bindables", to "bind" to [EIP-721](./eip-721.md) NFTs.
...
In contrast with other standards that deal with token ownership at the account level, this standard aims to address token ownership at the NFT level. Its objective is to build a universal interface for token bundling, compatible with existing [EIP-721](./eip-721.md) and [EIP-1155](./eip-1155.md) standards.
...
interface IERC1155Bindable /* is IERC1155 */ {
    ...
    /// @notice This event emits when unbound token(s) are bound to an NFT.
    /// @param operator The address approved to perform the binding.
    ...
    /// @notice This event emits when unbound token(s) of different types are
    ///  bound to one or more NFTs belonging to a specific contract address.
    ...
    /// @param bindIds The identifiers of the NFTs being bound to.
    ...
    /// @notice This event emits when NFT-bound token(s) are unbound.
    /// @param operator The address approved to perform the unbinding.
    ...
    /// @param bindIds The identifiers of the NFTs being unbound from.
    ...
    /// @notice Binds `amounts` tokens of `tokenIds` to NFTs `bindIds` at address `bindAddress`.
    /// @dev The function MUST throw unless `msg.sender` is an approved operator
    ///  for `from`. It also MUST throw if the length of `amounts` is not the 
    ///  same as `tokenIds` or `bindIds`, or if any balances of `tokenIds` for
    ///  `from` is less than that of `amounts`. Finally, it MUST throw if the
    ///  NFT contract does not support the ERC-721 interface or if any of the
    ///  NFTs being bound to do not exist. Before binding, tokens MUST be
    ///  transferred to the contract address of the NFTs. On bind completion,
    ///  the function MUST emit `TransferBatch` and `BindBatch` events to
    ///  reflect the implicit batch token transfers and subsequent batch bind.
    /// @param from The owner address of the unbound tokens.
    /// @param bindAddress The contract address of the NFTs being bound to.
    /// @param bindIds The identifiers of the NFTs being bound to.
    /// @param tokenIds The identifiers of the binding token types.
    ...
    /// @dev The function MUST throw unless `msg.sender` is an approved operator
    ///  for `from`. It also MUST throw if the length of `amounts` is not the
    ///  same as `tokenIds` or `bindIds`, if any balances of `tokenIds` for the
    ///  NFT is less than that of `amounts`, or if `to` is the zero addresss.
    ///  After unbinding, tokens MUST be transferred to `to`, during which the 
...
The bindable token interface is designed to be compatible with existing EIP-721 and EIP-1155 standards.

## Reference Implementation

For a reference EIP-721 implementation:

- [EIP-721 Bindable](../assets/eip-5700/erc721/ERC721Bindable.sol).

For a reference EIP-1155 implementation:
