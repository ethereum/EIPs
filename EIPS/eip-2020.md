...
ethereum.on(subscriptionType: String, params: Array<any>, listener: (result: any) => void): Promise<String>;
...
The implementing Ethereum Provider **MUST** create a subscription using `eth_subscribe` with params `[subscriptionType, {...params}]` and **MUST** return a Promise that resolves with `subscriptionId: String`.
