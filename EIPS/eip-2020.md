...
Results will emit on `subscriptionId` using [EventEmitter](https://nodejs.org/api/events.html). Attach listeners using:
...
This will also call `ethereum.removeAllListeners(subscriptionId)`.

### Events

Emitted using [EventEmitter](https://nodejs.org/api/events.html).
