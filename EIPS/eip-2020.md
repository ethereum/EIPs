...
ethereum.on(subscriptionType: String, params: Array<any>, listener: (response: any) => void): Promise<String>;
...
The implementing Ethereum Provider **MUST** return any response for the subscription to the `listener` callback untouched.

If there an error during the listening of the subscription, the Ethereum Provider **MUST** return with an Error object on the `response.error` property containing a human readable string message describing the error and **SHOULD** populate code and data properties on the error object with additional error details.
