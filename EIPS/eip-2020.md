...
```js
// Ethereum JSON-RPC API requests (https://github.com/ethereum/wiki/wiki/JSON-RPC)
ethereum.send(method: String, params: Array<any>): Promise<any>;

// Subscriptions: RPC pub-sub (https://github.com/ethereum/go-ethereum/wiki/RPC-PUB-SUB)
ethereum.subscribe(subscriptionType: String, params: Array<any>, listener: (result: any) => void): Promise<any>;
ethereum.unsubscribe(subscriptionId: String): Promise<any>;
...
function start(ethereum) {
  ...
  // Example: Log latest blocks
  let subscriptionId;
  ethereum
    .subscribe('newHeads', [], result => {
    ...
    .then(id => {
      // Promise resolves with subscriptionId
      subscriptionId = id;
    })
    .catch(error => {
      console.error(
  ...
  // Example: Log accounts if changed
...
The implementing Ethereum Provider **MUST** return every subscription response `result` to the `listener` callback untouched.

If an error occurs during the listening of the subscription, the Ethereum Provider **MUST** return an Error object to the `listener` callback containing a human readable string message describing the error and **SHOULD** populate code and data properties on the error object with additional error details.
...
If the network connection closes, the Ethereum Provider **MUST** emit an event named `close` with args `code: Number, reason: String` following the [list of status codes for `CloseEvent`](https://developer.mozilla.org/en-US/docs/Web/API/CloseEvent) and a short human readable reason.
