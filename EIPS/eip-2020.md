...
See [methods](https://github.com/ethereum/wiki/wiki/JSON-RPC#json-rpc-methods).
...
See [subscription types](https://github.com/ethereum/go-ethereum/wiki/RPC-PUB-SUB#supported-subscriptions).

Results will emit on `subscriptionId` using [EventEmitter](https://nodejs.org/api/events.html). Attach listeners with:
...
The provider will also call the [EventEmitter](https://nodejs.org/api/events.html) method `ethereum.removeAllListeners(subscriptionId)`.
...
The event emits with the `networkId` of the new network returned from `net_version`.
...
The event emits with an array of the accounts public keys.
...
function start(ethereum) {
  ...
  ethereum
    ...
    .then(subscriptionId => {
      ...
      ethereum.on(subscriptionId, result => {
...
If the `unsubscribe` method returns successfully with a `True` result, the implementing provider **MUST** remove all listeners on the subscription using `ethereum.removeAllListeners(subscriptionId);`.
