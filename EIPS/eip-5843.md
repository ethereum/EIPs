...
Modular multiplication, addition and subtraction are the most common operations.  Other operations such as modular inverse and square root commonly appear, but do not appear to constitute the majority of overhead in most use-cases.  However, for many moduli of interest these operations can be implemented using only addition/subtraction/multiplication.
...
evmmax_state = EVMMAXState(mod, r_squared, mod_inv_small, input_size)
```
