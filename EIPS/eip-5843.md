...
* can be represented in `MAX_INPUT_SIZE` * 4 bytes.
...
`mulmont` is the generic algorithm for Montgomery Multiplication.  There are other algorithms with quadratic complexity that are more performant than the generic algorithm at small limb counts.  Appendix 1 describes the most popular of these. (TODO: note about difference between `mulmont` mod_inv and the one used in the subquadratic algorithms).
