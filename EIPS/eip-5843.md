...
* The skillset required to implement performant crypto software is incredibly niche.  This makes developing and maintaining multiple production-grade implementations of the same specification difficult.
    * This can have the effect that multiple clients will default to use the same implementation.  This introduces a potential single point of failure for the network and can effectively enshrine crypto implementations into the spec if the precompile pricing is aggressive.
...
Instead of introducing many specific algorithms as siloed precompiles, the EVM should be improved to address bottlenecks which make user-implementable crypto prohibitively expensive.  All previously-mentioned precompiles share a common performance bottleneck: modular arithmetic over a static, odd modulus.
...
**Note**: Gas models are based on benchmarks of the Go implementation for the arithmetic on consumer hardware (i7-6500U CPU @ 2.50GHz, 512kb L2 cache).  They assume a target gas rate of 25 nanoseconds of execution time per gas.
...
```
# given two equally-sized, bigint numbers x, y: return x >= y
