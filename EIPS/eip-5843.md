...
When the modulus is prime, modular inverse of a value can be computed as `(val ** (modulus - 2)) % modulus` using Fermat's Little Theorem.  This should be sufficient for most cases unless it can be demonstrated that there is a desired use-case which has inverse as a major bottleneck.
