...
`EVMMAX_INPUT_SIZE` is tentatively set to 16 until cache-miss-causing benchmarks are implemented.

### Trading more Precomputation (and Code Size) for Better Performance:

For contracts with a static modulus, we embed the modulus, Montgomery constant and some execution hints in a new EOF section in the deployed bytecode if `EVMMAXState` has been set during the contract creation.

We want to provide enough execution hints to allow blocks of consecutive EVMMAX arithmetic ops to be executed "atomically":
...
* jumping to any opcode past the first position in a block is prohibited.

So the per-block execution hints provided here are:
* a list of block input slots
* the maximum slot accessed by a block
* a count of the number of each operation type performed (multiplication/addition/subtraction) for calculating block gas cost

This approach removes much of the non-arithmetic overhead in blocks of EVMMAX arithmetic ops, potentially warranting the use of [particle gas costs](eip link) for arithmetic ops in blocks.
...
* setting modulus at contract deployment time and embedding modulus/Montgomery parameters in an EOF section in the deployed bytecode.  advantages: better static analysis?
    * opens the possibility to add an EOF section which can remove most interpreter-loop overhead (remove memory checks, remove stack checks, charge gas once for blocks of multiple EVMMAX opcodes, particle gas metering for EVMMAX opcodes)
