...
## Motivation
Additional crypto precompiles have long been desired for the EVM. Among these, operations for various additional elliptic curves beyond currently-supported bn128 have been proposed and not yet adopted ([BLS12-381](./eip-2537.md), [BW6-761](./eip-3026.md), [MNT4](./eip-1895.md), [Baby Jubjub](./eip-2494.md)).  Other EIPs took an additional step, and try to introduce generic precompiles (TODO: better summary of what they actually do): ([EIP-1962](./eip-1962.md), [EIP-1829](./eip-1829.md)).
...
* They introduce complicated algorithms into the Ethereum protocol which are not understood by most client developers, and often lack good explanatory resources beyond academic papers.
    * Input deserialization code and gas-charging logic for precompiles are other sources of protocol surface which have previously seen consensus bugs.
* It takes significant development/testing time and effort to ensure performance and correctness.  And even this effort is often not sufficient as precompiles have been the source of previous consensus bugs (e.g.: bn128 ecmul not doing subgroup checks, ecrecover bug from 2016, modexp bug, others?).
* The skillset required to implement performant crypto software is very niche.  This makes developing and maintaining multiple production-grade implementations of the same specification difficult.
...
* The useless Blake2 precompile made it to production (TODO: as a result of core-dev lack of domain-knowledge to evaluate its suitability? don't want to insult people who were involved here).
...
For prime moduli, modular inverse can be computed using a modular exponentiation by a constant via Fermat's Little Theorem.  This should be sufficient for most cases unless it can be demonstrated that there is a desired use-case which has inverse as a bottleneck.
...
* `SETMODX` has linear complexity:  the operation is dominated by two modular reductions by the modulus and a word-sized modular inverse.  Modular reduction should have `O(N**2)` complexity.  But for the benchmarks I've ran, I saw linear behavior.
...
### Aggressive Pricing for Certain Bitwidths

### Interpreter-loop Overhead Reduction
