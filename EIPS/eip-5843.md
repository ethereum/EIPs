...
input_size = len(binascii.hexlify(mod)) - 2
...
# modular addition
# input
#    x, y, mod - arrays of limbs, big-endian limb-order.
#    x, y < mod
#    len(x) == len(y) == len(mod)
# returns
#    (x + y) % mod expressed as an array of limbs with big-endian order
...
# 	mod_inv_full - pow(-mod, -1, R) == (-mod**-1) % R
# output:
#	(x * y * pow(R, -1, mod)) % mod
#
...
`mulmont` is the generic algorithm to perform Montgomery Multiplication.  There are other algorithms which achieve better performance at small bit-widths.  These use a different Montgomery constant (`mod_inv`).  However, an implementation that uses naive `mulmont` can compute `mod_inv_full` from `mod_inv` with minimal overhead.
...
#
# input:
#	x, y, mod - multiprecision numbers represented by arrays of limbs
#	x, y < mod
#	mod % 2 != 0
#	mod_inv_small = pow(-mod, -1, 1<<SYSTEM_WORD_SIZE_BITS)
# output:
#	(x * y * pow(R, -1, mod)) % mod represented by an array of limbs in ascending order of significance
#
