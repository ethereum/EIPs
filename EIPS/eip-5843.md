...
When the modulus is prime, modular inverse can be computed using a modular exponentiation by a `modulus - 2` using Fermat's Little Theorem.  This should be sufficient for most cases unless it can be demonstrated that there is a desired use-case which has inverse as a major bottleneck.
