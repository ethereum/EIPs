...
- Any user of the script learned from `scriptURI` MUST validate the script is either at an immutable location, its URI contains its hash digest, or it implements ERC-5170, which asserts authenticity using signatures instead of a digest.
