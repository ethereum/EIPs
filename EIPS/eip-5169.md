...
- Any user of the script learned from `scriptURI` MUST validate the script is either at an immutable location, its URI contains its hash digest, or it implements [EIP-5170](./eip-5170.md), which asserts authenticity using signatures instead of a digest.
