...
3. Many NFTs have airdrops. When their NFTs are staked, users cannot directly receive airdrops, which brings huge losses to users.
...
1. Compared with transferring the NFT to the project contract, it is more secure and can guarantee user's ownership of the NFT. Unless sold and transferred, in any other scenario, the NFT can participate and can be kept in the user's wallet.
...
3. The NFT is always kept in the user's wallet. Even if the NFT is locked during the staking, the NFT airdrop can still be claimed directly.
