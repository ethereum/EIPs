...
author: Wilkins Chung (@wwhchung), Jalil Wahdatehagh (@jwahdatehagh), Cry (@crydoteth), Sillytuna (@sillytuna)
...
Control of `mainAddress` and ownership of `mainAddress` assets by `authAddress` is proven if all the following conditions are met:
 ...
 - `authENS` has an ENS TEXT record `vault` in the format `eip5131:<authKey>:<mainAddress>`
 - `mainAddress` has an ENS resolver record and a reverse record set to `mainENS`.
 - `mainENS` has an ENS TEXT record `<authKey>`
 

### Setting up one or many `authAddress` records on a single ENS domain
The pre-requisite assumes that the `mainAddress` has an ENS resolver record and reverse record configured.

1. Using your `mainAddress` wallet create a subdomain record for `mainENS`, which will be the `authENS`.
...
4. Using `mainAddress` configure the `<authKey>` (this can be any string in the format `[0-0A-Za-z]+`) TEXT record of `mainENS` to `authAddress`
5. Using `authAddress`, set the `vault` TEXT record to `authENS` to `eip5131:<authKey>:<mainAddress>`
...
2. Get the `vault` TEXT record of `authENS`
3. Parse `eip5131:<authKey>:<mainAddress>` to determine the `authKey` and `mainAddress`.
...
To revoke permission of `authAddress`, delete the `authKey` TEXT record of `mainENS` or update it to point to a new `authAddress`.
