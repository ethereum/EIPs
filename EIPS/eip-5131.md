---
eip: 5131
title: Secondary Account Authentication For ENS (SAAFE)
...
2. Set a TXT record `eip5131:<authKey>` on `mainENS`, with the value set to the desired `authAddress`.
3. Set a TXT record `eip5131:vault` on `authENS`, with the value set to the `<authKey>:mainAddress`.
