...
1. Determine the effect the instruction has on the operand stack:
   1. **Check** if the recorded stack height satisfies the instruction requirements. Specifically:
      - for `CALLF` instruction the required stack height is the number of inputs of the called function according to its type defined in the type section.
...
4. **Check** if all instruction where reached by the analysis.
5. Determine the function maximum operand stack height:
