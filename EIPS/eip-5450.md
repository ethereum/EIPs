...
6. `ceil(maximum data stack height / 8)` matches the corresponding code section's `max_stack_height` within the type section body.
7. No unreachable instructions exist in the code section.

To examine every reachable code path, validation needs to traverse every instruction in order, while also following each non-conditional jump, and following both possible branches for each conditional jump. After completing this, verify each instruction was visited at least once. Fail if any instructions were not visited as this invalidates 9). See below for reference implementation.
