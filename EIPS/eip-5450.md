...
In case function pushed on the stack more items than inputs required by the jumped-to function, previously defined `JUMPF` instuction behaviour included removing extra items:
...
Given new deploy-time guarantees of function execution never underflowing its stack frame, `JUMPF` instruction can check only that there is enough items for input arguments of the callee on the stack, without making sure there is exactly `inputs` items and not more. 

Therefore previously defined behavior of discarding extra items is removed, and only the check for enough inputs is done:
