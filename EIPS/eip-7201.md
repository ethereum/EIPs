...
$
\begin{align}
L_{root} := & \; root                                     && \nonumber \text{--- root of the tree}  \\
            & |\ L_{root} + k                             && \nonumber \text{--- consecutive values, structures and fixed size arrays} \\
            & |\ \texttt{keccak256}(L_{root})             && \nonumber \text{--- dynamic arrays}     \\
            & |\ \texttt{keccak256}(H(q) \oplus L_{root}) && \nonumber \text{--- mappings (Solidity)}  \\
            & |\ \texttt{keccak256}(L_{root} \oplus H(q)) && \nonumber \text{--- mappings (Vyper)}     \\
\end{align}
$
