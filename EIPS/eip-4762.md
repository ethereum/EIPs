...
Whenever the state is read, an access event of the form`(address, sub_key, leaf_key)` takes place, determining what data is being accessed. We define access events as follows:
...
If a `*CALL` or `SELFDESTRUCT` is value-bearing (ie. it transfers nonzero wei), whether or not the `callee` is a precompile, process these additional access events:
