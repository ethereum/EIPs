...
If a `*CALL` or `SELFDESTRUCT`, `EXTCODESIZE`, `EXTCODEHASH` or `EXTCODECOPY` targetting a non-precompile, non-system contract, also process the access event:
...
In the case of an EIP-7702 delegating contract, replace `address` with `delegated_address`.
...
If the authorization is valid, process the following write events:
