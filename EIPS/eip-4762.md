...
When:
 ...
 1. a non-precompile or a system contract is the target of a `*CALL`, `SELFDESTRUCT`, `EXTCODESIZE`, or `EXTCODECOPY` opcode,
 2. a non-precompile or a system contract  is the target address of a contract creation whose initcode starts execution,
...
When calling `EXTCODEHASH` on a non-precompile and neither system contract target, process the access event:
