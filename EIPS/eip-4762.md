 ...
 * Perform the following steps unless `address` is either:
   ...
   * the address of the originator of a `CALL`, `STATICCALL`, `DELEGATECALL` and `CALLCODE` instruction; or,
   * the address of the target of a `CALL`, `STATICCALL`, `DELEGATECALL` and `CALLCODE` instruction.
