...
interface IERC5380 is IERC165 {
    /// @notice Emitted when the amount of entitlement a user changes. If user is the zero address, then the user is the owner
    event Entitle(address indexed user, address indexed contract, uint256 indexed tokenId);
    ...
    /// @notice             Get the maximum number of users that can recieve this entitlement
