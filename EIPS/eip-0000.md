...
discussions-to: https://ethereum-magicians.org/t/eip-0000-unified-multidimensional-fee-market/25010
status: Draft
...
```python
def get_max_priority_fee(tx, gas_used: list[int], base_fees: list[int]) -> int:
    # Calculate total gas and fees for all resources excluding blobs
    tip_indices = [0, 2]
    tgas = sum(gas_used[i] for i in tip_indices)
    tfee = sum(gas_used[i] * base_fees[i] for i in tip_indices)

    # Return maximum specified priority fee, implied by gas usage
    if tx.type == NEW_TX_TYPE:
        if len(tx.max_priority_fee_per_gas) == 1:
            tmax_fee = tx.max_fee - gas_used[1] * base_fees[1] # Ignore blobs
            tpriority_fee = tmax_fee - tfee if tmax_fee > tfee else 0
            return min(tx.max_priority_fee_per_gas[0] * tgas, tpriority_fee)
        else:
            return sum(vector_mul(tx.max_priority_fee_per_gas, gas_used))
    elif is_legacy(tx.type): # Legacy tx
        tmax_fee = tx.gasprice * tgas
        return tmax_fee - tfee if tmax_fee > tfee else 0
    else: # EIP-1559 or Blob tx
        tmax_fee = tx.max_fee_per_gas * tgas
        tpriority_fee = tmax_fee - tfee if tmax_fee > tfee else 0
        return min(tx.max_priority_fee_per_gas * tgas, tpriority_fee)
```
...
**At the start of processing a transaction**:
  ...
  * Check that `max_fee >= get_required_max_fee(base_fees, tx_gas_limits)`
  * Check that `all_less_or_equal(vector_add(gas_used_so_far, tx_gas_limits), gas_limits)`. The block's `gas_limits` are defined in the next subsection.
  * Deduct `max_fee` wei from the sender, which we define as the address recovered from the transaction’s signature. 

**At the end of processing a transaction**:

  * Compute: 
    * `tx_gas_consumed` as a three-item vector, where the first item is the amount of execution gas actually consumed, and the second and third items are the blob and calldata gas amounts, which are equal to their limits from `tx_gas_limits`.
    * `max_priority_fee = get_max_priority_fee(tx, tx_gas_consumed, base_fees)`
  * Burn `base_fee_paid = sum(vector_mul(base_fees, tx_gas_consumed))`.
  * Transfer to the coinbase `priority_fee_paid = min(max_priority_fee, max_fee - base_fee_paid)`.
  * Refund the sender `max_fee - base_fee_paid - priority_fee_paid`.
    ...
    for i in range(len(parent.excess_gas)):
        if GAS_RESERVE_FACTOR[i] > 0 \
            ...
            if parent.gas_used[i] >= targets[i]: # Add
                ...
                excess = max(0, parent.excess_gas[i] - delta * GAS_NORMALIZATION_FACTOR // limits[i])
...
$$
p'_i = \min(p_i,\;f_i-b_i)
$$

for all dimensions $i$. Assume that when the transaction is submitted, the user specifies a `max_fee_per_gas` vector $\mathbf{f}$ such that all entries individually satisfy all base fee $\mathbf{b}$  criteria

$$
f_i \ge b_i \;\;\;\; \text{for all dimensions } i.
$$

The gas limits also satisfy the actual gas consumption

$$
l_i \ge g_i \;\;\;\; \text{for all dimensions } i.
$$

The `max_priority_fee_per_gas` vector $\mathbf{p}$ is also considered sufficient by many proposers, when they jointly weigh the reward against competing transactions using a weight vector $\mathbf{w}$, considering contention across relevant dimensions:

$$
\sum p'_i g_i \ge \sum w_i g_i
$$
...
It should here be noted that the required priority fee per gas in some dimension cannot be directly ascertained from the priority fee per gas that has been stipulated for that dimension in recent transactions. Assume that there are 10 dimensions and each transaction includes a vector stipulating `max_priority_fee_per_gas` for each. The builder will in its inclusion decision operate on the aggregate, and its requirements across dimensions can thus only be inferred, not observed directly. This means that per-dimension estimates of required priority fees are not trivial, and most may still prefer the simplicity of a single one.
...
Accordingly, calldata is treated as an "unconditional" resource, as defined in the specification. An includer MUST therefore signal `INVALID_INCLUSION_LIST` if—and only if—a calldata‑using IL transaction is omitted despite sufficient capacity remaining in all conditional resources it uses.

#### Reserve price

An EIP-7918 reserve price is used for calldata just as for blobs. This ensures that the equilibrium price does not fall to levels where the fee market update mechanism stops working satisfactorily. Furthermore, unlike blobs, calldata will be stored in history forever, making a modest relative reserve price motivated from a resource preservation perspective. Finally, without a reserve price on calldata, it could become cheaper than blob data (particularly below the blob reserve price), and thus a rational choice for L2s. 
