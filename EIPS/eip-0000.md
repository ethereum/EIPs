...
The MPT is hash-keyed and cannot be iterated back into raw keys, so key preimages MUST be distributed alongside the PBT snapshot. Preimages serve (a) self-converters running on hash-keyed clients and (b) verifiers performing the consensus-anchoring check. Preimages MUST be extracted at `ANCHOR_BLOCK`, so the preimage set corresponds exactly to the state committed by `ANCHOR_BLOCK`'s `stateRoot`.
...
### Why the artifact carries no chunking or manifest

An optional design chunkes the PBT snapshot at fixed, stem-aligned byte boundaries and ships a manifest of per-chunk hashes inside the byte-canonical artifact. That is deliberately not specified here. The [dual-check](#verification-dual-check) is the sole root of trust: it authenticates the reassembled artifact against `ANCHOR_BLOCK`'s `stateRoot`, so a manifest adds no security. Its only value is download efficiency (partial-download integrity, parallelism, resumption), and the transports already in use for distribution (snap-sync-style peer-to-peer, era-style files, CDNs, torrents) provide exactly that. Specifying a normative chunking scheme would duplicate transport-layer functionality and would force producers to cut boundaries bit-identically merely to keep the in-artifact manifest canonical. Dropping it leaves the artifact as a byte-canonical, PBT-key-sorted leaf stream with a small header, and leaves splitting for transport to the distribution layer, which MAY publish a non-normative chunk-hash index as a download accelerator.
...
Shadow-root publication is deliberately kept out of consensus, and the wire mechanism fixed by the companion specification MUST NOT be a block-validity rule during the shadow-commitment period. Enforcing shadow roots as a validity condition would require every validator to compute the PBT post-state root for every block, which puts PBT construction back on the consensus-critical path. That is the exact property the offline model exists to avoid, and it would re-introduce the two-trees-consensus-live attack surface. For the same reason, the shadow root is not carried in a consensus container (the `BeaconBlockBody` or the execution payload): changing an SSZ container is itself a hard-fork change, so even an unchecked field would pull a fork change in purely for observability.

The intended mechanism is therefore out-of-consensus: a proposer-signed sidecar on a dedicated gossip topic (the "proposer-signed post-import sidecar" referenced in [Security Considerations](#security-considerations)). It needs no validity rule and no container change. Publication stays a `SHOULD`; a missing shadow root dents the coverage metric rather than counting as a divergence, and cross-client agreement accumulates by observation rather than by enforcement.

The sole scoped exception is the final blocks before `SWAP_FORK`. As noted under [Unvalidated-flip input](#security-considerations), the PBT root activated for the last pre-fork block is never validated by consensus. Hard-enforcing the shadow root for just those final blocks is a deliberate, narrowly-scoped consensus change confined to the swap boundary, not the whole shadow period; the alternative mitigation, relying on sustained cross-client agreement over the period, needs no consensus change at all.
...
- **Execution semantics.** `SWAP_FORK` changes the state commitment **only**. Gas costs, opcode behavior, transaction validity, and the meaning of `code_hash` are unchanged across the fork. `EXTCODEHASH` MUST return the same value for every account across `SWAP_FORK`, because `code_hash = keccak256(bytecode)` is independent of the PBT merkelization hash. Post-swap PBT-native gas changes (e.g. chunk-granular code access, stem warm/cold semantics) are out of scope for this EIP and MUST NOT be bundled with `SWAP_FORK`.
