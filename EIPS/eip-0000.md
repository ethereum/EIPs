...
The `ciphertext_envelope` decrypts to `plaintext_tx`, an EIP-1559 transaction to be executed onchain.
...
This allows users to delegate trust selection to their chosen decryptor instead of listing trusted third parties in every ST. It also reduces pressure toward a single dominant decryptor. For example, a small decryptor can trust a larger, widely used decryptor, allowing its users to be placed after that decryptor’s commitments without requiring those commitments to count against their safety bound. The difference to EIP-8105 is that there is no hard requirement that only trusted decryptors may execute before the transaction, unless `max_preceding_untrusted_commitments = 0`. This expansion is viable because of the additional steps that the design takes against probabilistic front-running, including the offchain decryption process. A few untrusted decryptors are therefore unlikely to be a concern for most users.
