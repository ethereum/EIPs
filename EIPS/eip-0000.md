...
This standard proposes an extension to [EIP-721](./EIPS/eip-721.md) Non-Fungible Tokens (NFTs) to construct reference relationships among NFTs.

## Abstract
This standard is an extension of [EIP-721](./EIPS/eip-721.md). It proposes two referrable indicators, referring and referred, and a time-based indicator createdTimestamp. The relationship between each NFT forms a Directed acyclic graph (DAG). The standard allows users to query, track and anlayse their relationships.
...
*Easy Integration*: This standard makes it easier for the existing token standards or third-party protocols. For instance, the referable NFT can be applied to rentable scenarios (cf. [EIP-5006](./EIPS/eip-5006.md) to build a hierarchical rental market, where multiple users can rent the same NFT during the same time or one user can rent multiple NFTs during the same duration). 

## Backwards Compatibility
This standard can be fully [EIP-721](./EIPS/eip-721.md) compatible by adding an extension function set.
