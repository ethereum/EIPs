...
description: Gas accounting by EVM resource, increasing throughput and improving resource abuse controls, with minimal changes to the protocol and UX
...
But what is the benefit of doing this? First, using a multidimensional scheme to meter resources allows us to pack blocks more efficiently, while retaining a finer-grain control over resource abuse. In these schemes, even if a block has already reached the limit of one resource, we can still add more transactions to that block if they do not use the bottleneck resource.

Second, even though full multidimensional pricing enables more flexible pricing, this flexibility comes at the cost of changing how users interact with Ethereum. [EIP-7999](./eip-7999.md) puts forward a proposal for how to improve the UX under a multidimensional pricing scheme. However, it still requires additional changes beyond tracking gas costs by resource independently. So, multidimensional metering is a middle step towards full multidimensional pricing, which only touches the internals of how the EVM tracks gas consumption. Additionally, the protocol changes are minimal and does not require a new transaction type.
...
The header sequence with the new field is `[..., excess_blob_gas, max_gas_metered]`.
...
2. Finer-grained control over resource abuse: metering gas costs independently for each resource allows us to tailor each operation's gas cost to the actual resource limits.
3. Keeps UX identical: users still specify one `gas_limit` and see one `gas_used` value.
4. Simplicity: This simpler metering change lays the groundwork for eventual multidimensional pricing without disrupting the current fee market. Thus, this proposal serves as a stepping stone for future multidimensional gas pricing designs.

### Why are we choosing this resource split?

Ethereum’s slot-based structure introduces a strict temporal constraint: all attestations must be processed, aggregated, and propagated within a single 12-second slot. This fact makes time a fundamental resource. To maintain network health, validators must execute blocks, validate them, and gossip attestations quickly enough to avoid missed slots and penalties.
...
4. **Long-term Memory**: Tracks persistent changes to disk usage. There are two dimensions here:
...
### Worse-case blocks and resource abuse
