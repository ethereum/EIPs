---
eip: 0000
title: Mili-gas Counter for High-precision Gas Metering
description: 
author: 
discussions-to: 
status: Draft
...
This proposal introduces a new `Uint` counter to the EVM, `mili_gas_left`. Gas costs are defined in `mili_gas` and internal EVM gas accounting is entirely carried out in `mili_gas`.
...
The current `gas_left` variable from the internal state of the EVM is replaced by `mili_gas_left`:
...
All references to `evm.gas_left` are replaced by `evm.mili_gas_left`.
...
```python
tx_output.gas_left = ceil(evm.mili_gas_left / 1000)
```

At the same time, after the transaction is executed, `evm.mili_gas_left` is rounded up to `evm.mili_gas_used = ceil(evm.mili_gas_used / 1000)`.
...
With the introduction of a new counter, there are two levels at each we could convert `mili_gas` to `gas`, namely at transactions-level or block-level. Conversion at block-level leads to the least rounding errors on block utilization. However, it requires a bigger change to implement as `mili_gas` will need to be used when computing transaction fees and limits and `mili_gas` will need to be incorporated in the block environment.

So, the question is whether a conversion at transaction-level is good-enough at reducing rounding errors. To estimate this, we extracted the opcode counts per transaction for 1000 mainnet blocks and computed the new gas prices of all compute operations assuming an anchor of 50Mgas/s and the estimated execution times from [EIP-7904's empirical analysis](../assets/eip-7904/gas-cost-estimator-report.pdf). With this repricing, the fastest opcode (`JUMPDEST`) would be priced at 90 mili-gas units (i.e., 0.09 gas units). We also assumed that the non-compute operations and precompiles would have the same price, and we excluded any other transaction costs such as intrinsic costs and calldata.

Then, we computed the rounding error of each transaction assuming a conversion at transaction versus at opcode level. The opcode-level conversion is equivalent to not using fractional gas and rounding all opcodes to an integer. The next plot shows the distribution for both errors. The x-axis is in logarithmic scale.

![round_error_by_anchor](../assets/eip-0000/round_err_tx.png)

In this sample of 1000 blocks, a transaction-level conversion is already successful at reducing the rounding errors from an average of 4% to 0.05% per transaction.
