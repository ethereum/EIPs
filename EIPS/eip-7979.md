...
After the EIP as been activated code beginning with the `MAGIC` bytes MUST be the leading bytes of a `valid` program.  Execution begins immediately after the MAGIC bytes.  (Opcode and magic values are still to be determined.)
