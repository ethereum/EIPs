...
description: Three new EVM opcodes to support calls and returns.  Optional validated conrol flow.
...
Especially relevant to the EVM's design are the JVM, Wasm, and .NET VMs.  They share some important features:  their programs are all represented with a portable bytecode that  can be directly interpreted, and can also be translated in one pass into more efficient code for the application.  For the EVM this could include machine code for off-chain performance, virtual register code for faster interpreters, and ZK-optimized code for rollups. 
...
### Why do dynamic jumps cause quadratic control-flow analysis?
