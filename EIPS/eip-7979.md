...
2) The `JUMP` and `JUMPI` instructions MUST NOT address immediate data, and MUST address a `JUMPDEST`.
3) The `CALLSUB` instruction MUST NOT address immediate data, and MUST address an `ENTERSUB`.
4) The `JUMP`, `JUMPI` and `CALLSUB` instructions MUST be preceded by a `PUSH` instruction.
...
7) The `stack height` is the absolute difference between the current `stack pointer` and the `stack pointer` at the most recent `ENTERSUB`;
