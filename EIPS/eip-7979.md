...
* represented with a portable bytecode
* that can be directly interpreted
* and can be validated before runtime
* to have static control flow that can be
* compiled to machine code with a one-pass JIT.
...
Again, this would break the promise of "the smallest possible change," and [EIP-3540: EOF - EVM Object Format](./eip-3540) remains available. But note that this restriction impedes important optimizations, which EOF supported by adding special opcodes.  But if we want restricted code sections this becomes a much bigger change, and we should consider [EOF - Functions](./eip-4750) and related EIPs separately.  This proposal uses the same return-stack mechanism and poses no forwards compatability issues with EOF Functions.
