...
Jumps and conditional jumps were supported by the hardware of the Analytical Engine.  Calls and returns required manual shuffling of punch card decks. In 1945 Turing proposed his Automatic Computing Engine, which supported calls directly with a stack of memory crystals holding return addresses:
...
Turing's ACE was a 32-bit RISC machine with integer and floating point operations, 32 32-bit registers, a 1024-entry return stack, and 25K of RAM on a 1-MHz bus.  The Pilot ACE was for a while the world's fastest computer.
...
Unlike these machines, the Ethereum Virtual Machine _does not_ provide facilities for calls and returns. Instead, they must be synthesized using the `JUMP` instruction, which takes its argument on the stack. Further, the EVM provides _only_ this jump.  The EVM's dynamic jump causes problems. First, the need to synthesize static jumps and calls with dynamic jumps wastes some space and gas, as we will show below.  The much bigger problem is this: jumps that can dynamically branch to any destination in the program are a denial-of-service vulnerability that can cause quadratic "path explosions" when traversing the program's flow of control.  Dynamic jumps are common enough in phsyical machines, but less so in virtual machines, whose code is often the source for an AOT or JIT compiler.  For this reason the JVM, Wasm, and .NET do not support dynamic jumps.

#### Going quadratic

Static analysis amounts to any analysis of a program that you can do without actually running it.  And traversing the flow of control -- in a way that follows each possible path only once -- is a fundamental first step for many static analyses.  One of the simplest ways to traverse the control flow of a program is via the standard "depth-first search" (DFS) algorithm for searching a directed graph.  In essense:

* Traverse the code instruction by instruction:
  * When a branch or call is encountered:
    * Recurse to traverse its destination(s).
  * When a destination is re-encountered:
    * Return from recursion.
  
The algorithm is "depth-first" in that it recurses into the destinations of branches and calls before proceeding with the main loop.  In the simple form above the algorithm is searching for cycles, but it can be extended to do much more.  E.g.  As a validation algorithm it can track stack height and search for underflows and overflows, as a JIT it can serialize virtual stack code directly into machine code in one pass, and for many other static analyses it can extract a control flow graph in time and space linear in the size of the program.  

When all jumps are static the number of steps is linear in the number of instructions: only one or two paths must be explored for each jump.  But with dynamic jumps every possible destination must be explored at every jump: at worst, the number of steps is quadratic in the number of instructions.  Consider:
...
The trace of the execution of this program is as follows:

```
PUSH1 -> JUMP -> JUMPDEST -> PUSH1 -> JUMP -> JUMPDEST -> PUSH1 -> JUMP -> JUMPDEST
```

But control flow traversal does not account for the actual numeric values.  So every JUMP can lead to any JUMPDEST, and the trace of the traversal is as follows:
...
Appending another `PUSH1 -> JUMP -> JUMPDEST` to the sequence increases the size of the graph not just by the number of instructions, but also by the _product_ of the number of JUMPs and the number of JUMPDESTS.  So at the limit the size of this graph is quadratic in the number of instructions.

#### Dynamic jumps must die

For Ethereum, quadratic traversal times are a denial-of-service vulnerability for any online static analysis.
