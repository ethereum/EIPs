...
[EIP-8173](./eip-8173.md), *Foundations of EVM Control Flow*, lays out the background.  The short of it: a synthesized return is a jump whose destination is data, so tools must guess where it goes — and guesses multiply.  Explicit calls and returns leave nothing to guess.
...
Today these zkVMs run the EVM the only way they can — an EVM interpreter, compiled to RISC-V.  That is the first row of the table above.  Validated code offers the alternative the last row measures: compile the EVM code itself to RISC-V.
