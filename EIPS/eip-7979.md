...
Call and return facilities of various levels of complexity have proven their worth across a long line of important machines over the last 80 years:  Burroughs' baroque ALGOL support, Intel's simple `CALL` and `RET`, RISC-V's subtly dynamic `JAL` and `JALR`, ARM's bare mimimum `BL`, and on most every other machine we have programmed or implemented: physical machines including the, CDC 7600, IBM 360, PDP-11, VAX, Motorola 68000, and others, as well as virtual machines for Scheme, Forth, Pascal, Java, Wasm, .NET and others.  All of these facilities provide support for the basic mechanism -- aave the current machine state, transfer control to another state, and later, restore the saved state.
...
* "Analyzing contract binaries is vital ... Unfortunately, it is challenging to identify functions ... due to the lack of internal function call statements."[^7]
...
To prevent control-flow traversal from "going quadratic" we must prevent the dynamic use of jumps.  On-chain, most uses of `JUMP` and `JUMPI` are preceded by a `PUSH` — that is, they are effectively static.  This proposal _requires_ that in `valid` code `JUMP` and `JUMPI` are _always_ used statically.  Currently, the only places that jumps _must_ be used dynamically are to support calls and returns.  For that purpose we propose `CALLSUB` and `RETURNSUB` opcodes as replacements.  So not quite dead, but provably paralyzed.
...
The above is a purely semantic specification, placing no constraints on the syntax of bytecode beyond being an array of opcodes and immediate data.  "Subsidiary operations" here are not contiguous sequences of bytecode: they are subgraphs of the bytecode's full control-flow graph.  The EVM is a simple state machine, and the control-flow graph for a program simply represents every possible change of state.  Each instruction simply advances the state machine one more step.  We only promise that `valid` code will not, as it were, jam up the gears of the machine.
