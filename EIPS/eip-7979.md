...
* `CALLSUB` — jump to the `PC` on top of the stack
* `ENTERSUB` — marks a `CALLSUB` destination 
* `RETURNSUB` — return to the `PC` after the most recent `CALLSUB`

and restricts `valid` control flow to static uses of `JUMP`, `JUMPI`, and `CALLSUB`.
...
To prevent control-flow traversal from "going quadratic" we must prevent the dynamic use of jumps.  On-chain, most all uses of JUMP and JUMPI instructions in EVM code are preceded by a `PUSH` — that is, they are static.  This proposal ensures that in `valid` code `JUMP` and `JUMPI` are always used statically.  Currently, the only places that jumps must be used dynamically are to support calls to Turing's _subsidiary operations._  For that purpose we propose `CALLSUB` and `RETURNSUB` opcodes as replacements. 
...
* `is_imm_data(pc)` returns true iff `pc` is immediate data.
    ...
    def validate_code(code: bytes, pc: int, sp: int, bp: int, max: int) -> int, boolean:
        while pc < len(code):
            ...
            if opcode == JUMP:
                ...
                max_depth, valid = validate_code(pc, sp, bp)
                if !valid:
                    return 0, false
