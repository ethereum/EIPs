...
1. **Inefficiency**: Synthesizing calls and returns with jumps wastes bytecode space and gas.
2. **Analysis complexity**: Dynamic jumps can cause quadratic "path explosions" during control-flow analysis, creating a denial-of-service vulnerability for static analysis tools and preventing efficient compilation to machine code. They can also trigger exponential "state space explosions" during formal analysis and ZK work.

For detailed historical context, technical foundations of static control flow, and implications for Ethereum's scaling roadmap, see [EIP-XXXX: Static Control Flow in the EVM — Foundational Concepts and Context](./eip-xxxx).
...
This proposal aims to be the smallest possible change to the EVM. We introduce only three new instructions—`CALLSUB`, `ENTERSUB`, and `RETURNSUB`—sufficient to eliminate the need for dynamic jumps in synthesizing calls and returns.

We do _not_ include:

* **Immediate arguments** for jump destinations, which would improve performance but increase complexity. See [EIP-8013: Static relative jumps and calls for the EVM](./eip-8013) for a more comprehensive proposal offering improvements in cost, performance, and static analysis.

* **Code sections** or other structural constraints, which would increase bytecode size and impose syntactic restrictions. See [EIP-3540: EOF - EVM Object Format](./eip-3540) and [EIP-4750: EOF - Functions](./eip-4750) for complementary proposals that provide such structure.

By minimizing the scope, this proposal:
1. Reduces implementation complexity and deployment risk
2. Remains forward-compatible with larger proposals like EIP-8013 and EOF
3. Solves the immediate problem without anticipating future needs

### Why the return-stack mechanism?

Most stack machines have separate stacks for data and for returns from calls. The EVM is a stack machine, and we adopt the same proven approach: a separate return stack isolated from the data stack.

**Safety advantages of a separate return stack:**

* **Protection from stack manipulation**: Return addresses are not accessible to EVM code. They cannot be read, modified, or popped by ordinary stack operations. This eliminates an entire class of vulnerabilities where code could corrupt its own control flow.

* **Constant-time validation**: Because return addresses are controlled exclusively by `CALLSUB` and `RETURNSUB`, they are intrinsically safe to validate. Unlike data-stack values (which may depend on arbitrary computation), return-stack values are guaranteed to be valid `PC` values—we can validate all return addresses at compile time.

* **Simplicity**: A separate stack requires no extra opcodes or encoding schemes to distinguish return addresses from data. The mechanism is straightforward and requires no compromises.

* **Proven track record**: This mechanism has been used effectively in numerous machines over the last eight decades since Alan Turing introduced it (ACE, 1945). It has been implemented, tested, and deployed in countless systems, and has been ready to ship in multiple client implementations for the last nine years.

**Comparison to single-stack approaches:**

Register machines (x86, ARM, RISC-V) typically use a single stack for both data and return addresses, relying on registers and calling conventions to prevent confusion. Virtual machines with a data-only stack (like Java, Wasm, .NET) use a separate return stack. The EVM, being a stack machine without registers, benefits from the clarity and safety of the two-stack approach.

### Code size and gas savings

The difference these instructions make is significant even in simple code. Consider a routine that squares a number:

SQUARE (with JUMP): SQUARE (with CALLSUB): jumpdest ; 1 gas entersub ; 1 gas dup ; 3 gas dup ; 5 gas mul ; 5 gas mul ; 5 gas swap1 ; 3 gas returnsub ; 5 gas jump ; 8 gas

CALL_SQUARE: CALL_SQUARE: jumpdest ; 1 gas entersub ; 1 gas push RTN_CALL ; 3 gas push 2 ; 3 gas push 2 ; 3 gas push SQUARE ; 3 gas push SQUARE ; 3 gas callsub ; 8 gas jump ; 8 gas returnsub ; 5 gas RTN_CALL:
swap1 ; 3 gas
jump ; 8 gas

Size in bytes: 18 Size in bytes: 13 Consumed gas: 49 Consumed gas: 38


That's 32% fewer bytes and 30% less gas using `CALLSUB` versus `JUMP`. The distinct opcodes also make the code's intent clearer to both people and tools.

### Real-time performance

Some real-time interpreter performance gains come from the lower gas costs. But larger gains come from AOT and JIT compilers. Crucially, the constraint that stack depths be constant means that in `MAGIC` code, a very fast streaming JIT can traverse the control flow in one pass—as the code is executed—generating machine code on the fly. (The Wasm, JVM, and .NET VMs share this property.)

The EVM is a stack machine, but real machines are register machines. Generating virtual register code for a faster interpreter yields significant gains (4X speedups are possible on JVM code), and generating good machine code yields orders of magnitude improvements. However, for most transactions, storage dominates execution time, and gas counting and other overhead always take their toll. These gains would be most visible in contexts where this overhead is absent, such as L1 precompiles, some L2s, and EVM-compatible chains.

### ZK and scaling performance

Static control flow enables the construction of simpler, more efficient circuits for ZK verification. See [EIP-XXXX](./eip-xxxx) for details on how static control flow improves ZK-Rollup and optimistic rollup efficiency, and enables future migrations to other execution environments like RISC-V.
...
For a detailed explanation and examples of pathological programs, see [EIP-XXXX: Part II - Technical Foundations](./eip-xxxx).

The quadratic behavior is not merely a theoretical concern. For Ethereum, it represents a denial-of-service vulnerability for any online static analysis tool—including bytecode validation, translation to other representations, and AOT or JIT compilation at runtime.
...
_(Note: this implementation is likely incomplete and incorrect.)_

The following is a Python implementation of an algorithm for validating EVM bytecode. This algorithm uses a stack of continuations to traverse the code, emulating its control flow and stack use, and checking for violations of the rules above. It runs in time proportional to the size of the code, and the size of the list is proportional to the size of the code.  And equivalent validation MUST be run at `CREATE` time.
