...
### How can dynamic jumps exlode control flow analysis?

Recovering a program's control-flow is a fundamental first step for many analyses.  When all jumps are static, the number of analysis steps is linear in the number of instructions: a fixed number of paths must be explored for each jump.  With dynamic jumps, every possible destination must be explored at every jump.  Intuitively, the number of possible paths through code just explodes.  At worst, the number of paths can go up as the square of number of instructions and symbolic execution of the paths can go up exponentially.  See [EIP-8173](./eip-8173.md) for a more detailed explanation and example exploit.

This behavior is not merely a theoretical concern.  For Ethereum, it represents a denial-of-service vulnerability for many tools — including bytecode validation and AOT or JIT compilation at runtime.
...
The algorithm is a single unified CFG traversal starting from PC 0.  Only reachable instructions are visited and checked — unreachable bytes, whether hidden data or dead code, are never examined.  The traversal carries stack state through a worklist of continuations, one per reachable control-flow edge, and uses a `visited` table to detect join points and enforce consistent stack heights across all paths.
...
def validate(code: Bytes[MAX_CODE_SIZE]) -> bool:
    ...
    # first visits pc.  After the traversal, any entry that is still set
    # but whose pc was never visited means the destination was unreachable
    # (e.g. inside a PUSH immediate) — a failure.
    ...
    for _step: uint256 in range(MAX_WORKLIST):
        ...
        # Helper: read destination from the immediately preceding PUSH.
        # pp is the PC of that PUSH, ps is its total size (1 + imm bytes).
        # Returns the destination value and whether a valid PUSH preceded.
        have_push: bool    = ps > 0 and pp + ps == pc
        dest: uint256      = 0
        if have_push:
            imm_sz: uint256 = ps - 1  # number of immediate bytes
        ...
        if op == OPCODE_ENTERSUB:
            ...
            worklist[wl_tail] = Continuation({
                ...
                prev_push_pc:   0,
                prev_push_size: 0,        # ENTERSUB is not a PUSH
            })
            wl_tail += 1
            ...
            if dest >= code_len:
                return False
            # Record or check destination requirement.
            if visited_data[dest] != UNVISITED:
                # Already visited: verify it was an ENTERSUB.
                if required_at[dest] != REQUIRE_NONE and required_at[dest] != REQUIRE_ENTERSUB:
                    return False
                # The join-point check when dest was first visited already
                # confirmed the opcode there; nothing more to do here.
            else:
                if required_at[dest] != REQUIRE_NONE and required_at[dest] != REQUIRE_ENTERSUB:
                    return False
                required_at[dest] = REQUIRE_ENTERSUB
            # Increment return-stack depth.
            new_rh: int256 = rh + 1
            ...
            worklist[wl_tail] = Continuation({
                ...
                entersub_height: eh,
                prev_push_pc:   pc,
                prev_push_size: 0,        # JUMPI is not a PUSH
            })
            wl_tail += 1
    ...
    # After traversal: any required_at entry that was set but whose PC
    # was never visited means a jump into unreachable bytes — a failure.
    for _i: uint256 in range(MAX_CODE_SIZE):
        if _i >= code_len:
            break
        if required_at[_i] != REQUIRE_NONE and visited_data[_i] == UNVISITED:
            return False
