...
Rather than enforce semantic constraints via syntax, (that is a job for higher-level languages) this proposal enforces them via a symbolic execution that traces the code's flow of control, checking constraints as it goes.  A reference implementation is given below — it takes, as it must, time and space linear in the size of the code.  Other algorithms are of course allowable, so long as they enforce the same constraints and use time and space linear in the size of code.
...
Why choose this mechanism?  There really is no other choice — return addresses get stacked somewhere.  In general, machines either use a dedicated return stack like ACE, Forth, Java, Wasm, and many others; or they have a general-purpose stack and special-purpose registers to support subroutines.  The EVM is not a register machine.
...
We illustrate here how these simple instructions can be used to modesly reduce the complexity and the gas costs of calls and returns compared to using `JUMP`.
...
These changes are backwards compatible — the semantics of EVM code _is not_ affected by whether the contract begins with `MAGIC` and excution of code _does not_ require maintaining two interpreters.
