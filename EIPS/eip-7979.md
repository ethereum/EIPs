...
The Yellow Paper defines the machine state μ, including the data stack μ_s —
whose depth we write |μ_s| — and the program counter μ_pc.  This EIP adds the
return stack μ_r, pushed only by `CALLSUB`, popped only by `RETURNSUB`, and
not otherwise accessible to EVM code.

An instruction is a byte of code that is not `PUSH` immediate data.  A path
is a possible sequence of executed instructions, starting at μ_pc = 0;
...
* **`stack offset`**: |μ_s|, minus |μ_s| at the most recent `ENTERSUB` in
  the current *frame* — or at the start of the *frame*, if there is none.
...
4. On every path: each instruction MUST find at least as many items on the
   ...
   `CALLSUB` MUST NOT leave more than 1024 items on the return stack; each
   `RETURNSUB` MUST find at least one.
...
_The following bytecodes exercise the validator itself: the last column is
...
small vectors; the algorithm is independent of the limit._
...
The contract is embedded below so that this EIP reads as a single document.  It is also provided as a separate file, together with the test suite `test_validator.py`, which runs all of the validation test cases above, and the one-pass control-flow-graph extractor `extract_cfg.py`.
...
number of items the region uses from its caller's stack, below its baseline
— and its *stack growth* — the greatest offset it reaches above it
(Constraint 4).
...
* `RETURNSUB` must be *framed*: a matching `CALLSUB` on every path, since
  ...
  Resolving an *entry* releases its pending return points and resolves
  parents along *fall edges*, in cascade.
...
After the traversal, the per-entry summaries propagate from callees to
parents over the recorded edges: *inputs*(parent) ≥ *inputs*(callee) −
offset at the edge; *stack growth*(parent) ≥ offset +
*stack growth*(callee); *call depth*(parent) ≥ *call depth*(callee), plus
one *frame* per call.  The entry graph is sorted topologically.  If it is
acyclic — the program is not recursive — one pass in topological order
computes the summaries exactly, and the outermost region's *stack growth*
...
in code size.  The summary propagation is a topological pass, one
relaxation per edge; under recursion the *inputs* relaxation re-visits an
...
# ---------------------------------------------------------------------------
# Internal helpers
# (opcode_info, the opcode table, is defined at the end of this file)
# ---------------------------------------------------------------------------
...
def validate(code: Bytes[MAX_CODE_SIZE]) -> bool:
    ...
    # Call and fall edges, a linked list per callee entry.  The inputs,
    # growth, and call_depth summaries flow from callee to parent over
    # every edge; net effects flow only over fall edges.  List indices
    ...
    # -----------------------------------------------------------------
    # Propagate the per-entry summaries from callees to parents.
    #
    # The entry graph is sorted topologically (Kahn).  If it is acyclic
...
def opcode_info(op: uint8) -> (uint256, int256, int256, bool):
    ...
    # DUP1..DUP16 (0x80..0x8F): DUPn pops n and pushes n + 1
    if op >= 128 and op <= 143:
        dup_n: int256 = convert(op, int256) - 127
        return (1, dup_n, dup_n + 1, False)
    # SWAP1..SWAP16 (0x90..0x9F): SWAPn pops and pushes n + 1
    if op >= 144 and op <= 159:
        swap_n: int256 = convert(op, int256) - 143
        return (1, swap_n + 1, swap_n + 1, False)
    # LOG0..LOG4 (0xA0..0xA4): LOGn pops n + 2
    if op >= 160 and op <= 164:
        log_n: int256 = convert(op, int256) - 160
        return (1, log_n + 2, 0, False)
    # STOP (0x00): terminator
    if op == 0:
        return (1, 0, 0, True)
    # Binary operations, pop 2 push 1:
    # ADD MUL SUB DIV SDIV MOD SMOD (0x01..0x07), EXP SIGNEXTEND (0x0A..0x0B)
    if (op >= 1 and op <= 7) or op == 10 or op == 11:
        return (1, 2, 1, False)
    # LT GT SLT SGT EQ (0x10..0x14), AND OR XOR (0x16..0x18),
    # BYTE SHL SHR SAR (0x1A..0x1D), KECCAK256 (0x20)
    if (op >= 16 and op <= 20) or (op >= 22 and op <= 24) or (op >= 26 and op <= 29) or op == 32:
        return (1, 2, 1, False)
    # ADDMOD MULMOD (0x08..0x09): pop 3 push 1
    if op == 8 or op == 9:
        return (1, 3, 1, False)
    # Unary operations, pop 1 push 1: ISZERO (0x15), NOT (0x19),
    # BALANCE (0x31), CALLDATALOAD (0x35), EXTCODESIZE (0x3B),
    # EXTCODEHASH (0x3F), BLOCKHASH (0x40), BLOBHASH (0x49),
    # MLOAD (0x51), SLOAD (0x54), TLOAD (0x5C)
    if op == 21 or op == 25 or op == 49 or op == 53 or op == 59 or op == 63 or op == 64 or op == 73 or op == 81 or op == 84 or op == 92:
        return (1, 1, 1, False)
    # Nullary, push 1: ADDRESS (0x30), ORIGIN CALLER CALLVALUE (0x32..0x34),
    # CALLDATASIZE (0x36), CODESIZE (0x38), GASPRICE (0x3A),
    # RETURNDATASIZE (0x3D), COINBASE TIMESTAMP NUMBER PREVRANDAO GASLIMIT
    # CHAINID SELFBALANCE BASEFEE (0x41..0x48), BLOBBASEFEE (0x4A),
    # PC MSIZE GAS (0x58..0x5A)
    if op == 48 or (op >= 50 and op <= 52) or op == 54 or op == 56 or op == 58 or op == 61 or (op >= 65 and op <= 72) or op == 74 or (op >= 88 and op <= 90):
        return (1, 0, 1, False)
    # Copies, pop 3: CALLDATACOPY (0x37), CODECOPY (0x39),
    # RETURNDATACOPY (0x3E), MCOPY (0x5E)
    if op == 55 or op == 57 or op == 62 or op == 94:
        return (1, 3, 0, False)
    # EXTCODECOPY (0x3C): pops 4
    if op == 60:
        return (1, 4, 0, False)
    # POP (0x50)
    if op == 80:
        return (1, 1, 0, False)
    # Stores, pop 2: MSTORE MSTORE8 (0x52..0x53), SSTORE (0x55), TSTORE (0x5D)
    if op == 82 or op == 83 or op == 85 or op == 93:
        return (1, 2, 0, False)
    # JUMP (0x56): pops its destination; terminator
    if op == OPCODE_JUMP:
        return (1, 1, 0, True)
    # JUMPI (0x57): pops destination and condition; falls through
    if op == OPCODE_JUMPI:
        return (1, 2, 0, False)
    # JUMPDEST (0x5B)
    if op == OPCODE_JUMPDEST:
        return (1, 0, 0, False)
    # CALLSUB: pops its destination; terminator (this EIP)
    if op == OPCODE_CALLSUB:
        return (1, 1, 0, True)
    # ENTERSUB: a label (this EIP)
    if op == OPCODE_ENTERSUB:
        return (1, 0, 0, False)
    # RETURNSUB: terminator (this EIP)
    if op == OPCODE_RETURNSUB:
        return (1, 0, 0, True)
    # CREATE (0xF0): pops 3 pushes 1; CREATE2 (0xF5): pops 4 pushes 1
    if op == 240:
        return (1, 3, 1, False)
    if op == 245:
        return (1, 4, 1, False)
    # CALL CALLCODE (0xF1..0xF2): pop 7 push 1
    if op == 241 or op == 242:
        return (1, 7, 1, False)
    # DELEGATECALL (0xF4), STATICCALL (0xFA): pop 6 push 1
    if op == 244 or op == 250:
        return (1, 6, 1, False)
    # RETURN (0xF3), REVERT (0xFD): pop 2; terminators
    if op == 243 or op == 253:
        return (1, 2, 0, True)
    # INVALID (0xFE): a valid opcode per Constraint 1; terminator
    if op == 254:
        return (1, 0, 0, True)
    # SELFDESTRUCT (0xFF): pops 1; terminator
    if op == 255:
        return (1, 1, 0, True)
    # Unknown or deprecated opcode: Constraint 1 violation.
    return (0, 0, 0, False)
```
...
The canonical validation contract is consensus-critical, and correcting a faulty validator is hard: invalid contracts it admitted are already on the blockchain and cannot be removed.  A mechanism to retroactively invalidate them may be needed; like the header layout, it is deferred.
