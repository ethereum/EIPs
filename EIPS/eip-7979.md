...
* **`baseline`**: |μ_s| at the start of the current *region* on a given
  path — at its *entry*'s `ENTERSUB`, or at the start of execution in the
  outermost region.  Baselines vary from call to call; *stack offsets*,
  measured from them, do not.

* **`stack offset`**: |μ_s|, minus the current *baseline*.
