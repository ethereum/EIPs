...
To create an exploit keep appending `-> PUSH -> JUMP -> JUMPDEST` to the sequence.  This increases the length of the trace not just by a constant three steps but also by the quadratically increasing _product_ of the number of JUMPs and the number of JUMPDESTS.  At the limit the length of the trace is quadratic in the number of instructions.  And there are worse exploits.
