...
When a destination has not yet been reached, its required type is recorded and checked when it is.  Any recorded requirement that is never reached — because the destination is in unreachable bytes — is a failure.  Unreachable bytes are never examined.
