...
Why choose this mechanism?  There really is no other choice — return addresses have to get stacked somewhere.  In general, stack machines (include ACE, Forth, Java, Wasm, and EOF) use a dedicated return stack, whereas register machines have a general-purpose stack and special-purpose registers to support subroutines.  The EVM is not a register machine.
