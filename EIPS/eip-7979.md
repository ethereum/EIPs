...
Jumps and conditional jumps were supported by the hardware of the Analytical Engine.  Calls and returns required manual shuffling of punch card decks. In 1945 Turing further proposed using a stack of memory crystals holding return addresses as a further means of organizing the logic of the code for his Automatic Computing Engine:
...
* Starting at the first instruction, traverse instruction by instruction.
...
The algorithm is "depth-first" in that it recurses into the destinations of jumps and calls before proceeding.  In its bare form above the algorithm is searching for cycles, but it can be extended to do much more.  As a validation algorithm it can track stack height and search for underflows and overflows.  As a JIT code generator it can serialize virtual stack code directly into virtual register code.  And so on for any other static analysis that requires the control flow graph.
...
To prevent control-flow traversal from "going quadratic" we must prevent the dynamic use of jumps.  On-chain, most all uses of `JUMP` and `JUMPI` are preceded by a `PUSH` -- that is, they are effectively static.  This proposal ensures that in `valid` code `JUMP` and `JUMPI` are always used statically.  Currently, the only places that jumps _must_ be used dynamically are to support calls and returns.  For that purpose we propose `CALLSUB` and `RETURNSUB` opcodes as replacements. 
...
Again, this would break the promise of "the smallest possible change," and [EIP-3540: EOF - EVM Object Format](./eip-3540) remains available. In the presence of EOF code sections `CALLSUB` could be restricted to section boundaries.
