...
- **MUST** be used in combination with an [onchain Identity system](../assets/eip-3643/ONCHAINID)
...
This standard is backwards compatible with ERC-20, therefore, all ERC-20 functions can be called on an ERC-3643 token, the interfaces being compatible.
But the functions are not implemented in the same way as a classic ERC-20 as [ERC-3643](./eip-3643.md) is a permissioned token, which implies a check to be performed on each single token transfer to validate the compliance of the transfer and the eligibility of the stakeholder’s identities.
...
wallet address is in the `Identity Registry` of the token, and if the `ONCHAINID`contract linked to his wallet 
...
Token contract(implementation of the IERC3643 interface), but the standard allows all flexibility in terms of roles in the implementations.
A detailed description of the functions can be found in the [interfaces folder](../assets/eip-3643/interfaces).
