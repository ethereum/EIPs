...
Additionally, this standard could be used as a primitive for Soulbound tokens, whereby the recovery of a Soul would be done by obtaining signatures from co-owners of EIP-1238 tokens with the same id.

## Specification
Every EIP-1238 compliant contract must implement the EIP-1238 interface:
...
In addition, in order for a contract to be compliant with EIP-1238, it MUST also abide by the following:
...
The **URI Storage extension** is OPTIONAL for EIP-1238 smart contracts. This allows your contract to associate a unique URI for each token id.
...
The **Expirable extension** is OPTIONAL for EIP-1238 smart contracts. This allows your contract to associate a unique URI for each token id.
...
The **Holdable extension** is OPTIONAL for EIP-1238 smart contracts. This enables EIP-1238 token owners to let their tokens be held by another address or to stake them in a smart contract.
...
EIP-1155 presents several interesting features that are also applicable to EIP-1238 tokens:
...
However a batch often times only concerns one address (as seen here in the reference implementation of [EIP-1155](https://github.com/enjin/erc-1155/blob/678092281094b7cd5328b417c677be05338ea0d6/contracts/ERC1155.sol#L102)). While minting a batch of tokens to an address in one transaction is convenient, we felt the need to support minting to multiple addresses in one transaction. 
...
Regarding privacy, even though this standard can be used for identity claims, issuers should refrain from using EIP-1238 tokens to store any sensitive or personally identifiable information (given and family name, date of birth etc...) on-chain as this action cannot be undone. Issuers should warn users about the immutable and public nature of receiving a token, what it represents and must obtain the informed consent from recipients before minting a badge to them.
