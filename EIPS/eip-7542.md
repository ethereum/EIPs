...
With the implementation of [EIP-4444](./eip-4444.md), where nodes might choose not to hold the whole historical data of the chain, other nodes might be interested in specific histroic block ranges, such as when syncing full archive node. Currently, nodes need to connect to peers and request specific blocks to determine if a peer has the required data. This can be inefficient, leading to unnecessary data requests and wasting both bandwidth and time.
