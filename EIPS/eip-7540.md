...
Asynchronous deposit Vault MUST return the constant value `true` if `0xea446681` is passed through the `interfaceID` argument.

Asynchronous redemption Vault MUST return the constant value `true` if `0x2e9dd5bd` is passed through the `interfaceID` argument.
