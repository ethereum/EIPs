...
class ExecutionChunkHeader(Container):
    ...
    chunk_access_list_hash: Root     # keccak256(RLP(ChunkAccessList))
...
Chunks and CALs propagate as separate network sidecars, identified by beacon block root and chunk index. Sidecars may arrive in any order and before the beacon block itself.

```python
class ExecutionChunkSidecar(Container):
    beacon_block_root: Root             # Beacon block this chunk belongs to
    chunk: ExecutionChunk               # The execution chunk
    builder_signature: BLSSignature     # Builder signature over (beacon_block_root, chunk_header)

class ChunkAccessListSidecar(Container):
    beacon_block_root: Root             # Beacon block this CAL belongs to
    ...
    builder_signature: BLSSignature     # Builder signature over (beacon_block_root, chunk_index, cal_hash)
```

Signature domain and signing procedures follow the conventions established in [EIP-7732](./eip-7732.md).
...
1. Verify `builder_signature` against the builder public key from the beacon block
...
3. Verify `keccak256(RLP(chunk_access_list))` matches the committed hash in the chunk header

Nodes MUST subscribe to all chunk and CAL subnets to ensure block availability. Sidecars received before the beacon block are stored pending and validated upon block arrival.
...
def on_chunk_access_list_received(store: Store, cal_sidecar: ChunkAccessListSidecar) -> None:
    root = cal_sidecar.beacon_block_root
    chunk_index = cal_sidecar.chunk_index

    # Wait for beacon block (may already be available)
    block = wait_for_beacon_block(store, root)

    # Validate sidecar (signature, index bounds, hash commitment)
...
def on_chunk_received(store: Store, chunk_sidecar: ExecutionChunkSidecar) -> None:
    root = chunk_sidecar.beacon_block_root
    ...
    # Validate sidecar (signature, gas bounds, transaction root)
...
def maybe_finalize_block(store: Store, root: Root) -> None:
    ...
    # Compute total gas from validated chunks
    total_gas_used = 0
    for chunk_index in sorted(chunk_statuses.keys()):
        # Verify sequential chunk coverage
        if chunk_index not in chunk_statuses:
            return  # Gap in chunk sequence

        status = chunk_statuses[chunk_index]
        if not status.valid:
            # Any invalid chunk invalidates the block
            store.block_valid[root] = False
            return

        total_gas_used += chunk_headers[chunk_index].gas_used

    # Verify all chunks received (total gas must match payload header)
    if total_gas_used < payload_header.gas_used:
        return  # Missing chunks

    # Final verification: chunk chaining and aggregate state root
    # EL verifies: chunk headers chain correctly, final state root matches payload header
...
def el_execute_chunk(
    ...
    result = execute_transactions(
        ...
        pre_state,
        chunk.transactions,
        cached_cals[chunk_index]  # Expected state diffs for validation
    )
...
Finalizes block validation after all chunks have been executed. Verifies chunk header chaining and that the final state root matches the payload header.
...
| **CAL Withholding** | Builder publishes chunks but withholds CALs | Streaming execution blocked; falls back to sequential | CALs reconstructible by sequential execution of available chunks |
| **Reverse Propagation** | Builder sends chunks/CALs in reverse order (N→0) | No parallel execution until chunk 0 and CAL 0 arrive | Timeliness requirements; rational builders propagate in order for faster attestations |
