...
The first CAL contains pre-execution access list and the last CAL contains post-execution access list.

#### Network Sidecars

Chunks and CALs propagate as separate sidecars. They can arrive in any order, including before the beacon block.
...
3. **Backward Propagation**: Proposers/Builders could send chunks and CALs in the reverse order
   ...
   - **Mitigation**: Attestation rules requires chunks and CALs to be propagated or validated in the timely manner

4. **Orphan Chunk Handling**: Chunks arriving before beacon blocks
