...
2. When `SELFDESTRUCT` is executed in the same transaction as the contract was created 
   ...
   - `SELFDESTRUCT` continues to behave as previously specified, this includes the following actions
     - The current execution frame halts.
...
Removing pre-existing "hollow" accounts at addresses that are smart contract addresses awaiting creation and that only held a balance prior to contract creation provides a better user experience than leaving an account with a non-zero nonce, possible storage, and no code to access that storage.
