...
This EIP adds many math ckecks to EVM arithmetic. The list includes underflows, overflows, division by zero. A new opcode is added to get the flags and clear them.
...
Two new flags are added to the EVM state: unsigned error (`uerr`) and signed error (`serr`). The scope of those flags are the same as the program counter. Each frame of execution has their own flags. At the frame cration they are unset and they are inaltered by any kind of call.
...
The conditions described for each opcode is made with implementation friendlyness in mind. The only exception is EXP as it is hard to give a concise test as most of the others relyied on the inverse operation and there is no native `LOG`. Most `EXP` implementations will internaly use `MUL` so the flag can be drawn from that instruction.

The flag being carryed to the stack is to make it possible to give control to the code implementation on how it will be utilized. It may perform an exceptional halting or any other behavior of its choosing.
