...
author: Danny Ryan (@djrtwo), Dankrad Feist (@dankrad), Francesco D'Amato (@fradamt), Hsiao-Wei Wang (@hwwhww), Alexey Osipov (@flcl42)
...
Additionally we propose adding a configurable limit to the number of blobs that can be included in a single transaction. This setting will be incorporated into the `blobSchedule` section of the client configuration.
...
`blobSchedule` is required to be extended with an additional optional field
