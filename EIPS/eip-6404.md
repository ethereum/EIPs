...
def validate_execution_signature(
    ...
    verify_signature(sig_hash, signature)
...
def calculate_transaction_intrinsic_gas(tx: Transaction) -> uint:
    ...
    if hasattr(tx_data, "authorization_list"):
        for auth in tx_data.authorization_list:
            gas_cost += get_signature_gas_cost(auth.signature, compute_auth_hash(auth), expected_algorithm=expected_signature_algorithm)

    gas_cost += get_signature_gas_cost(tx.signature, compute_sig_hash(tx), expected_algorithm=expected_signature_algorithm)
