...
class IndexedReceipt(Container):
...
To compute the new `receipts_root`, the list of individual `IndexedReceipt` containers is represented as a SSZ `List`.

```python
payload.receipts_root = hash_tree_root(List[IndexedReceipt, MAX_TRANSACTIONS_PER_PAYLOAD](
    indexed_receipt_0,
    indexed_receipt_1,
    indexed_receipt_2,
    ...
