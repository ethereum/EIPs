...
category: Core
created: 2023-01-30
requires: 155, 1559, 2718, 2930, 4844, 7495, 7702
---
...
```python
class Transaction(Union[RlpTransaction]):
    pass

class RlpTransaction(Union[
...
```python
class RlpAuthorization(Union[RlpReplayableAuthorizationPayload, RlpBasicAuthorizationPayload]):
    pass

class RlpReplayableAuthorizationPayload(ProgressiveContainer[active_fields=[1, 0, 1, 1]]):
...
RLP and SSZ transactions may clash when encoded. It is essential to use a single format within one channel.
