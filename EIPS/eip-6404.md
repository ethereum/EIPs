...
def ecdsa_recover_tx_from(signature: TransactionSignature, sig_hash: Hash32) -> ExecutionAddress:
    public_key = ECDSA.ecdsa_recoverable_deserialize(sig_hash, signature[64])
    return ExecutionAddress(keccak(public_key)[12:32])
```
...
| Transaction type | Native | Baseline | SSZ Union | Normalized | Base + Snappy | Union + Snappy | Norm + Snappy |
| - | :-: | :-: | :-: | :-: | :-: | :-: | :-: |
| `LegacyTransaction` | RLP | 106 bytes | 210 bytes | 272 bytes | 109 bytes | 138 bytes | 195 bytes |
| [EIP-155](./eip-155.md) transaction | RLP | 108 bytes | 210 bytes | 272 bytes | 111 bytes | 139 bytes | 194 bytes |
| [EIP-2930](./eip-2930.md) transaction | RLP | 111 bytes | 215 bytes | 272 bytes | 114 bytes | 145 bytes | 194 bytes |
| [EIP-1559](./eip-1559.md) transaction | RLP | 117 bytes | 247 bytes | 272 bytes | 117 bytes | 148 bytes | 194 bytes |
| [EIP-4844](./eip-4844.md) transaction | SSZ | 315 bytes | 315 bytes | 340 bytes | 186 bytes | 186 bytes | 234 bytes |
...
SSZ generally encodes less compact than RLP. The normalized `Transaction` is larger than the SSZ Union due to the extra `tx_hash` and `tx_from` commitments, as well as the inclusion of `max_priority_fee_per_gas` for pre-[EIP-1559](./eip-1559.md) transactions.

The [EIP-4844](./eip-4844.md) transaction's SSZ Union representation differs only in the first byte, where it encodes the SSZ Union selector (`0x03`) vs the [EIP-2718](./eip-2718.md) transaction type (`0x05`).
...
3. **Sender:** Proof that a specific sender sent a certain minimum amount to a specific destination
...
| Amount | 874 bytes | 874 bytes | 834 bytes | 874 bytes | 874 bytes | 853 bytes |
| Sender | 938 bytes (\*\*) | 938 bytes (\*\*) | 867 bytes (\*\*) | 907 bytes (\*\*) | 907 bytes | 853 bytes |
| Info | 946 bytes (\*\*) | 946 bytes (\*\*) | 915 bytes (\*\*) | 1,003 bytes (\*\*) | 979 bytes (\*\*\*) | 957 bytes |
...
The normalized `Transaction` provides a consistent experience across both SSZ and non-SSZ transaction types, allows continued use of existing `tx_hash` infrastructure, and neither requires secp256k1 public key recovery to obtain `tx_from` nor RLP encoding and keccak hashing to obtain the address of newly deployed contracts. Furthermore, the scheme allows the `transactions_root` commitment and future transaction type schemas to change independently of each other, including changes to the hashing method.
