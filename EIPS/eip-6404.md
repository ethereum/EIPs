...
| Sender | SHA256, secp256k1 (\*) | SHA256 |
| Info | SHA256, secp256k1 (\*), RLP (\*\*), keccak256 (\*\*) | SHA256 |
...
- (\*) `tx_from` is recovered from `signature` and `sig_hash` using secp256k1 public key recovery
- (\*\*) `tx_to` for transactions that deploy a new smart contract is computed from `tx_from` and `nonce`
