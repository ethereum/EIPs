...
Non-unique addresses and storage keys are not disallowed, though they will be charged for multiple times, and aside from the higher gas cost there is no other difference in execution flow or outcome from multiple-inclusion of a value as opposed to the recommended single-inclusion.

> [!TIP]
> It should be noted that `tx sender`, `tx recipient`, `block producer`, created contracts and precompiles are considered warm and should *not* be included in access lists.
...
```
That is, it gives you the list of addresses and storage keys that will be used by that transaction, plus the gas consumed if the access list is included.
