...
- From any other caller with calldata of exactly the contract's input size: the write path. The predeploy MUST validate the request and the value sent (see the request sections below), append one record to its queue, increment the per-block count, and emit the accepted request as an anonymous log (`LOG0`, no topics) — exactly as the EIP-7002/7251 contracts log theirs.
- From any other caller with empty calldata: the fee getter. The predeploy MUST return the current fee without modifying state, and MUST revert if any value is attached (preventing accidentally lost funds, as in EIP-7002/7251).
...
| `80:88` | `amount` — big-endian `uint64`, in gwei (the big-endian input convention of [EIP-7002](./eip-7002.md)) |
| `88:184` | `signature` — 96-byte BLS proof-of-possession |

A deposit request serves both a builder's first deposit and subsequent top-ups. The contract MUST reject the request unless all of the following hold:

1. The calldata is exactly 184 bytes (the fixed-size layout subsumes the field-length checks).
2. `amount * 1 gwei >= BUILDER_MIN_DEPOSIT`.
3. `msg.value >= fee`, where `fee` is the current request fee, **and** `msg.value - fee >= amount * 1 gwei` — the value beyond the fee fully funds the stake. (Checking the fee and the stake in sequence keeps the arithmetic overflow-free.) Any value beyond `amount * 1 gwei + fee` is retained by the contract and not credited to the builder.

On success it MUST append the 184 input bytes to its queue and emit them as an anonymous log. The dequeued `BUILDER_DEPOSIT_REQUEST_TYPE` record is `pubkey (48) ++ withdrawal_credentials (32) ++ amount (8, little-endian) ++ signature (96)` — the input verbatim, with the amount converted from its big-endian calldata encoding to the little-endian SSZ encoding, the same conversion [EIP-7002](./eip-7002.md) applies to its amount (the log, emitted on the write path, carries the big-endian input form). The `signature` is carried in the record and verified by the consensus layer, which checks the proof-of-possession only on the `pubkey`'s first appearance and treats a later deposit to an existing builder as a stake top-up (see [Consensus-layer processing of records](#consensus-layer-processing-of-records)).
...
A type's `request_data` is the concatenation of the fixed-size SSZ serializations of its records — 184 bytes per `BuilderDepositRequest` (`pubkey ++ withdrawal_credentials ++ amount ++ signature`) and 68 bytes per `BuilderExitRequest` (`source_address ++ pubkey`), with `amount` little-endian — in the FIFO order the system call returns them. This matches the bytes the system call returns, byte for byte. `BuilderDepositRequest` is the validator [EIP-6110](./eip-6110.md) `DepositRequest` without the `index` field; the consensus layer verifies its `signature` (the proof-of-possession) on the builder's first registration.
...
Both predeploys are written in `geas` (the assembler the deployed EIP-7002/7251 contracts are written in) and are line-level derivatives of those contracts' source in `ethereum/sys-asm`: [`deposits/main.eas`](../assets/eip-8282/src/deposits/main.eas) and [`exits/main.eas`](../assets/eip-8282/src/exits/main.eas), each with the standard inhibitor-setting constructor ([`deposits/ctor.eas`](../assets/eip-8282/src/deposits/ctor.eas), [`exits/ctor.eas`](../assets/eip-8282/src/exits/ctor.eas)) and sharing the EIP-7002 [`fake_expo.eas`](../assets/eip-8282/src/common/fake_expo.eas) fee routine verbatim. The header of each `main.eas` enumerates its full diff against the EIP-7002/7251 source. The parent source is not just an approximation of what is on mainnet: `sys-asm`'s withdrawals and consolidations contracts assemble byte-identically to the runtime code deployed at the EIP-7002 and EIP-7251 predeploy addresses (504 and 414 bytes respectively). The assembled runtime bytecode here is 566 bytes for the deposit contract and 396 bytes for the exit contract — the same order of size, and far within the [EIP-170](./eip-170.md) 24 KiB limit. The Foundry test harness and configuration are published alongside the sources.
