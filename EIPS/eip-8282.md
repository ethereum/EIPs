...
Both contracts are deployed by a `CREATE2` factory, rather than by the presigned single-use-deployer transaction (Nick's method; see [ERC-2470](./eip-2470.md)) used for the [EIP-7002](./eip-7002.md) and [EIP-7251](./eip-7251.md) predeploys. Each address is determined by the factory, a salt, and the contract's init code. The salt was mined so that the addresses above result from the current reference bytecode.
...
The reference contracts live in the `ethereum/sys-asm` repository, under `src/builder_deposits` and `src/builder_exits` as of commit `3bba94c696bc46beeecef05db6c2df0f18b4b239`.
