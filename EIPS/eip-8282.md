...
Both contracts are deployed the same way as the [EIP-7002](./eip-7002.md) and [EIP-7251](./eip-7251.md) predeploys, with a one-time presigned transaction from a single-use deployer account (Nick's method). The addresses above are derived from the presigned deployment transactions for the current reference bytecode. Each contract's init code sets its `excess` slot to `EXCESS_INHIBITOR`.
