...
The inhibition mechanism allows an old contract to finish processing queued requests while a replacement accepts new ones. During the transition, the old contract receives system calls with non-empty calldata. Each call drains up to the per-block limit and sets the inhibitor, keeping new user requests disabled. These calls continue until the old queue is empty. An empty system call would re-enable user requests on the old contract.

A future upgrade must specify how the replacement contract is activated and how system calls and returned requests from both contracts are handled.
