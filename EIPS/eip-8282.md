...
| `EXCESS_INHIBITOR` | `2**256-1` | Set at deployment and cleared by the first system call |
...
Both contracts are deployed the same way as the [EIP-7002](./eip-7002.md) and [EIP-7251](./eip-7251.md) predeploys, with a one-time presigned transaction from a single-use deployer account (Nick's method). The final addresses are derived from those transactions and will be fixed once the runtime bytecode is frozen. Each contract's init code sets its `excess` slot to `EXCESS_INHIBITOR`.
...
Both predeploys follow the [EIP-7002](./eip-7002.md) and [EIP-7251](./eip-7251.md) contract design and reuse their storage layout. There is no ABI. Each contract dispatches on the caller and on `calldatasize` alone. Calls that match none of the cases below MUST revert.
...
At the end of each block, the contract is called by `SYSTEM_ADDRESS` with empty calldata. It MUST dequeue up to its per-block maximum of records (`MAX_DEPOSIT_REQUESTS_PER_BLOCK` or `MAX_EXIT_REQUESTS_PER_BLOCK`, oldest first), return their concatenation as its `request_data`, update `excess` to `max(0, excess + count - target)` (treating `EXCESS_INHIBITOR` as `0`), and reset the per-block count. Records beyond the cap remain queued for subsequent blocks.
...
Because `excess` starts at `EXCESS_INHIBITOR` and the fee getter reverts while it holds that value, no request can be submitted until the first system call at the activation block.
