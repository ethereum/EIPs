...
* `FALCON_HASH_TO_POINT_SHAKE256` — NIST-conformant Hash-to-Point using SHAKE256.
* `FALCON_HASH_TO_POINT_KECCAKPRNG` — EVM-friendly Hash-to-Point using a Keccak-based XOF/PRNG.
* `FALCON_CORE` — Core verification given a challenge polynomial.

The canonical names above MUST be used in this specification. Any prior synonyms are non-normative.
