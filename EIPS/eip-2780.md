...
- `TX_BASE_COST = 12,000` — ECDSA recovery, the sender account's access and write, and the inclusion of the transaction's bytes in the block (block space, which is transient and expires with history).
...
The primitives are split across two phases. **Intrinsic gas** is the state-independent cost of including the transaction in a block and is the sole input to the transaction-validity check. **Runtime gas** covers the state-dependent charges — the recipient cold surcharge, new-account creation, and delegation resolution — charged as the first frame is entered, exactly as a `CALL` frame is metered. Running out of gas in runtime does not invalidate the transaction it is considered as halt. Transaction is included, execution is skipped, and all state changes are reverted, as for any out-of-gas halt inside a frame.

Transactions that grow the state — value transfers that create a new account and contract-creation transactions — additionally pay `STATE_BYTES_PER_NEW_ACCOUNT × CPSB` in state gas, per [EIP-8037](./eip-8037.md). These transactions therefore cost more than `21,000`, internalizing the durable state growth the flat base left unpriced — durable, unlike block bytes, because state does not expire.

[EIP-7702](./eip-7702.md) authorizations are decomposed the same way: a fixed `REGULAR_PER_AUTH_BASE_COST` per authorization — which includes the authority's warm access — is charged as intrinsic gas, while the state-dependent charges — the cold-authority access surcharge, new-account creation, the delegation-indicator bytes, and the account write — are deferred to runtime and billed only for the authorities that actually consume them. This replaces EIP-7702's worst-case `PER_EMPTY_ACCOUNT_COST` intrinsic charge and its later refund.
...
The flat cost also conflates two kinds of charge. Part of it — signature recovery, the sender's access and write, the transaction's bytes in the block — is knowable from the transaction alone, without reading state, and is exactly what is needed to decide whether the transaction can be included. The rest — touching the recipient, creating an account, resolving a delegation — depends on state and cannot be priced without reading it.

This EIP separates the two so that intrinsic gas becomes a simple, state-independent quantity computed from the transaction's fields alone. **Intrinsic gas** represents exactly the gas required to include the transaction in a block — signature recovery, the sender's access and write, the transaction's block bytes, the value charges, the warm-access floor of the recipient touch, and the per-authorization base — and remains the sole input to the transaction-validity check. Everything that depends on state is deferred to **runtime gas**, charged as the first frame is entered and metered exactly like the charges inside a `CALL` frame: if the transaction supplies enough gas to be included but not enough to cover a runtime charge, the frame halts out-of-gas — the transaction stays valid and included, the sender pays for the gas consumed, and all state changes are reverted, just as for an out-of-gas anywhere else in execution. This cleans up the pre-execution accounting and sets a clear expectation for it: validity rests on a quantity a builder can compute without touching state, and future state-dependent charges can be priced at runtime without ever moving the validity boundary.
...
| `REGULAR_PER_AUTH_BASE_COST` | 5,657 | [EIP-8037](./eip-8037.md) | Per-authorization base: calldata, ECDSA recovery (per [EIP-7904](./eip-7904.md)), warm authority access, and warm writes |
...
The two state-gas products change whenever a fork changes `CPSB`. Regular-gas and state-gas semantics are defined by [EIP-8037](./eip-8037.md).

### Intrinsic gas costs

The intrinsic gas is the state-independent cost of including the transaction in a block, computed without any state access, and is the sole input to the transaction-validity gas check. It is the sum of:

- `TX_BASE_COST` in regular gas — one ECDSA recovery, the sender's account access and write, and the inclusion of the transaction's bytes in the block — charged for every transaction;
- if `tx.to` is not `None` and the transaction is not a self-transfer (`tx.sender != tx.to`), `WARM_ACCESS` in regular gas — the state-independent floor of the recipient touch; the cold surcharge, if any, is charged at runtime;
- if `tx.to` is `None` (contract-creation transaction), `CREATE_ACCESS` in regular gas and `STATE_BYTES_PER_NEW_ACCOUNT × CPSB` in state gas;
- value charges, from `tx.value` and whether the transaction is a self-transfer (`tx.sender == tx.to`):
  - if `tx.value == 0`, or the transaction is a self-transfer, no charge;
  - if `tx.value > 0` and contract creation, `TRANSFER_LOG_COST` in regular gas;
  - otherwise, `TRANSFER_LOG_COST + TX_VALUE_COST` in regular gas;
- `REGULAR_PER_AUTH_BASE_COST` in regular gas per [EIP-7702](./eip-7702.md) authorization;
- calldata and access-list costs, unchanged.

Only the warm floor of the recipient touch is intrinsic; the cold surcharge (`COLD_ACCOUNT_ACCESS − WARM_ACCESS`) is state-dependent and is charged at the runtime phase below. These charges replace the legacy contract-creation transaction charges and EIP-7702's flat `PER_EMPTY_ACCOUNT_COST × authorization_list_length` intrinsic charge. Calldata and access-list metering remain unchanged.
...
- if the authority address is cold — not already in `accessed_addresses`, per [EIP-2929](./eip-2929.md) — charge the cold surcharge `COLD_ACCOUNT_ACCESS − WARM_ACCESS` in regular gas;
- if the authority did not exist, charge `STATE_BYTES_PER_NEW_ACCOUNT × CPSB` in state gas and `ACCOUNT_WRITE` in regular gas for the new account leaf;
...
`REGULAR_PER_AUTH_BASE_COST` prices the authority access at the warm rate; the cold surcharge above is the only account-access charge added at runtime for the authority.

Then the recipient account is loaded, and unless the transaction is a self-transfer or a contract creation:

- if the recipient is cold — not warmed by an applied authorization or an access list, per [EIP-2929](./eip-2929.md) — charge the cold surcharge `COLD_ACCOUNT_ACCESS − WARM_ACCESS` in regular gas; the warm floor is already covered by the intrinsic `WARM_ACCESS`;
...
Running out of runtime gas does **not** invalidate the transaction. Validity is decided solely by the intrinsic gas check, which needs no state access; the runtime charges, which do, can never retroactively invalidate an already-validated transaction. On out-of-gas the transaction is included, halts, the sender pays for all gas consumed, and all state changes — including any [EIP-7702](./eip-7702.md) delegations applied by its authorizations — are reverted, exactly as for an out-of-gas inside any call frame.
...
Costs are split between the intrinsic phase and the runtime phase, each in regular gas and state gas, per [EIP-8037](./eip-8037.md). "execution" denotes contract execution charged at the standard schedule; it, and every runtime charge, is drawn from the gas passed into the first frame, so that frame is entered with the gas remaining after the runtime regular and state charges. The recipient's warm floor `WARM_ACCESS` is intrinsic; its cold surcharge `COLD_ACCOUNT_ACCESS − WARM_ACCESS` and any delegation-target access are runtime. Cases show the cold rate; a recipient warmed beforehand pays no runtime surcharge, and a delegation target warmed beforehand pays `WARM_ACCESS` rather than `COLD_ACCOUNT_ACCESS`. The intrinsic + runtime totals match the legacy headline numbers.

| Case | Intrinsic (regular / state) | Runtime (regular / state) | Total (regular / state) |
| :---- | :---- | :---- | :----: |
| ETH transfer to self | `TX_BASE_COST` / 0 | 0 / 0 | 12,000 / 0 |
| No-transfer to EOA / empty account | `TX_BASE_COST + WARM_ACCESS` / 0 | `COLD_ACCOUNT_ACCESS − WARM_ACCESS` / 0 | 15,000 / 0 |
| No-transfer to contract | `TX_BASE_COST + WARM_ACCESS` / 0 | `COLD_ACCOUNT_ACCESS − WARM_ACCESS` + execution / 0 | 15,000 + execution / 0 |
| ETH transfer to existing EOA | `TX_BASE_COST + WARM_ACCESS + TX_VALUE_COST + TRANSFER_LOG_COST` / 0 | `COLD_ACCOUNT_ACCESS − WARM_ACCESS` / 0 | 21,000 / 0 |
| ETH transfer to contract | `TX_BASE_COST + WARM_ACCESS + TX_VALUE_COST + TRANSFER_LOG_COST` / 0 | `COLD_ACCOUNT_ACCESS − WARM_ACCESS` + execution / 0 | 21,000 + execution / 0 |
| No-transfer to 7702 delegated | `TX_BASE_COST + WARM_ACCESS` / 0 | `2 × COLD_ACCOUNT_ACCESS − WARM_ACCESS` + execution / 0 | 18,000 + execution / 0 |
| ETH transfer to 7702 delegated | `TX_BASE_COST + WARM_ACCESS + TX_VALUE_COST + TRANSFER_LOG_COST` / 0 | `2 × COLD_ACCOUNT_ACCESS − WARM_ACCESS` + execution / 0 | 24,000 + execution / 0 |
| ETH transfer creating new account | `TX_BASE_COST + WARM_ACCESS + TX_VALUE_COST + TRANSFER_LOG_COST` / 0 | `COLD_ACCOUNT_ACCESS − WARM_ACCESS` / `STATE_BYTES_PER_NEW_ACCOUNT × CPSB` | 21,000 / 183,600 |
| Create transaction, `tx.value` = 0 | `TX_BASE_COST + CREATE_ACCESS` / `STATE_BYTES_PER_NEW_ACCOUNT × CPSB` | 0 / 0 | 23,000 / 183,600 |
| Create transaction, `tx.value` > 0 | `TX_BASE_COST + CREATE_ACCESS + TRANSFER_LOG_COST` / `STATE_BYTES_PER_NEW_ACCOUNT × CPSB` | 0 / 0 | 24,756 / 183,600 |

### Interactions with other EIPs

- **[EIP-2929](./eip-2929.md) (warm/cold accounting).** The sender access folded into `TX_BASE_COST` is always charged at the intrinsic phase, as is the recipient's `WARM_ACCESS` floor. The recipient cold surcharge (`COLD_ACCOUNT_ACCESS − WARM_ACCESS`), the authority cold surcharge, and the delegation-target access are charged at the runtime phase and follow the standard EIP-2929 warm/cold model: a recipient or authority already warmed — by an access list or by an [EIP-7702](./eip-7702.md) authorization applied earlier in the same transaction — incurs no cold surcharge, and a warm delegation target pays `WARM_ACCESS` rather than `COLD_ACCOUNT_ACCESS`. The same model governs all execution-level account touches, including internal `CALL`s.
...
The split follows a single line: can the charge be computed without reading state? Signature recovery, the sender's access and write, the transaction's block bytes, the value charges, the recipient's warm-access floor, and the per-authorization base (including its warm authority access) are all fixed by the transaction's fields alone, so they are intrinsic and decide validity. The state-touching remainder is runtime, billed as each account is actually loaded: the cold surcharge on the recipient or an authority, whether the recipient or an authority exists, and whether a delegation must be resolved.
...
[EIP-7702](./eip-7702.md) charges `PER_EMPTY_ACCOUNT_COST` per authorization at the intrinsic phase and refunds it when the authority already exists, over-reserving gas the transaction may never use. Splitting the cost removes the over-charge: only `REGULAR_PER_AUTH_BASE_COST` — the calldata, signature recovery, warm authority access, and warm writes every authorization performs — is knowable without state access and stays intrinsic. The cold surcharge for a cold authority, the new-account leaf and its `ACCOUNT_WRITE`, and the delegation-indicator bytes depend on state and are charged at runtime during authorization processing, billed only for the authorities that incur them.
...
2. Zero-value transaction to any address (existing, empty, or contract): `12,100` intrinsic (`TX_BASE_COST + WARM_ACCESS`) + `COLD_ACCOUNT_ACCESS − WARM_ACCESS` runtime = `15,000` regular, `0` state. A self-target is `12,000` with no runtime charge.
3. Value transfer to an existing EOA: `18,100` intrinsic + `COLD_ACCOUNT_ACCESS − WARM_ACCESS` runtime = `21,000` regular, `0` state.
4. Value transfer creating a new account (recipient non-existent): `18,100` intrinsic + `COLD_ACCOUNT_ACCESS − WARM_ACCESS` runtime = `21,000` regular; `STATE_BYTES_PER_NEW_ACCOUNT × CPSB` = `183,600` runtime state.
5. Value transfer to a 7702-delegated account: `18,100` intrinsic + `2 × COLD_ACCOUNT_ACCESS − WARM_ACCESS` runtime = `24,000` regular plus execution.
6. Contract-creation transaction, `value = 0`: `23,000` intrinsic regular, `183,600` intrinsic state, no runtime charge.
7. Contract-creation transaction, `value > 0`: `24,756` intrinsic regular, `183,600` intrinsic state.
8. EIP-7702 transaction: `TX_BASE_COST` is unchanged even when the sender assumes code. Each authorization is charged `REGULAR_PER_AUTH_BASE_COST` (which already covers the warm authority access) at the intrinsic phase; at runtime, a cold authority additionally pays the cold surcharge `COLD_ACCOUNT_ACCESS − WARM_ACCESS` regular, a non-existent authority additionally pays `STATE_BYTES_PER_NEW_ACCOUNT × CPSB` state and `ACCOUNT_WRITE` regular, an existing warm authority pays neither, and net-new delegation bytes add `STATE_BYTES_PER_AUTH_BASE × CPSB` state — replacing the flat `PER_EMPTY_ACCOUNT_COST` intrinsic charge.
...
10. Transaction whose gas covers intrinsic cost but not a runtime charge — e.g. a zero-value call funded for its intrinsic gas (`TX_BASE_COST + WARM_ACCESS`) but not the recipient cold surcharge (`COLD_ACCOUNT_ACCESS − WARM_ACCESS`), a value transfer to a non-existent recipient lacking gas for the new-account state charge, or a transfer to a delegated account lacking gas for the delegation-target access: the transaction is valid and included, halts out-of-gas, and all runtime state changes — including any applied EIP-7702 delegations — are reverted; it is **not** invalidated.
