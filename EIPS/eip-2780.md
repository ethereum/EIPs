...
Reducing ETH transfer cost from 21,000 => 6,000 gas (+250 % throughput for payments) increases the frequency and granularity of settlement.
At unchanged basefee, the minimum economical payment size on L1 falls by roughly 70 %, allowing more, smaller transactions to clear directly in ETH.

That raises the **velocity of money** - balances turn over more often, and value circulates natively instead of being batched or routed through intermediaries.

This improves ETH's **moneyness**:

- **Medium of exchange:** lower friction makes direct ETH payments habitual rather than occasional.
- **Unit of account:** regular use for settlement encourages pricing and accounting in ETH terms.
- **Store of value:** explicit, workload-based gas costs build confidence in the system's fee realism.

Price volatility may still lead users to prefer stablecoins for budgeting and short-term certainty, but removing artificial friction restores ETH's competitiveness where volatility tolerance already exists.

The `25,000` surcharge on creating new accounts prices state growth honestly while keeping routine transfers cheaper and faster, promoting native ETH circulation without subsidising state expansion.
