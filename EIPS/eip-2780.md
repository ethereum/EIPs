...
ETH underpins most onchain liquidity. Pairs are denominated in it, and gas is paid in it; a de-facto tax base that anchors value. Bitcoin’s monetary role now runs mainly through exchange pairs, which is why it drifted toward store-of-value. ETH still clears real activity directly onchain, yet outside NFTs few people think in ETH terms. In that world ETH already *is* money; prices are quoted in ETH and settlement is native, but only for large tickets.

For ETH to function as money across all scales, small payments must be viable. That wasn’t possible when gas sat at 300 gwei, or even 90 gwei on quiet weekends. Post-4844, blobs absorb L2 data and higher gaslimits ease congestion; network capacity is no longer the barrier. The 21 k floor for a plain transfer remains the main friction. Users can bypass it by batching, which overcharges direct use and underprices complexity.

This proposal corrects that imbalance. Reducing `TX_BASE_COST` to reflect the actual work path — signature recovery, one sender leaf write, one cold touch — and repricing `CALL` value costs removes the penalty on simple payments. ETH transfers then cost what they consume, not what history dictates. The same mechanism still charges full state-growth cost when a new account is created, keeping the economics sound.

- **Velocity.** High fixed costs slow circulation; lowering them increases the cadence of native settlement. This proposal removes the artificial floor that forced payments into batches or off-chain IOUs, raising onchain transaction velocity without changing calldata or storage pricing.
- **Neutral pricing.** Gas now tracks real computation. The network stops cross-subsidising complex calls at the expense of simple transfers.
- **Native denomination.** When small transfers are natural and cheap, ETH becomes the unit people think, spend, pay and trade in again. This proposal makes that habit economically rational.
- **Monetary competition.** Stablecoins and LSTs are welcome, but they can compete for onchain *moneyness*. By restoring low-cost native payments, this proposal keeps ETH central - not plumbing beneath other monies, but the settlement layer they clear to.
