...
- **[EIP-7708](./eip-7708.md) (ETH transfers emit a log).** EIP-7708 emits a LOG3 transfer event for every nonzero-value ETH transfer to a different account. Under the legacy `21,000` base this cost was absorbed; with a reduced base it must be charged explicitly. This EIP introduces `TRANSFER_LOG_COST = 1,756` (LOG3 equivalent: `375 + 3×375 + 32×8`) applied to top-level value transfers (including top-level `CREATE` transactions with `value > 0`), value-moving `CALL`s, value-bearing `CREATE` / `CREATE2`, and nonzero-value `SELFDESTRUCT` when it transfers value to a different account. For `SELFDESTRUCT`, the charge is applied at the point that transfer is processed; for `CREATE` / `CREATE2`, at the point that the endowment is transferred.
