...
ETH already anchors most liquidity. LPs pair against it, fees are paid in it - a kind of economic tax base that sets a floor under its value. Bitcoin's "money" case now mostly runs through BTC exchange pairings, which is why it drifted toward store-of-value rather than medium of exchange.

ETH still clears real economic activity on-chain, but few people think in ETH terms outside NFTs. In that niche, ETH already *is* money - prices are quoted directly, trade settles natively - but the goods are expensive, so it only works for large tickets.

For ETH to act as money, it has to work for small ones too. That was impossible when gas sat at 300 gwei, or even 90 gwei on a quiet weekend. Post-4844, with blobs absorbing data pressure from "pay at any price" L2s and higher gaslimits keeping congestion in check, low-value payments become plausible again.

The 21 k minimum for a plain transfer is the leftover friction. Users can dodge it by bundling calls, which means the network overcharges straighforward use and underprices complexity. Dropping the base cost and fixing `CALL` value pricing closes that gap - not to make ETH grander, just usable as money across scales.

Stablecoins and LSTs are welcome, but they're competitors for "moneyness". If all flows and LPs settle in LSTs or USD{T|C}, ETH becomes plumbing, not principle. The goal here is simple: make native ETH cheap enough to spend and pay that people think in it again.
