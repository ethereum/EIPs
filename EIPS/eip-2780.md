...
- `CREATE` transactions do not pay `GAS_NEW_ACCOUNT`; their `GAS_CREATE = 32,000` opcode base already includes account creation. Top-level `CREATE` transactions with `value > 0` do pay `TRANSFER_LOG_COST` at intrinsic level (see *Pseudocode*), since they have no `GAS_CREATE` charge to absorb the EIP-7708 transfer log.
...
The `GAS_NEW_ACCOUNT` surcharge does not apply to `CREATE`/`CREATE2`: their existing `GAS_CREATE = 32,000` opcode base already prices account creation. Top-level `CREATE` transactions with `value > 0` do pay `TRANSFER_LOG_COST` for the [EIP-7708](./eip-7708.md) transfer log, since they have no `GAS_CREATE` charge to absorb it. Access-list pricing is unchanged.
