...
category: Core
created: 2020-07-11
requires: 1559, 2718, 2929, 2930, 6780, 7708
---
...
| `BURN_LOG_COST`            |  1,381 | [EIP-7708](./eip-7708.md) LOG2 burn event (375 + 2×375 + 32×8)         |
...
- `CREATE` transactions do not pay `GAS_NEW_ACCOUNT`; the `GAS_CREATE = 32,000` opcode base for the `CREATE` / `CREATE2` opcodes already prices account creation. Top-level `CREATE` transactions are not charged `GAS_CREATE` (it is opcode-only).
- Both top-level `CREATE` transactions with `value > 0` (see *Pseudocode*) and in-EVM `CREATE` / `CREATE2` with non-zero endowment (see *EVM gas schedule adjustments*) pay `TRANSFER_LOG_COST` for the [EIP-7708](./eip-7708.md) transfer log emitted on the endowment transfer.
...
def CalculateIntrinsicGas(tx, state_at_start):
    ...
    # log applies whenever value > 0. (Top-level CREATE transactions are
    # not charged `GAS_CREATE`, which is opcode-only.)
...
- **Value-bearing `CREATE` / `CREATE2`.** When the endowment is nonzero, charge `TRANSFER_LOG_COST = 1,756` for the [EIP-7708](./eip-7708.md) transfer log emitted on the endowment transfer to the newly created account. The charge is applied at the point that the endowment transfer is processed, in addition to the existing `GAS_CREATE = 32,000` opcode base, init-code costs, and (for `CREATE2`) the salt-hashing cost. The newly created account is, by construction, distinct from the caller, so the self-transfer carve-out does not apply. `GAS_CREATE` already prices account creation, so `GAS_NEW_ACCOUNT` is **not** charged on top.

- **Burn logs.** [EIP-7708](./eip-7708.md) emits a LOG2 burn event in two cases: (a) `SELFDESTRUCT` to self by a contract created in the same transaction with non-zero balance, and (b) any non-zero-balance account marked for deletion at transaction finalization. Per [EIP-6780](./eip-6780.md), an account is only marked for deletion when SELFDESTRUCTed by a contract created in the same transaction; cases (a) and (b) can therefore both be priced via a single charge at flagging time:

  - Charge `BURN_LOG_COST = 1,381` once at any `SELFDESTRUCT` whose originator was created in the same transaction. This single charge covers both case (a) (immediate burn log when `beneficiary == originator` and `originator_balance > 0`) and case (b) (finalization-time burn log if the flagged account has non-zero balance at finalization), since at most one of (a) or (b) emits per `SELFDESTRUCT`.
  - The charge is applied unconditionally at flagging time so all gas is paid before transaction finalization, which runs after EVM gas accounting closes. If no burn log actually emits (e.g. balance is zero at `SELFDESTRUCT`, or was transferred out), the charge is not refunded; the over-charge is bounded by `BURN_LOG_COST` per `SELFDESTRUCT`.
...
| `CREATE` tx, `value = 0`            | `TX_BASE_COST` + **execution** (init-code, `GAS_CREATE` inside EVM)                      | 4,500 + **execution** |
| `CREATE` tx, `value > 0`            | `TX_BASE_COST` + `TRANSFER_LOG_COST` + **execution** (init-code, `GAS_CREATE` inside EVM) | 6,256 + **execution** |
...
- **[EIP-7708](./eip-7708.md) (ETH transfers emit a log).** EIP-7708 emits a LOG3-shaped transfer event on every non-zero-value ETH transfer to a different account, and a LOG2-shaped burn event when ETH leaves circulation. Under the legacy `21,000` intrinsic base, the LOG3 cost of a transfer event was implicitly absorbed; with `TX_BASE_COST` reduced to `4,500`, that subsidy disappears and each emission must be priced explicitly. This EIP introduces:
  ...
  - `TRANSFER_LOG_COST = 1,756` (LOG3 equivalent: `375 + 3×375 + 32×8`), applied to: top-level value-transferring transactions (including `CREATE` transactions with `value > 0`); value-moving `CALL`s with `caller != to`; non-zero-value `SELFDESTRUCT` with `beneficiary != originator`; and value-bearing `CREATE` / `CREATE2`.
  - `BURN_LOG_COST = 1,381` (LOG2 equivalent: `375 + 2×375 + 32×8`), charged once at any `SELFDESTRUCT` whose originator was created in the same transaction. This single charge covers both the immediate self-burn case and the finalization-time burn case (see *EVM gas schedule adjustments*).

  Top-level transaction transfers are charged at intrinsic gas computation (see *Pseudocode*); in-EVM `TRANSFER_LOG_COST` is charged at the point the corresponding value transfer is processed; `BURN_LOG_COST` is charged at the `SELFDESTRUCT` that flags the originator for deletion, regardless of whether the burn log emits immediately or at finalization.
...
The `GAS_NEW_ACCOUNT` surcharge does not apply to `CREATE`/`CREATE2`: the `GAS_CREATE = 32,000` opcode base already prices account creation, and top-level `CREATE` transactions are not charged `GAS_CREATE` at all. Both endowed in-EVM `CREATE` / `CREATE2` and top-level `CREATE` transactions with `value > 0` do pay `TRANSFER_LOG_COST` for the [EIP-7708](./eip-7708.md) transfer log. For in-EVM `CREATE` / `CREATE2`, the LOG3 cost was historically absorbed by the over-priced `GAS_CREATE` constant; this EIP unbundles them and prices the log explicitly, treating `GAS_CREATE` as a charge for state growth alone. Access-list pricing is unchanged.
