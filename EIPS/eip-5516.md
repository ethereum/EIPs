...
### Multi token
The multi token functionality was used in order to ease organization, all emitted tokens are stored in the same contract, preventing redundant bytecode from being deployed to the blockchain. It also facilitates transfer to token issuers, given that all issued tokens are stored and can be accessed under the same address. It is also easy to implement non-fungible and semi-fungible tokens in the same contract.
