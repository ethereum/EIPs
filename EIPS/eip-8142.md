...
**Note on `USABLE_BYTES_PER_FIELD_ELEMENT`**: Field elements are interpreted big-endian. This EIP sets the first byte (MSB) to zero to ensure that every 32-byte element is a canonical BLS field element (`value < BLS_MODULUS`).
...
def blobs_to_execution_payload_data(blobs: List[Blob]) -> ExecutionPayloadData:
    ...
    # Rest must be padding, no trailing data allowed
