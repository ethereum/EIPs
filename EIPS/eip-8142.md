...
def execution_payload_data_to_blobs(data: ExecutionPayloadData) -> List[Blob]:
    ...
    Encoding steps:
      1. payload_bytes = RLP.encode(data.transactions)
      2. return bytes_to_blobs(payload_bytes)

    Note: Transactions in the ExecutionPayload are already RLP-encoded, so this
    RLP-encodes the list of already-encoded transaction bytes.
    """
    payload_bytes = RLP.encode(data.transactions)
...
def blobs_to_execution_payload_data(blobs: List[Blob]) -> ExecutionPayloadData:
    ...
    Decoding steps:
      ...
      2. transactions = RLP.decode(payload_bytes)
      3. return ExecutionPayloadData(transactions)
    """
    payload_bytes = blobs_to_bytes(blobs)
    transactions = RLP.decode(payload_bytes)
    return ExecutionPayloadData(transactions)
```
...
See [What is included in execution-payload data?](#what-is-included-in-execution-payload-data) in the Rationale for why only transactions are included.
...
```python
fn get_payload(payload_id: PayloadId) -> GetPayloadResponse:
    ...
    # 4. Compute blob commitments and versioned hashes
    ...
    # 5. Extract type-3 transaction blob hashes
    ...
    all_blobs = payload_blobs + extract_type3_blobs(payload.transactions)
    ...
    return GetPayloadResponse(
        ...
        blobs_bundle=BlobsBundle(
            ...
            blobs=all_blobs,
            proofs=compute_blob_kzg_proofs(all_blobs, all_commitments)
        ),
...
For the zkEVM-optimized variant, the builder must additionally compute KZG proofs for the payload blobs, which will be used as private inputs in the zkEVM circuit:

```python
fn get_payload_zk(payload_id: PayloadId) -> GetPayloadResponse:
    # 1. Build the block (select transactions, etc.)
    payload = build_execution_payload(payload_id)
    ...
    # 4. Compute KZG commitments and proofs for payload blobs
    payload_commitments = [blob_to_kzg_commitment(b) for b in payload_blobs]
    payload_proofs = [compute_blob_kzg_proof(b, c) for b, c in zip(payload_blobs, payload_commitments)]
    payload_versioned_hashes = [kzg_commitment_to_versioned_hash(c) for c in payload_commitments]

    # 5. Extract type-3 transaction blobs
    type3_blobs = extract_type3_blobs(payload.transactions)
    type3_commitments = [blob_to_kzg_commitment(b) for b in type3_blobs]
    type3_proofs = [compute_blob_kzg_proof(b, c) for b, c in zip(type3_blobs, type3_commitments)]
    type3_versioned_hashes = []
    for tx in payload.transactions:
        if tx.type == BLOB_TX_TYPE:
            type3_versioned_hashes.extend(tx.blob_versioned_hashes)
    ...
    all_proofs = payload_proofs + type3_proofs
    ...
    return GetPayloadResponse(
        ...
        block_value=calculate_block_value(payload),
        # Additional metadata for zkEVM prover
        payload_blob_metadata=PayloadBlobMetadata(
            count=payload_blob_count,
            commitments=payload_commitments,
            proofs=payload_proofs
        )
    )
```

The `payload_blob_metadata` provides the zkEVM prover with the commitments and proofs needed to verify payload blob consistency without recomputing expensive MSMs in-circuit. See the zkEVM variant of `engine_newPayload` for how they are used.
...
**Note:** Once zkEVM proofs are required for consensus, `newPayload` will be executed inside a zkEVM to generate a proof, rather than being executed natively by validators. This variant is designed to be cheaper that context.
...
Execution-payload data includes only `transactions`.

Under the current protocol, transaction data is the only component of the execution payload that is not required for consensus-layer and cannot be derived from other components:

- **Withdrawals** can be derived on the consensus layer
- **Execution requests** are the output of executing transactions and can be recomputed
