...
**Row:** In [EIP-7594](./eip-7594.md), each blob (extended with its erasure code) forms a row of the block's data matrix. The row index of a blob corresponds to its position in the ordered list of blob KZG commitments referenced by the block. 

**Payload row:** a row whose index is less than `payload_blob_count`, i.e. a row that carries payload-blob data rather than type-3 transaction blob data.
...
BiB reuses the existing blob networking mechanism and does not introduce a new gossipsub topic or message type.
...
#### Payload propagation via data columns

**Phase 1 (before mandatory proofs):** Every node subscribes to *all* column subnets, not just its custody columns:

- For **custody columns**, nodes request the cells for every row, exactly as under standard EIP-7594 custody.
- For **non-custody columns**, nodes request only cells belonging to **payload rows**, leaving cells for type-3 transaction blob rows unrequested.

This is done via [gossipsub v1.3 partial messages](https://github.com/libp2p/specs/blob/master/pubsub/gossipsub/partial-messages.md) by advertising a bitmap of requested cells for each column. This ensures that payload data is propagated to all nodes, while minimizing bandwidth usage for non-custody columns.

![](../assets/eip-8142/phase1.png)

**Phase 2 (once zkEVM proofs are mandatory):** Validators that only need to attest via zk-proof no longer need the execution payload and revert to the standard custody-only column subscription. This is a local parameter change, not a protocol upgrade. Nodes that still need the full payload — builders, RPC providers, indexers, and supernodes — continue subscribing to all columns and requesting payload rows as in Phase 1 or alternatively switch to an improved version of the `execution_payload` gossip topic (see [Open questions and future considerations](#open-questions-and-future-considerations)).
...
- The need for a dedicated gossip topic for execution payloads once zkEVM proofs are mandatory for consensus:
  – Future gossip designs could leverage message segmentation and erasure coding without requiring subscription to all columns. For the nodes that require the full payload, this could become more efficient than subscribing to all columns
...
Two alternatives to propagating execution payloads via BiB were considered:

- **A dedicated `execution_payload` gossip topic** ([EIP-7732](./eip-7732.md)): the current gossip implementation propagates the full payload as a single message and does not benefit from the erasure coding naturally provided by PeerDAS
- **Erasure-coded broadcast (EC broadcast)**: a proposed ethp2p mechanism that erasure codes and propagates arbitrarily large messages without relying on the PeerDAS mechanism. This is not yet a standardized protocol, and it would require downloading the full execution payload regardless of a node's custody configuration, even after zkEVM proofs become mandatory.
