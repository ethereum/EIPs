...
def blobs_to_execution_payload_data(blobs: List[Blob]) -> ExecutionPayloadData:
    ...
    Decoding steps:
      ...
      2. payload_bytes = blobs_to_bytes(blobs)
      3. transactions = RLP.decode(payload_bytes)
      4. return ExecutionPayloadData(transactions)
    """
    # Special case: empty payload (zero blobs)
    if len(blobs) == 0:
        return ExecutionPayloadData(transactions=[])

    # Normal case: decode from blobs
...
- **Execution requests and BALs** are the output of executing transactions and can be recomputed
