...
### Why propagate payloads via PeerDAS columns instead of a dedicated gossip topic or EC broadcast?

Payload-blobs are not pre-propagated through the transaction mempool before block building (unlike type-3 blob transactions), so their propagation latency matters more. 
