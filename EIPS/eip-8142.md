...
author: Kevaundray Wedderburn (@kevaundray), Ignacio Hagopian (@jsign), Jihoon Song <jihoon.song@ethereum.org>, Francesco Risitano (@frisitano), Thomas Thiery (@soispoke) <thomas.thiery@ethereum.org>, Toni Wahrstätter (@nerolation), Péter Garamvölgyi (@thegaram), Kamil Salakhiev (@kamilsa)
discussions-to: https://ethereum-magicians.org/t/eip-8142-block-in-blobs-bib/27621
status: Draft
type: Standards Track
category: Core
created: 2026-01-29
requires: 4844, 7594, 7732, 7892, 7928, 8136
---
...
BiB builds on the existing blob networking mechanism.

We note the following for considerations:

- Because the existing DAS guarantee ensures that payload data is always recoverable from blob columns, BiB makes a separate `execution_payload` gossip topic ([EIP-7732](./eip-7732.md)) for payload retrieval redundant.
- Nodes that only need to verify zkEVM proofs for consensus do not need to download the execution payload, and therefore need only subscribe to the custody columns (as in [EIP-7594](./eip-7594.md)).
...
The exact networking implementation depends on whether zkEVM proofs are mandatory for consensus. The following describes the two phases of BiB rollout.

**Phase 1 (before mandatory proofs):** Data columns and payload columns are propagated separately:

- Nodes keep subscribing to `data_column_sidecar_{subnet_id}` only for their custody column subnets, and request every cell in those custody columns exactly as under standard EIP-7594 custody.
- Nodes subscribe to all `payload_column_sidecar_{subnet_id}` subnets and request the payload cells needed to reconstruct the execution payload.
- The block builder commits to the payload columns with a Merkle root, which is included into the execution bid ([EIP-7732](./eip-7732.md)). This done to avoid introducing a delay when builder encodes the payload. Individual payload column sidecars carry Merkle multi-proofs rather than KZG proofs.

![](../assets/eip-8142/phase1.png)

**Phase 2 (once zkEVM proofs are mandatory):** Payload column subnets and data column subnets are merged together by their subnet ID. Validators only subscribe to subnets that correspond to their custody set. Nodes that still need the full payload — builders, RPC providers, indexers, and supernodes — subscribe to all column subnets. They can use partial messages mechanism to request only the payload rows if they are not interested in downloading the full data for non-custody columns.
<!-- TODO -->
TODO: The exact commitment scheme for blobs in Phase 2 is TBD. It may be a Merkle root, KZG commitment, or some other scheme.
...
> Note that the following discussion only corresponds to phase 2 of BiB rollout, when zkEVM proofs are mandatory for consensus. For phase 1, no changes to fee accounting are required, as the execution payload is still verified via re-execution and payload does not compete with transaction blobs for capacity.
...
- The need for a dedicated gossip topic for execution payloads once zkEVM proofs are mandatory for consensus:
  – Future gossip designs could leverage message segmentation and erasure coding without requiring subscription to all column subnets. For the nodes that require the full payload, this could become more efficient than subscribing to all column subnets
...
### Why propagate payloads via PeerDAS columns?

BiB propagates the execution payload over dedicated payload columns in Phase 1, which are merged back into data columns in Phase 2. Two alternatives to propagating execution payloads via BiB were considered:

- **An existing dedicated `execution_payload` gossip topic** ([EIP-7732](./eip-7732.md)): the current gossip implementation propagates the full payload as a single message and does not benefit from the erasure coding naturally provided by PeerDAS
- **Erasure-coded broadcast (EC broadcast)**: a proposed ethp2p mechanism that erasure codes and propagates arbitrarily large messages without relying on the PeerDAS mechanism. While close by many semantics to the PeerDAS mechanism, this is not yet a standardized protocol, and it would require downloading the full execution payload regardless of a node's custody configuration, even after zkEVM proofs become mandatory.
