...
This EIP defines the `ssz_rest` communication channel for the Engine API, building on the protocol discovery mechanism introduced by EIP-8177. Each JSON-RPC Engine API method is mapped to a REST endpoint using SSZ-encoded request and response bodies with `application/octet-stream` content type.
