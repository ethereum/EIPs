...
discussions-to: https://ethereum-magicians.org/t/discussion-topic-for-eip-8178-update-log-2026-2-3/27891
status: Draft
...
Exchange the list of supported Engine API endpoints between CL and EL. Capability names use the format `"METHOD /path"` (e.g., `"POST /engine/v4/payloads"`).
...
**Response:** `200 OK` — `ExchangeCapabilitiesResponse`
...
### Why negotiation through `engine_exchangeCapabilities`?

The CL already calls `engine_exchangeCapabilities` on startup to discover what the EL supports. Including SSZ REST endpoints in the capabilities list is a natural extension — no new method needed, no extra configuration step. If the EL doesn't advertise SSZ endpoints, the CL keeps using JSON-RPC. No flag day, no forced migration.
...
The CL discovers SSZ REST availability through `engine_exchangeCapabilities` and can fall back to JSON-RPC at any time.
