...
category: ERC
created: 2023-02-23
requires: 155, 165, 721, 1167, 1271, 5313
---

## Abstract

This proposal defines a system which gives every non-fungible token a smart contract account. These token bound accounts allow NFTs to own assets and interact with applications, without requiring changes to existing smart contracts or infrastructure.

## Motivation

The [ERC-721](./eip-721.md) standard enabled an explosion of non-fungible token applications. Some notable use cases have included breedable cats, generative artwork, and liquidity positions.

NFTs are increasingly becoming a form of on-chain identity. This follows quite naturally from the ERC-721 specification - each NFT has a globally unique identifier, and by extension, a unique identity.

Unlike other forms of on-chain identity, NFTs cannot act as agents or associate with other on-chain assets. This limitation stands in contrast with many real-world instances of non-fungible assets. For example:
...
This proposal grants every NFT the full capabilities of an Ethereum account. It does so by deploying unique, deterministically-addressed smart contract accounts for each NFT via a permissionless registry. Importantly, this proposal does not require any smart contract changes to the NFT smart contracts themselves. As a result, this proposal is backwards compatibile with all previously deployed NFTs.

Each token bound account is owned by a single NFT, allowing the token to interact with the blockchain, record transaction history, and own on-chain assets. Control of each token bound account is delegated to the owner of the NFT, allowing the owner to initiate on-chain actions on behalf of their token.
...
- `createAccount` - deploys a token bound account for an NFT given an `implementation` address
- `account` - a read-only function that computes the token bound account address for an NFT given an `implementation` address
...
The registry SHALL deploy all token bound account contracts using the `create2` opcode so that the account address for every NFT is deterministic. The account address for each NFT SHALL be derived from the unique combination of implementation address, token contract address, token ID, [EIP-155](./eip-155.md) chain ID, and an optional salt.
...
interface IERC6551Account {
     ...
     *
     * All token bound accounts MUST implement a `receive` function.
     *
    ...
    /**
     * @dev Returns a value that is updated on each transaction
     *
    ...
    /**
     * @dev Returns whether a given signer is authorized to act on behalf of the token bound account
     *
...
Token bound accounts MUST implement an execution interface which allows valid signers to execute arbitrary contract calls on behalf of the account. Support for a given execution interface be exposed by the token bound account using EIP-165 interface detection.

Token bound accounts MAY choose to support the following execution interface:
...
Token bound accounts MAY implement [ERC-5313](./eip-5313.md) light contract ownership. If so, the return value of `owner()` SHOULD be the holder of the non-fungible token which owns the account.
...
The specification proposed above allows NFTs to have multiple token bound accounts, one per implementation address. During the development of this proposal, alternative architectures were considered which would have assigned a single token bound account to each NFT, making each token bound account address an unambiguous identifier.
...
This proposal uses EIP-155 the chain ID to identify an NFT along with its contract address and token ID. Token identifiers are globally unique on a single Ethereum chain, but may not be unique across multiple Ethereum chains. Using chain ID to uniquely identify an NFT allows smart contract authors wishing to implement this proposal to optionally support multi-chain token bound accounts.
