...
This proposal grants every NFT the full capabilities of an Ethereum account, allowing them to own assets and take actions. It does so by deploying unique, deterministically-addressed smart contract accounts for each NFT via a registry. Importantly, this proposal does not require any smart contract changes to the NFT smart contracts themselves. As a result, this proposal is backwards compatibile with all previously deployed NFTs.
...
Token bound accounts MUST implement an execution interface which allows valid signers to execute arbitrary contract calls on behalf of the account. Support for a given execution interface MUST be exposed by the token bound account using EIP-165 interface detection.
