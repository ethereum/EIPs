...
All token bound accounts MUST implement an execution interface which allows valid signers to execute arbitrary contract calls on behalf of the account. Support for a given execution interface MUST be exposed by the token bound account using ERC-165 interface detection.
