...
```typescript
interface BaseNativeCurrency {
  name: string;
  symbol: string;
}

interface NativeCurrencyWithDecimals extends BaseNativeCurrency {
  decimals: number;
}

interface NativeCurrencyWithScalingFactor extends BaseNativeCurrency {
  scalingFactor: string;
}

type NativeCurrency =
  | NativeCurrencyWithDecimals
  | NativeCurrencyWithScalingFactor;

interface AddEthereumChainParameter<T extends NativeCurrency> {
  chainId: string;
...
- `nativeCurrency`
  - If provided, **MUST** describe the native currency of the chain using the `name` and `symbol` fields, and `decimals` or `scalingFactor` fields.
    - Either `decimals` or `scalingFactor` **MUST** be provided.
      - If provided, `decimals` **MUST** be a non-negative integer.
      - If provided, `scalingFactor` **MUST** be the `0x`-prefixed, hexadecimal string representation of a non-negative integer.
  ...
  "params": [
    {
      ...
      "nativeCurrency": {
        "name": "xDAI",
        "symbol": "xDAI",
        "scalingFactor": "0xde0b6b3a7640000"
      }
    }
  ]
}
...
In the xDAI Chain example, notice that:

- The `iconUrls` array contains URLs pointing to two different image formats.
- The `nativeCurrency` object uses the `scalingFactor` field instead of `decimals`, which in this case is 10^18 in hexadecimal notation.
