...
contract('RoyaltyBearingToken', (accounts) => {
    ...
    describe('Transfer NFT token using ERC20', async () => {
        ...
        it('buyer execute the ERC20 payment', async () => {
            await token.executePayment(accOwner4, accSeller, [tokenId_1, tokenId_2], costOfNFT, 'ST2', 1, { from: accBuyer });
            //assert.equal((await token.checkPayment(tokenId, 'ST2', { from: accBuyer })).toString(), costOfNFT, 'Payment after transfer must changed');
        });
