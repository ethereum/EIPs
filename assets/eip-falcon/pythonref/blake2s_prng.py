...
# We implement the Blake2s PRNG following:
# https://github.com/zhenfeizhang/falcon-go/blob/main/c/keccak_prng.c
# but with Blake2s as the internal hash function (implemented in hashlib)

# Constants
MAX_BUFFER_SIZE = 1024  # Adjust based on needs
BLAKE2S_OUTPUT = 32  # Keccak output size in bytes
...
class Blake2sPRNG:
    def __init__(self):
        ...
        self.state = bytearray(BLAKE2S_OUTPUT)
        self.buffer_len = 0
        self.counter = 0
        self.finalized = False

        # Output buffer management
        self.out_buffer = bytearray(BLAKE2S_OUTPUT)
    ...
    def flip(self):
        ...
        blake2s_ctx = blake2s(digest_size=BLAKE2S_OUTPUT)
        blake2s_ctx.update(self.buffer[:self.buffer_len])

        # Generate initial state
        self.state = blake2s_ctx.digest()
        self.finalized = True
    ...
    def extract(self, length: int) -> bytes:
        ...
        while offset < length:
            ...
            blake2s_ctx = blake2s(digest_size=BLAKE2S_OUTPUT)
            blake2s_ctx.update(block)
            self.out_buffer = blake2s_ctx.digest()

            # Update buffer state
            self.out_buffer_len = BLAKE2S_OUTPUT
            self.out_buffer_pos = 0

            # Copy output
            remaining = length - offset
            to_copy = min(remaining, BLAKE2S_OUTPUT)
