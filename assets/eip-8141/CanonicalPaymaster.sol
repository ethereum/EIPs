...
///      over TXPARAM(0x0B) (the frame-elide sig hash). TXPARAM(0x0B) binds both
...
contract CanonicalPaymaster {
    ...
    function _handleGuarantorMode(uint256 currentFrame) internal {
        ...
        //    frame data is not elided from TXPARAM(0x0B), so these checks
        ...
        // 3. Authenticate the signer over TXPARAM(0x0B), the frame-elide sig
    ...
    function _txFrameSigHash() internal returns (bytes32 sigHash) {
        assembly {
            // TXPARAM(0x0B) -> frame-elide signature hash (elides only current frame's data)
            sigHash := verbatim_0i_1o(hex"600bb0")
        }
    }
    ...
    function _numFrames() internal returns (uint256 n) {
        assembly {
            // TXPARAM(0x09) -> len(frames)
            n := verbatim_0i_1o(hex"6009b0")
        }
    }

    function _currentFrameIndex() internal returns (uint256 idx) {
        assembly {
            // TXPARAM(0x0A) -> currently executing frame index
            idx := verbatim_0i_1o(hex"600ab0")
        }
    }
