 ...
 *  gas limits allow it.
 */
contract NestableToken is Context, IERC165, IERC721, IERCXYZ {
    ...
    function _nestTransfer(
        ...
        if (!IERC165(to).supportsInterface(type(IERCXYZ).interfaceId))
    ...
    function _sendToNFT(
        ...
        IERCXYZ destContract = IERCXYZ(to);
    ...
    function _checkForInheritanceLoop(
        ...
        for (uint256 i; i < _MAX_LEVELS_TO_CHECK_FOR_INHERITANCE_LOOP; ) {
            (
                ...
                bool isNft
            ) = IERCXYZ(targetContract).directOwnerOf(targetId);
    ...
    function ownerOf(
        uint256 tokenId
    ) public view virtual override(IERCXYZ, IERC721) returns (address) {
        (address owner, uint256 ownerTokenId, bool isNft) = directOwnerOf(
            tokenId
        );
        if (isNft) {
            owner = IERCXYZ(owner).ownerOf(ownerTokenId);
        }
        return owner;
    }
    ...
    function _burn(
        ...
        for (uint256 i; i < length; ) {
            ...
            totalChildBurns +=
                IERCXYZ(children[i].contractAddress).burn(
                    children[i].tokenId,
    ...
    function _transferChild(
        ...
        if (to != address(0)) {
            if (destinationId == 0) {
                ...
                IERCXYZ(child.contractAddress).nestTransferFrom(
                    address(this),
                    to,
                    child.tokenId,
                    destinationId,
