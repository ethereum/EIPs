...
def upgrade_rlp_transaction_to_ssz(tx_bytes: bytes):
    ...
    if type_ == 0x04:  # EIP-7702
        ...
        def upgrade_authorization(auth: SetCodeRlpAuthorization):
            ...
            return RlpSetCodeAuthorization(
                selector=0x01,
                data=RlpReplayableBasicAuthorization(
    ...
    if type_ == 0x01:  # EIP-2930
        ...
        if len(tx.to) == 0:
            return Transaction(
                selector=0x06,
                data=RlpAccessListCreateTransaction(
        ...
        return Transaction(
            selector=0x05,
            data=RlpAccessListBasicTransaction(
    ...
    if 0xc0 <= type_ <= 0xfe:  # Legacy
        ...
        if tx.v not in (27, 28):
            if len(tx.to) == 0:
                return Transaction(
                    selector=0x04,
                    data=RlpLegacyCreateTransaction(
            ...
            return Transaction(
                selector=0x03,
                data=RlpLegacyBasicTransaction(
        ...
        if len(tx.to) == 0:
            return Transaction(
                selector=0x02,
                data=RlpLegacyReplayableCreateTransaction(
        ...
        return Transaction(
            selector=0x01,
            data=RlpLegacyReplayableBasicTransaction(
