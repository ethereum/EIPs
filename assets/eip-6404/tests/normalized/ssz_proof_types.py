...
AMOUNT_PROOF_INDICES = get_helper_indices([
...
class AmountProof(Container):
    ...
    tx_value: uint256
    multi_branch: Vector[Root, len(AMOUNT_PROOF_INDICES)]
    tx_hash: Hash32
    tx_index: uint32
    tx_branch: Vector[Root, 1 + floorlog2(MAX_TRANSACTIONS_PER_PAYLOAD)]
...
SENDER_PROOF_INDICES = get_helper_indices([
    get_generalized_index(TransactionPayload, 'tx_from'),
    get_generalized_index(TransactionPayload, 'nonce'),
    get_generalized_index(TransactionPayload, 'tx_to'),
    get_generalized_index(TransactionPayload, 'tx_value'),
...
class SenderProof(Container):
    ...
    tx_to: DestinationAddress
    tx_value: uint256
    multi_branch: Vector[Root, len(SENDER_PROOF_INDICES)]
    tx_hash: Hash32
    tx_index: uint32
    tx_branch: Vector[Root, 1 + floorlog2(MAX_TRANSACTIONS_PER_PAYLOAD)]
...
INFO_PROOF_INDICES = get_helper_indices([
    get_generalized_index(TransactionPayload, 'tx_from'),
    get_generalized_index(TransactionPayload, 'nonce'),
    get_generalized_index(TransactionPayload, 'tx_to'),
    get_generalized_index(TransactionPayload, 'tx_value'),
...
class InfoProof(Container):
    ...
    multi_branch: Vector[Root, len(INFO_PROOF_INDICES)]
    tx_hash: Hash32
    tx_index: uint32
    tx_branch: Vector[Root, 1 + floorlog2(MAX_TRANSACTIONS_PER_PAYLOAD)]
