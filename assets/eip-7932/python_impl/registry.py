...
class Secp256k1(AlgorithmEntry):
    ...
    def secp256k1_unpack(
        cls, signature: ByteVector[SECP256K1_SIGNATURE_SIZE]
    ...
    @classmethod
    def secp256k1_validate(cls, signature: ByteVector[SECP256K1_SIGNATURE_SIZE]):
        SECP256K1N = 0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFEBAAEDCE6AF48A03BBFD25E8CD0364141
        r, s, y_parity = cls.secp256k1_unpack(signature)
        assert 0 < r < SECP256K1N
        assert 0 < s <= SECP256K1N // 2
        assert y_parity in (0, 1)

    @classmethod
    def verify(cls, signature_info: bytes, payload_hash: Hash32) -> bytes:
        assert len(signature_info) == (cls.SECP256K1_SIGNATURE_SIZE + 1)
        cls.secp256k1_validate(signature_info[1:])
...
algorithm_registry[uint8(0xFF)] = Secp256k1
