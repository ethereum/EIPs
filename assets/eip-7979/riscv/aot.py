"""Ahead-of-time translation of validated EVM code to RISC-V.

This is the client-side transformation that validation makes legal.
The input is bytecode whose static control flow has been proven by
a validator (see validator.py in ../yul-compiler); the translator
...
one-time deploy costs (measured in EIP-8337's native-cost/ assets); this file's output is
...
Subset: what the yul-compiler emits.  A JUMP into a CALLDEST
