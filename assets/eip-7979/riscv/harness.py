...
The metric is executed RISC-V instructions, counted exactly by qemu
...
the counts — they are one-time and measured in EIP-8337's native-cost/ assets.
...
sys.path.insert(0, os.path.join(HERE, "..", "yul-compiler"))
...
def count_instructions(path):
    """Exact executed-instruction count: -d nochain,exec logs every executed
...
def cells(name, src, calldata, zisk=False):
    ...
    assert validator.validate(new256), f"{name}: 7979 must validate"
    assert not validator.validate(leg256), f"{name}: legacy must not"
    t = name.replace(" ", "_")
    z = zisk
    return {
