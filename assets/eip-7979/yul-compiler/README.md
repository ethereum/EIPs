# A Yul-subset compiler for EIP-7979
...
four things: the 7979 output validates (`validator.py`); the
...
* `validator.py`, `opcodes.py` — the validation EIP's reference
  validator and its opcode table.  Exact copies: these assets must
  stand alone, so the files are repeated here verbatim and must stay
