...
<template>
    <div v-for="status in $frontmatter.statuses">
        <h2 v-if="$frontmatter.eips.filter(eip => eip.status == status && Object.keys($frontmatter.filter).every(key => $frontmatter.filter[key].includes({ ...eip }[key]))).length">{{ status }}</h2>
        <table style="width: 100%; display: table;" v-if="$frontmatter.eips.filter(eip => eip.status == status && Object.keys($frontmatter.filter).every(key => $frontmatter.filter[key].includes({ ...eip }[key]))).length">
            <thead>
                <tr>
                    <th>EIP</th>
                    <th>Title</th>
