...
discussions_to = "https://ethereum-magicians.org/t/eip-2711-separate-gas-payer-from-msg-sender/4353"
status = "Draft"
...
## Specification
### Definitions
**`TransactionType`** 2.  See [EIP-2718](@/EIPS/eip-2718.md)
...
As of `FORK_BLOCK_NUMBER` an [EIP-2718](@/EIPS/eip-2718.md) transaction with a `TransactionType` of `2` will have its `Payload` interpreted as an RLP encoded tuple of:
```
...
As of `FORK_BLOCK_NUMBER` an [EIP-2718](@/EIPS/eip-2718.md) transaction receipt with a `TransactionType` of `2` will have its `Payload` interpreted as a `rlp([status, cumulativeGasUsed, logsBloom, logs][])` where each item of the array corresponds to the child-transaction at matching offset in the transaction type 2 `Payload`.
