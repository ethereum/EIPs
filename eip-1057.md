...
The name of the algorithm comes from the fact that the work loop is programmatically generated based on the blockchain after each fixed number of blocks. While most of these changes are parameter tweaks to better match the memory architecture on contemporary GPUs, the key difference is the randomly evolving math sequence for the main loop. The main loop ensures the math operations not only run efficiently on commodity GPUs but also cover most GPU functionality to minimize any potential efficiency gain from a more specialize chip design.  Most importantly, this block-based evolution prevents any pre-defined, fixed-pipeline implementation that a specialize ASIC might use as another optimization. 

In contrast to Ethash, the changes detailed below make ProgPoW dependent on the core architecture in addition to memory bandwidth and size.
