...
<div v-for="status in $frontmatter.statuses">
    <h2 v-if="$frontmatter.eips.filter(eip => eip.status == status && eip.category == 'Interface').length">{{ status }}</h2>
    <table style="width: 100%; display: table;" v-if="$frontmatter.eips.filter(eip => eip.status == status && eip.category == 'Interface').length">
        <thead>
            <tr>
                <th>EIP</th>
                <th>Title</th>
