...
There is an alternative to this debt register fix, which would require a redeployment of the entire L2 contract suite and then ensuring the fix mint event is sufficiently large as to not trigger this precision issue with the debt register. if we took this path it would require all SNX stakers to migrate again, given the impact to the system and the cost of migration this appears to be a much worse alternative.
